...
bool NetherBridgeFeature::isFeatureChunk(
    int x, int z, bool bIsSuperflat) {  // 4J Stu - New implementation to force
                                        // a nether fortress
    if (!isSpotSelected) {
        // Set the random
        ...
        // [(-3,-3),(3,3)] (7x7). This is 49 possible chunks that should
        // give the fortress enough room to grow within our limited
        // nether
    ...
    NetherBridgeFeature::NetherBridgeStart::NetherBridgeStart(
        ...
        NetherBridgePieces::StartPiece* start =
            new NetherBridgePieces::StartPiece(random, (chunkX << 4) + 2,
                                               (chunkZ << 4) + 2, level);
