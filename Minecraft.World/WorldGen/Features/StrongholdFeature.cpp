...
StrongholdFeature::StrongholdFeature(
    ...
    for (AUTO_VAR(it, options.begin()); it != options.end(); ++it) {
        ...
        } else if (it->first.compare(OPTION_COUNT) // 4J-JEV: Removed, we only have the one stronghold.// strongholdPos = new ChunkPos[ Mth::getInt(it->second,// strongholdPos_length, 1) ];
            assert(false);
    }
...
bool StrongholdFeature::isFeatureChunk(int x, int z, bool bIsSuperflat) {
    if (!isSpotSelected) {
        ...
        int c  // 4J Stu - Changed so that we keep trying more until we have
               // found// somewhere in the world to place a
               // stronghold
            ...
            for (int i = 0; i < strongholdPos_length; i++) {
                ...
                if (level->dimension->getXZSize() < (2.25f * 32.0f)) {
                    ...
                    dist = (1.25 * circle + random.nextDouble()) *
                           (distance * circle) #else
                }
                // 4J Stu - Design change: Original spawns at *32 chunks
                // rather// than *10 chunks from (0,0) but that is outside our
                // world// double dist = (1.25 + random->nextDouble()) * 32.0;//
                // The max of the first part is 2.25, and we have 27 chunks in//
                // each direction Therefore 27/2.25 = 12, which should be the//
                // max of the second part The constant part and random part
                // can// be tuned to move the strongholds further from the spawn
                // 4J// Stu - The original (pre-TU9) calculation for selecting
                // a// start point could put the stronghold very close to the
                // edge// of the world, causing some parts to fail to generate.
                // If the// save is a newer save then we bring that generation
                // in
                if (level->getOriginalSaveVersion() >=
                    SAVE_FILE_VERSION_MOVED_STRONGHOLD) {
                    // Post TU9
                    // The stronghold cannot extend more than 7 chunks in
                    // any direction from the start position
                    // Therefore as long as the the start x/z
                    // are less than 20 it will be fully
                    // contained
                    dist =
                        (1.25 + random.nextDouble()) * (3 + random.nextInt(4));
                } else {
                ...
                TilePos* position = level->getBiomeSource()->findBiome(
                    (selectedX << 4) + 8, (selectedZ << 4) + 8, 7 << 4,
                    allowedBiomes, &random);
                if (position != NULL) {
                    ...
                    selectedZ
                        ...
                        app.DebugPrintf(
                            "Placed stronghold in valid biome at (%d, %d), "
                            "(%d, "
                            "%d)\n",
                    ...
                    // 4J Stu - We want to make sure that we have at least
                    // one// stronghold in this
                    // world
            ++findAtt// 4J Stu - Randomise the angles for retries as well#ifdef _LARGE_WORLDS
            angle = random.nextDouble() * PI * 2.0 * circl#endifouble)spread;
                }
                while (!hasFoundValidPos &&
                       findAttempts < MAX_STRONGHOLD_ATTEMPTS);

        if (!hasFoundVali// Even if it's not a valid position we are still creating the last// one we tried, so store it in the save so Eye of Ender works Fix// for #81933 - GAMEPLAY: The Eye of Ender occasionally does not// appear when used to try and locate the End Portal.
    ...
    StrongholdStart* start = new Strongho// 4J - front() was get(0)z);

    
    while (start->getPieces()->empty() ||
           ((StrongholdPieces::StartPiece*)start->getPieces()->front())
                   ->portalRoomPiece == NUL// regenerate stronghold without changing seed
        start = new StrongholdStart(level, random, x,// System.out.println("Creating stronghold at (" + x + ", " + z + ")");// return new StrongholdStart(level, random, x, z);
    }

    StrongholdFeature::Strong  // for reflectionholdStart() {
}
