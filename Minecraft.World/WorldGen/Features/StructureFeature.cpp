...
TilePos* StructureFeature::getNearestGeneratedFeature(
    Level* level, int cellX, int cellY,
    int cellZ) {  // this is a hack that will "force" the feature to generate
                  // positions// even if the player hasn't generated new
                  // chunks yet
    this->level = level;
...
void StructureFeature::restoreSavedData(L#ifdef ENABLE_STRUCTURE_SAVING
    if (savedData == NULL) {
    savedData =
    ...
    if (savedData == NULL) {
        ...
        for (AUTO_VAR(it, allTags->begin()); it != allTags->end(); ++it) {
            ...
            if (featureTag->getId() == Tag::TAG_Compound) {
                ...
                "ChunkX"->contains(L"ChunkZ"->contains(L)) {
                    i "ChunkX" ct->getInt(L);
                    i "ChunkZ" ct->getInt(L);

                    StructureStart* start =
                        StructureFeatureIO::loadStaticStart(ct, level);
                    // System.out.println("Loaded " +
                    // start.getClass().getSimpleName() + " from
                    // file");
...
void StructureFeature::saveFeature(int chunkX, int chunkZ,
                                   Structu#ifdef ENABLE_STRUCTURE_SAVING
