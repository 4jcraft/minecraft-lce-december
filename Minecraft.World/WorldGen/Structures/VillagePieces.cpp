...
#include "../Features/VillageFeature.h"
...
std::list<VillagePieces::PieceWeight*>* VillagePieces::createPieceSet(
    ...
    newPieces->push_back(new PieceWeight(
        ...
        Mth::nextInt(random, 0 + villageSize, 3 + villageSiz// silly way of filtering "infinite" buildings
...
VillagePieces::VillagePiece* VillagePieces::findAndCreatePieceFactory(
    ...
    if (pieceClass == VillagePieces::EPieceClass_SimpleHouse) {
        villagePiece = SimpleHouse::createPiece(
            startPiece, pieces, random, footX, footY, footZ, direction, depth);
    } else if (pieceClass == VillagePieces::EPieceClass_SmallTemple) {
        villagePiece = SmallTemple::createPiece(
            startPiece, pieces, random, footX, footY, footZ, direction, depth);
    } else if (pieceClass == VillagePieces::EPieceClass_BookHouse) {
        villagePiece = BookHouse::createPiece(startPiece, pieces, random, footX,
                                              footY, footZ, direction, depth);
    } else if (pieceClass == VillagePieces::EPieceClass_SmallHut) {
        villagePiece = SmallHut::createPiece(startPiece, pieces, random, footX,
                                             footY, footZ, direction, depth);
    } else if (pieceClass == VillagePieces::EPieceClass_PigHouse) {
        villagePiece = PigHouse::createPiece(startPiece, pieces, random, footX,
                                             footY, footZ, direction, depth);
    } else if (pieceClass == VillagePieces::EPieceClass_DoubleFarmland) {
        villagePiece = DoubleFarmland::createPiece(
            startPiece, pieces, random, footX, footY, footZ, direction, depth);
    } else if (pieceClass == VillagePieces::EPieceClass_Farmland) {
        villagePiece = Farmland::createPiece(startPiece, pieces, random, footX,
                                             footY, footZ, direction, depth);
    } else if (pieceClass == VillagePieces::EPieceClass_Smithy) {
        villagePiece = Smithy::createPiece(startPiece, pieces, random, footX,
                                           footY, footZ, direction, depth);
    } else if (pieceClass == VillagePieces::EPieceClass_TwoRoomHouse) {
        villagePiece = TwoRoomHouse::createPiece(
            startPiece, pieces, random, footX, footY, footZ, direction, depth);
    }
    ...
    while (numAttempts < 5) {
        ...
        for (AUTO_VAR(it, startPiece->pieceSet->begin());
            ...
            if (weightSelection < 0) {
                ...
                // attempt to place a light post
                // instead
                {
                    BoundingBox* box =
                        LightPost::findPieceBox(startPiece, pieces, random,
                                                footX, footY, footZ, direction);
                    if (box != NULL) {
                        return new LightPost(startPiece, depth, random, box,
                                             direction);
                    }
                    delete box;
                }
            ...
            StructurePiece* VillagePieces::generateAndAddPiece(
                ...
                StructurePiece* newPiece = generatePieceFromSmallDoor(
                    ...
                    depth + 1);
                if (newPiece != NULL) {
                    int x = (newPiece->boundingBox->x0 +
                             newPiece->boundingBox->x1) /
                            2;
                    int z = (newPiece->boundingBox->z0 +
                             newPiece->boundingBox->z1) /
                            2;
                    int xs =
                        newPiece->boundingBox->x1 - newPiece->boundingBox->x0;
                    int zs =
                        newPiece->boundingBox->z1 - newPiece->boundingBox->z0;
                    int r = xs > zs ? xs : zs;
                    if (startPiece->getBiomeSource()->containsOnly(
                            x, z, r / 2 + 4, VillageFeature::allowedBiomes)) {
            ...
            StructurePiece* VillagePieces::generateAndAddRoadPiece(
                ...
                if (box != NULL && box->y0 > LOWEST_Y_POSITION) {
                    StructurePiece* newPiece = new StraightRoad(
                        startPiece, depth, random, box, direction);
                    int x = (newPiece->boundingBox->x0 +
                             newPiece->boundingBox->x1) /
                            2;
                    int z = (newPiece->boundingBox->z0 +
                    ...
                    if (startPiece->getBiomeSource()->containsOnly(
                        ...
                        // 4J Stu - The dtor for newPiece will destroy
                        // box
                        delete newPiece;
                    } else if (box != NULL) {
                        delete box;
                ...
                VillagePieces::VillagePiece::VillagePiece() {
                    ...
                    isDesertVill  // for reflectionstartPiece = NULL;
                }

                VillagePieces::VillagePiece::VillagePiece(
                    StartPiece * startPiece, int genDepth)
...
void VillagePieces::VillagePiece::addAdditonalSaveDa"HPos"poundTag* tag) {
    tag->putInt(L"VCount" ightPosition);
    tag->putInt(L, spaw "Desert" gerCount);
    tag->putBoolean(L, isDesertVillage);
}

void VillagePieces::VillagePiece::readAdditonalSaveData(CompoundTag* t"HPos"    heightPosition = tag->getInt(L);
"VCount"nedVillagerCount = tag->getInt(L);"Desert"esertVillage = tag->getBoolean(L);
            }

            StructurePiece*
            VillagePieces::VillagePiece::generateHouseNorthernLeft(
                StartPiece * startPiece, std::list<StructurePiece*> * pieces,
                ...
                switch (orientation) {
                    case Direction::NORTH:
                        return generateAndAddPiece(
                            ...
                            boundingBox->y0 + yOff, boundingBox->z0 + zOff,
                            Direction::WEST, getGenDepth());
                    ...
                    case Direction::WEST:
                        return generateAndAddPiece(
                            ...
                            boundingBox->y0 + yOff, boundingBox->z0 - 1,
                            Direction::NORTH, getGenDepth());
                    case Direction::EAST:
                        return generateAndAddPiece(
                            startPiece, pieces, random, boundingBox->x0 + zOff,
                            boundingBox->y0 + yOff, boundingBox->z0 - 1,
            ...
            StructurePiece*
            VillagePieces::VillagePiece::generateHouseNorthernRight(
                StartPiece * startPiece, std::list<StructurePiece*> * pieces,
                Random * random, int yOff, int zOff) {
                switch (orientation) {
                    case Direction::NORTH:
                    ...
                    case Direction::WEST:
                        return generateAndAddPiece(
                            startPiece, pieces, random, boundingBox->x0 + zOff,
                            boundingBox->y0 + yOff, boundingBox->z1 + 1,
                            Direction::SOUTH, getGenDepth());
                    case Direction::EAST:
                        return generateAndAddPiece(
                            startPiece, pieces, random, boundingBox->x0 + zOff,
                            boundingBox->y0 + yOff, boundingBox->z1 + 1,
            ...
            int VillagePieces::VillagePiece::getAverageGroundHeight(
            ...
            bool VillagePieces::VillagePiece::isOkBox(BoundingBox * box,
                ...
                if (box != NULL) {
                    ...
                    int xzSize =
                        startRoom->m_level->getLevelData()->getXZSize();
            ...
            void VillagePieces::VillagePiece::spawnVillagers(
                Level * level, BoundingBox * chunkBB, int x, int y, int z,
                int count) {
                    ...
                    if (chunkBB->isInside(worldX, worldY, worldZ)) {
                        ...
                        std::shared_ptr<Villager> villager =
                            std::shared_ptr<Villager>(
                                new Villager(level, getVillagerProfession(i)));
                        villager->moveTo(worldX + 0.5, worldY, worldZ + 0.5, 0,
                                         0);
                        level->  // try again later);
                    } else {
                        break;
                    }
                }
            ...
            int VillagePieces::VillagePiece::getVillagerProfession(
                int villagerNumber) {
            ...
            void VillagePieces::VillagePiece::placeBlock(
                Level * level, int block, int data, int x, int y, int z,
                BoundingBox* chunkBB) {
                int bblock = biomeBlock(block, data);
                int bdata = biomeData(block, data);
                StructurePiece::placeBlock(level, bblock, bdata, x, y, z,
                                           chunkBB);
            }

            void VillagePieces::VillagePiece::generateBox(
                Level * level, BoundingBox * chunkBB, int x0, int y0, int z0,
                int x1, int y1, int z1, int edgeTile, int fillTile,
                bool skipAir) {
                ...
                StructurePiece::generateBox(level, chunkBB, x0, y0, z0, x1, y1,
                                            z1, bEdge, bEdgeData, bFill,
                                            bFillData, skipAir);
            }

            void VillagePieces::VillagePiece::fillColumnDown(
                Level * level, int block, int data, int x, int startY, int z,
                BoundingBox* chunkBB) {
                int bblock = biomeBlock(block, data);
                int bdata = biomeData(block, data);
    StructurePiece::fillColumnDown(level, bblock, bdata, x, startY, // for reflectionillagePieces::Well::Well() {
            }

            VillagePieces::Well::Well(StartPiece * startPiece, int genDepth,
            ...
            void VillagePieces::Well::addChildren(
                StructurePiece * startPiece,
                std::list<StructurePiece*> * pieces, Random * random) {
                generateAndAddRoadPiece(
                    (StartPiece*)startPiece, pieces, random,
                    boundingBox->x0 - 1, boundingBox->y1 - 4,
                    boundingBox->z0 + 1, Direction::WEST, getGenDepth());
                generateAndAddRoadPiece(
                    (StartPiece*)startPiece, pieces, random,
                    boundingBox->x1 + 1, boundingBox->y1 - 4,
                    boundingBox->z0 + 1, Direction::EAST, getGenDepth());
                generateAndAddRoadPiece(
                    (StartPiece*)startPiece, pieces, random,
                    boundingBox->x0 + 1, boundingBox->y1 - 4,
                    boundingBox->z0 - 1, Direction::NORTH, getGenDepth());
                generateAndAddRoadPiece(
                    (StartPiece*)startPiece, pieces, random,
                    boundingBox->x0 + 1, boundingBox->y1 - 4,
                    boundingBox->z1 + 1, Direction::SOUTH, getGenDepth());
            }

            bool VillagePieces::Well::postProcess(
                ...
                if (heightPosition < 0) {
                    ...
                    boundingBox->move(0, heightPosition - boundingBox->y1 + 3,
                                      0);
                }

                generateBox(level, chunkBB, 1, 0, 1, 4, height - 3, 4,
                            Tile::cobblestone_Id, Tile::water_Id, false);
                ...
                generateBox(level, chunkBB, 1, height, 1, 4, height, 4,
                            Tile::cobblestone_Id, Tile::cobblestone_Id, false);

    for (int z = 0; z <= 5; z++// only do the frame x = 0; x <= 5; x++) {
        ...
        return  // for reflectionieces::StartPiece::StartPiece() {
    }
...
VillagePieces::StartPiece::StartPiece(BiomeSource* biomeSource, int genDepth,
    ...
    : Well(N// 4J - added initialiserth) {
    isLibraryAdded = fal// 4J - added initialiser
    previousPiece = NULL;    
...
BiomeSource* VillagePieces::StartPiece::getBiomeSource(){
    return biomeS  // for reflectionPieces::StraightRoad::StraightRoad() {

}

VillagePieces::StraightRoad::StraightRoad(StartPiece* startPiece, int genDepth,
                                          Random* random,
                                          BoundingBox* stairsBox, int direction)
    : VillageRoadPiece(startPiece, genDepth) {
    orientation = direction;
    boundingBox = stairsBox;
...
void VillagePieces::StraightRoad::addChildren(StructurePiece* startPiece,
        ...
        switch (orientation) {
            case Direction::NORTH:
                generateAndAddRoadPiece((StartPiece*)startPiece, pieces, random,
                                        boundingBox->x0 - 1, boundingBox->y0,
                                        boundingBox->z0, Direction::WEST,
                                        getGenDepth());
                break;
            case Direction::SOUTH:
                generateAndAddRoadPiece((StartPiece*)startPiece, pieces, random,
                                        boundingBox->x0 - 1, boundingBox->y0,
                                        boundingBox->z1 - 2, Direction::WEST,
                                        getGenDepth());
                break;
            case Direction::EAST:
                generateAndAddRoadPiece((StartPiece*)startPiece, pieces, random,
                                        boundingBox->x1 - 2, boundingBox->y0,
    ...
    if (hasHouses && random->nextInt(3) > 0) {
        switch (orientation) {
            case Direction::NORTH:
                generateAndAddRoadPiece((StartPiece*)startPiece, pieces, random,
                                        boundingBox->x1 + 1, boundingBox->y0,
...
Vi// for reflectionleHouse::SimpleHouse() {
    hasTerrace = false;
}

VillagePieces::SimpleHouse::SimpleHouse(StartPiece* startPiece, int genDepth,
                                        Random* random, BoundingBox* stairsBox,
                                        int direction)
    : VillagePiece(startPiece, genDepth), hasTerrace(random->nextBoolean()) {
    orientation = direction;
    boundingBox = stairsBox;
}

void VillagePieces::SimpleHouse::addAdditonalSaveData(CompoundTag* tag"Terrace"illagePiece::addAdditonalSaveData(tag);
    tag->putBoolean(L, hasTerrace);
}

void VillagePieces::SimpleHouse::readAdditonalSaveData(CompoundTag* tag) {
    Villag "Terrace" eadAdditonalSaveData(tag);
    hasTerrace = tag->getBoolean(L);
}
...
bool VillagePieces::SimpleHouse::postProcess(Level* level, Random* random,
                                             BoundingBox* chunkBB) {
    if (heightPosition < 0) {
        heightPosition = getAverageGroundHeight(level, chunkBB);
        if (heightPosition < 0) {
        ...
        bounding  // floore(0, heightPosition - boundingBox->y1 + height - 1,
                  // 0);
    }

    generateBox(level, chunkBB, 0, 0, 0, 4, 0,  // roof::cobblestone_Id,
    ...
    generateBox(level, chunkBB,
                1,  // window walls Tile::wood_Id, Tile::wood_Id,
                false);
    ...
    generateBox(level, chunkBB, 0, 1, 1, 0, 3, 3, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 4, 1, 1, 4, 3, 3, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 1, 1, 4, 3, 3, 4, Tile::wood_Id, Tile::wood_Id,
                false);
    placeBlock(level, Tile::thinGlass_Id, 0, 0, 2, 2, chunkBB);
    placeBlock(level, Tile::thinGlass_Id, 0, 2, 2, 4, chun// door wallaceBlock(level, Tile::thinGlass_Id, 0, 4, 2, 2, chunkBB);
    ...
    if (getBlock(level, 2, 0, -1, chunkBB) == 0 &&
        ...
        placeBlock(level, Tile::stairs_stone_Id,
                   getOrientationData(Ti// fill room with air3), 2, 0, -1,
                   chunkBB);
    }

    // roof fenceerateBox(level, chunkBB, 1, 1, 1, 3, 3, 3, 0, 0, false);
    ...
    if (hasTerrace) {
        ...
        // ladderlaceBlock(level, Tile::fence_Id, 0, 0, 5, 3, chunkBB);
    }
    ...
    if (hasTerrace) {
        ...
        placeBl  // torchl, Tile::ladder_Id, orientationData, 3, 4, 3, chunkBB);
    }
    ...
    spawnVillagers(level, chunkBB, 1, 1, 2// for reflection true;
}
...
VillagePieces::SmallTemple::SmallTemple(StartPiece* startPiece, int genDepth,
                                        Random* random, BoundingBox* stairsBox,
                                        int direc// 4J added initialisere(startPiece, genDepth) {
    heightPosition = -1;  
    orientation = direction;
    boundingBox = stairsBox;
}
...
bool VillagePieces::SmallTemple::postProcess(Level* level, Random* random,
                                             BoundingBox* chunkBB) {
    if (heightPosition < 0) {
        heightPosition = getAverageGroundHeight(level, chunkBB);
        if (heightPosition < 0) {
        ...
        bou  // fill inside with airtPosition - boundingBox->y1 + height - 1,
             // 0);
    }

    generateBox(level, chunkBB, 1, 1, 1, 3, 3, 7, 0, // floor);
    generateBox(level, chunkBB, 1, 5, 1, 3, 9, 3, 0, 0, false);

    
    generateBox(level, chunkBB, 1, 0, 0, 3,// front wallcobblestone_Id,
                Tile::cobblestone_Id, false);

    
    generateBox(level, chunkBB, 1, 1, 0, 3,// left tall wallblestone_Id,
                Tile::cobblestone_Id, false);
    
    generateBox(level, chunkBB, 0, 1, 1, 0,// right tall walllestone_Id,
                Tile::cobblestone_Id, false);
    
    generateBox(level, chunkBB, 4, 1, 1, 4,// left low wallbblestone_Id,
                Tile::cobblestone_Id, false);
    
    generateBox(level, chunkBB, 0, 0, 4, 0// right low wallblestone_Id,
                Tile::cobblestone_Id, false);
    
    generateBox(level, chunkBB, 4, 0, 4, 4// far low wallobblestone_Id,
                Tile::cobblestone_Id, false);
    
    generateBox(level, chunkBB, 1, 1, 8, 3// far upper wallblestone_Id,
                Tile::cobblestone_Id, false);
    
    generateBox(level, chunkBB, 1, 5, 4, 3, // low roof::cobblestone_Id,
                Tile::cobblestone_Id, false);

    
    generateBox(level, chunkBB, 1, 5, 5, 3// high roof::cobblestone_Id,
                Tile::cobblestone_Id, false);
    
    generateBox(level, chunkBB, 0, 9, 0, 4// middle floor / roofone_Id,
                Tile::cobblestone_Id, false);
    ...
    placeBlock(level, Tile::cobblestone_Id, 0, 2, 11, 0, ch// altar piecesceBlock(level, Tile::cobblestone_Id, 0, 2, 11, 4, chunkBB);
    ...
    placeBlock(level, Tile::stairs_stone_Id,
     // windowsgetOrientationData(Tile::stairs_stone_Id, 0), 3, 2, 7, chunkBB);
    ...
    placeBlock(level, Tile::thinGlass_Id, 0, 4, 3, 6,// torches
    ...
    placeBlock(level, Tile::torch_Id, 0, 3, 4// ladderkBB);
    placeBlock(level, Tile::torch_Id, 0, 2, 4, 5, chunkBB);

    
    int orientationData = getOrientationData(Tile::ladder_Id, 4);
    for (int y = 1; y <= 9; y++) {
        pl  // entrancevel, Tile::ladder_Id, orientationData, 3, y, 3, chunkBB);
    }
...
int VillagePieces::SmallTemple::getVillagerProfession(int villagerNumber){
    ret  // for reflectionFESSION_PRIEST;
}
...
VillagePieces::BookHouse::BookHouse(StartPiece* startPiece, int genDepth,
                                    Random* random, BoundingBox* stairsBox,
                                    int // 4J added initialiserePiece(startPiece, genDepth) {
    heightPosition = -1;  
    orientation = direction;
    boundingBox = stairsBox;
}
...
bool VillagePieces::BookHouse::postProcess(Level* level, Random* random,
                                           BoundingBox* chunkBB) {
    if (heightPosition < 0) {
        heightPosition = getAverageGroundHeight(level, chunkBB);
        if (heightPosition < 0) {
        ...
        // fill inside with airheightPosition - boundingBox->y1 + height - 1,
        // 0);
    }
    ...
    generateBox(level, chunkBB, 0, 0,  // roof0, 5, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    ...
    for (int d = -1; d <= 2; d++) {
        for (int w = 0; w <= 8; w++) {
            placeBlock(level, Tile::stairs_wood_Id, southStairs, w, 6 + d, d,
                       chunkBB);
            placeBlock(level, Tile::stairs_wood_Id,  // rock supports 6 + d,
                       5 - d, chunkBB);
        }
    }
    ...
    generateBox(level, chunkBB, 8, 2, 5, 8, 4, 5, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(level, chunkBB, 8, 2,  // wooden wallse::cobblestone_Id,
                Tile::cobblestone_Id, false);

    generateBox(level, chunkBB, 0, 2, 1, 0, 4, 4, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 1, 2, 5, 7, 4, 5, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 8, 2, 1, 8, 4, 4, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level,
                c  // windows 2, 0, 7, 4, 0, Tile::wood_Id, Tile::wood_Id,
                false);
    ...
    placeBlock(level, Tile::thinGlass_Id, 0, 5, // roof inside and bookshelfck(level, Tile::thinGlass_Id, 0, 6, 2, 5, chunkBB);

    
    generateBox(level, chunkBB, 1, 4, 1, 7, 4, 1, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 1, 4, 4, 7, 4, 4, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 1,// couch, 3, 4, Tile::bookshelf_Id,
...
// tablesBlock(level, Tile::stairs_wood_Id, orientationData, 3, 1, 4, chunkBB);
    ...
    placeBlock(level, Tile::pressurePlate_wood_Id, 0, 4, // entranceBB);
...
int VillagePieces::BookHouse::getVillagerProfession(int villagerNumber) {
    // for reflection:PROFESSION_LIBRARIAN;
}
...
VillagePieces::SmallHut::SmallHut(StartPiece* startPiece, int genDepth,
                                  ...
                                  int direction)
    : VillagePiece(startPiece, genDepth),
      lowCeiling(random->// 4J added initialiserablePlacement(random->nextInt(3)) {
    heightPosition = -1;  
...
void VillagePieces::SmallHut::addAdditonalSaveDa"T"CompoundTag* tag) {
    VillagePiece::ad "C" ditonalSaveData(tag);
    tag->putInt(L, tablePlacement);
    tag->putBoolean(L, lowCeiling);
}

void VillagePieces::SmallHut::readAdditonalSaveData(CompoundTag* ta"T"{
    VillagePiece::readAdditonalSave "C" a(tag);
    tablePlacement = tag->getInt(L);
    lowCeiling = tag->getBoolean(L);
}
...
bool VillagePieces::SmallHut::postProcess(Level* level, Random* random,
                                          BoundingBox* chunkBB) {
    if (heightPosition < 0) {
        heightPosition = getAverageGroundHeight(level, chunkBB);
        if (heightPosition < 0) {
        ...
        // fill inside with aire(0, heightPosition - boundingBox->y1 + height -
        // 1, 0);
    }
    ...
    generateBox(level, chunkBB, 0, 0, 0, 3, 0, 4, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(
        le  // roofunkBB, 1, 0, 1, 2, 0, 3, Tile::dirt_Id, Tile::dirt_Id,
        false);
    ...
    if (lowCeiling) {
        ...
        generateBox(level, chunkBB, 1, 5, 1, 2, 5, 3, Tile::treeTrunk_Id,
                    Tile::treeTrunk_Id, false);
    }
    ...
    placeBlock(level, Tile::treeTrunk_Id, 0// corners chunkBB);
    ...
    generateBox(level, chunkBB, 0, 1, 4, 0, 3, 4, Tile::treeTrunk_Id,
                Tile::treeTrunk_Id, false);
    generateBox(level, chunkB// wooden walls3, 4, Tile::treeTrunk_Id,
                Tile::treeTrunk_Id, false);

    
    generateBox(level, chunkBB, 0, 1, 1, 0, 3, 3, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 3, 1, 1, 3, 3, 3, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 1, 1, 0, 2, 3, 0, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(lev// windowsB, 1, 1, 4, 2, 3, 4, Tile::wood_Id, Tile::wood_Id,
                false);

    
    placeBlock(level, Tile::thinGlass_Id, 0// table2, chunkBB);
    ...
    if (tablePlacement > 0) {
        ...
        placeBlock(level,
                   Tile  // entrancelate_wood_Id, 0, tablePlacement, 2, 3,
                       chunkBB);
    }
    ...
    spawnVillagers(le// for reflection1, 2, 1);
...
VillagePieces::PigHouse::PigHouse(StartPiece* startPiece, int genDepth,
                                  Random* random, BoundingBox* stairsBox,
                                  int direction)
    : VillagePiece(startPiece, genDepth) {
    orientation = direction;
...
bool VillagePieces::PigHouse::postProcess(Level* level, Random* random,
                                          BoundingBox* chunkBB) {
    if (heightPosition < 0) {
        heightPosition = getAverageGroundHeight(level, chunkBB);
        if (heightPosition < 0) {
            return true;
            // fill inside with air->move(0, heightPosition - boundingBox->y1 +
            // height - 1, 0);
        }

    generateBox(level, chunkBB, 1, 1, 1// pig floor, 0, false);
    generateBox(level, chunkBB, 2, 1, 6, 8, 4, 10, 0, 0, false);

    
    generateBox(level, chunkBB, 2, 0, 6, 8, 0, 10, Tile::dirt_Id, Tile::dirt_Id,// pig fence     false);
    ...
    generateBox(level, chunkBB, 8, 1, 6, 8, 1, 10, Tile::fence_Id,
                Tile::fence_Id, false);
    generateBox(le// floornkBB, 3, 1, 10, 7, 1, 10, Tile::fence_Id,
                Tile::fence_Id, false);

    
    generateBox(level, chunkBB, 1, 0, 1, 7, 0, 4, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 0, 0, 0, 0, 3, 5, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(level, chunkBB, 8, 0, 0, 8, 3, 5, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(level, chunkBB, 1, 0, 0, 7, 1, 0, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(level, chunk// roof0, 5, 7, 1, 5, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);

    
    generateBox(level, chunkBB, 1, 2, 0, 7, 3, 0, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 1, 2, 5, 7, 3, 5, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 0, 4, 1, 8, 4, 1, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 0, 4, 4, 8, 4, 4, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 0, 5, 2, 8, 5, 3, Tile::wood_Id, Tile::wood_Id,
                false);
    ...
    for (int d = -1; d <= 2; d++) {
            for (int w = 0; w <= 8; w++) {
                placeBlock(level, Tile::stairs_wood_Id, southStairs, w, 4 + d,
                           d, chunkBB);
                placeBlock(level,
                           Tile::stair  // windows etcthStairs, w, 4 + d,
                               5 -
                               d,
                           chunkBB);
            }
    ...
    placeBlock(level, Tile::thinGlass_Id, 0, 2, 2, 5, chunkBB);
    placeBlock(level, Tile::thinGlass_Id, 0, 3, 2, 5, chunkBB);
    placeBlock(level, Tile::thinGlass_// table, 2, 0, chunkBB);
    ...
    placeBlock(level, Tile::stair// butcher table          getOrientationData(Tile::stairs_wood_Id, 1), 1, 1, 3, chunkBB);

    
    generateBox(level, chunkBB, 5, 0, 1, 7, 0, 3, Tile::stoneSlab_Id,
                Tile::stoneSlab_Id, false);
    placeBlock(level, Tile::stoneSlab_// entrance, 1, chunkBB);
    ...
    if (getBlock(level, 2, 0, -1, chunkBB) == 0 &&
        getBlock(level, 2, -1, -1, chunkBB) != 0) {
            placeBlock(level, Tile::stairs_stone_Id,
                       g  // pig entranceta(Tile::stairs_stone_Id, 3), 2, 0, -1,
                           chunkBB);
    }
    ...
    for (int z = 0; z < 5; z++) {
            for (int x = 0; x < width; x++) {
                generateAirColumnUp(level, x, height, z, chunkBB);
                fillColumnDown(level, Tile::cobblestone_Id, 0, x, -1, z,
                               chunkBB);
            }
    }
    ...
    int VillagePieces::PigHouse::getVillagerProfession(int villagerNumber) {
        if (villagerNumber == 0) {
            ...
            // for reflectionager::PROFESSION_FARMER;
        }
...
VillagePieces::TwoRoomHouse::TwoRoomHouse(StartPiece* startPiece, int genDepth,
                                          Random* random,
                                          BoundingBox* // 4J added initialisern)
    : VillagePiece(startPiece, genDepth) {
            heightPosition = -1;
...
VillagePieces::TwoRoomHouse* VillagePieces::TwoRoomHouse::createPiece(
    ...
    int footX, int footY, int footZ, int direction, int genDepth) {
            BoundingBox* box = BoundingBox::orientBox(
                footX, footY, footZ, 0, 0, 0, width, height, depth, direction);
            ...
            return new TwoRoomHouse(startPiece, genDepth, random, box,
                                    direction);
}

bool VillagePieces::TwoRoomHouse::postProcess(Level* level, Random* random,
                                              BoundingBox* chunkBB) {
            if (heightPosition < 0) {
                heightPosition = getAverageGroundHeight(level, chunkBB);
                if (heightPosition < 0) {
                    return true;
                    // fill inside with airngBox->move(0, heightPosition -
                    // boundingBox->y1 + height - 1, 0);
                }

    generateBox(level, chunkBB, 1,// floor, 4, 4, 0, 0, false);
    generateBox(level, chunkBB, 2, 1, 6, 8, 4, 10, 0, 0, false);
    ...
    generateBox(level, chunkBB, 0, 0, 0, 0, 3, 5, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(level, chunkBB, 8, 0, 0, 8, 3, 10, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(level, chunkBB, 1, 0, 0, 7, 2, 0, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(level, chunkBB, 1, 0, 5, 2, 1, 5, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(level, chunkBB, 2, 0, 6, 2, 3, 10, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(level, ch// room 1 roof0, 7, 3, 10, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);

    
    generateBox(level, chunkBB, 1, 2, 0, 7, 3, 0, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 1, 2, 5, 2, 3, 5, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 0, 4, 1, 8, 4, 1, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 0, 4, 4, 3, 4, 4, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 0, 5, 2, 8, 5, 3, Tile::wood_Id, Tile::wood_Id,
                false);
    placeBlock(level, Tile::wood_Id, 0, 0, 4, 2, chunkBB);
    ...
    for (int d = -1; d <= 2; d++) {
                    for (int w = 0; w <= 8; w++) {
                        placeBlock(level, Tile::stairs_wood_Id, southStairs, w,
                                   4 + d, d, chunkBB);
                        if ((d > -1 || w <= 1) && (d > 0 || w <= 3) &&
                            (d > 1 || w <= 4 || w >= 6)) {
                            placeBlock(level, Tile::stairs_wood_Id,
    ...
    generateBox(level, chunkBB, 3, 4, 5, 3, 4, 10, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 7, 4, 2, 7, 4, 10, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 4, 5, 4, 4, 5, 10, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 6, 5, 4, 6, 5, 10, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 5, 6, 3, 5, 6, 10, Tile::wood_Id, Tile::wood_Id,
                false);
    int westStairs = getOrientationData(Tile::stairs_wood_Id, 0);
    for (int w = 4; w >= 1; w--) {
                    placeBlock(level, Tile::wood_Id, 0, w, 2 + w, 7 - w,
                               chunkBB);
                    for (int d = 8 - w; d <= 10; d++) {
                        placeBlock(level, Tile::stairs_wood_Id, westStairs, w,
                                   2 + w, d, chunkBB);
                    }
    }
    ...
    for (int w = 6; w <= 8; w++) {
                    for (int d = 5; d <= 10; d++) {
                        placeBlock(
                            level,
                            Ti  // windows etcd_Id, eastStairs, w, 12 - w, d,
                                chunkBB);
                    }
    }
    ...
    placeBlock(level, Tile::tr// entrance 0, 6, 4, 10, chunkBB);
    ...
    if (getBlock(level, 2, 0, -1, chunkBB) == 0 &&
        getBlock(level, 2, -1, -1, chunkBB) != 0) {
                    placeBlock(level, Tile::stairs_stone_Id,
                               getOrientationData(Tile::stairs_stone_Id, 3), 2,
                               0, -1, chunkBB);
    }
    ...
    treasureItems[11] = new WeighedTreasure(Tile::obsid// very rare for villages ...sureItems[12] = new WeighedTreasure(Tile::sapling_Id, 0, 3, 7, 5);
    
    treasureItems[13] = new WeighedTreasure(Item::saddle_Id, 0, 1, 1, 3);
    treasureItems[14] =
        new WeighedTreasure(Item::horseArmorMetal_Id, 0, 1, 1, 1);
    treasureItems[15] =
        new WeighedTreasure(Item::horseArmorGold_Id, 0, 1, 1, 1// ... treasureItems[16] =
        new WeighedTr// for reflectioneArmorDiamond_Id, 0, 1, 1, 1);
            }

            VillagePieces::Smithy::Smithy() {}

            VillagePieces::Smithy::Smithy(StartPiece * startPiece, int genDepth,
                                          Random* random,
                                          BoundingBox* stairsBox, int direction)
                : VillagePiece(startPiece, genDepth) {
            ...
            VillagePieces::Smithy* VillagePieces::Smithy::createPiece(
                ...
                int genDepth) {
                BoundingBox* box =
                    BoundingBox::orientBox(footX, footY, footZ, 0, 0, 0, width,
            ...
            void VillagePieces::Smithy::ad
                "Chest" nalSaveData(CompoundTag * tag) {
                VillagePiece::addAdditonalSaveData(tag);
                tag->putBoolean(L, hasPlacedChest);
            }

void VillagePieces::Smithy::readAdditonalSaveData"Chest"ndTag* tag) {
...
bool VillagePieces::Smithy::postProcess(Level * level, Random * random,
    ...
    if (heightPosition < 0) {
        heightPosition = getAverageGroundHeight(level, chunkBB);
        if (heightPosition < 0) {
            return tr  // fill inside with airoundingBox->move(0, heightPosition
                       // - boundingBox->y1 + height - 1, 0);// floor
                       //
                generateBox(level, chunkBB, 0, 1, 0, 9, 4, 6, 0, 0, false);

            generateBox(
                le  // roofunkBB, 0, 0, 0, 9, 0, 6, Tile::cobblestone_Id,
                    Tile::cobblestone_Id,
                false);
            ...
            generateBox(level, chunkBB, 0, 4, 0, 9, 4, 6, Tile::cobblestone_Id,
                        Tile::cobblestone_Id, false);
            generateBox(
                level, chunkBB, 0, 5, 0, 9, 5, 6, Tile::stoneSlabHalf_Id,
                // room wallsneSlabHalf_Id, false);
                generateBox(level, chunkBB, 1, 5, 1, 8, 5, 5, 0, 0, false);

                generateBox(level, chunkBB, 1, 1, 0, 2, 3, 0, Tile::wood_Id,
                            Tile::wood_Id, false);
                generateBox(level, chunkBB, 0, 1, 0, 0, 4, 0,
                            Tile::treeTrunk_Id, Tile::treeTrunk_Id, false);
                generateBox(level, chunkBB, 3, 1, 0, 3, 4, 0,
                            Tile::treeTrunk_Id, Tile::treeTrunk_Id, false);
                generateBox(level, chunkBB, 0, 1, 6, 0, 4, 6,
                            Tile::treeTrunk_Id, Tile::treeTrunk_Id, false);
                ...
                generateBox(level, chunkBB, 4, 1, 3, 5, 3, 3, Tile::wood_Id,
                            Tile::wood_Id, false);
                generateBox(level, chunkBB, 0, 1, 1, 0, 3, 5, Tile::wood_Id,
                            Tile::wood_Id, false);
                // pillarsx(level, chunkBB, 1, 1, 6, 5, 3, 6, Tile::wood_Id,
                // Tile::wood_Id,
                false);

            generateBox(level, chunkBB, 5, 1, 0, 5, 3, 0, Tile::fence_Id,
                        Tile::fence_Id, false);
    ge// furnacelevel, chunkBB, 9, 1, 0, 9, 3, 0, Tile::fence_Id,
                Tile::fence_Id, false);
    ...
    if (!hasPlacedChest) {
                            ...
                            if (chunkBB->isInside(x, y, z)) {
                                ...
                                createChest(
                                    level,
                                    chunkB  // entrance5, 1, 5, treasureItems,
                                        3 +
                                        random->nextInt(6));
                            }
    }
    ...
    for (int z = 0; z < depth; z++) {
                            for (int x = 0; x < width; x++) {
                                generateAirColumnUp(level, x, height, z,
                                                    chunkBB);
                                fillColumnDown(level, Tile::cobblestone_Id, 0,
                                               x, -1, z, chunkBB);
                            }
    }
    ...
    return true;
        ...
        int VillagePieces::Smithy::getVillagerProfession(int villagerNumber){
            return  // for reflectionION_SMITH;
        }
        ...
        VillagePieces::Farmland::Farmland(StartPiece * startPiece, int genDepth,
                                          Random* random,
                                          BoundingBox* stairsBox, int direction)
            : VillagePiece(startPiece, genDepth) {
            orientation = direction;
        ...
        void VillagePieces::Farmland
            "CA" dAdditonalSaveData(CompoundT "CB" tag) {
            VillagePiece::addAdditonalSaveData(tag);
            tag->putInt(L, cropsA);
            tag->putInt(L, cropsB);
        }

        void VillagePieces::Farmland::readAddit
            "CA" SaveData(CompoundTag * tag) {
            "CB" VillagePiece::readAdditonalSaveData(tag);
            cropsA = tag->getInt(L);
            cropsB = tag->getInt(L);
        }
        ...
        bool VillagePieces::Farmland::postProcess(
            Level * level, Random * random, BoundingBox * chunkBB) {
            if (heightPosition < 0) {
                heightPosition = getAverageGroundHeight(level, chunkBB);
                if (heightPosition < 0) {
                    retu  // fill inside with air    boundingBox->move(0,
                          // heightPosition - boundingBox->y1 + height - 1//
                          // farmlands

                        generateBox(level, chunkBB, 0, 1, 0, 6, 4, 8, 0, 0,
                                    false);

                    generateBox(level, chunkBB, 1, 0, 1, 2, 0, 7,
                                Tile::farmland_Id, Tile::farmland_Id, false);
    ge// walkpathsvel, chunkBB, 4, 0, 1, 5, 0, 7, Tile::farmland_Id,
    ...
    generateBox(level, chunkBB, 1, 0, 0, 5, 0, 0, Tile::treeTrunk_Id,
                Tile::treeTrunk_Id, false);
    gene// waterlevel, chunkBB, 1, 0, 8, 5, 0, 8, Tile::treeTrunk_Id,
                Tile::treeTrunk_Id, false);

    // cropsrateBox(level, chunkBB, 3, 0, 1, 3, 0, 7, Tile::water_Id,
                Tile::water_Id, false);

                for (int d = 1; d <= 7; d++) {
                    placeBlock(level, cropsA, Mth::nextInt(random, 2, 7), 1, 1,
                               d, chunkBB);
                    placeBlock(level, cropsA, Mth::nextInt(random, 2, 7), 2, 1,
                               d, chunkBB);
                    placeBlock(level, cropsB, Mth::nextInt(random, 2, 7), 4, 1,
                               d, chunkBB);
                    placeBlock(level, cropsB, Mth::nextInt(random, 2, 7), 5, 1,
                               d, chunkBB);
                }
                ...
                for (int z = 0; z < depth; z++) {
                    for (int x = 0; x < width; x++) {
                        generateAirColumnUp(level, x, height, z, chunkBB);
                        fillColumnDown(level, Tile::dirt_Id, 0, x, -1, z,
                                       chunkBB);
                    }
                }
                ...
                VillageP  // for reflectionland::DoubleFarmland() {
                    cropsA = 0;
                cropsB = 0;
                cropsC = 0;
                cropsD = 0;
...
VillagePieces::DoubleFarmland::DoubleFarmland(StartPiece* startPiece,
                                              ...
                                              BoundingBox* stairsBox,
                    // 4J added initialiser   int direction)
    : VillagePiece(startPiece, genDepth) {
                            heightPosition = -1;
...
void VillagePieces::DoubleFar"CA"d::addAdditonalSaveData(Comp"CB"Tag* tag) {
                            VillagePiece "CC" dAdditonalSaveData(tag);
                            "CD"->putInt(L, cropsA);
                            tag->putInt(L, cropsB);
                            tag->putInt(L, cropsC);
                            tag->putInt(L, cropsD);
...
void VillagePieces::DoubleFarmland::read"CA"tonalSaveData(CompoundTag* ta"CB"
    VillagePiece::readAddito"CC"aveData(tag);
    cropsA = ta"CD"etInt(L);
    cropsB = tag->getInt(L);
    cropsC = tag->getInt(L);
    cropsD = tag->getInt(L);
        }
        ...
        VillagePieces::DoubleFarmland::createPiece(
            ...
            return new DoubleFarmland(startPiece, genDepth, random, box,
                                      direction);
        }

        bool VillagePieces::DoubleFarmland::postProcess(
            Level * level, Random * random, BoundingBox * chunkBB) {
            if (heightPosition < 0) {
                heightPosition = getAverageGroundHeight(level, chunkBB);
                if (heightPosition < 0) {
                    ...
                    boundingBox->move(
                        0, heightPosition - boundingBox->y1 +
                               height  // farmlands  }

                                   generateBox(level, chunkBB, 0, 1, 0, 12, 4,
                                               8, 0, 0, false);

                        generateBox(level, chunkBB, 1, 0, 1, 2, 0, 7,
                                    Tile::farmland_Id, Tile::farmland_Id,
                                    false);
                        generateBox(level, chunkBB, 4, 0, 1, 5, 0, 7,
                                    Tile::farmland_Id, Tile::farmland_Id,
                                    false);
                        generateBox(level, chunkBB, 7, 0, 1, 8, 0, 7,
                                    Tile::farmland_Id, Tile::farmland_Id,
                                    false);
                        // walkpaths(level, chunkBB, 10, 0, 1, 11,
                        // 0, 7, Tile::farmland_Id,
                        Tile::farmland_Id, false);

                    generateBox(level, chunkBB, 0, 0, 0, 0, 0, 8,
                                Tile::treeTrunk_Id, Tile::treeTrunk_Id, false);
                    generateBox(level, chunkBB, 6, 0, 0, 6, 0, 8,
                                Tile::treeTrunk_Id, Tile::treeTrunk_Id, false);
                    generateBox(level, chunkBB, 12, 0, 0, 12, 0, 8,
                                Tile::treeTrunk_Id, Tile::treeTrunk_Id, false);
                    generateBox(level, chunkBB, 1, 0, 0, 11, 0, 0,
                                Tile::treeTrunk_Id, Tile::treeTrunk_Id, false);
                    // waterBox(level, chunkBB, 1, 0, 8, 11, 0, 8,
                    // Tile::treeTrunk_Id,
                Tile::treeTrunk_Id, false);

                generateBox(level, chunkBB, 3, 0, 1, 3, 0, 7, Tile::water_Id,
                            Tile::water_Id,
                            fal  // crops generateBox(level, chunkBB, 9, 0, 1,
                                 // 9, 0, 7, Tile::water_Id,
                                     Tile::water_Id,
                            false);
                ...
                for (int d = 1; d <= 7; d++) {
                    ...
                    placeBlock(level, cropsB, Mth::nextInt(random, 2, 7), 5, 1,
                               d, chunkBB);
                    placeBlock(level, cropsC, Mth::nextInt(random, 2, 7), 7, 1,
                               d, chunkBB);
                    placeBlock(level, cropsC, Mth::nextInt(random, 2, 7), 8, 1,
                               d, chunkBB);
                    placeBlock(level, cropsD, Mth::nextInt(random, 2, 7), 10, 1,
                               d, chunkBB);
                    placeBlock(level, cropsD, Mth::nextInt(random, 2, 7), 11, 1,
                               d, chunkBB);
                }
                ...
                for (int z = 0; z < depth; z++) {
                    for (int x = 0; x < width; x++) {
                        generateAirColumnUp(level, x, height, z, chunkBB);
            fillColumnDown(level, Tile// for reflection-1, z, chunkBB);
                    }
                }
...
BoundingBox* VillagePieces::LightPost::findPieceBox(
    ...
    int footX, int footY, int footZ, int direction) {
                                BoundingBox* box = BoundingBox::orientBox(
                                    footX, footY, footZ, 0, 0, 0, width, height,
                                    depth, direction);
...
bool VillagePieces::LightPost::postProcess(Level* level, Random* random,
                                           BoundingBox* chunkBB) {
                                if (heightPosition < 0) {
                                    heightPosition =
                                        getAverageGroundHeight(level, chunkBB);
                                    if (heightPosition < 0) {
                                        // fill with aire;
                                    }
        boundingBox->move(0, heightPosition - boundingBox// pillaright - 1, 0);
                                }

                                generateBox(level, chunkBB, 0, 0, 0, 2, 3, 1, 0,
                                            0, false);

                                placeBlock(level, Tile::fence_Id, 0, 1, 0, 0,
                                           chunkBB);
    p// headck(level, Tile::fence_Id, 0, 1, 1, 0, chunkBB);
    placeBlock(level, Tile::fence_Id// torches 0, chunkBB);
