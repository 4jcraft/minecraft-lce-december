...
VillagePieces::VillagePiece::VillagePiece(StartPiece* startPiece, int genDepth)
    ...
    for (int z = boundingBox->z0; z <= boundingBox->z1; z++) {
        for (int x = boundingBox->x0; x <= boundingBox->x1; x++) {
            if (chunkBB->isInside(x, 64, z)) {
                total += Math::_max(level->getTopSolidBlock(x, z),
...
void VillagePieces::VillagePiece::spawnVillagers(Level* level,
                                                 BoundingBox* chunkBB, int x,
    ...
    for (int i = spawnedVillagerCount; i < count; i++) {
        ...
        if (chunkBB->isInside(worldX, worldY, worldZ)) {
            ...
            std::shared_ptr<Villager> villager = std::shared_ptr<Villager>(
...
void VillagePieces::VillagePiece::placeBlock(Level* level, int block, int data,
                                             int x, int y, int z,
                                             BoundingBox* chunkBB) {
...
void VillagePieces::VillagePiece::generateBox(Level* level,
                                              BoundingBox* chunkBB, int x0,
    ...
    StructurePiece::generateBox(level, chunkBB, x0, y0, z0, x1, y1, z1, bEdge,
                                bEdgeData, bFill, bFillData, skipAir);
}

void VillagePieces::VillagePiece::fillColumnDown(Level* level, int block,
                                                 int data, int x, int startY,
...
VillagePieces::Well::Well(StartPiece* startPiece, int genDepth, Random* random,
                          int west, int north)
...
void VillagePieces::Well::addChildren(StructurePiece* startPiece,
    ...
    generateAndAddRoadPiece((StartPiece*)startPiece, pieces, random,
                            ...
                            boundingBox->z0 + 1, Direction::WEST,
                            getGenDepth());
    generateAndAddRoadPiece((StartPiece*)startPiece, pieces, random,
...
bool VillagePieces::Well::postProcess(Level* level, Random* random,
                                      BoundingBox* chunkBB) {
    if (heightPosition < 0) {
    ...
    generateBox(level, chunkBB, 1, 0, 1, 4, height - 3, 4, Tile::cobblestone_Id,
                Tile::water_Id, false);
...
BiomeSource* VillagePieces::StartPiece::getBiomeSource() { return biomeSource; }
...
bool VillagePieces::BookHouse::postProcess(Level* level, Random* random,
    ...
    for (int d = -1; d <= 2; d++) {
        for (int w = 0; w <= 8; w++) {
            placeBlock(level, Tile::stairs_wood_Id, southStairs, w, 6 + d, d,
                       chunkBB);
            placeBlock(level, Tile::stairs_wood_Id, northStairs, w, 6 + d,
                       5 - d, chunkBB);
        }
    }
    ...
    generateBox(level, chunkBB, 8, 2, 1, 8, 4, 4, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 1, 2, 0, 7, 4, 0, Tile::wood_Id, Tile::wood_Id,
                false);
    ...
    generateBox(level, chunkBB, 1, 4, 4, 7, 4, 4, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 1, 3, 4, 7, 3, 4, Tile::bookshelf_Id,
...
bool VillagePieces::SmallHut::postProcess(Level* level, Random* random,
    ...
    generateBox(level, chunkBB, 1, 1, 0, 2, 3, 0, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 1, 1, 4, 2, 3, 4, Tile::wood_Id, Tile::wood_Id,
                false);
...
bool VillagePieces::PigHouse::postProcess(Level* level, Random* random,
    ...
    generateBox(level, chunkBB, 1, 0, 0, 7, 1, 0, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(level, chunkBB, 1, 0, 5, 7, 1, 5, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    ...
    for (int d = -1; d <= 2; d++) {
        for (int w = 0; w <= 8; w++) {
            placeBlock(level, Tile::stairs_wood_Id, southStairs, w, 4 + d, d,
                       chunkBB);
            placeBlock(level, Tile::stairs_wood_Id, northStairs, w, 4 + d,
                       5 - d, chunkBB);
        }
    }
...
bool VillagePieces::TwoRoomHouse::postProcess(Level* level, Random* random,
    ...
    generateBox(level, chunkBB, 2, 0, 6, 2, 3, 10, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    generateBox(level, chunkBB, 3, 0, 10, 7, 3, 10, Tile::cobblestone_Id,
                Tile::cobblestone_Id, false);
    ...
    for (int w = 4; w >= 1; w--) {
        ...
        for (int d = 8 - w; d <= 10; d++) {
            placeBlock(level, Tile::stairs_wood_Id, westStairs, w, 2 + w, d,
                       chunkBB);
        }
    }
    ...
    for (int w = 6; w <= 8; w++) {
        for (int d = 5; d <= 10; d++) {
            placeBlock(level, Tile::stairs_wood_Id, eastStairs, w, 12 - w, d,
                       chunkBB);
        }
    }
...
bool VillagePieces::Smithy::postProcess(Level* level, Random* random,
    ...
    generateBox(level, chunkBB, 1, 1, 0, 2, 3, 0, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 0, 1, 0, 0, 4, 0, Tile::treeTrunk_Id,
                Tile::treeTrunk_Id, false);
    generateBox(level, chunkBB, 3, 1, 0, 3, 4, 0, Tile::treeTrunk_Id,
                Tile::treeTrunk_Id, false);
    generateBox(level, chunkBB, 0, 1, 6, 0, 4, 6, Tile::treeTrunk_Id,
                Tile::treeTrunk_Id, false);
    placeBlock(level, Tile::wood_Id, 0, 3, 3, 1, chunkBB);
    generateBox(level, chunkBB, 3, 1, 2, 3, 3, 2, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 4, 1, 3, 5, 3, 3, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 0, 1, 1, 0, 3, 5, Tile::wood_Id, Tile::wood_Id,
                false);
    generateBox(level, chunkBB, 1, 1, 6, 5, 3, 6, Tile::wood_Id, Tile::wood_Id,
                false);
    ...
    if (!hasPlacedChest) {
        ...
        if (chunkBB->isInside(x, y, z)) {
            hasPlacedChest = true;
            createChest(level, chunkBB, random, 5, 1, 5, treasureItems,
                        3 + random->nextInt(6));
        }
    }
...
bool VillagePieces::Farmland::postProcess(Level* level, Random* random,
                                          BoundingBox* chunkBB) {
    if (heightPosition < 0) {
        heightPosition = getAverageGroundHeight(level, chunkBB);
        if (heightPosition < 0) {
...
bool VillagePieces::DoubleFarmland::postProcess(Level* level, Random* random,
                                                BoundingBox* chunkBB) {
    if (heightPosition < 0) {
        heightPosition = getAverageGroundHeight(level, chunkBB);
        if (heightPosition < 0) {
