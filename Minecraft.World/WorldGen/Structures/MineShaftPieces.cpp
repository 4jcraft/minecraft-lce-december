...
MineShaftPieces::MineShaftRoom::MineShaftRoo  // for reflection
}
...
void MineShaftPieces::MineShaftRoom::addChildren(
    ...
    if (heightSpace <= 0) {
        ...
        while (pos < boundingBox->getXSpan()) {
            ...
            pos += DEFAULT_SHAFT_WIDTH  // southern exits
                pos = 0;
            while (pos < boundingBox->getXSpan()) {
                pos += random->nextInt(boundingBox->getXSpan());
                if ((pos + DEFAULT_SHAFT_WIDTH) > boundingBox->getXSpan()) {
                ...
                if (child != NULL) {
                    BoundingBox* childBox = child->getBoundingBox();
                    childEntranceBoxes.push_back(new BoundingBox(
                        childBox->x0, childBox->y0, boundingBox->z1 - 1,
                        childBox->x1, childBox->y1, boundingBox->z1));
                }
                pos += DEFAULT_SHAFT_WIDTH  // western exits
                    pos = 0;
                while (pos < boundingBox->getZSpan()) {
                    ...
                    pos += DEFAULT_SHAFT_WIDTH  // eastern exits
                        pos = 0;
                    while (pos < boundingBox->getZSpan()) {
                        pos += random->nextInt(boundingBox->getZSpan());
                        if ((pos + DEFAULT_SHAFT_WIDTH) >
                        ...
                        if (child != NULL) {
                            BoundingBox* childBox = child->getBoundingBox();
                            childEntranceBoxes.push_back(new BoundingBox(
                                boundingBox->x1 - 1, childBox->y0, childBox->z0,
                ...
                bool MineShaftPieces::MineShaftRoom::postProcess(
    ...
    generateBox(level, chunkBB, boundingBox->x0, boundingBox->y0,
                ...
                boundingBox->z1, Tile::dirt_Id, // room air    
    generateBox(level, chunkBB, boundingBox->x0, boundingBox->y0 + 1,
                boundingBox->z0, boundingBox->x1,
                std::min(boundingBox->y0 + 3, boundingBox->y1), boundingBox->z1, 0,
                0, false);
    for (AUTO_VAR(it, childEntranceBoxes.begin());
         it != childEntranceBoxes.end(); ++it) {
                            BoundingBox* entranceBox = *it;
                            generateBox(
                                level, chunkBB, entranceBox->x0,
                                entranceBox->y1 - (DEFAULT_SHAFT_HEIGHT - 1),
                                entranceBox->z0, entranceBox->x1,
                                entranceBox->y1, entranceBox->z1, 0, 0, false);
    }
    ...
    return true;
                    ...
                    void MineShaftPieces::MineShaftRoom::addAdditonalSaveData(
                        CompoundTag * tag) {
                        ListTag<IntArrayTag>* entrances =
                            new Li "Entrances" rayTag > (L);
                        for (AUTO_VAR(it, childEntranceBoxes.begin());
                             it != childEntranceBoxes.end(); ++it) {
                            BoundingBox* bb = *it;
                            entrances - "" dd(bb->createTag(L));
                            "Entrances" ag->put(L, entrances);
                        }

                        void
                        MineShaftPieces::MineShaftRoom::readAdditonalSaveData(
                            CompoundTag * tag) {
    ListTag<IntArrayTag>* entrances =
        (ListTag<IntArray"Entrances"getList(L);
    for (int i = 0; i < entrances->size(); i++) {
                                childEntranceBoxes.push_back(
                                    new BoundingBox(entrances->get(i)->data));
    }
                        }
                    ...
                    void
                    MineShaftPieces::MineShaftCorridor::addAdditonalSaveData(
                        CompoundTag * tag) {
                        "hr" ag->putBoolean(L, hasRails);
                        "sc" ag->putBoolean(L, spiderCorridor);
 "hps"g->putBoolean(L, hasPlacedSpider"Num"  tag->putInt(L, numSections);
                    }

                    void
                    MineShaftPieces::MineShaftCorridor::readAdditonalSaveData(
                        CompoundTag * tag) {
                        hasRails "hr" ag->getBoolean(L);
                        spiderCorridor "sc" ag->getBoolean(L);
                        hasPlacedSpider "hps" g->getBoolean(L);
                        numSect "Num" = tag->getInt(L);
                    }

                    MineShaftPieces::MineShaftCorridor::MineShaftCorridor(
                        int genDepth, Random* random, BoundingBox* corridorBox,
                        int direction)
                        : StructurePiece(genDepth) {
                        ...
                        spiderCorridor =
                            !hasRails &&
                            random->ne  // debug) == // spiderCorridor =
                                        // !hasRails ;//&& random->nextInt(23)
                                        // == 0;

                            if (orientation == Direction::NORTH ||
                                orientation == Direction::SOUTH) {
                            numSections =
                                corridorBox->getZSpan() / DEFAULT_SHAFT_LENGTH;
                        }
                        else {
                            numSections =
                        ...
                        void MineShaftPieces::MineShaftCorridor::addChildren(
                            ...
                            switch (orientation) {
                                case Direction::NORTH:
                                    if (endSelection <= 1) {
                                        generateAndAddPiece(
                                            startPiece, pieces, random,
                                            boundingBox->x0,
                                            boundingBox->y0 - 1 +
                                                random->nextInt(3),
                                            boundingBox->z0 - 1, orientation,
                                            depth);
                                ...
                                case Direction::EAST:
                                    ...
                                    // generate cross sections using higher
                                    // depth
                                    if (depth < MAX_DEPTH) {
                                        if (orientation == Direction::NORTH ||
                                            orientation == Direction::SOUTH) {
                                            for (int z = boundingBox->z0 + 3;
                                                 (z + DEFAULT_SHAFT_WIDTH) <=
                                                 boundingBox->z1;
                            ...
                            MineShaftPieces::MineShaftCorridor::createChest(
                                ...
                                if (chunkBB->isInside(worldX, worldY, worldZ)) {
                                    if (level->getTile(worldX, worldY,
                                                       worldZ) == 0) {
                                        ...
                                        std::shared_ptr<MinecartChest> chest =
                                            std::shared_ptr<MinecartChest>(
                                                new MinecartChest(
                                                    level, worldX + 0.5f,
                                                    worldY + 0.5f,
                                                    worldZ + 0.5f));
                                        WeighedTreasure::addChestItems(
                                            random, treasure, chest, numRolls);
    ...
    const int length = (numSections * DE// corridor airGTH) - 1;
    ...
    if (spiderCorridor) {
        generateMaybeBox(level, chunkBB, random, .6f, x0, 0, y0, x1, y1 - 1,
                         length, Tile::// place a support in every section
    for (int section = 0; section < numSections; section++) {
                                        int z =
                                            2 +
                                            section *
                                                D  // 4J-PB - Bringing forward
                                                   // the changes
                                                   // in 1.2.3
                                                    generateBox(level, chunkBB,
                                                                ...
                                                                Tile::fence_Id,
                                                                0, false);
                                        generateBox(level, chunkBB, x1, y0, z,
                                                    x1, y1 - 1, z,
                                                    Tile::fence_Id, 0, false);
                                        if (random->nextInt(4) == 0) {
                                            generateBox(
                                                level, chunkBB, x0, y1, z, x0,
                                                y1, z, Tile::wood_Id, 0, false);
                                            generateBox(
                                                level, chunkBB, x1, y1, z, x1,
                                                y1, z, Tile::wood_Id, 0, false);
                                        } else {
                                            generateBox(
                                                level, chunkBB, x0, y1, z, x1,
                                                y1, z, Tile::wood_Id, 0, false);
                                        }
                                        maybeGenerateBlock(
                                            level, chunkBB, random, .1f, x0, y1,
                                            z - 1, Tile::web_Id, 0);
                                        maybeGenerateBlock(
                                            level, chunkBB, random, .1f, x1, y1,
                                            z - 1, Tile::web_Id, 0);
                                        maybeGenerateBlock(
                                            level, chunkBB, random, .1f, x0, y1,
                                            z + 1, Tile::web_Id, 0);
                                        maybeGenerateBlock(
                                            ...
                                            z + 1, Tile::web_Id, 0);
                                        maybeGenerateBlock(
                                            level, chunkBB, random, .05f, x0,
                                            y1, z - 2, Tile::web_Id, 0);
                                        maybeGenerateBlock(
                                            level, chunkBB, random, .05f, x1,
                                            y1, z - 2, Tile::web_Id, 0);
                                        maybeGenerateBlock(
                                            level, chunkBB, random, .05f, x0,
                                            y1, z + 2, Tile::web_Id, 0);
                                        ...
                                        maybeGenerateBlock(level, chunkBB,
                                                           random, .05f, x0 + 1,
                                                           y1, z - 1,
                                                           Tile::torch_Id, 0);
                                        maybeGenerateBlock(level, chunkBB,
                                        ...
                                        if (random->nextInt(100) == 0) {
                                            createChest(
                                                level, chunkBB, random, x1, y0,
                                                z - 1,
                                                WeighedTreasure::addToTreasure(
                                                    smallTreasureItems,
                                                    Item::enchantedBook
                                                        ->createForRandomTreasure(
                                                            random)),
                                                3 + random->nextInt(4));
                                        }
                                        if (random->nextInt(100) == 0) {
                                            createChest(
                                                level, chunkBB, random, x0, y0,
                                                z + 1,
                                                WeighedTreasure::addToTreasure(
                                                    smallTreasureItems,
                                                    Item::enchantedBook
                                        ...
                                        if (spiderCorridor &&
                                            ...
                                            if (chunkBB->isInside(x, y, newZ)) {
                                                ...
                                                level->setTileAndData(
                                                    x, y, newZ,
                                                    Tile::mobSpawner_Id, 0,
                                                    Tile::UPDATE_CLIENTS);
                                                std::shared_ptr<
                                                    MobSpawnerTileEntity>
                                                    entity = dynamic_pointer_cast<
                                                        MobSpawnerTileEntity>(
                                                        level->getTileEntity(
                                                            x, y, newZ));
                                                if (entity != NULL)
                                        ...
                                        MineShaftPieces::MineShaftCrossing::
                                            ...
                                            : StructurePiece(genDepth),
                                              direction(direction),
                                              isTwoFloored(
                                                  crossingBox->getYSpan() >
                                                  DEFAULT_SHAFT_HEIGHT) {
                                            boundingBox = crossingBox;
                                        }

                                        BoundingBox* MineShaftPieces::
                                            MineShaftCrossing::findCrossing(
                                                std::list<StructurePiece*> *
                                                    pieces,
...
void MineShaftPieces::MineShaftCrossing::addChildren(
    StructurePiece* startPiece, std::list<StructurePiece*>* pieces, Random* rando// crossings are coming from a direction and will generate children// in the// remaining three directions
...
bool MineShaftPieces::MineShaftCrossing::postProcess(Level* level,
                                                     Random* random,
                                                     BoundingBox* chunkBB) {
    if (edgesLiquid(level, chunk// corridor airreturn false;
    }

    
    if (isTwoFloored) {
                                        generateBox(
                                            level, chunkBB, boundingBox->x0 + 1,
                                        ...
                                        generateBox(
                                            ...
                                            boundingBox->z0,
                                            boundingBox->x1 - 1,
                                            boundingBox->y1, boundingBox->z1, 0,
                                            0, false);
                                        generateBox(
                                            level, chunkBB, boundingBox->x0,
                                            boundingBox->y1 -
    ...
    generateBox(level, chunkBB, boundingBox->x1 - 1, boundingBox->y0,
                ...
                boundingBox-// prevent air floating 0, f// note: use world coordinates because the corridor hasn't defined// orientation
    
    for (int x = boundingBox->x0; x <= boundingBox->x1; x++) {
                                        for (int z = boundingBox->z0;
                                             z <= boundingBox->z1; z++) {
                                            int block = getBlock(
                                                level, x, boundingBox->y0 - 1,
                                                z, chunkBB);
                                            if (block == 0) {
...
MineShaftPieces::M// for reflectionineShaftStairs() {
                            ...
                            void MineShaftPieces::MineShaftStairs::
...
BoundingBox* MineShaftPieces::MineShaftStairs::findStairs(
    std::list<StructurePiece*>* pieces, Random* random, int footX, in// stairs are two steps in, 5x5 steps down, two steps out
...
void MineShaftPieces::MineShaftStairs::addChildren(
    StructurePiece* startPiece, std::list<StructurePiece*>* pieces, Ran// crossings are coming from a direction and will generate children// in the// remaining three directions
    
    
    switch (orientation) {
                            case Direction::NORTH:
                                generateAndAddPiece(
                                    startPiece, pieces, random, boundingBox->x0,
                    ...
                    bool MineShaftPieces::MineShaftStairs::postProcess(
                        Level * level, Random * random, BoundingBox * chunkBB) {
    if (edgesLiquid(le// upper floor {
        return false;
                    }
    ...
    generateBox(level, chunkBB, 0, 0, 7, (DEFAULT_SHAFT_WIDTH - 1),
               // stairs_SHAFT_HEIGHT - 1), 8, 0, 0, false);
    
    for (int i = 0; i < 5; i++) {
                        generateBox(level, chunkBB, 0,
                                    5 - i - ((i < 4) ? 1 : 0), 2 + i,
                                    (DEFAULT_SHAFT_WIDTH - 1),
                                    5 + (DEFAULT_SHAFT_HEIGHT - 1) - i, 2 + i,
                                    0, 0, false);
    }
