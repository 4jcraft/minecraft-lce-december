...
StructurePiece* StructurePiece::findCollisionPiece(
    ...
    int z1 = Math::_min(boundingBox->z1 + 1, chunkBB->// roof and floor
    for (int x = x0; x <= x1; x++) {
            for (int z = z0; z <= z1; z++) {
            ...
            for (int x = x0; x <= x1; x++) {
                ...
                for (int z = z0; z <= z1; z++) {
                    for (int y = y0; y <= y1; y++) {
                        ...
                        if (tile > 0 &&
                            Tile::tiles[tile]->material->isLiquid()) {
                            return true;
                        }
            ...
            int StructurePiece::getOrientationData(int tile, int data) {
                ...
                } else if (tile == Tile::door_wood_Id ||
                           tile == Tile::door_iron_Id) {
            ...
            void StructurePiece::placeBlock(Level * level, int block, int data,
                                            int x, int y, int z,
                                            BoundingBox* chunkBB) {
                ...
                if (!chunkBB->isInside(worldX, worldY, worldZ)) {
                    return;
                    // 4J Stu - We shouldn't be removing bedrock when generating
                    // things (eg in// SuperFlat)
                    if (worldY == 0) return;
...
void StructurePiece::generateBox(Level* level, BoundingBox* chunkBB, int x0,
                        ...
                        for (int y = y0; y <= y1; y++) {
                            for (int x = x0; x <= x1; x++) {
                                for (int z = z0; z <= z1; z++) {
                                    if (skipAir && getBlock(level, x, y, z,
                                                            chunkBB) == 0) {
                                        continue;
                                    }
                                    if (y == y0 || y == y1 || x == x0 ||
                                        x == x1 || z == z0 || z == z1) {
                                        placeBlock(level, edgeTile, 0, x, y, z,
                                                   chunkBB);
                                    } else {
                                        placeBlock(level, fillTile, 0, x, y, z,
                                                   chunkBB);
                                    }
                                }
                            }
                        ...
                        for (int y = y0; y <= y1; y++) {
                            for (int x = x0; x <= x1; x++) {
                                for (int z = z0; z <= z1; z++) {
                                    ...
                                    if (y == y0 || y == y1 || x == x0 ||
                                        x == x1 || z == z0 || z == z1) {
                                        placeBlock(level, edgeTile, edgeData, x,
                                                   y, z, chunkBB);
                                    } else {
                                        placeBlock(level, fillTile, fillData, x,
                                                   y, z, chunkBB);
                                    }
                                }
                            }
...
void StructurePiece::generateBox(Level* level, BoundingBox* chunkBB,
                                 ...
                                 bool skipAir) {
                        generateBox(level, chunkBB, boxBB->x0, boxBB->y0,
                                    boxBB->z0, boxBB->x1, boxBB->y1, boxBB->z1,
                                    edgeTile, fillTile, skipAir);
}
...
void StructurePiece::generateUpperHalfSphere(Level* level, BoundingBox* chunkBB,
                        ...
                        for (int y = y0; y <= y1; y++) {
                            ...
                            for (int x = x0; x <= x1; x++) {
                                float normalizedXDistance =
                                    (float)(x - cx) / (diagX * 0.5f);

                                for (int z = z0; z <= z1; z++) {
                                    float normalizedZDistance =
                                        (float)(z - cz) / (diagZ * 0.5f);
                                    ...
                                    float dist = (normalizedXDistance *
                                                  normalizedXDistance) +
                                                 (normalizedYDistance *
                                                  normalizedYDistance) +
                                                 (normalizedZDistance *
...
void StructurePiece::fillColumnDown(Level* level, int tile, int tileData, int x,
                        ...
                        while ((level->isEmptyTile(worldX, worldY, worldZ) ||
                                level->getMaterial(worldX, worldY, worldZ)
                                    ->isLiquid()) &&
                               worldY > 1) {
                            level->setTileAndData(worldX, worldY, worldZ, tile,
                                                  tileData,
                                                  Tile::UPDATE_CLIENTS);
                            worldY--;
                        }
...
bool StructurePiece::createChest(Level* level, BoundingBox* chunkBB,
                        ...
                        if (chunkBB->isInside(worldX, worldY, worldZ)) {
                            if (level->getTile(worldX, worldY, worldZ) !=
                                Tile::chest->id) {
                                level->setTileAndData(worldX, worldY, worldZ,
                                                      Tile::chest->id, 0,
                                                      Tile::UPDATE_CLIENTS);
                                std::shared_ptr<ChestTileEntity> chest =
                                    dynamic_pointer_cast<ChestTileEntity>(
                                        level->getTileEntity(worldX, worldY,
                                                             worldZ));
                                if (chest != NULL)
                        ...
                        if (chunkBB->isInside(worldX, worldY, worldZ)) {
                            if (level->getTile(worldX, worldY, worldZ) !=
                                ...
                                level->setTileAndData(
                                    ...
                                    getOrientationData(Tile::dispenser_Id,
                                                       facing),
                                    Tile::UPDATE_CLIENTS);
                                std::shared_ptr<DispenserTileEntity> dispenser =
                                    dynamic_pointer_cast<DispenserTileEntity>(
                                        level->getTileEntity(worldX, worldY,
                                                             worldZ));
                                if (dispenser != NULL)
                                    WeighedTreasure::addDispenserItems(
                                        random, items, dispenser, numRolls);
                                return true;
                            }
                        }
                        return false;
...
void StructurePiece::createDoor(Level* level, BoundingBox* chunkBB,
                        ...
                        if (chunkBB->isInside(worldX, worldY, worldZ)) {
                            DoorItem::place(level, worldX, worldY, worldZ,
                                            orientation, Tile::door_wood);
                        }
}
