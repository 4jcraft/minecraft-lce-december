...
BufferedOutputStream::~BufferedOutputStream() { delete buf.data; }
    ...
    stream->close()  // Writes len bytes from the specified uint8_t array
        ...
        void BufferedOutputStream::write(
            ...
            unsigned int length) {  // Over the length of what we can store in
                                    // our buffer - just flush the// buffer
                                    // and output directly
        if (length >= buf.length) {
            ...
            for (unsigned int i = 0; i < length; i++) {
                write(b[offset + i]);
            }  // Writes b.length bytes to this output stream.// The write
               // method of FilterOutputStream calls its write method of
               // three// arguments with the arguments b, 0, and
