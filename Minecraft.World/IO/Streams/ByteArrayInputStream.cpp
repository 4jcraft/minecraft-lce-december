...
ByteArrayInputStream::ByteArrayInputStream(byteArray buf, unsigned int offset,
                                           unsigned int length)
    : pos(offset),
      std::count(std::min(offset + length, buf.length)),
      mark(offset) {
    this->buf  // Creates a ByteArrayInputStream so that it uses buf as its
               // buffer array. The// buffer array is not copied. The initial
               // value of pos is 0 and the initial// value of count is the
               // length of buf. Parameters: buf - the input buffer.
    ...
    std::count(buf.length), mark(0) {
        this -  // Reads the next uint8_t of data from this input stream. The
            ...
            if (pos >= count)
                ...
                return  // Reads some number of bytes from the input stream and
                    ...
                    // buffer, or -1 is there is no more// data because the end
                    // of the stream has been reached.
                    int ByteArrayInputStream::read(byteArray b) { return read(b, 0,// Reads up to len bytes of data into an array of bytes from this input stream.// If pos equals count, then -1 is returned to indicate end of file. Otherwise,// the number k of bytes read is equal to the smaller of len and count-pos. If k// is positive, then bytes buf[pos] through buf[pos+k-1] are copied into b[off]// through b[off+k-1] in the manner performed by System.arraycopy. The value k// is added into pos and k is returned. This read method cannot block.// Parameters:
// b - the buffer into which the data is read.// off - the start offset in the destination array b// len - the maximum number of bytes read.// Returns:// the total number of bytes read into the buffer, or -1 if there is no more// data because the end of the stream has been reached.
int ByteArrayInputStream::read(byteArray b, unsigned int offset,
                               unsigned int length) {
                        if (pos == count) return -1;

                        int k = std::min(length, count - pos);
    XMemCpy(&b[offset],// std::copy( buf->data+pos, buf->data+pos+k, b->data + offset ); // Or this// instead?
    

    pos += k// Closing a ByteArrayInputStream has no effect.// The methods in this class can be called after the stream has been closed// without generating an IOException.
void ByteArrayInputStream::c// Skips n bytes of input from this input stream. Fewer bytes might be skipped// if the end of the input stream is reached. The actual number k of bytes to be// skipped is equal to the smaller of n and count-pos. The value k is added into// pos and k is returned. Overrides: skip in class InputStream Parameters: n -// the number of bytes to be skipped. Returns: the actual number of bytes// skipped.
