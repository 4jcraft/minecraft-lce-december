...
// Creates a new uint8_t array output stream, with a buffer capacity of the
...
void ByteArrayOutputStream::write(byteArray b, unsigned int offset,
    ...
    XMemCpy(&buf[count], &b[offset],
            length);  // std::copy( b->data+offset, b->data+offset+length,
                      // buf->data + count ); //// Or this instead?

    count += length  // Closing a ByteArrayOutputStream has no effect.// The
                     // methods in this class can be called after the stream has
                     // been closed// without generating an IOException.
        void ByteArrayOutputStream::
            close()  // Creates a newly allocated uint8_t array. Its size is the
                     // current size of this// output stream and the valid
                     // contents of the buffer have been copied into it.//
                     // Returns:// the current contents of this output stream,
                     // as a uint8_t array.
