...
// result.
int DataInputStream::read() { return stream->read(); }
...
// otherwise, there is an attempt to read at least one uint8_t. If no uint8_t is
...
int DataInputStream::read(byteArray b) { return read(b, 0, b.length); }
...
//
// If the first uint8_t of a group matches the pattern 1111xxxx or the pattern
...
int DataInputStream::readUTFChar() {
    ...
    // ERROR, or second/third uint8_t// 1111xxxx = 0xF0 //ERROR//
    // 0xxxxxxx = 0x00 // One uint8_t UTF// 110xxxxx = 0xC0 // Two uint8_t
    // UTF// 1110xxxx = 0xE0 // Three uint8_t UTF
    if (((firstByte & 0xC0) == 0x80) || ((firstByte & 0xF0) == 0xF0)) {
        // TODO 4J Stu - UTFDataFormatException
        return returnValue;
    } else if ((firstByte & 0x80) == 0x00) {
