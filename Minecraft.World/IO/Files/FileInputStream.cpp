...
FileInputStream::FileInputStream(const File& file) {
    ...
    m_fileHandle = CreateFile(
        ...
        0,  // share mode // TODO 4J Stu - Will we need to share file? Probably
            // not but...
        NULL,           // Unused
        OPEN_EXISTING,  // how to create // TODO 4J Stu - Assuming that the file
                        // already exists if we are opening to read from it
...
// Reads a uint8_t of data from this input stream. This method blocks if no
// input is yet available. Returns: the next uint8_t of data, or -1 if the end
...
__int64 FileInputStream::skip(__int64 n) {
    ...
    li.QuadPart = n;
    li.LowPart =
        SetFilePointer(m_fileHandle, li.LowPart, &li.HighPart, FILE_CURRENT);
