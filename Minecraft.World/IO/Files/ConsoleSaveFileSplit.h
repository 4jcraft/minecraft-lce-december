...
private:
    ...
    public:
        ...
        bool operator<(const DirtyRegionFile& rhs) {
...
private:
    void _init(const std::wstring& fileName, LPVOID pvSaveData, DWORD fileSize,
               ESavePlatform plat);

public:
    ConsoleSaveFileSplit(const std::wstring& fileName, LPVOID pvSaveData = NULL,
                         DWORD fileSize = 0, bool forceCleanSave = false,
    ...
    virtual ~ConsoleSaveFileSpli  // 4J Stu - Initial implementation is intended
                                  // to have a similar interface// to the
                                  // standard Xbox file access
                                  // functions

        virtual FileEntry* createFile(const ConsoleSavePath& fileName);
    virtual void deleteFile(FileEntry* file);

    virtual void setFilePointer(FileEntry* file, LONG lDistanceToMove,
                                PLONG lpDistanceToMoveHigh, DWORD dwMoveMethod);
    virtual BOOL writeFile(FileEntry* file, LPCVOID lpBuffer,
                           DWORD nNumberOfBytesToWrite,
                           LPDWORD lpNumberOfBytesWritten);
    virtual BOOL zeroFile(FileEntry* file, DWORD nNumberOfBytesToWrite,
                          LPDWORD lpNumberOfBytesWritten);
    virtual BOOL readFile(FileEntry* file, LPVOID lpBuffer,
    ...
    virtual void Flush(bool autosave, bool updateThumbnail
#ifndef _CONTENT_PACKAGE
    virtual void DebugFlushToFile(void* compressedData = NULL,
                       unsigned int compressedDataS #endif0);
    ...
    virtual std::vector<File#endif  //__PS3__PlayerDatFiles();
