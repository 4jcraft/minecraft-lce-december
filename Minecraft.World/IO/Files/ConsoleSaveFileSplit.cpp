...
void ConsoleSaveFileSplit::_init(const std::wstring& fileName,
                                 LPVOID pvSaveData, DWORD fileSize,
                                 ESavePlatform plat) {
    ...
    for (unsigned int i = 0; i < regionCount; i++) {
        ...
        StorageManager.GetSubfileDetails(i, &regionIndex, &regionDataCompressed,
...
FileEntry* ConsoleSaveFileSplit::createFile(const ConsoleSavePath& fileName) {
    ...
    PIXBeginName"Growing file by %d bytes"          , bytesToGrowBy);
        ...
        BOOL ConsoleSaveFileSplit::writeFile(FileEntry * file, LPCVOID lpBuffer,
            ...
            if (file->isRegionFile()) {
                ...
                RegionFileReference* fileRef =
                    regionFiles[file->data.regionIndex];
        ...
        BOOL ConsoleSaveFileSplit::readFile(FileEntry * file, LPVOID lpBuffer,
            ...
            if (file->isRegionFile()) {
                ...
                if (file->currentFilePointer + nNumberOfBytesToRead >
                    file->data.length) {
                    actualBytesToRead =
                        file->data.length - file->currentFilePointer;
                }
                RegionFileReference* fileRef =
                    regionFiles[file->data.regionIndex];
                        ...
                        for (AUTO_VAR(it, writeHistory.begin());
                            ...
                            for (AUTO_VAR(it, regionFiles.begin());
                                ...
                                if (it->second->dirty) {
                                    ...
                                    unsigned int bytesInTimePeriod =
                                        bytesWritten;
                                    unsigned int bytesAddedThisTick = 0;
                                    for (int i = 0; i < dirtyRegions.size();
                                         i++) {
                                        RegionFileReference* regionRef =
                                            regionFiles[dirtyRegions[i]
                                                            .fileRef];
                                        unsigned int compressedSize =
                                            regionRef->GetCompressedSize();
                                        ...
                                        bytesAddedThisTick +=
                                            ...
                                            if ((bytesInTimePeriod /
                                                ...
                                                writeEvent.writeSize =
                                                    compressedSize;
                                                writeEvent.writeTime =
                                                    System::currentTimeMillis();
                                                writeHistory.push_back(
                                                    writeEvent);
...
int ConsoleSaveFileSplit::getSaveVersion() {
                                                    ...
                                                    bool ConsoleSaveFileSplit::
                                                        isSaveEndianDifferent() {
                                                        return header
                                                            .isSaveEndianDifferent();
                                                    }
