...
ConsoleSaveFileOriginal::ConsoleSaveFileOriginal(
    const std::wstring& fileName, LPVOID pvSaveData /*= NULL*/,
    DWORD dFileSize /*= 0*/, bool forceCleanSave /*= false*/,
...
// Initially committ enough room to store headSize bytes (using// CSF_PAGE_SIZE pages, so rounding up here). We should only ever have one// save file at a time, and the pages should be decommitted in the dtor, so// pages committed should always be zero at this point.
    if (pagesCommitted != #ifndef _CONTENT_PACKAGE
        __debugbrea#endif
}
...
void* pvRet = VirtualAlloc(pvHeap, pagesRequired* CSF_PAGE_SIZE,
    ...
    if (pvRet == NUL#ifndef _CONTENT_PACKAGE
   // Out of physical memory
    ...
    if (fileSize > 0) {
        ...
        if (pvSaveData != NUL#ifdef __PSVITA__
       // AP - use this to access the virtual memory
            VirtualCopyTo(pvSaveMem, pvSaveData, fileSi#else
            memcpy(pvSaveMem, pvSaveData, fileSize);
            if (bLevelGenBaseSave) {
        levelGen->deleteBaseSaveData();
#endif 
        } else {
        unsigned int storageLen #ifdef __PSVITA__
            // create a buffer to hold the compressed data
            pvSaveData = malloc(fileSize);
        AllocData = true;
            StorageManager.GetSaveData(pvSaveData, &storageLeng#else
            StorageManager.GetSaveData(pvSaveMem, &storageLeng#endif#ifdef __PS3__
            StorageManager.FreeSaveDat#endif
            app.DebugPr"Filesize - %d, Adjusted size - %d\n", fileSize,
                            storageLength);
        ...
        if (plat == SAVE_FILE_PLATFORM_PSVITA) {
...
#ifdef __PSVITA__
            void* pvSourceData = pvSaveD#else
            void* pvSourceData = pvSave#endif
            ...
            if (decompSize == 0) {
                ...
                // will have this problem
                    ...
                    } #endif
                    {
                        Compression::getCompression()->SetDecompressionType(
                            pla// if this save is from another platform,
                               // set the correct decompression type
                        Compression::getCompression()->Decompress(
                            buf, &decompSize, (unsigned char*)pvSourceData + 8,
                            fileSize - 8);
                        Compression::getCompression()->SetDecompressionType(
                            SAVE_FILE_PLATFORM_LOCA// and then set the
               ...
               // and align to 512 uint8_t boundaries
                    ...
                    pagesCommitted = pagesRequired;
#ifdef __PSVITA__
                    VirtualCopyTo(pvSaveMem, buf, decompSi #else 
                    memcpy(pvSaveMem, buf, decompSi #endif 
                    delete[] buf;
                }
            }
        }
    ...
    ConsoleSaveFileOriginal::~ConsoleSaveFileOriginal() {
        ...
        pagesCommitted = 0;  // Make sure we don't have any thumbnail data still
                             // waiting round - we can't// need it now we've
                             // destroyed the save file anyway#if defined _XBOX
    app.GetSaveThumbnail(NULL, NU#elif defined __PS3__
    app.GetSaveThumbnail(NULL, NULL, NULL, NU#endif

    DeleteCriticalSection(&m_lock)// Add the file to our table of internal files if not already there// Open our actual save file ready for reading/writing, and the set the file// pointer to the start of this file
...
void ConsoleSaveFileOriginal::deleteFile(FileEntry* file) {
        ...
        while (true) {
            ...
            if (amountToRead == 0) br
#ifdef __PSVITA__
       // AP - use this to access the virtual memory
        VirtualCopyFrom(buffer, readStartOffset, amountToRe#else
        memcpy(buffer, readStartOffset, amountToRe#endif
        numberOfBytesRead = amountToRead;

        bufferDataSize = amountToRead;
        readStartOffset += numberOfBytesRead;

   // Write buffer to file#ifdef __PSVITA__
   // AP - use this to access the virtual memory
        VirtualCopyTo((void*)writeStartOffset, buffer, bufferDataSi#else
        memcpy((void*)writeStartOffset, buffer, bufferDataSi#endif
...
void ConsoleSaveFileOriginal::setFilePointer(FileEntry* file,
        ...
        ReleaseSaveAccess()  // If this file needs to grow, move the data after
                             // along
            ...
            // 4J Stu - Not forcing a minimum size, it is up to the caller to
            // write data// in sensible amounts This lets us keep some of
            // the smaller files small// if( bytesToGrowBy < 1024 )//
            // bytesToGrowBy = 1024; Move all the data beyond us
        ...
        BOOL ConsoleSaveFileOriginal::writeFile(
            FileEntry * file, LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite,
            ...
            char* writeStartOffset =
                (char*)pvSaveMem +
                file->currentFilePointer;  // printf("Write: pvSaveMem = %0xd,
                                           // currentFilePointer = %d,//
                                           // writeStartOffset = %0xd\n",
                                           // pvSaveMem,
                                           // file->currentFilePointer,//
                                           // writeStartOffset);
#ifdef __PSVITA__  // AP - use this to access the virtual memory
    VirtualCopyTo((void*)writeStartOffset, (void*)lpBuffer,
                  nNumberOfBytesToWri#else
    memcpy((void*)writeStartOffset, lpBuffer, nNumberOfBytesToWri#endif
        ...
        BOOL ConsoleSaveFileOriginal::zeroFile(FileEntry * file,
            ...
            char* writeStartOffset =
                ...
                file->currentFilePointer;  // printf("Write: pvSaveMem = %0xd,
                                           ...
                                           // writeStartOffset);
#ifdef __PSVITA__  // AP - use this to access the virtual memory
    VirtualMemset((void*)writeStartOffset, 0, nNumberOfBytesToWri#else
    memset((void*)writeStartOffset, 0, nNumberOfBytesToWri#endif
    *lpNumberOfBytesWritten = nNumberOfBytesToWrite;
...
// wprintf(L"Wrote %d bytes to %s, new file pointer is %I64d\n",// *lpNumberOfBytesWritten, file->data.filename, file->currentFilePointer);
    ...
    return 1;
        ...
        BOOL ConsoleSaveFileOriginal::readFile(
            FileEntry * file, LPVOID lpBuffer, DWORD nNumberOfBytesToRead,
...
#ifdef __PSVITA__  // AP - use this to access the virtual memory
    VirtualCopyFrom(lpBuffer, readStartOffset, actualBytesToRe#else
    memcpy(lpBuffer, readStartOffset, actualBytesToRe#endif
...
// wprintf(L"Read %d bytes from %s, new file pointer is %I64d\n",// *lpNumberOfBytesRead, file->data.filename, file->currentFilePointer);
    ...
    return 1;
            ...
            void ConsoleSaveFileOriginal::MoveDataBeyond(
                ...
                DWORD desiredSize =
                    header.GetFileSize() + nNumberOfBytesToWrite;
                ...
                // This is the start of where we want the space to be, and the
                // start of the// data that we need to move
                char* spaceStartOffset = (char*)pvSaveMem +
                                         file->data.startOffset +
                                         file->getFileSize();
                ...
                char* beginEndOfDataOffset =
                    (char*)pvSaveMem + header.GetStartOfNextData();
                // This is where the end of the data is going to be
                char* finishEndOfDataOffset =
                    beginEndOfDataOffset + nNumberOfBytesToWrite;
                // This is where we are going to read from (with the amount we
                // want to read// subtracted before we read)
                char* readStartOffset = beginEndOfDataOffset;
                // This is where we can safely write to (with the amount we want
                // write// subtracted before we write)
                char* writeStartOffset = finishEndOfDataOffset;
                // printf("\n******* MOVEDATABEYOND *******\n");//
                // printf("Space start: %d, space end: %d\n", spaceStartOffset -
                // (char// *)pvSaveMem, spaceEndOffset - (char
                // *)pvSaveMem); printf("Current end of// data: %d, new end
                // of data: %d\n", beginEndOfDataOffset - (char//
                // *)pvSaveMem, finishEndOfDataOffset - (char *)pvSaveMem);
                // Optimisation for things that are being moved in whole region
                // file sector// (4K chunks). We could generalise this a
                // bit more but seems safest at the// moment to identify
                // this particular type of move and code explicitly for//
                // this situation
                if ((nNumberOfBytesToWrite & 4095) == 0) {
                    if (nNumberOfBytesToWrite > 0) {
                        // Get addresses for start & end of the region we are
                        // copying from as uintptr_t, for easier maths
                        ...
                        uintptr_t uiFromStartChunk =
                            uiFromStart & ~((uintptr_t)4095);
                        uintptr_t uiFromEndChunk =
                            (uiFromEnd - 1) & ~((uintptr_t)4095);

                        // Loop through all the affected source 4096 chunks,
                        // going backwards so we don't overwrite anything
                        // we'll need in the future
                        for (uintptr_t uiCurrentChunk = uiFromEndChunk;
                             uiCurrentChunk >= uiFromStartChunk;
                             uiCurrentChunk -= 4096) {
                            // Establish chunk we'll need to copy
                            uintptr_t uiCopyStart = uiCurrentChunk;
                            uintptr_t uiCopyEnd = uiCurrentChunk + 4096;
                            // Clamp chunk to the bounds of the full region we
                            // are trying to copy
                            if (uiCopyStart < uiFromStart) {
                                // Needs to be clampged against the start of our
                                // region
                                uiCopyStart = uiFromStart;
                            }
                            if (uiCopyEnd > uiFromEnd) {
                                // Needs to be clamped to the end of our
                                // region
                                uiCopyEnd = uiFromEnd;
#ifdef __PSVITA__
                                // AP - use this to access the virtual memory
                VirtualMove((void*)(uiCopyStart + nNumberOfBytesToWrite),
                            (void*)uiCopyStart, uiCopyEnd - uiCopySta#else
                XMemCpy((void*)(uiCopyStart + nNumberOfBytesToWrite),
                        (void*)uiCopyStart, uiCopyEnd - uiCopySta#endif
                            }
                        }
                    } else {
                        while (true) {
                            ...
                            if ((readStartOffset - bufferSize) <
                                spaceStartOffset) {
                                amountToRead =
                                    (DWORD)(readStartOffset - spaceStartOffset);
                            } else {
                                amountToRead = bufferSize;
                            }

                            // Push the read point back by the amount of bytes
                            // that we are going to read
                            readStartOffset -= amountToRead;

                            // printf("About to read %u from %d\n",
                            // amountToRead, readStartOffset - (char
                            // *)pvSaveMem );#ifdef __PSVITA__ AP - use
                            // this to access the virtual memory
            VirtualCopyFrom(buffer1, readStartOffset, amountToRe#else
            memcpy(buffer1, readStartOffset, amountToRe#endif
            numberOfBytesRead = amountToRead;
       ...
       // Write buffer 2 to file
            if ((writeStartOffset + buffer2Size) <= finishEndOfDataOffset) {
                                ...
                                // buffer2Size, writeStartOffset - (char
                                // *)pvSaveMem );#ifdef __PSVITA__ AP -
                                // use this to access the virtual memory
                VirtualCopyTo((void*)writeStartOffset, buffer2, buffer2Si#else
                memcpy((void*)writeStartOffset, buffer2, buffer2Si#endif
            ...
            if (numberOfBytesRead == 0) {
                ...
                bool ConsoleSaveFileOriginal::doesFileExist(
                ...
                void ConsoleSaveFileOriginal::Flush(bool autosave,
                                                    bool updateThumbnail) {
                    LockSaveAcces
#ifdef __PSVITA__  // On Vita we've had problems with saves being corrupted
                       // on rapid// save/save-exiting so seems prudent to wait
                       // for idle
                        while (StorageManager.GetSaveState() !=
                               C4JStorage::ESaveGame_Idle) {
        app.DebugPr"Flush wait\n");
        Sleep(10);
#endif
        ...
        // Assume that the compression will make it smaller so initially attempt
        // to// allocate the current file size We add 4 bytes to the start
        // so that we can// signal compressed data And another 4 bytes to
        // store the decompressed data// size
        unsigned int compLength = fileSize + 8;
                        // 4J Stu - Added TU-1 interim
#ifdef __PS3__  // On PS3, don't compress the data as we can't really afford
                    // the extra// memory this requires for the output buffer.
                    // Instead we'll be writing// directly from the save data.
...
#ifdef __PSVITA__  // AP - make sure we always allocate just what is needed
                       // so it will only// SAVE what is needed. If we don't do
                       // this the StorageManager will save a// file of
                       // uncompressed size unnecessarily.
    compData = N#endif
// If we failed to allocate then compData will be NULL// Pre-calculate the compressed data size so that we can attempt to allocate// a smaller buffer
    if (compData == NULL) {
                            // Length should be 0 here so that the compression
                            // call knows that we want to know the length
                            // back
                            compLength = 0;

                            // Pre-calculate the buffer size required for the
                            // compressed data
        PIXBeginNamedEven"Pre-calc save compression");
        // Save the start time
        QueryPerformanceCounter(&qwTi#ifdef __PSVITA__
   // AP - get the compressed size via the access function. This uses a
   // special RLE format
        VirtualCompress(NULL, &compLength, pvSaveMem, fileSi#else
        ...
        app.DebugPr"Check buffer size: Elapsed time %f\n", fElapsedTime);
        PIXEndNamedEvent();
   ...
   // Attempt to allocate the required memory
    ...
    if (compData != NULL) {
                                // No compression on PS3 - see comment
                                // above#ifndef __PS3__ Re-compress all save
                                // data before we save it to disk
        PIXBeginNamedEven"Actual save compression");
        // Save the start time
        QueryPerformanceCounter(&qwTi#ifdef __PSVITA__
   // AP - compress via the access function. This uses a special RLE format
        VirtualCompress(compData + 8, &compLength, pvSaveMem, fileSi#else
        Compression::getCompression()->Compress(compData + 8, &compLength,
                                                pvSaveMem, fileSi#endif
        QueryPerformanceCounter(&qwNewTime);

        qwDeltaTime.QuadPart = qwNewTime.QuadPart - qwTime.QuadPart;
        fElapsedTime = fSecsPerTick * ((FLOAT)(qwDeltaTime.QuadPart));

        app.DebugPr"Compress: Elapsed time %f\n", fElapsedTime);
        PIXEndNamedEvent();
        ...
        app.DebugPr"Save data compressed from %d to %d\n", fileSize,
                        compLeng#endif
        ...
        DWORD dwDataSizeSaveImage
#if (defined _XBOX || defined _DURANGO)
        app.GetSaveThumbnail(                                             \
        &pbThumbnailData,                                                 \
        &dwThumbnailDataSi #elif (defined __PS3__ || defined __ORBIS__ || \
                                  defined __PSVITA__)
        app.GetSaveThumbnail(&pbThumbnailData, &dwThumbnailDataSize,
                             &pbDataSaveImage, &dwDataSizeSaveIma#endif
        ...
        TelemetryManager->RecordLevelSaveOrCheckpoint(
            ProfileManager.GetPrimaryPad(), saveOrCheckpointId, compLength +
#ifdef _XBOX
        StorageManager.SaveSaveData(compLength + 8, pbThumbnailData,
                                    dwThumbnailDataSize, bTextMetadata,
                                    iTextMetadataBytes);
        delete[] pbThumbnailD#ifndef _CONTENT_PACKAGE
        if (app.DebugSettingsOn()) {
                                    if (app.GetWriteSavesToFolderEnabled()) {
                                        DebugFlushToFile(compData,
                                                         compLength + 8);
                                    }
#endif 
    } else {
                                    // We have failed to allocate the memory
                                    // required to save this file. Now
                                    // what?
    }

    ReleaseSaveAcces#elif (defined __PS3__ || defined __ORBIS__ || defined __PSVITA__ || \
   ...
   // set the icon and save image
        ...
        app.DebugPr"Save thumbnail size %d\n", dwThumbnailDataSize);

   // save the data
        StorageManager.SaveSaveData(
            &ConsoleSaveFileOriginal::SaveSaveDataCallback, th#ifndef _CONTENT_PACKAGE
        if (app.DebugSettingsOn()) {
                                    if (app.GetWriteSavesToFolderEnabled()) {
                                        DebugFlushToFile(compData,
                                                         compLength + 8);
                    ...
                    ReleaseSaveAcces #endif
#if (defined __PS3__ || defined __ORBIS__ || defined __PSVITA__ || \
         defined _DURANGO ||
     defined _WINDOWS64)

                        int ConsoleSaveFileOriginal::SaveSaveDataCallback(
...
void ConsoleSaveFileOriginal::DebugFlushToFile(
    void* compressed/*= NULL*/, unsigned int compressedData/*= 0*/) {
        LockSaveAccess();
        ...
        DWORD numberOfBytesWritten #ifdef _XBOX
    File targetFile "GAME:\\Saves"#else 
    File targetFile "Saves"#endif  // _XBOX
        ...
        SYSTEMTIME t;
        GetSystemTime(&t);
        // 14 chars for the digits// 11 chars for the separators +
        // suffix// 25 chars total
        std::wstring cutFileName = m_fileName;
        if (m_fileName.length() > XCONTENT_MAX_FILENAME_LENGTH - 25) {
            cutFileName =
                m_fileName.substr(0, XCONTENT_MAX_FILENAME_LENGTH - 25);
        }
    swprintf(fileName, XCONTENT_MAX_FILENAME_LENGTH + 1,
    "\\v%04d-%ls%02d.%02d.%02d.%02d.%02d.mcs", VER_PRODUCTBUILD,
             cutFileName.c_str(), t.wMonth, t.wDay, t.wHour, t.wMinute,
             t.
#ifdef _UNICODE
    std::wstring wtemp = targetFileDir.getPath() + std::wstring(fileName);
    LPCWSTR lpFileNa#elsewtemp.c_str();

    LPCSTR lpFileName =
        wstringtofilename(targetFileDir.getPath() + #endifs#ifndef __PSVITA__

    HANDLE hSaveFile = CreateFile(lpFileName, GENERIC_WRITE, 0, NULL,
                                  OPEN_ALWAYS, FILE_FL#endifDOM_ACCESS, NULL);


    if (compressedData != NULL && c#ifdef __PSVITA__ > 0) {
                                            // AP - Use the access function to save
                                            VirtualWriteFile(
                                                lpFileName, compressedData,
                                                compressedDataSize,
                                                &numb #elseytesWritten, NULL);
                                        
        WriteFile(hSaveFile, compressedData, compressedDataSize,
                                            &numb #endiftesWritten, NULL);

        assert(numberOfBytesWritten == compresse#ifdef __PSVITA__ else {
                                                // AP - Use the access function to save
                                                VirtualWriteFile(
                                                    lpFileName, compressedData,
                                                    compressedDataSize,
                                                    &numb #elseytesWritten,
                                                    NULL);
                                            
        WriteFile(hSaveFile, pvSaveMem, fileSize, &numb #endiftesWritten, NULL);

        assert(numberOfBytesWri#ifndef __PSVITA__
    }

#endifHandle(hSaveFile);
...
std::vector<FileEntry*>* ConsoleSaveFileOriginal::getFilesWithPrefix(
    const std::wstring& prefix) {
                                        return header.getFilesWithPrefix(
                                            prefix);
}
...
std::wstring ConsoleSaveFileOriginal::getPlayerDataFilenameForLoad(
    const PlayerUID& pUID) {
                                        return header
                                            .getPlayerDataFilenameForLoad(pUID);
}
std::wstring ConsoleSaveFileOriginal::getPlayerDataFilenameForSave(
    const PlayerUID& pUID) {
                                        return header
                                            .getPlayerDataFilenameForSave(pUID);
}
std::vector<FileEntry*>* ConsoleSaveFileOriginal::getValidPlayerDatFiles(#endif  return header.getValidPlayerDatFiles();
    }
...
int ConsoleSaveFileOriginal::getSaveVersion() {
    ...
    void ConsoleSaveFileOriginal::ConvertRegionFile(File sourceFile) {
        ...
        // saves all the endian swapped offsets back out
        // to// the file (not all of these are
        // written in the  // above processing).
        // 
    }

    void ConsoleSaveFileOriginal::ConvertToLocalPlatform() {
    if (getSave// already in the correct format_LOCAL) {
        // convert each of the region files to the local platform
    std::vector<Fil""ntry*>* allFilesInSave = getFilesWithPrefix(std::wstring(L));
...
"Processing a region file: %ls\n" == 0) {
                                            app.DebugPrintf(,
                                                            fName.c_str());
            ConvertRegionFile(F"%ls is not a region file, ignoring\n"e {
            app.DebugPrintf(,
                // set the platform of this save to the local platform,alPlatform();  // now that it's been coverted
                         
}

void* ConsoleSaveFileOriginal::getWritePointer(FileEntry* file) {
                                                return (char*)pvSaveMem +
                                                       file->currentFilePointer;
                                                ;
}
