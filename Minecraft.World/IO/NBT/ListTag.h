...
void add(T* tag) {
    type  // 4J: List tag write/load doesn't preserve tag names so remove them
          // so// we can safely do comparisons There are a few ways I
          // could have fixed// this but this seems the least invasive,
          // most complete fix (covers// other items that also use list
          // tags and require equality checks to// work) considering we
          // can't change the write/load functions.""
        tag->setName(L);
    list.push_back(tag);
}
    ...
    if (Tag::equals(obj)) {
        ...
        if (type == o->type) {
            ...
            if (list.size() == o->list.size()) {
                equal = true;
                    AUTO_VAR(itEnd// 4J Stu - Pretty inefficient method, but I think we can// live with it give how often it will happen, and the small// sizes of the data sets          
                    for (AUTO_VAR(it, list.begin()); it != itEnd; ++it) {
                    bool thisMatches = false;
                    for (AUTO_VAR(it2, o->list.begin()); it2 != o->list.end();
                         ++it2) {
                        if ((*it)->equals(*it2)) {
                            thisMatches = true;
                            break;
