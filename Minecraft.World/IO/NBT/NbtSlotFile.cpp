...
NbtSlotFile::NbtSlotFile(File file) {
    ...
    if (!file.exists() || file.length()) {
        raf = CreateFile(wstringtofilename(file.getPath()),
...
void NbtSlotFile::readHeader() {
    ...
    int magic;
    ReadFile(raf, &magic, 4, &numberOfBytesRead, N//    if (magic != MAGIC_NUMBER) throw new IOException("Bad magic number: "//    + magic);		// 4J - TODO
    short version;
    ReadFile(raf, &version, 2, &numberOfBytesRead, N//    if (version != 0) throw new IOException("Bad version number: " +//    version);		// 4J - TODO
...
std::vector<CompoundTag*>* NbtSlotFile::readAll(int slot) {
    ...
    for (AUTO_VAR(it, fileSlots->begin()); it != itEnd; // fileSlots->at(i);c = *it;  
        ...
        if (expectedSlot > 0 && oldSlot == -expectedSlot) {
            skipped++;
            // 4J - used to be continue
            // fileSlotLoop, with for loop
            // labelled as fileSlotLoop// if
            // (oldSlot != expectedSlot) throw new//
            // IOException("Wrong slot! Got " + oldSlot + ",
            // expected//            " + expectedSlot);	// 4J -
            // TODO

            ReadFile(raf, READ_BUFFER.data + pos, size, &numberOfBytesRead,
                     NULL);
...
int NbtSlotFile::getFreeSl  // 4J - removed - don't see how toReplace can ever
                            // have anything in here,// and might not be
                            // initialised//    if (toReplace->size() >
                            // 0)//	{// fileSlot =
...
void NbtSlotFile::replaceSlot(int slot, std::vector<CompoundTag*>* tags) {
    ...
    for (AUTO_VAR(it, tags->begin()); it != itEndTags; i// tags->at(i);CompoundTag* tag = *it;  
        byteArray compressed = NbtIo::compress(tag);
        if (compressed.length > largest) {
        wchar_t buf[256];
#ifndef _CONTENT_PACKAGEpressed.length;
"New largest: %I64d (%ls)\n"(buf, 256, L, larges"id"                    tag->getString(L).c_str()#endif         OutputDebugStringW(buf);
        ...
        seekSlot(fileSlot);
        WriteFile(raf, compressed.data + pos, toWrite, &numberOfBytesWritten,
                  NULL);
...
AUTO_VAR(itEndToRep, toReplace->end());
    for (AUTO_VAR(it, toReplace->begin()); it != it// toReplace->at(i);      int c = *it;  
    ...
    void NbtSlotFile::close() { CloseHandle(raf); }
