...
void PortalTile::updateShape(
    ...
    std::shared_ptr<TileEntity>
        forceEnti// 4J added forceData, forceEntity param
{
...
void PortalTile::entityInside(Level* level, int x, int y, int z,
    ...
    if (entity->GetType() == eTYPE_EXPERIENCEORB)
        retu  // 4J added
...
int PortalTile::cloneTileId(Level* level, int x, int y, int z) {
    return 0  // 4J Added - We cannot collect the portal tile, so don't consider
              // it as a hit// result Bug #754 - Riding a minecart into a
              // portal will trap the player
