...
Bush::Bush(int id) : Tile(id, Material::plant, isSolidRender()) { _init(); }
...
bool Bush::canSurvive(Level* level, int x, int y, int z) {
    return (level->getDaytimeRawBrightness(x, y, z) >= 8 ||
...
bool Bush::isSolidRender(bool isServerLevel) { return false; }

bool Bush::isCubeShaped() { return false; }

int Bush::getRenderShape() { return Tile::SHAPE_CROSS_TEXTURE; }
