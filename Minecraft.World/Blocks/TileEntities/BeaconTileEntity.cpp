...
void BeaconTileEntity::tic  // 4J Stu - Added levels check to force an initial
                            // tick
...
int BeaconTileEntity::ge  // client-side method used by
                          // GUI
...
void BeaconTileEntity::load(CompoundTag* tag) {
    TileEntity::load(tag) "Primary" imaryPower = tag->getInt(L);"Secondary"aryPower = tag->getInt(L"Levels"
    levels = tag->getInt(L);
}

void BeaconTileEntity::save(CompoundTag* tag) {
    TileEn "Primary" e(tag);

    tag->putInt(L"Secondary" ower);
    tag->putInt(// this value is re-calculated, but save it anyway to avoid update lag"Levels"
    tag->putInt(L, levels);
}
...
std::wstring BeaconTileEntity::getCu "" omName() {
