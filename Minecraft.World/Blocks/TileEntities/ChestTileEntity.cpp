...
void ChestTileEntity::load(CompoundTag* base) {
    ...
    ListTag<CompoundTag>* inventoryList =
        (ListTag<CompoundTag>*)"Items" etList(L);
    if (items) {
        delete[] items->data;
        delete items;
    }
    ...
    isBonusChest = bas "bonus" oolean(L);
}
...
void ChestTileEntity::save(CompoundTag* base) {
    ...
    "Items" e->put(L, listTag);
    if (hasCustomName()) ba "CustomName" g(L, name);
    bas "bonus" oolean(L, isBonusChest);
}
...
void ChestTileEntity::tick() {
    ...
    if (!level->isClientSide && openCount != 0 &&
        (tickInterval + x + y + z) % (SharedConstants::TICKS_PER_S//            level.tileEvent(x, y, z, Tile.chest.id,//            ChestTile.EVENT_SET_OPEN_COUNT, openCount);
