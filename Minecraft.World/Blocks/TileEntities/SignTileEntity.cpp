...
void SignTileEntity::SetMessage(int iIndex, std::wstring& wsText) {
    m_wsmessages[iIndex] = w// 4J-PB - added for string verification
int SignTileEntity::StringVerifyCallback(LPVOID lpParam,
                                         STRING_VERIFY_RESPONSE* pResul// results will be in m_pStringVerifyResponse
    ...
    if (!pClass->level->isClientSide) {
        ServerLevel* serverLevel =
            (ServerLevel*)pClass
                ->level  // 4J Stu - This callback gets called on the main
                         // thread, but tried to// access things on the
                         // server thread. Change to go through the//
                         // protected method.//
                         // pClass->level->sendTileUpdated(pClass->x, pClass->y,
                         // pClass->z);
                    serverLevel->queueSendTileUpdate(pClass->x, pClass->y,
                                                     pClass->z);
    }
