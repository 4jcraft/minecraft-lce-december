...
void PistonPieceEntity::load(CompoundTag* tag) {
    ...
    "blockId" g->getInt(L);
    "blockData" > getInt(L);
    fa "facing" ag->getInt(L);
    progressO = progre "progress" getFloat(L);
    extending "extending" Boolean(L);
}

void PistonPieceEntity::save(CompoundTag* tag) {
    TileEntity::save(tag)"blockId"g->putInt(L, id"blockData">putInt(L, data"facing"ag->putInt(L, facing);"progress"putFloat(L, progressO);
