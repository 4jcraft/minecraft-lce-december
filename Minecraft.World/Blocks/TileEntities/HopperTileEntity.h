...
public:
	...
	static std::shared_ptr<ItemInstance> addItem(Container *container, std::shared_ptr<ItemInstance> item, int face);

private:
	static bool canPlaceItemInContainer(Container *container, std::shared_ptr<ItemInstance> item, int slot, int face);
	static bool canTakeItemFromContainer(Container *container, std::shared_ptr<ItemInstance> item, int slot, int face);
	static std::shared_ptr<ItemInstance> tryMoveInItem(Container *container, std::shared_ptr<ItemInstance> item, int slot, int face);
	virtual std::shared_ptr<Container> getAttachedContainer();

public:
	static std::shared_ptr<Container> getSourceContainer(Hopper *hopper);
	static std::shared_ptr<ItemEntity> getItemAt(Level *level, double xt, double yt, double zt);
	static std::shared_ptr<Container> getContainerAt(Level *level, double x, double y, double z);

private:
	static bool canMergeItems(std::shared_ptr<ItemInstance> a, std::shared_ptr<ItemInstance> b);
