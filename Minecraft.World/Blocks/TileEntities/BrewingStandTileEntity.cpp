    ...
    if (PotionBrewing::SIMPLIFIED_BREWING) {
        ...
        for (int dest = 0; dest < 3; dest++) {
            if (items[dest] != NULL && items[dest]->id == Item::potion_Id) {
                ...
                // "currentEffects.equals(newEffects)" in the
                // java.
                // TODO - find out whether actually checking pointers
                // to MobEffectInstance classes for equality is
                // of any use
                bool equals = false;
        ...
        bool isWater = ingredient->id ==
                       Item::bucket_wate  // at least one destination potion
                                          // must have a result
                       bool oneResult = false;
        for (int dest = 0; dest < 3; dest++) {
            if (items[dest] != NULL && items[dest]->id == Item::potion_Id) {
    ...
    if (PotionBrewing::SIMPLIFIED_BREWING) {
        for (int dest = 0; dest < 3; dest++) {
            if (items[dest] != NULL && items[dest]->id == Item::potion_Id) {
                ...
                std::vector<MobEffectInstance*>* newEffects =
                    Item::potion
                        ->getMobEffec  // 4J - this code replaces an
                                       // expression//
                                       // "currentEffects.equals(newEffects)" in
                                       // the java.// TODO -
                                       // find out whether actually checking
                                       // pointers to//
                                       // MobEffectInstance classes for equality
                                       // is of any
                                       // use
                    bool equals = false;
                if ((currentEffects != NULL) && (newEffects != NULL)) {
...
int BrewingStandTileEntity::applyIngredient(
    ...
    if
        ...
        ED_BREWIN  // 4J Stu - SIMPLIFIED_BREWING is on, so we never use
                   // this
...
if (Item::items[ingredient->id]->hasPotionBrewingFormula()) {
    return PotionBrewing::applyBrew(
        currentBrew, Item::items[ingredient->id]->getPotionBrewingFormula());
}
return currentBrew;
}
...
void BrewingStandTileEntity::load(CompoundTag* base) {
    ...
    ListTag<CompoundTag>* inventoryList =
      "Items"Tag<CompoundTag>*)base->getList(L);
    delete[] items.data;
    ...
    void BrewingStandTileEntity::save(CompoundTag * base) {
        TileE "BrewTime" e(base);

        base->putShort(L, (short)(brewTime));
        ...
        base->put(L, listTag);
  "CustomName"tomName()) base->putString(L, name);
    }
...
// 4J Stu - Changed the implementation of this function to be the same as// ChestTileEntity to enable the "Pickup Half" option on the ingredients// slot Fix for #65373 - TU8: Content: UI: Command "Take Half" in the// Brewing Stand interface doesn't work as intended.
...
bool BrewingStandTileEntity::canTakeItemThroughFace(
    int slot, std::shared_p// 4J Addedance> item, int face) {
    return true;
}
