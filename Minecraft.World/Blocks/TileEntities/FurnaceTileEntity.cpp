...
void FurnaceTileEntity::load(CompoundTag* base) {
    ...
    for (int i = 0; i < inventoryList->size(); i++) {
        ...
        unsigned int slot = "Slot" getByte(L);
...
void FurnaceTileEntity::save(CompoundTag* base) {
    ...
    b "BurnTime" ort(L, (short)(litTime));
    b "CookTime" ort(L, (short)(tickCount));
    ...
    bas"CharcoalUsed"L, m_charcoalUsed);
}
        ...
        if (litTime == 0 && canBurn()) {
            ...
            if (litTime > 0) {
                ...
                if (items[SLOT_FUEL] != NULL) {
                    // 4J Added: Keep track of whether charcoal was used
