...
// thread safe
bool PistonBaseTile::ignoreUpdate() { return (TlsGetValue(tlsIdx) != NULL); }

void PistonBaseTile::ignoreUpdate(bool std::set) {
    TlsSetValue(tlsIdx, (LPVOID)(set ? 1 : 0));
}

PistonBaseTile::PistonBaseTile(int id, bool isSticky)
    : Tile(id, Material::piston,
           isSolidRender()) {  // 4J - added initialiser
    ignoreUpdate(false);
...
bool PistonBaseTile::use(Level* level, int x, int y, int z,
                         ...
                         bool sound/*=false*/// 4J added soundOnly param
{
        return false;
}
...
void PistonBaseTile::checkIfExtend(Level* level, int x, int y, int z) {
        ...
        if (extend && !isExtended(data)) {
            ...
            /**
             * This method checks neighbor signals for this block and the
             * block above, and directly beneath. However, it
             * avoids checking blocks that would be pushed by
             * this block.
             *
             * @param level
             * @param x
            ...
            bool PistonBaseTile::getNeighborSignal(
                ...
                if (facing != Facing::DOWN &&
                    ...
                    return true;
                if (facing != Facing::UP &&
                    level->hasSignal(x, y + 1, z, Facing::UP))
                    return true;
                if (facing != Facing::NORTH &&
                    level->hasSignal(x, y, z - 1, Facing::NORTH))
                    return true;
                if (facing != Facing::SOUTH &&
                    level->hasSignal(x, y, z + 1, Facing::SOUTH))
                    return true;
                if (facing != Facing::EAST &&
                    level->hasSignal(x + 1, y, z, Facing::EAST))
                    return true;
                if (facing != Facing::WEST &&
                    level->hasSignal(x - 1, y, z, Facing::WEST))
                    return true;
                // check signals above
            ...
            bool PistonBaseTile::triggerEvent(Level * level, int x, int y,
                                              int z, int param1, int facing) {
                ignoreUpdate(true);
        ...
        PIXBeginNamedEven"Create push\n");
        if (createPush(level, x, y, z, facing)) {
            // 4J - it is (currently) critical that this setData sends data
            // to the client, so have added a bool to the method so that it
            // sends data even if the data was already set to the same
            // value as before, which was actually its behaviour until a
            // change in 1.0.1 meant that setData only conditionally sent
            // updates to listeners. If the data update Isn't sent, then
            // what can happen is: (1) the host sends the tile event to the
            // client (2) the client gets the tile event, and sets the
            // tile/data value locally. (3) just before setting the
            // tile/data locally, the client will put the old value in the
            // vector of things to be restored should an update not be
            // received back from the host (4) we don't get any update of
            // the tile from the host, and so the old value gets restored
            // on the client (5) the piston base ends up being restored to
            // its retracted state whilst the piston arm is extended We
            // really need to spend some time investigating a better way
            // for pistons to work as it all seems a bit scary how the
            // host/client interact, but forcing this to send should at
            // least restore the behaviour of the pistons to something
            // closer to what they were before the 1.0.1 update. By sending
            // this data update, then (4) in the list above doesn't happen
            // because the client does actually receive an update for this
            // tile from the host after the event has been processed on the
            // cient.
        ...
        PIXBeginNamedEven"Contract phase A\n");
        ...
        stopSharingIfServer(level, x// 4J added
        ...
        PIXEndNamedEvent();// sticky movement
        if (isSticky) {
            PIXBeginName"Contract sticky phase A\n");
            ...
            if (block == Tile::pistonMovingPiece_Id) {
                PIXBeginName"Contract sticky phase B\n");
                // the block two steps away is a moving piston block piece,
                // so replace it with the real data, since it's
                // probably this piston which is changing too
                // fast
            ...
            if (!pistonPiece && block > 0 &&
                ...
                stopSharingIfServer(level, // 4J added twoZ);  
...
{
                int data =
                    (forceData == -1) ? level->getData(x, y, z) : forceData;
...
int PistonBaseTile::getNewFacing(Level* level, int x, int y, int z,
    ...
    if (Mth::abs((float)player->x - x) < 2 &&
        Mth::abs((float)player->z // If the player is above the block, the slot is on the top
        double py = player->y + 1.82 - player->heightOffset;
        if (py - y > 2) {
                    return Facing::UP  // If the player is below the block, the
                                       // slot is on the
                                       // bottom
                        if (y - py > 0) {
                        return Facing::DOWN;
                        // The slot is on the side
                        int i = Mth::floor(player->yRot * 4.0f / 360.0f + 0.5) &
                                0x3;
...
bool PistonBaseTile::isPushable(int block, Level* level, int cx, int cy, int cz,
                                bool allo// special case for obsidian
    if (block == Tile::obsidian_Id) {
                        return false;
    }

    if (block == Tile::pistonBase_Id || block == Tile::pistonStick// special case for piston bases
        if (isExtended(level->getData(cx, cy, cz))) {
                        return false;
        }
    } else {
                    if (Tile::tiles[block]->getDestroySpeed(level, cx, cy,
                                                            cz) ==
    ...
    if (Tile::tiles[block]
           // 4J - java uses instanceof EntityTile here// may not push tile entities
        return false;
    }
    ...
    return true;
        ...
        bool PistonBaseTile::canPush(Level * level, int sx, int sy, int sz,
                                     int facing) {
        ...
        if (cy <= 0 || cy >= (Level::maxBuildHeight // out of bounds    
            return false;// 4J - added to also check for out of bounds in x/z for our finite// world
        ...
        if (i == MAX_PUSH_D// we've reached the maximum push depth// without finding air or a breakable block
            return false;
            }
...
bool PistonBaseTile::createPush(Level* level, int sx, int sy, int sz,
        ...
        if (Tile::tiles[block]->getPistonPushReaction() ==
            Material::PUSH_DES// this block is destroyed when pushed
            Tile::tiles[block]->spawnResources(level, cx, cy, cz,
                                               level->getData(cx, cy, c// setting the tile to air is actually superflous, but helps vs// multiplayer problems
            stopSharingIfServer(l// 4J addedy, cz);  
