...
void RedStoneOreTile::stepOn(Level* level, int x, int y, int z,
...
int RedStoneOreTile::getResource(int data, Random* random,
                                 int playerBonusLevel) {
    return Item::redStone->id;
}

int RedStoneOreTile::getResourceCountForLootBonus(int bonusLevel,
                                                  Random* random) {
...
void RedStoneOreTile::spawnResources(Level* level, int x, int y, int z,
                                     int data, float odds,
                                     int playerBonusLevel) {
...
void RedStoneOreTile::animateTick(Level* level, int x, int y, int z,
                                  Random* random) {
    if (lit) {
        poofParticles(level, x, y, z);
    }
...
void RedStoneOreTile::poofParticles(Level* level, int x, int y, int z) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (xx < x || xx > x + 1 || yy < 0 || yy > y + 1 || zz < z ||
            zz > z + 1) {
