...
bool EggTile::use(Level* level, int x, int y, int z,
                  ...
                  bool /*=false*/// 4J added soundOnly param
{
...
void EggTile::teleport(Level* level, int x, int y, int z) {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        if (level->getTile(xt, yt, zt) == 0) {
            // Fix for TU9: Content: Art: Dragon egg teleport particle
            // effect isn't present. Don't set tiles on client, and
            // don't create particles on the server (matches later
            // change in Java)
            if (!level->isClientSide) {
                ...
                // 4J Stu - The PC version is wrong as the particles
                // calculated on the client side will point towards a
                // different location to the one where the egg has
                // actually moved. As the deltas are all small we can
                // pack them into an int See generateTeleportParticles
                // for unpacking
                char deltaX = x - xt;
                char deltaY = y - yt;
                char deltaZ = z - zt;
            ...
            // float za = (level->random->nextFloat() - 0.5f) *
            // 0.2f;

            //			double _x = xt + (x - xt) * d +
            //(level->random->nextDouble() - 0.5) * 1 + 0.5f;
            // double _y = yt + (y - yt) * d +
            // level->random->nextDouble() * 1 - 0.5f; 			double
...
int EggTile::cloneTileId(Level* level, int x, int y, int z) {
    ret  // 4J Added for Fix for #77475 - TU9: Content: Art: Dragon egg teleport
         // particle// effect isn't present.
        void EggTile::generateTeleportParticles(Level * level, int xt, int yt,
                                                int zt, int deltas) {
        int count =  // See above for packing
        ...
        for (int j = 0; j < count; j++) {
            ...
            double _x = xt + deltaX * d +
                        (level->random->nextDouble() - 0.5) * 1 + 0.5f;
            double _y =
                yt + deltaY * d + level->random->nextDouble() * 1 - 0.5f;
            double _z = zt + deltaZ * d +
                        (level->random->nextDouble() - 0.5) * 1 + 0.5f;
