...
TripWireTile::TripWireTile(int id)
    : Tile(id, Material::decoration, isSolidRender()) {
...
bool TripWireTile::isSolidRender(bool isServerLevel) { return false; }

bool TripWireTile::isCubeShaped() { return false; }

int TripWireTile::getRenderLayer() { return 1; }

int TripWireTile::getRenderShape() { return Tile::SHAPE_TRIPWIRE; }
...
void TripWireTile::neighborChanged(Level* level, int x, int y, int z,
                                   int type) {
...
void TripWireTile::updateShape(LevelSource* level, int x, int y, int z,
                               int forceData,
...
void TripWireTile::onRemove(Level* level, int x, int y, int z, int id,
                            int data) {
    updateSource(level, x, y, z, data | MASK_POWERED);
}

void TripWireTile::playerWillDestroy(Level* level, int x, int y, int z,
                                     int data, std::shared_ptr<Player> player) {
    ...
    for (int dir = 0; dir < 2; dir++) {
        for (int i = 1; i < TripWireSourceTile::WIRE_DIST_MAX; i++) {
            ...
            if (tile == Tile::tripWireSource_Id) {
                int sourceDir =
                    level->getData(xx, y, zz) & TripWireSourceTile::MASK_DIR;

                if (sourceDir == Direction::DIRECTION_OPPOSITE[dir]) {
                    Tile::tripWireSource->calculateState(
                        level, xx, y, zz, tile, level->getData(xx, y, zz), true,
                        i, data);
                }
...
void TripWireTile::entityInside(Level* level, int x, int y, int z,
...
void TripWireTile::checkPressed(Level* level, int x, int y, int z) {
    ...
    vector<shared_ptr<Entity> >* entities = level->getEntities(
...
bool TripWireTile::shouldConnectTo(LevelSource* level, int x, int y, int z,
                                   int data, int dir) {
