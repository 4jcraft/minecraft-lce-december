...
ChestTile::ChestTile(int id, int type)
    : BaseEntityTile(id, Material::wood, isSolidRender()) {
    random = new Random();
    this->type = type;
...
ChestTile::~ChestTile() { delete random; }

bool ChestTile::isSolidRender(bool isServerLevel) { return false; }

bool ChestTile::isCubeShaped() { return false; }

int ChestTile::getRenderShape() { return Tile::SHAPE_ENTITYTILE_ANIMATED; }

void ChestTile::updateShape(LevelSource* level, int x, int y, int z,
                            int forceData,
...
void ChestTile::recalcLockDir(Level* level, int x, int y, int z) {
    ...
    if (n == id || s == id) {
        ...
        int otherDir = -1;
        if (n == id)
            otherDir = level->getData(x, y, z - 1);
        else
            otherDir = level->getData(x, y, z + 1);
        if (otherDir == 4) lockDir = 4;

        if ((Tile::solid[w] || Tile::solid[w2]) && !Tile::solid[e] &&
            !Tile::solid[e2])
            lockDir = 5;
        if ((Tile::solid[e] || Tile::solid[e2]) && !Tile::solid[w] &&
            !Tile::solid[w2])
            lockDir = 4;
    } else if (w == id || e == id) {
        ...
        int otherDir = -1;
        if (w == id)
            otherDir = level->getData(x - 1, y, z);
        else
            otherDir = level->getData(x + 1, y, z);
        if (otherDir == 2) lockDir = 2;

        if ((Tile::solid[n] || Tile::solid[n2]) && !Tile::solid[s] &&
            !Tile::solid[s2])
            lockDir = 3;
        if ((Tile::solid[s] || Tile::solid[s2]) && !Tile::solid[n] &&
            !Tile::solid[n2])
            lockDir = 2;
    } else {
        lockDir = 3;
...
std::shared_ptr<Container> ChestTile::getContainer(Level* level, int x, int y,
            ...
            if (level->getTile(x - 1, y, z) == id)
                container = std::shared_ptr<Container>(new CompoundContainer(
                    IDS_CHEST_LARGE,
                    dynamic_pointer_cast<ChestTileEntity>(
...
int ChestTile::getSignal(LevelSource* level, int x, int y, int z, int dir) {
            ...
            int openCount = dynamic_pointer_cast<ChestTileEntity>(
                                level->getTileEntity(x, y, z))
                                ->openCount;
            return Mth::clamp(openCount, Redstone::SIGNAL_NONE,
                              Redstone::SIGNAL_MAX);
}
...
bool ChestTile::isCatSittingOnChest(Level* level, int x, int y, int z) {
            std::vector<std::shared_ptr<Entity> >* entities =
                level->getEntitiesOfClass(
                    typeid(Ocelot),
                    AABB::newTemp(x, y + 1, z, x + 1, y + 2, z + 1));
            for (AUTO_VAR(it, entities->begin()); it != entities->end(); ++it) {
                std::shared_ptr<Ocelot> ocelot =
                    dynamic_pointer_cast<Ocelot>(*it);
                if (ocelot->isSitting()) {
                    delete entities;
                    return true;
                }
