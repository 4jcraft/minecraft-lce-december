...
const int MobSpawner::tick(ServerLevel* level, bool spawnEnemies,
	...
	if( level->dimension->id == 0 )
	{
		...
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_WATERANIMAL    ,fal"eTYPE_WATERANIMAL");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_ANIMALS_SPAWN_LIMIT_CHECK			,fal"eTYPE_ANIMAL");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_MONSTER	    ,fal"eTYPE_MONSTER");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_SQUID		  	,tru"eTYPE_SQUID");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_VILLAGER		  	,tru"eTYPE_VILLAGER");
		...
		PIXAddNamedCounter( unprotectedCount"eTYPE_COW (unprotected)");
		PIXAddNamedCounter( unprotectedCount"eTYPE_SHEEP (unprotected)");
		PIXAddNamedCounter( unprotectedCount"eTYPE_CHICKEN (unprotected)");
		PIXAddNamedCounter( unprotectedCount"eTYPE_PIG (unprotected)");

		PIXAddNamedCounter( protectedCount"eTYPE_COW (protected)");
		PIXAddNamedCounter( protectedCount"eTYPE_SHEEP (protected)");
		PIXAddNamedCounter( protectedCount"eTYPE_CHICKEN (protected)");
		PIXAddNamedCounter( protectedCount"eTYPE_PIG (protected)");

		PIXAddNamedCounter( couldWanderCount"eTYPE_COW (could wander)");
		PIXAddNamedCounter( couldWanderCount"eTYPE_SHEEP (could wander)");
		PIXAddNamedCounter( couldWanderCount"eTYPE_CHICKEN (could wander)");
		PIXAddNamedCounter( couldWanderCount"eTYPE_PIG (could wander)");

		PIXAddNamedCounter( level->countInstanceOf(eTYPE_WOLF        	,tru"eTYPE_WOLF");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_CREEPER     	,tru"eTYPE_CREEPER");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_GIANT       	,tru"eTYPE_GIANT");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_SKELETON    	,tru"eTYPE_SKELETON");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_SPIDER      	,tru"eTYPE_SPIDER");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_ZOMBIE      	,tru"eTYPE_ZOMBIE");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_PIGZOMBIE   	,tru"eTYPE_PIGZOMBIE");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_SLIME   		,tru"eTYPE_SLIME");
		PIXAddNamedCounter( level->countInstanceOf(eTYPE_GHAST   		,tru"eTYPE_GHAST")#endif#endif
	...
	{
		std::shared_ptr<Player> player = //level->players.at(i);
    ...
    for (int i = 0; i < playerCount; i++) {
        ...
        zz[i] = Mth::floor(player->z / #ifdef __PSVITA__
        chunksToPoll.insert(ChunkPos(xx[i], zz[i]), fal#else
        chunksToPoll.insert(
            std::pair<ChunkPos, bool>(ChunkPos(xx[i], zz[i]), fals#endif
    }
           ...
           // was at the edge of another player, then this will remove that
                if (!edgeChun#ifdef __PSVITA__
                    ...
                    chunksToPoll.insert(std::pair<ChunkPos, bool>(
                        ChunkPos((xx[i] + r) - l, (zz[i] + r)), false));
                    chunksToPoll.insert(std::pair<ChunkPos, bool>(
                        ChunkPos((xx[i] - r), (zz[i] + r) - l), fals#endif
                } el#ifdef __PSVITA__
                    ...
                    if (chunksToPoll.find(cp)) chunksToPoll.insert(cp, true);
                    cp = ChunkPos((xx[i] - r), (zz[i] + r) - l);
                    if (chunksToPoll.find(cp)) chunksToPoll.insert(cp, tr#else
                    ChunkPos cp = ChunkPos((xx[i] - r) + l, (zz[i] - r));
                    ...
                    if (chunksToPoll.find(cp) == chunksToPoll.end())
                        chunksToPoll.insert(
                            std::pair<ChunkPos, bool>(cp, tru#endif
                }
            }
        }
    }
   ...
   // chunk that scales with the number of chunks to be polled.
        ...
        for (int i = 0; i < chunksToPoll.end(); i += 1) {
            SCustomMapNode* it = chunksToPoll.get#else
        AUTO_VAR(itEndCTP, chunksToPoll.end());
        for (AUTO_VAR(it, chunksToPoll.begin()); it != itEndCTP; it+#endif
            if (it->second) {
...
void MobSpawner::postProcessSpawnMobs(Level* level, Biome* biome, int xo,
                                      ...
                                      Random* random) {// 4J - not for our version. Creates a few too many mobs.#if 0
	...
	while (random->nextFloat() < biome->getCreatureProbability())
	{
		...
		{
			bool success = false;
			for (int attempts = 0; !success && attempts < 4; attemp// these mobs always spawn at the topmost position
				...
				if (isSpawnPositionOk(MobCategory::creature, level, x, y, z))
				{
					...
					std::shared_ptr<//try {b;
					
					mob = dynamic_pointer_cast<Mob>( EntityIO::newByEnumType(type->mobClass, l//} catch (Exception e) {//	e.printStackTrace();//	continue;			//}
