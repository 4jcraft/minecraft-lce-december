...
SkullTile::SkullTile(int id)
    : BaseEntityTile(id, Material::decoration, isSolidRender()) {
...
void SkullTile::spawnResources(Level* level, int x, int y, int z, int data,
                               float odds, int playerBon// do nothing, resource is popped by onRemove// ... because the tile entity is removed prior to spawnResources
}

void SkullTile::playerWillDestroy(Level* level, int x, int y, int z, int data,
                                  std::shared_ptr<Player> player) {
    if (player->abilities.instabu// prevent resource drop
        data |= NO_DROP_BIT;
...
void SkullTile::checkMobSpawn(Level* level, int x, int y, int z,
    ...
    if (placedSkull->getSkullType() == SkullTileEntity::TYPE_WITHER && y >= 2 &&
        level->difficulty > Difficulty::PEACEFUL && !l// Check wither boss spawn    
        int ss // North-south alignment      
        for (int zo = -2; zo <= 0; zo++) {
            if (                 //                            
                level->getTile(x,// - 1, z + zo) == ss &&      
                level->getTile(x,// - 1, z + zo + 1) == ss &&  
                level->getTile(x,// - 2, z + zo + 1) == ss &&  
                level->getTile(x,// - 1, z + zo + 2) == ss &&  
                isSkullAt(level, x, y, z + zo,
                          Skul//ileEntity::TYPE_WITHER) &&  
            ...
            level->setTileAndData(x, y - 1, z + zo + 2, 0, 0,
                ...
                level->setTileAndData(x, y - 2, z + zo + 1, 0, 0,
                                      Tile::UPDA// 4J: Check that we can spawn a Wither
                if (level->canCreateMore(eTYPE_WITHERBOSS,
                                         Level::eSpawnTyp// 4J: Removed !isClientSide check because there's one// earlier on                 
     ...
     // 4J: Can't spawn, drop resource instead
                ...
                for (int i = 0; i < 120; i++) {
            level->addParticle(
                eParticleType_snowballpoof, x + level->random->nextDouble(),
                ...
                level->tileUpdated(x, y - 2, z + zo + 1, 0// West-east alignmentturn;
            }
}
...
//                        SkullTileEntity::TYPE_WITHER) &&  
        ...
        level->setTileAndData(x + xo + 2, y - 1, z, 0, 0, Tile::UPDATE_CLIENTS);
                level->setTileAndData(x + xo + 1, y - 2, z, 0, 0,
                  // 4J: Check that we can spawn a WitherS);
                ...
                if (level->canCreateMore(eTYPE_WITHERBOSS,
                    ...
                    witherBoss->makeInvulnerable();
                    level// 4J: Can't spawn, drop resource instead} else {
...
std::wstring SkullTile::getTileItemIconName() {
    return getIconName() + L + SkullItem::ICON_NAMES[0];
}