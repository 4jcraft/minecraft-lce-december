...
PumpkinTile::PumpkinTile(int id, bool lit)
    : DirectionalTile(id, Material::vegetable, isSolidRender()) {
    iconTop = NULL;
    iconFace = NULL;
    setTicking(true);
    this->lit = lit;
    ...
    if (level->getTile(x, y - 1, z) == Tile::snow_Id &&
        ...
        if (!level->isClientSide) {
            ...
            if (level->canCreateMore(eTYPE_SNOWMAN, Level::eSpawnType_Egg)) {
                ...
                shared_ptr<SnowMan> snowMan =
                    shared_ptr<SnowMan>(new SnowMan(level));
        ...
        if (xArms || zArms) {
            if (!level->isClientSide) {
                ...
                if (level->canCreateMore(eTYPE_VILLAGERGOLEM,
                    ...
                    shared_ptr<VillagerGolem> villagerGolem =
