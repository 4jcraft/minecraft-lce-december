...
bool Mushroom::mayPlaceOn(int tile) { return Tile::solid[tile]; }
...
bool Mushroom::canSurvive(Level* level, int x, int y, int z) {
    ...
    return below == Tile::mycel_Id ||
           (level->getDaytimeRawBrightness(x, y, z) < 13 && mayPlaceOn(below));
}
