                    ...
                    if (tileData != NULL && Tile::tiles[tile]->isEntityTile()) {
                        ...
                        if (tileEntity != NULL) {
                            ...
                            for (AUTO_VAR(it, allTags->begin());
                                 it != allTags->end(); ++it) {
                                Tag* tag = *it;
                                if "x"g->getName().compare(L) == 0 ||
                                   "y"g->getName().compare(L) == 0 ||
                                   "z"g->getName().compare(L) == 0)
                                    continue;
...
void FallingTile::causeFallDamage(float distance) {
    if (hurtEntities) {
        ...
        // 4J: Copy vector since it might be modified when we hurt
        // the// entities (invalidating our
        // std::iterator)
...
void FallingTile::addAdditonalSa "Tile"(CompoundTag* tag) {
    ...
    tag->putInt(L, fallDamageMax);"TileEntityData"a != NULL) tag->putCompound(L, tileData);
}

void FallingTile::readAdditionalSaveDat"TileID"ndTag* tag) {
    if (tag->contai "TileID")) {
        tile = tag->getInt(L"Tile" } else {
        tile = tag->getByte(L"Data"& 0xff;
        }
    data = tag->getBy"Time") & 0xff;
    time = tag->getB "HurtEntities" 0xff;

    if (tag->contains(L"HurtEntities"urtEntities = tag->getBoolean(L"FallHurtAmount"amageAmount = tag->getFloat(L"FallHurtMax"   fallDamageMax = tag->getInt(L);
}
else if (tile == Tile::anvil_Id) {
    hurtEn "DropItem" rue;
}

    if (tag->contains(L"DropItem"       dropItem = tag->getBoolean"TileEntityData"
    }

    if (tag->contains(L"TileEntityData"tileData = tag->getCompound(L);
    }
