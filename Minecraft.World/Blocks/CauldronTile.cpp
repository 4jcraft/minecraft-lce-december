...
CauldronTile::CauldronTile(int id)
    : Tile(id, Material::metal, isSolidRender()) {
    iconInner = NULL;
    iconTop = NULL;
    iconBottom = NULL;
}
                       ...
                       bool soundOnly /*=false*/)  // 4J added soundOnly param
{
    ...
    if (item->id == Item::bucket_water_Id) {
        ...
        if (fillLevel > 0) {
            ...
            if (!player->inventory->add(potion)) {
                ...
                // 
                    ...
                    // 4J-PB - don't lose the water in creative
                    // mode
                    ...
                    level->setData(x, y, z, fillLevel - 1,
                                   Tile::UPDATE_CLIENTS);
                    level->updateNeighbourForOutputSignal(x, y, z, id);
                }
            } else if (fillLevel > 0) {
                ...
                if (armor &&
                    armor->getMaterial() == ArmorItem::ArmorMaterial::CLOTH) {
        ...
        int CauldronTile::getResource(int data, Random* random,
        ...
        int CauldronTile::getAnalogOutputSignal(Level * level, int x, int y,
                                                int z, int dir) {
            int data = level->getData(x, y, z);
