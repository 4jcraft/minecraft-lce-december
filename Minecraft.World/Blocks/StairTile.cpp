...
StairTile::StairTile(int id, Tile* base, int basedata)
    : Tile(id, base->material, isSolidRender()) {
    this->base = base;
...
bool StairTile::isSolidRender(bool isServerLevel) { return false; }

bool StairTile::isCubeShaped() { return false; }

int StairTile::getRenderShape() { return Tile::SHAPE_STAIRS; }
...
bool StairTile::setStepShape(LevelSource* level, int x, int y, int z) {
    ...
    if (dir == DIR_EAST) {
        ...
        if (isStairs(backTile) &&
            ((data & UPSIDEDOWN_BIT) == (backData & UPSIDEDOWN_BIT))) {
...
bool StairTile::setInnerPieceShape(LevelSource* level, int x, int y, int z) {
    ...
    if (dir == DIR_EAST) {
        ...
        if (isStairs(frontTile) &&
            ((data & UPSIDEDOWN_BIT) == (frontData & UPSIDEDOWN_BIT))) {
...
void StairTile::attack(Level* level, int x, int y, int z,
...
int StairTile::getRenderLayer() { return base->getRenderLayer(); }
...
void StairTile::handleEntityInside(Level* level, int x, int y, int z,
                                   std::shared_ptr<Entity> e, Vec3* current) {
    base->handleEntityInside(level, x, y, z, e, current);
}

bool StairTile::mayPick() { return base->mayPick(); }
...
void StairTile::stepOn(Level* level, int x, int y, int z,
...
HitResult* StairTile::clip(Level* level, int xt, int yt, int zt, Vec3* a,
                           Vec3* b) {
    HitResult* results[8];
