...
void GoalSelector::tick() {
    ...
    if (tickCount++ % newGoalRate == 0)  // for (InternalGoal ig : goals)
        for (AUTO_VAR(it, goals.begin()); it != goals.end(); ++it) {
            ...
            AUTO_VAR(usingIt, std::find(usingGoals.begin(), usingGoals.end(),
                                        ig))  // if (isUsing)
    ...
    for (AUTO_VAR(it, usingGoals.begin()); it != usingGoals.end();) {
        ...
        // bool debug = false;// if (debug && toStart.size() > 0)
        // System.out.println("Starting: ");// for (InternalGoal ig :
        // toStart)
    for (AUTO_VAR(it, toStart.begin()); it != toStart.end(); +// if (debug) System.out.println(ig.goal.toString() + ", ");
        (*it)->goal->start// if (debug && usingGoals.size() > 0) System.out.println("Running: ");// for (InternalGoal ig : usingGoals)
    for (AUTO_VAR(it, usingGoals.begin()); it != usingGoals.end(); +// if (debug) System.out.println(ig.goal.toString());
        (*it)->goal->tick();
    }
}
...
bool GoalSelector::canUseInSystem(GoalSelector::Intern// for (InternalGoal ig : goals)
    for (AUTO_VAR(it, goals.begin()); it != goals.end(); ++it) {
    InternalGoal* ig = *it;
    if (ig == goal) continue;
