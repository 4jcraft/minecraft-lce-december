...
bool MoveThroughVillageGoal::canUse() {
    ...
    Vec3* pos = RandomPos::getPosTowards(
        dynamic_pointer_cast<PathfinderMob>(mob->shared_from_this()), 10, 7,
...
std::shared_ptr<DoorInfo> MoveThroughVillageGoal::getNextDoorInfo(
    ...
    std::vector<std::shared_ptr<DoorInfo> >*
        d  // for (DoorInfo di : doorInfos)();
...
bool MoveThroughVillageGoal::ha  // for (DoorInfo di2 : visited)fo> di) {
