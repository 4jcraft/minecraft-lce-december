...
void Player::_init() {
    ...
    enderChestInventory =
        shared_ptr<PlayerEnderChestContainer>(new PlayerEnderChestContainer());
...
std::shared_ptr<ItemEntity> Player::drop(std::shared_ptr<ItemInstance> item,
    ...
    shared_ptr<ItemEntity> thrownItem = shared_ptr<ItemEntity>(
...
Player::BedSleepingResult Player::startSleepInBed(int x, int y, int z,
    ...
    if (!level->isClientSide || bTestUse) {
        ...
        if (!bTestUse) {
            ...
            double hRange = 8;
            double vRange = 5;
            vector<shared_ptr<Entity> >* monsters = level->getEntitiesOfClass(
                typeid(Monster),
...
void Player::SetAdditionalModelParts(
    vector<ModelPart*>* ppAdditionalModelParts) {
