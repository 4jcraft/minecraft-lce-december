...
Player::Player(Level* level, const std::wstring& name)
    : LivingEntity(
          lev  // 4J Stu - This function call had to be moved here from the
               // Entity ctor to// ensure that the derived version of the
               // function is called
          ...
          _init(); MemSect(11); inventoryMenu = new InventoryMenu(
                                    inventory, !level->isClientSide, this);
          MemSect(0);

          containerMenu = inventoryMenu;

          heightOffset = 1.62f; Pos* spawnPos = level->getSharedSpawnPos();
          moveTo(spawnPos->x + 0.5, spawnPos->y + 1, spawnPos->z + 0.5, 0, 0);
          delete spawnPos;

          rotOffs = 180; flameTime = 20;

          m_skinIndex = eDefaultSkins_Skin0; m_playerIndex = 0; m_dwSkinId = 0;
          m_dwCapeId  // 4J Added
              m_xuid = INVALID_XUID;
          m_OnlineXuid = INVALID_  // m_bShownOnMaps = true;
              setShowOnMaps(app.GetGameHostOption(eGameHostOption_Gamertags) !=
                                    0
                                ? true
                                : false);
          m_bIsGuest
...
Player::~Playe         // TODO 4J// printf("A player has been
                       // destroyed.\n");
    delete inventoryM  // 4J Stu - Fix for #10938 - CRASH - Game hardlocks when
                       // client has an open// chest and Xbox Guide while
                       // host exits without saving. If the container//
                       // menu is not the inventory menu, then the player has a
                       // menu open. These// get deleted when the xui scene
                       // is destroyed, so we can not delete it here// if(
                       // containerMenu != inventoryMenu ) delete
                       // containerMenu;
}
...
void Player::releaseUsingItem() {
    if (useItem != NULL) {
        useItem->releaseUsing(
            ...
            useItemDuration);  // 4J Stu - Fix for various bugs where an
                               // incorrect bow was displayed// when it
                               // broke (#70859,#93972,#93974)
        if (useItem->count == 0) {
            removeSelectedItem();
        }
    }
...
bool Player::isBlocking() {
    return isUsingItem() &&
           Item::items[useItem->id]->getUseAnimation(useItem) ==
...
void Player::updateFrameTick() {
    ...
    void Player::tick() {
    if (level->isClientS// 4J Stu - Server player calls this differently so that it only happens// once per simulation tick
        updateFrameTick();
    }
    ...
    if (!level->isClientSide) {
        foodData.tick(dynamic_pointer_cast<Player>(shared_from_this()// 4J Stu Debugging
    if (!level->isClientSide) {
            static int count = 0;
        if (cou#if 0=#ifdef _WINDOWS64// Drop some items so we have them in inventory to play with
			...
			this->drop( std::shared_ptr<ItemInstance>(new ItemI#endife
#ifdef __PS3___dia  // #ifdef _DEBUG// 		// Drop some items so we
                    // have them in inventory to play with//
                    // this->drop( shared_ptr<ItemInstance>( new
                    // ItemInstance(Tile::recordPlayer) ) );//
                    // this->drop( shared_ptr<ItemInstance>( new
                    // ItemInstance(Item::map) ) );//
                    // this->drop( shared_ptr<ItemInstance>( new
                    // ItemInstance(Item::record_01) ) );//
                    // this->drop( shared_ptr<ItemInstance>( new
                    // ItemInstance(Item::record_02) ) );//
                    // this->drop( shared_ptr<ItemInstance>(new ItemInstance(
                    // Item::pickAxe_diamond, 1 )) );// #endif#endif
#ifdef _DURANGO
    ...
    //            ItemInstance itemInstance = new  {
		...
		if( !madeTrack )
		{
                this->drop(std::shared_ptr<ItemInstance>(
                    new ItemInstance(Item::minecart, 1)));
                this->drop(std::shared_ptr<ItemInstance>(
                    new ItemInstance(Tile::goldenRail, 10)));
                this->drop(std::shared_ptr<ItemInstance>(
                    new ItemInstance(Tile::lever, 10)));

                int poweredCount = 0;
                for (int i = 10; i < 2800; ++i) {
                    level->setTileAndData(x + i, y - 1, z - 2,
                                          Tile::quartzBlock_Id, 0,
                                          Tile::UPDATE_CLIENTS);
                    level->setTileAndData(x + i, y, z - 2, Tile::quartzBlock_Id,
                                          0, Tile::UPDATE_CLIENTS);
                    level->setTileAndData(x + i, y + 1, z - 2,
                                          Tile::quartzBlock_Id, 0,
                                          Tile::UPDATE_CLIENTS);
                    level->setTileAndData(x + i, y + 2, z - 2,
                                          Tile::glowstone_Id, 0,
                                          Tile::UPDATE_CLIENTS);
                    level->setTileAndData(x + i, y + 3, z - 2,
                                          Tile::quartzBlock_Id, 0,
                                          Tile::UPDATE_CLIENTS);

                    level->setTileAndData(x + i, y - 1, z - 1,
                                          Tile::stoneBrick_Id, 0,
                                          Tile::UPDATE_CLIENTS);
                    if (i % 20 == 0) {
                        level->setTileAndData(x + i, y, z - 1,
                                              Tile::redstoneTorch_on_Id, 0,
                                              Tile::UPDATE_CLIENTS);
                        poweredCount = 4;
                    } else {
                        level->setTileAndData(x + i, y, z - 1, 0, 0,
                                              Tile::UPDATE_CLIENTS);
                    }
                    level->setTileAndData(x + i, y + 1, z - 1, 0, 0,
                                          Tile::UPDATE_CLIENTS);
                    level->setTileAndData(x + i, y + 2, z - 1, 0, 0,
                                          Tile::UPDATE_CLIENTS);
                    level->setTileAndData(x + i, y + 3, z - 1, 0, 0,
                                          Tile::UPDATE_CLIENTS);

                    level->setTileAndData(x + i, y - 1, z, Tile::stoneBrick_Id,
                                          0, Tile::UPDATE_CLIENTS);
                    if (poweredCount > 0) {
                        level->setTileAndData(x + i, y, z, Tile::goldenRail_Id,
                                              0, Tile::UPDATE_CLIENTS);
                        --poweredCount;
                    } else {
                        level->setTileAndData(x + i, y, z, Tile::rail_Id, 0,
                                              Tile::UPDATE_CLIENTS);
                    }
                    level->setTileAndData(x + i, y + 1, z, 0, 0,
                                          Tile::UPDATE_CLIENTS);
                    level->setTileAndData(x + i, y + 2, z, 0, 0,
                                          Tile::UPDATE_CLIENTS);
                    level->setTileAndData(x + i, y + 3, z, 0, 0,
                                          Tile::UPDATE_CLIENTS);
                    ...
                    level->setTileAndData(x + i, y - 1, z + 1,
                                          ...
                                          Tile::UPDATE_CLIENTS);
                    if ((i + 5) % 20 == 0) {
                        level->setTileAndData(x + i, y, z + 1, Tile::torch_Id,
                                              0, Tile::UPDATE_CLIENTS);
                    } else {
                        level->setTileAndData(x + i, y, z + 1, 0, 0,
                                              Tile::UPDATE_CLIENTS);
                    ...
                    level->setTileAndData(x + i, y + 2, z + 2,
                                          Tile::glowstone_Id, 0,
                                          Tile::UPDATE_CLIENTS);
                                level->setTileAndData(x+i,y+3,#endifle::quartzB// End 4J sTUe::UPDATE_CLIENTS);
                }
                madeTrack = true;
		}
    ...
    int Player::getPortalWaitTime() {
        return abilities.invulnerable ? 0
                                      : SharedConstants::TICKS_PER_SECOND * 4;
    }

    int Player::getDimensionChangingDelay() {
        return SharedConstants::
            TICKS_PER_SEC  // this sound method will play locally for the local
                           // player, andat pi// broadcast to remote
                           // players
    ...
    void Player::spawnEatParticles(std::shared_ptr<ItemInstance> useItem,
        ...
        if (useItem->getUseAnimation() == UseAnim_eat) {
            ...
            // 4J Stu - Was L"mob.eat" which doesnt exist + 0.05, d->z);
            }
    ...
    void Player::ride(std::shared_ptr<Entity> e) {
        if (riding != NULL && e == NULL) {
            ...
            return;
#ifndef _CONTENT_PACKAGEde(e);
        }

void"Setting default skin to %d for player %ls\n"kin) {
#endif
    wprintf(L, skin, na#ifndef _CONTENT_PACKAGEm_skinIndex = "Attempting to set skin to %08X for player %ls\n" {
    ...
    setAnimOverrideBitmask(getS// GET_UGC_SKIN_ID_FROM_BITMASK will always be zero - this was for aSK(skinId// possible custom skin editor skin
    ...
    if (playerSkin == eDefaultSk// We always set a default skin, since we may be waiting for the player's+ 1);// custom skin to be transmitted
    
    setPlayerDefaultSkin(playerSkin);

// set the new player additional boxestomTe/*vector<ModelPart *> *pvModelParts=app.GetAdditionalModelParts(m_dwSkinId);
                ...
                if (pDLCSkinFile != NULL) {
                    ...
                    if (dwBoxC != 0) {
                        app.DebugPrintf(
                            ...
                            m_dwSkinId);
                        pvModelParts = app.SetAdditionalSkinBoxes(
...
unsigned int Player::getSkinAnimOverrideBitmask(
    DWORD skinId)  // Temp check for anim overrideL;
    if (GET_IS_DLC_SKIN_FROM_BITMASK(skinId)) {
    // SP1_ZOMBIE:   switch (GET_DLC_SKIN_ID// SP1_HEROBRINE:nId)) {
    case 0                 // SP3_ZOMBIE_PIGMAN:
    case 0x3:  // SP3_ZOMBIE_HEROBRINE:      case 0xc8:   // SPH_4JMUMMY
case 0xc9:     // SPH_AOT_MUMMY
case           // SPH_CLIMAX_ZOMBIEBUSINESSMAN    case 0x220:  //
      // SPH_CLIMAX_EVILROBOTe 0x23a:  // SPH_CLIMAX_ZOMBIE case
      // 0x23d:  // SOA_DEADLIGHT_SKINNY_ZOMBIE case 0x247:  //
      // SOA_DEADLIGHT_FEMALE_ZOMBIE4:  
    case 0x195:
    
                bitmask  // SPH_GHOST:idModel::eAnim_ArmsOutFront;
        break;
case 0x1fa:
    
                bitmask = 1 << HumanoidModel::eAnim_ArmsOutFront |
                          // SPH_GRIMREAPER:Model::eAnim_NoLegAnim;
                          break;
case 0x1f4:
    
                bitmask =
        1 << HumanoidModel::eAnim_ArmsDown |
        // SPH_4J_FRANKENSTEINl::eAnim_NoLegAni// bitmask =
        // 1<<HumanoidModel::eAnim_HasIdle;1f7:  
                    // This is not one of the prefined skins      break;
        // Does the app have an anim override for this
        // skin?
                
                bitmask = app.GetAnimOverrideBitmask(skinId);
    break;
#ifdef _XBOX_ONEurn b  // 4J Stu - For XboxOne (and probably in the future all
                       // other platforms) we// store a UUID for the player
                       // to use as the owner key for tamed animals// This
                       // should just be a string version of the
                       // xuid#endif
                       // #ifndef
                       // _CONTENT_PACKAGEtoString());
"Attempting to set cape to %08X for player %s\n") {
...
if (capeId > 0) {
    ...
    MOJANG_DATA* pMojan  // Cape app.GetMojangDataForXuid(getOnlineXuid());
        if (pMojangData) {
            ...
            "Special_Cape.png"tCapeExists()) {
                this->customTextureUrl2 = std::wstring(L"");
            }
            else {
                this->c  // if there is a custom default cloak, then set it here
            }
        }
        else {
            "Special_Cape.png"if (app.DefaultCapeExists()) {
                this->customTextureUrl2 = st "" : wstring(L);
            }
            else {
                this->customTextureUrl2 = std::wstring(L);
            }
...
DWORD Player::getCapeIdFromPath(const std::wstring& cape) {
    bool dlcCape = f "dlc" unsigned int capeId = 0;

    if (cape.size() >= 14) {
        dlcCape = cape.substr(0, 3).compare(L) == 0;

    std:'.'tring capeValue = cape.substr(7, cape.size()// 4J Stu - dlc skins are numbered using decimal to make it easier for));

   // artists/people to number manually Everything else is numbered using// hex
        ...
        capeId = MAKE_SKI// 4J Stu - This function maps the encoded DWORD we store in the playerayer:// profile to a filename that is stored as a memory texture and shared// between systems in game
    // 4J Stu - DLC skins are numbered using decimal rather than hex to makeS_DLC_SKI// it easier to number manually   "dlccape%08d.png"
        
        swprintf(chars, 256, L,
                 GET_DLC_SKIN_ID_FROM_BITMASK(capeId));

    } else {
        DWORD ugcCapeIndex = GET_UGC_SKIN_ID_FROM_BITMASK(capeId);
        DWORD defaultCapeIn "defcape%08X.png"_SKIN_ID_FROM_BITMASK(capeId);
        if (ugcCapeIndex == 0) {
        "ugccape%08X.png"s, 256, L, defaultCapeIndex);
        } else {
            swprintf(chars, 256, L, ugcCapeIndex);
        }
    }
    return chars;
}
...
void Player::ChangePlayerSkin() {
    if (app.vSkinNames.size() > 0) {
        ...
        if (m_uiPlayerC "" rentSkin > app.vSkinNames.size()) {
            m_uiPlayerCurrentSkin = 0;
            // change this players custom texture url    } else {
...
void Player::prepareCustomText  // Skin{
    MOJANG_DATA* pMojangData = app.GetMojangDataForXuid(getOnlineXuid());

if (pMojangData) {
        // 4J Stu - Don't update the cape here, it gets set elsewhereTextureUr// CapejangData-// if(pMojangData->wchCape)  //{//	this->customTextureUrl2= pMojangData->wchCape;  //}      // else//{
        ...
        //=wstring(L"");//}/*cloakTexture =
        * wstring(L"http://s3.amazonaws.com/MinecraftCloaks/")
            .append(name*)
            .append(L".png");
...
void Player::rideTick() {
    ...
    Livi  // riding can be set to null inside
          // 'Entity::rideTick()'.heckRidingStatistiscs(x - preX, y - preY, z -
          // preZ);

        // 4J Stu - I don't know why we would want to do this, but it means thatype() & e// the players head is locked in position and can't move around// xRot = preXRot;// yRot = preYRot;
        
        

        std::shared_ptr<Pig>
        pig = dynamic_pointer_cast<Pig>(riding);
...
void Player::aiStep() {
    ...
    if (isSprinting()) {
        flyingSpeed += defaul  // 4J added - we were getting a NaN with zero xd
                               // & zdetValue());
    ...
    if (entities != NULL) {
            AUTO_VAR(itEnd, enti// entities->at(i);       for (AUTO_VAR(it, entities->begin()); it != itEnd; it++) {
    ...
    voi  // 4J - TODO need to use a xuidrce) {
...
void Player::awardKillScore(std::shared_ptr<Entity> victim, int awardPoints) {
    increase  // if (victim instanceof Player)tor<O//{ctive//
              // awardStat(Stats::playerKills, 1);rd()-//
...
float Player::getDestroySpeed(Tile* tile, bool hasProperTool) {
    ...
    if (isUnderLiquid(Mat// 4J Stu - onGround is set to true on the client when we are flying, which   dy// means the dig speed is out of sync with the server. Removing this speed

   // change when flying so that we always dig as the same speed// if (!onGround) speed /= 5;
...
void Player::readAdditionalSaveData(CompoundTag*"Inventory" {
    LivingEntity::readAdditionalSaveData(entityTag);
    ListTag<CompoundTag>* in"SelectedItemSlot"    (ListTag<CompoundTag>*)entityTag->getList"Sleeping");
    inventory->load(inventoryList);
 "SleepTimer"->selected = entityTag->getInt(L"XpP"  m_isSleeping = entityTag->getBoolean(L"XpLevel"    sleepCounter = entityTag->getShort(L"XpTotal";

    experienceProgress = entityT"Score"Float(L);
    experienceLevel = entityTag->getInt(L);
    totalExperience = entityTag->getInt(L);
    setScore(entityTag->getInt(L));

    if (m_isSleepin"SpawnX"     bedPosition = new Pos"SpawnY"oor(x), Mth::floor(y), Mth::floor("SpawnZ"     stopSleepInBed(true, true, false);
    }

    if (entityTag->cont"SpawnX") && entityTag->"SpawnY"(L) &&
        entityTag->contains"SpawnZ")) {
        respawnPosition =
            new Po"SpawnForced"getInt(L), entityTag->getInt(L),
                    entityTag->getInt(L));
        respawnForced = ent "EnderItems" oolean(L);
}

foodData.readAdditionalSaveData(entityTag);
abilities.loadSaveData(e "EnderItems" if (entityTag->contains(L)) {
    ListTag<CompoundTag>  // 4J AddedsList =
        (ListTag<CompoundTag>*) en "GamePrivileges"(L);
    enderChestInventory->setItemsByTag(enderItemsList);
}

    
    m_uiGamePrivileges = entityTag->g "Inventory");
}

void Player::addAdditonalSaveData(
    CompoundTag* entity
    "SelectedItemSlot" ntity::addAdditonalSaveData(entityTag);
    entityT "Sleeping",
    inventory->save(new ListTag < Co "SleepTimer"));
entityTag->putInt(L, inv "XpP"y->selected);
entityTag->putBoolean(L"XpLevel" isSleeping);
entityTag->putShort(L"XpTotal"(short)sleepCounter);

    entityTag->putFl"Score", experienceProgress);
    entityTag->putInt(L, experienceLevel);
    "SpawnX" tyTag->putInt(L, totalExperience);
    en "SpawnY" > putInt(L, getScore());

    if (respawnPos "SpawnZ" NULL) {
        entityTag->putInt(L, respawnP"SpawnForced"        entityTag->putInt(L, respawnPosition->y);
        entityTag->putInt(L, respawnPosition->z);
        entity"EnderItems"ean(L, respawnForced);
    }
    // 4J
    // Addedta.addAdditonalSaveData("GamePrivileges"abilities.addSaveData(entityTag);

    entityTag->put(L, enderChestInventory->createTag());

    
    entityTag->putInt(L, m_uiGamePrivileges);
    }
    ...
    bool Player::hurt(DamageSource* source, float dmg) {
        if  // 4J-JEV: Fix for PSVita: #3987 - [IN GAME] The user can take
            // damage/die,     // when attempting to re-enter fly mode when
            // falling from a height.n false;

            
    
    if (source == DamageSource::fall && isAllowedToFly() &&
        abilities.flying) return false;
        ...
        if (attacker != NULL && attacker->instanceof(eTYPE_ARROW)) {
            std::shared_ptr<Arrow> arrow =
                dynamic_pointer_cast<Arrow>(attacker);
            if (arrow->owner != NULL) {
    ...
    bool Player::openBeacon(std::shared_ptr<BeaconTileE /**
                                                         * Opens an
                                                         * iteminstance-dependent
                                                         * user interface.
                                                         *
                                                         * @param itemInstance
                                                         */ nt>
...
void Player::openItemInstanceGui(std::shared_ptr<ItemInstance> itemInstance) {}
    ...
    bool Player::interact(std::shared_ptr<Entity> entity) {
        ...
        std::shar  // [EB]: Added rude check to see if we're still talking about
                   // ther<ItemIns// same item; this code caused
                   // bucket->milkbucket to be deleted because if (enti// the
                   // milkbuckets' stack got decremented to
                   // 0.
        ...
        if (item != NULL && item == getSelectedItem()) {
            if (item->count <= 0 && !abilities.instabuild) {
                removeSelectedItem();
            } else if (item->count < itemClone->count && abilities.// 4J - PC Comments         // Hack to prevent item stacks from decrementing if the player hasreturn tr// the ability to instabuildNULL) && entity->instanceof(eTYPE_LIVINGENTITY)) {
        
        
        
   // 4J - PC Commentsities.instabu// Don't remove the item in hand if the player has the abilityyer,
        // to instabuild        dynamic_pointer_cast<LivingEntity>(entity))) {
    ...
    void Player::attack(std::shared_ptr<Entity> entity) {
        ...
        float dmg = (float)getAttribute(SharedMonsterAttributes::ATTACK_DAMAGE)
                        ->getValue();
        ...
        bool bCrit = fallDistance > 0 && !onGround && !onLadder() &&
                     !isInWater() && !hasEffect(MobEffect// Ensure we put the entity on fire if we're hitting with atity->ins// fire-enchanted weaponTY);
        if (bCrit && dmg > 0) {
            dmg *= 1.5f;
        }
        dmg += magicBoost;
        ...
        if (entity->instanceof(eTYPE_MULTIENTITY_MOB_PART)) {
            std::shared_ptr<Entity> multiMob = dynamic_pointer_cast<Entity>(
        ...
        if ((item != NULL) && hurtTarget->instanceof(eTYPE_LIVINGENTITY)) {
            item->hurtEnemy(dynamic_pointer_cast<LivingEntity>(hurtTarget),
                       // awardStat(Stats.damageDealt, (int) Math.round(dmg * 10));        if (item->count <= 0) {
                removeSelectedItem();
            }
        }
        if (entity->instanceof(eTYPE_LIVINGENTITY)) {
            ...
            if (fireAspect > 0 && wasHurt) {
                // if (SharedConstants::INGAME_DEBUG_OUTPUT)     // {se if//
                // //sendMessage(ChatMessageComponent.forPlainText("DMG " + dmg
                // +     // ", " + magicBoost + ", " + knockback));n(Foo//
                // }stants::EXHAUSTION_ATTACK);
            }
        ...
        void Player::animateRespawn(std::shared_ptr<Player> player,
                                    Level * level) {
            for (int i = 0; i < 45; i++) {
                ...
                level->addParticle(
                    eParticleType_netherportal, player->x + xo,
...
Player::BedSleepingResult Player::startSleepInBed(int x, int y, int z,
                                             // may not sleep in this dimensionel->isClientSide || bTestUse) {
        if (isSleeping() |// 4J-PB - I'm going to move the position of these tests below       if// The distance check should be before the day check, otherwise you can// use the bed in daytime from far away and you'll get the message about// only sleeping at night
        
    // too far away
        // 4J-PB - We still want the tooltip for Sleep3 || abs(this->y - y) > 2 ||
            abs(this->z - z) > 3) {
        ...
        double vRange = 5;
            std::vector<std::shared_ptr<Entity> >* monsters = level->getEntitiesOfClass(
                typeid(Monster),
                AABB::newTemp(x - hRange, y - vRange, z - hRange, x + // This causes a message to be displayed, so we do want to show the         // tooltip in test mode {
                delete monsters;
                retu// may not sleep during day            delete monsters;
        }

        // 4J-PB - we're just testing use, and we get here, then the bed can be  if (!bT// used&& level->isDay()) {
    ...
    m_isSleeping = true;
    // place position on pillow and feet at bottom(x, y, z);
    xd = zd = yd = 0;
...
void Player::stopSleepInBed(bool forcefulWakeUp, bool updateLevelList,
    ...
    if (saveRespawnPoint) {
        setRespawnPosition(bedPosition, false)// make sure the chunks around the bed existurn (level->getTile(bedPosition->x, bedPosition->y, bedPosition->z) ==
            Tile::bed_Id);
}
...
Pos* Player::checkBedValidRespawnPosition(Level* level, Pos* pos, bool forced) {
    ...
    chunkSource->create((po// make sure the bed is still standing  chunkSource->create((pos->x + 3) >> 4, (pos->z - 3) >> 4);
    ...
    if (level->getTile(pos->x, pos->y, pos->z) != Tile::bed_Id) {
        ...
        bool freeFeet =
            !bottomMaterial->i// make sure the bed still has a stand-up positionool freeHead = !topMaterial->isSolid() && !topMaterial->isLiquid();
...
float Player::getSleepRotation() {
    if (bedPosition != NULL) {
        ...
        case Direction::EAST:
            return 180;
        }  // 4J-PB - added for death fadeayer::isSleeping() { return
           // m_isSleeping; }
...
int Player::getDeathFadeTimer() {
        ...
        bool Player::getPlayerFlag(int flag) {
            return (entityData->getByte(DATA_PLAYER_FLAGS_ID) & (1 << flag)) !=
                   0;
        }
...
void Player::displayClientMessage(int messageId) {}
        ...
        void Player::setRespawnPosition(Pos * respawnPosition, bool forced) {
            if (respawnPosition != NULL) {
                ...
                respawnFo  // 4J Stu - This seems to have been missed
                           // from 1.7.3, but do we care?ramBl//
                           // awardStat(Stats::jump, 1);!= NULL) {
    ...
    if (isSprinting()) {
        ...
        void Player::checkMovementStatistiscs(
            double dx, double dy,
            double dz)  // awardStat(Stats::diveOneCm, distance);rn;
    }
            ...
            if (distance > 0) {
                if (riding->instanceof(eTYPE_MINECART)) {
                    ...
                    if (distanceMinecart >= 100) {
                        ...
                        awardStat(
                            GenericStats::minecartOneM(),
                            GenericStats::param_minecart(newDistance / 100));
                    }

                    // 4J-PB - changed this because our world isn't big enough
                    // to go NULL) {
                    // 1000m    minecartAchievementPos =
                    // 4-JEV, changed slightly to add extra parameters for
                    // event);
                }
                // on durango.
                // 
                    ...
                    if (m_bAwardedOnARail == false) {
                            if (dist < 500) {
                                if ((dist > 0) && (dist % 100 == 0)) {
                                    awardStat(
                                        GenericStats::onARail(),
                                        GenericStats::param_onARail(dist));
                                }
                            } else {
#else awardStat(GenericStats::onARail(),
                                      GenericStats::param_onARail(dist));
                            m_bAwardedOnARail = true;
                        }
                    }

                    if ((m_bAwardedOnARail =#endife) && (dist >= 500)) {
...
void Player::causeFallDamage(float distance) {
    ...
    if (distance >= 2) {
        ...
        if (distanceFall >= 100) {
           ...
           // 4J-PB - added the lavaslime enemy - fix for #64007 - TU7: Code:Stats// Achievements: TCR#073: Killing Magma Cubes doesn't unlock "MonsterauseF// Hunter" Achievement.
...
void Player::giveExperienceLevels(int amount) {
    ...
    if (amount > 0 && experienceLevel % 5 == 0 &&
        ...
        float vol = experienceLevel > 30 ? 1 : experienceL// Update xp calculations from 1.3ntitySound(shared_from_this(), eSoundType_RANDOM_LEVELUP,
                               vol * 0.75f, 1);
 ...
 */// 4J Stu - Added 1.8.2 bug fix (TU6) - If players cannot eat, then theiredToI// food bar should not decrease due to exhaustiones.flying))
        return;
...
void Player::startUsingItem(std::shared_ptr<ItemIn// 4J-JEV, hook for ItemUsed event, and ironbelly achievement.    if (instance == useItem) return;
    useItem = instance;
...
int Player::getExperienceReward(std::shared_ptr<Player> killedBy) {
    if (level-// players always drop experienceules::RULE_KEEPINVENTORY))
        return 0;
...
// PlayerTeam.formatNameForTeam(getTeam(), name);flying// If player display name is not set, return nameayer::setGameMode(GameType* mode) {}

std::wstring Player::getName() { return name; }

std::wstring Player::// 4J: We can only transmit gamertag in network packets
...
void Player::setAbsorptionAmount(float absorptionAmount) {
    ...
    getEntityData()->set(DATA_PLAYER_ABSORPTION_ID, absorptionAmou// 4J - was L"/mob/char.png";rptionAmount() {
    return getEntityData()->getFloat(DATA_PLAYER_ABS// 4J - was L"/mob/char1.png";tTexture() {
    switch (m_skinIndex) {
        case eDefaultSkins_Sk// 4J - was L"/mob/char2.png";_CHAR;  
        case eDefaultSkins_Skin// 4J - was L"/mob/char3.png";HAR1;  
        case eDefaultSkins_Skin// 4J - was L"/mob/char4.png";HAR2;  
        case eDefaultSkins_Skin// 4J - was L"/mob/char5.png";HAR3;  
        case eDefaultSkins_Skin// 4J - was L"/mob/char6.png";HAR4;  
        case eDefaultSkins_Skin// 4J - was L"/mob/char7.png";HAR5;  
        case e// 4J - was L"/mob/char.png";   return TN_MOB_CHAR6;  
        case eD// TODO 4J Stu - Should we just be using the pointers and hashing them?#ifdef __PS3__       default:
            return TN_MOB_CHAR;  // 4J Stu - Names are completely unique?y#elseash_fnct(const std::shared_ptr<Player> k) {
    // 4J Stu - Names are completely unique?#endif  //__PS3__

    return (int)boost::hash_value(
        k->name);  // TODO 4J Stu - Should we just be using the pointers and comparing them for// equality?

}

bool Player::eq_test(const // 4J Stu - Names are completely unique?        const std::shared_ptr<Player> y) {
    ...
    if (privilege == ePlayerGamePrivilege_All) {
        ...
        if (value == 0) {
            ...
            Player::setPlayerGamePrivilege(
                ...
                1);
            Player::setPlayerGamePrivilege(uiGamePrivileges,
                // Some privileges will turn other things off as wellleport, 1);
        }
    } else if (privilege < ePlayerGamePrivilege_MAX) {
        if (value != 0) {
            ...
            switch (privilege) {
                ...
                case ePlayerGamePrivilege_Op:
                    ...
                    Player::setPlayerGamePrivilege(
                        uiGamePrivileges,
                       // offerGamePrivilege_CanToggleClassicHunger, 0);
                    Player::setPlayerGamePrivilege(
...
bool Player::isAllowedToUse(std::shared_ptr<ItemInstance> item) {
    bool allowed = true;
  // 4J Stu - TU8 Players should always be able to eat food items, even iftPlayers)// the build option is turned ofmePrivilege(Player::ePlayerGamePrivilege_Cann// food) !=
            0) {
            allowed = false;
        }
...
bool Player::isAllowedToHurtEntity(std::shared// 4J-JEV: Fix for #88212, bool allowed = t// Untrusted players shouldn't be able to damage minecarts oretType()) {
     // boats.se eTYPE_HANGING_ENTITY:
            case eTYPE_PAINTING:
...
std::vector<ModelPart*>* Player::GetAddi"def"lModelParts() {
 // see if we can find the parts == NULL && !m_bCheckedForModelParts) {
        bool hasCustomTexture = !customTexture// If it's a default texture (which has no parts), we have the parts, ormTextureU// we already have the texture (in which case we should have parts if// there are any) then we are done= app.GetAdditionalModelParts(m_dwSkinId);
        ...
        if (!hasCustomTexture || customTextureIsDefaultSkin ||
            m_ppAdditionalModelParts != NULL ||
        // we don't have the data from the dlc skin yet)) {
            m_bCheckedForModelParts = tru"m_bCheckedForModelParts Couldn't get model parts for skin " !m_bCheckedDLCFo"%X\n"Parts) {
            m_bCheckedDLCForModelPa// do we have it from the DLC pack?
            app.DebugPrintf(
            ...
            DLCSkinFile* pDLCSkinFile =
                app.m_dlcManager.g"m_bCheckedForModelParts Got model parts from DLCskin "LCSkinFile != NULL) {
   "for skin %X\n"ORD dwBoxC = pDLCSkinFile->getAdditionalBoxesCount();
                if (dwBoxC != 0) {
                    app.DebugPrintf(
        ...
        if (m_bCheckedForModelParts)
            setAnimOve
#if defined(__PS3__) || defined(__ORBIS__) wSkinId));
    }
...
Player::ePlayerNameValidState Player::GetPlayerNameValidState(voi#endif   return m_ePlayerNameValidState;
}
