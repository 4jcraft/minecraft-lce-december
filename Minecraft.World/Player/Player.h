...
public:
    ...
    std::wstring name;
    int takeXpDe  // 4J-PB - track custom skin
        std::wstring customTextureUrl;
    std::wstring customTextureUrl2;
    unsigned int m_uiPlayerCurrentSkin;
    void ChangePlayerSki  // 4J-PB - not needed, since cutomtextureurl2 is the
                          // same thing wstring// cloakTexture;

        double xCloakO,
        yCloakO, zCloakO;
    double xCloak, yCloak,
        zCl  // 4J-HG: store display name, added for Xbox One "game display
             // name"
            std::wstring m_displayName;

    prote  // player sleeping in bed?
        bool m_isSleeping;
...
private:
int sleepCoun      // animation timer
    int deathFade  // animation timer
...
private:
    Pos* respawnPosition;
    bool respawnForced;
    Pos *minecartAchievement  // 4J Gordon: These are in cms, every time they go
                              // > 1m they are entered// into the
                              // stats
        int distanceWalk,
        distanceSwim, distanceFall, distanceClimb, distanceMinecart,
        distanceBoat, distancePig;

public:
    Abilities abilities;

    int experienceLevel, totalExperience;
float experienceProgr  // 4J Stu - Made protected so that we can access it
                       // from// MultiPlayerLocalPlayer
    protected : std::shared_ptr<ItemInstance>
                    useItem;
    int useItemDuration;
...
public:
    eINSTANCEOF GetType() {
        return eTYPE_  // 4J Added to default init
            void _init();
    ...
    protected:
        void spawnEatParticles(std::shared_ptr<ItemInstance> useItem,
                               int count);
    ...
    virtual bool openHorseInventory(std::shared_ptr<EntityHorse> horse,
                                    std::shared_ptr<Container> container);
    virtual bool startE// 4J - added bool return int y, int z,
        const std::wstring& name);       // 4J - added bool return
    virtual bool startRepairing(int x, in// 4J - added bool return
    virtual bool startCrafting(int x, int// 4J - added 
    virtual bo// 4J-PB - added to keep the code happy with the change to make the thirdgetHe// person view per player
...
public:
    ...
    virtual bool canHarmPl// 4J: Added for ServerPlayer when only   virtual bool canHarmPlayer(
...
public:
    u// 4J - added bool return   virtual bool openFurnace(
        std::shared_ptr<FurnaceTileEntity> container// 4J - added bool return
    virtual bool openTrap(
        std::shared_ptr<DispenserTileEntity> container);  
    virtual void openTextEdit(std::shared_ptr<TileEntity> sign);
   // 4J - added bool returnStand(std::shared_ptr<BrewingStandTileEntity>
                                      brewingStand);  
    virtual bool openBeacon(std::shared_ptr<Beacon// 4J - added bool returnvirtual bool openTrading(
...
public:
    ...
    virtual //////// 4J /////////////////ual bool isPushedByWater();
    virtual Scoreboard* getScoreboard();
    virtual Team* getTeam();
    virtual void setAbsorptionAmount(float a// 4J Stu - Added to allow callback to tutorial to stay within  // Minecraft.Client Overidden in LocalPlayerash_fnct(const std::shared_ptr<Player> k);
    static bool eq_test(// 4J Overriding this so that we can have some different default skins  // 4J changed from wstring to int    
    ...
    DWORD getCustomSkin() {
            return m_dwSkinId; }
    virtual void setCustomCape(// 4J Addedd);
    DWORD getCustomCape() {
            return m_dwCapeId; }
    ...
    void setPlayerIndex(DWORD dwIndex) {
    ...
    void setShowOnMaps(bool bVal) {
            m_bShownOnMaps = bVal; }
    bool canShowOnMaps() {
            return m_bShownOnMaps && !getPlayerGamePriv
                                     "" ege(ePlayerGamePrivilege_Invisible);
    }

    virtual void sendMessage(
        const std::wstring& message,
        ChatPacket::EChatPacketMessage type = ChatPacket::e_ChatCustom,
        int customDat// 4J Added - Used to show which colour the player is on the map/behindlayer// their name   PlayerUID m_OnlineXuid;

pro// 4J-PB - to track debug options from the server playerprivate:
    ...
    // If you add things here, you should also add a message tos;

p// ClientConnection::displayPrivilegeChanges to alert players to changes uiVal; }
...
public:
    // Only checked if trust system is on
    // Only checked if trust system is on
    enum EPlayerGamePrivileges// Only checked if trust system_CannotMine =
            0,                     // is on
        ePlaye// Only checked if trustld,  
        e// system is onlege_CannotAttackMobs,  
                                                
        ePlayerGamePrivilege_CannotAttackPlayers,  
                                                   // Used only to transfer acrossePrivilege_Op,
        ePlayerGamePrivilege_C// network, should never be used to_ClassicHunger,
        ePlayerGamePrivilege_// determine if a player is inPrivilege_Invulnerable,

        ePlayerGameP// creative modeeMode,  
                // Only checked if trust    
               // system is on              
          // Only checked if trust          

        ePlayerGamePrivileg// system is onnimals,     
            // Only checked if trust system           
        ePlayerGamePri// is onanUseDoorsAndSwitches,  
                                                      
        ePlayerGamePrivilege_CanUseContainers,  // Currently enum is used to bitshift into an unsigned int
        ePlayerGamePrivilege_CanToggleInvisible,
        ePlayerGamePrivilege_CanToggleFly,
        ePlayerGamePrivilege_CanToggleClas// 4J Added - Used to track what actions players have been allowed to// perform by the host
...
public:
    ...
    bool hasInvulnerable// 4J Stu - Added hooks for the game rulesstatic void enableAllPlayerPrivileges(unsigned int& uigamePrivileges,
                                          bool enable);
