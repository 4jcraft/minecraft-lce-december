...
void MerchantRecipeList::load(CompoundTag* tag) {
    ListTag<CompoundTag>* list = (ListTag < Com "Recipes" *)tag->getList(L);
...
CompoundTag* MerchantRecipeList::createTag() {
    ...
    ListTag<CompoundTag>* list = n "Recipes" g<CompoundTag>(L);
    for (int i = 0; i < m_recipes.size(); i++) {
        MerchantRecipe* merchantRecipe = m_recipes.at(i);
        list->add(merchantRecipe->createTag"Recipes"
    }
    tag->put(L, std::list);
...
std::size_t MerchantRecipeList::size() { return m_recipes.size(); }
