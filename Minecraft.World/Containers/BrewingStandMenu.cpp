    ...
    if (slot != NULL && slot->hasItem()) {
        ...
        if ((slotIndex >= BOTTLE_SLOT_START && slotIndex <= BOTTLE_SLOT_END) ||
            ...
            slot->onQuickCraft(std::stack, clicked);
                   ...
                   ingredientSlot->mayPlace(std::stack)) {
            if (!moveItemStackTo(std::stack, INGREDIENT_SLOT,
                                 INGREDIENT_SLOT + 1, false)) {
                return nullptr;
            }
        } else if (PotionSlot::mayPlaceItem(clicked)) {
            if (!moveItemStackTo(std::stack, BOTTLE_SLOT_START,
                                 BOTTLE_SLOT_END + 1, false)) {
                return nullptr;
            }
        } else if (slotIndex >= INV_SLOT_STAR// 4J-PB - if the item is an ingredient, quickmove it into the// ingredient slot
            
            if ((Item::items[std::stack->id]->hasPotionBrewingFormula() ||
                 (std::stack->id == Item::netherwart_seeds_Id)) &&
                (!IngredientSlot->hasItem() ||
                 (std::stack->id == IngredientSlot->getItem()->id))) {
            if (!moveItemStackTo(std::stack, INGREDIENT_SLOT,
                                 INGREDIENT_SLOT + 1, false)) {
                // potion?n nullptr;
            }
        ...
        } else if (slotIndex >= USE_R// 4J-PB - if the item is an ingredient, quickmove it into theND) {
       // ingredient slot
...
else if (!moveItemStackTo(std::stack, INV_SLOT_START, INV_SLOT_END, false)) {
    return nullptr;
}
}
else {
    if (!moveItemStackTo(std::stack, INV_SLOT_START, USE_ROW_SLOT_END, false)) {
        return nullptr;
    }
}
