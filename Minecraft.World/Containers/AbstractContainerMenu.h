...
bool m_bN  // 4J addedd;
    ...
    protected : std::vector<ContainerListener*>
                    conta  // 4J Stu - The java does not have ctor here (being
                           // an abstract) but we need// one to initialise
                           // the member variables// TODO Make sure all
                           // derived classes also call this
...
public:
    ...
    virtual v// 4J used to take a std::shared_ptr<Container> container// but wasn't using it, so removed to simplify things
    bool isPauseScreen();
...
public:
    ...
    virtual bool stillValid(std::shared_// 4J Stu Added for UI0;
    ...
    public:
        static bool canItemQuickReplace(
    ...
    static int getRedstoneSignalFromContainer(
        std::shar// 4J Addedainer> container);
