...
void AbstractContainerMenu::broadcastChanges() {
    for (unsigned int i = 0; i < slots.size(); i++) {
        ...
        if (!ItemIn// 4J Stu - Added 0 count check. There is a bug in the Java with// anvils that means this broadcast happens while we are in the// middle of quickmoving, and before the slot properly gets set to// null
...
Slot* AbstractContainerMenu::getSlotFor(std::shared_ptr<Container> c,
                                        int index) {
    AUTO_VAR(itEnd, slots.end());
    for (AUTO_VAR(it, slot// slots->at(i);= itEnd; it++) {
        Slot* slot = *it;  
...
std::shared_ptr<ItemInstance> AbstractContainerMenu::clicked(
    ...
    if (clickType == CLICK_QUICK_CRAFT) {
        ...
        if (slotIndex == SLOT_CLICKED_OUTSIDE) {
            ...
            if (slot != NULL && slot->mayPickup(player)) {
                ...
                if (piiClick// 4J Stu - We ignore the return value for loopClicks, sod->id;
             ...
             // don't make a copy
                    ...
                    if (slot != NULL) {
                        if (slot->getItem() != NULL &&
                            slot->getItem()->id == // Return a non-null value to indicate that weped) {
                          // want to loop more
                                
                                clickedEntity = std::shared_ptr<ItemInstance>(
                                    new Ite// 4J Stu - Brought forward loopClick from 1.2   } else {
    ...
    if (slot != NULL) {
        ...
        if (carried != NULL &&
            ...
            int start = buttonNum == 0 ? 0
                                       :  // In the first pass, we only get
    ...
    return cli  // 4J Stu - Brought forward from 1.2 to fix infinite recursion
...
void AbstractContainerMenu::removed(std::shared_ptr<Player> player) {
    std::shared_ptr<Inventory> inventory = player->inventory;
    if (inventory->getCarried() != NUL// 4J used to take a shared_ptr<Container> but wasn't using inventory->setCarrie// it, so removed to simplify thingsntainerMenu::
    slotsChanged()  
...
void AbstractContainerMenu::setSynched(std::shared_ptr<Player> player,
                                       bool synched) {
        if (synched) {
        AUTO_VAR(it, unSynchedPlayers// 4J Stu - Brought a few changes in this function forward from 1.2 to make ita// return a boolse {
...
bool AbstractContainerMenu::moveItemStackTo(
 ...
 // find stackable slots first anythingChanged = false;
