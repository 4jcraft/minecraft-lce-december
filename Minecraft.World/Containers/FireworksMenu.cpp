...
void FireworksMenu::slotsC  // 4J used to take a shared_ptr<Container>
...
std::shared_ptr<ItemInstance> FireworksMenu::quickMoveStack(
    ...
    if (slot != NULL && slot->hasItem()) {
        ...
        if (slotIndex == RESULT_SLOT) {
            ...
            if (isValidIngredient(std::stack, -1) &&
                moveItemStackTo(std::stack, CRAFT_SLOT_START, CRAFT_SLOT_END,
                                false)) {
            ...
            if (isValidIngredient(std::stack, -1) &&
                moveItemStackTo(std::stack, CRAFT_SLOT_START, CRAFT_SLOT_END,
                                false)) {
            } else if (!moveItemStackTo(std::stack, INV_SLOT_START,
                                        INV_SLOT_END, false)) {
                return nullptr;
            }
        } else {
