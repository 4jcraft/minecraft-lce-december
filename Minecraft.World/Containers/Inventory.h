...
public:
    ...
    std::shared_ptr<ItemInstance>
        getS  // 4J-PB - Added for the in-game tooltips
        bool IsHeldItem();
...
public:
    void tick();
    bool remove  // 4J-PB added to get the right resource from the inventory for
                 // removal
        bool removeResource(int type, int iAuxVal);
    void removeResources(
        std::sha// 4J Added for tradingtem); // 4J-Stu added to the get the item that would be affected by the// removeResource functions   
...
public:
    std::shared_ptr<Inventory> std::copy();
