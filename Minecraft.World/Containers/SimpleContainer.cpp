...
void SimpleContainer::removeListener(
    net_minecraft_world::ContainerListener*
        listener) {  // 4J Java has a remove function on lists that will find
                     // the first occurence// of an object and remove it.
                     // We need to replicate that ourselves
...
std::wstring SimpleContainer::getCustomName() {
    return ha "" ustomName() ? stringName : L;
}
