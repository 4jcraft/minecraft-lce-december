...
void Inventory::replaceSlot(Item* item, int data) {
    if (item != NULL) {
        ...
        if (oldSlot >= 0) {
            ...
            items[selected] = shared_ptr<ItemInstance>(
...
int Inventory::addResource(std::shared_ptr<ItemInstance> itemInstance) {
    ...
    if (slot < 0) return count;
    if (items[slot] == NULL) {
        items[slot] = shared_ptr<ItemInstance>(
...
void Inventory::load(ListTag<CompoundTag>* inventoryList) {
    ...
    for (int i = 0; i < inventoryList->size(); i++) {
        ...
        shared_ptr<ItemInstance> item =
            shared_ptr<ItemInstance>(ItemInstance::fromTag(tag));
        if (item != NULL) {
...
unsigned int Inventory::getContainerSize() { return items.length + 4; }
...
void Inventory::hurtArmor(float dmg) {
    ...
    for (unsigned int i = 0; i < armor.length; i++) {
        if (armor[i] != NULL &&
            dynamic_cast<ArmorItem*>(armor[i]->getItem()) != NULL) {
            armor[i]->hurtAndBreak((int)dmg, std::dynamic_pointer_cast<LivingEntity>(
