...
void Inventory::replaceSlot(Item* item, int data) {
    if (item != NULL)  // It's too easy to accidentally pick block and lose
                       // enchanted items.
...
int Inventory::addResource(std::shared_ptr<ItemInstance> itemInstance) {
    ...
    int count = itemIn  // 4J Stu - Brought forward from 1.2
    ...
    if (items[slot] == NULL) {
        items[slot] = std::shared_ptr<ItemInstance>(
            new ItemInstance(type, 0, itemInstance->// 4J Stu - Brought forward from 1.2
...
bool Inventory::add(std::shared_ptr<ItemInstance> item) {
    ...
    if (!item->isDamaged()) {
        ...
        if (item->count == lastSize && player->abilit// silently destroy the item when having a full inventory
            item->count = 0;
            return true;
    }
...
if (slot >= 0) {
    ...
    item->count = 0;
    return true;
    } else if (player->ab// silently destroy the item when having a full inventory
        item->count = 0;
        return true;
    }
    return false;
    ...
    if (ite"Inventory::setItem - slot = %d,\t item = %d, aux = %d\n", slot,
#endif  // 4J Stu - Changed this a little from Java to be less
            // funn
    if (slot >= items.length) {
...
ListTag<CompoundTag>* Inventory::save(ListTag<CompoundTag>* listTag) {
        for (unsigned int i = 0; i < items.length; i++) {
            if (items[i] != NULL) {
                CompoundTag* t "Slot" ew CompoundTag();
...
void Inventory::load(ListTag<CompoundTag>* inventoryList) {
        ...
        for (int i = 0; i < inventoryList->size(); i++) {
            CompoundTag* tag = inventoryList->"Slot";
            unsigned int slot = tag->getByte(L) & 0xff;
            std::shared_ptr<ItemInstance> item =
                std::shared_ptr<ItemInstance>(ItemInstance::fromTag(tag));
            if (item != NULL) {
                if (slot >= 0 && slot < items.length) items[slot] = item;
                if (slot >= 100 && slot < armor.length + 100)
                    armor[slot - 100] = item;
            }
        }
}
...
std::shared_pt// 4J Stu - Changed this a little from the Java so it's less funny
    if (slot >= items.length) {
        return armor[slot - items./*
    ItemInstanceArray pile = items;
    if (slot >= pile.length)
    {
    slot -= pile.length;
...
std::wstring Inventory::getName() {
        return app.GetString(IDS_INVEN "" RY); }
...
bool Inventory::contains(std::shared_ptr<ItemInstance> itemInstance) {
        for (unsigned int i = 0; i < armor.length; i++) {
            if (armor[i] != NULL && armor[i]->sameItem(itemInstance))
                return true;
        }
        for (unsigned int i = 0; i < items.length; i++) {
            if (items[i] != NULL &&
                items[i]->sameItem(
                    itemInstance))  // TODO Auto-generated method stubse;
        }
...
int Inventory::countMatches(std::shared_ptr<ItemInstance> item// for (unsigned int i = 0; i < armor.length; i++);
   //{t cou//	if (armor[i] != NULL && armor[i]->sameItem(itemInstance)) count +=
   //items[i]->count;// }
