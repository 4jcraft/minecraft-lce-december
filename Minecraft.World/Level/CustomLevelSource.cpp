...
CustomLevelSource::CustomLevelSource(Level* level, __int64 seed,
    ...
    if (file == INVALID_HANDLE_VALUE) {
        ...
        DWORD bytesRead, dwFileSize = GetFileSiz#endif, NULL);
...
#ifdef _UNICODE
    std::wstring \
    "GAME:\\GameRules\\waterheight.bin"
#else #ifdef _WINDOWS64

    std::"GameRules\\waterheight.bin"#else ;

    "GAME:\\GameRules\\waterheight.bin"#endif#endif;


    file = CreateFile(waterHeightPath.c_str(), GENERIC_READ, 0, NULL,
                      OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (file == INVALID_HANDLE_VALUE) {
        DWORD er// assert(false);r();
...
pprandom  // 4J - added, so that we can have a separate random for
                    ...
                    for (int x = 0; x < CHUNK_WIDTH; x++) {
                        for (int z = 0; z < CHUNK_WIDTH; z++) {
                            ...
                            waterHeight = m_waterheightOverride
                                [mapIndex]  // app.DebugPrintf("MapHeight = %d,
                                            // y =
                                            // %d\n",//
                                            // mapHeight, yc * CHUNK_HEIGHT +
                                            // y);/////////////////////////////////////////////////////////////////////
                                            // 4J - add this chunk of code to
                                            // make
                                            // land//
                                            // "fall-off" at the edges of a
                                            // finite world -
                                            // size//
                                            // of that world is currently
                                            // hard-coded in
                                            // here
                            ...
                            if (xxx1 < 0)
                                xxx1 = 0;  // Get distance to edges of world in
                                           // z
                            int zzz0 = zzz + (worldSize / 2);
                            if (zzz0 < 0) zzz0 = 0;
                            int zzz1 = ((worldSize / 2) - 1) - zzz;
                            if (zzz1 < 0)
                                zzz1 = 0;  // Get min distance to any
                                           // edge
                            int emin = xxx0;
                            ...
                            float comp =
                                0.0f;  // Calculate how much we want the world
                                       // to fall//
                                       // away, if we're in the defined region
                                       // to do so
...
void CustomLevelSource::buildSurfaces(int xOffs, int zOffs, byteArray blocks,
    ...
    // 4J - used to be declared with class level
...
LevelChunk* CustomLevelSource::get#ifdef _OVERRIDE_HEIGHTMAP) {
    ...
    prepareHeight// 4J - Some changes made here to how biomes, temperatures and downfalls are// passed around for thread safety
    BiomeArray biomes;
    ...
    caveFeature->apply(this, leve// 4J Stu Design Change - 1.8 gen goes stronghold, mineshaft, village,// canyon this changed in 1.2 to canyon, mineshaft, village, stronghold This// change makes sense as it stops canyons running through other structures
    ...
    if (generateStructures) {
        ...
        scatteredFeature->apply(this, level, xOf//        canyonFeature.apply(this, level, xOffs, zOffs, blocks);// townFeature.apply(this, level, xOffs, zOffs, blocks);// addCaves(xOffs, zOffs, blocks);// addTowns(xOffs, zOffs, blocks);//    levelChunk->recalcHeightmap();		// 4J - removed & moved//    into its own method
    // 4J - this now creates compressed block data from the blocks array passed// in, so moved it until after the blocks are actually finalised. We also// now need to free the passed in blocks as the LevelChunk doesn't use the// passed in allocation anymore.
    LevelChunk* levelChunk = new LevelChunk(level, blocks, xOffs, zOffs);
    XPhysicalFree(tileD#else
...
void CustomLevelSource::postProcess(ChunkSour #ifdef _OVERRIDE_HEIGHTMAP) {
	...
	if (!hasVillage && pprandom->nextInt(8) == 0)
	{
		...
		if (y < level->seaLevel || pprandom->nextInt(10) == 0)
		{
			...
			delete calmLava#endif	}
	PIXEndNamedEvent();
"Monster rooms"nNamedEvent(0, );
    for (int i = 0; i < 8; i++) {
        int x = xo + pprandom->nextInt(16) + 8;
        int y = pprandom->nextInt(Level::maxBuildHeight);
        int z = zo + pprandom->nextInt(16) + 8;
    ...
    PIXEndNamedEvent("Biome decorate"NamedEvent(0, );
...
std::wstring CustomLevelSo"CustomLevelSource" { return L; }

std::vector<Biome::MobSpawnerData*>* CustomLevelSource::getMobsAt(
    MobCategory* m#ifdef _OVERRIDE_HEIGHTMAPint z) {
    ...
    r#else biome->getMobs(mo#endifory);
...
TilePos* CustomLevelSource::findNearestMapFeature(
    Level* level, const std::wstring& f#ifdef _OVERRIDE_HEIGHTMAPint z) {

    if (LargeFeature::STRONGHOLD == featureName && strongholdFeature != NULL) {
        return strongholdFeature->getNearestGener#endifature(level, x, y, z);
    }
...
void CustomLevelSource::recreateLogicStructuresForChunk(int chunkX,
                                                        int chunk#ifdef _OVERRIDE_HEIGHTMAPtures) {
        ...
        scatteredFeature->apply(t#endifevel, chunkX, chunkZ, NULL);
