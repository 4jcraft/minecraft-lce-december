    ...
    if (y < minHeight) {
        ...
        for (int _x = 0; _x < 16; _x++)
            for (int _z = 0; _z < 16; _z++) {
                if ((heightmap[_z << 4 | _x] & 0xff) < min)
                    min = (heightmap[_z << 4 | _x] & 0xff);
            }
        this->minHeight = min;
    }
    ...
    if (includeLighting) {
        ...
        if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
            p += lowerBlockLight->getDataRegion(
                *data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p);
        if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
...
int LevelChunk::setBlocksAndData(byteArray data, int x0, int y0, int z0, int x1,
    ...
    if (includeLighting) {
        ...
        if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
            p += lowerSkyLight->setDataRegion(data, x0, y0, z0, x1,
                                              min(compressedHeight, y1), z1, p);
        if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
