...
std::shared_ptr<Entity> BaseMobSpawner::loadDataAndAddEntity(
    ...
    if (getNextSpawnData() != NULL) {
        ...
        entity->load(data);
        if (entity->level != NULL) entity->level->addEntity(ent// add mounts  
        ...
        while (data->contains(Entity::RIDING_TAG)) {
            ...
            std::shared_ptr<Entity> mount =
                EntityIO::newEnti"id"idingTag->getString(L), entity->level);
            if (mount != NULL) {
...
void BaseMobSpawner::load(CompoundTag* tag"EntityId"tityId = tag->getString(L"Delay"spawnDelay = tag->getShort("SpawnPotentials"f (tag->contains(L)) {
    spawnPotentials = new std::vector<SpawnData*>();
        ListTag<CompoundTag>* potentials =
         "SpawnPotentials"undTag>*)tag->getList(L);
    ...
    setNextSpawnDa "MinSpawnDelay"
        ...
        if (tag->contains(L)) {
        "MinSpawnDelay" wnDelay = tag->getShort(L);"MaxSpawnDelay"wnDelay = tag->getShort(L"SpawnCount"spawnCount = tag->getShort(L"MaxNearbyEntities"  if (tag->contains(L)) {
            "MaxNearbyEntities" ies = tag->getShort(L);
     "RequiredPlayerRange"e = tag->getShort(L"SpawnRange"  }

    if (tag->contains(L"SpawnRange"spawnRange = tag->getShort(L);
    ...
    void BaseMobSpawner::save("EntityId" g * tag) {
        tag->putString(L"Delay" tEntityId());
        tag->putShort(L, "MinSpawnDelay" lay);
    tag->putShort(L, (s"MaxSpawnDelay"lay);
    tag->putShort(L, (s"SpawnCount"nDelay);
    tag->putShort(L,"MaxNearbyEntities";
    tag->putShort(L, (short"RequiredPlayerRange"    tag->putShort(L, (short)r"SpawnRange"rRange);
    ...
    if (getNextSpawnData() != NULL ||
        (spawnPotentials != NULL && spawnPotentials->size() > 0)) {
            ...
            list->add(getNextSpawnData()"SpawnPotentials"
            }
    ...
    std::shared_ptr<Entity> BaseMobSpawner::getDisplayEntity() {
        if (displayEntity == NULL) {
            std::shared_ptr<Entity> e =
                EntityIO::newEntity(getEntityId(), NULL);
...
BaseMobSpawner::SpawnData::SpawnData(Compoun"Weight"se)
    : WeighedRandomItem(base->getInt(L"Properties"CompoundTag* tag = base->getCompound(L"Type");
    std::wstring _ty"Minecart">getString(L);

    if (_type.compare(L) == 0) {
        "Type"(tag != NULL) {
            switch (tag->getInt(L)) {
                "MinecartChest" TYPE_CHEST : type = L;
                break;
                ca "MinecartFurnace"_FURNACE : type = L;
                break;
                cas "MinecartRideable" IDEABLE : type = L;
                bre "MinecartRideable"
            }
            else {
                type = L;
            }
...
BaseMobSpawner::SpawnData::SpawnData(CompoundTag* tag, std::wstring _typ"Minecart"eighedRandomItem(1) {
        if (_type.compare(L) == 0) {
            "Type"(tag != NULL) {
                switch (tag->getInt(L)) {
                    c "MinecartChest" YPE_CHEST : _type = L;
                    break;
                    cas "MinecartFurnace" FURNACE : _type = L;
                    break;
                    case "MinecartRideable" DEABLE:
                        _type = L;
                        brea "MinecartRideable"
                }
                else {
                    _type = L;
                }
        ...
        CompoundTag* BaseMobSpawner::SpawnData::save() {
            CompoundTag * "Properties" w CompoundTag();

            result->p "Type" ound(L, tag);
            "Weight" > putString(L, type);
            result->putInt(L, randomWeight);
