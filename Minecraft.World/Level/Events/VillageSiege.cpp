...
void VillageSiege::tick() {
    ...
    nextSpawnTime =  // 50 + level.random.nextInt(100);
        if (siegeCount > 0) {
        trySpawn();
        --siegeCount;
    }
...
bool VillageSiege::tryToSetupSiege() {
    std::vector<std::shared_ptr<Player> >* players =
        &level->pla  // for (Player player : players)
        ...
        for (int i = 0; i < 10; ++i) {
            spawnX = center->x +
                     (int)(Mth::cos(level->random->nextFloat() * PI * 2.f) *
                           radius * 0.9);
            spawnY = center->y;
            spawnZ = center->z +
            ...
            std::vector<std::shared_ptr<Village> >* villages =
                level->villages->getVillages(// for (Village v : level.villages.getVillages())
...
bool VillageSiege::trySpawn() {
...

    {
        ...
        mob->setVillager(fal// catch (Exception e) {//	e.printStackTrace();//	return false;// }
    ...
    Vec3* VillageSiege::findRandomSpawnPos(int x, int y, int z) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (MobSpawner::isSpawnPositionOk(MobCategory::monster, level, xx,
                                              yy, zz))
