...
void Explosion::explode() {
    ...
    for (AUTO_VAR(it, entities.begin()); it != itEnd; it++) {
        std::shar  // entities->at(i);*it;  // 4J Stu - If the entity is not
                   // in a block that would be blown up, then// they
        ...
        if (dist <= 1) {
            ...
            double da = sqrt(xa * xa + ya * ya // 4J Stu - Added this check to remove divide by zero errors (or// rather the issues caused by the values being set to NaN and used// in comparisons a bit later on e.g. fireball)
            if (da == 0) {
                xa = ya = za = 0.0;
            } else {
                xa /= da;
            ...
            if (e->instanceof(eTYPE_PLAYER)) {
                std::shared_ptr<Player> player =
                    dynamic_pointer_cas  // app.DebugPrintf("Adding player
                                         // knockback (%f,%f,%f)\n", xa
                                         // *// pow, ya * pow,
                                         // za * pow);
                                         //
...
void Explosion::finalizeExplosion(
    ...
    std::vec/*=NULL*/os>// 4J - added toBlowDirect parameter
{
    level->playSound(
    ...
    if (r < 2 || !destroyBlocks) {
        ...
        level->addParticle(eParticleType_hugeexplosion// 4J - use pointer to vector directly passed in if this is available - used// to speed up calling this from an incoming packet
    std::vector<TilePos>* toBlowArray =
        toBlowDirect ? toBlowDirect
                     : new std::vector<TilePos>(toBlow.begin(), toB// toBlowArray.addAll(toBlow);cks) {
  //  TODO 4J Stu - Reverse iterator
        "Finalizing explosion size %d"nNamedEvent(0, "Finalizing explosion size %d\n".DebugPrintf(, toBlow.size());
        static c// 4J - try and make at most MAX_EXPLODE_PARTICLES pairs of particles
       ...
       // for (size_t j = toBlowArray->size() - 1; j >= 0; j--)
        for (AUTO_VAR(it, toBlowArray->rbegin()); it != toBlowArray->rend();
           //&toBlowArray->at(j);TilePos* tp = &(*it);  
            int xt = tp->x;
            int yt = tp// if (xt >= 0 && yt >= 0 && zt >= 0 && xt < width && yt < depth &&// zt < height) {
            ...
            if (tile->dropFromExplosion(this)) {
                tile->spawnResources(level, xt, yt, zt,
                                     level->getData(xt, yt, zt), 1.0f / r, 0);
            }
