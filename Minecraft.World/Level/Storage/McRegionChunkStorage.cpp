...
McRegionChunkStorage::McRegionChunkStorage(ConsoleSaveFile* saveFile,
    ...
    ConsoleSavePath currentFile =
        ConsoleSavePath(m_prefix + std::wstr"entities.dat"));
    ...
    if (regionChunkInputStream == NULL) {
        ...
        if (!levelChunk->isAt(x, z)) {
            ...
            return NULL;
                ...
                // 4J Stu - We delete the data within OldChunkStorage::load, so
                // we can never reload from it
...
#ifdef SPLIT_SAVES
        loadEntities(level, levelChu #endif 
        delete chunkData;
    ...
    if (m_saveFile->getOriginalSaveVersion() >=
        SAVE_FILE_VERSION_COMPRESSED_CHUNK_STORAGE) {
        PIXBeginNamedEven"Writing chunk data");
        OldChunkStorage::save(levelChunk, level, output);
        PIXEndNamedEvent();

        PIXBeginNamedEven"Updating chunk queue");
        ...
        EnterCriticalSection(&cs_memory);
        PIXBeginNamedEven"Creating tags\n");
        CompoundTag* tag = new CompoundTag();
        CompoundTag* levelData = new CompoundTag();
        tag->"Level", levelData);
        OldChunkStorage::save(levelChunk, level, levelData);
        PIXEndNamedEvent();
        PIXBeginNamedEven"NbtIo writing\n");
        NbtIo::write(tag, output);
        PIXEndNamedEvent();
        LeaveCriticalSection(&cs_memory);
        PIXBeginNamedEven"Output closing\n");
        output->close();
        PIXEndNamedEvent();

        // 4J Stu - getChunkDataOutputStream makes a new DataOutputStream
        // that points to a new ChunkBuffer( ByteArrayOutputStream ) We
        // should clean these up when we are done
        EnterCriticalSection(&cs_memory);
        PIXBeginNamedEven"Cleaning up\n");
    ...
    levelInfo->setSizeOnDisk(
        ...
        void McRegionChunkStorage::saveEntities(
            Level * level, LevelChunk * levelChun #ifdef SPLIT_SAVES
    ...
    PIXBeginName"Writing to stream");
