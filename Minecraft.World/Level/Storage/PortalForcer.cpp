...
PortalForcer::PortalPosition::PortalPosition(int x, int y, int z, __int64 time)
    : Pos(x, y, z) {
    lastUsed = time;
}
...
bool PortalForcer::findPortal(std::shared_ptr<Entity> e, double xOriginal,
    ...
    AUTO_VAR(it, cachedPortals.find(std::hash));
    ...
    if (closest >= 0) {
        ...
        if (updateCache) {
            ...
            cachedPortalKeys.push_back(std::hash);
        }
...
bool PortalForcer::createPortal(std::shared_ptr<Entity>// 4J Stu - Increase the range at which we try and create a portal to stop// creating them floating in mid air over lava
    int r = 16 * 3;
    double closest = -1;

    int xc = Mth::floor(e->x);
    int yc = Mth::floor(e->y);
    int zc = Mth::floor(e-// 4J Stu - Changes to stop Portals being created at the border of the// nether inside the bedrock
    int XZSIZE = level->dimension->getXZSize() *
            // XZSize is chunks, convert to blocks
    int XZOFFSET =
        (XZSIZE /// Subtract 4 to stay away from the edges // TODO
                 // Make the 4 a constant in HellRandomLevelSource// Move the positions that we want to check away from the edge of the world
    if ((xc - r) < -XZOFFSET) {
    app.DebugPrintf(
        "Adjusting portal creation x due to being too close to the "
        "edge\n");
    xc -= ((xc - r) + XZOFFSET);
    ...
    if ((zc - r) < -XZOFFSET) {
    app.DebugPrintf(
        "Adjusting portal creation z due to being too close to the "
        "edge\n");
    zc -= ((zc - r) + XZOFFSET);
                        ...
                        for (int b = 0; b < 3; b++) {
                            for (int s = 0; s < 4; s++) {
                                for (int h = -1; h < 4; h++) {
                                    ...
                                    // 4J Stu - Changes to stop
                                    // Portals being created at the
                                    // border of the nether inside the
                                    // bedrock
                        ...
                        for (int s = 0; s < 4; s++) {
                            for (int h = -1; h < 4; h++) {
                                ...
                                if (h < 0 &&
                                    ...
                                    goto next_second;
                                if (h >= 0 && !level->isEmptyTile(xt, yt, zt))
                                    goto next_second;
                            }
                        }
    ...
    for (int s = 0; s < 4; s++) {
        for (int h = -1; h < 4; h++) {
            ...
            level->setTileAndData(
                xt, yt, zt, border ? Tile::obsidian_Id : Tile::portalTile_Id, 0,
                Tile::UPDATE_CLIENTS);
        }
    }
