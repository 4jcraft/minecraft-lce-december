...
bool m_bIsEmp  // 4J added

    public : RegionFile(ConsoleSaveFile* saveFile, File* path);
    ~RegionFile();
    /* the modification date of the region file when it was first
     * opened
     */
    __int64 lastModified();
    /* gets how much the region file has grown since it was last
     * checked
     */
    int getSizeDelta();
    /*
     * gets an (uncompressed) stream representing the chunk data returns
     * null if the chunk is not found or an
     * error occurs
     */
...
private:
    ...
    void zero(int sectorNumber, int lengt// 4J added
