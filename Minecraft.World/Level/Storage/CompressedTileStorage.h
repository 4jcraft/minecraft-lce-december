...
public:
    ...
    int setDataRegion(
        ...
        int // Sets a region of tile values with the data at offset
    ...
    bool testSetDataRegion(byteArray dataIn, int x0, int y0, int z0, int x1,
                           int y1, int z1,
                           int // Tests whether setting data would
                               // actually change anything
    int getDataRegion(
        byteArray dataInOut, int x0, int y0, int z0, int x1, int y1, int z1,
        int // Updates the data at offset position dataInOut with a
            // region of tile information - external ordering
            // compatible with java DataLayer

    static void staticCtor();

    void compress(int upgradeBlo#ifdef __PS3__
    void compress_SPU(int upgradeBlo#endif1);
