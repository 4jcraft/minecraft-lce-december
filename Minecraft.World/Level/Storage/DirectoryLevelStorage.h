...
private:
    ...
    defined(_DURANGO)
        ...
        std::unordered_map<std::wstring, ByteArrayOutputStream*>
            m_cachedSaveData;
    std::vector<short>
        // Temp list of files that couldn't be deleted
        // immediately due to saving being disabled

        protected : ConsoleSaveFile* m_saveFile;
    ...
    virtual CompoundTag* load(
        std::shared_ptr<// 4J Changed return val to bool to check if new player or// loaded player              
    virtual CompoundTag* loadPlayerDataTag(PlayerUID xuid);
    virtual// 4J AddedOldPlayerFiles();  
    PlayerIO* getPlayerIO();
    virtual void closeAll();
    ConsoleSavePath getDataFile(const std::wstring& id);
    st// 4J AddedgetLevelId();
    ...
    void r// 4J AddedlayerPositions();  
    static std::wstring getPlayerDir() {
            return sc_szPlayerDir; }
