...
ChunkStorage* McRegionLevelStorage::createChunkStorage(Dimension* dimension) {
    ...
    if (dynamic_cast<HellDimension*>(dimension) != NULL) {
        ...
        if (dynamic_cast<TheEndDimension*>(
                dimension))  // File dir2 = new File(folder,
                             // LevelStorage.ENDER_FOLDER);//
                             // dir2.mkdirs();// return new
                             // ThreadedMcRegionChunkStorage(dir2);//
                             // 4J-PB - save version 0 at this point means it's
                             // a create new world
            int iSaveVersion = m_saveFile->getSaveVersion();

        if ((iSaveVersion != 0) && (iSaveVersion < SAVE_FILE_VERSION_NEW_END)) {
            // For versions before TU9 (TU7 and 8) we generate a part of
            // The End, but we want to scrap it if it exists so that
            // it is replaced with the TU9+ version
            app.DebugPrintf(
            ...
            std::vector<FileEntry*>* endFiles = m_saveFile->getFilesWithPrefix(
                LevelStorage::ENDER_FOLDER)  // 4J-PB - There will be no End in
                                             // early saves
                                                if (endFiles != NULL) {
    ...
    return new McRegionChunkStorag""m_saveFile, L);
}
