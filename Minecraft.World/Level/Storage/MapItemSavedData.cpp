...
void MapItemSavedData::load(CompoundTag* tag) {
    dimension = tag->getB"dimension");
    x = tag->get"xCenter");
    z = tag->get"zCenter");
    scale = tag->getB"scale");
    if (scale < 0) scale = 0;
    if (scale > MAX_SCALE) scale = MAX_SCALE;

    int width = tag->getSh"width");
    int height = tag->getSh"height");
    if (width == MapItem::IMAGE_WIDTH && height == MapItem::IMAGE_HEIGHT) {
        colors = tag->getByteAr"colors");
    } else {
        byteArray newColors = tag->getByteAr"colors");
        // 4J
        if (colors.data != NULL) {
            delete[] colors.data;
        }
...
void MapItemSavedData::save(CompoundTag* tag) {
    tag->putB"dimension", dimension);
    tag->put"xCenter", x);
    tag->put"zCenter", z);
    tag->putB"scale", scale);
    tag->putSh"width", (short)MapItem::IMAGE_WIDTH);
    tag->putSh"height", (short)MapItem::IMAGE_HEIGHT);
    tag->putByteAr"colors", colors);
}
...
void MapItemSavedData::tickCarriedBy(std::shared_ptr<Player> player,
    ...
    for (AUTO_VAR(it, carriedBy.begin()); it != carriedBy.end();) {
        ...
        if (hp//|| (!hp->player->inventory->contains(item)
              //&& !item->isFramed() ))
        {
            ...
            if (i// addDecoration(1, player.level, "frame-" +// item.getFrame().entityId, item.getFrame().xTile,// item.getFrame().zTile, item.getFrame().dir * 90);
                        ...
                        nonPlayerDecorations.insert(
                            std::unordered_map<int, MapDecoration*>::value_type(
                                ...
                                new MapDecoration(12, x, y, rot,
                                                  item->getFrame()->entityId,
                                                  true)));
                        // 4J Stu - Put this block back in if you want to
			...
			for (AUTO_VAR(it, playerLevel->entities.begin());
				...
				if (xd < -size || yd < -size || xd > size ||
                                ...
                                if (#endifsize)  // decorations.push_back(new
                                                 // MapDecoration(7, x, y,
                                                 // 0));
                                nonPlayerDecorations.insert( std::unordered_map<int, MapDecoration *>::value_type( ent->entityId, new MapDecora#endif, x, y, rot, e// 4J-PB - display all the players in the map     // For the xbox, x and z are 0
            ...
            if (hp->player->dimension == this->dimension && !addedPlayers) {
                                        ...
                                        PlayerList* players =
                                            MinecraftServer::getInstance()
                                                ->getPlayerList();
                                        for (AUTO_VAR(it3,
                                                      players->players.begin());
                                             it3 != players->players.end();
                                             ++it3) {
                                            std::shared_ptr<ServerPlayer>
                                                decorationPlayer = *it3;
                                            if (decorationPlayer != NULL &&
                                                decorationPlayer->dimension ==
                                                    this->dimension) {
                                                float xd =
                                                    (float)(decorationPlayer
                                                                ->x -
                                                            x) /
                                                    (1 << scale);
                                                float yd =
                                                    ...
                                                    if (dimension < 0) {
                                                        ...
                                                        imgIndex =
                                                            (int)decorationPlayer
                                                                ->getPlayerIndex();
                            if (#ifdef _LARGE_WORLDSex += 4;
                      // if (abs(xd) < MAP_SIZE * 5 && abs(yd) <       else  // MAP_SIZE * 5)
                              // 4J Stu - As we have added new icons for players     // on a new row below other icons used in Java we      // need to move our index to the next row              
                   ...
                   // Added to match the old adjusted size       size--;  
                        ...
                        MemSect(45);
                                        ...
                                        MapItemSavedData::getUpdatePacket(
                                            ...
                                            AUTO_VAR(it, carriedByPlayers.find(
                                                             player));
                                            ...
                                            std::shared_ptr<HoldingPlayer> hp =
                                                it->second;
                                            return hp->nextUpdatePacket(
                                                itemInstance);
                                        }

                                        void MapItemSavedData::setDirty(
                                            int x, int y0, int y1) {
                                            SavedData::setDirty();

                                            AUTO_VAR(itEnd, carriedBy.end());
    for (AUTO_VAR(it, carriedBy.begin()); it != // carriedBy.at(i);    std::shared_ptr<HoldingPlayer> hp = *it;  
...
MapItemSavedData::getHoldingPlayer(std::shared_ptr<Player> player) {
                                        std::shared_ptr<HoldingPlayer> hp =
                                            nullptr;
                                        AUTO_VAR(it,
                                                 carriedByPlayers.find(player));

                                        if (it == carriedByPlayers.end()) {
                                            hp = std::shared_ptr<HoldingPlayer>(
                                                new HoldingPlayer(player,
                                                                  this));
                                            ...
                                            carriedBy
                                                .pus  // 4J Added
                                                      ...
                                                      // per player per
                                                      // dimension, so if they
                                                      // pickup someone// elses
                                                      // map we merge their map
                                                      // data with ours so that
                                                      // we can see
...
void MapItemSavedData::mergeInMapData(std::shared_ptr<MapItemSavedData>
                                            ...
                                            void MapItemSavedData::
                                                removeItemFrameDecoration(
                                                    std::shared_ptr<
                                                        ItemInstance>
                                                        item) {
                                                AUTO_VAR(
                                                    frameDecoration,
                                                    nonPlayerDecorations.find(
                                                        item->getFrame()
                                                            ->entityId));
                                                if (frameDecoration !=
                                                    nonPlayerDecorations
                                                        .end()) {
                                                    delete frameDecoration
                                                        ->second;
                                                    nonPlayerDecorations.erase(
                                                        frameDecoration);
                                                }
                                            }
