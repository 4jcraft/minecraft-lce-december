...
bool OldChunkStorage::saveEntities(LevelChunk* lc, Level* level,
...
#ifdef _ENTITIES_RW_SECTION
    LeaveCriticalRWSection(&lc->m_csEntities, tr #else 
    LeaveCriticalSection(&lc->m_csEntiti #endif 
    ...
    PIXBeginNamedEven"Getting block data");
    lc->writeCompressedBlockData(dos);
    PIXEndNamedEvent();

    PIXBeginNamedEven"Getting data data");
    lc->writeCompressedDataData(dos);
    PIXEndNamedEvent();

    PIXBeginNamedEven"Getting sky and block light data");
    ...
    "TileEntities", tileEntityTags);
    PIXEndNamedEvent();

    PIXBeginName"Saving tile tick data");
    ...
    if (ticksInChunk != NULL) {
        ...
        for (int i = 0; i < ticksInChunk->size(); i++) {
            ...
            CompoundTag* teTag = new CompoundTag();
            "i" ag->putInt(L, td.tileId);
            "x" ag->putInt(L, td.x);
            "y" ag->putInt(L, td.y);
            "z" ag->putInt(L, td.z);
            "t" ag->putInt(L, (int)(td.m_delay - levelTime));

            tickTags->add(teTag);
        }
        "TileTicks" ut(L, tickTags);
    }
    delete ticksInChunk;
    PIXEndNamedEvent();
    ...
    void OldChunkStorage::save(LevelChunk * lc, Level * level,
        ...
        "xPos" > putInt(L, lc->x);
        "zPos" > putInt(L, lc->z);
        "LastUpdate" g(L, level->getGameTime());
        "InhabitedTime",
            lc->inhab  // 4J - changes here for new storage. Now have static
                       // storage for getting// lighting data for block,
                       // data, and sky & block lighting. This wasn't//
                       // required in the original version as we could just
                       // reference the// information in the level itself,
                       // but with our new storage system the full// data
                       // doesn't normally exist & so
                       // getSkyLightData/getBlockLightData etc.// need
                       // somewhere to output this data. Making this static so
                       // that we aren't// dynamically allocating memory in
                       // the server thread when writing chunks as// this
                       // causes serious stalling on the main thread. Will be
                       // fine so long as// we only actually create tags
                       // for once chunk at a time.// 4J Stu - As we now
                       // save on multiple threads, the static data has
                       // been// moved to TLS
            ThreadStorage* tls = (ThreadStorage*)TlsGetValue(tlsIdx);
        ...
        tag - "BlockLight"y(L, tls->blockLightData);
        PIXEndNamedEvent();

        tag - "HeightMap" ay(L, lc->heightmap);
        tag->putS "TerrainPopulatedFlags",
            lc->terra  // 4J - changed from "TerrainPopulated" to
                       // "TerrainPopulatedFlags" as now stores a
                       // bitfield, java stores a bool
                    tag -
                "Biomes" Array(L, lc->getBiomes());
    ...
    if (ticksInChunk != NULL) {
        ...
        for (int i = 0; i < ticksInChunk->size(); i++) {
            TickNextTickData td = ticksInChunk->at(i);
            CompoundTag* teTag = new CompoundTag();
            "i" teTag->putInt(L, td.tileId);
            "x" teTag->putInt(L, td.x);
            "y" teTag->putInt(L, td.y);
            "z" teTag->putInt(L, td.z);
            "t" teTag->putInt(L, (int)(td.m_delay - levelTime));
            "p" teTag->putInt(L, td.priorityTilt);

            tickTags->add(teTag);
            "TileTicks" tag->put(L, tickTags);
        }
        delete ticksInChunk;
        PIXEndNamedEvent();
        PIXEndNamedEvent();
    ...
    void OldChunkStorage::loadEntities(LevelChunk * lc, Level * level,
        ...
        ListTag<CompoundTag>* entityTags =
            (ListTag < Com "Entities") tag->getList(L);
        if (entityTags != NULL) {
    ...
    ListTag<CompoundTag>* tileEntityTags =
        (ListTag<Com"TileEntities"->getList(L);
    ...
    LevelChunk* OldChunkStorage::load(Level * level, DataInputStream * dis) {
        ...
        levelChunk
            ->terrainPopulated  // If all neighbours have been post-processed,
                                // then we should have done the//
                                // post-post-processing now. Check that this is
                                // set as if it isn't then we// won't be
                                // able to send network data for chunks, and we
                                // won't ever try and// set it again as all
                                // the directional flags are now already set -
                                // should// only be an issue for old maps
                                // before this flag was
                                // added.
            if ((levelChunk->terrainPopulated &
                 LevelChunk::sTerrainPopulatedAllNeighbours) ==
                LevelChunk::sTerrainPopulatedAllNeighbours) {
        levelChunk->terrainPopulated |= LevelChunk::sTerrain
#ifndef _CONTENT_PACKAGE
    if (app.DebugSettingsOn() &&
        app.GetGameSettingsDebugMask(ProfileManager.GetPrimaryPad()) &
            (1L << eDebugSetting_EnableB// Read the biome data from the stream, but don't use it
    ...
    LevelChunk* OldChunkStorage::load(Level * level, CompoundTag * tag) {
        "xPos" t x = tag->getInt(L);
        "zPos" t z = tag->getInt(L);

    LevelChunk* levelChunk = new Leve// 4J - the original code uses the data in the tag directly, but this is now// just used as a source when creating the compressed data, so we need to// free up the data in the tag once we are done
    levelChunk->setBloc"Blocks"g->getByteArray(L));
    del"Blocks"g->getByteAr//	levelChunk->blocks = tag->getByteArray(L"Blocks");// 4J - the original code uses the data in the tag directly, but this is now// just used as a source when creating the compressed data, so we need to// free up the data in the tag once we are done
    levelChunk->setDat"Data"tag->getByteArray(L));
    del"Data"tag->getByteA// 4J - changed to use our new methods for accessing lighting
    levelChunk->setSkyLigh"SkyLight">getByteArray(L));
    levelChunk->setBlockLigh
        "BlockLight" etByteArr  // In the original code (commented out below)
                                // constructing DataLayers from// these
                                // arrays uses the data directly and so it
                                // doesn't need deleted. The// new
                                // setSkyLightData/setBlockLightData take a copy
                                // of the data so we need// to delete the
                                // local one now
        del "SkyLight" > getByteArray(L).data;
    ...
    "TerrainPopulated"->get(L// Java bool type or uint8_t bitfield
        ...
        if (levelChunk->terrainPopulated >= 1)
            levelChunk->terrainPopulated =
                LevelChunk::sTerrainPopulatedAllNeighbours |
                LevelChunk::sTe// Convert from old bool 
                               // type to new bitfield  // New style short       
        levelChunk->terrainPo"TerrainPopulatedFlags"(L// If all neighbours have been post-processed, then we should have done// the post-post-processing now. Check that this is set as if it isn't// then we won't be able to send network data for chunks, and we won't// ever try and set it again as all the directional flags are now// already set - should only be an issue for old maps before this flag// was added.
        
        if ((levelChunk->terrainPopulated &
             LevelChunk::sTerrainPopulatedAllNeighbours) ==
            LevelChunk::sTerrainPopulatedAllNeighbours) {
                    levelChunk->terrainPopulated |=
                        LevelChunk::sTerrainPostPostPr
