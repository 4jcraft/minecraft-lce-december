...
// member, to TLS
bool Level::getInstaTick() { return ((size_t)TlsGetValue(tlsIdx)) != 0; }
...
void Level::tickEntities() {
    ...
    if (!pendingTileEntities.empty()) {
        for (AUTO_VAR(it, pendingTileEntities.begin());
            ...
            if (!e->isRemoved()) {
                if (std::find(tileEntityList.begin(), tileEntityList.end(), e) ==
                    tileEntityList.end()) {
...
void Level::removeTileEntity(int x, int y, int z) {
    ...
    if (te != NULL && updatingTileEntities) {
        ...
        if (te != NULL) {
            ...
            AUTO_VAR(it2,
                     std::find(tileEntityList.begin(), tileEntityList.end(), te));
...
__int64 Level::getGameTime() { return levelData->getGameTime(); }

__int64 Level::getDayTime() { return levelData->getDayTime(); }

void Level::setDayTime(__int64 newTime) { levelData->setDayTime(newTime); }
