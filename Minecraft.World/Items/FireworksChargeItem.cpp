...
void FireworksChargeItem::appendHoverText(CompoundTag* expTag,
                                          std::vector<HtmlString>* lin// shape  
    ...
    if (colorList.length > 0) {
        ...
        for (unsigned int i = 0; i < colorList.length; ++i) {
            int c = colorList[i];
            if (!first) {
                output += ",\n"  // 4J-PB  - without the newline, they tend to
                                 // go offscreen in split-screen
                                 // or localised languages
            }
            first = false  // find color name by lookup
                bool found = false;
        ...
        lines->push_back(outpu// has fade?   
    ...
    if (fadeList.length > 0) {
            ...
            for (unsigned int i = 0; i < fadeList.length; ++i) {
                ...
                if (!first) {
                    output ",\n"  // 4J-PB  - without the newline, they tend to
                                  // go
                                  // offscreen in split-screen or localised
                                  // languages
                }
                fir  // find color name by lookup
                    bool found = false;
                for (int dc = 0; dc < 16; dc++) {
                    if (c == DyePowderItem::COLOR_RGB[dc]) {
                        found = true;
                        output +=
                            app.GetString(FIREWORKS_CHARGE_COLOUR_NAME[dc]);
                        break;
                    }
                }
                if (!found) {
            ...
            lines->push_  // has trail;
    }

    
    bool trail = expTag->getBoolean(FireworksItem::TAG_E_TRAIL);
    if (trail) {
        lines->push_back(HtmlString(app.GetString(IDS_FIREWORKS_CHA// has flicker    }
...
void FireworksChargeItem::registerIcons(IconRegister* iconRegister) {
    ...
    overlay = iconRegister->regis"_overlay"tIconName() + L);
}