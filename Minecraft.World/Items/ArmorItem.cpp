...
int ArmorItem::getColor(std::shared_ptr<ItemInstance> item) {
    ...
    CompoundTag* display "display" tCompound(L);
    if (display == NULL)
        return Minecraft::GetInstance()->getColourTable()->getColor(
            DEFAULT_LEATHER_COLOR);

    if "color"y->contains(L)) {
        retu"color"lay->getInt(L);
    }
    else {
        return Minecraft::GetInstance()->getColourTable()->getColor(
            DEFAULT_LEATHER_COLOR);
...
void ArmorItem::clearColor(std::shared_ptr<ItemInstance> item) {
    ...
    if (tag == NULL) return;
    CompoundTag* display "display" tCompound(L);
    if "color"y->contains(L"color"lay->remove(L);
}
...
CompoundTag* display "display" tCompound(L);
"display" > contains(L) "display" tCompound(L, display);

"color" lay->putInt(L, color);
}
