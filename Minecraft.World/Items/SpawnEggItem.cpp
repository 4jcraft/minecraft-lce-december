...
Icon* SpawnEggItem::getLayerIcon(int auxValue, int spriteLayer) {
    ...
    return Item::getLayerIcon(auxValue,
                              spriteLayer)  // 4J-PB - added for dispenser
        std::shared_ptr<Entity> SpawnEggItem::canSpawn(
            int iAuxVal, Level* level, int* piResult) {
        ...
        if (newEntity != NULL) {
            ...
            switch (newEntity->GetType()) {
                ...
                case eTYPE_MUSHROOMCOW:
                    ...
                    break;
                case eTYPE_SQUID:
                    if (level->canCreateMore(eTYPE_SQUID,
                                             Level::eSpawnType_Egg)) {
                        canSpawn = true;
                    } else {
                    ...
                    break;
                case eTYPE_BAT:
                    if (level->canCreateMore(eTYPE_BAT,
                                             Level::eSpawnType_Egg)) {
                        canSpawn = true;
                    } else {
                ...
                default:
                    ...
                    // 4J: Use eTYPE_ENEMY instead of monster (slimes and
                    // ghasts aren't monsters)
                    else if (newEntity->instanceof(eTYPE_ENEMY)) {
                        // 4J-PB - check if the player is trying to spawn an
                        // enemy in peaceful mode
                        if (level->difficulty == Difficulty::PEACEFUL) {
                            *piResult = eSpawnResult_FailCantSpawnInPeaceful;
                        } else if (level->canCreateMore(
                                       newEntity->GetType(),
                                       Level::eSpawnType_Egg)) {
                            canSpawn = true;
                        } else {
    ...
    if (app.DebugArtToolsOn() && \
        tile == Tile::mobSpawner_Id)  // 4J Stu - Force adding this as a tile
                                      // update
...
std::shared_ptr<Entity> SpawnEggItem::spawnMobAt(Level* level, int auxVal,
                                                 ...
                                                 int* piResult) {
    int mobId = auxVal;
    // 4J Stu - Enable spawning specific entity
    // sub-types
    mobId = auxVal & 0xFFF;
    extraData = auxVal >> 12;
    ...
    for (int i = 0; i < SPAWN_COUNT; i++) {
        newEntity = canSpawn(mobI// 4J-JEV: DynCasting to Mob not LivingEntity; so change instanceof to// check for Mobs.
...
void SpawnEggItem::registerIcons(IconRegister* iconRegister) {
    ...
    overlay = iconRegis "_overlay" terIcon(getIconName() + L);
}
