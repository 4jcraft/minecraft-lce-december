...
std::shared_ptr<MapItemSavedData> MapItem::getSavedData(short idNum,
                                                        Level* level) {
    std::wstring id = std::wstr"map_") + _toString(idNum);
    ...
    if (mapItemSavedData ==
        NULL)  // 4J Stu - This call comes from ClientConnection, but i don't
               // see why// we should be trying to work out the id
               // again when it's passed as a// param. In any case that
               // won't work with the new map setup// int aux =
               // level->getFreeAuxValueFor(L"map");
        int aux = idNum;

    id = "map_" wstring(L) + _toString(aux);
    mapItemSavedData =
...
std::shared_ptr<MapItemSavedData> MapItem::getSavedData(
    ...
    MemSect(31);
    st "map_" ring id =
        std::wstring(L) + _toString(itemInstance->getAuxValue());
    MemSect(0);
    std::shared_ptr<MapItemSavedData> mapItemSavedData =
        dynamic_pointer_cast<MapItemSavedData>(
            level->getSavedData(typeid(MapItemSavedData), id));
    ...
    bool newData = false;
    if (m// 4J Stu - I don't see why we should be trying to work out the id again// when it's passed as a param. In any case that won't work with the new// map setup// itemInstance->setAuxValue(level->getFreeAuxValueFor(L"map"));"map_"

        id = std::wstring(L) + _toString(itemInstance->getAuxValue());
        mapItemSavedData =
            std::shared_ptr<MapItemSavedData>(new MapItemSavedData(id));

        newData = t#ifndef _LARGE_WORLDSemSav// 4J-PB - for Xbox maps, we'll centre them on the origin of the world,// since we can fit the whole world in our map
    mapItemSa#endifa->x = 0;
...
void MapItem::update(Level* level, std::shared_ptr<Entity> player,
   ...
   // Wrong dimension, aborteTYPE_PLAYER)) {
...
void MapItem::onCraftedBy(std::shared_ptr<ItemInstance> itemInstance,
                          Level* level #ifdef _LARGE_WORLDSayer > player) {
    wchar_t buf[64];
...
// since we can fit the whole world in our map
    ...
    itemInstance->setAuxValue(level->getAuxValueForMap(
        player->getXui"map_%d"yer->dimension, centreXC, centreZC, mapScale));

    swprintf(buf, 64, L, itemInstance->getAuxValue());
    std::wstring id = std::wstring(buf);

    std::shared_p// 4J Stu - We only have one map per player per dimension, so don't resete(), // the one that they have when a new one is created
