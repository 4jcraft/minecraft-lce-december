...
void Throwable::addAdditonalSaveData(Comp "xTile" * tag) {
    tag->putShort(L,"yTile")xTile);
    tag->putShort(L,"zTile")yTile);
    ...
    if (ownerName.empty() && (owner != NULL) &&
        ...
        ownerName = owner->getA"ownerName" }

    tag->putString("", ownerName.empty() ? L : ownerName);
}

void Throwable::readAdditionalSaveData(CompoundTag*"xTile"
    xTile = tag->getShort(L"yTile"
    yTile = tag->getShort(L"zTile"
    zTile = tag->getShort(L"inTile"  lastTile = tag->getByte(L) & 0"shake"  shakeTime = tag->getByte(L) & "inGround"inGround = tag->getByte(L) == "ownerName"rName = tag->getString(L);
    i""(ownerName.empty()) ownerName = L;
}
