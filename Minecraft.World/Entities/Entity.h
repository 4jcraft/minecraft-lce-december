...
protected:
    bool fireImm  // values that need to be sent to clients in SMP
        std::shared_ptr<SynchedEntityData>
            entityData;  // shared flags that are sent to clients (max
                         // 8)
    static const int DATA_SHARED_FLAGS_ID = 0;
    static const int FLAG_ONFIRE = 0;
    static const int FLAG_SNE  // static const int FLAG_ = 2;
    ...
    static const int FLAG_EFFECT_WEAKENED  // 4J ADDED, needed for cure villager
                                           // tooltip.
...
public:
    Entity(Level* level,
           bool useSma// 4J - added useSmallId parameter
    virtual ~Entity();
...
public:
    std::shared_ptr<SynchedEntityData> ge/*
...
public:
    ...
    virtual int getL// 4J - change brought forward from 1.8.2
    ...
    virtual void push(double xa, double ya, double z// 4J Added damageSource param to enable telemetry on player deaths
...
public:
    ...
    virtual void setInvisible(b// 4J-ADDED, we need to see if this is weakened
    // on the client for the cure villager tooltip. 
    
    bool isWeakened();
    void setWeakened(bool value);

prot// 4J Stu - Brought forward from 1.2.3 to fix 38654 - Gameplay: Player willool v// take damage when air bubbles are present if resuming game from// load/autosave underwater.  
...
protected:
    bool checkInTile(double x, dou// 4J - added to manage allocation of small idseStuckInWeb();// Things also added here to be able to manage the concept of a number of// extra "wandering" entities - normally path finding entities aren't// allowed to randomly wander about once they are a certain distance away// from any player, but we want to be able to (in a controlled fashion)// allow some to be able to move so that we can determine whether they have// been enclosed in some kind of farm, and so be able to better determine// what shouldn't or shouldn't be despawned.// Number of entities that can simultaneously wander (in addition tot int EXTRA_W// the ones that would be wandering in java)
            // Number of ticks each extra entity will be allowed to wander for.DER_TICKS =
  // This should be enough for it to realistically be able to walk// further than the biggest enclosure we want to consider
...
public:
    ...
    virtual std::wstring// 4J: Added    virtual bool isPushedByWater();
    virtual std::wstring getDisplayName();
    virtual std::wstring getNetworkName();  

private:
    unsigned int m_ui// 4J addedeBitmask;
