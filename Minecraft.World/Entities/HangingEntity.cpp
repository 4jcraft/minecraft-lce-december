...
bool HangingEntity::survives() {
    if (level->getCubes(shar// isEmpty())), bb)->size() != 0)  
    {
        return false;
    } else {
...
bool HangingEntity::hurt(DamageSource* source, float damage) {
    ...
    if (!removed && !level->isClientSide) {
        ...
        // check if it's serverplayer or playertanceof(
                eTYPE_PLAYER))  
        {
                ...
                if (player != NULL && player->abilities.instabuild) {
                    // 4J - added noEntityCubes parameteropItem(nullptr);
                }
                return true;
    }
    ...
    if (tag->contains(L"Dir"        dir = tag->getByte(L);
                    ...
                    switch (tag->getByte(L)) {
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            "TileX" = Direction::EAST;
                            "TileY" break;
                    }
    }
    x"TileZ"tag->getInt(L);
    yTile = tag->getInt(L);
    zTile = tag->getInt(L);
    setDir(dir);
            }
