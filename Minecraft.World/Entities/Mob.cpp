...
void Mob::addAdditonalSaveData(CompoundTag* entityTag) {
    ...
    if (leashHolder != NULL) {
        ...
        if (leashHolder->instanceof(eTYPE_LIVINGENTITY)) {
            ...
            leashTag->put "X"(L, hangInThere->xTile);
            leashTag->put "Y"(L, hangInThere->yTile);
            leashTag->put "Z"(L, hangInThere->zTile);
        }
        entityTag->"Leash", leashTag);
    }
}
...
void Mob::readAdditionalSaveData(CompoundTag* tag) {
    ...
    setCanPickUpLoot(tag->getBool"CanPickUpLoot"));
    persistenceRequired = tag->getBool"PersistenceRequired");
    if (tag->conta "CustomName") &&
        tag->getStr"CustomName").length() > 0)
        setCustomName(tag->getStr"CustomName"));
    setCustomNameVisible(tag->getBool"CustomNameVisible"));

    if (tag->conta "Equipment")) {
        ListTag<CompoundTag>* gear =
            (ListTag<CompoundTag>*)tag->getL"Equipment");
    ...
    if (tag->conta "DropChances")) {
        ListTag<FloatTag>* items =
            (ListTag<FloatTag>*)tag->getL"DropChances");
        for (int i = 0; i < items->size(); i++) {
            dropChances[i] = items->get(i)->data;
        }
        }

    _isLeashed = tag->getBool"Leashed");
    if (_isLeashed && tag->conta "Leash")) {
        leashInfoTag = (CompoundTag*)tag->getCompo"Leash")->copy();
        }
}
...
PIXBeginNamedEvent(0, );
sensing->tick();
P "Tick target selector" PIXBeginNamedEvent(0, );
targetSelector.tick();
P "Tick goal selectors" PIXBeginNamedEvent(0, );
goalSelector.tick();
P "Tick navigation";
PIXBeginNamedEvent(0, );
navigation->tick();
P "Tick server ai mob step" XBeginNamedEvent(0, );
serverAiMobStep();
P "Tick move" vent();
PIXBeginNamedEvent(0, );
moveControl->tick();
...
P "Tick jump" vent();
PIXBeginNamedEvent(0, );
jump  // Consider this for extra strolling if it is protected against
      // despawning.// We aren't interested in ones that aren't protected
      // as the whole point of// this extra wandering is to potentially
      // transition from protected to not//
      // protected."Consider extra
      // wandering"BeginNamedEvent(0, );
...
float Mob::rotlerp(float a, float b, float max) {
    ...
    if (diff < -max) {
        diff = -max;
        // 4J - altered to use special containsAnyLiquid
        // variant
 ...
 * @return TODO
 */
/*= 0*/pDa// 4J Added extraData param    Mob// 4J Stu - Take this out, it's not great and nobody will notice. Also not{
   // great for performance.// getAttribute(SharedMonsterAttributes::FOLLOW_RANGE)->addModifier(new
// AttributeModifier(random->nextGaussian() * 0.05,// AttributeModifier::OPERATION_MULTIPLY_BASE));
    ...
    return groupData;
            ...
            bool Mob::interact(std::shared_ptr<Player> player) {
                ...
                std::shared_ptr <
                    ItemInstance  // it's inconvenient to have the leash code
                ...
                if (shared_from_this()->instanceof(eTYPE_TAMABLE_ANIMAL) &&
                    (tamableAnimal = dynamic_pointer_cast<TamableAn// 4J-JEV: excuse the assignment operatorom_this()))
                ...
                if (!level->isClientSide && synch && serverLevel != NULL) {
                    serverLevel->getTracker()->broadcast(
                        ...
                        std::shared_ptr<SetEntityLinkPacket>(
                            new SetEntityLinkPacket(SetEntityLinkPacket::LEASH,
                                                    shared_from_this(),
                                                    nullptr)));
                }
            }
        ...
        serverLevel->getTracker()->broadcast(
            ...
            std::shared_ptr<SetEntityLinkPacket>(new SetEntityLinkPacket(
                SetEntityLin// after being added to the world, attempt to recreate leash bond Mob::restoreLeashFromSave() {
    "UUID"
    if (_isLeashed && leashInfoTag != NULL) {
                    "UUID"(leashInfoTag->contains(L)) {
                        ...
                        std::vector<std::shared_ptr<Entity> >* livingEnts =
                            level->getEntitiesOfClass(typeid(LivingEntity),
                                                      bb->grow(10, 10, 10));
                        for (AUTO_VAR(it, livingEnts->begin());
                             it != livingEnts->end(); ++it) {
                    ...
                    else if (leashI "Z" Tag->contains(L) &&
                             leash "X" oTag->contains(L) &&
                             le "Y" InfoTag->contains(L)) {
                        int x "Z" eashInfoTag->getInt(L);
                        int y = leashInfoTag->getInt(L);
                        int z = leashInfoTag->getInt(L);

                        std::shared_ptr<LeashFenceKnotEntity> activeKnot =
                            LeashFenceKnotEntity::findKnotAt(level, x, y, z);
                        if (activeKnot == NULL) {
                            activeKnot = LeashFenceKnotEntity::createAndAddKnot(
                                level, x, y, z);
                        }
                        leashHolder = activeKnot;
                        setLe  // 4J added so we can not render mobs before
                               // their chunks are loaded - to // resolve bug
                               // 10327 :Gameplay: NPCs can spawn over chunks
