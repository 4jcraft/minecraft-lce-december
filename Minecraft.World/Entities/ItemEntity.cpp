...
bool ItemEntity::merge(std::shared_ptr<ItemEntity> target) {
    ...
    if (targetItem->count < myItem->count)
        return target->merge(
            dynamic_pointer_cast<ItemEntity>(shared_from_this()));
...
bool Item  // 4J - added next line: found whilst debugging an issue with item
           // entities// getting into a bad state when being created by a
           // cactus, since entities// insides cactuses get hurt and
           // therefore depending on the timing of things// they could get
...
void ItemEntity::readAdditio "Health" ata(CompoundTag* tag) {
    health "Age"->getShort(L) & 0xff;
    age = tag->get"Item"L);
    CompoundTag* itemTag = tag->getCompound(L);
    setItem(ItemInstance::fromTag(itemTag));
    if (getItem() == NULL) std::remove();
}

void ItemEntity::playerTouch(std::shared_ptr<Player> player) {
    if (level->isClientSi// 4J Stu - Fix for duplication glitchnstance> item = getItem();
    ...
    int orgCount = item// if (item.id == Tile.treeTrunk.id)layer->in// player.awardStat(Achievements.mineWood); if (item.id ==
// Item.leather.id) player.awardStat(Achievements.killCow); if (item.id    // == Item.diamond.id) player.awardStat(Achievements.diamonds); if
        // (item.id == Item.blazeRod.id)// player.awardStat(Achievements.blazeRod);
        
        if (item->id == Item::diamond_Id) {
            player->awardStat(GenericStats::
#ifdef _EXTENDED_ACHIEVEMENTS GenericStats::param_diamonds());
...
#endif GenericStats::param_diamondsToYou());
            }
            }
