    ...
    MobGroupData* group/*= 0*/nt // 4J Added extraData param
{
    ...
    if (getCarried(SLOT_HELM) == NULL) {
        if (Calendar::GetMonth() + 1 == 10 && Calendar::GetDayOfMonth() == 31 &&
            random->nextFloat(// Halloween! OooOOo! 25% of all skeletons/zombies can wear pumpkins// on their heads.       
...
void Skeleton::readAdditionalSaveData(CompoundTag* tag) {
    Monster::readAddi "SkeletonType"(tag);

    if (tag->contains(L"SkeletonType"     int value = tag->getByte(L);
