...
void Arrow::tick() {
    ...
    if ((res != NULL) && (res->entity != NULL) &&
        ...
        std::shared_ptr<Player> player =
            dynamic_pointer_cast <  // 4J: Check for owner being
                                    // null
    ...
    if (res != NULL) {
        if (res->entity != NULL) {
            ...
            if (res->entity->hurt(damageSo// Firx for #67839 - Customer Encountered: Bows enchanted with// "Flame" still set things on fire if pvp/attack animals is// turned off 4J Stu - We should not set the entity on fire// unless we can cause some damage (this doesn't necessarily// mean that the arrow hit lowered their health) set targets on// fire first because we want cooked pork/chicken/steak
                ...
                if (knockback > 0) {
                    ...
                    if (pushLen > 0) {
                        res->entity->push(xd * knockback * .6f / pushLen, 0.1,
                ...
                if (owner != NULL && res->entity != owner &&
                    ...
                    dynamic_pointer_cast<ServerPlayer>(owner)->connection->send(
             ...
             // target is now deady->isAlive()       
                    && (res->entity->GetType() ==
 // target is a creeper eTYPE_CREEPER))  
                ...
                if (res->entity->GetType() != eTYPE_ENDERDRAGON) std::remove();
                } else {
                    xd *= -0.1f;
                    yd *= -0.1f;
                    zd *= -0.1f;
                ...
                if (lastTile != 0) {
                    Tile::tiles[lastTile]->entityInside(
                        level, xTile, yTile, zTile, shared_from_this());
                }
        }
        }
        delete res;
...
void Arrow::addAdditonalSav"xTile"ompoundTag* tag) {
...
void Arrow::readAdditionalSaveData(Co "xTile" ag * tag) {
    xTile = tag->ge"yTile"L);
    yTile = tag->ge"zTile"L);
    zTile = tag->getS"inTile");
    lastTile = tag->getByte(L"inData"& 0xff;
    lastData = tag->getByte(L"shake"& 0xff;
    shakeTime = tag->getByte(L"inGround"0xff;
    inGround = tag->getBy"damage") == 1;
    if (tag->contains(L"damage"        baseDamage = tag->getDoub"pickup");
}
