...
std::shared_ptr<Entity> Silverfish::findAttackTarget() {
    ...
    int Silverfish::getAmbientSound() {  // 4J - brought sound change forward
                                         // from 1.2.3
        return eSoundType_MOB_SILVERFISH_AMBIENT;
    }

    int Silverfish::getHurtSound() {  // 4J - brought sound change forward
                                      // from 1.2.3
        return eSoundType_MOB_SILVERFISH_HURT;
    }

    int Silverfish::getDeathSound() {  // 4J - brought sound change forward
                                       // from 1.2.3
    ...
    void Silverfish::checkHurtTarget(
        std::shared_ptr<Entity> target,
        float d) {  //        super.checkHurtTarget(target, d);
    ...
    void Silverfish::tick() {  // rotate the whole body to the same angle as the
                               // head
        yBodyRot = yRot;
    ...
    void Silverfish::serverAiStep() {
                        ...
                        for (int zOff = 0;
                            ...
                            if (tile == Tile::monsterStoneEgg_Id) {
                                if (!level->getGameRules()->getBoolean(
                                    ...
                                    int data = level->getData(baseX + xOff,
                                                              baseY + yOff,
                                                              baseZ + zOff);
                                    ...
                                    if (data ==
                                        StoneMonsterTile::HOST_STONEBRICK) {
                                        restoreTile = Tile::stoneBrick;
                                    }

                                    level->setTileAndData(
                                        baseX + xOff, baseY + yOff,
                                        baseZ + zOff, restoreTile->id, 0,
                                        Tile::UPDATE_ALL);
                                } else {
                                    level->destroyTile(baseX + xOff,
                                                       baseY + yOff,
                                                       baseZ + zOff, false);
                                }
                                Tile::monsterStoneEgg->destroy(
                                    level, baseX + xOff, baseY + yOff,
                                    baseZ + zOff, 0);
        ...
        if (attackTarget == NULL && !isPathFinding()) {
            // if the silverfish isn't doing anything special, it will
            // merge with any rock tile it is nearby
...
float Silverfish::getWalkTargetValue(int x, int y, int// silverfish LOVES stone =)
