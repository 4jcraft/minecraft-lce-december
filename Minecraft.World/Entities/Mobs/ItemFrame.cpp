...
void ItemFrame::removeFramedMap(std::shared_ptr<ItemInstance> item) {
    ...
    if (item->id == Item::map_Id) {
        ...
        mapItemSavedData
            ->removeItemFrameDecora  // mapItemSavedData.decorations.remove("frame-"
                                     // + entityId);
    }
...
void ItemFrame::addAdditonalSaveData(CompoundTag* tag) {
    if (getItem() != NULL) {
        "Item" > putCompound(L, getItem()->save(new CompoundTag()));
        "ItemRotation" Byte(L, (uint8_t)getRotation());
        "ItemDropChance" at(L, dropChance);
    }
    HangingEntity::addAdditonalSaveData(tag);
}

void ItemFrame::readAdditionalSaveData(CompoundTag* tag) {
    CompoundTag* itemTag "Item" > getCompound(L);
    if (itemTag != NULL && !itemTag->isEmpty()) {
        setItem(ItemInstance::fromTag(itemTag));
        setRot"ItemRotation"Byte(L));

       "ItemDropChance"ns(L))
            dropChan"ItemDropChance"at(L);
    }
