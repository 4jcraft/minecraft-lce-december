...
bool MushroomCow::mobInteract(std::shared_ptr<Player> player) {
    ...
    if (item != NULL && item->id == Item::bowl_Id && getAge() >= 0) {
        ...
        if (player->inventory->add(std::shared_ptr<ItemInstance>(
            ...
            return true;
            // 4J: Do not allow shearing if we can't create more
            // cows
            if (item != NULL && item->id == Item::shears_Id && getAge() >= 0 &&
                level->canCreateMore(eTYPE_COW, Level::eSpawnType_Breed)) {
                std::remove();
                level->addParticle(eParticleType_largeexplode, x,
                                   y + bbHeight / 2, z, 0, 0, 0);
                if (!level->isClientSide) {
                    std::remove();
                    std::shared_ptr<Cow> cow =
                        std::shared_ptr<Cow>(new Cow(level));
                ...
                return true;
                // 4J - added so that mushroom cows have more of a chance of
