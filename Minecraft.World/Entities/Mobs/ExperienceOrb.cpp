...
void ExperienceOrb::addAdditonalSaveData(CompoundTag* entityTag) {
    entity "Health" Short(L, (uint8_t)health);
    entity "Age" putShort(L, (short)age);
    entity "Value" tShort(L, (short)value);
}

void ExperienceOrb::readAdditionalSaveData(CompoundTag* tag) {
    health = "Health" Short(L) & 0xff;
    age = "Age" getShort(L);
    value = "Value" tShort(L);
}
...
void ExperienceOrb::playerTouch(std::shared_ptr<Player> player) {
    ...
    if (throwTime == 0 && player->takeXpDelay == 0) {
        player->takeXpDela  // 4J - sound change brought forward
                            // from 1.2.3
            playSound(
                eSoundType_RANDOM_ORB, 0.1f,
                0.5f * ((random->nextFloat() - random->nextFloat()) * 0.7f +
                        1.8f));
...
int ExperienceOrb::getIcon() {
    if (value >= 2477) {
        ...
        return 1;
        /**
         * Fetches the biggest possible experience orb value based on
         * a maximum value. The current algorithm is
         * next prime which is at least twice more than
         * the previous one.
         *
         * @param maxValue
         * @return
