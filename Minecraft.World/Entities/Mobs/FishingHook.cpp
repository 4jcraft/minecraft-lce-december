...
void FishingHook::tick() {
    ...
    if (res != NULL) {
        // 4J Stu Move fix for : fix for #48587 - CRASH: Code:
        // Gameplay:// Hitting another player with the fishing
...
if (waterPercentage > 0) {
    if (nibble > 0) {
        ...
        if (random->nextInt(nibbleOdds) == 0) {
            ...
            yd -= 0.2f;
            playSound(eSoundType_RANDOM_SPLASH, 0.25f,
...
int FishingHook::retrieve() {
    ...
    if (hookedIn != NULL) {
        ...
        owner->level->addEntity(std::shared_ptr<ExperienceOrb>(new ExperienceOrb(
            owner->level, owner->x, owner->y // 4J Stu brought forward from 1.4   random->nextInt(6) + 1)));  
        dmg = 1;
    }
