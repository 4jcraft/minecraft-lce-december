...
void Slime::serverAiStep() {
    ...
    if (onGround && jumpDelay-- <= 0) {
        ...
        // 4J Removed TU7 to bring forward change to fix lava slime render in
        // MP targetSquish = 1;
...
void Slime::remove() {
    ...
    if (!level->isClientSide && size > 1 && getHealth() <= 0) {
        ...
        for (int i = 0; i < count; i++) {
            // The mob spawner can currently make a maximum of 25
            // slimes (limited to 50% of the total amount of monsters
            // which is 50) and so limit to slightly more than this so
            // we have some head room to make a few spawned children.
            // Also always create at least one new slime since we are
            // getting rid of this one anyway.
...
bool Slime::canSpawn() {
    ...
    Random* lcr =
        lc->getRandom// 4J - separated out so we can delete
    if ((getSize() == 1 || level->difficulty > Difficulty::PEACEF// spawn slime in swamplands at night
