...
std::shared_ptr<Entity> EnderMan::findAttackTarget() {
    ...
    if (app.GetMobsDontAttackEnabled()) {
        ...
        if (player != NULL) {
            if (isLookingAtMe(player)) {
                ...
                if (aggroTime == 0)
                    level->playEntitySound(player,
                                           eSoundType_MOB_ENDERMAN_STARE, 1, 1);
                if (aggroTime++ == 5) {
                    aggroTime = 0;
                    setCreepy(true);
                    return player;
        ...
        if (!level->isClientSide && isAlive()) {
            if (attackTarget != NULL) {
                if (attackTarget->instanceof(eTYPE_PLAYER) &&
                    ...
                    if (attackTarget->distanceToSqr(shared_from_this()) <
                        4 * 4) {
                        teleport();
                    }
                    teleportTime = 0;
    ...
    bool EnderMan::teleportTowards(std::shared_ptr<Entity> e) {
        Vec3* dir = Vec3::newTemp(
            x - e->x, bb->y0 + bbHeight / 2 - e->y + e->getHeadHeight(),
            z - e->z);
        dir = dir->normalize();
        double d = 16;
    ...
    bool EnderMan::teleport(double xx, double yy, double zz) {
        ...
        if (ok) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                double _x = xo + (x - xo) * d +
                            (random->nextDouble() - 0.5) * bbWidth * 2;
    ...
    void EnderMan::dropDeathLoot(bool wasKilledByPlayer, int playerBonusLevel) {
        ...
        if (loot > 0) {
            ...
            for (int i = 0; i < count; i++) spawnAtLocation(loot// 4J Brought forward from 1.2.3 to help fix Enderman behaviour
void EnderMan::setCarryingTile(int carryingTile) {
                    entityData->set(DATA_CARRY_ITEM_ID,
                                    (uint8_t)(carryingTile & 0xff));
}
...
void EnderMan::setCarryingData(int carryingData) {
                    entityData->set(DATA_CARRY_ITEM_DATA,
                                    (uint8_t)(carryingData & 0xff));
}
...
bool EnderMan::hurt(DamageSource* source, float damage) {
                    ...
                    if (dynamic_cast<IndirectEntityDamageSource*>(source) !=
                        NULL) {
                        aggroedByPlayer = false;
                        for (int i = 0; i < 64; i++) {
                            if (teleport()) {
