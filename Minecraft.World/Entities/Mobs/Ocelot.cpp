...
bool Ocelot::mobInteract(std::shared_ptr<Player> player) {
    ...
    if (isTame()) {
        ...
        if (temptGoal->isRunning() && item != NULL &&
...
std::shared_ptr<AgableMob> Ocelot::getBreedOffspring(
    std::shared_ptr<AgableMob> target) {  // 4J - added limit to number of
...
void Ocelot::setCatType(int type) { entityData->set(DATA_TYPE_ID, (byte)type); }
...
std::wstring Ocelot::getAName() {
    if (hasCustomName()) retu #ifdef _DEBUGame();
...
MobGroupData* Ocelot::finalizeMobSpawn(
    MobGroupData* group/*= 0*/nt // 4J Added extraData param
{
...
void Ocelot::setSittingOnTile(bool val) {
    ...
    entityData->set(DATA_FLAGS_ID,
                    val ? (byte)(current | 0x02) : (byte)(current & ~0x02));
}
