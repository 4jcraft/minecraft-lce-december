...
AABB* Minecart::getCollideBox() { return NULL; }
...
bool Minecart::hurt(DamageSource* source, float hurtDamage) {
    ...
    if (isInvul// 4J-JEV: Fix for #88212,    // Untrusted players shouldn't be able to damage minecarts or boats.
    ...
    setHurtTime(10);
    markHurt();
    setDamage(getD// 4J Stu - If someone is riding in this, then it can tick multiple times// which causes the damage to decrease too quickly. So just make the damage// a bit higher to start with for similar behaviour to an unridden one. Only// do this change if the riding player is attacking it.
    ...
    if (creativePlayer // 4J HEG - Fixed issue with player falling through the ground on// destroying a minecart while riding (issue #160607)
...
bool Minecart::isPickable() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (!level->isClientSide && dynamic_cast<ServerLevel*>(level) != NULL) {
            ...
            if (changingDimensionD// 4J Stu - Fix for #8284 - Gameplay: Collision: Minecart clips into/// through blocks at the end of the track, prevents player from riding// && lSteps > 0)
            ...
            for (AUTO_VAR(it, entities->begin()); it // entities->at(i);               std::shared_ptr<Entity> e = (*it);  
                if (e != rider.lock() && e->isPushable() &&
                    e->instanceof(eTYPE_MINECART)) {
                        ...
                        cart->m_bHasPushedCartThisTick =
                            false  // 4J Added - We should only be pushed by one
                                   // minecart per                 // tick,
                                   // the closest one Fix for #46937 - TU5:
                                   // Gameplay:                   //
                                   // Crash/Freeze occurs when a minecart with
                                   // an animal inside               //
                                   // will be forced to
                                   // despawn
    ...
    void Minecart::moveAlongTrack(int xt, int yt, int zt, double maxSpeed,
        ...
        if (rider.lock() != NULL &&
            ...
            if (forward > 0) {
                ...
                if (ownDist < 0.01) {
                    xd += riderXd * 0.1;
                    zd  // on golden rails without power, stop the cart false;
                }
            }
        }
        ...
        int zn = Mth::floor(z);
        if (xn != xt || zn != zt) {
            pow = sq  // if on golden rail with power, increase speed- xt);
                zd = pow * (zn - zt);
        }
    ...
    Vec3* Minecart::getPosOffs(double x, double y, double z, double offs) {
        ...
        if (BaseRailTile::isRail(tile)) {
            ...
            // 4J TODO Is this a good way to copy the bit of the array that we
            // need? 1;
        }
...
Vec3* Minecart::getPos(double x, double y, double z) {
    ...
    if (BaseRailTile::isRail(tile)) {
        ...
        if (((BaseRailTile*)Tile::tiles[tile])->isUsesDataBit()) {
            data &=
                BaseRailTile::RAIL_DIRECTION_MAS  // 4J TODO Is this a good way
                                                  // to copy the bit of the
    ...
    retu"CustomDisplayTile"necart::readAdditionalSaveData(CompoundTa"DisplayTile" if (tag->getBoolean(L"DisplayData" setDisplayTile(tag->getInt(L"DisplayOffset"DisplayData(tag->getInt(L"CustomName"    setDisplayOffset(tag->get"CustomName"));
}

    if (tag->contains(L"CustomName"        tag->getString(L).length() > 0)
        name = tag->getString(L);
    }
    ...
    void Minecart::a "CustomDisplayTile"(CompoundTag* tag) {
        ...
        if (!name.empty()) tag->putString(L, name);
    }
    ...
    void Minecart::push(std::shared_ptr<Entity> e) {
        ...
        if (e->instanceof(eTYPE_LIVINGENTITY) && !e->instanceof(eTYPE_PLAYER) &&
            !e->instanceof(eTYPE_VILLAGERGOLEM) &&
            (getType() == TYPE_RIDEABLE) && (xd * xd + zd * zd > 0.01)) {
        ...
        if (e->instanceof(eTYPE// 4J Stu - Brought forward changes to fix minecarts pushing eache zo = e->z -// other         //  Fix for #38882 - TU5: Gameplay: Minecart with furnace is not          //  able to move another minecart on the rail.
            ...
            } else if (cart != NULL && cart->getType() != TYPE_FURNACE &&
                       getType() == TYPE_FURNACE) {
            e->xd *= 0.2f;
            e->zd *= 0.2f;
            ...
            e->xd *= 0.2f;
            e->zd *= 0.2f;
            // 4J Stu - Fix for #46937 - TU5: Gameplay: Crash/Freeze
            // occursartThisTick = tru// when a minecart with an animal inside
            // will be forced to// despawn Minecarts can end up
    ...
    Tile* Minecart::getDefaultDisplayTile() { return NULL; }
