...
void Sheep::dropDeathLoot(bool wasKilledByPlayer, int playerBonusLevel) {
    if (!isS// killing a non-sheared sheep will drop a single block of cloth
...
bool Sheep::mobInteract(std::shared_ptr<Player> player) {
    std::shared_ptr<ItemInstance> item = player->i// 4J-JEV: Fix for #88212,    // Untrusted players shouldn't be able to sheer sheep.
    if (!player->isAllowedToInteract(shared_from_th// Animal::interact(player);  
...
void Sheep::readAdditionalSaveData(CompoundTag* tag) {
    Animal::readAdditionalSaveDat"Sheared"   setSheared(tag->getBoolean(L"Color"
    setColor((int)tag->getByte(L));
}
...
std::shared_ptr<AgableMob> Sheep::getBreedOffspring(
    s// 4J - added limit to number of animals that can be bred
...
void Sheep::ate(// remove a minute from aging  if (isBaby()) {
