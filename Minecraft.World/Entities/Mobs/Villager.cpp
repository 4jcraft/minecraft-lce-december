...
void Villager::_init(in// 4J Stu - This function call had to be moved here from the Entity ctor to// ensure that the derived version of the function is called
...
void Villager::serverAiMobStep() {
    ...
    if (!isTrading() && updateMerchantTimer > 0) {
        ...
        if (updateMerchantTimer <= 0) {
            if (ad// improve max uses for all obsolete recipes
                if (offers-// for (MerchantRecipe recipe : offers)
    ...
    addEffect(new MobEffectInstance(MobEffect::regeneration->id,
...
bool Villager::mobInteract(std// [EB]: Truly dislike this code but I don't see another easy way
    std::shared_ptr<ItemInstance> item = player->inventory->getSelected();
    bool holdingSpawnEgg = item != NULL && item->id == Item::spawnEgg_Id;

    if (!holdingSpawnEgg && isAlive() && !isTrading() && !isBaby()) {
    // note: stop() logic is controlled by trading ai
    // goal 4J-JEV: Villagers in PC game
    // don't display professions.
...
void Villager::addAdditonalSaveData(CompoundTag* tag) {
    AgableMob::addA "Profession" Data(tag);
    tag->putInt(L"Riches" Profession());
    tag->putInt(L, riches);
    if (offe "Offers" LL) {
        tag->putCompound(L, offers->createTag());
    }
}

void Villager::readAdditionalSaveData(CompoundTag* tag) {
    AgableMob::readAdditionalSaveDa
        "Profession" setProfession(tag->getInt(L"Riches"));
    riches = tag->ge"Offers");
    if (tag->contains(L)) {
        Co "Offers" g* compound = tag->getCompound(L);
        delete offers;
...
void Villager::die(DamageSource* source) {
    ...
    if (_village != NULL) {
        ...
        if (sourceEntity != NULL) {
            ...
            // if the villager was killed by the world (such as lava
            // or// falling), blame the nearest player by not
            // reproducing for a
            // while
...
bool Villager::isTrading() { return tradingPlayer.lock() != NULL; }

void Villager::notifyTrade(MerchantRecipe* activeRecipe) {
    activeRecipe->increaseUses();
    ambientSoundTime = -getAmbientSoundInterval();
    playSound(eSoun// when the player buys the latest item, we improve the merchant a little// while later
...
void Villager::addOffers(int addCount) {
    ...
    switch (getProfession()) {
        ...
        case PROFESSION_LIBRARIAN:
            addItemForTradeIn(newOffers, Item::paper_Id, random,
                              getRecipeChance(.8f));
            addItemForTradeIn(newOffers, Item::bo// addItemForTradeIn(newOffers, Item::writtenBook_Id, random,.8f));
    ...
    // shuffle the list to make it more interestingadeIn(newOffers,
...
if (offers == NULL) {
    ...
    MIN_MAX_VALUES[Item::diamond_Id] = std::pair<int, int>(4,// MIN_MAX_VALUES.insert(Item::writtenBook_Id, pair<int,int>(1, 1)););
 ...
 * @param random
 * @param likelyHood
 */IN_MAX_PRICES[Item::beef_cooked_Id] = std::pair<int, int>(-7, -5);
    ...
    std::shared_ptr<ItemInstance> Villager::getItemTradeInValue(
        int itemId, Random* random) {
        return std::shared_ptr<ItemInstance>(
            /**
             * Adds a merchant recipe that trades rubies for an item. If the
             * cost is negative, one ruby will give several of that item.
             *
             * @param list
             * @param itemId
             * @param random
    ...
    for (int i = 0; i < 5; i+/*= 0*/   // 4J Added extraData paramGaussian() * 0.02;
        double ya = random->nextGaussian() * 0.02;
        double za = random->nextGaussian() * 0.02;
        level->addParticle(
            particle, x + random->nextFloat() * bbWidth * 2 - bbWidth,
            y + 1.0f + random->nextFloat() * bbHeight,
            z + random->nextFloat() * bbWidth * 2 - bbWidth// 4J - added limit to villagers that can be bredinalizeMobSpawn(
