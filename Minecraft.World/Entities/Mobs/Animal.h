...
public:
    void setInLoveValue(int valu// 4J added
    void setInLove(std::shared_ptr<Player>
                       playe// 4J added, then modified to match latest Java
...
private:
    ...
    static const int MAX_WANDER_DISTANCE =
        // Maximum distance that the entity can wander before being
        // considered as not enclosed & therefore not protected (in tiles)
    short m_minWanderX,
        m_maxWande// Bounding x values that this has moved since last being
                  // set to protected ( in tiles )
    short m_minWanderZ,
        m_maxWande// Bounding z values that this has moved since last being
                  // set to protected ( in tiles )
