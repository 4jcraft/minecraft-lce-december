...
Fireball::Fireball(Level* level, double x, double y, double z, double xa,
    ...
    // Fix for #69150 - [CRASH] TU8: Code: Gameplay: Nether portal mechanics
    // can// become permanently broken, causing a hard lock upon usage. IF
    // xa, ya and// za are 0 then dd is 0 and the xa/dd etc return NAN
    if (dd == 0.0) {
        xPower = 0.0;
        yPower = 0.0;
        zPower = 0.0;
...
void Fireball::tick() {  // 4J-PB - Moved forward from 1.2.3// if
                         // (!level->isClientSide && (owner == NULL ||
                         // owner->removed))
    if (!level->isClientSide) {
        if ((owner != NULL && owner->removed) ||
           ...
           "owner\n");
            ...
            // 4J-PB - TU9 bug fix - fireballs can hit the edge of the
            ...
            if ((x <= minXZ) || (x >= maxXZ) || (z <= minXZ) || (z >= maxXZ)) {
                std::remove();
                a"Fireball removed - end of world\n");
                return;
            }
        }
    }

    Entity  // app.DebugPrintf("Fireball x %d, y %d,
            // z%d\n",(int)x,(int)y,(int)z);
    ...
    if (inGround) {
        ...
        if (tile == lastTile) {
            ...
            if (life == SharedConstants::TICKS_PER_SECOND * 30) {
                std::remove();
            "Fireball removed - life is 20*60\n");
            }
            return;
        } else {
            inGround = false;
    ...
    zd += zPower;
    xd *= inertia;
    // 4J-PB - bug fix for the fireballs in a saved game - they are saved
    // with// no/very small velocity, so end up hanging around in the
    // air
    if (!level->isClientSide) {
        if ((abs(xd) < 0.002) && (abs(yd) < 0.002) && (abs(zd) < 0.002)) {
            xd = 0.0;
            zd = 0.0;
          "Removing a fireball with zero velocity\n");
          std::remove();
        }
    }
...
void Fireball::addAdditonalSav"xTile"ompoundTag* tag) {
...
void Fireball::readAdditionalSaveData(Co "xTile" ag* tag) {
    xTile = tag->ge"yTile"L);
    yTile = tag->ge"zTile"L);
    zTile = tag->getS"inTile");
    lastTile = tag->getByte(L"inGround"0xff;
    inG// Load the stored direction and apply it to the fireball//   if it has no stored direction, remove it."direction"
    if (tag->contains(L)) {
        ListTag<DoubleTag>* listTag =
  "direction"ListTag<DoubleTag>*)tag->getList(L);
