...
bool Boat::hurt(DamageSource* source, float hurtDamage) {
    ...
    setHurtTime(10);
    // 4J Stu - If someone is riding in this, then it can tick multiple
    // times// which causes the damage to decrease too quickly. So just
    // make the damage// a bit higher to start with for similar behaviour
    // to an unridden one. Only// do this change if the riding player is
    // attacking it.
    ...
    // creative mode 4J-PB - Fix for XB1 #175735 - [CRASH] [Multi-Plat]:
    // Code:// Gameplay: Placing a boat on harmful surfaces causes the game
    // to crash
    bool creativePlayer =
...
void Boat::tick() {
    ...
    if (level->isClientSide && doLerp) {
        if (lSteps > 0) {
            ...
            yd *= 0.95f;
#else *= 0.99f;
// 4J Stu - Fix for #8280 - Gameplay : Boats behave erratically when// exited next to land. The client shouldn't change the position of// the boat    
            d// + xd; = x;  
            double yt = y + yd;
            d// + zd; = z;  
            this->setPos(xt, yt, zt)// 4J Stu - Fix for #9579 - GAMEPLAY: Boats with a player in them// slowly sink under the water over time, and with no player in them// they float into the sky. Just make the boats bob up and down// rather than any other client-side movement when not receiving// packets from server
           ...
           // if (onGround)//{            
            xd *= 0.5f;
            yd *= 0.5f;
            zd *= 0.5//}            // xd *= 0.99f;// yd *= 0.95f;// zd *= 0.99f;#endif
   ...
   // entities->at(i);d_ptr<Entity> e = (*it);  
...
float Boat::getShadowHeightOffs() { re"Boat"; }

std::wstring Boat::getName() { return L; }

bool Boat::interact(std::shared_ptr<Player> player) {
    if ((rider.lock() != NULL) && rider.lock()->instanceof(eTYPE_PLAYER) &&
        (rider.lock() != player))
        ret// 4J HEG - Fixed issue with player not being able to dismount boat// (issue #4446)
