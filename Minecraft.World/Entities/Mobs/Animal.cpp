...
void Animal::defineSynchedData() {
    ...
    entityData->define(DATA_IN_LOVE, (int)// 4J added
}
...
void Animal::checkHurtTarget(
    std::shared_ptr<Entity> target,
    float d) {  // 4J-JEV: Changed from dynamic cast to use eINSTANCEOF
                ...
                if (a->attackTarget == shared_from_this() && d < 3.5) {
                    ...
                    if (loveTime % 4 == 0) {
                        level->addParticle(
                            ...
                            x + random->nextFloat() * bbWidth * 2 - bbWidth,
                            y + .5f + random->nextFloat() * bbHeight,
                            z + random->nextFloat() * bbWidth * 2 - bbWidth, 0,
                            0, 0);
                    }
    ...
    void Animal::breedWith(std::shared_ptr<Animal> target) {
        ...
        target->loveTime = 0;
    target->setI// 4J - we have offspring of NULL returned when we have hit our limits of// spawning any particular type of animal. In these cases try and do// everything we can apart from actually spawning the entity.
    if (offspring// Only want to set the age to this +ve value if something is actually// spawned, as during this period the animal will attempt to follow// offspring and ignore players.
        setAge(5 * 60 * 20);
...
bool Animal::hurt(DamageSource* dmgSource, float dmg) {
    ...
    if (dynamic_cast<EntityDamageSource*>(dmgSource) != NULL) {
        std::shared_ptr<Entity> source =
            dmgSource  // 4J-JEV: Changed from dynamic cast to use
                       // eINSTANCEOF
...
void Animal::addAdditonalSaveData(CompoundTag* tag) {
    AgableMob::addAddito "InLove" ata(tag);
    tag->putInt(L, getInLoveValue());
}

void Animal::readAdditionalSaveData(CompoundTag* tag) {
    AgableMob::readAdditionalSaveData(tag"InLove"etInLoveValue(tag->getInt(L));
    setDespawnProtected();
}
                    ...
                    switch (GetType()) {
                        case eTYPE_CHICKEN:
                            if (!level->canCreateMore(
                                    eTYPE_CHICKEN, Level::eSpawnType_Breed)) {
                        ...
                        case eTYPE_WOLF:
                            if (!level->canCreateMore(
                                    eTYPE_WOLF, Level::eSpawnType_Breed)) {
                                player->displayClientMessage(
                                    IDS_MAX_WOLVES_BRED);
                                return false;
                            }
                        ...
                        default:
                            if ((GetType() & eTYPE_ANIMALS_SPAWN_LIMIT_CHECK) ==
                                ...
                                if (!level->canCreateMore(
                                        GetType(), Level::eSpawnType_Breed)) {
                                    player->displayClientMessage(
