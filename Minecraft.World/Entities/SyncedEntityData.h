...
public:
    ...
    void std::set(int id, uint8_t value);
    void std::set(int id, int value);
    void std::set(int id, short value);
    void std::set(int id, float value);
    void std::set(int id, const std::wstring& value);
    void std::set(int id, std::shared_ptr<ItemInstance>);
    void markDirty(int id);
    bool isDirty();
    static void pack(std::vector<std::shared_ptr<DataItem> >* items,
              // TODO throws IOExceptionutput);  
    std::vector<std::shared_ptr<DataItem> >* packDirty();
    v// throws IOExceptiontStream* output);  
    std::vector<std::shared_ptr<DataItem> >* getAll();

private:
    static void writeDataItem(
        DataOutputStream* output,
 // throws IOExceptionr<DataItem> dataItem);  

public:
    static std::vector<std::shared_ptr<DataI// throws IOException   Dat/**
     * Assigns values from a list of data items.
     *
     * @param items
     */
public:
    void assignValues(std::vector<std::shared_ptr<DataItem> >* i// 4J Addedbool isEmpty();
    void clearDirty();
