...
void FireworksRocketEntity::addAdditonalSaveData(CompoundTag* tag)"Life" tag->putInt(L, life"LifeTime"->putInt(L, lifetime);
    std::shared_ptr<ItemInstance> itemInstance =
        entityData->getItemInstance(DATA_ID_FIREWORKS_ITEM);
    if (itemInstance != NULL) {
    CompoundTag* itemTag = new CompoundTag();
    itemInstance->save(itemTag);
    "FireworksItem" Compound(L, itemTag);
    }
}

void FireworksRocketEntity::readAdditionalSaveData(CompoundTag* tag) {
    "Life" ife = tag->getInt(L);
    "LifeTime" = tag->getInt(L);

    CompoundTag* ite "FireworksItem" Compound(L);
    if (itemTag != NULL) {
