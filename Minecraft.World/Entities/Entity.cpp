...
void Entity::useSmallIds() { TlsSetValue(tlsIdx, (LPVOID)1); }
            ...
            void Entity::setSize(float w, float h) {
                if (w != bbWidth || h != bbHeight) {
                    ...
                    if (bbWidth > oldW && !firstTick && !level->isClientSide) {
                        std::move(oldW - bbWidth, 0, oldW - bbWidth);
                    }
                }
            }
...
void Entity::setRot(float yRot/* JAVA:
...
void Entity::setOnFire(int numberOfSeconds) {
        ...
        newValue = ProtectionEnchantment::getFireAfterDampener(
            shared_from_this(), newValue);
        if (onFire < newValue) {
            onFire = newValue;
        }
}
...
if (makeStepSound() && !isPlayerSneaking && riding == NULL) {
    ...
    if (moveDist > nextStep && t > 0) {
        ...
        if (isInWater()) {
            float speed =
                Mth::sqrt(xd * xd * 0.2f + yd * yd + zd * zd * 0.2f) * 0.35f;
            if (speed > 1) speed = 1;
            playSound(eSoundType_LIQUID_SWIM, speed,
...
void Entity::moveRelative(float xa, float za, float speed) {
    ...
    zd += za *  // 4J - change brought forward from 1.8.2
        ...
        if (level->hasChunkAt(xTile, 0, zTile)) {
            ...
            void Entity::absMoveTo(double x, double y, double z, float yRot,
                                   float xRot) {
                xo = this->x = x;
                yo = this->y = y;
                zo = this->z = z;
            ...
            void Entity::moveTo(double x, double y, double z, float yRot,
                                float xRot) {
                xOld = xo = this->x = x;
            ...
            void Entity::push(std::shared_ptr<Entity> e) {
                if (e->rider.lock().get() == this || e->riding.get() == this)
                    return;
            ...
            bool Entity::intersects(double x0, double y0, double z0, double x1,
                                    double y1, double z1) {
            ...
            void Entity::awardKillScore(std::shared_ptr<Entity> victim,
                                        int score) {}
  ...
  "PortalCooldown"ionDelay = tag->getInt(L"UUID");
    ...
    if (repositionEntityAfterLoad()) setPos(x, y, z);
            ...
            const std::wstring Entity::getEncodeId() {
                ...
                std::shared_ptr<ItemEntity> Entity::spawnAtLocation(
                    int resource, int count, float yOffs) {
                    return spawnAtLocation(
                        std::shared_ptr<ItemInstance>(
                            new ItemInstance(resource, count, 0)),
                        yOffs);
                }
                ...
                std::shared_ptr<ItemEntity> Entity::spawnAtLocation(
                    ...
                    std::shared_ptr<ItemEntity> ie =
                        std::shared_ptr<ItemEntity>(new ItemEntity(
                            level, x, y + yOffs, z, itemInstance));
                    ie->throwTime = 10;
                    level->addEntity(ie);
                    return ie;
                }
                ...
                AABB* Entity::getCollideAgainstBox(
                ...
                void Entity::positionRider() {
                    ...
                    lockedRider->setPos(
                        x, y + getRideHeight() + lockedRider->getRidingHeight(),
                        z);
                }
...
void Ent  // ItemInstance[]) {}

    ItemInstanceArray Entity::getE  // Default ctor creates NULL internal
...
bool Entity::checkInTile(double x, double y, double z) {
                ...
                if ((cubes && !cubes->empty()) ||
                    ...
                    bool north =
                        !level->isFullAABBTile(xTile, yTile, zTile - 1);
                    bool south =
                        !level->isFullAABBTile(xTile, yTile, zTile + 1);
...
void Entity::copyPosition(std::shared_ptr<Entity> target) {
                moveTo(target->x, target->y, target->z, target->yRot,
                       target->xRot);
}
            ...
            return;
                ...
                if (newEntity != NULL) {
                    ...
                    if (lastDimension == 1 && i == 1) {
                        ...
                        spawnPos->y =
                            level->getTopSolidBlock(spawnPos->x, spawnPos->z);
