...
bool Monster::hurt(DamageSource* source, float dmg) {
    ...
    if (PathfinderMob::hurt(source, dmg)) {
        ...
        return true;
        /**
         * Performs hurt action, returns if
         * successful
         *
         * @param target
         * @return
        ...
        bool Monster::doHurtTarget(std::shared_ptr<Entity> target) {
            ...
            if (wasHurt) {
                if (knockback > 0) {
                    target->push(-Mth::sin(yRot * PI / 180) * knockback * .5f,
                                 0.1,
                ...
                if (target->instanceof(eTYPE_LIVINGENTITY)) {
                    std::shared_ptr<LivingEntity> livingTarget =
                        dynamic_pointer_cast<LivingEntity>(target);
                    ThornsEnchantment::doThornsAfterAttack(
                        shared_from_this(), livingTarget, random);
                }
            }
        ...
        void Monster::checkHurtTarget(std::shared_ptr<Entity> target,
                                      float distance) {
            if (attackTime <= 0 && std::distance < 2.0f &&
        ...
        bool Monster::isDarkEnoughToSpawn() {
            ...
            int zt = Mth::floor(z);
            if (level->getBrightness(LightLayer::Sky, xt, yt, zt) >
                random->nextInt(32))
                return false;
        ...
        bool Monster::canSpawn() {
            return level->difficulty > Difficulty::PEACEFUL &&
                   isDarkEnoughToSpawn() && PathfinderMob::canSpawn();
        }
