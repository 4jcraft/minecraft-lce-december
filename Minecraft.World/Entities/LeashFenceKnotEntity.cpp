...
bool LeashFenceKnotEntity::shouldRenderAtSqrDistance(double distance) {
    return std::distance < 32 * 32;
}

void LeashFenceKnotEntity::dropItem(std::shared_ptr<Entity> causedBy) {}

bool LeashFenceKnotEntity::save(
    CompoundTag* entityTag) {  // knots are not saved, they are recreated by the
                               // entities that are tied
    return false;
}
...
bool LeashFenceKnotEntity::interact(std::shared_ptr<Player> player) {
    ...
    if (!level->isClientSide && !attachedMob) {
        ...
        if (player->abilit// if the player is in creative mode, attempt to remove all leashed// mobs without dropping additional items
            double range = 7;
            std::vector<std::shared_ptr<Entity> >* mobs = level->getEntitiesOfClass(
                typeid(Mob), AABB::newTemp(x - range, y - range, z - range,
