...
StatArray
    ...
    Stats::fallOneM = (new GeneralStat(2002, L,
        ...
        (new GeneralStat(2003, L,
                         (StatFormatter*)Stat::distanceFormatter))
            ->setAwardLocallyOnly()
            ->postC "stat.minecartOneM" ts::minecartOneM =
            (new GeneralStat(2004, L,
                             (StatFormatter*)Stat::distanceFormatter))
                ->setAwardLocallyOnly() "stat.boatOneM" truct();
    Stats::boatOneM = (new GeneralStat(2005, L,
                                       (StatFormatter*)Stat::distanceFormatter))
    ...
    Stats::pigOneM = (new GeneralStat(2006, L,
        ...
        (new GeneralStat(2007, L))
            ->pos "stat.cowsMilked" Stats::cowsMilked =
...
void Stats::buildCraftableStats() {
    ...
    "mineBlock.wheat" newStat =
        new ItemStat(BLOCKS_MINED_OFFSET + 18, L,
                     Tile::wheat_Id);
    blocksMinedStats->push_back(newStat);
    blocksMined[Tile::wheat_Id] = newStat;
    newStat->po "mineBlock.mushroom1" ewStat =
        new ItemStat(BLOCKS_MINED_OFFSET + 19, L,
                     Tile::mushroom_brown_Id);
    blocksMinedStats->push_back(newStat);
    blocksMined[Tile::mushroom_brown_Id] = newStat;
    newStat->po "mineBlock.sugar" newStat = new ItemStat(
        BLOCKS_MINED_OFFSET + 17, L, Tile::reeds_Id);
    blocksMinedStats->push_back(newStat);
#if 0ksMined [Tile::reeds_Id] = newStat;
    newStat->postCo "collectItem.wheat" ewStat = new ItemStat(
        ITEMS_COLLECTED_OFFSET + 1, L, Item::wheat->id);
    itemsCollectedStats->push_back(newStat);
    itemsCollected[Item::wheat->id] = newStat;
    newStat->p "collectItem.mushroom" tat =
        new ItemStat(ITEMS_COLLECTED_OFFSET + 2, L,
                     Tile::mushroom1->id);
    itemsCollectedStats->push_back(newStat);
    itemsCollected[Tile::mushroom1->id] = newStat;
    itemsCollected[Tile::mushroom2->id] = newStat;
    newStat->p "collectItem.sugarCane" at = new ItemStat(
        ITEMS_COLLECTED_OFFSET + 3, L, Item::reeds->id);
    itemsCollectedStats->push_back(newStat);
#endifollected[Item::reeds->id] = newStat;
    newStat->postConstru "collectItem.pumpkin" Stat = new ItemStat(
        ITEMS_COLLECTED_OFFSET + 4, L, Tile::pumpkin->id);
    itemsCollectedStats->push_back(newStat);
    ...
    itemsCrafted = Sta "craftItem.plank" newStat = new ItemStat(
        ITEMS_CRAFTED_OFFSET + 0, L, Tile::wood->id);
    itemsCraftedStats->push_back(newStat);
    itemsCrafted[Tile::wood->id] = newStat;
    newStat->po "craftItem.workbench" ewStat = new ItemStat(
        ITEMS_CRAFTED_OFFSET + 1, L, Tile::workBench->id);
    itemsCraftedStats->push_back(newStat);
    itemsCrafted[Tile::workBench->id] = newStat;
    newStat->po "craftItem.stick" newStat = new ItemStat(
        ITEMS_CRAFTED_OFFSET + 2, L, Item::stick->id);
    itemsCraftedStats->push_back(newStat);
    itemsCrafted[Item::stick->id] = newStat;
    newStat->po "craftItem.woodenShovel" tat =
        new ItemStat(ITEMS_CRAFTED_OFFSET + 3, L,
                     Item::shovel_wood->id);
    itemsCraftedStats->push_back(newStat);
    ...
    newStat->pos "craftItem.bowl" newStat = new ItemStat(
        ITEMS_CRAFTED_OFFSET + 25, L, Item::bowl->id);
    itemsCraftedStats->push_back(newStat);
    itemsCrafted[Item::bowl->id] = newStat;
    newStat->pos "craftItem.bucket" newStat = new ItemStat(
        ITEMS_CRAFTED_OFFSET + 26, L, Item::bucket_empty->id);
    itemsCraftedStats->push_back(newStat);
    ...
    newStat->pos "craftItem.fishingRod" Stat =
        new ItemStat(ITEMS_CRAFTED_OFFSET + 28, L,
                     Item::fishingRod->id);
    itemsCraftedStats->push_back(newStat);
    itemsCrafted[Item::fishingRod->id] = newStat;
    newStat->pos "craftItem.clock" newStat = new ItemStat(
        ITEMS_CRAFTED_OFFSET + 29, L, Item::clock->id);
    itemsCraftedStats->push_back(newStat);
    itemsCrafted[Item::clock->id] = newStat;
    newStat->pos "craftItem.compass" newStat = new ItemStat(
        ITEMS_CRAFTED_OFFSET + 30, L, Item::compass->id);
    itemsCraftedStats->push_back(newStat);
    itemsCrafted[Item::compass->id] = newStat;
    newStat->pos "craftItem.map" newStat = new ItemStat(
        ITEMS_CRAFTED_OFFSET + 31, L, Item::map->id);
    itemsCraftedStats->push_back(newStat);
...
void Stats::bu// The order of these stats should not be changed, as the map directly to// bits in the profile data// The number of times this player has dealt the killing blow to the  // Enderdragon
 ...
 "craftItem.flowerPot"* itemStat =
    ...
    itemStat =
        new ItemStat(offset++, L, Tile::flowerPot_Id);
    ...
    for (unsigned int i = 0; i < 16; i++) {
        "rainbowCollection." tat = new GeneralStat(
            offset++, L + _toString<unsigned int>(i));
    ...
    for (unsigned int i = 0; i < 23; i++) {
        "biomesVisited." ralStat = new GeneralStat(
            offset++, L + _toString<unsigned int>(i));
        generalStats->push_back(generalStat);
    ...
    itemStat = new ItemStat(offset++, L,
    ...
    itemStat =
        new ItemStat(offset++, L, Item::porkChop_cooked_Id);
    blocksPlacedStats->push_back(itemStat);
