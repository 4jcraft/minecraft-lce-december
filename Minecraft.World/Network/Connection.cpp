...
bool Connection::writeTick() {
    ...
    if (!outgoing.empty() &&
        ...
        Packet::writePacket(packet, bufferedD
        ...
        (fakeLag == 0 ||
         System::currentTimeMillis() - outgoing_slow.front()->createTime >=
            ...
            LeaveCriticalSection(
                &writeLock);  // If the shouldDelay flag is still set at this
                              // point then we want to// write it to
            ...
            if (packet->shouldDelay) {
                ...
                // 4J Stu - Changed this so that rather than writing to the
                // network stream through a buffered stream we want
                // to: a) Only push whole "game" packets to QNet,
                // rather than amalgamated chunks of data that may
                // include many packets, and partial packets b) To be
                // able to change the priority and queue of a packet
                    ...
                    if (dis == NULL) return didSometh  // try {
                        ...
                        std::shared_ptr<Packet>
                            packet = Packet::readPacket(
                                dis, packetListener->isServerPacketListener());

                if (packet != NULL) {
                    readSizes[packet->getId()] +=
                        packet->getEstimatedSize() + 1;
                    ...
                    didSomething = true;
                }  //		printf("Con:0x%x readTick close
                   // EOS\n",this);// 4J Stu - Remove this
                   // line// Fix for #10410 - UI: If the player is
                   // removed from a splitscreened// host�s game, the
                   // next game that player joins will produce a
                   // message// stating that the host has
                   // left.//
    ...
    return di/* 4J JEV, removed try/catch
...
void Connection::close(DisconnectPacket::eDisconnectReason reas//	printf("Con:0x%x close\n",this);
    if (!runnin//	printf("Con:0x%x close doing something\n",this);
    disconnected = true;

    va_list input;
    va_start(input, reason);

    disconnectRea// va_arg( input, const wstring );
    ...
    if (objs.size()) {
                ...
                disconnectReasonOb  //	int count = 0, sum = 0, i =
                                    // first;//	va_list marker;
    ...
    if (dis// The input stream needs closed before the readThread,// or the readThread may get stuck whilst blocking// waiting on a read     // Make sure that the read & write threads are dead before we go and kill// the streams that they depend on
    readThread->WaitForCompletion(INFINITE);
    writeThread->WaitForCompletion(INFINITE);
...
void Connection::tick() {
    ...
    LeaveCriticalSection(&incomi#if CONNECTION_ENABLE_TIMEOUT_DISCONNECT
        if (noInputTicks++ == MAX_TICKS_WITHOUT_INPUT) {
                    close(DisconnectPacket::eDiscon #endifimeOut);
                    // 4J Stu - Moved this a bit later in the function to stop
                    // the race// condition of Disconnect packets not being
                    ...
                    // close(DisconnectPacket::eDisconnect_Closed);//}
                        ...
                        noIn  // 4J Added - Send a KeepAlivePacket every now and
                            ...
                            tickCount++;
                        if (tickCount % 20 == 0) {
        send(std::shared_ptr<KeepAlivePacket>(new Ke// 4J Stu - 1.8.2 changed from 100 to 1000// 4J-PB - NEEDS CHANGED!!!   // If we can call connection.close from within a packet->handle, then we can// lockup because the loop below has locked incoming_cs, and the// connection.close will flag the read and write threads for the connection// to close. they are running on other threads, and will try to lock// incoming_cs We got this with a pre-login packet of a player who wasn't// allowed to play due to parental controls, so was kicked out This has been// changed to use a eAppAction_ExitPlayerPreLogin which will run in the main// loop, so the connection will not be ticked at that point

    EnterCriti// 4J Stu - If disconnected, then we shouldn't process incoming packets
    ...
    L// MGH - moved the packet handling outside of the incoming_cs block, as it// was locking up sometimes when disconnecting
    for (int i = 0; i < packetsToHandle.size("Handling packet %d\n"eginNamedEvent(0, ,
                           packetsToHandle[i]->getId());
        packetsToHandle[i]->handle(packetListener);
        PIXE// 4J Stu - Moved this a bit later in the function to stop the race// condition of Disconnect packets not being processed when local client// leaves
    
    if (socket && socket->isClosing()) {
        close(Disco// 4J - split the following condition (used to be disconnect &&// iscoming.empty()) so we can wrap the access in a critical section
    if (disconnected) {
                                    EnterCriticalSection(&incoming_cs);
                                    bool empty = incoming.empty();
                                    LeaveCriticalSection(&incoming_cs);
                                    if (empty) {
                                        ...
                                        // 4J added - don't keep sending this
                                        // every
                                        // tick
                                    }
    }
...
void Connection::sendAndQuit() {
    ...
    while (
        ...
        ShutdownManager::ShouldRun(ShutdownManager::eConnectionReadThre// try {        w// Sleep(100L);Tick());
// TODO - 4J Stu - 1.8.2 changes these sleeps to 2L, but not sure// whether we should do that as well
        
        con->m_hWakeReadThread/* 4J JEV, removed try/catch
    ...
    return 0;
                        ...
                        int Connection::runWrite(void* lpParam) {
                            ...
                            Connection* con =
                                dynamic_cast<Connection*>((Connection*)lpParam);
                            ...
                            EnterCriticalSection(cs);
                            con->w  // 4J Stu - Adding this to force us to run
                                    // through the writeTick at least//
                                    // once after the event is fired Otherwise
    ...
    while (
        ...
        ShutdownManager::ShouldRun(ShutdownManager::eConnectionWriteThrea// Sleep(100L);while (co//  TODO - 4J Stu - 1.8.2 changes these sleeps to 2L, but not sure//  whether we should do that as well
    ...
    Connection* con = dynamic_cast<Connection*>((Connectio// try {am);
...
// 4J TODO writeThread.interrupt();ing) {
        ...
        con->close(/* 4J Jev, removed try/catch
    } catch (Exception e) {
    e.printStackTrace();
    } */
