...
private:
    ...
    public:
        ...
        bool m_hos  // true if this is the connection between
                    // the host player and
                    // server
            // true if this player on the same machine as the
            // host 0 for client side or 1 for host
            // side// For local connections between the host player and
            // the server
    ...
    Socket(bool // 4J - Create a local socket, for end 0 or
                // 1 of a connection
    Socket(
        INetworkPlayer* player, bool response = false,
        bool h// 4J - Create a socket for an INetworkPlayer
    SocketAddress* getRemoteSocketAddress();
    void pushDataToQueue(const BYTE* pbData, DWORD dwDataSize,
                         bool fromHost = true);
    ...
    bool isClosing() {
            return m_endClosed[SOCKET_CLIENT_END] ||
