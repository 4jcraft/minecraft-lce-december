...
public:
    ...
    BYTE m_szUniqueSaveName[m_iSaveNameLen];  // added for checking if the level
                                              // is in the ban list
    DWORD m_serverSettings;  // A bitfield of server settings constructed with
                             // the MAKE_SERVER_SETTINGS macro
    BYTE m_hostIndex;  // Rather than sending the xuid of the host again, send
                       // an index into the m_playerXuids array
    ...
    PreLoginPacket(std::wstring userName, PlayerUID* playerXuids,
                   DWORD playerCount, BYTE friendsOnlyBits,
                   DWORD ugcPlayersVersion, char* pszUniqueSaveName,
                   DWORD serverSettings, BYTE hostIndex, DWORD texturePackId);
    ~PreLoginPacket();
