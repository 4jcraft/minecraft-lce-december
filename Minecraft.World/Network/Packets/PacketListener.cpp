...
void PacketListener::handleComplexItemData(
   // 1.8.2ared_ptr<ComplexItemDataPacket> packet) {
    onUnhandledPacket((std::shared_ptr<Packet>)packet);
}
...
void PacketListen// 1.0.1leSetCreativeModeSlot(
...
void PacketListener::handleTileEntityData(
    std::shared_ptr// 1.2.3ityDataPacket> tileEntityDataPacket) {
...
void PacketListener::handleSoundEvent(
    ...
    o  // void PacketListener::handleLevelChunks(shared_ptr<LevelChunksPacket>
       // packet)d//{h//	onUnhandledPacket(packet);t// }cket) {
        onUnhandledPacket(packet);
}
...
void PacketListener::handleUpdateAttributes(
 // 4J Addedred_ptr<UpdateAttributesPacket> packet) {
    onUnhandledPacket(packet);
}
