...
public:
    static std::unordered_map<int, packetCreateFn>
        idToCreateM  // IntHashMap in 1.8.2 ... needed? // Made public
                     // in 1.0.1
    ...
    static std::unordered_set<int>
        sendToAny  // 4J Stu - Added the sendToAnyClient param so we can limit
                   // some packets to// be only sent to one player on a
                   // system 4J Stu - Added renderStats param// for use in
                   // debugging
        static void std::map(int id, bool receiveOnClient, bool receiveOnServer,
...
static std::unordered_map <  // static int nextPrint;statistics;
    ...
    virtual bool isInvalidatedBy(std::shared_ptr<Packet> p// 4J Stu - Brought these functions forward for enchanting/game rules
