...
void AddPlayerPacket::read(DataInputStream* d// throws IOException
{
    id = dis->readInt();
...
void AddPlayerPacket::write(DataOutputStream* d// throws IOException
{
    dos->writeInt(id);
    writeUtf(name, dos);
    dos->writeInt(x);
    ...
    dos->writeByte(yRot);
    dos->writeByte(xRot);
    dos->writeByte(yHeadRo// 4J Added
