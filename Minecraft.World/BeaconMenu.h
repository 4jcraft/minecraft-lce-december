...
private:
	...
	{
	public:
		PaymentSlot(std::shared_ptr<Container> container, int slot, int x, int y);
...
public:
	BeaconMenu(std::shared_ptr<Container> inventory, std::shared_ptr<BeaconTileEntity> beacon);
