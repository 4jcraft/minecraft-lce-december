...
void Random::nextBytes(uint8_t* bytes, unsigned int count) {
    for (unsigned int i = 0; i < count; i++) {
        bytes[i] = (uint8_t)std::next(8);
    }
}

double Random::nextDouble() {
    return (((__int64)std::next(26) << 27) + std::next(27)) /
           (double)(1LL << 53);
}
...
double Random::nextGaussian() {
    if (haveNextNextGaussian) {
        ...
        double v1, v2, s;
        do {
            v1 = 2 * next      // between -1.0 and 1.0
                v2 = 2 * next  // between -1.0 and 1.0
                    s = v1 * v1 + v2 * v2;
        } while (s >= 1 || s == 0);
...
int Random::nextInt() { return std::next(32); }

int Random::nextInt(int n) {
    assert(n > 0);

    // i.e., n is a power of 2
        return (int)(((__int64)std::next(31) * n) >>
  // 4J Stu - Made __int64 instead of long
...
float Random::nextFloat() { return std::next(24) / ((float)(1 << 24)); }

__int64 Random::nextLong() {
    return ((__int64)std::next(32) << 32) + std::next(32);
}

bool Random::nextBoolean() { return std::next(1) != 0; }