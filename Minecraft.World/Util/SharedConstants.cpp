...
std::wstring SharedConstants::readAcceptableChars() {
    ...
    " ÇüéâäàåçêëèïîìÄÅÉæÆôöòûùÿÖÜø£Ø×ƒáíóúñÑªº¿®¬½¼¡«»ã"#if 0  // 4J - do we actually really need to get this from a file?
  // try {
  // BufferedReader br = new BufferedReader(new
  // InputStreamReader(SharedConstants.class.getResourceAsStream("/font.txt"),
  // "UTF-8"));
		BufferedReader *br = new BufferedReader(new InputStreamReader( new FileInputStream( new File( std:"/font.txt") ) ) ) );
    std::wstring line;
...
bool SharedConstants::isAllowedChatCharacter(// return ch != '§' && (acceptableLetters.indexOf(ch) >= 0 || (int) ch >// 32);
 //  4J Unused  
    return true;
}
