...
DamageSource* DamageSource::bypassArmor() {
    _bypassArmor = true;  // these kinds of damages don't cause the player to
                          // grow more hungry
    exhaustion = 0;
    return this;
}
...
DamageSource* DamageSource::setMagic() {
    _isMagic = true;
    return this  // wstring
                 // DamageSource::getLocalizedDeathMessage(shared_ptr<Player>
                 // player)//{//	return L"death." + msgId +
    ...
    DamageSource::getMsgId() {  // 4J: Very limited check for equality (used to
