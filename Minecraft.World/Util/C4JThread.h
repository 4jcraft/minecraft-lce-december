...
#ifdef _XBOX_ONE
    // 4J Stu - On XboxOne the main thread is not the one that does all the
    // static init, so we have to set this up later
...
public:
    CriticalSectionScopeLock(CRITICAL_SECTION* pCS) {
        m_pCS = pCS;
