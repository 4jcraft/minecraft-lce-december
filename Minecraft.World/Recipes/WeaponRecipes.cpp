    ...
    for (unsigned int m = 0; m < map[0].size(); m++) {
        ...
        for (int t = 0; t < MAX_WEAPON_RECIPES - 1; t++) {
            ...
            if (pObjMaterial->GetType() == eType_TILE) {
                ...
                '#' L, Item::sti'X' L, pObjMaterial->ti'T' L);
            } else {
                // must be Item
                wchTypes[4'i' L;
                r->addShapedRecipy(
                ...
                '#' L, Item::sti'X' L, pObjMaterial->it'T' L);
            }
        }
    }
    /* 4J-PB - moved out to main recipes so we can avoid them stacking on
       the group display name r->addShapedRecipy(new
       ItemInstance(Item::bow, 1), // L"ssscicig", L" #X", //
                L"# X", //
                L" #X", //
