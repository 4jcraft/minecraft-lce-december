...
bool FireworksRecipe::matches(std::shared_ptr<CraftingContainer> craftSlots,
    ...
    if (sulphurCount > 3 || paperCount > 1) {
        ...
        if (sulphurCount >= 1 && paperCount == 1 && chargeComponents == 0) {
            resultItem = std::shared_ptr<ItemInstance>(
                new ItemInstance(Item::fireworks));
            if (chargeCount > 0) {
                ...
                for (int slot = 0; slot < craftSlots->getContainerSize();
                    ...
                    std::shared_ptr<ItemInstance> item =
                        craftSlots->getItem(slot);
                    if (item == NULL || item->id != Item::fireworksCharge_Id)
                        continue;

                    if (item->hasTag() && item->getTag()->contains(
                                              FireworksItem::TAG_EXPLOSION)) {
                        expTags->add(
                            (CompoundTag*)item->getTag()
                ...
                fireTag->putByte(FireworksItem::TAG_FLIGHT,
                                 (uint8_t)sulphurCount);
                itemTag->put(FireworksItem::TAG_FIREWORKS, fireTag);

                resultItem->setTag(itemTag);
            }
            setResultItem(resultItem);
            ret  // create firecharge
                ...
                CompoundTag* itemTag = new CompoundTag();
                CompoundTag* expTag =
                    new CompoundTag(FireworksItem::TAG_EXPLOSION);
                ...
                for (int slot = 0; slot < craftSlots->getContainerSize();
                    ...
                    if (item->id == Item::dye_powder_Id) {
                        colors.push_back(
                            DyePowderItem::COLOR_RGB[item->getAuxValue()]);
            } else if (item->id == Item// glowstone dust gives flickering 
                expTag->putBoolean(FireworksItem::TAG_E_FLICKER, true);
                } else if (item->id == I// diamonds give trails            
        ...
        setResultItem(resultIte// apply fade colors to firecharge 
    if (sulphurCount == 0 && paperCount == 0 && chargeCount == 1 &&
        colorCount > 0 && colorCount == chargeComponents) {
            ...
            if (resultItem != NULL && resultItem->hasTag()) {
                CompoundTag* compound = resultItem->getTag()->getCompound(
                    FireworksItem::TAG_EXPLOSION);
                if (compound == NULL) {
                ...
                compound->putIntArray(FireworksItem::TAG_E_FADECOLORS,
                                      colorArray);
            } else {
                delete colorArray.data;
    ...
    ThreadStorage* tls = (ThreadStorage*)TlsGetValue(tlsI// return resultItem->copy();em->copy();
    }

    int FireworksRecipe::size() { return 10; }

    const ItemInstance* FireworksRecipe::getResultItem() {
    ThreadStorage* tls = (ThreadStorage*)TlsGetValue(tl// return resultItem.get();ltItem.get();
    }
        ...
        *fade = false;
        ...
        for (int slot = 0; slot < craftSlots->getContainerSize(); slot++) {
            ...
            if (item->id == Item::gunpowder_Id) {
                ...
                i  // glowstone dust gives flickering) {

                    chargeComponents++;
        }
        els  // diamonds give trails:diamond_Id) {

            chargeComponents++;
    }
    else  // fireball gives larger explosion) {

        typeComponents++;
}
els  // burstem->id == Item::feather_Id) {

    typeComponents++;
}
else i  // star->id == Item::goldNugget_Id) {

    typeComponents++;
}
e  // creeperem->id == Item::skull_Id) {
...
chargeComponents += colorCount + typeComponents;
    ...
    if (sulphurCount > // create fireworks) {
        return;
    }

    if (paperCount <= 1 && chargeCompo// create firecharge  *firework = true;
    }

    if (sulphurCount <= 1 && colorCount >= 0 && paperCount == 0 &&
        chargeCount == 0 && typeCom// apply fade colors to firechargetrue;
    }
