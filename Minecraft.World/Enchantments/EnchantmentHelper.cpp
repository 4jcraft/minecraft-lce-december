...
void EnchantmentHelper::setEnchantments(
    ...
    if (list->size() > 0) {
        if (item->id != Item::enchantedBook_Id) {
            "ench" em->addTagElement(L, std::list);
        }
    } else if (item->hasTag()) {
        "ench" item->getTag()->remove(L);
    }
}
...
void EnchantmentHelper::runIterationOnInventory(
    EnchantmentIterationMethod& method, ItemI// for (ItemInstance piece : inventory)
    for (unsigned int i = 0; i < inventory.length; ++i) {
...
EnchantmentHelper::GetDamageProtectionIteration
    ...
    if (getDamageProtectionIteration.sum > 25) {
        getDamageProtection  // enchantment protection is on the scale of 0 to
                             // 25, where 20 or more// will nullify nearly
                             // all damage (there will be damage
                             // spill)
    ...
    int EnchantmentHelper::getKnockbackBonus(
        std::shared_ptr<LivingEntity> source,
        std::shared_ptr<LivingEntity> target) {
    ...
    int EnchantmentHelper::getArmorThorns(
        std::shared_ptr<LivingEntity> source) {
    ...
    std::shared_ptr<ItemInstance> EnchantmentHelper::getRandomItemWith(
        ...
        for (unsigned int i = 0; i < items.length; ++i) {
            ...
            if (item != NULL &&
                getEnchantmentLevel(enchantment->id, item) > 0) {
                return item;
                /**
                 *
                 * @param random
                 ...
                 * 0-2
                 * @param bookcases
                 *            How many book cases that are found around
                 * the
                 * table.
                 * @param itemInstance
                 *            Which item that is
                 * being
                 * enchanted.
                 * @return The enchantment cost, 0 means unchantable, 50
                 * is
                 * max.
                 */
...
std::vector<EnchantmentInstance*>* EnchantmentHelper::selectEnchantment(
    Random* random, std::shared_ptr<Item// withdraw bonus from item int enchantmentCost) {
  ...
  // 4J Stu - Update function to 1.3 version for TU7
    ...
    in// the final enchantment cost will have another random span of +- 15%
    ...
    if (availableEnchantments != NULL && !availableEnchantments->empty()) {
                            ...
                            EnchantmentInstance* instance =
                                (EnchantmentInstance*)
                                    WeighedRandom::getRandomItem(random,
                                                                 &values);
                            values.clear();

                            if (instance != NULL) {
            results = new std::vector<EnchantmentInstanc// 4J Stu - Inserting a copy so we can clear           instance->copy());  // memory from the availableEnchantments                                   // collection
                                    

            int bonusChance = realValue// remove incompatible enchantments from previous result {
              // final Iterator<Integer> mapIter =// availableEnchantments.keySet().iterator(); while
         // (mapIter.hasNext())
                
                for (AUTO_VAR(it, availableEnchantments->begin());
                     it != availableEnc// mapIter.next(); {
                    int nextEnchantment = it->first;  // for (EnchantmentInstance *current : results)= true;
            ...
            EnchantmentHelper::getAvailableEnchantmentResults(
                ...
                std::unordered_map < int,
                    EnchantmentIn  // for (Enchantment e :
                ...
                for (int level = e->getMinLevel(); level <= e->getMaxLevel();
                     level++) {
                    if (value >= e->getMinCost(level) &&
                        value <= e->getMaxCost(level)) {
                        if (results == NULL) {
                            results =
                                new std::unordered_map<int,
                                                       EnchantmentInstance*>();
                        }
