...
Enchantment* Enchantment::thorns = NU  // weapon
...
Enchantment* Enchantment::lootBonus = NU  // digger
...
Enchantment* Enchantment::resourceBonus = NU  // bows
...
HtmlString Enchantment::getFullname(int level) {
    ...
    swprintf(formatted, 2 "%ls %ls", app.GetString(getDescriptionId()),
...
bool Enchantment::canEnchant(std::shared_ptr<ItemInstance> item) {
    return category->canEnchant(item->getItem())  // 4J Added
    ...
    return app.GetString(stringI// I18n.get("enchantment.level." + level);
    }