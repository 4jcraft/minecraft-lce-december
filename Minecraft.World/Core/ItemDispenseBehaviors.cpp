...
std::shared_ptr<Projectile> ArrowDispenseBehavior::getProjectile(
    ...
    return /* ThrownEgg */
        ...
        EggDispenseBehavior::getProjectile(Level * world, Position * position) {
    return std::shared_ptr<Projectile>(new ThrownEgg(
        world, position->getX(), position->getY(), position/* Snowball */


std::shared_ptr<Projectile> SnowballDispenseBehavior::getProjectile(
    Level* world, Position* position) {
    return std::shared_ptr<Projectile>(new Snowball(
        world, position->getX(), position->getY(), pos/* Exp Bottle */
}
...
float ExpBottleDispenseBehavior::getPower() {
            return AbstractProjectileDispenseBehavior /* Thrown Potion */ f;
}
...
float ThrownPotionDispenseBehavior::getPower() {
            return AbstractProjectileDispenseBeh /* Potion */ wer() * 1.25f;
}
...
std::shared_ptr<ItemInstance> PotionDispenseBehavior::dispense(
    BlockSource* source, std::shared_ptr<ItemInstance> dispensed) {
        ...
        return DefaultDispenseItemBehavior::dispense/* SpawnEggItem */);
            }
}
...
std::shared_ptr<ItemInstance> SpawnEggDispenseBehavior::execute(
    BlockSource* source, std::shared_ptr<ItemInstance> dispensed,
    eOUTCOME& outcome) {
            FacingEnum* facing = DispenserTile::getFa  // Spawn entity in the
                                                       // middle of the block in
                                                       // front of the dispenser
                double spawnX = source->getX() + facing->getStepX();
            double spawnY  // Above pressure plates.2f;
                double spawnZ = source->getZ() + facing->getStepZ();

            int iResult = 0;
    std::shared_ptr<Entity> entity =
        SpawnEggItem::spawnMobAt(source->getWorld(), dispensed->getAuxValue(),
                                 spawn// 4J-JEV: Added in-case spawn limit is encountered.
    if (entity == NULL) {
                outcome = LEFT_ITEM;
                return dispensed;
    }
    ...
    dispensed->r/* Fireworks*/return dispensed;
}
...
std::shared_ptr<ItemInstance> FireworksDispenseBehavior::execute(
    ...
    eOUTCOME& outcome) {
            ...
            std::shared_ptr<FireworksRocketEntity> firework =
                std::shared_ptr<FireworksRocketEntity>(
                    new FireworksRocketEntity(world, spawnX, spawnY, spawnZ,
                                              dispensed));
...
void FireworksDispenseBehavior::playSound(BlockSource* source,
                     // 4J-JEV: This is exactly the same as the default at the moment.// source->getWorld()->levelEvent(LevelEvent::SOUND_CLICK,// source->getBlockX(), source->getBlockY(), source->getBlockZ(), 0);

    DefaultDispense/* Fireballs */laySound(source, outcome);
    }
    ...
    std::shared_ptr<ItemInstance> FireballDispenseBehavior::execute(
        ...
        world->addEntity(std::shared_ptr<SmallFireball>(new SmallFireball(
            world, spawnX, spawnY, spawnZ, dirX, dirY, dirZ)));
    ...
    void FireballDispenseBehavior::playSound(BlockSource * source,
        ...
        if (outcome == ACTIVATED_ITEM) {
            source->getWorld()->levelEvent(
                LevelEvent::SOUND_BLAZE_FIREBALL, source->getBlockX(),
                source->getBlockY(), source->getBlockZ(), 0);
        } else {
            DefaultDispenseI /* Boats */ ::playSound(source, outcome);
        }
    }
    ...
    std::shared_ptr<ItemInstance> BoatDispenseBehavior::execute(
        BlockSource * source, std::shared_ptr<ItemInstance> dispensed,
        eOUTCOME & outcome) {
    FacingEnum* facing = DispenserTile::getFacing(source->getDat// Spawn the boat 'just' outside the dispenser, it overlaps 2 'pixels' now.
    ...
    Material* inFront = world->getMaterial(f// 4J: If we're at limit, just dispense item (instead of adding boat)
    ...
    void
        BoatDispenseBehav  // 4J-JEV: This is exactly the same as the default at
                           // the moment.//
                           // source->getWorld()->levelEvent(LevelEvent::SOUND_CLICK,
                           // // source->getBlockX(), source->getBlockY(),
                           // source->getBlockZ(),
                           // 0);
            /* FilledBucket */ mBehavior::playSound(source, outcome);
}
...
std::shared_ptr<ItemInstance> FilledBucketDispenseBehavior::execute(
    ...
    return D /* EmptyBucket */ mBehavior::dispense(source, dispensed);
}
...
std::shared_ptr<ItemInstance> EmptyBucketDispenseBehavior::execute(
    ...
    if (--dispensed->count == 0) {
        ...
        DefaultDispenseItemBehavior::dispense(
            source, std::shared_ptr<ItemInstance>(new ItemInstance(targetT/* Flint and Steel */tcome = ACTIVATED_ITEM;
    return dispensed;
    }
    ...
    void FlintAndSteelDispenseBehavior::playSound(BlockSource * source,
        ...
        if (outcome == ACTIVATED_ITEM) {
            source->getWorld()->levelEvent(
                LevelEvent::SOUND_CLICK, source->getBlockX(),
                source->getBlockY(), source->getBlockZ(), 0);
        } else {
            source->getWorld()->levelEvent(LevelEvent::SOUND_CLICK_FAIL,
                                           source->getBlockX(),
                                           source->getBlockY(),
                                           /* Dye */ source->getBlockZ(), 0);
        }
    }
    ...
    std::shared_ptr<ItemInstance> DyeDispenseBehavior::execute(
        ...
        if (dispensed->getAuxValue() == DyePowderItem::WHITE) {
            ...
            if (DyePowderItem::growCrop(dispensed, world, targetX, targetY,
                                        targetZ, false)) {
                if (!world->isClientSide)
                    world->levelEvent(LevelEvent::PARTICLES_PLANT_GROWTH,
            ...
            return DefaultDispenseItemBehavior::execute(source, dispensed,
                                                        outcome);
        }
    }

    void DyeDispenseBehavior::playSound(BlockSource * source,
                                        eOUTCOME outcome) {
        if (outcome == ACTIVATED_ITEM) {
            source->getWorld()->levelEvent(
                LevelEvent::SOUND_CLICK, source->getBlockX(),
    ...
    std::shared_ptr<ItemInstance> TntDispenseBehavior::execute(
        ...
        if (world->newPrimedTntAllowed() &&
            ...
            std::shared_ptr<PrimedTnt> tnt = std::shared_ptr<PrimedTnt>(
                new PrimedTnt(world, targetX + 0.5f, targetY + 0.5f,
                              targetZ + 0.5f, nullptr));
            world->addEntity(tnt);
