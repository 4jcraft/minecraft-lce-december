...
#define MATCHING_EVENT_HANDLER_STACK_SIZE \
...
static const bool sc_voiceChatEnabled =
    false;  //  don't think we'll need voice chat in ad-hoc, will need some work
            //  to get the mesh connections if we do.
...
int (*SQRNetworkManager_AdHoc_Vita::s_SignInCompleteCallbackFn)(void* pParam,
                                                                bool bContinue,
                                                                int pad) = NULL;
void* SQRNetworkManager_AdHoc_Vita::s_SignInCompleteParam = NULL;
sce::Toolkit::NP::PresenceDetails
    SQRNetworkManager_AdHoc_Vita::s_lastPresenceInfo;
...
// SQRNetworkManager_AdHoc_Vita::s_lastInviteIdToRetry =
...
SQRNetworkManager_AdHoc_Vita*
    s_pAdhocVitaManager;  //  have to use a static var for this as the callback
                          //  function doesn't take an arg
static bool s_attemptSignInAdhoc =
    true;  // false if we're trying to sign in to the PSN while in adhoc mode,
           // so we can ignore the error if it fails
...
const SQRNetworkManager_AdHoc_Vita::eSQRNetworkManagerState SQRNetworkManager_AdHoc_Vita::
    m_INTtoEXTStateMappings[SQRNetworkManager_AdHoc_Vita::
                                SNM_INT_STATE_COUNT] = {
...
SQRNetworkManager_AdHoc_Vita::SQRNetworkManager_AdHoc_Vita(
...
#define NP_IN_GAME_MESSAGE_POOL_SIZE (16 * 1024)
    ...
    memcpy(&adhocId.data[0], s_npCommunicationId.data,
...
bool SQRNetworkManager_AdHoc_Vita::IsInitialised() { return m_bIsInitialised; }
...
SceNpMatching2RoomMemberId getRoomMemberID(SceNetInAddr* addr) {
    ...
    // the last 2 digits of the IP address should be unique, so we're using them
    // as a room member ID value
...
bool SQRNetworkManager_AdHoc_Vita::CreateMatchingContext(
    bool bServer /*= false*/) {
    int matchingMode;
    if (bServer) {
    ...
    int ret = sceNetAdhocMatchingCreate(
        matchingMode,  // create this as a client (child) context at first so we
                       // can search for other servers (parents)
        MAX_ONLINE_PLAYER_COUNT, MATCHING_PORT, MATCHING_RXBUFLEN,
        HELLO_INTERVAL, KEEPALIVE_INTERVAL, INIT_COUNT, REXMT_INTERVAL,
    ...
    ret = sceNetAdhocMatchingStart(
        m_matchingContext, SCE_KERNEL_DEFAULT_PRIORITY_USER,
        MATCHING_EVENT_HANDLER_STACK_SIZE,
        SCE_KERNEL_THREAD_CPU_AFFINITY_MASK_DEFAULT, 0,
        NULL);  // sizeof(g_myInfo.name), &g_myInfo.name);
...
// ContextCallback. The SQRNetworkManager_AdHoc_Vita is then finally moved into
...
void SQRNetworkManager_AdHoc_Vita::InitialiseAfterOnline() {
    ...
    // stage (SQRNetworkManager_AdHoc_Vita::Initialise) or we've had a network
    ...
    // callback or SQRNetworkManager_AdHoc_Vita::Initialise) by which this could
    // be called
    ...
    int ret = sceNetAdhocMatchingInit(SCE_NET_ADHOC_MATCHING_POOLSIZE_DEFAULT,
                                      s_Matching2Pool);
...
void SQRNetworkManager_AdHoc_Vita::Tick() {
    ...
    updateNetCheckDialog();  // CD - Added this to match the
                             // SQRNetworkManager_Vita class
    RoomCreateTick();
    FriendSearchTick();
    TickRichPresence();
    // 	TickInviteGUI();  //  TODO

    // to fix the crash when spamming the x button on signing in to PSN, don't
    // bring up all the disconnect stuff till the pause menu disappears
...
void SQRNetworkManager_AdHoc_Vita::CreateAndJoinRoom(int hostIndex,
                                                     int localPlayerMask,
                                                     void* extData,
                                                     int extDataSize,
                                                     bool offline) {
...
void SQRNetworkManager_AdHoc_Vita::UpdateExternalRoomData() {
    ...
    if (m_isHosting) {
        ...
        CPlatformNetworkManagerSony::SetSQRPresenceInfoFromExtData(
            &presenceInfo.m_presenceSyncInfo, m_joinExtData, m_room,
            m_serverId);
        assert(m_joinExtDataSize == sizeof(GameSessionData));
        memcpy(&presenceInfo.m_gameSessionData, m_joinExtData,
               sizeof(GameSessionData));
        memcpy(&presenceInfo.m_roomSyncData, &m_roomSyncData,
               sizeof(RoomSyncData));
        SQRNetworkManager_AdHoc_Vita::UpdateRichPresenceCustomData(
            &presenceInfo, sizeof(HelloSyncInfo));
...
bool SQRNetworkManager_AdHoc_Vita::FriendRoomManagerSearch() {
    ...
    // few seconds
    // 	m_getFriendCountThread = new
    // C4JThread(&GetFriendsThreadProc,this,"GetFriendsThreadProc");
...
bool SQRNetworkManager_AdHoc_Vita::FriendRoomManagerSearch2() {
    ...
    // 			{
    // 				uniqueRooms.insert(
    // m_aFriendSearchResults[i].m_RoomId );
    // 			}
    // 		}
    //
...
// server yet - will be returning SCE_NP_BASIC_ERROR_BUSY in this case
// 		manager->m_friendCount = 0;
// 	}
//
//
...
// 		SceSize numRecieved;
// 		ret = sceNpBasicGetFriendListEntries(0, friendIDs,
...
//
//
// 	// It is possible that the size of the friend list might vary from what
// we just received, so only add in friends that we successfully get an entry
// for 	for( unsigned int i = 0; i < manager->m_friendCount; i++ )
// 	{
...
// &contextState); 		if (ret < 0)
// 		{
// 			app.DebugPrintf("sceNpBasicGetFriendContextState()
// failed: ret = 0x%x\n", ret); 			contextState =
// SCE_NP_BASIC_FRIEND_CONTEXT_STATE_UNKNOWN;
// 		}
// 		if(contextState == SCE_NP_BASIC_FRIEND_CONTEXT_STATE_IN_CONTEXT)
...
// sizeof(SceNpId)); 				result.m_RoomFound = false;
//
// 				// Only include the friend's game if its the
// same network id ( this also filters out generally Zeroed HelloSyncInfo, which
// we do when we aren't in an active game session)
// // 				if( presenceDetails.size ==
// sizeof(HelloSyncInfo) )
// 				{
...
// returns false
void SQRNetworkManager_AdHoc_Vita::FriendRoomManagerGetRoomInfo(
    int idx, SQRNetworkManager_AdHoc_Vita::SessionSearchResult* searchResult) {
    assert(idx < m_aFriendSearchResults.size());
    assert(m_friendSearchState == SNM_FRIEND_SEARCH_STATE_IDLE);

    searchResult->m_NpId = m_aFriendSearchResults[idx].m_NpId;

    ZeroMemory(&searchResult->m_sessionId,
               sizeof(SQRNetworkManager::SessionID));
    searchResult->m_sessionId.m_RoomId =
        m_aFriendSearchResults[idx].m_netAddr.s_addr;
...
SQRNetworkManager_AdHoc_Vita::eSQRNetworkManagerState
SQRNetworkManager_AdHoc_Vita::GetState() {
    return m_stateExternal;
    ;
}

bool SQRNetworkManager_AdHoc_Vita::IsHost() { return m_isHosting; }
...
// has asked to do one of these things (even if the state P hasn't really caught
// up with this request yet), and it also requires that it is informed of the
// state changes leading up to not being in the session, before this should
// report false.
bool SQRNetworkManager_AdHoc_Vita::IsInSession() { return m_isInSession; }
...
SQRNetworkPlayer* SQRNetworkManager_AdHoc_Vita::GetLocalPlayerByUserIndex(
    int idx) {
    EnterCriticalSection(&m_csRoomSyncData);
    for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++) {
        if ((m_roomSyncData.players[i].m_roomMemberId == m_localMemberId) &&
...
SQRNetworkPlayer* SQRNetworkManager_AdHoc_Vita::GetPlayerIfReady(
...
void SQRNetworkManager_AdHoc_Vita::SetState(
    SQRNetworkManager_AdHoc_Vita::eSQRNetworkManagerInternalState state) {
...
void SQRNetworkManager_AdHoc_Vita::ResetToIdle() {
    ...
    if (!m_isHosting) {
        ...
        // we don't get signalling back from the matching libs to destroy
        // connections, so we need to kill everything here
...
// FriendRoomManagerGetCount(). Only valid when FriendRoomManagerIsBusy()
// returns false
bool SQRNetworkManager_AdHoc_Vita::JoinRoom(
    SQRNetworkManager_AdHoc_Vita::SessionSearchResult* searchResult,
    int localPlayerMask) {
    // Set up the presence info we would like to synchronise out when we have
    // fully joined the game
    // 	CPlatformNetworkManagerSony::SetSQRPresenceInfoFromExtData(&s_lastPresenceSyncInfo,
    // searchResult->m_extData, searchResult->m_sessionId.m_RoomId,
    // searchResult->m_sessionId.m_ServerId);
    return JoinRoom(searchResult->m_netAddr, localPlayerMask, NULL);
}

bool SQRNetworkManager_AdHoc_Vita::JoinRoom(SceNpMatching2RoomId roomId,
                                            SceNpMatching2ServerId serverId,
                                            int localPlayerMask,
                                            const PresenceSyncInfo* presence) {
...
bool SQRNetworkManager_AdHoc_Vita::JoinRoom(SceNetInAddr netAddr,
                                            int localPlayerMask,
                                            const HelloSyncInfo* presence) {
    ...
    if (presence) {
        ...
        for (int i = 0; i < m_aFriendSearchResults.size(); i++) {
            if (m_aFriendSearchResults[i].m_netAddr.s_addr == netAddr.s_addr) {
                memcpy(&m_roomSyncData,
                       &m_aFriendSearchResults[i].m_roomSyncData,
    ...
    if (!CreateMatchingContext()) return false;

    int err =
        sceNetAdhocMatchingSelectTarget(m_matchingContext, &netAddr, 0, NULL);
...
void SQRNetworkManager_AdHoc_Vita::LeaveRoom(bool bActuallyLeaveRoom) {
    ...
    if (bActuallyLeaveRoom) {
        if ((m_state == SNM_INT_STATE_HOSTING_CREATE_ROOM_SUCCESS) ||
            ...
            if (!m_isHosting) {
                int ret = sceNetAdhocMatchingCancelTarget(m_matchingContext,
                                                          &m_hostIPAddr);
                if (ret < 0) {
                    app.DebugPrintf(
                        "sceNetAdhocMatchingCancelTarget error :[%d] [%x]\n",
                        ret, ret);
                    assert(0);
                }
            }
            ...
            // 			if( ( ret < 0 ) ||
            // ForceErrorPoint(SNM_FORCE_ERROR_LEAVE_ROOM) )
            // 			{
...
bool SQRNetworkManager_AdHoc_Vita::AddLocalPlayerByUserIndex(int idx) {
    assert(0);  // we should never be adding a local player on Vita, as there is
                // only ever 1 local player
    return false;
}

bool SQRNetworkManager_AdHoc_Vita::RemoveLocalPlayerByUserIndex(int idx) {
    assert(0);  // we should never be removing a local player on Vita, as there
                // is only ever 1 local player
    return false;
}
...
void SQRNetworkManager_AdHoc_Vita::SendInviteGUI() { PSVITA_STUBBED; }

void SQRNetworkManager_AdHoc_Vita::RecvInviteGUI() { PSVITA_STUBBED; }

void SQRNetworkManager_AdHoc_Vita::TickInviteGUI() { PSVITA_STUBBED; }
...
// persists forever.
void SQRNetworkManager_AdHoc_Vita::GetInviteDataAndProcess(
...
void SQRNetworkManager_AdHoc_Vita::FindOrCreateNonNetworkPlayer(
...
void SQRNetworkManager_AdHoc_Vita::LocalDataSend(SQRNetworkPlayer* playerFrom,
...
void SQRNetworkManager_AdHoc_Vita::MapRoomSlotPlayers(
...
bool SQRNetworkManager_AdHoc_Vita::AddRemotePlayersAndSync(
...
void SQRNetworkManager_AdHoc_Vita::RemoveRemotePlayersAndSync(
...
void SQRNetworkManager_AdHoc_Vita::SetLocalPlayersAndSync() {
    ...
    if (IsHost()) m_hostMemberId = m_localMemberId;
...
void SQRNetworkManager_AdHoc_Vita::SyncRoomData() {
    ...
    for (AUTO_VAR(iter, m_RudpCtxToPlayerMap.begin());
         iter != m_RudpCtxToPlayerMap.end(); ++iter) {
        int ctx = iter->first;
        SQRNetworkPlayer* pPlayer = GetPlayerFromRudpCtx(ctx);
        assert(pPlayer);
        sendDataPacket(*GetIPAddrFromRudpCtx(ctx), e_dataTag_RoomSync,
                       &m_roomSyncData, sizeof(m_roomSyncData));
    }
}

void SQRNetworkManager_AdHoc_Vita::MatchingEventHandler(int id, int event,
                                                        SceNetInAddr* peer,
                                                        int optlen, void* opt) {
    ...
    switch (event) {
        case SCE_NET_ADHOC_MATCHING_EVENT_HELLO:
            app.DebugPrintf(
                "P2P SCE_NET_ADHOC_MATCHING_EVENT_HELLO Received!!\n");
            ...
            if (manager->m_isHosting) {
                ...
                if (optlen == sizeof(HelloSyncInfo)) {
                    ...
                    if (pso->m_presenceSyncInfo.netVersion ==
                        MINECRAFT_NET_VERSION) {
                        if (!pso->m_presenceSyncInfo.inviteOnly) {
                            result.m_netAddr = *peer;
                            result.m_RoomFound = true;
                            memcpy(result.m_NpId.handle.data,
                                   pso->m_presenceSyncInfo.hostPlayerUID
                                       .getOnlineID(),
                            ...
                            CPlatformNetworkManagerSony::
                                MallocAndSetExtDataFromSQRPresenceInfo(
                                    &result.m_RoomExtDataReceived,
                                    &pso->m_presenceSyncInfo);
                            result.m_gameSessionData =
                                malloc(sizeof(GameSessionData));
                            memcpy(result.m_gameSessionData,
                                   &pso->m_gameSessionData,
                                   sizeof(GameSessionData));
                            memcpy(&result.m_roomSyncData, &pso->m_roomSyncData,
                                   sizeof(RoomSyncData));
                            // check we don't have this already
                            int currIndex = -1;
                            bool bChanged = false;
                            for (int i = 0;
                                 i < manager->m_aFriendSearchResults.size();
                                 i++) {
                                if (manager->m_aFriendSearchResults[i]
                                        .m_netAddr.s_addr == peer->s_addr) {
                                    currIndex = i;
                                    if (memcmp(
                                            result.m_gameSessionData,
                                            manager->m_aFriendSearchResults[i]
                                                .m_gameSessionData,
                                            sizeof(GameSessionData)) != 0)
                                        bChanged = true;
                                    if (memcmp(
                                            &result.m_roomSyncData,
                                            &manager->m_aFriendSearchResults[i]
                                                 .m_roomSyncData,
                                            sizeof(RoomSyncData)) != 0)
                                        bChanged = true;
                                    if (memcmp(
                                            &result.m_roomSyncData,
                                            &manager->m_aFriendSearchResults[i]
                            ...
                            if (currIndex >= 0 && bChanged)
                                manager->m_aFriendSearchResults.erase(
                                    manager->m_aFriendSearchResults.begin() +
                                    currIndex);
                            if (currIndex < 0 || bChanged)
                                manager->m_aFriendSearchResults.push_back(
                                    result);
                            app.DebugPrintf(
                                "m_aFriendSearchResults playerCount : %d\n",
                                result.m_roomSyncData.players[0].m_playerCount);
                        }
                    } else {
                        app.DebugPrintf(
                            "mismatching net version, expected %d, but got %d "
                            "!!\n",
                            MINECRAFT_NET_VERSION,
                            pso->m_presenceSyncInfo.netVersion);
                    }
                } else {
                    app.DebugPrintf(
                        "Wrong size for HelloSyncInfo, should be %d bytes, but "
                        "was %d bytes!!\n",
        ...
        case SCE_NET_ADHOC_MATCHING_EVENT_REQUEST:  // A join request was
                                                    // received
            app.DebugPrintf(
                "P2P SCE_NET_ADHOC_MATCHING_EVENT_REQUEST Received!!\n");
            if (optlen > 0 && opt != NULL) {
                ret = SCE_OK;  // parentRequestAdd(opt);
                if (ret != SCE_OK) {
                    ret = sceNetAdhocMatchingCancelTarget(
                        manager->m_matchingContext, peer);
                    if (ret < 0) {
                        app.DebugPrintf(
                            "sceNetAdhocMatchingCancelTarget error :[%d] "
                            "[%x]\n",
                            ret, ret);
                        assert(0);
                        break;
            ...
            sceNetAdhocMatchingSelectTarget(manager->m_matchingContext, peer,
                                            sizeof(manager->m_roomSyncData),
                                            &manager->m_roomSyncData);
            break;

        case SCE_NET_ADHOC_MATCHING_EVENT_ACCEPT:  // The join request was
                                                   // accepted
            app.DebugPrintf(
                "P2P SCE_NET_ADHOC_MATCHING_EVENT_ACCEPT Received!!\n");
            if (manager->m_isHosting == false) {
                assert(opt && optlen == sizeof(m_roomSyncData));
                memcpy(&manager->m_roomSyncData, opt,
                       sizeof(manager->m_roomSyncData));
            }
            break;

        case SCE_NET_ADHOC_MATCHING_EVENT_ESTABLISHED:  // A participation
                                                        // agreement was
                                                        // established
        {
            app.DebugPrintf(
                "P2P SCE_NET_ADHOC_MATCHING_EVENT_ESTABLISHED Received!!\n");
            ...
            if (manager->m_isHosting) {
                ...
                bool success = manager->AddRemotePlayersAndSync(
                    getRoomMemberID(peer), 1, &isFull);
                if (success) {
                    bool success2 = manager->CreateRudpConnections(*peer);
                    if (success2) {
                        manager->SyncRoomData();  // have to syn the room data
                                                  // again now we have the peer
                                                  // in our list
                        break;
                    }
                }
                // Something has gone wrong adding these players to the room -
                // kick out the player
                assert(0);
            } else {
                // Local players can establish their UID at this point
            ...
            // 		// If we've created a player, then we want to try and
            // patch up any connections that we should have to it
        ...
        case SCE_NET_ADHOC_MATCHING_EVENT_LEAVE:  // The participation agreement
                                                  // was canceled by the target
                                                  // player
        ...
        case SCE_NET_ADHOC_MATCHING_EVENT_DATA_TIMEOUT:
            ...
            ret = sceNetAdhocMatchingCancelTarget(manager->m_matchingContext,
                                                  peer);
            if (ret < 0) {
                app.DebugPrintf(
                    "sceNetAdhocMatchingCancelTarget error :[%d] [%x]\n", ret,
            ...
            // ->
        case SCE_NET_ADHOC_MATCHING_EVENT_BYE:  // Target participating player
                                                // has stopped matching
        {
            app.DebugPrintf(
                "P2P SCE_NET_ADHOC_MATCHING_EVENT_BYE Received!!\n");

            if (event == SCE_NET_ADHOC_MATCHING_EVENT_BYE &&
                !manager->IsInSession())  //
            {
                // the BYE event comes through like the HELLO event, so even
                // even if we're not connected so make sure we're actually in
                // session
                break;
            }
            ...
            if (manager->m_isHosting) {
                ...
                // too. Finish everything up now, including deleting the server
                // context which belongs to this gaming session This also might
                // be a response to a request to leave the game from our end too
                // so don't need to do anything in that case
                if (manager->m_state != SNM_INT_STATE_LEAVING) {
                    manager->DeleteServerContext();
                }
            } else {
                if (sc_voiceChatEnabled) {
                    // we've lost connection to another client (voice only) so
                    // kill the voice connection no players left on the remote
                    // machine once we remove this one
        ...
        case SCE_NET_ADHOC_MATCHING_EVENT_DATA: {
            app.DebugPrintf(
                "P2P SCE_NET_ADHOC_MATCHING_EVENT_DATA Received!!\n");
        ...
        case SCE_NET_ADHOC_MATCHING_EVENT_DATA_ACK: {
            SQRNetworkPlayer* player =
                manager->GetPlayerFromRudpCtx(peer->s_addr);
            // This event signifies that the previous packet has been sent, so
            // we can try to send some more
            if (player) {
...
// registered callback.
void SQRNetworkManager_AdHoc_Vita::NetworkPlayerConnectionComplete(
    SQRNetworkPlayer* player) {
...
void SQRNetworkManager_AdHoc_Vita::NetworkPlayerSmallIdAllocated(
...
void SQRNetworkManager_AdHoc_Vita::NetworkPlayerInitialDataReceived(
...
void SQRNetworkManager_AdHoc_Vita::NonNetworkPlayerComplete(
    SQRNetworkPlayer* player, unsigned char smallId) {
    player->ConnectionComplete();
    player->SmallIdAllocated(smallId);
}

void SQRNetworkManager_AdHoc_Vita::HandlePlayerJoined(
    SQRNetworkPlayer* player) {
    if (m_listener) {
...
bool SQRNetworkManager_AdHoc_Vita::CreateVoiceRudpConnections(
...
bool SQRNetworkManager_AdHoc_Vita::CreateRudpConnections(SceNetInAddr peer) {
    ...
    if (m_isHosting) {
        m_RudpCtxToPlayerMap[rudpCtx] =
            new SQRNetworkPlayer(this, SQRNetworkPlayer::SNP_TYPE_REMOTE, true,
                                 getRoomMemberID((&peer)), 0, rudpCtx, NULL);
        ...
        m_RudpCtxToPlayerMap[rudpCtx] =
            new SQRNetworkPlayer(this, SQRNetworkPlayer::SNP_TYPE_LOCAL, false,
                                 m_localMemberId, 0, rudpCtx, &localUID);
    ...
    // initiation needs to happen on both client & host sides of the connection
    // to complete.
    ret = sceRudpBind(rudpCtx, m_soc, 1 + 0, SCE_RUDP_MUXMODE_P2P);
    if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_RUDP_BIND)) return false;
    if (ret < 0) app.DebugPrintf(" sceRudpBind failed with error 0x%08x\n");

    ret = sceRudpInitiate(rudpCtx, &sinp2pPeer, sizeof(sinp2pPeer), 0);
    if (ret < 0) app.DebugPrintf(" sceRudpInitiate failed with error 0x%08x\n");
    if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_RUDP_INIT2)) return false;

    return true;
}

SQRNetworkPlayer* SQRNetworkManager_AdHoc_Vita::GetPlayerFromRudpCtx(
    int rudpCtx) {
...
SQRNetworkPlayer*
SQRNetworkManager_AdHoc_Vita::GetPlayerFromRoomMemberAndLocalIdx(int roomMember,
                                                                 int localIdx) {
    for (AUTO_VAR(it, m_RudpCtxToPlayerMap.begin());
...
bool SQRNetworkManager_AdHoc_Vita::RegisterCallbacks() {
    ...
    // 	if (ret < 0)
    // 	{
    // 		app.DebugPrintf("SQRNetworkManager::RegisterCallbacks -
    ...
    // SignallingCallback, this); 	if (ret < 0)
    // 	{
    // 		return false;
    // 	}
    //
    ...
    // RoomEventCallback, this); 	if (ret < 0)
    // 	{
    // 		app.DebugPrintf("SQRNetworkManager::RegisterCallbacks -
...
void SQRNetworkManager_AdHoc_Vita::HandleMatchingContextStart() {
    // on the standard networking model this happens during a callback signalled
    // from the context starting Some special cases to detect when this event is
    // coming in, in case we had to start the matching context because there
    // wasn't a valid context when we went to get a server context. These two
    // responses here complete what should then happen to get the server context
    // in each case (for hosting or joining a game)
    if (m_state == SNM_INT_STATE_IDLE_RECREATING_MATCHING_CONTEXT) {
        ...
        // no world matching stuff to setup here, we can just signal that we're
        // ready
        SetState(SNM_INT_STATE_HOSTING_WAITING_TO_PLAY);
        // Now we know the local member id we can update our local players
        SetLocalPlayersAndSync();
...
int SQRNetworkManager_AdHoc_Vita::BasicEventCallback(int event, int retCode,
                                                     uint32_t reqId,
                                                     void* arg) {
    PSVITA_STUBBED;
    // 	SQRNetworkManager_AdHoc_Vita *manager = (SQRNetworkManager_AdHoc_Vita
...
void SQRNetworkManager_AdHoc_Vita::OnlineCheck() {
    ...
    bool bConnected =
        (state ==
         SCE_NET_CTL_STATE_IPOBTAINED);  // ProfileManager.IsSignedInLive(ProfileManager.GetPrimaryPad());
...
void SQRNetworkManager_AdHoc_Vita::SysUtilCallback(uint64_t status,
                                                   uint64_t param,
                                                   void* userdata) {
    // 	SQRNetworkManager_AdHoc_Vita *manager = (SQRNetworkManager_AdHoc_Vita
    // *)userdata;
    ...
    if (ProfileManager.IsSystemUIDisplayed()) {
        if (sceNetCheckDialogGetStatus() == SCE_COMMON_DIALOG_STATUS_FINISHED) {
            ...
            if (netCheckResult.result == SCE_COMMON_DIALOG_RESULT_OK) {
                ...
                if (s_attemptSignInAdhoc)  // don't fail if it was an attempted
                                           // PSN signin
                {
                    SetState(SNM_INT_STATE_INITIALISE_FAILED);
                }
...
void SQRNetworkManager_AdHoc_Vita::RudpContextCallback(int ctx_id, int event_id,
                                                       int error_code,
                                                       void* arg) {
    ...
    switch (event_id) {
        ...
        case SCE_RUDP_CONTEXT_EVENT_WRITABLE: {
            ...
            if (player) {
                ...
                if (sc_voiceChatEnabled) {
                    SQRVoiceConnection* pVoice =
                        SonyVoiceChat_Vita::GetVoiceConnectionFromRudpCtx(
                            ctx_id);
                    assert(pVoice);
                }
            }
        ...
        case SCE_RUDP_CONTEXT_EVENT_READABLE:
            if (manager->m_listener) {
                ...
                if (pVoice) {
                    ...
                    // small id confirmed, then the first uint8_t sent to us
                    // should be this id
                    if (manager->m_isHosting) {
...
int SQRNetworkManager_AdHoc_Vita::RudpEventCallback(
...
SQRNetworkManager_AdHoc_Vita::GetSceNpCommsSig() {
...
int SQRNetworkManager_AdHoc_Vita::GetOldMask(
...
void SQRNetworkManager_AdHoc_Vita::GetExtDataForRoom(
    ...
    for (int i = 0; i < m_aFriendSearchResults.size(); i++) {
        if (m_aFriendSearchResults[i].m_netAddr.s_addr == roomId) {
            memcpy(extData, m_aFriendSearchResults[i].m_gameSessionData,
                   sizeof(GameSessionData));
        }
    }
...
void SQRNetworkManager_AdHoc_Vita::AttemptAdhocSignIn(
    ...
    app.DebugPrintf(
        "s_SignInCompleteCallbackFn - 0x%08x : s_SignInCompleteParam - "
        "0x%08x\n",
        (unsigned int)s_SignInCompleteCallbackFn,
        (unsigned int)s_SignInCompleteParam);
    SceNetCheckDialogParam param;
    memset(&param, 0x00, sizeof(param));
    sceNetCheckDialogParamInit(&param);

    s_attemptSignInAdhoc = true;  // so we know which sign in we're trying to
                                  // make in the netCheckUpdate
    ...
    app.DebugPrintf(
        "------------>>>>>>>>   sceNetCheckDialogInit : Adhoc Mode\n");
...
void SQRNetworkManager_AdHoc_Vita::AttemptPSNSignIn(
    int (*SignInCompleteCallbackFn)(void* pParam, bool bContinue, int pad),
    void* pParam, bool callIfFailed /*=false*/) {
    s_SignInCompleteCallbackFn = SignInCompleteCallbackFn;
    s_signInCompleteCallbackIfFailed = callIfFailed;
    ...
    s_attemptSignInAdhoc = false;  // so we know which sign in we're trying to
                                   // make in the netCheckUpdate

    // -------------------------------------------------------------
    // MGH -  this code is duplicated in the PSN network manager now too, so any
...
void SQRNetworkManager_AdHoc_Vita::UpdateRichPresenceCustomData(
    void* data, unsigned int dataBytes) {
    if (m_isHosting == false) return;
    if (m_matchingContextServerValid)  // if the wifi connection has dropped
                                       // (say after the console has woken from
                                       // sleep) this can be invalid
    {
        int err =
            sceNetAdhocMatchingSetHelloOpt(m_matchingContext, dataBytes, data);
        assert(err == SCE_OK);
    } else {
        app.DebugPrintf(
            "UpdateRichPresenceCustomData failed, m_matchingContextServerValid "
            "== false\n");
    }
...
void SQRNetworkManager_AdHoc_Vita::SendLastPresenceInfo() {
    ...
    // 		s_lastPresenceInfo.presenceType =
    // SCE_TOOLKIT_NP_PRESENCE_STATUS;
    // 	}
...
void SQRNetworkManager_AdHoc_Vita::SetPresenceDataStartHostingGame() {
    if (m_offlineGame) {
        SQRNetworkManager_AdHoc_Vita::UpdateRichPresenceCustomData(
            &c_presenceSyncInfoNULL, sizeof(HelloSyncInfo));
    } else {
        HelloSyncInfo presenceInfo;
        CPlatformNetworkManagerSony::SetSQRPresenceInfoFromExtData(
            &presenceInfo.m_presenceSyncInfo, m_joinExtData, m_room,
...
void SQRNetworkManager_AdHoc_Vita::removePlayerFromVoiceChat(
    SQRNetworkPlayer* pPlayer) {
    if (!sc_voiceChatEnabled) return;
...
int SQRNetworkManager_AdHoc_Vita::sendDataPacket(SceNetInAddr addr,
                                                 EAdhocDataTag tag, void* data,
                                                 int dataSize) {
    ...
    int ret = sceNetAdhocMatchingSendData(m_matchingContext, &addr,
                                          dataSize + 4, s_dataBuffer);
    // 	assert(ret == SCE_OK);
    return ret;
}

int SQRNetworkManager_AdHoc_Vita::sendDataPacket(SceNetInAddr addr, void* data,
                                                 int dataSize) {
    int ret =
        sceNetAdhocMatchingSendData(m_matchingContext, &addr, dataSize, data);
    // 	assert(ret == SCE_OK);
    return ret;
}
...
SQRNetworkPlayer* SQRNetworkManager_AdHoc_Vita::GetPlayerByXuid(
    PlayerUID xuid) {
    EnterCriticalSection(&m_csRoomSyncData);
    for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++) {
