...
enum EAdhocDataTag { e_dataTag_Normal, e_dataTag_RoomSync };
...
class SQRNetworkManager_AdHoc_Vita : public SQRNetworkManager {
    ...
    static const eSQRNetworkManagerState
        m_INTtoEXTStateMappings[SNM_INT_STATE_COUNT];
...
public:
    ...
    // Session management
    void CreateAndJoinRoom(int hostIndex, int localPlayerMask, void* extData,
                           int extDataSize, bool offline);
    ...
    void FriendRoomManagerGetRoomInfo(int idx,
                                      SessionSearchResult* searchResult);
    bool JoinRoom(SessionSearchResult* searchResult, int localPlayerMask);
    bool JoinRoom(SceNetInAddr netAddr, int localPlayerMask,
                  const HelloSyncInfo* presence);
    bool JoinRoom(SceNpMatching2RoomId roomId, SceNpMatching2ServerId serverId,
                  int localPlayerMask, const PresenceSyncInfo* presence);
    void StartGame();
    ...
    // 	void
    // GetInviteDataAndProcess(SceNpBasicAttachmentDataId id);
    static bool UpdateInviteData(HelloSyncInfo* invite);
    void GetExtDataForRoom(SceNpMatching2RoomId roomId, void* extData,
    ...
    static void GetInviteDataAndProcess(
        sce::Toolkit::NP::MessageAttachment* pInvite);
    static bool GetAdhocStatus() { return m_adhocStatus; }

    int sendDataPacket(SceNetInAddr addr, EAdhocDataTag tag, void* data,
                       int dataSize);
...
private:
    ...
    int m_localPlayerJoined;  // Client only, keep a count of how many local
                              // players we have confirmed as joined to the
                              // application
    ...
    // 	HelloSyncInfo
    // m_inviteReceived[MAX_SIMULTANEOUS_INVITES]; 	int
    // m_inviteIndex; 	static HelloSyncInfo
    // *m_gameBootInvite; 	static HelloSyncInfo
    // m_gameBootInvite_data; 	bool
    // m_doBootInviteCheck;
    bool m_isInSession;
    // 	static SceNpBasicAttachmentDataId
    // s_lastInviteIdToRetry;
...
private:
    ...
    SQRNetworkPlayer*
        m_aRoomSlotPlayers[MAX_ONLINE_PLAYER_COUNT];  // Maps from the players
                                                      // in m_roomSyncData, to
                                                      // SQRNetworkPlayers
    void FindOrCreateNonNetworkPlayer(int slot, int playerType,
    ...
    void LocalDataSend(SQRNetworkPlayer* playerFrom, SQRNetworkPlayer* playerTo,
                       const void* data, unsigned int dataSize);
    int GetSessionIndex(SQRNetworkPlayer* player);

    bool AddRemotePlayersAndSync(SceNpMatching2RoomMemberId memberId,
                                 int playerMask, bool* isFull = NULL);
    void RemoveRemotePlayersAndSync(SceNpMatching2RoomMemberId memberId,
                                    int mask);
    ...
    void NetworkPlayerSmallIdAllocated(SQRNetworkPlayer* player,
                                       unsigned char smallId);
    void NetworkPlayerInitialDataReceived(SQRNetworkPlayer* player, void* data);
    void NonNetworkPlayerComplete(SQRNetworkPlayer* player,
                                  unsigned char smallId);
    ...
    // 	C4JThread
    // *m_getFriendCountThread; 	static int
    // GetFriendsThreadProc( void* lpParameter );
    ...
    // Results from searching for rooms that friends are playing in - 5 matched
    // arrays to store their NpIds, rooms, servers, whether a room was found,
    // and whether the external data had been received for the room. Also a
    // count of how many elements are used in this array.
    class FriendSearchResult {
    public:
        SceNpId m_NpId;
        SceNetInAddr m_netAddr;
        // 		SceNpMatching2RoomId
        // m_RoomId; 		SceNpMatching2ServerId
        // m_ServerId;
        bool m_RoomFound;
    ...
    std::unordered_map<SceNetInAddr_t, SQRVoiceConnection*>
        m_NetAddrToVoiceConnectionMap;

    bool CreateRudpConnections(SceNetInAddr peer);
    bool CreateVoiceRudpConnections(SceNpMatching2RoomId roomId,
    ...
    SQRNetworkPlayer* GetPlayerFromRoomMemberAndLocalIdx(int roomMember,
                                                         int localIdx);
    ...
    // #ifdef __PS3__
    // 	static void
    // DefaultRequestCallback(SceNpMatching2ContextId id,
    // SceNpMatching2RequestId reqId, SceNpMatching2Event event,
    // SceNpMatching2EventKey eventKey, int errorCode, std::size_t dataSize,
    // void *arg); 	static void
    // RoomEventCallback(SceNpMatching2ContextId id, SceNpMatching2RoomId
    // roomId, SceNpMatching2Event event, SceNpMatching2EventKey eventKey, int
    // errorCode, std::size_t dataSize, void *arg); #else
    // 	static void
    // DefaultRequestCallback(SceNpMatching2ContextId id,
    // SceNpMatching2RequestId reqId, SceNpMatching2Event event, int errorCode,
    // const void *data, void *arg); 	static void
    // RoomEventCallback(SceNpMatching2ContextId id, SceNpMatching2RoomId
    // roomId, SceNpMatching2Event event, const void *data, void *arg); #endif
    // 	static void
    // SignallingCallback(SceNpMatching2ContextId ctxId, SceNpMatching2RoomId
    // roomId, SceNpMatching2RoomMemberId peerMemberId, SceNpMatching2Event
    // event, int error_code, void *arg);

    // Callback for NpBasic
    static int BasicEventCallback(int event, int retCode, uint32_t reqId,
                                  void* arg);
    ...
    static void SysUtilCallback(uint64_t status, uint64_t param,
                                void* userdata);
    void updateNetCheckDialog();  // get the status of the dialog and run any
                                  // callbacks needed [CD - Added to match
                                  // SQRNetworkManager_Vita]

    // Callbacks for rudp
    static void RudpContextCallback(int ctx_id, int event_id, int error_code,
                                    void* arg);
    static int RudpEventCallback(int event_id, int soc, uint8_t const* data,
                                 std::size_t datalen,
    ...
    static void MatchingEventHandler(int id, int event, SceNetInAddr* peer,
                                     int optlen, void* opt);

public:
    static void AttemptPSNSignIn(int (*SignInCompleteCallbackFn)(void* pParam,
                                                                 bool bContinue,
                                                                 int pad),
                                 void* pParam, bool callIfFailed = false);
    static void AttemptAdhocSignIn(
        int (*SignInCompleteCallbackFn)(void* pParam, bool bContinue, int pad),
        void* pParam, bool callIfFailed = false);
    static int (*s_SignInCompleteCallbackFn)(void* pParam, bool bContinue,
                                             int pad);
