...
SQRLocalVoiceDevice
    SonyVoiceChat_Vita::m_localVoiceDevices[MAX_LOCAL_PLAYER_COUNT];
uint32_t SonyVoiceChat_Vita::m_voiceOutPort;
bool SonyVoiceChat_Vita::m_forceSendPacket =
    false;  // force a packet across the network, even if there's no data, so we
            // can update flags
RingBuffer SonyVoiceChat_Vita::m_recordRingBuffer(sc_ringBufferSize);
VoicePacket::Flags
    SonyVoiceChat_Vita::m_localPlayerFlags[MAX_LOCAL_PLAYER_COUNT];
...
void LoadPCMVoiceData() {
    for (int i = 0; i < 4; i++) {
        ...
        if (g_loadedPCMVoiceDataSizes[i] != 0) {
            ...
            DWORD bytesRead;
            BOOL bSuccess =
                ReadFile(file, g_loadedPCMVoiceData[i],
                         g_loadedPCMVoiceDataSizes[i], &bytesRead, NULL);
            assert(bSuccess);
        }
...
void SonyVoiceChat_Vita::init() {
    ...
    if (returnCode < 0) {
        app.DebugPrintf(
            "Error: sceSysmoduleLoadModule(SCE_SYSMODULE_VOICE), ret 0x%08x\n",
            returnCode);
        assert(0);
    }
    ...
    // 	LoadPCMVoiceData();
    initLocalPlayer(0);
    connectPorts(m_localVoiceDevices[0].m_microphonePort,
                 m_localVoiceDevices[0].m_headsetPort);
...
void SonyVoiceChat_Vita::shutdown() {
    ...
    if (returnCode < 0) {
        app.DebugPrintf(
            "Error: sceSysmoduleUnloadModule(SCE_SYSMODULE_VOICE), ret "
            "0x%08x\n",
            returnCode);
        assert(0);
    }
...
void SonyVoiceChat_Vita::start() {
    if (m_bVoiceStarted == false) {
        startParam.container = sceKernelAllocMemBlock(
            "SceUserVoiceEvent", SCE_KERNEL_MEMBLOCK_TYPE_USER_RWDATA,
            SCE_VOICE_MEMORY_CONTAINER_SIZE, SCE_NULL);
        int err;
...
void SQRVoiceConnection::readRemoteData() {
    ...
    if (dataSize > 0) {
        ...
        unsigned int bytesRead =
            sceRudpRead(m_rudpCtx, &packet, dataSize, 0, NULL);
    ...
    if (g_loadedPCMVoiceDataPos[0] + readSize < g_loadedPCMVoiceDataSizes[0]) {
        for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT; i++) {
            if (m_localVoiceDevices[i].isValid()) {
                result = sceVoiceWriteToIPort(
                    m_localVoiceDevices[i].m_microphonePort,
                    &g_loadedPCMVoiceData[0][g_loadedPCMVoiceDataPos[0]],
                    &readSize, 0);
            }
        }
    }
...
void SonyVoiceChat_Vita::sendAllVoiceData() {
    ...
    for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT; i++) {
        if (m_localVoiceDevices[i].isValid()) {
            ...
            ProfileManager.GetChatAndContentRestrictions(
                i, true, &bChatRestricted, NULL, NULL);
            ...
            SceVoiceBasePortInfo portInfo;
            int result = sceVoiceGetPortInfo(
                m_localVoiceDevices[i].m_microphonePort, &portInfo);
            assert(result == SCE_OK);
            switch (portInfo.state) {
                case SCEVOICE_PORTSTATE_READY:
                case SCEVOICE_PORTSTATE_BUFFERING:
        ...
        if (m_localPlayerFlags[i].m_bHasMicConnected !=
                lastPlayerFlags[i].m_bHasMicConnected ||
    ...
    outputPortBytes = (outputPortBytes > sizeof(packetToSend.m_data))
                          ? sizeof(packetToSend.m_data)
                          : outputPortBytes;
    if (outputPortBytes || flagsChanged || m_forceSendPacket) {
        frameSendIndex += lastReadFrameCnt;
        if (outputPortBytes) {
            readSize = outputPortBytes;
            result = sceVoiceReadFromOPort(m_voiceOutPort, packetToSend.m_data,
                                           &readSize);
            if (result != SCE_OK) {
        ...
        for (int rIdx = 0; rIdx < m_remoteConnections.size(); rIdx++) {
            ...
            if (pVoice->m_bConnected)
                m_remoteConnections[rIdx]->SendInternal(&packetToSend,
                                                        packetSize);
        }
...
void SonyVoiceChat_Vita::playAllReceivedData() {
    ...
    for (int rIdx = 0; rIdx < m_remoteConnections.size(); rIdx++) {
        SQRVoiceConnection* pVoice = m_remoteConnections[rIdx];
        VoicePacket packet;
        while (pVoice->getNextPacket(
            packet))  // MGH -  changed to a while loop, so all the packets are
                      // sent to the voice port, and it can handle delayed
                      // packets due to the size of it's internal buffer
        {
            int frameGap;
            if (pVoice->m_nextExpectedFrameIndex == packet.m_frameSendIndex) {
                ...
                pVoice->m_nextExpectedFrameIndex =
                    packet.m_frameSendIndex + packet.m_numFrames;
            } else if (pVoice->m_nextExpectedFrameIndex <
                       packet.m_frameSendIndex) {
                // has voice frame drop, dropped forwarding frames
                frameGap =
                    packet.m_frameSendIndex - pVoice->m_nextExpectedFrameIndex;
                if (sc_verbose)
                    app.DebugPrintf(
                        "index@%d gets dropped forwarding frames %d\n",
                        pVoice->m_nextExpectedFrameIndex, frameGap);
                pVoice->m_nextExpectedFrameIndex =
                    packet.m_frameSendIndex + packet.m_numFrames;
            } else if (pVoice->m_nextExpectedFrameIndex >
                       packet.m_frameSendIndex) {
                // has voice frame drop, dropped preceding frames, no reset on
                // pVoice->m_nextExpectedFrameIndex
                frameGap =
                    packet.m_frameSendIndex - pVoice->m_nextExpectedFrameIndex;
                if (sc_verbose)
            ...
            if (result != SCE_OK) {
                if (sc_verbose)
                    app.DebugPrintf(
                        "sceVoiceGetPortInfo LoopbackVoiceInPort failed %x\n",
                        result);
                assert(0);
                LeaveCriticalSection(&m_csRemoteConnections);
                return;
            ...
            inputPortBytes =
                (inputPortBytes > writeSize) ? writeSize : inputPortBytes;
            writeSize = inputPortBytes;
            result = sceVoiceWriteToIPort(pVoice->m_voiceInPort, packet.m_data,
                                          &writeSize, frameGap);
            if (result != SCE_OK) {
                if (sc_verbose)
            ...
            // copy the flags
            for (int flagIndex = 0; flagIndex < MAX_LOCAL_PLAYER_COUNT;
                 flagIndex++)
                pVoice->m_remotePlayerFlags[flagIndex] =
                    packet.m_localPlayerFlags[flagIndex];
        }
    }
    LeaveCriticalSection(&m_csRemoteConnections);
}
    ...
    if (pNetPlayer->IsLocal()) {
        ...
        for (int i = 0; i < m_remoteConnections.size(); i++) {
            ...
            if (pVoice->m_remoteRoomMemberId == pNetPlayer->m_roomMemberId) {
                bool bMicConnected =
                    pVoice
                        ->m_remotePlayerFlags[pNetPlayer->GetLocalPlayerIndex()]
                        .m_bHasMicConnected;
                LeaveCriticalSection(&m_csRemoteConnections);
...
void SonyVoiceChat_Vita::mutePlayer(
    const SceNpMatching2RoomMemberId member_id,
    bool bMute) /*Turn chat audio from a specified player on or off */
{}

void SonyVoiceChat_Vita::muteLocalPlayer(
    bool bMute) /*Turn microphone input on or off */
{}

bool SonyVoiceChat_Vita::isMuted() { return false; }

bool SonyVoiceChat_Vita::isMutedPlayer(const PlayerUID& memberUID) {
    return false;
}

bool SonyVoiceChat_Vita::isMutedLocalPlayer() { return false; }
...
void SQRLocalVoiceDevice::init(bool bChatRestricted) {
    ...
    portArgs.volume = 1.0f;
    portArgs.device.playerId = 0;
    // 	portArgs.device.type			   =
    // SCE_AUDIO_OUT_PORT_TYPE_VOICE; 	portArgs.device.index = 0;
...
SQRVoiceConnection* SonyVoiceChat_Vita::addRemoteConnection(
    ...
    m_forceSendPacket =
        true;  // new connection, so we'll force a packet through for the flags
    LeaveCriticalSection(&m_csRemoteConnections);
...
void SonyVoiceChat_Vita::connectPorts(uint32_t inPort, uint32_t outPort) {
    ...
    if (returnCode != SCE_OK) {
        app.DebugPrintf(
            "sceVoiceConnectIPortToOPort failed (0x%08x), inPort 0x%08x, "
            "outPort 0x%08x\n",
...
void SonyVoiceChat_Vita::disconnectPorts(uint32_t inPort, uint32_t outPort) {
    ...
    if (returnCode != SCE_OK) {
        app.DebugPrintf(
            "sceVoiceDisconnectIPortFromOPort failed (0x%08x), inPort 0x%08x, "
            "outPort 0x%08x\n",
            returnCode, inPort, outPort);
        assert(0);
    }
            ...
            for (int j = 0; j < MAX_LOCAL_PLAYER_COUNT; j++) {
                ...
                if ((pConnectFrom != pConnectTo) && pConnectTo->isValid()) {
                    if (pConnectFrom->m_localConnections[j] == false) {
                        if (pConnectTo->m_bChatRestricted == false &&
                            pConnectFrom->m_bChatRestricted == false) {
                            connectPorts(pConnectFrom->m_microphonePort,
                                         pConnectTo->m_headsetPort);
        ...
        for (int j = 0; j < MAX_LOCAL_PLAYER_COUNT; j++) {
            if (pConnectedFrom->m_localConnections[j] == true) {
                ...
                if (i == playerIdx || j == playerIdx) {
                    if (pConnectedTo->m_bChatRestricted == false &&
                        pConnectedFrom->m_bChatRestricted == false) {
                        disconnectPorts(pConnectedFrom->m_microphonePort,
                                        pConnectedTo->m_headsetPort);
...
void SonyVoiceChat_Vita::initLocalPlayer(int playerIndex) {
    if (m_localVoiceDevices[playerIndex].isValid() == false) {
        ...
        if (m_localVoiceDevices[playerIndex].m_bChatRestricted == false) {
            connectPorts(m_localVoiceDevices[playerIndex].m_microphonePort,
                         m_voiceOutPort);
        }
        m_forceSendPacket = true;  // new local device, so we'll force a packet
                                   // through for the flags
    }
...
void SonyVoiceChat_Vita::connectPlayer(SQRVoiceConnection* pConnection,
    ...
    if ((pConnection->m_headsetConnectionMask & (1 << playerIndex)) == 0) {
        initLocalPlayer(
            playerIndex);  // added this as we can get a client->client
                           // connection coming in first, and the network player
                           // hasn't been created yet (so this hasn't been
                           // initialised)
        if (m_localVoiceDevices[playerIndex].m_bChatRestricted == false) {
            connectPorts(pConnection->m_voiceInPort,
                         m_localVoiceDevices[playerIndex].m_headsetPort);
        }
        ...
        m_forceSendPacket = true;  // new connection, so we'll force a packet
                                   // through for the flags
    }
}

SQRVoiceConnection* SonyVoiceChat_Vita::GetVoiceConnectionFromRudpCtx(
    int RudpCtx) {
    for (int i = 0; i < m_remoteConnections.size(); i++) {
...
SQRVoiceConnection* SonyVoiceChat_Vita::getVoiceConnectionFromRoomMemberID(
...
void SonyVoiceChat_Vita::disconnectLocalPlayer(int localIdx) {
    if (m_localVoiceDevices[localIdx].isValid() == false) return;

    EnterCriticalSection(&m_csRemoteConnections);

    if (m_localVoiceDevices[localIdx].m_bChatRestricted == false) {
        disconnectPorts(m_localVoiceDevices[localIdx].m_microphonePort,
                        m_voiceOutPort);

        for (int i = 0; i < m_remoteConnections.size(); i++) {
            disconnectPorts(m_remoteConnections[i]->m_voiceInPort,
                            m_localVoiceDevices[localIdx].m_headsetPort);
            m_remoteConnections[i]->m_headsetConnectionMask &=
                (~(1 << localIdx));
    ...
    if (m_numLocalDevicesConnected ==
        0)  // no more local players, kill all the remote connections
    {
        for (int i = 0; i < m_remoteConnections.size(); i++) {
            delete m_remoteConnections[i];
        }
...
void SonyVoiceChat_Vita::disconnectRemoteConnection(
    SQRVoiceConnection* pVoice) {
    EnterCriticalSection(&m_csRemoteConnections);

    int voiceIdx = -1;
    for (int i = 0; i < m_remoteConnections.size(); i++) {
        if (m_remoteConnections[i] == pVoice) voiceIdx = i;
    }
    assert(voiceIdx >= 0);
    if (voiceIdx >= 0) {
...
int RingBuffer::Write(char* data, int len_) {
    ...
    if (data_size != len) memcpy(buffer, data + data_size, len - data_size);
    buf_free += len;
    return len;
}
...
int RingBuffer::Read(char* data, int max_bytes_) {
    ...
    if (chunk > result) chunk = result;
    memcpy(data, buffer + (buf_full % buf_size), chunk);
    if (chunk != result) memcpy(data + chunk, buffer, result - chunk);
    buf_full += result;
    return result;
}
