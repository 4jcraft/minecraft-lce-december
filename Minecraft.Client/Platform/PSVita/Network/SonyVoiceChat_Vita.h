...
public:
    ...
    static int getPacketSize(int dataSize) {
        return (uint64_t)&((VoicePacket*)0)->m_data[dataSize];
    }
    void setChecksum(int dataSize) {
        m_checkSum = 0;
        for (int i = 0; i < dataSize; i++) m_checkSum += m_data[i];
    }
    bool verifyData(int packetSize, int frameSize) {
        if (m_numFrames == 0) return false;
        int dataSize = m_numFrames * frameSize;
        if (packetSize != getPacketSize(dataSize)) return false;
        int checkSum = 0;
        for (int i = 0; i < dataSize; i++) checkSum += m_data[i];
        if (checkSum != m_checkSum) return false;
        return true;
    }
};
...
public:
    ...
    uint8_t m_localConnections[4];  // connection between this devices mic and
                                    // other local player's headsets
...
public:
    ...
    {
        for (int i = 0; i < 4; i++) m_localConnections[i] = 0;
    }
...
public:
    int m_rudpCtx;
    bool m_bConnected;
    uint32_t m_voiceInPort;  // 1 input port per connection, incoming UDP
                             // packets are written to each of these, and then
                             // they're connected out to all headsets
    int m_headsetConnectionMask;  // 1 bit per player, if the headset connection
                                  // has been made
    RingBuffer m_playRingBuffer;
    SceNpMatching2RoomMemberId
        m_remoteRoomMemberId;  // Assigned by Matching2 lib, we can use to
    ...
    SQRVoiceConnection(int rudpCtx,
                       SceNpMatching2RoomMemberId remoteRoomMemberId);
    ~SQRVoiceConnection();
...
public:
    ...
    static void muteLocalPlayer(bool bMute);  // Turn microphone input on or
                                              // off;
    ...
    static SQRVoiceConnection* addRemoteConnection(
    ...
    static SQRVoiceConnection* getVoiceConnectionFromRoomMemberID(
...
private:
    ...
    static uint32_t
        m_voiceOutPort;  // single output port that all local devices are mixed
                         // to, and then sent out to all other remote machines
    ...
    static bool
        m_forceSendPacket;  // force a packet across the network, even if
                            // there's no data, so we can update flags
