	...
	if( m_friendSearchState == SNM_FRIEND_SEARCH_STATE_GETTING_FRIEND_COUNT )
	{
        ...
        int SQRNetworkManager_Vita::BasicEventThreadProc(void* lpParameter) {
            ...
            return  // 	SQRNetworkManager_Vita *manager =
                    ...
                    // 	int ret = SCE_OK;  // 	SceKernelEvent event;  // 	int
            // outEv;  // 	do  // 	{  // 		ret =
            ...
            return 0  // Get count of rooms that friends are playing in. Only
                ...
                return m_aFriendSearchResults
                    ...
                    searchResult->m_sessionId.m_RoomId =
                        m_aFriendSearchResults[idx].m_RoomId;
                    ...
                    bool SQRNetworkManager_Vita::IsReadyToPlayOrIdle() {
                        return (
                            ...
                            (m_state == SNM_INT_STATE_PLAYING) ||
                            (m_state ==
                             SNM_INT_STATE_IDLE));  // Consider as "in session"
                                                    // from the moment that a
                                                    // game is created or
                                                    // joined, until the point
                        ...
                        bool SQRNetworkManager_Vita::IsInSession() {
                            return m_isInSession  // Get count of players
                                ...
                                return m_roomSyncData
                                    ...
                                    int onlineCount = 0;
                                    for (int i = 0;
                                         i < m_roomSyncData.getPlayerCount();
                                         i++) {
                                ...
                                SQRNetworkPlayer*
                                SQRNetworkManager_Vita::GetPlayerByIndex(
                                    int idx) {
                                    if (idx < MAX_ONLINE_PLAYER_COUNT) {
                                        return GetPlayerIfReady(
                                            m_aRoomSlotPlayers[idx]);
                                    } else {
                                        return NULL;
                                    }
                                }
                                ...
                                SQRNetworkPlayer*
                                SQRNetworkManager_Vita::GetPlayerBySmallId(
                                    int idx) {
                                ...
                                SQRNetworkPlayer* SQRNetworkManager_Vita::
                                    GetLocalPlayerByUserIndex(int idx) {
                                    EnterCriticalSection(&m_csRoomSyncData);
                                    for (int i = 0;
                                         i < m_roomSyncData.getPlayerCount();
                                         i++) {
                                ...
                                SQRNetworkPlayer*
                                SQRNetworkManager_Vita::GetHostPlayer() {
                                    EnterCriticalSection(&m_csRoomSyncData);
                                    SQRNetworkPlayer* player =
                                        GetPlayerIfReady(m_aRoomSlotPlayers[0]);
                                    LeaveCriticalSection(&m_csRoomSyncData);
                                    return player;
                                }

                                SQRNetworkPlayer*
                                SQRNetworkManager_Vita::GetPlayerIfReady(
                                    SQRNetworkPlayer * player) {
...
void SQRNetworkManager_Vita::SetState(SQRNetworkManager_Vita::eSQRNetworkManagerInternalState stat#ifdef _DEBUG     
	...
	eSQRNetworkManagerState oldState = m_INTtoEXTStateMappings[m_state];
                                               eSQRNetworkManagerState
                                                   newState =
                                                       m_INTtoEXTStateMappings
                                                           [state];
                                    bool setIdleReasonSessionFull = false;
                                    if ((state == SNM_INT_STATE_IDLE) &&
                                        m_nextIdleReasonIsFull) {
                                        setIdleReasonSessionFull = true;
                                        m_nextIdleReasonIsFull = false;
                                    }
                                    m_state =
                                        sta  // Queue any important (ie
                                        ...
                                        EnterCriticalSection(
                                            &m_csStateChangeQueue);
                                        m_stateChangeQueue.push(StateChangeInfo(
                                            oldState, newState,
                                            setIdleReasonSessionFull));
                                        LeaveCriticalSection(
                                            &m_csStateChangeQueue);
                                ...
                                void SQRNetworkManager_Vita::ResetToIdle() {
                                    ...
                                    memset(m_aRoomSlotPlayers, 0,
                                           sizeof(m_aRoomSlotPlayers));
                                    memset(&m_roomSyncData, 0,
...
bool SQRNetworkManager_Vita::JoinRoom(SceNpMatching2RoomId roomId, SceNpMatching2ServerId serverId, int localPlayerMask, const SQRNetworkManager_Vita::PresenceSyncInfo *presence// The presence info will be directly passed in if we are joining from an invite, otherwise it has already been set up. This is synchronised out when we have fully joined the game.     
	...
	return GetServerContext( serverId );
                                ...
                                void SQRNetworkManager_Vita::LeaveRoom(
	...
	if( bActuallyLeaveRoom )
	{
		...
		else
		{
                        ...
                        bool SQRNetworkManager_Vita::SessionHasSpace(
                            int spaceRequired) {
                            return ((m_roomSyncData.getPlayerCount() +
                                     spaceRequired) <= MAX_ONLINE_PLAYER_COUNT);
                        }

                        bool SQRNetworkManager_Vita::AddLocalPlayerByUserIndex(
                            int idx) {
                            if (m_isHosting) {
                                if (m_roomSyncData.getPlayerCount() ==
                                    MAX_ONLINE_PLAYER_COUNT)
                                    return false  // Host's players are always
                                                  // at the start of the sync
                                                  // data, so we just need to
                                ...
                                for (int i = 0;
                                     i < m_roomSyncData.getPlayerCount(); i++) {
                                    if (m_roomSyncData.players[i]
                                            .m_roomMemberId !=
                                        m_localMemberId) {
                                        insertAtIdx = i;
                                    ...
                                    for (int i =
                                         ...
                                         i > insertAtIdx; i--) {
                                        m_roomSyncData.players[i] =
                                            m_roomSyncData.players[i - 1];
                                    }
                                    m_roomSyncData.players[insertAtIdx]
                                        .m_localIdx = idx;
                                    m_roomSyncData.players[insertAtIdx]
                                        .m_roomMemberId = m_localMemberId;
                                    m_roomSyncData.players[insertAtIdx]
                                        .m_smallId = m_currentSmallId++;
                ...
                if ((ret < 0) ||
                    ...
                    if (rudpOk) {
                        bool ret = CreateVoiceRudpConnections(
                            m_room, m_roomSyncData.players[0].m_roomMemberId,
                            mask);
                        assert(ret);
                        return true;
                    } else {
                        ...
                        return false;
                            ...
                            bool SQRNetworkManager_Vita::
                                RemoveLocalPlayerByUserIndex(int idx) {
                                if (m_isHosting) {
                                    EnterCriticalSection(&m_csRoomSyncData);

                                    int roomSlotPlayerCount =
                                        m_roomSyncData.getPlayerCount();
...
void SQRNetworkManager_Vita::SendInviteGUI()
{
                ...
                char* body = (char*)mallocAndCreateUTF8ArrayFromString(
                    IDS_INVITATION_BODY);
                ...
                return;
                                ...
                                int ret = sce::Toolkit::NP::Messaging::
                ...
                return;
                                ...
                                void SQRNetworkManager_Vita::TickInviteGUI() {
                                    PSVITA_STUBB  // 	if(b_inviteRecvGUIRunning)  //
                                        ...
                                        // &sentOnlineIdList;  // 			int32_t
                                        // ret =
                                        // sceGameCustomDataDialogGetResult(
                                        // &dialogResult );  // 			if( SCE_OK !=
                                        // ret )  // 			{
                                        //   //
                                        // app.DebugPrintf( "*****
                                        // sceGameCustomDataDialogGetResult
        ...
        if (pInvite->getAttachmentSize() == sizeof(m_gameBootInvite_data)) {
            ...
            m_gameBootInvite = &m_gameBootInvite_data;
                                    ...
                                    void SQRNetworkManager_Vita::
                                        ProcessJoinablePresenceData() {
                                        static SceNpBasicGamePresence
                                            presenceDetails;
                                        int ret =
                                            sceNpBasicGetGamePresenceOfFriend(
                                                &m_joinablePresenceParam.npId,
                                                &presenceDetails);
                                        if (ret == 0) {
                                            PresenceSyncInfo* pso =
                                                (PresenceSyncInfo*)
                                                    presenceDetails
                                                        .inGamePresence.data;
                                            memcpy(
                                                &m_gameBootInvite_data, pso,
                                                sizeof(m_gameBootInvite_data));
                                            m_gameBootInvite =
                                                &m_gameBootInvite_data;
                                        }
                                        m_bJoinablePresenceWaitingForOnline =
                                            false;
                                    }  // This case happens when we were in the
                                    ...
                                    int MustSignInReturnedPresenceInvite(
                                        ...
                                        if (result ==
                                            C4JStorage::EMessage_ResultAccept) {
                                            SQRNetworkManager_Vita::
                                                AttemptPSNSignIn(
                                                    &SQRNetworkManager_Vita::
                                                        PSNSignInReturnedPresenceInvite,
                                                    pParam, true);
                                        }
                                        return 0;
                                    }
                                    ...
                                    int SQRNetworkManager_Vita::
                                        PSNSignInReturnedPresenceInvite(
                                            void* pParam, bool bContinue,
                                            int iPad) {
                                        INVITE_INFO* inviteInfo = (INVITE_INFO*)
                                            pPara  // If the invite data isn't
                                                   // set up yet (indicated by
                                                   // it being all zeroes,
                                            ...
                                            if (ProfileManager.IsSignedInLive(
                                                ...
                                                m_bJoinablePresenceWaitingForOnline =
                                                    fa  // will be set to true
                                                        ...
                                                        // for connecting when
                                                        // already signed
                                                        // in    // if (ProfileManager.IsSignedInPSN(ProfileManager.GetPrimaryPad()))    // 		{    // 			// Signed in to PSN but not connected (no internet access)    // 			UINT uiIDA[1];    // 			uiIDA[0] = IDS_OK;    // 			ui.RequestMessageBox( IDS_ERROR_NETWORK_TITLE, IDS_ERROR_NETWORK, uiIDA, 1, ProfileManager.GetPrimaryPad(), NULL, NULL, app.GetStringTable());    // 		}    // 		else     
                                                {  // Not signed in to PSN     
                                                    ...
                                                    ui.RequestAlertMessage(
                                                        ...
                                                        ProfileManager
                                                            .GetPrimaryPad(),
                                                        &MustSignInReturnedPresenceInvite,
                                                        NULL);
                                                }
                                            }
                                        }
                                    ...
                                    bool
                                    SQRNetworkManager_Vita::UpdateInviteData(
                                        SQRNetworkManager_Vita::
                                            PresenceSyncInfo *
                                        invite) {
                                        PSVITA_STUBBED;
                                        return fals  // 	std::size_t
                                            ...
                                            for (AUTO_VAR(
                                                     it,
                                                     m_vecTempPlayers.begin());
                                                 it != m_vecTempPlayers.end();
                                                 it++) {
                                                if (((*it)->m_type ==
                                                     playerType) &&
                                                    ((*it)->m_localPlayerIdx ==
                                                   ...
                                                   // non-network players can be
                                                   // considered complete as
                                                   // soon as we create them as
                                                   // we aren't waiting on their
                                                   // network connections
	...
	for( int i = 0; i < m_roomSyncData.getPlayerCount(); )
	{
                                                                                            if ((m_roomSyncData
                                                                                                ...
                                                                                                ((1
                                                                                                 ...
                                                                                                 mask)) {
                                                                                                SQRNetworkPlayer*
                                                                                                    player = GetPlayerFromRoomMemberAndLocalIdx(
                                                                                                        memberId,
                                                                                                        m_roomSyncData
                                                                                                            .players
                                                                                                                [i]
...
bool SQRNetworkManager_Vita::GetServerContext2()
{
        ...
        *m_aServerId = server.serverId;
        ...
        return SelectRandomServer()  // Overloaded method for (as before)
	...
	if( !GetMatchingContext(SNM_INT_STATE_JOINING_STARTING_MATCHING_CONTEXT) )
	{
                ...
                m_aServerId = (SceNpMatching2ServerId*)realloc(
                    m_aServerId,
                    sizeof(SceNpMatching2ServerId) * m_serverCount);
                m_aServerId[0] =
                    serverI  // If one of the previous GetMatchingContext calls
            ...
            void SQRNetworkManager_Vita::RoomCreateTick() {
                switch (m_state) {
                    ...
                    case SNM_INT_STATE_HOSTING_CREATE_ROOM_WORLD_FOUND: {
                        ...
                        SceNpMatching2BinAttr roomBinAttr;
                        memset(&reqParam, 0, sizeof(reqParam));
                        memset(&optSignalingParam, 0,
                               sizeof(optSignalingParam));
                        memset(&roomBinAttr, 0, sizeof(roomBinAttr));
                    ...
                    case SNM_INT_STATE_HOSTING_CREATE_ROOM_SUCCESS:
                        SetState(
                            SNM_INT_STATE_HOSTING_WAITING_TO_PLAY)  // Now we
                                                                    // know the
                                                                    // local
                                                                    // member id
                                                                    // we can
                    ...
                    default:
                        ...
                        SQRNetworkManager_Vita::NetworkPlayerConnectionComplete(
                            ...
                            bool wasClientReady =
                                player->HasConnectionAndSmallId();
                            player->ConnectionComplete();
                            bool isReady = player->IsReady();
                            bool isClientReady =
                                player->HasConnectionAndSmallId();
                            if (!m_isHosting)
                                // For clients, if we are ready (up the the
                            ...
                            if ((!wasReady) && (isReady)) {
                                ...
                                void SQRNetworkManager_Vita::
                                    ...
                                    SQRNetworkPlayer::InitSendData* ISD =
                                        (SQRNetworkPlayer::InitSendData*)data;
                                    bool wasReady = player->IsReady();
...
SQRNetworkPlayer *SQRNetworkManager_Vita::GetPlayerFromRoomMemberAndLocalIdx(int roomMember, int localIdx)
{
	...
	switch( event )
	{
                ...
                if (errorCode < 0) {
                    ...
                    if (manager->m_state ==
                        ...
                        if (errorCode < 0) {
                            ...
                            manager->SetState(
                                SNM_INT_STATE_IDLE);  // 4J-PB -
                                                      // SQRNetworkManager_PS3::AttemptPSNSignIn
                                                      // was causing crashes in
                                                      // Iggy by calling
                                                      // LoadMovie from a
                                                      // callback, so call it
                                                      // from the tick
                                                      // instead     
                            m_bCallPSNSignInCallback = true  // 				if(s_SignInCompleteCallbackFn)    //
                                                             // {    //
                                ...
                                // boot-up     
                                if (manager->m_doBootInviteCheck) {  //					ORBIS_STUBBED;     //
                                                                     //unsigned
                                                                     //int type,
                                                                     //attributes;     //
                                                                     //CellGameContentSize
		...
		break;
                    ...
                    LeaveCriticalSection(
                        &manager->m_csMatching)  // This is an implementation of
                        ...
                        void SQRNetworkManager_Vita::DefaultRequestCallback(
                            ...
                            SceNpMatching2Event event, int errorCode,
...
// This is the response to sceNpMatching2GetWorldInfoList, which is called as part of the process to create a room (which needs a world to be created in). We aren't anticipating   // using worlds in a meaningful way so just getting the first world we find on the server here, and then advancing the state so that the tick can get on with the rest of the process.     
	case SCE_NP_MATCHING2_REQUEST_EVENT_GET_WORLD_INFO_LIST:
		{
                        ...
                        if (errorCode ==
                            SCE_NP_MATCHING2_ERROR_NP_SIGNED_OUT) {  // If we've
                                                                     // already
                                                                     // signed
                                                                     // out,
                                                                     // then we
			...
			break;
                    ...
                    if (errorCode == 0) {
                        if (data != 0) {
                            ...
                            manager->SetState(
                                SNM_INT_STATE_HOSTING_CREATE_ROOM_SUCCESS);
                            manager->m_room =
                                roomData->roomDataInternal->roomId;
                            break;
                        }
                    }
                ...
                if (errorCode == 0) {
                    if (data != 0) {
                        ...
                        manager->m_room = roomData->roomDataInternal
                ...
                if (errorCode ==
                    SCE_NP_MATCHING2_SERVER_ERROR_ROOM_F  // MGH - added to fix
                                                          // "host has exited"
                                                          // error when 2
                                                          // players go after
                                                          // the final slot     
                        ...
                        if (manager->m_isHosting) {
                            int playerMask =
                                *((int*)(pRoomMemberData->roomMemberDataInternal
                                             ->roomMemberBinAttrInternal->data
                                             .ptr));
                            ...
                            if (success1) {
                                ...
                                if (success2) {
                                    bool ret =
                                        manager->CreateVoiceRudpConnections(
                                            manager->m_room,
                                            pRoomMemberData
	...
	LeaveCriticalSection(&manager->m_csMatching);
        ...
        void SQRNetworkManager_Vita::RoomEventCallback(
	...
	case SCE_NP_MATCHING2_ROOM_EVENT_UPDATED_ROOM_MEMBER_DATA_INTERNAL:
		...
		// We'll get this sync'd round all the connected clients, but we only care about it on the host where we can use it to work out if any RUDP connections need to be made or released     
            ...
            if ((data) &&
                ...
                int playerMask =
                    *((int*)(pRoomMemberData->newRoomMemberBinAttrInternal[0]
                                 ->data.ptr));
                int oldMask = manager->GetOldMask(
                    pRoomMemberData->newRoomMemberDataInternal->memberId);
                ...
                if (addedMask != 0) {
                    ...
                    if ((!success) ||
                        ...
                        int ret = sceNpMatching2SetRoomMemberDataInternal(
                            manager->m_matchingContext, &reqParam, NULL,
                            &manager->m_setRoomMemberInternalDataRequestId);
                    } else {
            ...
            if ((data) &&
                ...
                SceNpMatching2RoomMemberDataInternalUpdateInfo*
                    pRoomMemberData =
                        (SceNpMatching2RoomMemberDataInternalUpdateInfo*)(data);
                assert(pRoomMemberData->newRoomMemberBinAttrInternalNum == 1);
                if (pRoomMemberData->newRoomMemberDataInternal->memberId ==
                    manager->m_localMemberId) {
                    int playerMask = *(
                        (int*)(pRoomMemberData->newRoomMemberBinAttrInternal[0]
                                   ->data.ptr));
                    if (playerMask != manager->m_localPlayerJoinMask) {
                        int playersToRemove =
                            manager->m_localPlayerJoinMask & (~playerMask);
        ...
        case SCE_NP_MATCHING2_ROOM_EVENT_UPDATED_SIGNALING_OPT_PARAM:
            ...
            void SQRNetworkManager_Vita::SignallingCallback(
                ...
                SceNpMatching2Event event, int error_code, void* arg) {
                SQRNetworkManager_Vita* manager = (SQRNetworkManager_Vita*)arg;
                ...
                switch (event) {
                    case SCE_NP_MATCHING2_SIGNALING_EVENT_DEAD: {
                        if (manager->m_isHosting) {  // Remove any players
                            ...
                            SQRVoiceConnection* pVoice = SonyVoiceChat_Vita::
                                getVoiceConnectionFromRoomMemberID(
                                    peerMemberId);
                            if (pVoice) {
                                ...
                                if(peerMemberId == manager->m_hostMemberId || pVoice == N//  MGH - added check for voice, as we sometime get here before m_hostMemberId has been filled in      
				{
                    ...
                    case SCE_NP_MATCHING2_SIGNALING_EVENT_ESTABLISHED: {  // MGH
                                                                          // -
                                                                          // changed
                                                                          // this
                                                                          // to
		...
		if(GetOnlineStatus() == true)
		{
                        ...
                        bool bSignedIn = ProfileManager.IsSignedInLive(
                            ProfileManager.GetPrimaryPad());
                        if (GetOnlineStatus() == false) {
                            if (bSignedIn) {
                                SceNpServiceState serviceState;
                                int retVal =
                                    sceNpGetServiceState(&serviceState);
                                assert(retVal == 0);
                                assert(serviceState ==
                            ...
                            switch (event_id) {
                                ...
                                case SCE_RUDP_CONTEXT_EVENT_ESTABLISHED: {
                                    ...
                                    if (player) {  // Flag connection stage as
                                        ...
                                        SonyVoiceChat_Vita::setConnected(
                                            ctx_id);
                                    }
                                } break;
                                ...
                                case SCE_RUDP_CONTEXT_EVENT_WRITABLE: {
                                    SQRNetworkPlayer* player =
                                        manager->GetPlayerFromRudpCtx(
                                            ctx_id)  // This event signifies
...
{
                                                                                                                                                                                                                                                static SceNpMatching2GetRoomDataExternalListRequest
                                                                                                                                                                                                                                                    reqParam;
...
void SQRNetworkManager_Vita::AttemptPSNSignIn(int (*SignInCompleteCallbackFn)(void *pParam, bool bContinue, int pad), void *pParam, bool callIfF/*=false*/     )
{
                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                    s_SignInCompleteCallbackFn =
                                                                                                                                                                                                                                                                        SignInCompleteCallbackFn;
                                                                                                                                                                                                                                                                    s_signInCompleteCallbackIfFailed =
                                                                                                                                                                                                                                                                        callIfFailed;
                                                                                                                                                                                                                                                                    s_SignInCompleteParam =
                                                                                                                                                                                                                                                                        pParam;
                ...
                if(s_SignInCompleteCallbac// MGH - added after crash on PS4     
		{
                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                            SQRNetworkManager_Vita::
                                                                                                                                                                                                                                                                SetRichPresence(
                                                                                                                                                                                                                                                                    const void*
                                                                                                                                                                                                                                                                        data) {
                                                                                                                                                                                                                                                                const sce::Toolkit::NP::PresenceDetails* newPresenceInfo = (const sce::Toolkit::NP::
                                                                                                                                                                                                                                                                                                                                PresenceDetails*)
                                                                                                                                                                                                                                                                    data;
