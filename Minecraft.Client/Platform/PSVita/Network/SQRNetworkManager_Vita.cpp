...
int (*SQRNetworkManager_Vita::s_SignInCompleteCallbackFn)(void* pParam,
                                                          bool bContinue,
                                                          int pad) = NULL;
...
SQRNetworkManager_Vita::PresenceSyncInfo
    SQRNetworkManager_Vita::s_lastPresenceSyncInfo = {0};
SQRNetworkManager_Vita::PresenceSyncInfo
    SQRNetworkManager_Vita::c_presenceSyncInfoNULL = {0};
// SceNpBasicAttachmentDataId SQRNetworkManager_Vita::s_lastInviteIdToRetry =
// SCE_NP_BASIC_INVALID_ATTACHMENT_DATA_ID;
long long SQRNetworkManager_Vita::s_roomStartTime = 0;
bool SQRNetworkManager_Vita::b_inviteRecvGUIRunning = false;
SQRNetworkManager_Vita::PresenceSyncInfo*
    SQRNetworkManager_Vita::m_gameBootInvite;
SQRNetworkManager_Vita::PresenceSyncInfo
    SQRNetworkManager_Vita::m_gameBootInvite_data;
...
SceAppUtilNpBasicJoinablePresenceParam
    SQRNetworkManager_Vita::m_joinablePresenceParam;
...
// This maps internal to extern states, and needs to match element-by-element
// the eSQRNetworkManagerInternalState enumerated type
const SQRNetworkManager_Vita::eSQRNetworkManagerState SQRNetworkManager_Vita::
    m_INTtoEXTStateMappings[SQRNetworkManager_Vita::SNM_INT_STATE_COUNT] = {
...
SQRNetworkManager_Vita::SQRNetworkManager_Vita(
    ISQRNetworkManagerListener* listener) {
    ...
    memset(
        &m_roomSyncData, 0,
        sizeof(
            m_roomSyncData));  // MGH -  added to fix problem when joining a
                               // full room, and the sync data wasn't populated
    ...
    // Event Handler"); 	m_basicEventThread->Run();
}

// First stage of initialisation. This initialises a few things that don't
// require the user to be signed in, and then kicks of the network start dialog
// utility. Initialisation continues in InitialiseAfterOnline once this
// completes.
...
#define NP_IN_GAME_MESSAGE_POOL_SIZE (16 * 1024)
    ...
    //  	ret = sceNetCtlInit();
    //  	if( ( ret < 0 && ret != SCE_NET_CTL_ERROR_NOT_TERMINATED ) ||
    //  ForceErrorPoint( SNM_FORCE_ERROR_NET_CTL_INIT ) )
    //  	{
    ...
    // Initialise RUDP
    const int RUDP_POOL_SIZE =
        (500 * 1024);  // TODO - find out what we need, this size is copied from
                       // library reference
    ...
    ret = sceRudpEnableInternalIOThread(RUDP_THREAD_STACK_SIZE,
                                        SCE_KERNEL_DEFAULT_PRIORITY);
    if (ret < 0) {
        app.DebugPrintf(
            "sceRudpEnableInternalIOThread failed with error code 0x%08x\n",
            ret);
        assert(0);
    }
    // Already online? the callback won't catch this, so carry on initialising
    // now
    if (ProfileManager.IsSignedInLive(ProfileManager.GetPrimaryPad())) {
        InitialiseAfterOnline();
    } else {
        // On PS3 we'd be running the netstart dialog here, but we don't want to
        // do that on Vita since we could be switching from ad-hoc mode
        SetState(SNM_INT_STATE_INITIALISE_FAILED);
        m_offlineSQR = true;
    }
...
bool SQRNetworkManager_Vita::IsInitialised() { return m_bIsInitialised; }
...
void SQRNetworkManager_Vita::Terminate() {
    // If playing, attempt to nicely leave the room before shutting down so that
    // our friends won't still think this game is in progress
    if ((m_state == SNM_INT_STATE_HOSTING_CREATE_ROOM_SUCCESS) ||
        ...
        if (!m_offlineGame) {
            ...
            app.DebugPrintf(CMinecraftApp::USER_RR,
                            "Attempted to leave room, %dms used\n", count * 10);
        }
    }
    ...
    // 	sceKernelTriggerUserEvent(m_basicEventQueue, sc_UserEventHandle,
    // (void*)1);
...
// Second stage of initialisation, that requires NP Manager to be online & the
// player to be signed in. This kicks of the creation of a context for Np
// Matching 2. Initialisation is finally complete when we get a callback to
// ContextCallback. The SQRNetworkManager_Vita is then finally moved into
// SNM_INT_STATE_IDLE at this stage.
void SQRNetworkManager_Vita::InitialiseAfterOnline() {
    // MGH -  added, so we don't init the matching2 stuff in trial mode -
    // devtrack #5921
    if (!ProfileManager.IsFullVersion()) return;

    // 	SceNpId npId;
    // 	int option = 0;

    // We should only be doing this if we have come in from an initialisation
    // stage (SQRNetworkManager_Vita::Initialise) or we've had a network
    // disconnect and are coming in from an offline state. Don't do anything
    // otherwise - this mainly to catch a bit of a corner case in the
    // initialisation phase where potentially we could register for the callback
    // that would call this with sceNpManagerRegisterCallback. and could then
    // really quickly go online so that the there becomes two paths (via the
    // callback or SQRNetworkManager_Vita::Initialise) by which this could be
    // called
    if ((m_state != SNM_INT_STATE_SIGNING_IN) &&
        !((m_state == SNM_INT_STATE_IDLE) && m_offlineSQR)) {
        // If we aren't going to continue on with this sign-in but are expecting
        // a callback, then let the game know that we have completed the bit we
        // are expecting to do. This will happen whilst in game, when we want to
        // be able to sign into PSN, but don't expect the full matching stuff to
        // get set up.
    ...
    app.DebugPrintf(
        "SQRNetworkManager::InitialiseAfterOnline - matching context is now "
        "valid\n");
    ...
    ret = sceNpMatching2CreateContext(&npID, GetSceNpCommsId(),
    ...
    // State should be starting context until the callback that this has been
    // created happens
...
// functions should be called from here.
void SQRNetworkManager_Vita::Tick() {
    ...
    if ((m_gameBootInvite) && (s_safeToRespondToGameBootInvite)) {
        m_listener->HandleInviteReceived(ProfileManager.GetPrimaryPad(),
                                         m_gameBootInvite);
        m_gameBootInvite = NULL;
    }

    ErrorHandlingTick();
    // If we ever fail to send the external room data, we start a countdown so
    // that we attempt to resend. Not sure how likely it is that updating this
    // will fail without the whole network being broken, but if in particular we
    // don't update the flag to say that the session is joinable, then nobody is
    // ever going to see this session.
    ...
    // Client only - do the final transition to a starting & playing state once
    // we have fully joined the room, And told the game about all the local
    // players so they are also all valid
    if (m_state == SNM_INT_STATE_JOINING_WAITING_FOR_LOCAL_PLAYERS) {
        if (m_localPlayerJoined == m_localPlayerCount) {
            // Since we're now fully joined, we can update our presence info so
            // that our friends could find us in this game. This data was set up
            // at the point that we joined the game (either from search info, or
            // an invitation).
            UpdateRichPresenceCustomData(&s_lastPresenceSyncInfo,
                                         sizeof(PresenceSyncInfo));
    ...
    if (m_state == SNM_INT_STATE_SERVER_DELETING_CONTEXT) {
        // make sure we've removed all the remote players and killed the udp
        // connections before we bail out
        if (m_RudpCtxToPlayerMap.size() == 0) ResetToIdle();
    }

    EnterCriticalSection(&m_csStateChangeQueue);
    while (m_stateChangeQueue.size() > 0) {
        if (m_listener) {
            m_listener->HandleStateChange(
                m_stateChangeQueue.front().m_oldState,
                m_stateChangeQueue.front().m_newState,
                m_stateChangeQueue.front().m_idleReasonIsSessionFull);
    ...
    // 4J-PB - SQRNetworkManager_PS3::AttemptPSNSignIn was causing crashes in
    // Iggy by calling LoadMovie from a callback, so call it frmo the tick
    // instead
...
// Detect any states which reflect internal error states, do anything required,
// and transition away again
void SQRNetworkManager_Vita::ErrorHandlingTick() {
    switch (m_state) {
        case SNM_INT_STATE_INITIALISE_FAILED:
            ...
            if (m_isInSession &&
                m_offlineGame)  // m_offlineSQR )  // MGH - changed this to
                                // m_offlineGame, as m_offlineSQR can be true
                                // when running an online game but the init has
                                // failed because the servers are down
            {
                // This is a fix for an issue where a player attempts (and
                // fails) to sign in, whilst in an offline game. This was
                // setting the state to idle, which in turn sets the game to Not
                // be in a session anymore (but the game wasn't generally aware
                // of, and so keeps playing). Howoever, the game's connections
                // use their tick to determine whether to empty their queues or
                // not and so no communications (even though they don't actually
                // use this network manager for local connections) were
                // happening.
                SetState(SNM_INT_STATE_PLAYING);
            } else {
                m_offlineSQR = true;
        ...
        case SNM_INT_STATE_HOSTING_SERVER_SEARCH_FAILED:
            app.DebugPrintf(
                "Network error: SNM_INT_STATE_HOSTING_SERVER_SEARCH_FAILED\n");
            ResetToIdle();
            break;
        case SNM_INT_STATE_HOSTING_CREATE_ROOM_FAILED:
            app.DebugPrintf(
                "Network error: SNM_INT_STATE_HOSTING_CREATE_ROOM_FAILED\n");
            DeleteServerContext();
            break;
        case SNM_INT_STATE_JOINING_SERVER_SEARCH_FAILED:
            app.DebugPrintf(
                "Network error: SNM_INT_STATE_JOINING_SERVER_SEARCH_FAILED\n");
            ResetToIdle();
            break;
        case SNM_INT_STATE_JOINING_JOIN_ROOM_FAILED:
            app.DebugPrintf(
                "Network error: SNM_INT_STATE_JOINING_JOIN_ROOM_FAILED\n");
            DeleteServerContext();
            break;
...
// Start hosting a game, by creating a room & joining it. We explicity create a
// server context here (via GetServerContext) as Sony suggest that this means we
// have greater control of representing when players are actually "online". The
// creation of the room is carried out in a callback after that server context
// is made (ServerContextValidCallback_CreateRoom). hostIndex is the index of
// the user that is hosting the session, and localPlayerMask has bit 0 - 3 set
// to indicate the full set of local players joining the game. extData and
// extDataSize define the initial state of room data that is externally visible
// (eg by players searching for rooms, but not in it)
void SQRNetworkManager_Vita::CreateAndJoinRoom(int hostIndex,
                                               int localPlayerMask,
    ...
    // Initialise room data that will be synchronised. Slot 0 is always reserved
    // for the host. We don't know the room member until the room is actually
    // created so this will be set/updated at that point
    ...
    // Remove the host player that we've already added, then add any other local
    // players specified in the mask
    localPlayerMask &= ~((1 << hostIndex) & localPlayerMask);
    for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT; i++) {
        if (localPlayerMask & (1 << i)) {
            m_roomSyncData.players[m_roomSyncData.getPlayerCount()].m_smallId =
                m_currentSmallId++;
            m_roomSyncData.players[m_roomSyncData.getPlayerCount()].m_localIdx =
                i;
    ...
    // For offline games, we can jump straight to the state that says we've just
    // created the room (or would have, for an online game)
    if (m_offlineGame) {
        SetState(SNM_INT_STATE_HOSTING_CREATE_ROOM_SUCCESS);
    } else {
        // Kick off the sequence of events required for an online game, starting
        // with getting the server context
        m_isInSession = GetServerContext();
    }
}

// Updates the externally visible data that was associated with the room when it
// was created with CreateAndJoinRoom.
void SQRNetworkManager_Vita::UpdateExternalRoomData() {
    ...
    if (m_isHosting) {
        ...
        int ret = sceNpMatching2SetRoomDataExternal(
            m_matchingContext, &reqParam, NULL, &m_setRoomDataRequestId);
        app.DebugPrintf(CMinecraftApp::USER_RR,
                        "sceNpMatching2SetRoomDataExternal returns 0x%x, "
                        "number of players %d\n",
                        ret, ((char*)m_joinExtData)[174]);
        if ((ret < 0) ||
            ForceErrorPoint(SNM_FORCE_ERROR_SET_EXTERNAL_ROOM_DATA)) {
            // If we ever fail to send the external room data, we start a
            // countdown so that we attempt to resend. Not sure how likely it is
            // that updating this will fail without the whole network being
            // broken, but if in particular we don't update the flag to say that
            // the session is joinable, then nobody is ever going to see this
            // session.
...
// Determine if the friend room manager is busy. If it isn't busy, then other
// operations (searching for a friend, reading the found friend's room lists)
...
// Initiate a search for rooms that the signed in user's friends are in. This is
// an asynchronous operation, this function returns after it kicks off a search
// across all game servers for any of the player's friends.
bool SQRNetworkManager_Vita::FriendRoomManagerSearch() {
    ...
    // Get friend list - doing this in another thread as it can lock up for a
    // few seconds
    m_getFriendCountThread =
        new C4JThread(&GetFriendsThreadProc, this, "GetFriendsThreadProc");
...
bool SQRNetworkManager_Vita::FriendRoomManagerSearch2() {
    ...
    if (m_aFriendSearchResults.size() > 0) {
        // If we have some results, then we also want to make sure that we don't
        // have any duplicate rooms here if more than one friend is playing in
        // the same room.
...
// The handler for basic events can't actually get the events themselves, this
// has to be done on another thread. Instead, we send a sys_event_t to a queue
// on This thread, which has a single data item used which we can use to
// determine whether to terminate this thread or get a basic event & handle
// that.
int SQRNetworkManager_Vita::BasicEventThreadProc(void* lpParameter) {
    ...
    // 	do
    // 	{
    // 		ret = sceKernelWaitEqueue(manager->m_basicEventQueue, &event, 1,
    // &outEv, NULL);
    //
    // 		// If the sys_event_t we've sent here from the handler has a
    // non-zero data1 element, this is to signify that we should terminate the
    // thread 		if( event.udata == 0 )
    // 		{
    // 			// 			int iEvent;
    // 			// 			SceNpUserInfo from;
    // 			// 			uint8_t
    // buffer[SCE_NP_BASIC_MAX_MESSAGE_SIZE];
    // 			// 			std::size_t bufferSize =
    // SCE_NP_BASIC_MAX_MESSAGE_SIZE;
    // 			// 			int ret =
    // sceNpBasicGetEvent(&iEvent, &from, &buffer, &bufferSize);
    // 			// 			if( ret == 0 )
    // 			// 			{
    // 			// 				if( iEvent ==
    ...
    // 			// 				{
    // 			// 					// 4J Stu -
    // Don't do this here as it can be very disruptive to gameplay. Players can
    // bring this up from LoadOrJoinMenu, PauseMenu and InGameInfoMenu
    // 			//
...
int SQRNetworkManager_Vita::GetFriendsThreadProc(void* lpParameter) {
    ...
    if ((ret < 0) ||
        manager->ForceErrorPoint(SNM_FORCE_ERROR_GET_FRIEND_LIST_ENTRY_COUNT)) {
        // This is likely when friend list hasn't been received from the server
        // yet - will be returning SCE_NP_BASIC_ERROR_BUSY in this case
        manager->m_friendCount = 0;
    }
    ...
    if (manager->m_friendCount > 0) {
        ...
        if (ret < 0) {
            app.DebugPrintf(
                "sceNpBasicGetFriendListEntries() failed: ret = 0x%x\n", ret);
            manager->m_friendCount = 0;
        } else {
    ...
    // It is possible that the size of the friend list might vary from what we
    // just received, so only add in friends that we successfully get an entry
    // for
    for (unsigned int i = 0; i < manager->m_friendCount; i++) {
        ...
        if (ret < 0) {
            app.DebugPrintf(
                "sceNpBasicGetFriendContextState() failed: ret = 0x%x\n", ret);
            contextState = SCE_NP_BASIC_FRIEND_CONTEXT_STATE_UNKNOWN;
        }
        if (contextState ==
            SCE_NP_BASIC_FRIEND_CONTEXT_STATE_IN_CONTEXT)  // using the same
                                                           ...
                                                           // Minecraft
        {
            ret = sceNpBasicGetGamePresenceOfFriend(&friendIDs[i],
                                                    &presenceDetails);
            if ((ret == 0) && (!manager->ForceErrorPoint(
                        ...
                        if (!pso->inviteOnly) {
                            ...
                            CPlatformNetworkManagerSony::
                                MallocAndSetExtDataFromSQRPresenceInfo(
                                    &result.m_RoomExtDataReceived, pso);
...
// Get count of rooms that friends are playing in. Only valid when
...
// Get details of a found session that a friend is playing in. 0 < idx <
// FriendRoomManagerGetCount(). Only valid when FriendRoomManagerIsBusy()
// returns false
void SQRNetworkManager_Vita::FriendRoomManagerGetRoomInfo(
    int idx, SQRNetworkManager_Vita::SessionSearchResult* searchResult) {
    ...
    searchResult->m_sessionId.m_ServerId =
        m_aFriendSearchResults[idx].m_ServerId;
...
bool SQRNetworkManager_Vita::IsHost() { return m_isHosting; }

bool SQRNetworkManager_Vita::IsReadyToPlayOrIdle() {
    return ((m_state == SNM_INT_STATE_HOSTING_WAITING_TO_PLAY) ||
            (m_state == SNM_INT_STATE_PLAYING) ||
            (m_state == SNM_INT_STATE_IDLE));
}

// Consider as "in session" from the moment that a game is created or joined,
// until the point where the game itself has been told via state change that we
// are now idle. The game code requires IsInSession to return true as soon as it
// has asked to do one of these things (even if the state system hasn't really
// caught up with this request yet), and it also requires that it is informed of
// the state changes leading up to not being in the session, before this should
// report false.
bool SQRNetworkManager_Vita::IsInSession() { return m_isInSession; }
...
SQRNetworkPlayer* SQRNetworkManager_Vita::GetPlayerIfReady(
...
#endif
void SQRNetworkManager_Vita::SetState(
    SQRNetworkManager_Vita::eSQRNetworkManagerInternalState state) {
#ifdef _DEBUG
    ...
    m_state = state;
    // Queue any important (ie externally relevant) state changes - we will do a
    // call back for these in our main tick. Don't do it directly here as we
    // could be coming from any thread at this stage, with any stack size etc.
    // and so we don't generally want to expect the game to be able to handle
    // itself in such circumstances.
    if ((newState != oldState) || setIdleReasonSessionFull) {
        EnterCriticalSection(&m_csStateChangeQueue);
        m_stateChangeQueue.push(
...
void SQRNetworkManager_Vita::ResetToIdle() {
    ...
    // If we're the client, remove any networked players properly ( this will
    // destory their rupd context etc.)
    if (!m_isHosting) {
...
// Join a room that was found with FriendRoomManagerSearch. 0 < idx <
// FriendRoomManagerGetCount(). Only valid when FriendRoomManagerIsBusy()
// returns false
bool SQRNetworkManager_Vita::JoinRoom(
    SQRNetworkManager_Vita::SessionSearchResult* searchResult,
    int localPlayerMask) {
    ...
    CPlatformNetworkManagerSony::SetSQRPresenceInfoFromExtData(
        &s_lastPresenceSyncInfo, searchResult->m_extData,
        searchResult->m_sessionId.m_RoomId,
        searchResult->m_sessionId.m_ServerId);
    return JoinRoom(searchResult->m_sessionId.m_RoomId,
                    searchResult->m_sessionId.m_ServerId, localPlayerMask,
                    NULL);
}

// Join room with a specified roomId. This is used when joining from an invite,
// as well as by the previous method
bool SQRNetworkManager_Vita::JoinRoom(
    SceNpMatching2RoomId roomId, SceNpMatching2ServerId serverId,
    int localPlayerMask,
    const SQRNetworkManager_Vita::PresenceSyncInfo* presence) {
    // The presence info will be directly passed in if we are joining from an
    // invite, otherwise it has already been set up. This is synchronised out
    // when we have fully joined the game.
    if (presence) {
...
void SQRNetworkManager_Vita::StartGame() {
    assert((m_state == SNM_INT_STATE_HOSTING_WAITING_TO_PLAY) ||
           ((m_state == SNM_INT_STATE_IDLE) && m_offlineSQR));
...
void SQRNetworkManager_Vita::LeaveRoom(bool bActuallyLeaveRoom) {
    ...
    UpdateRichPresenceCustomData(&c_presenceSyncInfoNULL,
                                 sizeof(PresenceSyncInfo));

    // 	SonyVoiceChat::shutdown();

    // Attempt to leave the room if we are in any of the states we could be in
    // if we have successfully created it
    if (bActuallyLeaveRoom) {
        if ((m_state == SNM_INT_STATE_HOSTING_CREATE_ROOM_SUCCESS) ||
            ...
            int ret = sceNpMatching2LeaveRoom(m_matchingContext, &reqParam,
                                              NULL, &m_leaveRoomRequestId);
            ...
            // Haven't created the room yet, but will have created the server
            // context so need to recover from that
            DeleteServerContext();
        } else {
            SetState(SNM_INT_STATE_IDLE);
        }
    } else {
        // We have created a room but have now had some kind of connection error
        // which means that we've been dropped out of the room and it has been
        // destroyed, so no need to leave it again since it doesn't exist
        // anymore. Still need to destroy server context which may be valid
        DeleteServerContext();
    }
}
...
bool SQRNetworkManager_Vita::SessionHasSpace(int spaceRequired) {
    return ((m_roomSyncData.getPlayerCount() + spaceRequired) <=
            MAX_ONLINE_PLAYER_COUNT);
}

bool SQRNetworkManager_Vita::AddLocalPlayerByUserIndex(int idx) {
    if (m_isHosting) {
        if (m_roomSyncData.getPlayerCount() == MAX_ONLINE_PLAYER_COUNT)
            return false;

        // Host's players are always at the start of the sync data, so we just
        // need to find the first entry that isn't us to determine what we want
        // to insert before
        ...
        // And do any adjusting necessary to the mappings from this room data,
        // to the SQRNetworkPlayers. This will also create the required new
        // SQRNetworkPlayer and do all the callbacks that requires etc.
        MapRoomSlotPlayers();

        // Sync this back out to our networked clients...
        SyncRoomData();

        // no connections being made because we're all on the host, so add this
        // player to the existing connections
        SonyVoiceChat_Vita::connectPlayerToAll(idx);
        return true;
    } else {
        // Don't attempt to join if our client's view of the players indicates
        // that there aren't any free slots
        if (m_roomSyncData.getPlayerCount() == MAX_ONLINE_PLAYER_COUNT)
            return false;

        // Add the requested player to the mask of local players currently in
        // the game, and update this data - this will also then resync with the
        // server which can respond appropriately
        int mask = 1 << idx;
        ...
        if ((ret < 0) ||
            ForceErrorPoint(SNM_FORCE_ERROR_SET_ROOM_MEMBER_DATA_INTERNAL)) {
            return false;
        }

        // Create the client's end of the rudp connections... note that
        // m_roomSyncData.players[0].m_roomMemberId is always be the host's room
        // member id.
        ...
        if (rudpOk) {
            bool ret = CreateVoiceRudpConnections(
                m_room, m_roomSyncData.players[0].m_roomMemberId, mask);
            assert(ret);
            return true;
        } else {
    ...
    if (m_isHosting) {
        ...
        for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++) {
            if ((m_roomSyncData.players[i].m_roomMemberId == m_localMemberId) &&
                ...
                m_roomSyncData.setPlayerCount(m_roomSyncData.getPlayerCount() -
                                              1);
                for (int j = i; j < m_roomSyncData.getPlayerCount(); j++) {
                    m_roomSyncData.players[j] = m_roomSyncData.players[j + 1];
                }

                // Zero last element that isn't part of the currently sized
                // array anymore
                memset(&m_roomSyncData.players[m_roomSyncData.getPlayerCount()],
                       0, sizeof(PlayerSyncData));

                // And do any adjusting necessary to the mappings from this room
                // data, to the SQRNetworkPlayers. This will also delete the
                // SQRNetworkPlayer and do all the callbacks that requires etc.
        ...
        // Remove the requested player from the mask of local players currently
        // in the game, and update this data - this will also then resync with
        // the server which can respond appropriately
        int mask = 1 << idx;
        ...
        if ((ret < 0) ||
            ForceErrorPoint(SNM_FORCE_ERROR_SET_ROOM_MEMBER_DATA_INTERNAL2)) {
            return false;
        }
...
// Bring up a Gui to send an invite so a player that the user can select. This
// invite will contain the room Id so that
void SQRNetworkManager_Vita::SendInviteGUI() {
    ...
    // Set invitation information - this is now exactly the same as the presence
    // information that we synchronise out.

    // If we joined a game, we'll have already set s_lastPresenceSyncInfo up
    // (whether we came in from an invite, or joining a game we discovered). If
    // we were hosting, then we'll need to set this up now from the external
    // dasta.
    if (m_isHosting) {
        CPlatformNetworkManagerSony::SetSQRPresenceInfoFromExtData(
            &s_lastPresenceSyncInfo, m_joinExtData, m_room, m_serverId);
    }

    sce::Toolkit::NP::MessageData messData;
    memset(&messData, 0, sizeof(messData));

    char* subject = (char*)mallocAndCreateUTF8ArrayFromString(
        IDS_INVITATION_SUBJECT_MAX_18_CHARS);
...
void SQRNetworkManager_Vita::RecvInviteGUI() {
    ...
    int ret = sce::Toolkit::NP::Messaging::Interface::displayReceivedMessages(
    ...
    // 	if(ret != SCE_OK)
    // 	{
    // 		app.DebugPrintf("sceGameCustomDataDialogInitialize() failed. ret
    // = 0x%x\n", ret);
    // 	}
    // 	else
    // 	{
    ...
    // 		if( SCE_OK != ret )
    // 		{
    // 			app.DebugPrintf("sceGameCustomDataDialogOpen() failed.
    // ret = 0x%x\n", ret);
    // 		}
    // 		else
    // 		{
...
void SQRNetworkManager_Vita::TickInviteGUI() {
    ...
    // 			{
    // 				app.DebugPrintf( "*****
    // sceGameCustomDataDialogGetResult error:0x%x\n", ret);
    // 			}
...
// Get the data for an invite into a statically allocated array of invites, and
// pass a pointer of this back up to the game. Elements in the array are used in
// a circular fashion, to save any issues with handling freeing of this invite
// data as the qnet equivalent of this seems to just assume that the data
// persists forever.
void SQRNetworkManager_Vita::GetInviteDataAndProcess(
    sce::Toolkit::NP::MessageAttachment* pInvite) {
...
void SQRNetworkManager_Vita::GetJoinablePresenceDataAndProcess(
    SceAppUtilNpBasicJoinablePresenceParam* pJoinablePresenceData) {
    memcpy(&m_joinablePresenceParam, pJoinablePresenceData,
           sizeof(SceAppUtilNpBasicJoinablePresenceParam));
    if (s_safeToRespondToGameBootInvite &&
...
void SQRNetworkManager_Vita::ProcessJoinablePresenceData() {
    static SceNpBasicGamePresence presenceDetails;
    int ret = sceNpBasicGetGamePresenceOfFriend(&m_joinablePresenceParam.npId,
                                                &presenceDetails);
    if (ret == 0) {
        PresenceSyncInfo* pso =
            (PresenceSyncInfo*)presenceDetails.inGamePresence.data;
...
// handle this
int MustSignInReturnedPresenceInvite(void* pParam, int iPad,
                                     C4JStorage::EMessageResult result) {
    if (result == C4JStorage::EMessage_ResultAccept) {
        SQRNetworkManager_Vita::AttemptPSNSignIn(
            &SQRNetworkManager_Vita::PSNSignInReturnedPresenceInvite, pParam,
            true);
    }
    return 0;
}

int SQRNetworkManager_Vita::PSNSignInReturnedPresenceInvite(void* pParam,
                                                            bool bContinue,
                                                            int iPad) {
...
void SQRNetworkManager_Vita::TickJoinablePresenceData() {
    if (s_safeToRespondToGameBootInvite &&
        ...
        if (ProfileManager.IsSignedInLive(ProfileManager.GetPrimaryPad()))
            ...
            m_bJoinablePresenceWaitingForOnline =
                false;  // will be set to true again if we sign in succesfully
            // Determine why they're not "signed in live"
            // MGH - we need to add a new message at some point for connecting
            // when already signed in 		if
            ...
            // 		else
            {
                ...
                ui.RequestAlertMessage(IDS_PRO_NOTONLINE_TITLE,
                                       ...
                                       &MustSignInReturnedPresenceInvite, NULL);
            }
        }
    }
}

bool SQRNetworkManager_Vita::UpdateInviteData(
    SQRNetworkManager_Vita::PresenceSyncInfo* invite) {
    PSVITA_STUBBED;
    return false;

    // 	std::size_t dataSize = sizeof(SQRNetworkManager_Vita::PresenceSyncInfo);
    // 	int ret = sceNpBasicRecvMessageAttachmentLoad(s_lastInviteIdToRetry,
    // invite, &dataSize); 	return (ret == 0);
}

// This method is a helper used in MapRoomSlotPlayers - tries to find a player
// that matches: (1) the playerType (2) if playerType is remote, memberId (3)
// localPlayerIdx The reason we don't care about memberid when the player isn't
// remote is that it doesn't matter (since we know the player is either on this
// machine, or it is the host and there's only one of those), and there's a
// period when starting up the host game where it doesn't accurately know the
// memberId for its own local players
void SQRNetworkManager_Vita::FindOrCreateNonNetworkPlayer(
    int slot, int playerType, SceNpMatching2RoomMemberId memberId,
    int localPlayerIdx, int smallId) {
    for (AUTO_VAR(it, m_vecTempPlayers.begin()); it != m_vecTempPlayers.end();
         it++) {
        if (((*it)->m_type == playerType) &&
            ((*it)->m_localPlayerIdx == localPlayerIdx)) {
            if ((playerType != SQRNetworkPlayer::SNP_TYPE_REMOTE) ||
                ((*it)->m_roomMemberId == memberId)) {
    ...
    // Create the player - non-network players can be considered complete as
    // soon as we create them as we aren't waiting on their network connections
    // becoming complete, so can flag them as such and notify via callback
    PlayerUID* pUID = NULL;
    PlayerUID localUID;
    ...
    SQRNetworkPlayer* player = new SQRNetworkPlayer(
        this, (SQRNetworkPlayer::eSQRNetworkPlayerType)playerType, m_isHosting,
        memberId, localPlayerIdx, 0, pUID);
    // For offline games, set name directly from gamertag as the PlayerUID will
    // be full of zeroes.
    if (m_offlineGame) {
...
// For data sending on the local machine, used to send between host and
// localplayers on the host
void SQRNetworkManager_Vita::LocalDataSend(SQRNetworkPlayer* playerFrom,
                                           SQRNetworkPlayer* playerTo,
                                           const void* data,
                                           unsigned int dataSize) {
    assert(m_isHosting);
    if (m_listener) {
        m_listener->HandleDataReceived(playerFrom, playerTo,
...
// Updates m_aRoomSlotPlayers, based on what is in m_roomSyncData. This needs to
// be updated when room members join & leave, and when any SQRNetworkPlayer is
// created externally that this should be mapping to
void SQRNetworkManager_Vita::MapRoomSlotPlayers(
    int roomSlotPlayerCount /*=-1*/) {
    EnterCriticalSection(&m_csRoomSyncData);

    // If we pass an explicit roomSlotPlayerCount, it is because we are removing
    // a player, and this is the count of slots that there were *before* the
    // removal.
    ...
    if (m_isHosting) {
        for (int i = 0; i < roomSlotPlayerCount; i++) {
            if (m_aRoomSlotPlayers[i]) {
                // On host, remote players are created and destroyed by the Rudp
                // connections being established and removed, so don't go
                // deleting them here. Other types are managed by this mapping.
                // Note that m_vecTempPlayers is used as a pool of players to
                // consider by FindOrCreateNonNetworkPlayer
                if (m_aRoomSlotPlayers[i]->m_type !=
                    SQRNetworkPlayer::SNP_TYPE_REMOTE) {
        ...
        for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++) {
            if (i == 0) {
                ...
                FindOrCreateNonNetworkPlayer(
                    i, SQRNetworkPlayer::SNP_TYPE_HOST,
                    m_roomSyncData.players[i].m_roomMemberId,
                    m_roomSyncData.players[i].m_localIdx,
                    m_roomSyncData.players[i].m_smallId);
                m_roomSyncData.players[i].m_UID =
                    m_aRoomSlotPlayers[i]
                        ->GetUID();  // On host, UIDs flow from player data ->
                                     // m_roomSyncData
            } else {
                if (m_roomSyncData.players[i].m_roomMemberId ==
                    m_localMemberId) {
                    FindOrCreateNonNetworkPlayer(
                        i, SQRNetworkPlayer::SNP_TYPE_LOCAL,
                        m_roomSyncData.players[i].m_roomMemberId,
                        m_roomSyncData.players[i].m_localIdx,
                        m_roomSyncData.players[i].m_smallId);
                    m_roomSyncData.players[i].m_UID =
                    ...
                    m_aRoomSlotPlayers[i] = GetPlayerFromRoomMemberAndLocalIdx(
                        m_roomSyncData.players[i].m_roomMemberId,
                        m_roomSyncData.players[i].m_localIdx);
                    // If we're the host, then we allocated the small id so can
                    // flag now if we've got a player to flag...
                    if (m_aRoomSlotPlayers[i]) {
        ...
        for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++) {
            if (m_aRoomSlotPlayers[i]) {
                // On clients, local players are created and destroyed by the
                // Rudp connections being established and removed, so don't go
                // deleting them here. Other types are managed by this mapping.
                // Note that m_vecTempPlayers is used as a pool of players to
                // consider by FindOrCreateNonNetworkPlayer
                if (m_aRoomSlotPlayers[i]->m_type !=
                    SQRNetworkPlayer::SNP_TYPE_LOCAL) {
                    m_vecTempPlayers.push_back(m_aRoomSlotPlayers[i]);
                    m_aRoomSlotPlayers[i] = NULL;
                }
            }
        ...
        for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++) {
            if (i == 0) {
                ...
                FindOrCreateNonNetworkPlayer(
                    ...
                    m_roomSyncData.players[i].m_localIdx,
                    m_roomSyncData.players[i].m_smallId);
                m_aRoomSlotPlayers[i]->SetUID(
                    m_roomSyncData.players[i]
                        .m_UID);  // On client, UIDs flow from
                                  // m_roomSyncData->player data
            } else {
                if (m_roomSyncData.players[i].m_roomMemberId ==
                    m_localMemberId) {
                    // This player is local to this machine - don't bother
                    // setting UID from sync data, as it will already have been
                    // set accurately when we (locally) made this player
                    m_aRoomSlotPlayers[i] = GetPlayerFromRoomMemberAndLocalIdx(
                        m_roomSyncData.players[i].m_roomMemberId,
                        m_roomSyncData.players[i].m_localIdx);
                    // If we've got the room sync data back from the server,
                    // then we've got our smallId. Set flag for this.
                    if (m_aRoomSlotPlayers[i]) {
                        NetworkPlayerSmallIdAllocated(
                            m_aRoomSlotPlayers[i],
                            m_roomSyncData.players[i].m_smallId);
                    ...
                    FindOrCreateNonNetworkPlayer(
                        i, SQRNetworkPlayer::SNP_TYPE_REMOTE,
                        m_roomSyncData.players[i].m_roomMemberId,
                        m_roomSyncData.players[i].m_localIdx,
                        m_roomSyncData.players[i].m_smallId);
                    m_aRoomSlotPlayers[i]->SetUID(
    ...
    // Clear up any non-network players that are no longer required - this would
    // be a good point to notify of players leaving when we support that
    // FindOrCreateNonNetworkPlayer will have pulled any players that we Do need
    // out of m_vecTempPlayers, so the ones that are remaining are no longer in
    // the game
    for (AUTO_VAR(it, m_vecTempPlayers.begin()); it != m_vecTempPlayers.end();
         it++) {
        if (m_listener) {
    ...
    for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++) {
        if (m_aRoomSlotPlayers[i]) {
            if (i == 0) {
                ...
                // Don't sync local players as we already set those up with
                // their UID in the first place...
                if (m_roomSyncData.players[i].m_roomMemberId !=
                    m_localMemberId) {
                    m_aRoomSlotPlayers[i]->SetUID(
                        m_roomSyncData.players[i].m_UID);
...
// Host only - add remote players to our internal storage of player slots, and
// synchronise this with other room members.
bool SQRNetworkManager_Vita::AddRemotePlayersAndSync(
    SceNpMatching2RoomMemberId memberId, int playerMask,
    bool* isFull /*==NULL*/) {
    assert(m_isHosting);
    ...
    if ((m_roomSyncData.getPlayerCount() + addCount) >
        MAX_ONLINE_PLAYER_COUNT) {
        if (isFull) {
            *isFull = true;
        }
    ...
    // We want to keep all players from a particular machine together, so search
    // through the room sync data to see if we can find any pre-existing players
    // from this machine.
    int firstIdx = -1;
    for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++) {
    ...
    // We'll just be inserting at the end unless we've got a pre-existing player
    // to insert after. Even then there might be no following players.
    ...
    // Add all remote players determined from the player mask to our own slots
    // of active players
    for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT; i++) {
        if (playerMask & (1 << i)) {
...
// Host only - remove all remote players belonging to the supplied memberId, and
// in the supplied mask, and synchronise this with other room members
void SQRNetworkManager_Vita::RemoveRemotePlayersAndSync(
    SceNpMatching2RoomMemberId memberId, int mask) {
    assert(m_isHosting);
    EnterCriticalSection(&m_csRoomSyncData);
    ...
    for (int i = 0; i < m_roomSyncData.getPlayerCount();) {
        if ((m_roomSyncData.players[i].m_roomMemberId == memberId) &&
            ((1 << m_roomSyncData.players[i].m_localIdx) & mask)) {
            SQRNetworkPlayer* player = GetPlayerFromRoomMemberAndLocalIdx(
                memberId, m_roomSyncData.players[i].m_localIdx);
            if (player) {
                // Get Rudp context for this player, close that context down (
                // which will in turn close the socket if required)
                ...
                // Delete the player itself and the mapping from context to
                // player map as this context is no longer valid
                delete player;
            ...
            // Zero last element, that isn't part of the currently sized array
            // anymore
            memset(&m_roomSyncData.players[m_roomSyncData.getPlayerCount()], 0,
                   sizeof(PlayerSyncData));
            m_aRoomSlotPlayers[m_roomSyncData.getPlayerCount()] = NULL;
        } else {
            i++;
        }
...
void SQRNetworkManager_Vita::RemoveNetworkPlayers(int mask) {
    ...
    for (AUTO_VAR(it, m_RudpCtxToPlayerMap.begin());
         it != m_RudpCtxToPlayerMap.end();) {
        SQRNetworkPlayer* player = it->second;
        if ((player->m_roomMemberId == m_localMemberId) &&
            ((1 << player->m_localPlayerIdx) & mask)) {
            // Get Rudp context for this player, close that context down ( which
            // will in turn close the socket if required)
            int ctx = it->first;
            int err = sceRudpTerminate(ctx);
            assert(err == SCE_OK);
            if (m_listener) {
            ...
            // Delete the player itself and the mapping from context to player
            // map as this context is no longer valid
            delete player;
        } else {
            it++;
        }
...
// Host only - update the memberId of the local players, and synchronise with
// other room members
...
void SQRNetworkManager_Vita::SyncRoomData() {
    ...
    sceNpMatching2SetRoomDataInternal(m_matchingContext, &reqParam, NULL,
                                      &m_setRoomDataRequestId);
}

// Check if the matching context is valid, and if not attempt to create one. If
// to do this requires starting an asynchronous process, then sets the internal
// state to the state passed in before doing this. Returns true on success.
bool SQRNetworkManager_Vita::GetMatchingContext(
    eSQRNetworkManagerInternalState asyncState) {
    ...
    if (ret < 0) {
        app.DebugPrintf(
            "SQRNetworkManager::GetMatchingContext - sceNpMatching2Init2 "
    ...
    ret = sceNpMatching2CreateContext(&npId, &s_npCommunicationId,
                                      &s_npCommunicationPassphrase,
                                      &m_matchingContext /*, option*/);
    // ret = sceNpMatching2CreateContext(&npId, NULL,NULL, &m_matchingContext/*,
    // option*/);
    if (ret < 0) {
        app.DebugPrintf(
            "SQRNetworkManager::GetMatchingContext - "
            "sceNpMatching2CreateContext failed with code 0x%08x\n",
            ret);
        return false;
    }
    ...
    if (!RegisterCallbacks()) {
        app.DebugPrintf(
            "SQRNetworkManager::GetMatchingContext - RegisterCallbacks "
            "failed\n");
        return false;
    }

    // Set internal state & kick off async process that will actually start the
    // context.
    SetState(asyncState);
    app.DebugPrintf("sceNpMatching2ContextStart\n");
    ret = sceNpMatching2ContextStart(m_matchingContext, (10 * 1000 * 1000));
    if (ret < 0) {
        ...
        app.DebugPrintf(
            "SQRNetworkManager::GetMatchingContext - "
            "sceNpMatching2ContextStartAsync failed with code 0x%08x\n",
            ret);
        return false;
    }

    app.DebugPrintf(
        "SQRNetworkManager::GetMatchingContext - matching context is now "
        "valid\n");
    m_matchingContextValid = true;
    return true;
}

// Starts the process of obtaining a server context. This is an asynchronous
// operation, at the end of which (if successful), we'll be creating a room.
// General procedure followed here is as suggested by Sony - we get a list of
// servers, then pick a random one, and see if it is available. If not we just
// cycle round trying other random ones until we either find an available one or
// fail.
bool SQRNetworkManager_Vita::GetServerContext() {
    ...
    if (!GetMatchingContext(SNM_INT_STATE_HOSTING_STARTING_MATCHING_CONTEXT))
        return false;
    // If this caused an async thing to be started up, then we've done as much
    // as we can here - the rest of the code will happen when the async matching
    // 2 context starting completes ( event SCE_NP_MATCHING2_CONTEXT_EVENT_Start
    // is received )
...
// Code split out from previous method, so we can also call from creating
// matching context if required
bool SQRNetworkManager_Vita::GetServerContext2() {
    m_aServerId = (SceNpMatching2ServerId*)realloc(
        m_aServerId, sizeof(SceNpMatching2ServerId) * 1);
...
// Overloaded method for (as before) obtaining a server context. This version is
// so that can also get a server context for a specific server rather than a
// random one, using mainly the same code by making a single element list. This
// is used when joining an existing room.
bool SQRNetworkManager_Vita::GetServerContext(SceNpMatching2ServerId serverId) {
    if (m_state == SNM_INT_STATE_STARTING_CONTEXT) {
        // MGH - added for devtrack 5936 : race between the context starting
        // after going online, and trying to start it here, so skip this one if
        // we're already starting.
        m_serverCount = 1;
        m_totalServerCount = m_serverCount;
        m_aServerId = (SceNpMatching2ServerId*)realloc(
            m_aServerId, sizeof(SceNpMatching2ServerId) * m_serverCount);
    ...
    if (!GetMatchingContext(SNM_INT_STATE_JOINING_STARTING_MATCHING_CONTEXT)) {
        app.DebugPrintf(
            "SQRNetworkManager::GetServerContext - Failed due to no matching "
            "context\n");
        return false;
    }

    // 4J Stu - If this state is set, then we have successfully created a new
    // context but it won't have started yet Therefore the
    // sceNpMatching2GetServerIdListLocal call will fail. If we just skip this
    // check everything should be good.
    //	if( m_state != SNM_INT_STATE_JOINING_STARTING_MATCHING_CONTEXT )
    //	{
    // 		// Get list of server IDs of servers allocated to the
    // application. We don't actually need to do this, but it is as good a way
    // as any to try a matching2 service and check that
    // 		// the context *really* is valid.
    // 		int serverCount = sceNpMatching2GetServerIdListLocal(
    // m_matchingContext, NULL, 0 );
    // 		// If an error is returned here, we need to destroy and recerate
    // our server - if this goes ok we should come back through this path again
    // 		if( ( serverCount == SCE_NP_MATCHING2_ERROR_CONTEXT_UNAVAILABLE
    ...
    // // Also checking for this as a means of simulating the previous error
    // 		{
    // 			sceNpMatching2DestroyContext(m_matchingContext);
    // 			m_matchingContextValid = false;
    // 			if(
    ...
    // If one of the previous GetMatchingContext calls caused an async thing to
    // be started up, then we've done as much as we can here - the rest of the
    // code will happen when the async matching 2 context starting completes (
...
// Tick to update the search for a server which is available, for the creation
// of a server context.
void SQRNetworkManager_Vita::ServerContextTick() {
    switch (m_state) {
        ...
        case SNM_INT_STATE_JOINING_SERVER_SEARCH_SERVER_ERROR:
            ...
            SetState(
                (m_state == SNM_INT_STATE_HOSTING_SERVER_SEARCH_SERVER_ERROR)
                    ? SNM_INT_STATE_HOSTING_SEARCHING_FOR_SERVER
                    : SNM_INT_STATE_JOINING_SEARCHING_FOR_SERVER);
...
void SQRNetworkManager_Vita::RoomCreateTick() {
    switch (m_state) {
        ...
        case SNM_INT_STATE_HOSTING_CREATE_ROOM_WORLD_FOUND: {
            ...
            optSignalingParam.hubMemberId =
                0;  // Room owner is the hub of the star
            ...
            int ret = sceNpMatching2CreateJoinRoom(
                m_matchingContext, &reqParam, NULL, &m_createRoomRequestId);
            if ((ret < 0) ||
                ForceErrorPoint(SNM_FORCE_ERROR_CREATE_JOIN_ROOM)) {
...
// For a player using the network to communicate, flag as having its connection
// complete. This wraps the player's own functionality, so that we can determine
// if this call is transitioning us from not ready to ready, and call a
// registered callback.
void SQRNetworkManager_Vita::NetworkPlayerConnectionComplete(
    SQRNetworkPlayer* player) {
    ...
    if (!m_isHosting) {
        // For clients, if we are ready (up the the point of having received our
        // small id) then confirm to the host that this is the case, which makes
        // us now fully ready at this end
...
// For a player using the network to communicate, set its small id, thereby
// flagging it as having one allocated
void SQRNetworkManager_Vita::NetworkPlayerSmallIdAllocated(
    SQRNetworkPlayer* player, unsigned char smallId) {
    EnterCriticalSection(&m_csPlayerState);
    bool wasReady = player->IsReady();
    bool wasClientReady = player->HasConnectionAndSmallId();
...
// On host, for a player using the network to communicate, confirm that its
// small id has now been received back
void SQRNetworkManager_Vita::NetworkPlayerInitialDataReceived(
...
// For non-network players, flag that it is complete/ready, and assign its small
// id. We don't want to call any callbacks for these, as that can be explicitly
// done when local players are added. Also, we dynamically destroy & recreate
// local players quite a lot when remapping player slots which would create a
// lot of messages we don't want.
void SQRNetworkManager_Vita::NonNetworkPlayerComplete(SQRNetworkPlayer* player,
...
void SQRNetworkManager_Vita::HandlePlayerJoined(SQRNetworkPlayer* player) {
    ...
    // On client, keep a count of how many local players we have told the game
    // about. We can only transition to telling the game that we are playing
    // once the room is set up And all the local players are valid to use.
    if (!m_isHosting) {
        if (player->IsLocal()) {
            m_localPlayerJoined++;
        }
...
// Selects a random server from the current list, removes that server so it
// won't be searched for again, and then kick off an attempt to find out if that
// particular server is available.
bool SQRNetworkManager_Vita::SelectRandomServer() {
    ...
    if (m_serverCount == 0) {
        SetState((m_state == SNM_INT_STATE_HOSTING_SEARCHING_FOR_SERVER)
                     ...
                     : SNM_INT_STATE_JOINING_SERVER_SEARCH_FAILED);
        app.DebugPrintf(
            "SQRNetworkManager::SelectRandomServer - Server count is 0\n");
        return false;
    }

    // not really selecting a random server, as we've already been allocated
    // one, but calling this to match PS3
    int serverIdx;
    serverIdx = 0;
    m_serverCount--;
...
// Delete the current server context. Should be called when finished with the
// current host or client game session.
void SQRNetworkManager_Vita::DeleteServerContext() {
    // No server context on PS4, so we just set the state, and then we'll check
    // all the UDP connections have shutdown before setting to idle
...
// Creates a set of Rudp connections by the "active open" method. This requires
// that both ends of the connection call cellRudpInitiate to fully create a
// connection. We create one connection per local play on any remote machine.
//
...
// ie for the host (when matching incoming connections), these will be the same
// thing... and for the client, peerMemberId will be the host, whereas
// playersMemberId will be itself

static std::string getIPAddressString(SceNetInAddr add) {
    char str[32];
    unsigned char* vals = (unsigned char*)&add.s_addr;
    sprintf(str, "%d.%d.%d.%d", (int)vals[0], (int)vals[1], (int)vals[2],
            (int)vals[3]);
...
bool SQRNetworkManager_Vita::CreateSocket() {
    ...
    // Create socket & bind
    ret =
        sceNetSocket("rupdSocket", SCE_NET_AF_INET, SCE_NET_SOCK_DGRAM_P2P, 0);
    assert(ret >= 0);
    m_soc = ret;
    int optval = 1;
    ret = sceNetSetsockopt(m_soc, SCE_NET_SOL_SOCKET, SCE_NET_SO_USECRYPTO,
                           &optval, sizeof(optval));
    if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_SETSOCKOPT_0))
        return false;
    ret = sceNetSetsockopt(m_soc, SCE_NET_SOL_SOCKET, SCE_NET_SO_USESIGNATURE,
                           &optval, sizeof(optval));
    if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_SETSOCKOPT_1))
        return false;
    ret = sceNetSetsockopt(m_soc, SCE_NET_SOL_SOCKET, SCE_NET_SO_NBIO, &optval,
                           sizeof(optval));
    if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_SETSOCKOPT_2))
        return false;
...
bool SQRNetworkManager_Vita::CreateVoiceRudpConnections(
    SceNpMatching2RoomId roomId, SceNpMatching2RoomMemberId peerMemberId,
    int playerMask) {
    ...
    int ret = sceNpMatching2SignalingGetConnectionStatus(
        m_matchingContext, roomId, peerMemberId, &connStatus,
        &sinp2pPeer.sin_addr, &sinp2pPeer.sin_port);
    ...
    SQRVoiceConnection* pConnection =
        SonyVoiceChat_Vita::getVoiceConnectionFromRoomMemberID(peerMemberId);
    if (pConnection == NULL) {
        // Create an Rudp context for the voice connection, this will happen
        // regardless of whether the peer is client or host
        int rudpCtx;
        ret = sceRudpCreateContext(RudpContextCallback, this, &rudpCtx);
        if (ret < 0) {
            app.DebugPrintf("sceRudpCreateContext failed : 0x%08x\n", ret);
            assert(0);
        }
        if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_CREATE_RUDP_CONTEXT))
            return false;

        // Bind the context to the socket we've just created, and initiate. The
        // initiation needs to happen on both client & host sides of the
        // connection to complete.
        ret = sceRudpBind(rudpCtx, m_soc, 5, SCE_RUDP_MUXMODE_P2P);
        if (ret < 0) {
            app.DebugPrintf("sceRudpBind failed : 0x%08x\n", ret);
            assert(0);
        }
        if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_RUDP_BIND))
            return false;

        ret = sceRudpInitiate(rudpCtx, (SceNetSockaddr*)&sinp2pPeer,
                              sizeof(sinp2pPeer), 0);
        if (ret < 0) {
            app.DebugPrintf("sceRudpInitiate failed : 0x%08x\n", ret);
            assert(0);
        }
        if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_RUDP_INIT2))
            return false;

        app.DebugPrintf("-----------------------------\n");
        app.DebugPrintf("Voice rudp context created %d connected to %s\n",
                        rudpCtx,
                        getIPAddressString(sinp2pPeer.sin_addr).c_str());
        app.DebugPrintf("-----------------------------\n");

        pConnection =
            SonyVoiceChat_Vita::addRemoteConnection(rudpCtx, peerMemberId);
    }
...
bool SQRNetworkManager_Vita::CreateRudpConnections(
    SceNpMatching2RoomId roomId, SceNpMatching2RoomMemberId peerMemberId,
    int playerMask, SceNpMatching2RoomMemberId playersMemberId) {
    // First get details of the UDPP2P connection that has been established
    int connStatus;
    SceNetSockaddrIn sinp2pPeer;
    ...
    int ret = sceNpMatching2SignalingGetConnectionStatus(
        ...
        &sinp2pPeer.sin_addr, &sinp2pPeer.sin_port);
    app.DebugPrintf(CMinecraftApp::USER_RR,
                    "sceNpMatching2SignalingGetConnectionStatus returned 0x%x, "
                    "connStatus %d peer add:%s peer port:0x%x\n",
                    ret, connStatus,
                    getIPAddressString(sinp2pPeer.sin_addr).c_str(),
                    sinp2pPeer.sin_port);
    ...
    // Create an Rudp context for each local player that is required. These can
    // be used as individual virtual connections between room members (ie
    // consoles), which are multiplexed over the socket we have just made
    for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT; i++) {
        if ((playerMask & (1 << i)) == 0) continue;

        int rudpCtx;

        // Socket for the local network node created, now can create an Rupd
        // context.
        ret = sceRudpCreateContext(RudpContextCallback, this, &rudpCtx);
        if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_CREATE_RUDP_CONTEXT))
            return false;
        if (m_isHosting) {
            m_RudpCtxToPlayerMap[rudpCtx] =
                new SQRNetworkPlayer(this, SQRNetworkPlayer::SNP_TYPE_REMOTE,
                                     true, playersMemberId, i, rudpCtx, NULL);
        } else {
            // Local players can establish their UID at this point
            PlayerUID localUID;
            ProfileManager.GetXUID(i, &localUID, true);

            m_RudpCtxToPlayerMap[rudpCtx] = new SQRNetworkPlayer(
                this, SQRNetworkPlayer::SNP_TYPE_LOCAL, false, m_localMemberId,
                i, rudpCtx, &localUID);
        }

        // If we've created a player, then we want to try and patch up any
        // connections that we should have to it
        MapRoomSlotPlayers();

        // TODO - set any non-default options for the context. By default, the
        // context is set to have delivery critical and order critical both on
...
SQRNetworkPlayer* SQRNetworkManager_Vita::GetPlayerFromRoomMemberAndLocalIdx(
    int roomMember, int localIdx) {
    for (AUTO_VAR(it, m_RudpCtxToPlayerMap.begin());
         it != m_RudpCtxToPlayerMap.end(); it++) {
        if ((it->second->m_roomMemberId == roomMember) &&
            (it->second->m_localPlayerIdx == localIdx)) {
            return it->second;
        }
    }
    return NULL;
...
// This is called as part of the general initialisation of the network manager,
// to register any callbacks that the sony libraries require. Returns true if
...
bool SQRNetworkManager_Vita::RegisterCallbacks() {
    ...
    if (ret < 0) {
        app.DebugPrintf(
            "SQRNetworkManager::RegisterCallbacks - cellRudpSetEventHandler "
            "failed with code 0x%08x\n",
            ret);
        return false;
    }
    ...
    if (ret < 0) {
        app.DebugPrintf(
            "SQRNetworkManager::RegisterCallbacks - "
            "sceNpMatching2RegisterContextCallback failed with code 0x%08x\n",
            ret);
        return false;
    }
    ...
    if (ret < 0) {
        app.DebugPrintf(
            "SQRNetworkManager::RegisterCallbacks - "
            "sceNpMatching2SetDefaultRequestOptParam failed with code 0x%08x\n",
            ret);
        return false;
    }
    ...
    if (ret < 0) {
        app.DebugPrintf(
            "SQRNetworkManager::RegisterCallbacks - "
            "sceNpMatching2RegisterRoomEventCallback failed with code 0x%08x\n",
            ret);
        return false;
    }
...
// This is an implementation of SceNpMatching2ContextCallback. Used to determine
// whether the matching 2 context is valid or not.
void SQRNetworkManager_Vita::ContextCallback(
    ...
    switch (event) {
        case SCE_NP_MATCHING2_CONTEXT_EVENT_STARTED:
            ...
            if (errorCode < 0) {
                if (manager->m_state ==
                    ...
                    manager->m_state ==
                        SNM_INT_STATE_JOINING_STARTING_MATCHING_CONTEXT) {
                    // matching context failed to start (this can happen when
                    // you block the IP addresses of the matching servers on
                    // your router agent-0101.ww.sp-int.matching.playstation.net
                    // (198.107.157.191)
                    // static-resource.sp-int.community.playstation.net
                    // (203.105.77.140)
                    manager->SetState(SNM_INT_STATE_INITIALISE_FAILED);
                    break;
                }
            }
            // Some special cases to detect when this event is coming in, in
            // case we had to start the matching context because there wasn't a
            // valid context when we went to get a server context. These two
            // responses here complete what should then happen to get the server
            // context in each case (for hosting or joining a game)
            if (manager->m_state ==
                SNM_INT_STATE_IDLE_RECREATING_MATCHING_CONTEXT) {
            ...
            // Normal handling of context starting, from standard initialisation
            // procedure
            ...
            if (errorCode < 0) {
                ...
                // 4J-PB - SQRNetworkManager_PS3::AttemptPSNSignIn was causing
                // crashes in Iggy by calling LoadMovie from a callback, so call
                // it from the tick instead
                ...
                // once, the first time we have all our networking stuff set up
                // on boot-up
        ...
        case SCE_NP_MATCHING2_CONTEXT_EVENT_START_OVER:
            ...
            sceNpMatching2DestroyContext(manager->m_matchingContext);
            if (manager->m_state ==
                SNM_INT_STATE_IDLE_RECREATING_MATCHING_CONTEXT)  // MGH - added
                                                                 // this to
                                                                 // catch when
                                                                 // the context
                                                                 // start fails
                                                                 ...
                                                                 // GetExtDataForRoom
            {
                if (manager->m_FriendSessionUpdatedFn)
                    manager->m_FriendSessionUpdatedFn(
                        false, manager->m_pParamFriendSessionUpdated);
            }
            manager->m_matchingContextValid = false;
            manager->m_offlineSQR = true;
            manager->SetState(SNM_INT_STATE_INITIALISE_FAILED);
...
// This is an implementation of SceNpMatching2RequestCallback. This callback is
// used by default for any matching 2 request functions.
void SQRNetworkManager_Vita::DefaultRequestCallback(
    SceNpMatching2ContextId id, SceNpMatching2RequestId reqId,
    SceNpMatching2Event event, int errorCode, const void* data, void* arg) {
    SQRNetworkManager_Vita* manager = (SQRNetworkManager_Vita*)arg;
    EnterCriticalSection(&manager->m_csMatching);
    // 	int ret;
    if (id != manager->m_matchingContext) {
    ...
    switch (event) {
            // This is the response to sceNpMatching2GetWorldInfoList, which is
            // called as part of the process to create a room (which needs a
            // world to be created in). We aren't anticipating using worlds in a
            // meaningful way so just getting the first world we find on the
            // server here, and then advancing the state so that the tick can
            // get on with the rest of the process.
        case SCE_NP_MATCHING2_REQUEST_EVENT_GET_WORLD_INFO_LIST: {
            app.DebugPrintf(
                "SCE_NP_MATCHING2_REQUEST_EVENT_GET_WORLD_INFO_LIST\n");
            ...
            if (errorCode == SCE_NP_MATCHING2_ERROR_NP_SIGNED_OUT) {
                ...
                // this already elsewhere and so can silently ignore any errors
                // coming in for pending requests here
                break;
            }
            ...
            if (errorCode == 0) {
                if (data != 0) {
                    // Currently just using first world - this may well be all
                    // that we need anyway
                    SceNpMatching2GetWorldInfoListResponse* pWorldList =
                        (SceNpMatching2GetWorldInfoListResponse*)data;
            ...
            // We get this error when starting a new game after a disconnect
            // occurred in a previous game with at least one remote player. Fix
            // by stopping/starting the matching context. We stop the context
            // here, which is picked up in the callback, and started again. Then
            // the start event is picked up and reattempts the
            ...
            app.DebugPrintf(
                "SCE_NP_MATCHING2_REQUEST_EVENT_GET_WORLD_INFO_LIST failed, "
                "errorCode 0x%x, data %d\n",
                errorCode, data);

            manager->SetState(SNM_INT_STATE_HOSTING_CREATE_ROOM_FAILED);
        } break;
            // This is the response to sceNpMatching2CreateJoinRoom, which if
            // successful means that we are just about ready to move to an
            // online state as host of a game. The final transition actually
            // occurs in the create room tick, on detecting that the state has
            // transitioned to SNM_INT_STATE_HOSTING_CREATE_ROOM_SUCCESS here.
        case SCE_NP_MATCHING2_REQUEST_EVENT_CREATE_JOIN_ROOM:
            ...
            if (ProfileManager.IsSignedInLive(ProfileManager.GetPrimaryPad()) ==
                false) {
                // MGH - added to catch a case where the user signed out of PSN,
                // but this still called back with no error
                break;
            }

            app.DebugPrintf(
                CMinecraftApp::USER_RR,
                ">> Creating room complete, time taken %d, error 0x%x\n",
                System::currentTimeMillis() - s_roomStartTime, errorCode);
            assert(manager->m_state ==
                   SNM_INT_STATE_HOSTING_CREATE_ROOM_CREATING_ROOM);
            if (errorCode == 0) {
                if (data != 0) {
                    SceNpMatching2CreateJoinRoomResponse* roomData =
                        (SceNpMatching2CreateJoinRoomResponse*)data;
            ...
            // as the final stage of the process started when calling the
            // JoinRoom method. If this is successful, then the state can change
            // to SNM_INT_STATE_JOINING_WAITING_FOR_LOCAL_PLAYERS. We can
            // transition out of that state once we have told the application
            // that all the local players have joined.
        case SCE_NP_MATCHING2_REQUEST_EVENT_JOIN_ROOM:
            ...
            if (errorCode == 0) {
                if (data != 0) {
                    SceNpMatching2JoinRoomResponse* roomData =
                        (SceNpMatching2JoinRoomResponse*)data;
                    ...
                    for (int i = 0;
                         i < roomData->roomDataInternal->roomBinAttrInternalNum;
                         i++) {
                        if (roomData->roomDataInternal->roomBinAttrInternal[i]
                                .data.id ==
                            SCE_NP_MATCHING2_ROOM_BIN_ATTR_INTERNAL_1_ID) {
                            assert(roomData->roomDataInternal
                                       ->roomBinAttrInternal[i]
                                       .data.size ==
                                   sizeof(manager->m_roomSyncData));
                            memcpy(&manager->m_roomSyncData,
                                   roomData->roomDataInternal[i]
                                       .roomBinAttrInternal[0]
                                       .data.ptr,
                                   sizeof(manager->m_roomSyncData));
            ...
            if (errorCode ==
                SCE_NP_MATCHING2_SERVER_ERROR_ROOM_FULL)  // MGH - added to fix
                                                          // "host has exited"
                                                          // error when 2
                                                          // players go after
                                                          // the final slot
            {
                app.DebugPrintf(
                    "setting DisconnectPacket::eDisconnect_ServerFull\n");
                Minecraft::GetInstance()->connectionDisconnected(
                    ProfileManager.GetPrimaryPad(),
                    DisconnectPacket::eDisconnect_ServerFull);
                app.SetDisconnectReason(
                    DisconnectPacket::
                        eDisconnect_ServerFull);  // MGH - added to fix when
                                                  // joining from an invite
            }
            break;
            // This is the response to
            // sceNpMatching2GetRoomMemberDataInternal.This only happens on the
            // host, as a response to an incoming connection being established,
            // when we kick off the request for room member internal data so
            // that we can determine what local players that remote machine is
            // intending to bring into the game. At this point we can activate
            // the host end of each of the Rupd connection that this machine
            // requires. We can also update our player slot data (which gets
            // syncronised back out to other room members) at this point.
        case SCE_NP_MATCHING2_REQUEST_EVENT_GET_ROOM_MEMBER_DATA_INTERNAL:
            ...
            if (manager->m_state == SNM_INT_STATE_INITIALISE_FAILED ||
                ...
                // aren't killed off so just break out here if we're not in the
                // expected state fixes  devtrack #5807
                app.DebugPrintf(
                    "SCE_NP_MATCHING2_REQUEST_EVENT_GET_ROOM_MEMBER_DATA_"
                    "INTERNAL - manager->GetState() : %d\n",
                    manager->GetState());
                break;
            }
            if (errorCode == 0) {
                if (data != 0) {
                    SceNpMatching2GetRoomMemberDataInternalResponse*
                        pRoomMemberData =
                            (SceNpMatching2GetRoomMemberDataInternalResponse*)
                                data;
                    assert(pRoomMemberData->roomMemberDataInternal
                               ->roomMemberBinAttrInternalNum == 1);

                    if (manager->m_isHosting) {
                        int playerMask = *(
                            (int*)(pRoomMemberData->roomMemberDataInternal
                                       ->roomMemberBinAttrInternal->data.ptr));

                        bool isFull = false;
                        bool success1 = manager->AddRemotePlayersAndSync(
                            pRoomMemberData->roomMemberDataInternal->memberId,
                            playerMask, &isFull);
                        bool success2;
                        if (success1) {
                            success2 = manager->CreateRudpConnections(
                                manager->m_room,
                        ...
                        // Something has gone wrong adding these players to the
                        // room - kick out the player
                        ...
                        reqParam.target =
                            pRoomMemberData->roomMemberDataInternal->memberId;
                        // Set flag to indicate whether we were kicked for being
                        // out of room or not
                        ...
                        if (pRoomMemberData->roomMemberDataInternal
                                ->roomMemberBinAttrInternal->data.ptr == NULL) {
                            // the host doesn't send out data, so this must be
                            // the host we're connecting to

                            // If we are the client, then we locally know what
                            // Rupd connections we need (from
                            // m_localPlayerJoinMask) and can kick this off.
                            manager->m_hostMemberId =
                                pRoomMemberData->roomMemberDataInternal
                                    ->memberId;
                            bool ret = manager->CreateRudpConnections(
                                manager->m_room,
                                pRoomMemberData->roomMemberDataInternal
                                    ->memberId,
                                manager->m_localPlayerJoinMask,
                                manager->m_localMemberId);
                            if (ret == false) {
        ...
        case SCE_NP_MATCHING2_REQUEST_EVENT_LEAVE_ROOM:
            ...
            // This is the response to sceNpMatching2LeaveRoom - from the Sony
            // docs, this doesn't ever fail so no need to do error checking here
            // 			SonyVoiceChat::signalDisconnected();
            assert(manager->m_state == SNM_INT_STATE_LEAVING);
            manager->DeleteServerContext();
            break;
            // This is the response to
            // SceNpMatching2GetRoomDataExternalListRequest, which happens when
            // we request the full details of a room we are interested in
            // joining
                ...
                if (data != 0) {
                    SceNpMatching2GetRoomDataExternalListResponse*
                        pExternalData =
                            (SceNpMatching2GetRoomDataExternalListResponse*)
                                data;
                    SceNpMatching2RoomDataExternal* pRoomExtData =
                        pExternalData->roomDataExternal;
                    if (pExternalData->roomDataExternalNum == 1) {
                        if (pRoomExtData->roomBinAttrExternalNum == 1) {
                            memcpy(manager->m_pExtDataToUpdate,
                                   pRoomExtData->roomBinAttrExternal[0].ptr,
                                   pRoomExtData->roomBinAttrExternal[0].size);
                            manager->m_FriendSessionUpdatedFn(
                                true, manager->m_pParamFriendSessionUpdated);
                        } else {
                            manager->m_FriendSessionUpdatedFn(
                                false, manager->m_pParamFriendSessionUpdated);
                        }
        ...
        case SCE_NP_MATCHING2_REQUEST_EVENT_SET_ROOM_DATA_EXTERNAL:
            ...
            if ((errorCode != 0) ||
                ...
                app.DebugPrintf(CMinecraftApp::USER_RR,
                                ...
                                "SetRoomDataExternal event in callback)\n",
                                errorCode);
                // If we ever fail to send the external room data, we start a
                // countdown so that we attempt to resend. Not sure how likely
                // it is that updating this will fail without the whole network
                // being broken, but if in particular we don't update the flag
                // to say that the session is joinable, then nobody is ever
                // going to see this session.
...
void SQRNetworkManager_Vita::RoomEventCallback(SceNpMatching2ContextId id,
    ...
    switch (event) {
        ...
        case SCE_NP_MATCHING2_ROOM_EVENT_KICKEDOUT: {
            ...
            // We've been kicked out. This server has rejected our attempt to
            ...
            bool bIsFull = false;
            if ((data) &&
                !manager->ForceErrorPoint(SNM_FORCE_ERROR_UPDATED_ROOM_DATA)) {
                // 				gotEventData = true;
...

            {
                ...
                if (pUpdateInfo) {
                    app.DebugPrintf("Further info: Error 0x%x, cause %d\n",
                                    pUpdateInfo->errorCode,
                                    pUpdateInfo->eventCause);
                }
                // If we're hosting, then handle this a bit like a disconnect,
                // in that we will shift the game into an offline game - but
                // don't need to actually leave the room since that has been
                // destroyed and so isn't there to be left anymore. Don't do
                // this if we are disconnected though, as we've already handled
                // this.
                if ((manager->m_isHosting) && !manager->m_bLinkDisconnected) {
                    // MGH - we're not receiving an
                    // SCE_NP_MATCHING2_SIGNALING_EVENT_DEAD after this so we
                    // have to remove all the remote players
                    while (manager->m_RudpCtxToPlayerMap.size()) {
                        SQRNetworkPlayer* pRemotePlayer =
                            manager->m_RudpCtxToPlayerMap.begin()->second;
                        manager->RemoveRemotePlayersAndSync(
                            pRemotePlayer->m_roomMemberId, 15);
                    }

                    // MGH - added a check for the PSN sign in state, we don't
                    // seem to get the signed out matching2 error here
                    bool bSignedInPSN = ProfileManager.IsSignedInPSN(
                        ProfileManager.GetPrimaryPad());
                    bool bSignedOutError =
                        pUpdateInfo &&
                        (pUpdateInfo->eventCause ==
        ...
        case SCE_NP_MATCHING2_ROOM_EVENT_UPDATED_ROOM_DATA_INTERNAL:
            // We are using the room internal data to synchronise the player
            // data stored in m_roomSyncData from the host to clients. The host
            // is the thing creating the internal room data, so it doesn't need
            // to update itself.
            if (!manager->m_isHosting) {
                // 				assert ( dataSize <=
                ...
                if ((data) && !manager->ForceErrorPoint(
                    ...
                    // 				gotEventData = true;
                    SceNpMatching2RoomDataInternalUpdateInfo* pRoomData =
                        (SceNpMatching2RoomDataInternalUpdateInfo*)(data);
                    for (int i = 0; i < pRoomData->newRoomBinAttrInternalNum;
                         i++) {
                        if (pRoomData->newRoomBinAttrInternal[i]->data.id ==
                            SCE_NP_MATCHING2_ROOM_BIN_ATTR_INTERNAL_1_ID) {
                            assert(pRoomData->newRoomBinAttrInternal[i]
                                       ->data.size ==
                                   sizeof(manager->m_roomSyncData));
                            memcpy(
                                &manager->m_roomSyncData,
                                pRoomData->newRoomBinAttrInternal[i]->data.ptr,
                                sizeof(manager->m_roomSyncData));
        ...
        case SCE_NP_MATCHING2_ROOM_EVENT_UPDATED_ROOM_MEMBER_DATA_INTERNAL:
            if (/*( errorCode == 0 )  && */ (!manager->ForceErrorPoint(
                SNM_FORCE_ERROR_UPDATED_ROOM_MEMBER_DATA_INTERNAL1))) {
                // We'll get this sync'd round all the connected clients, but we
                // only care about it on the host where we can use it to work
                // out if any RUDP connections need to be made or released
                if (manager->m_isHosting) {
                    // 					assert( dataSize <=
                    ...
                    // sceNpMatching2GetEventData(manager->m_matchingContext,
                    // eventKey, (void
                    // *)(manager->cRoomMemberDataInternalUpdate),
                    // SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_RoomMemberDataInternalUpdateInfo);
                    if ((data) &&
                        (!manager->ForceErrorPoint(
                        ...
                        int playerMask =
                            *((int*)(pRoomMemberData
                                         ->newRoomMemberBinAttrInternal[0]
                                         ->data.ptr));
                        int oldMask = manager->GetOldMask(
                            pRoomMemberData->newRoomMemberDataInternal
                                ->memberId);
                        int addedMask =
                            manager->GetAddedMask(playerMask, oldMask);
                        int removedMask =
                            manager->GetRemovedMask(playerMask, oldMask);
                        ...
                        if (addedMask != 0) {
                            bool success = manager->AddRemotePlayersAndSync(
                                ...
                                addedMask);
                            if (success) {
                                success = manager->CreateRudpConnections(
                                    manager->m_room,
                                    pRoomMemberData->newRoomMemberDataInternal
                                        ->memberId,
                                    addedMask,
                            ...
                            if ((!success) ||
                                ...
                                // Failed for some reason - signal back to the
                                // client that this is the case, by updating its
                                // internal data back again, rather than have it
                                // wait for a timeout on its rudp connection
                                // initialisation.
                                ...
                                success = manager->CreateVoiceRudpConnections(
                                    manager->m_room,
                                    pRoomMemberData->newRoomMemberDataInternal
                                        ->memberId,
                                    0);
                    ...
                    // If, as a client, we receive an updated room member data
                    // this could be for two reason. (1) Another client in the
                    // game has updated their own data as someone has
                    // joined/left the session (2) The server has set someone's
                    ...
                    if ((data) &&
                        ...
                        assert(
                            pRoomMemberData->newRoomMemberBinAttrInternalNum ==
                            1);
                        if (pRoomMemberData->newRoomMemberDataInternal
                                ->memberId == manager->m_localMemberId) {
                            int playerMask =
                                *((int*)(pRoomMemberData
                                             ->newRoomMemberBinAttrInternal[0]
                                             ->data.ptr));
                            if (playerMask != manager->m_localPlayerJoinMask) {
                                ...
                                if (manager->m_listener) {
                                    for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT;
                                         i++) {
                                        if (playersToRemove & (1 << i)) {
                                            manager->m_listener
                                                ->HandleAddLocalPlayerFailed(i);
                                            break;
                                        }
                                    }
                                }
    ...
    // 	// If we didn't get the event data, then we need to clear it, or the
    // system even queue will overflow 	if( !gotEventData )
    // 	{
...
// This is an implementation of SceNpMatching2SignalingCallback. We configure
// our too automatically create a star network of connections with the host at
// the hub, and can respond here to the connections being set up to layer
// sockets and Rudp on top.
void SQRNetworkManager_Vita::SignallingCallback(
    SceNpMatching2ContextId ctxId, SceNpMatching2RoomId roomId,
    SceNpMatching2RoomMemberId peerMemberId, SceNpMatching2Event event,
    int error_code, void* arg) {
    SQRNetworkManager_Vita* manager = (SQRNetworkManager_Vita*)arg;
    ...
    switch (event) {
        case SCE_NP_MATCHING2_SIGNALING_EVENT_DEAD: {
            if (manager->m_isHosting) {
                ...
                SQRVoiceConnection* pVoice =
                    SonyVoiceChat_Vita::getVoiceConnectionFromRoomMemberID(
                        peerMemberId);
                if (pVoice) {
                    SonyVoiceChat_Vita::disconnectRemoteConnection(pVoice);
                }
                if (peerMemberId == manager->m_hostMemberId ||
                    pVoice == NULL)  //  MGH - added check for voice, as we
                                     //  sometime get here before m_hostMemberId
                                     //  has been filled in
                {
                    // Host has left the game... so its all over for this client
                    // too. Finish everything up now, including deleting the
                    // server context which belongs to this gaming session This
                    // also might be a response to a request to leave the game
                    // from our end too so don't need to do anything in that
                    // case
        ...
        case SCE_NP_MATCHING2_SIGNALING_EVENT_ESTABLISHED: {
            // MGH - changed this to always get the data now, as we need to know
            // if the connecting peer is the host or not If we're the host, then
            // we need to get the data associated with the connecting peer to
            // know what connections we should be trying to match. So the actual
            // creation of connections happens when the response for this
            ...
            SceNpMatching2AttributeId attrs[1] = {
                SCE_NP_MATCHING2_ROOMMEMBER_BIN_ATTR_INTERNAL_1_ID};
...
int SQRNetworkManager_Vita::BasicEventCallback(int event, int retCode,
                                               uint32_t reqId, void* arg) {
    PSVITA_STUBBED;
    // 	SQRNetworkManager_Vita *manager = (SQRNetworkManager_Vita *)arg;
    // 	// We aren't allowed to actually get the event directly from this
    // callback, so send our own internal event to a thread dedicated to doing
    // this 	sceKernelTriggerUserEvent(m_basicEventQueue, sc_UserEventHandle,
    // NULL);
...
void SQRNetworkManager_Vita::OnlineCheck() {
    ...
    if (s_bFullVersion != ProfileManager.IsFullVersion()) {
        ...
        // we've switched from trial to full version here, if we're already
        // online, call InitialiseAfterOnline, as this is now returns
        // immediately in trial mode (devtrack #5921)
    ...
    if (GetOnlineStatus() == false) {
        ...
        if (bSignedIn == false) m_listener->HandleDisconnect(false);
    }
    UpdateOnlineStatus(bSignedIn);
}

// Implementation of CellSysutilCallback
void SQRNetworkManager_Vita::SysUtilCallback(uint64_t status, uint64_t param,
                                             void* userdata) {
    ...
    // 			{
    // 				// Failed, or user may have decided not to sign
    // in - maybe need to differentiate here
    // 				manager->SetState(SNM_INT_STATE_INITIALISE_FAILED);
    // 				if( s_SignInCompleteCallbackFn )
    // 				{
    // 					if( s_signInCompleteCallbackIfFailed )
...
void SQRNetworkManager_Vita::updateNetCheckDialog() {
    if (ProfileManager.IsSystemUIDisplayed()) {
        if (sceNetCheckDialogGetStatus() == SCE_COMMON_DIALOG_STATUS_FINISHED) {
            ...
            bool bConnectedOK =
                (netCheckResult.result == SCE_COMMON_DIALOG_RESULT_OK);
            if (bConnectedOK) {
                SceNetCtlInfo info;
                sceNetCtlInetGetInfo(SCE_NET_CTL_INFO_DEVICE, &info);
                if (info.device ==
                    SCE_NET_CTL_DEVICE_PHONE)  // 3G connection, we're not going
                                               // to allow this
                {
            ...
            if (bConnectedOK) {
                ...
                // Failed, or user may have decided not to sign in - maybe need
                // to differentiate here
                SetState(SNM_INT_STATE_INITIALISE_FAILED);
                if (s_SignInCompleteCallbackFn) {
                    if (s_signInCompleteCallbackIfFailed) {
                        s_SignInCompleteCallbackFn(s_SignInCompleteParam, false,
                                                   0);
                    }
                    s_SignInCompleteCallbackFn = NULL;
                }
...
// Implementation of CellRudpContextEventHandler. This is associate with an Rudp
// context every time one is created, and can be used to determine the status of
// each Rudp connection. We create one context/connection per local player on
// the non-hosting consoles.
void SQRNetworkManager_Vita::RudpContextCallback(int ctx_id, int event_id,
                                                 int error_code, void* arg) {
    SQRNetworkManager_Vita* manager = (SQRNetworkManager_Vita*)arg;
    switch (event_id) {
        ...
        case SCE_RUDP_CONTEXT_EVENT_WRITABLE: {
            SQRNetworkPlayer* player = manager->GetPlayerFromRudpCtx(ctx_id);
            // This event signifies that room has opened up in the write buffer,
            // so attempt to send something
            if (player) {
        ...
        case SCE_RUDP_CONTEXT_EVENT_READABLE:
            if (manager->m_listener) {
                ...
                if (pVoice) {
                    ...
                    SQRNetworkPlayer* playerIncomingData =
                    ...
                    if (manager->m_isHosting) {
                        ...
                        if (playerFrom && !playerFrom->HasSmallIdConfirmed()) {
                            if (dataSize >=
                                sizeof(SQRNetworkPlayer::InitSendData)) {
                                SQRNetworkPlayer::InitSendData ISD;
                                int bytesRead = playerFrom->ReadDataPacket(
                                    &ISD,
                                    sizeof(SQRNetworkPlayer::InitSendData));
                                if (bytesRead ==
                                    sizeof(SQRNetworkPlayer::InitSendData)) {
                                    manager->NetworkPlayerInitialDataReceived(
                                        playerFrom, &ISD);
                                    dataSize -=
                                        sizeof(SQRNetworkPlayer::InitSendData);
                                } else {
                    ...
                    if (dataSize > 0) {
                        ...
                        int bytesRead =
                            playerIncomingData->ReadDataPacket(data, dataSize);
                        if (bytesRead > 0) {
                            SQRNetworkPlayer *playerFrom, *playerTo;
                            if (manager->m_isHosting) {
                                // Data always going from a remote player, to
                                // the host
                                playerFrom =
                                    manager->GetPlayerFromRudpCtx(ctx_id);
                                playerTo = manager->m_aRoomSlotPlayers[0];
                            } else {
                                // Data always going from host player, to a
                                // local player
                            ...
                            if ((playerFrom != NULL) && (playerTo != NULL)) {
                                manager->m_listener->HandleDataReceived(
                                    playerFrom, playerTo, data, bytesRead);
                            }
                        }
                        delete[] data;
                    }
...
#ifdef __PS3__
int SQRNetworkManager_Vita::RudpEventCallback(int event_id, int soc,
                                              uint8_t const* data,
                                              std::size_t datalen,
                                              struct sockaddr const* addr,
                                              socklen_t addrlen, void* arg)
#else
int SQRNetworkManager_Vita::RudpEventCallback(
    int event_id, int soc, uint8_t const* data, std::size_t datalen,
    struct SceNetSockaddr const* addr, SceNetSocklen_t addrlen, void* arg)
#endif
{
    SQRNetworkManager_Vita* manager = (SQRNetworkManager_Vita*)arg;
...
// Called when the context has been created, and we are intending to create a
// room.
void SQRNetworkManager_Vita::ServerContextValid_CreateRoom() {
    ...
    if (!ForceErrorPoint(SNM_FORCE_ERROR_GET_WORLD_INFO_LIST)) {
        ...
        ret = sceNpMatching2GetWorldInfoList(m_matchingContext, &reqParam, NULL,
                                             &m_getWorldRequestId);
    }
    if (ret < 0) {
        SetState(SNM_INT_STATE_HOSTING_CREATE_ROOM_FAILED);
...
// Called when the context has been created, and we are intending to join a
// pre-existing room.
void SQRNetworkManager_Vita::ServerContextValid_JoinRoom() {
    ...
    // Join the room, passing the local player mask as initial binary data so
    // that the host knows what local players are here
    ...
    int ret = sceNpMatching2JoinRoom(m_matchingContext, &reqParam, NULL,
                                     &m_joinRoomRequestId);
...
void SQRNetworkManager_Vita::GetExtDataForRoom(
    SceNpMatching2RoomId roomId, void* extData,
    ...
    // All parameters will be NULL if this is being called a second time, after
    // creating a new matching context via one of the paths below (using
    // GetMatchingContext). NULL parameters therefore basically represents an
    // attempt to retry the last sceNpMatching2GetRoomDataExternalList
    if (extData != NULL) {
        aRoomId[0] = roomId;
    ...
    if (!GetMatchingContext(SNM_INT_STATE_IDLE_RECREATING_MATCHING_CONTEXT)) {
        // No matching context, and failed to try and make one. We're really
        // broken here.
        m_FriendSessionUpdatedFn(false, m_pParamFriendSessionUpdated);
        return;
    }

    // Kicked off an asynchronous thing that will create a matching context, and
    // then call this method back again (with NULL params) once done, so we can
    // reattempt. Don't do anything more now.
    if (m_state == SNM_INT_STATE_IDLE_RECREATING_MATCHING_CONTEXT) {
        app.DebugPrintf(
            "Having to recreate matching context, setting state to "
            "SNM_INT_STATE_IDLE_RECREATING_MATCHING_CONTEXT\n");
        return;
    }

    int ret = sceNpMatching2GetRoomDataExternalList(
        m_matchingContext, &reqParam, NULL, &m_roomDataExternalListRequestId);

    // If we hadn't properly detected that a matching context was unvailable, we
    // might still get an error indicating that it is from the previous call.
    // Handle similarly, but we need to destroy the context first.
    if (ret ==
        SCE_NP_MATCHING2_ERROR_CONTEXT_NOT_STARTED)  // Also checking for this
                                                     ...
                                                     // the previous error
    {
        ...
        // Kicked off an asynchronous thing that will create a matching context,
        // and then call this method back again (with NULL params) once done, so
        // we can reattempt. Don't do anything more now.
        if (m_state == SNM_INT_STATE_IDLE_RECREATING_MATCHING_CONTEXT) {
            return;
        }
    }
...
void SQRNetworkManager_Vita::AttemptPSNSignIn(
    int (*SignInCompleteCallbackFn)(void* pParam, bool bContinue, int pad),
    void* pParam, bool callIfFailed /*=false*/) {
    ...
    // -------------------------------------------------------------
    // MGH -  this code is duplicated in the adhoc manager now too, so any
    // changes will have to be made there too
    // -------------------------------------------------------------
    ...
    memcpy(&param.npCommunicationId.data, &s_npCommunicationId,
           sizeof(s_npCommunicationId));
    ...
    ProfileManager.SetSysUIShowing(true);
    app.DebugPrintf(
        "------------>>>>>>>>   sceNetCheckDialogInit : PSN Mode\n");
...
int SQRNetworkManager_Vita::SetRichPresence(const void* data) {
    const sce::Toolkit::NP::PresenceDetails* newPresenceInfo =
        (const sce::Toolkit::NP::PresenceDetails*)data;

    s_lastPresenceInfo.status = newPresenceInfo->status;
    // 	s_lastPresenceInfo.userInfo = newPresenceInfo->userInfo;
    s_lastPresenceInfo.presenceType =
    ...
    if (s_resendPresenceCountdown == 0) {
        s_resendPresenceCountdown =
            5;  // wait a few ticks before setting the rich presence value, so
                // if there's a few being set at one time (like on game startup)
                // we can send them all in a single call
    }

    // Return as if no error happened no matter what, as we'll be resending
    // ourselves if we need to and don't want the calling system to retry
    return 0;
}

void SQRNetworkManager_Vita::UpdateRichPresenceCustomData(
    void* data, unsigned int dataBytes) {
...
void SQRNetworkManager_Vita::SendLastPresenceInfo() {
    ...
    // MGH - On Vita, change this to use
    // SCE_NP_BASIC_IN_GAME_PRESENCE_TYPE_GAME_JOINING  at some point
    ...
    if (ProfileManager.IsSignedInLive(
            0))  // ProfileManager.getQuadrant(s_lastPresenceInfo.userInfo.userId)))
    {
        s_lastPresenceInfo.presenceType =
            SCE_NP_BASIC_IN_GAME_PRESENCE_TYPE_DEFAULT;
        // MGH - if we have data being sent, that means we're in an online game,
        // so let others join from our presence info on the XMB
        if (s_lastPresenceInfo.size > 0) {
            ...
            if (!pso->inviteOnly) {
                s_lastPresenceInfo.presenceType =
                    SCE_NP_BASIC_IN_GAME_PRESENCE_TYPE_GAME_JOINING;
            }
        }
        err = sce::Toolkit::NP::Presence::Interface::setPresence(
            &s_lastPresenceInfo);
    }

    if (err != SCE_TOOLKIT_NP_SUCCESS) {
        s_resendPresenceCountdown =
            (20 * 65);  // Bit over a minute before attempting to resend, we
                        // should get a new token every minute
    } else {
...
void SQRNetworkManager_Vita::SetPresenceFailedCallback() {
    s_presenceStatusDirty = true;
    s_resendPresenceCountdown =
        (20 * 65);  // Bit over a minute before attempting to resend, we should
                    // get a new token every minute
}

void SQRNetworkManager_Vita::SetPresenceDataStartHostingGame() {
    if (m_offlineGame) {
        SQRNetworkManager_Vita::UpdateRichPresenceCustomData(
            &c_presenceSyncInfoNULL,
            sizeof(SQRNetworkManager_Vita::PresenceSyncInfo));
    } else {
        SQRNetworkManager_Vita::PresenceSyncInfo presenceInfo;
        CPlatformNetworkManagerSony::SetSQRPresenceInfoFromExtData(
            &presenceInfo, m_joinExtData, m_room, m_serverId);
        SQRNetworkManager_Vita::UpdateRichPresenceCustomData(
            &presenceInfo, sizeof(SQRNetworkManager_Vita::PresenceSyncInfo));
        //		OrbisNPToolkit::createNPSession();
    }
}

int SQRNetworkManager_Vita::GetJoiningReadyPercentage() {
    if ((m_state == SNM_INT_STATE_HOSTING_SEARCHING_FOR_SERVER) ||
        (m_state == SNM_INT_STATE_JOINING_SEARCHING_FOR_SERVER)) {
...
void SQRNetworkManager_Vita::removePlayerFromVoiceChat(
    SQRNetworkPlayer* pPlayer) {
    if (pPlayer->IsLocal()) {
        SonyVoiceChat_Vita::disconnectLocalPlayer(
            pPlayer->GetLocalPlayerIndex());
    } else {
        int numRemotePlayersLeft = 0;
        for (int i = 0; i < MAX_ONLINE_PLAYER_COUNT; i++) {
            if (m_aRoomSlotPlayers[i]) {
                if (m_aRoomSlotPlayers[i] != pPlayer) {
                    if (m_aRoomSlotPlayers[i]->m_roomMemberId ==
                        pPlayer->m_roomMemberId)
        ...
        if (numRemotePlayersLeft == 0) {
            ...
            SQRVoiceConnection* pVoice =
                SonyVoiceChat_Vita::getVoiceConnectionFromRoomMemberID(
                    pPlayer->m_roomMemberId);
            if (pVoice) SonyVoiceChat_Vita::disconnectRemoteConnection(pVoice);
        }
    }
}
