...
void SQRNetworkManager_Vita::SyncRoomData()
{
	...
	app.DebugPr"sceNpMatching2CreateContext\n");
        ret = sceNpMatching2CreateContext(
            ...
            &m_matchingCo /*, option*/  // ret =
                                        // sceNpMatching2CreateContext(&npId,
                                        // NULL,NULL, &m_matchingContext/*,
                                        // option*/);
            if (ret < 0) {
...
SQRNetworkPlayer *SQRNetworkManager_Vita::GetPlayerFromRoomMemberAndLocalIdx(int roomMember, int localIdx)
{
	...
	switch( event )
	{
                    ...
                    if (manager->m_state ==
                        ...
                        case SCE_NP_MATCHING2_CONTEXT_EVENT_STOPPED:
                            app.DebugPr
                                ...
                                "STOPPED\n"  // Can happen when we stop the PSN
                                             // to switch to adhoc
                                             // mode//assert(false);
                                if (manager->m_state ==
                                    SNM_INT_STATE_HOSTING_CREATE_ROOM_RESTART_MATCHING_CONTEXT) {
		...
		break;
                    ...
                    LeaveCriticalSection(
                        &manager
                             ->m_csMatching)  // This is an implementation of
