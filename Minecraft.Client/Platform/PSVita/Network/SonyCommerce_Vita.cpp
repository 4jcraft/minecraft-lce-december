...
// SonyCommerce_Vita::m_sessionInfo;
...
std::vector<SonyCommerce_Vita::ProductInfo>*
    SonyCommerce_Vita::m_pProductInfoList = NULL;
SonyCommerce_Vita::ProductInfoDetailed*
    SonyCommerce_Vita::m_pProductInfoDetailed = NULL;
...
SonyCommerce_Vita::DownloadListInputParams
    SonyCommerce_Vita::m_downloadInputParams;

SonyCommerce_Vita::CallbackFunc SonyCommerce_Vita::m_callbackFunc = NULL;
// sys_memory_container_t
// SonyCommerce_Vita::m_memContainer = SYS_MEMORY_CONTAINER_ID_INVALID;
...
bool SonyCommerce_Vita::m_contextCreated =
    false;  ///< npcommerce2 context ID created?
SonyCommerce_Vita::Phase SonyCommerce_Vita::m_currentPhase =
    e_phase_stopped;  ///< Current commerce2 util
// char
// SonyCommerce_Vita::m_commercebuffer[SCE_NP_COMMERCE2_RECV_BUF_SIZE];

C4JThread* SonyCommerce_Vita::m_tickThread = NULL;
bool SonyCommerce_Vita::m_bLicenseChecked =
    false;  // Check the trial/full license for the game
bool SonyCommerce_Vita::m_bLicenseInstalled =
    false;  // set to true when the licence has been downloaded and installed
            // (but maybe not checked yet)
bool SonyCommerce_Vita::m_bDownloadsPending =
    false;  // set to true if there are any downloads happening in the
            // background, so we check for them completing, and install when
            // finished
bool SonyCommerce_Vita::m_bDownloadsReady =
    false;  // set to true if there are any downloads ready to install
bool SonyCommerce_Vita::m_bInstallingContent =
    false;  // set to true while new content is being installed, so we don't
            // fire it mulitple times
int SonyCommerce_Vita::m_iClearDLCCountdown =
    0;  // tick for a set number of frames before clearing the DLC, as sometimes
        // it doesn't register as being installed in time
bool SonyCommerce_Vita::m_bPurchasabilityUpdated =
    false;  // set to when any purchase flags change
SonyCommerce_Vita::Message SonyCommerce_Vita::m_lastMessage;

sce::Toolkit::NP::Utilities::Future<std::vector<sce::Toolkit::NP::ProductInfo> >
    g_productList;
sce::Toolkit::NP::Utilities::Future<sce::Toolkit::NP::CategoryInfo>
    g_categoryInfo;
sce::Toolkit::NP::Utilities::Future<sce::Toolkit::NP::ProductInfoDetailed>
    g_detailedProductInfo;
...
void SonyCommerce_Vita::CheckForTrialUpgradeKey_Callback(LPVOID param,
                                                         bool bFullVersion) {
    ...
    if (ProfileManager.IsFullVersion()) {
        ...
        //  		ProfileManager.InitialiseTrophies(
        //  SQRNetworkManager_Vita::GetSceNpCommsId(),
...
bool SonyCommerce_Vita::LicenseChecked() { return m_bLicenseChecked; }
...
void SonyCommerce_Vita::checkBackgroundDownloadStatus() {
    if (m_bInstallingContent) return;

    Future<SceAppUtilBgdlStatus> status;
    int ret =
        sce::Toolkit::NP::Commerce::Interface::getBgdlStatus(&status, false);
    if (ret == SCE_OK) {
        ...
        if (m_bDownloadsReady) bInstallContent = true;
        // and if there are any downloads still pending, we'll call this
        // function again
...
int SonyCommerce_Vita::TickLoop(void* lpParam) {
    ...
    while ((m_currentPhase != e_phase_stopped) &&
        ...
        if (m_iClearDLCCountdown >
            0)  // tick for a set number of frames before clearing the DLC, as
                // sometimes it doesn't register as being installed in time
        {
            m_iClearDLCCountdown--;
            if (m_iClearDLCCountdown == 0) {
                app.ClearDLCInstalled();
                if (g_NetworkManager
                        .IsInSession())  // we're in-game, could be a purchase
                                         // of a pack after joining an invite
                                         // from another player
...
void SonyCommerce_Vita::copyProductList(
    std::vector<ProductInfo>* pProductList,
    std::vector<sce::Toolkit::NP::ProductInfo>* pNPProductList) {
    ...
    for (int i = 0; i < numProducts; i++) {
        ...
        strncpy(tempInfo.productId, npInfo.productId,
                SCE_NP_COMMERCE2_PRODUCT_ID_LEN);
        strncpy(tempInfo.productName, npInfo.productName,
                SCE_NP_COMMERCE2_PRODUCT_NAME_LEN);
        strncpy(tempInfo.shortDescription, npInfo.shortDescription,
                SCE_NP_COMMERCE2_PRODUCT_SHORT_DESCRIPTION_LEN);
        ...
        // but also keep the price as a value in case it's 0 - we need to show
        // "free" for that
    ...
    pNPProductList->clear();  // clear the std::vector now we're done, this
                              // doesn't happen automatically for the next query

    // Set our result
    *pProductList = tempProductVec;
}

int SonyCommerce_Vita::getProductList(std::vector<ProductInfo>* productList,
                                      char* categoryId) {
    int ret;
    ...
    ret = sce::Toolkit::NP::Commerce::Interface::getProductList(&g_productList,
                                                                params, true);

    app.DebugPrintf(
        " ----||||---- sce::Toolkit::NP::Commerce::Interface::getProductList : "
        "\n \t categoryId %s\n",
        categoryId);
    if (ret < 0) {
        app.DebugPrintf(
            "CommerceInterface::getProductList() error. ret = 0x%x\n", ret);
        return ret;
    }

    if (g_productList.hasResult()) {
        // result has returned immediately (don't think this should happen, but
        // was handled in the samples
...
void SonyCommerce_Vita::copyCategoryInfo(
    CategoryInfo* pInfo, sce::Toolkit::NP::CategoryInfo* pNPInfo) {
    ...
    strcpy(pInfo->current.categoryDescription,
           pNPInfo->current.categoryDescription);
    ...
    if (pInfo->countOfSubCategories > 0) {
        std::list<sce::Toolkit::NP::CategoryInfoSub>::iterator iter =
            pNPInfo->subCategories.begin();
        std::list<sce::Toolkit::NP::CategoryInfoSub>::iterator iterEnd =
            pNPInfo->subCategories.end();
        ...
        while (iter != iterEnd) {
            ...
            strcpy(tempSubCatInfo.categoryDescription,
                   iter->categoryDescription);
...
int SonyCommerce_Vita::getCategoryInfo(CategoryInfo* pInfo, char* categoryId) {
    ...
    ret = sce::Toolkit::NP::Commerce::Interface::getCategoryInfo(
        &g_categoryInfo, params, true);
    app.DebugPrintf(
        " ----||||---- sce::Toolkit::NP::Commerce::Interface::getCategoryInfo "
        ": \n \t userID %d\n \t categoryId %s\n",
        userId, categoryId);
    if (ret < 0) {
        // error
...
void SonyCommerce_Vita::copyDetailedProductInfo(
    ProductInfoDetailed* pInfo,
    sce::Toolkit::NP::ProductInfoDetailed* pNPInfo) {
    // populate our temp struct
    //	pInfo->ratingDescriptors = npInfo.ratingSystemId;
    strncpy(pInfo->productId, pNPInfo->productId,
            SCE_NP_COMMERCE2_PRODUCT_ID_LEN);
    strncpy(pInfo->productName, pNPInfo->productName,
            SCE_NP_COMMERCE2_PRODUCT_NAME_LEN);
    strncpy(pInfo->shortDescription, pNPInfo->shortDescription,
            SCE_NP_COMMERCE2_PRODUCT_SHORT_DESCRIPTION_LEN);
    strncpy(pInfo->longDescription, pNPInfo->longDescription,
            SCE_NP_COMMERCE2_PRODUCT_LONG_DESCRIPTION_LEN);
    strncpy(pInfo->legalDescription, pNPInfo->legalDescription,
    ...
    strncpy(pInfo->ratingSystemId, pNPInfo->ratingSystemId,
            SCE_NP_COMMERCE2_RATING_SYSTEM_ID_LEN);
...
int SonyCommerce_Vita::getDetailedProductInfo(ProductInfoDetailed* pInfo,
    ...
    if (g_detailedProductInfo
            .get())  // MGH - clear the price out, in case something is hanging
                     // around from a previous call
    {
        g_detailedProductInfo.get()->intPrice = -1;
        g_detailedProductInfo.get()->price[0] = 0;
    }
    ret = sce::Toolkit::NP::Commerce::Interface::getDetailedProductInfo(
        &g_detailedProductInfo, params, true);
    app.DebugPrintf(
        " ----||||---- "
        "sce::Toolkit::NP::Commerce::Interface::getDetailedProductInfo : \n \t "
        "userID %d\n \t categoryId %s\n \t productId %s\n",
        userId, categoryId, productId);

    if (ret < 0) {
        app.DebugPrintf(
            "CommerceInterface::getDetailedProductInfo() error. ret = 0x%x\n",
            ret);
        return ret;
    }
...
void SonyCommerce_Vita::copyAddDetailedProductInfo(
    ProductInfo* pInfo, sce::Toolkit::NP::ProductInfoDetailed* pNPInfo) {
    // populate our temp struct
    //	pInfo->ratingDescriptors = npInfo.ratingSystemId;
    // 	strncpy(pInfo->productId, npInfo.productId,
    // SCE_NP_COMMERCE2_PRODUCT_ID_LEN); 	strncpy(pInfo->productName,
    // npInfo.productName, SCE_NP_COMMERCE2_PRODUCT_NAME_LEN);
    // 	strncpy(pInfo->shortDescription, npInfo.shortDescription,
    // SCE_NP_COMMERCE2_PRODUCT_SHORT_DESCRIPTION_LEN);
    strncpy(pInfo->longDescription, pNPInfo->longDescription,
            SCE_NP_COMMERCE2_PRODUCT_LONG_DESCRIPTION_LEN);
...
int SonyCommerce_Vita::addDetailedProductInfo(ProductInfo* pInfo,
                                              const char* productId,
                                              char* categoryId) {
    int ret;
    sce::Toolkit::NP::DetailedProductInfoInputParams params;
    ...
    if (ret < 0) {
        app.DebugPrintf(
            "CommerceInterface::addDetailedProductInfo() error. ret = 0x%x\n",
            ret);
    }
...
int SonyCommerce_Vita::checkout(CheckoutInputParams& params) {
    ...
    while (iter != iterEnd) {
        npParams.skuIds.push_back(
            (char*)*iter);  // have to remove the const here, not sure why the
                            // libs pointers aren't const
        iter++;
    }
...
int SonyCommerce_Vita::downloadList(DownloadListInputParams& params) {
    ...
    ret = sce::Toolkit::NP::Commerce::Interface::displayDownloadList(npParams,
                                                                     true);
    if (ret < 0) {
        app.DebugPrintf(
            "Commerce::Interface::displayDownloadList error: 0x%x\n", ret);
    }
    return ret;
}
...
int SonyCommerce_Vita::checkout_game(CheckoutInputParams& params) {
    ...
    strncpy(Myparams.productId, app.GetUpgradeKey(),
            strlen(app.GetUpgradeKey()));
    ...
    // we don't seem to get any of the productBrowse completion callbacks on
    // Vita, so just force us into that state next
...
int SonyCommerce_Vita::downloadList_game(DownloadListInputParams& params) {
    ...
    app.DebugPrintf("Starting Store Download List...\n");
    // 	ret =
    // sce::Toolkit::NP::Commerce::Interface::displayDownloadList(npParams,
    // true); 	if (ret < 0)
    // 	{
    // 		app.DebugPrintf("Commerce::Interface::displayDownloadList error:
    // 0x%x\n", ret);
    // 	}
    ...
    Myparams.serviceLabel = 0;
    strncpy(Myparams.productId, "EP4433-PCSB00560_00-MINECRAFTVIT0452",
            strlen("EP4433-PCSB00560_00-MINECRAFTVIT0452"));
...
void SonyCommerce_Vita::UpgradeTrialCallback1(LPVOID lpParam, int err) {
    ...
    if (err == SCE_OK) {
        ...
        if (s_trialUpgradeProductInfoDetailed.purchasabilityFlag ==
            ...
            pCommerce->DownloadAlreadyPurchased_Game(UpgradeTrialCallback2,
                                                     pCommerce, skuID);
        }
    } else {
        UINT uiIDA[1];
...
void SonyCommerce_Vita::UpgradeTrial(CallbackFunc cb, LPVOID lpParam) {
    ...
    GetDetailedProductInfo(UpgradeTrialCallback1, this,
...
int SonyCommerce_Vita::createContext() {
    ...
    // 	if(ret < 0)
    // 	{
    // 		app.DebugPrintf(4,"createContext sceNpManagerGetNpId
    // problem\n"); 		return ret;
    // 	}
    //
    // 	if (m_contextCreated) {
    ...
    // 		if (ret < 0)
    // 		{
    // 			app.DebugPrintf(4,"createContext
    // sceNpCommerce2DestroyCtx problem\n"); 			return ret;
    // 		}
    // 	}
    //
    // 	// Create commerce2 context
    // 	ret = sceNpCommerce2CreateCtx(SCE_NP_COMMERCE2_VERSION, &npId,
    // commerce2Handler, NULL, &m_contextId); 	if (ret < 0)
    // 	{
    // 		app.DebugPrintf(4,"createContext sceNpCommerce2CreateCtx
    // problem\n"); 		return ret;
    // 	}
...
int SonyCommerce_Vita::createSession() {
    // this does nothing now, we only catch session expired errors now and
    // recreate the session when needed.
    int ret = 0;
...
void SonyCommerce_Vita::commerce2Handler(const sce::Toolkit::NP::Event& event) {
    ...
    if (event.returnCode == SCE_NP_COMMERCE2_SERVER_ERROR_SESSION_EXPIRED) {
        // this will happen on the first commerce call, since there is no
        // session, so we create and then queue the request again
    ...
    switch (event.event) {
        ...
        case sce::Toolkit::NP::Event::UserEvent::commerceSessionCreated: {
            // the seesion has been recreated after an error, so queue the old
            // request back up now we're running again
        ...
        case sce::Toolkit::NP::Event::UserEvent::commerceGotCategoryInfo: {
            // 		int ret =
            // sce::Toolkit::NP::Commerce::Interface::getBgdlStatus(&status,
            // false); 		if(ret == SCE_OK)
            // 		{
        ...
        case sce::Toolkit::NP::Event::UserEvent::
            commerceGotDetailedProductInfo: {
            if (m_pProductInfoDetailed) {
                copyDetailedProductInfo(m_pProductInfoDetailed,
                                        g_detailedProductInfo.get());
                m_pProductInfoDetailed = NULL;
            } else {
                copyAddDetailedProductInfo(m_pProductInfo,
                                           g_detailedProductInfo.get());
                m_pProductInfo = NULL;
            }
            m_event = e_event_commerceGotDetailedProductInfo;
        ...
        case sce::Toolkit::NP::Event::UserEvent::commerceCheckoutFinished: {
            m_messageQueue.push(
                e_message_commerceEnd);  // MGH - fixes an assert when switching
                                         // to adhoc mode after this
...
void SonyCommerce_Vita::processMessage() {
    ...
    if (msg != e_message_commerceRecreateSession) m_lastMessage = msg;
    m_messageQueue.pop();
    ...
    switch (msg) {
        ...
        case e_message_commerceGetCategoryInfo: {
            ...
            if (ret < 0) {
                m_event = e_event_commerceError;
                app.DebugPrintf(
                    4, "ERROR - e_event_commerceGotCategoryInfo - %s\n",
                    m_pCategoryID);
                m_errorCode = ret;
            }
            break;
        ...
        case e_message_commerceGetDetailedProductInfo: {
            ret = getDetailedProductInfo(m_pProductInfoDetailed, m_pProductID,
                                         m_pCategoryID);
            if (ret < 0) {
                m_event = e_event_commerceError;
                m_errorCode = ret;
        ...
        case e_message_commerceAddDetailedProductInfo: {
            ret = addDetailedProductInfo(m_pProductInfo, m_pProductID,
                                         m_pCategoryID);
            if (ret < 0) {
                m_event = e_event_commerceError;
                m_errorCode = ret;
            ...
            // 		{
            // 			Job<std::vector<SceNpEntitlement> >
            // tmpJob(static_cast<Future<std::vector<SceNpEntitlement> >
            // *>(msg.output));
            //
            // 			int state = 0;
            ...
            // 				}
            // 			}
            // 			_TOOLKIT_NP_DEL (EntitlementToConsume
            // *)msg.inputArgs;
            //
            // 			break;
            // 		}
...
void SonyCommerce_Vita::processEvent() {
    ...
    switch (m_event) {
        ...
        case e_event_commerceProductBrowseFinished:
            app.DebugPrintf(
                4, "e_event_commerceProductBrowseFinished succeeded: 0x%x\n",
                m_errorCode);
        ...
        case e_event_commerceCheckoutFinished:
            ...
            if (m_callbackFunc != NULL) {
                // get the detailed product info again, to see if the purchase
                // has happened or not
                EnterCriticalSection(&m_queueLock);
...
void SonyCommerce_Vita::CreateSession(CallbackFunc cb, LPVOID lpParam) {
    ...
    // I had this happen when I was offline on Vita, and accepted the PSN
    // sign-in the m_errorCode was picked up in the message queue after the
    // commerce init call
    if (m_errorCode != 0) {
...
void SonyCommerce_Vita::GetProductList(CallbackFunc cb, LPVOID lpParam,
...
void SonyCommerce_Vita::GetDetailedProductInfo(CallbackFunc cb, LPVOID lpParam,
                                               ProductInfoDetailed* productInfo,
                                               const char* productId,
                                               const char* categoryId) {
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
...
void SonyCommerce_Vita::AddDetailedProductInfo(CallbackFunc cb, LPVOID lpParam,
...
void SonyCommerce_Vita::GetCategoryInfo(CallbackFunc cb, LPVOID lpParam,
                                        CategoryInfo* info,
                                        const char* categoryId) {
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
...
void SonyCommerce_Vita::Checkout(CallbackFunc cb, LPVOID lpParam,
                                 ProductInfo* productInfo) {
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
...
void SonyCommerce_Vita::DownloadAlreadyPurchased(CallbackFunc cb,
                                                 LPVOID lpParam,
                                                 const char* skuID) {
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
...
void SonyCommerce_Vita::Checkout_Game(CallbackFunc cb, LPVOID lpParam,
                                      const char* skuID) {
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
    m_checkoutInputParams.skuIds.clear();
...
void SonyCommerce_Vita::DownloadAlreadyPurchased_Game(CallbackFunc cb,
                                                      LPVOID lpParam,
                                                      const char* skuID) {
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
...
void SonyCommerce_Vita::InstallContent(CallbackFunc cb, LPVOID lpParam) {
    if (m_callbackFunc == NULL &&
        m_messageQueue.size() == 0)  // wait till other processes have finished
    {
        EnterCriticalSection(&m_queueLock);
...
bool SonyCommerce_Vita::getDLCUpgradePending() {
    if (m_bDownloadsPending || m_bInstallingContent ||
        (m_iClearDLCCountdown > 0))
        return true;
    return false;
}
...
void SonyCommerce_Vita::Test()
{
        ...
        {
                GetCategoryInfo(testCallback, this, &g_categoryInfo2,
g_category); g_bDoCommerceGetCategoryInfo = false;
        }

        if(g_bDoCommerceGetProductInfoDetailed)
        {
                GetDetailedProductInfo(testCallback, this,
&g_productInfoDetailed, g_productInfo[0].productId, g_category);
                g_bDoCommerceGetProductInfoDetailed = false;
        }

        if(g_bDoCommerceCheckout)
        {
                //Checkout(testCallback, this,
g_skuID);//g_productInfoDetailed.skuId); Checkout(testCallback, this,
g_productInfoDetailed.skuId); g_bDoCommerceCheckout = false;
        }
