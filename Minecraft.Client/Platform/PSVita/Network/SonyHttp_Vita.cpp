    ...
    switch (sslErr) {
        case (SCE_HTTPS_ERROR_CERT): /* Verify error */
            ...
            if (sslErrDetail & SCE_HTTPS_ERROR_SSL_INVALID_CERT) {
                app.DebugPrintf(
                    "ssl verify error: invalid server cert or CA cert\n");
            }
...
bool SonyHttp_Vita::getDataFromURL(const char* szURL, void** ppOutData,
                                   int* pDataSize) {
    if (!bInitialised) return false;
    return http_get(szURL, ppOutData, pDataSize);
}

int SonyHttp_Vita::sslCallback(SceUInt32 verifyErr, SceSslCert* const sslCert[],
                               SceInt32 certNum, void* userArg) {
    SceInt32 i;
    (void)userArg;
...
bool SonyHttp_Vita::http_get_close(bool bOK, SceInt32 tmplId, SceInt32 connId,
                                   SceInt32 reqId) {
    SceInt32 ret;
    if (reqId > 0) {
...
bool SonyHttp_Vita::http_get(const char* targetUrl, void** ppOutData,
                             int* pDataSize) {
    ...
    ret =
        sceHttpCreateTemplate(TEST_USER_AGENT, SCE_HTTP_VERSION_1_1, SCE_TRUE);
    if (ret < 0) {
    ...
    ret =
        sceHttpCreateRequestWithURL(connId, SCE_HTTP_METHOD_GET, targetUrl, 0);
    if (ret < 0) {
