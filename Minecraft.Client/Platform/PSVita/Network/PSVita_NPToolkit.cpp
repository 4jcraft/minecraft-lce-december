...
// #define NP_TITLE_SECRET_HEX
// ""
// #define CLIENT_ID_FOR_SAMPLE	"c8c483e7-f0b4-420b-877b-307fcb4c3cdc"

// #define _USE_STANDARD_ALLOC

// sce::Toolkit::NP::Utilities::Future< sce::Toolkit::NP::NpSessionInformation >
// PSVitaNPToolkit::sm_createJoinFuture; sce::Toolkit::NP::NpSessionInformation
// PSVitaNPToolkit::m_currentSessionInfo;
sce::Toolkit::NP::Utilities::Future<sce::Toolkit::NP::MessageAttachment>
    PSVitaNPToolkit::m_messageData;
    ...
    switch (event.event) {
        ...
        case sce::Toolkit::NP::Event::messageError:
            app.DebugPrintf("NP messagingCallback - messageError: 0x%x\n",
                            event.returnCode);
            if (SQRNetworkManager_Vita::
                    m_bSendingInviteMessage)  // MGH - added to fix a sysUI
                                              // lockup on startup - devtrack
                                              // #5883
                ProfileManager.SetSysUIShowing(false);
            break;
        case sce::Toolkit::NP::Event::messageDialogTerminated:
            app.DebugPrintf(
                "NP messagingCallback - messageDialogTerminated: 0x%x\n",
                event.returnCode);
            ProfileManager.SetSysUIShowing(false);
            break;
        case sce::Toolkit::NP::Event::messageRetrieved:
            ...
            if (m_messageData.hasResult()) {
                SQRNetworkManager_Vita::GetInviteDataAndProcess(
                    m_messageData.get());
            } else {
        ...
        case sce::Toolkit::NP::Event::messageInGameDataReceived:
            app.DebugPrintf(
                "NP messagingCallback - messageInGameDataReceived: 0x%x\n",
                event.returnCode);
            break;
        case sce::Toolkit::NP::Event::messageInGameDataRetrievalDone:
            app.DebugPrintf(
                "NP messagingCallback - messageInGameDataRetrievalDone: 0x%x\n",
                event.returnCode);
            break;
...
void PSVitaNPToolkit::coreCallback(const sce::Toolkit::NP::Event& event) {
    switch (event.event) {
        case sce::Toolkit::NP::Event::
            enetUp:  ///< An event from the NetCtl service generated when a
                     ///< connection has been established.
            app.DebugPrintf("Received core callback: Network Up \n");
            break;
        case sce::Toolkit::NP::Event::
            enetDown:  ///< An event from the NetCtl service generated when the
                       ///< connection layer has gone down.
            app.DebugPrintf("Received core callback: Network down \n");
            break;
        case sce::Toolkit::NP::Event::
            loggedIn:  ///< An event from the NetCtl service generated when a
                       ///< connection to the PSN has been established.
            app.DebugPrintf("Received core callback: PSN sign in \n");
            SceNetCtlInfo info;
            sceNetCtlInetGetInfo(SCE_NET_CTL_INFO_DEVICE, &info);
            if (info.device ==
                SCE_NET_CTL_DEVICE_PHONE)  // 3G connection, we're not going to
                                           // allow this
            {
            ...
            break;
        case sce::Toolkit::NP::Event::
            loggedOut:  ///< An event from the NetCtl service generated when a
                        ///< connection to the PSN has been lost.
...
void PSVitaNPToolkit::sceNpToolkitCallback(
    const sce::Toolkit::NP::Event& event) {
...
// ///< An event generated when the %Np session creation process has been
// completed. 		app.DebugPrintf("npSessionCreateResult");
...
// 		else
// 		{
// 			app.DebugPrintf("Session Creation Failed
// 0x%x\n",sm_createJoinFuture.getError());
// 		}
// 		sm_createJoinFuture.reset();
// 		break;
// 	case sce::Toolkit::NP::Event::npSessionJoinResult:
// ///< An event generated when the join %Np session process has been completed.
...
// 		else
// 		{
// 			app.DebugPrintf("Session join Failed
// 0x%x\n",sm_createJoinFuture.getError());
// 		}
// 		sm_createJoinFuture.reset();
// 		break;
...
// 		break;
// 	case sce::Toolkit::NP::Event::npSessionModified:
// ///< An event generated when the %Np session has been modified.
// 		app.DebugPrintf("npSessionModified");
// 		break;
// 	case sce::Toolkit::NP::Event::npSessionUpdateResult:
// ///< An event generated when the %Np session has been updated.
// 		app.DebugPrintf("npSessionUpdateResult");
// 		break;
// 	case sce::Toolkit::NP::Event::npSessionGetInfoResult:
// ///< An event generated when the %Np session info has been retrieved.
// 		app.DebugPrintf("npSessionGetInfoResult");
// 		break;
// 	case sce::Toolkit::NP::Event::npSessionGetInfoListResult:
// ///< An event generated when the %Np session info has been retrieved.
// 		app.DebugPrintf("npSessionGetInfoListResult");
// 		break;
// 	case sce::Toolkit::NP::Event::npSessionGetSessionDataResult:	///< An
// event generated when the %Np session data has been retrieved.
...
// 		break;
// 	case sce::Toolkit::NP::Event::npSessionInviteGetInfoListResult:	///< An
// event generated when the %Np session info has been retrieved.
// 		app.DebugPrintf("npSessionInviteGetInfoListResult");
// 		break;
// 	case sce::Toolkit::NP::Event::npSessionInviteGetDataResult:
// ///< An event generated when the %Np session data has been retrieved.
...
void PSVitaNPToolkit::gameCustomDataCallback(
    const sce::Toolkit::NP::Event& event) {
    // 	switch(event.event)
    // 	{
    //
    ...
    // 		else
    // 		{
    // 			app.DebugPrintf("gameCustomDataMessageResult error
    // 0x%08x\n", m_messageData.getError());
    // 		}
    // 		break;
...
void hexStrToBin(const char* pHexStr, uint8_t* pBinBuf,
...
void PSVitaNPToolkit::init() {
    ...
    // 	nptTitleId.setTitleSecret(*SQRNetworkManager_Vita::GetSceNpTitleId(),
    // *SQRNetworkManager_Vita::GetSceNpTitleSecret());
    sce::Toolkit::NP::CommunicationId commsIds(s_npCommunicationId,
    ...
    ret = sce::Toolkit::NP::Interface::registerNpCommsId(
        commsIds, sce::Toolkit::NP::matching);
    if (ret < 0) {
        app.DebugPrintf("Failed to register TSS Comms ID : 0x%x\n", ret);
        assert(0);
    }

    // 	extern void npStateCallback(SceNpServiceState state, int retCode, void
    // *userdata);

    ret = sceNpRegisterServiceStateCallback(npStateCallback, NULL);
    if (ret < 0) {
        app.DebugPrintf(
            "sceNpRegisterServiceStateCallback() failed. ret = 0x%x\n", ret);
    }
...
// #define SESSION_IMAGE_PATH
// "/app0/orbis/session_image.png" #define SESSION_STATUS
// "Minecraft online game (this text needs defined and localised)" #define
// SESSION_NAME					"Minecraft(this text needs
...
// SCE_TOOLKIT_NP_CREATE_SESSION_TYPE_PUBLIC; 	createSessionRequest.maxSlots =
// maxSlots;
...
// 	ret =
// sce::Toolkit::NP::Sessions::Interface::create(&createSessionRequest,&sm_createJoinFuture);
// }
//
//
...
void PSVitaNPToolkit::getMessageData(SceAppUtilAppEventParam* paramData) {
    if (SCE_APPUTIL_APPEVENT_TYPE_NP_INVITE_MESSAGE == paramData->type) {
        sce::Toolkit::NP::Messaging::Interface::retrieveMessageAttachment(
            paramData, &m_messageData);
    } else if (SCE_APPUTIL_APPEVENT_TYPE_NP_APP_DATA_MESSAGE ==
               paramData->type) {
        sce::Toolkit::NP::Messaging::Interface::retrieveMessageAttachment(
            paramData, &m_messageData);
    } else if (SCE_APPUTIL_APPEVENT_TYPE_NP_BASIC_JOINABLE_PRESENCE ==
               paramData->type) {
        SceAppUtilNpBasicJoinablePresenceParam joinParam = {0};
        int ret = sceAppUtilAppEventParseNpBasicJoinablePresence(paramData,
                                                                 &joinParam);
        if (ret < 0) {
            app.DebugPrintf(
                "sceAppUtilAppEventParseNpBasicJoinablePresence() failed: "
                "0x%x\n",
                ret);
        } else {
            SQRNetworkManager_Vita::GetJoinablePresenceDataAndProcess(
                &joinParam);
        }
