...
void SonyRemoteStorage_Vita::staticInternalCallback(
    const SceRemoteStorageEvent event, int32_t retCode, void* userData) {
    ((SonyRemoteStorage_Vita*)userData)->internalCallback(event, retCode);
}

void SonyRemoteStorage_Vita::internalCallback(const SceRemoteStorageEvent event,
                                              int32_t retCode) {
    ...
    switch (event) {
        case ERROR_OCCURRED:
            ...
            m_status = e_error;
            // 		shutdown();				// removed, as
            // the remote storage lib now tries to reconnect if an error has
            // occurred
            runCallback();
        ...
        case GET_DATA_RESULT:
            if (retCode >= 0) {
                ...
                app.DebugPrintf(
                    "An error occurred while Get Data was being processed. "
                    "retCode: 0x%x \n",
                    retCode);
                m_status = e_error;
            }
        ...
        case GET_STATUS_RESULT:
            if (retCode >= 0) {
                ...
                for (int i = 0; i < outputGetStatus->numFiles; i++) {
                    ...
                    app.DebugPrintf(
                        "Visibility: \"%s\" \n",
                        (outputGetStatus->data[i].visibility == 0)
                            ? "Private"
                            : ((outputGetStatus->data[i].visibility == 1)
                                   ? "Public read only"
                ...
                app.DebugPrintf(
                    "An error occurred while Get Status was being processed. "
                    "retCode: 0x%x \n",
                    retCode);
                m_status = e_error;
            }
        ...
        case PSN_SIGN_IN_REQUIRED:
            app.DebugPrintf(
                "User's PSN sign-in through web browser is required \n");
        ...
        case SET_DATA_RESULT:
            if (retCode >= 0) {
                ...
                app.DebugPrintf(
                    "An error occurred while Set Data was being processed. "
                    "retCode: 0x%x \n",
                    retCode);
                m_status = e_error;
            }
        ...
        case WEB_BROWSER_RESULT:
            app.DebugPrintf(
                "This function is not used on PS Vita, as the account will be "
                "linked, it is not needed to open a browser to link it \n");
            assert(0);
            break;
...
bool SonyRemoteStorage_Vita::init(CallbackFunc cb, LPVOID lpParam) {
    ...
    int issuerId = 0;
    // 	ret = sceNpAuthGetAuthorizationCode(reqId, &authParams, &authCode,
    // &issuerId); 	if (ret < 0)
    // 	{
    ...
    // 	memcpy(params.authCode, authCode.code,
    // SCE_NP_AUTHORIZATION_CODE_MAX_LEN);
...
bool SonyRemoteStorage_Vita::getRemoteFileInfo(SceRemoteStorageStatus* pInfo,
                                               CallbackFunc cb,
                                               LPVOID lpParam) {
    m_callbackFunc = cb;
    m_callbackParam = lpParam;
...
void SonyRemoteStorage_Vita::abort() {
    ...
    if (m_bTransferStarted) {
        app.DebugPrintf(
            "transfer has started so we'll call sceRemoteStorageAbort...\n");
...
bool SonyRemoteStorage_Vita::getData(const char* remotePath,
                                     const char* localPath, CallbackFunc cb,
                                     LPVOID lpParam) {
    m_callbackFunc = cb;
    m_callbackParam = lpParam;
