...
// there. There's a bunch of code in this file, but most of it isn't really
// complicated. The bits that are somewhat tricky have a more detailed
// explanation at the top of the relevant section.
...
#define GDRAW_MIN_FREE_AMOUNT \
    (64 * 1024)  // always try to free at least this many bytes when throwing
                 // out old textures
...
struct GDraw {
    ...
    U32 z_stencil_key;  // field built from z/stencil test flags. 0 = no
                        // z/stencil test, ~0 is used for "unknown state"
    ...
    SceGxmFragmentProgram*
        main_fp[GDRAW_TEXTURE__count][3]
               [GDRAW_CBLEND__count];  // [texmode][additive][cblend]
...
static RADINLINE rrbool is_fence_pending(GDrawFence fence) {
    ...
    // flight!)
    if (gdraw->next_fence_index - fence.value > 0xffffffffu) return false;
...
static void wait_on_fence(GDrawFence fence) {
    if (is_fence_pending(fence)) {
        // Is the fence in a scene we haven't even submitted yet? We can't do
        // that.
        //
        // NOTE: you might see this from user code if you try to do the
        // following seqeuence within a single GXM scene:
        // - Render an Iggy
        ...
        // scene first.
        if (fence.value >=
            gdraw->next_fence_index)  // uh oh, it's in a scene we haven't even
                                      // submitted yet. this is a bug!
            RR_BREAK();
...
extern "C" void gdraw_psp2_wait(U64 fence_val) {
    ...
    // Debugger because you're stuck here, there's a problem with how you're
    // using GDraw. These two cases are related but seperate, and I'll cover
    // them here.
    //
    ...
    //
    //   Generally, GDraw will free resources in LRU (least recently used)
    //   order. This means that whatever memory is being freed has (hopefully)
    //   not been touched for a few frames, and GDraw should be able to reuse it
    ...
    //   pools. GDraw will warn about this - do you have an Iggy warning
    //   callback installed? Anyway, if there are resource thrashing warnings,
    //   try increasing the size of the corresponding resource pool and this
    //   stall should go away.
    //
    //   The other reason why GDraw would wait is because you're passing a
    //   "dynamic buffer" to gdraw_psp2_Begin() that the GPU is still reading
    //   from a previous frame. If that's the case (just check whether your
    //   callstack contains "WaitForDynamicBufferIdle"), you might be using just
    //   one dynamic buffer. Consider double- or triple-buffering it instead to
    //   avoid this kind of stall.
    //
    ...
    //   rendering a previous scene, because we're trying to reuse memory we
    //   used in that scene.
    //
    //   If you end up here, this could mean one of several things:
    //
    //   1. The GPU isn't writing the scene completion notifications that GDraw
    //   is
    //      waiting for. Did you remember to pass the SceGxmNotification
    //      returned by gdraw_psp2_End as "fragmentNotification" to
    //      sceGxmEndScene? This is necessary for GDraw's memory management to
    //      work!
    ...
    //      mechanism will break if we read unexpected valus! Just make sure
    //      that GDraw has its own notification label. There are lots of
    //      notification labels (SCE_GXM_NOTIFICATION_COUNT, 512 at the time of
    //      this writing) so this shouldn't be an issue.
    //   3. You are using gdraw_psp2_Begin incorrectly. You may only have one
    //   call
    ...
    //
    //   The latter needs a bit of explanation. If you're on an immediate
    //   context, it's pretty easy. In any GXM scene that's supposed to contain
    //   Iggy rendering, adhere to the following pattern:
    //
    ...
    while (is_fence_pending(fence)) sceGxmWaitEvent();
}
...
static void gdraw_gpu_memcpy(GDrawHandleCache* c, void* dst, void* src,
                             U32 num_bytes) {
    // "If srcFormat is SCE_GXM_TRANSFER_FORMAT_RAW128, then a maximum of 262144
    // pixels can be copied (512x512)."
    ...
    while (num_bytes - offs >= row_size) {
        ...
        sceGxmTransferCopy(
            max_width, num_rows, 0, 0, SCE_GXM_TRANSFER_COLORKEY_NONE,
            SCE_GXM_TRANSFER_FORMAT_RAW128, SCE_GXM_TRANSFER_LINEAR,
            srcp + offs, 0, 0, row_size, SCE_GXM_TRANSFER_FORMAT_RAW128,
            SCE_GXM_TRANSFER_LINEAR, dstp + offs, 0, 0, row_size, NULL, 0,
            NULL);
    ...
    if (offs < num_bytes) {
        U32 remaining_pixels =
            (num_bytes - offs + GPU_MEMCPY_ALIGN - 1) / GPU_MEMCPY_ALIGN;

        sceGxmTransferCopy(
            remaining_pixels, 1, 0, 0, SCE_GXM_TRANSFER_COLORKEY_NONE,
            SCE_GXM_TRANSFER_FORMAT_RAW128, SCE_GXM_TRANSFER_LINEAR,
            srcp + offs, 0, 0, row_size, SCE_GXM_TRANSFER_FORMAT_RAW128,
            SCE_GXM_TRANSFER_LINEAR, dstp + offs, 0, 0, row_size, NULL, 0,
            NULL);
...
static void handle_cache_tick(GDrawHandleCache* c, GDrawFence now,
                              GDrawStats* stats) {
...
static void RADLINK gdraw_UnlockHandles(GDrawStats* stats) {
    // We're on a tiled renderer; the Iggy side doesn't get to unlock
    // *anything*. We do the unlock ourselves once we're done with the scene, in
    // gdraw_psp2_End.
}
...
static void track_dynamic_alloc_attempt(U32 size, U32 align) {
    ...
    gdraw->dynamic_stats.largest_bytes_attempted =
        RR_MAX(gdraw->dynamic_stats.largest_bytes_attempted, size);
}

static void track_dynamic_alloc_failed() {
    if (gdraw->dynamic_stats.allocs_attempted ==
        gdraw->dynamic_stats.allocs_succeeded +
            1) {  // warn the first time we run out of mem
...
static void* alloc_dynamic(U32 size, U32 align) {
    ...
    if (ptr) {
        ...
        gdraw->dynamic_stats.largest_bytes_succeeded =
            RR_MAX(gdraw->dynamic_stats.largest_bytes_succeeded, size);
    } else
...
static void* alloc_and_set_fragment_uniforms(SceGxmContext* gxm, U32 slot,
                                             std::size_t size) {
    void* ptr = alloc_dynamic(size, sizeof(U32));
    if (ptr) sceGxmSetFragmentUniformBuffer(gxm, 0, ptr);
    return ptr;
}

static void* alloc_and_set_vertex_uniforms(SceGxmContext* gxm, U32 slot,
                                           std::size_t size) {
    void* ptr = alloc_dynamic(size, sizeof(U32));
    if (ptr) sceGxmSetVertexUniformBuffer(gxm, 0, ptr);
    return ptr;
}
...
void RADLINK gdraw_psp2_WrappedTextureChange(GDrawTexture* handle,
                                             SceGxmTexture* tex) {
...
static U32 align_down(U32 x, U32 align) { return x & ~(align - 1); }

static U32 align_up(U32 x, U32 align) { return (x + align - 1) & ~(align - 1); }
...
static U32 tex_linear_stride(U32 width) { return align_up(width, 8); }

static rrbool RADLINK gdraw_MakeTextureBegin(
    void* owner, S32 width, S32 height, gdraw_texture_format gformat, U32 flags,
    ...
    if (width > MAX_TEXTURE2D_DIM || height > MAX_TEXTURE2D_DIM) {
        IggyGDrawSendWarning(NULL,
                             "GDraw %d x %d texture not supported by hardware "
                             "(dimension limit %d)",
                             width, height, MAX_TEXTURE2D_DIM);
        return false;
    }
    ...
    if (flags & GDRAW_MAKETEXTURE_FLAGS_mipmap) {
        ...
        do {
            // mip offsets are based on size rounded up to pow2, so we pay for
            // that amount of space.
            size += RR_MAX(pow2_w >> mipmaps, 1) *
                    RR_MAX(pow2_h >> mipmaps, 1) * bytes_pixel;
            mipmaps++;
    ...
    sceGxmTextureInitLinear(t->handle.tex.gxm, t->raw_ptr, format, width,
                            height, mipmaps);
    ...
    if (flags & GDRAW_MAKETEXTURE_FLAGS_mipmap) {
        ...
        ok = gdraw_MipmapBegin(&gdraw->mipmap, width, height, mipmaps,
                               bytes_pixel, p->temp_buffer,
                               p->temp_buffer_bytes);
        if (!ok)
            RR_BREAK();  // this should never trigger unless the temp_buffer is
                         // way too small (Iggy bug)
...
gdraw_MakeTextureMore(GDraw_MakeTexture_ProcessingInfo* p) {
    ...
    if (p->p1) {
        ...
        do {
            ...
            mipstart += RR_MAX(w_pow2 >> level, 1) *
                        RR_MAX(h_pow2 >> level, 1) * c->bpp;
        ...
        return false;  // non-streaming upload; you got the full image first
                       // time!
}

static GDrawTexture* RADLINK
gdraw_MakeTextureEnd(GDraw_MakeTexture_ProcessingInfo* p, GDrawStats* stats) {
    if (p->p1) gdraw_MakeTextureMore(p);  // submit last piece of data
...
static void RADLINK gdraw_UpdateTextureRect(GDrawTexture* t, void* unique_id,
    ...
    // make sure texture is not active. note that we don't implement texture
    // ghosting; this is an actual wait and you can't update a texture you've
    // already used during the current frame. (we only use this path for font
    // cache updates)
    wait_on_fence(s->fence);

    U32 bpl = w * bpp;
    U32 dpitch =
        tex_linear_stride(sceGxmTextureGetWidth(s->handle.tex.gxm)) * bpp;
    U8* src = samples;
...
static void RADLINK gdraw_SetAntialiasTexture(S32 width, U8* rgba) {
    if (sceGxmTextureGetData(&gdraw->aa_tex) != NULL) return;

    assert(width <= MAX_AATEX_WIDTH);
    void* ptr = gdraw_arena_alloc(&gdraw->context_arena, width * 4,
                                  GDRAW_PSP2_TEXTURE_ALIGNMENT);
    if (!ptr) return;

    sceGxmTextureInitLinear(&gdraw->aa_tex, ptr,
                            SCE_GXM_TEXTURE_FORMAT_U8U8U8U8_ABGR, width, 1, 1);
...
static void RADLINK gdraw_FreeVertexBuffer(GDrawVertexBuffer* vb,
    ...
    if (h->owner == unique_id) gdraw_res_kill(h, stats);
}
...
static void set_gxm_texture(U32 unit, SceGxmTexture* tex, U32 wrap,
                            U32 nearest) {
    static const U32 addrbits[ASSERT_COUNT(GDRAW_WRAP__count, 4)] = {
#define CLAMPMODE(x)                           \
    ((SCE_GXM_PDS_DOUTT0_UADDRMODE_##x         \
      << SCE_GXM_PDS_DOUTT0_UADDRMODE_SHIFT) | \
     (SCE_GXM_PDS_DOUTT0_VADDRMODE_##x << SCE_GXM_PDS_DOUTT0_VADDRMODE_SHIFT))
        CLAMPMODE(CLAMP),   // GDRAW_WRAP_clamp
        CLAMPMODE(REPEAT),  // GDRAW_WRAP_repeat
        CLAMPMODE(MIRROR),  // GDRAW_WRAP_mirror
        CLAMPMODE(CLAMP),   // GDRAW_WRAP_clamp_to_border (unused in this impl -
                            // just use regular clamp)
#undef CLAMPMODE
    };
    static const U32 addrmask =
        SCE_GXM_PDS_DOUTT0_UADDRMODE_MASK | SCE_GXM_PDS_DOUTT0_VADDRMODE_MASK;

    static const U32 filterbits[2] = {
        // nearest off
        SCE_GXM_PDS_DOUTT0_MINFILTER_MASK | SCE_GXM_PDS_DOUTT0_MAGFILTER_MASK |
            SCE_GXM_PDS_DOUTT0_MIPFILTER_MASK,
        // nearest on
    ...
    static const U32 filtermask = SCE_GXM_PDS_DOUTT0_MINFILTER_MASK |
                                  SCE_GXM_PDS_DOUTT0_MAGFILTER_MASK |
                                  SCE_GXM_PDS_DOUTT0_MIPFILTER_MASK;
    ...
    texv.controlWords[0] = (tex->controlWords[0] & ~(addrmask | filtermask)) |
                           addrbits[wrap] | filterbits[nearest];
...
static inline void disable_scissor(bool force) {
    if (gdraw->scissor_state) remove_scissor();

    if (force || gdraw->scissor_state != 0) {
        gdraw->scissor_state = 0;
        sceGxmSetRegionClip(gdraw->gxm, SCE_GXM_REGION_CLIP_OUTSIDE,
                            gdraw->cview.x0, gdraw->cview.y0,
                            gdraw->cview.x1 - 1, gdraw->cview.y1 - 1);
    }
}
...
static void set_viewport_raw(S32 x, S32 y, S32 w, S32 h) {
    ...
    // AP - ZOffset/ZScale were set to 0.0/1.0 which is wrong. This fixed the
    // bad poly draw order effect on the models in Skin Select
    sceGxmSetViewport(gdraw->gxm, (F32)x + (F32)w * 0.5f, (F32)w * 0.5f,
                      (F32)y + (F32)h * 0.5f, (F32)h * -0.5f, 0.5f, 0.5f);
}
...
static void clear_renderstate() {
    ...
    sceGxmSetFrontStencilFunc(gxm, SCE_GXM_STENCIL_FUNC_ALWAYS,
                              SCE_GXM_STENCIL_OP_KEEP, SCE_GXM_STENCIL_OP_KEEP,
                              SCE_GXM_STENCIL_OP_KEEP, 0, 0);

    // AP - Added this in to reset the clip region to full screen. This fixed
    // the Splash text not appearing and the bad draw order on the 2x2 crafting
    // cursor.
...
static void do_screen_quad(gswf_recti* s, const F32* tc, F32 z,
                           GDrawStats* stats);
...
static void clear_whole_surf(bool clear_depth, bool clear_stencil,
                             const F32* clear_color, GDrawStats* stats) {
    SceGxmContext* gxm = gdraw->gxm;

    gdraw->z_stencil_key = ~0u;  // force reset on next draw
    SceGxmStencilOp stencil_op =
        clear_stencil ? SCE_GXM_STENCIL_OP_ZERO : SCE_GXM_STENCIL_OP_KEEP;

    sceGxmSetFrontDepthFunc(gxm, SCE_GXM_DEPTH_FUNC_ALWAYS);
    sceGxmSetFrontDepthWriteEnable(gxm, clear_depth
                                            ? SCE_GXM_DEPTH_WRITE_ENABLED
                                            : SCE_GXM_DEPTH_WRITE_DISABLED);
    sceGxmSetFrontStencilFunc(gxm, SCE_GXM_STENCIL_FUNC_ALWAYS, stencil_op,
                              stencil_op, stencil_op, 0xff, 0xff);

    set_fragment_program(gdraw->clear_fp);
    PixelCommonVars* para = (PixelCommonVars*)alloc_and_set_fragment_uniforms(
        gxm, 0, sizeof(PixelCommonVars));
    if (!para) return;
    ...
    if (clear_color)
        ...
        sceGxmSetFrontFragmentProgramEnable(gxm,
...
static void RADLINK gdraw_RenderingEnd(void) { clear_renderstate(); }
...
void gdraw_psp2_Begin(SceGxmContext* context, const SceGxmColorSurface* color,
                      const SceGxmDepthStencilSurface* depth,
                      gdraw_psp2_dynamic_buffer* dynamic_buf) {
    ...
    gdraw_arena_init(&gdraw->dynamic, dynamic_buf->start,
                     dynamic_buf->size_in_bytes);
    ...
    if (!depth || !sceGxmDepthStencilSurfaceIsEnabled(depth) ||
        sceGxmDepthStencilSurfaceGetFormat(depth) !=
        ...
        IggyGDrawSendWarning(
            NULL,
            "Iggy rendering will not work correctly unless a depth/stencil "
            "buffer in DF32M_S8 format is provided.");
    }
...
SceGxmNotification gdraw_psp2_End() {
    ...
    assert(gdraw->gxm !=
           NULL);  // please keep Begin / End pairs properly matched
    ...
    handle_cache_tick(gdraw->texturecache, gdraw->scene_end_fence,
                      &gdraw_stats);
...
static void draw_scissor_region(gswf_recti* r, SceGxmStencilFunc func) {
    ...
    F32* vpos =
        (F32*)alloc_and_set_vertex_uniforms(gd->gxm, 0, 8 * sizeof(F32));
    if (!vpos) return;
    ...
    sceGxmSetRegionClip(gd->gxm, SCE_GXM_REGION_CLIP_OUTSIDE, tile_rect.x0,
                        tile_rect.y0, tile_rect.x1 - 1, tile_rect.y1 - 1);
    ...
    // draw
    gdraw->z_stencil_key =
        ~0u;  // invalidate z state -> force reset on next draw
    sceGxmSetFrontStencilFunc(gd->gxm, func, SCE_GXM_STENCIL_OP_KEEP,
                              SCE_GXM_STENCIL_OP_KEEP, SCE_GXM_STENCIL_OP_KEEP,
                              0, 0);
...
static inline F32 depth_from_id(S32 id) {
    return (1.0f - 1.0f / MAX_DEPTH_VALUE) -
           id * (1.0f / MAX_DEPTH_VALUE);  // = 1 - (id + 1) / MAX_DEPTH_VALUE
}

static bool set_renderstate_full(const GDrawRenderState* RADRESTRICT r,
                                 GDrawStats* stats) {
    static const int canonical_blend[ASSERT_COUNT(GDRAW_BLEND__count, 6)] = {
        ...
        GDRAW_CBLEND_none,   // GDRAW_BLEND_multiply - UNSUPPORTED on PSP2 (only
                             // occurs in layer blends which we don't allow)
    ...
    // we need to handle scissor first, since it might require us to draw
    // things.
    if (r->scissor) {
        S32 xs = gd->tx0v;
        S32 ys = gd->ty0v;
    ...
    void* dyn_uniform = alloc_dynamic(
        sizeof(VertexVars) + sizeof(PixelCommonVars), sizeof(U32));
    if (!dyn_uniform) return false;

    VertexVars* RADRESTRICT vvars = (VertexVars*)dyn_uniform;
    PixelCommonVars* RADRESTRICT pvars =
        (PixelCommonVars*)(vvars +
                           1);  // right after VertexVars in dyn_uniform alloc
    ...
    float32x4_t s0_texgen =
        vld1q_f32(r->s0_texgen);  // always copy, even when unused.
    ...
    if (r->cxf_add)
        col_add = vmulq_n_f32(vcvtq_f32_s32(vmovl_s16(vld1_s16(r->cxf_add))),
                              1.0f / 255.0f);
    ...
    if (r->stencil_set) cblend_mode = GDRAW_CBLEND_nowrite;

    int additive_mode = 0;
    if (r->cxf_add) additive_mode = r->cxf_add[3] ? 2 : 1;

    set_fragment_program(gd->main_fp[tex0mode][additive_mode][cblend_mode]);

    // set textures
    if (tex0mode != GDRAW_TEXTURE_none) {
        if (!r->tex[0])  // this can happen if some allocs fail. just abort in
                         // that case.
            return false;

        if (gd->active_tex[0] != r->tex[0]) {
            gd->active_tex[0] = r->tex[0];
            set_gxm_texture(0, ((GDrawHandle*)r->tex[0])->handle.tex.gxm,
                            r->wrap0, r->nearest0);
        }
    }

    // z/stencil mode changed?
    U32 z_stencil_key = r->set_id | (r->test_id << 1) |
                        (r->stencil_test << 16) | (r->stencil_set << 24);

    if (z_stencil_key != gd->z_stencil_key) {
        gd->z_stencil_key = z_stencil_key;
        sceGxmSetFrontDepthFunc(gxm, r->test_id ? SCE_GXM_DEPTH_FUNC_LESS
                                                : SCE_GXM_DEPTH_FUNC_ALWAYS);
        sceGxmSetFrontDepthWriteEnable(gxm, r->set_id
                                                ? SCE_GXM_DEPTH_WRITE_ENABLED
                                                : SCE_GXM_DEPTH_WRITE_DISABLED);
        sceGxmSetFrontStencilFunc(
            gxm,
            r->stencil_test ? SCE_GXM_STENCIL_FUNC_EQUAL
                            : SCE_GXM_STENCIL_FUNC_ALWAYS,
            SCE_GXM_STENCIL_OP_KEEP, SCE_GXM_STENCIL_OP_KEEP,
            SCE_GXM_STENCIL_OP_REPLACE, r->stencil_test, r->stencil_set);
    }

    return true;
}

static RADINLINE bool set_renderstate(const GDrawRenderState* RADRESTRICT r,
                                      GDrawStats* stats) {
...
static const U32
    vfmt_stride_bytes[ASSERT_COUNT(GDRAW_vformat__basic_count, 3)] = {
        8,   // GDRAW_vformat_v2
...
static RADINLINE void fence_resources(void* r1, void* r2 = NULL,
                                      void* r3 = NULL) {
...
static RADINLINE void draw_ind_tris_u16(SceGxmContext* gxm, S32 vfmt,
                                        const void* verts, const void* inds,
                                        S32 num_inds) {
    sceGxmSetVertexStream(gxm, 0, verts);
    sceGxmDraw(gxm, SCE_GXM_PRIMITIVE_TRIANGLES, SCE_GXM_INDEX_FORMAT_U16, inds,
               num_inds);
}
...
static void RADLINK gdraw_DrawIndexedTriangles(GDrawRenderState* r,
    ...
    if (!set_renderstate(r, stats)) return;
    ...
    if (vb)
        draw_ind_tris_u16(
            gxm, vfmt, (U8*)vb->handle.vbuf.verts + (UINTa)p->vertices,
            (U8*)vb->handle.vbuf.inds + (UINTa)p->indices, p->num_indices);
    else if (p->indices) {
        ...
        while (pos < p->num_vertices) {
            ...
            draw_ind_tris_u16(gxm, vfmt, buf + pos * stride, gdraw->quad_ib,
                              (vert_count >> 2) * 6);
            pos += vert_count;
        }
    }
...
static void do_screen_quad(gswf_recti* s, const F32* tc, F32 z,
    ...
    VertexVars* vvars = (VertexVars*)alloc_and_set_vertex_uniforms(
        gdraw->gxm, 0, sizeof(VertexVars));
    if (!vvars) return;
    ...
    gswf_vertex_xyst* RADRESTRICT v =
        (gswf_vertex_xyst*)alloc_dynamic(4 * sizeof(gswf_vertex_xyst), 4);
    if (!v) return;
    ...
    sceGxmDraw(gdraw->gxm, SCE_GXM_PRIMITIVE_TRIANGLES,
               SCE_GXM_INDEX_FORMAT_U16, gdraw->quad_ib, 6);
}
...
static void RADLINK gdraw_FilterQuad(GDrawRenderState* r, S32 x0, S32 y0,
    ...
    if (r->blend_mode == GDRAW_BLEND_filter ||
        r->blend_mode == GDRAW_BLEND_special) {
        IggyGDrawSendWarning(
            NULL, "GDraw no filter or special blend support on PSP2");
...
static bool gxm_check(SceGxmErrorCode err) {
    if (err != SCE_OK) IggyGDrawSendWarning(NULL, "GXM error");

    return err == SCE_OK;
}

static bool register_shader(SceGxmShaderPatcher* patcher, ShaderCode* shader) {
    if (!shader->blob) return SCE_OK;

    bool ok = gxm_check(sceGxmShaderPatcherRegisterProgram(
        patcher, (const SceGxmProgram*)shader->blob, &shader->id));
    shader->registered = ok;
    return ok;
}

static void unregister_shader(SceGxmShaderPatcher* patcher,
                              ShaderCode* shader) {
...
static bool register_and_create_vertex_prog(SceGxmVertexProgram** out_prog,
                                            SceGxmShaderPatcher* patcher,
                                            ShaderCode* shader,
                                            U32 attr_bytes) {
    ...
    if (!register_shader(patcher, shader)) return NULL;
    ...
    *out_prog = NULL;
    ...
    if (attr_bytes) {
        ...
        return gxm_check(sceGxmShaderPatcherCreateVertexProgram(
            patcher, shader->id, &attr, 1, &stream, 1, out_prog));
    } else
        return gxm_check(sceGxmShaderPatcherCreateVertexProgram(
            patcher, shader->id, NULL, 0, NULL, 0, out_prog));
}

static void destroy_vertex_prog(SceGxmShaderPatcher* patcher,
                                SceGxmVertexProgram* prog) {
    if (prog) sceGxmShaderPatcherReleaseVertexProgram(patcher, prog);
}

static bool create_fragment_prog(SceGxmFragmentProgram** out_prog,
                                 SceGxmShaderPatcher* patcher,
                                 ShaderCode* shader,
                                 const SceGxmBlendInfo* blend,
                                 SceGxmOutputRegisterFormat out_fmt) {
    *out_prog = NULL;
    return gxm_check(sceGxmShaderPatcherCreateFragmentProgram(
        patcher, shader->id, out_fmt, SCE_GXM_MULTISAMPLE_NONE, blend, NULL,
        out_prog));
}

static void destroy_fragment_prog(SceGxmShaderPatcher* patcher,
                                  SceGxmFragmentProgram* prog) {
    if (prog) sceGxmShaderPatcherReleaseFragmentProgram(patcher, prog);
}
...
static bool create_all_programs(SceGxmOutputRegisterFormat reg_format) {
    ...
    // blend states
    static const SceGxmBlendInfo blends[ASSERT_COUNT(GDRAW_CBLEND__count,
                                                     4)] = {
        // GDRAW_CBLEND_none
        ...
        {SCE_GXM_COLOR_MASK_NONE, SCE_GXM_BLEND_FUNC_ADD,
         SCE_GXM_BLEND_FUNC_ADD, SCE_GXM_BLEND_FACTOR_ONE,
    ...
    for (int i = 0; i < GDRAW_vformat__basic_count; i++) {
        if (!register_and_create_vertex_prog(&gdraw->vp[i], patcher,
                                             vshader_vspsp2_arr + i,
                                             vfmt_stride_bytes[i]))
            return false;
    }

    if (!register_and_create_vertex_prog(&gdraw->mask_vp, patcher,
                                         vshader_vspsp2_mask_arr, 0))
        return false;
    ...
    for (int i = 0; i < GDRAW_TEXTURE__count; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            if (!register_shader(patcher, sh)) return false;

            for (int k = 0; k < GDRAW_CBLEND__count; k++)
                if (!create_fragment_prog(&gdraw->main_fp[i][j][k], patcher, sh,
                                          &blends[k], reg_format))
                    return false;
        }
    }
    ...
    if (!register_shader(patcher, pshader_manual_clear_arr) ||
        !create_fragment_prog(&gdraw->clear_fp, patcher,
                              pshader_manual_clear_arr, NULL, reg_format))
        return false;

    gdraw->mask_update_fp = NULL;
    return gxm_check(sceGxmShaderPatcherCreateMaskUpdateFragmentProgram(
        patcher, &gdraw->mask_update_fp));
}
...
static GDrawHandleCache* make_handle_cache(gdraw_psp2_resourcetype type,
                                           U32 align, rrbool use_twopool) {
    ...
    if (use_twopool) one_pool_bytes = align_down(one_pool_bytes / 2, align);

    if (one_pool_bytes < (S32)align) return NULL;
    ...
    if (cache) {
        ...
        // set up allocators
        cache->alloc = gfxalloc_create(gdraw_limits[type].ptr, one_pool_bytes,
                                       align, num_handles);
        if (!cache->alloc) {
            IggyGDrawFree(cache);
            return NULL;
        ...
        if (use_twopool) {
            cache->alloc_other =
                gfxalloc_create((U8*)gdraw_limits[type].ptr + one_pool_bytes,
                                one_pool_bytes, align, num_handles);
            ...
            U8* mem_near_end = (U8*)gdraw_limits[type].ptr +
                               2 * one_pool_bytes - GPU_MEMCPY_ALIGN;
...
void gdraw_psp2_InitDynamicBuffer(gdraw_psp2_dynamic_buffer* buf, void* ptr,
                                  U32 num_bytes) {
...
int gdraw_psp2_SetResourceMemory(gdraw_psp2_resourcetype type, S32 num_handles,
                                 void* ptr, S32 num_bytes) {
    GDrawStats stats = {0};

    assert(type >= GDRAW_PSP2_RESOURCE_texture &&
           type < GDRAW_PSP2_RESOURCE__count);
    ...
    switch (type) {
        ...
        case GDRAW_PSP2_RESOURCE_vertexbuffer:
            make_pool_aligned(&ptr, &num_bytes,
                              GDRAW_PSP2_VERTEXBUFFER_ALIGNMENT);
            break;
    ...
    // make sure GPU is done first (assuming we're in a state where we can
    // dispatch commands)
    assert(!is_fence_pending(
        gdraw->scene_end_fence));  // you may not call this while GPU is still
                                   // busy with Iggy command buffers!

    if (gdraw->texturecache) gdraw_res_reap(gdraw->texturecache, &stats);
    if (gdraw->vbufcache) gdraw_res_reap(gdraw->vbufcache, &stats);
    // in theory we can now check that the given cache is really empty at this
    // point
    ...
    switch (type) {
        case GDRAW_PSP2_RESOURCE_texture:
            ...
            gdraw->texturecache =
                make_handle_cache(GDRAW_PSP2_RESOURCE_texture,
                                  GDRAW_PSP2_TEXTURE_ALIGNMENT, true);
            return gdraw->texturecache != NULL;

        case GDRAW_PSP2_RESOURCE_vertexbuffer:
            free_handle_cache(gdraw->vbufcache);
            gdraw->vbufcache =
                make_handle_cache(GDRAW_PSP2_RESOURCE_vertexbuffer,
                                  GDRAW_PSP2_VERTEXBUFFER_ALIGNMENT, true);
...
GDrawFunctions* gdraw_psp2_CreateContext(
    SceGxmShaderPatcher* shader_patcher, void* context_mem,
    volatile U32* notification, SceGxmOutputRegisterFormat reg_format) {
    // mask index buffer:
    //
    // 0-----------------------3  y0
    // | \5---------------6 /  |  y1
    ...
    gdraw_arena_init(&gdraw->context_arena, context_mem,
                     GDRAW_PSP2_CONTEXT_MEM_SIZE);
    ...
    *notification = 0;
    ...
    for (int i = 0; i < GDRAW_PSP2_RESOURCE__count; i++)
        gdraw_psp2_SetResourceMemory(
            (gdraw_psp2_resourcetype)i, gdraw_limits[i].num_handles,
            gdraw_limits[i].ptr, gdraw_limits[i].num_bytes);

    // shaders and state
    gdraw->quad_ib = (U16*)gdraw_arena_alloc(
        &gdraw->context_arena, QUAD_IB_COUNT * 6 * sizeof(U16), sizeof(U32));
    gdraw->mask_ib = (U16*)gdraw_arena_alloc(&gdraw->context_arena,
                                             sizeof(mask_ib_data), sizeof(U32));

    if (!gdraw->quad_ib || !gdraw->mask_ib ||
        !create_all_programs(reg_format)) {
    ...
    gdraw->mask_draw_gpu = gdraw_arena_alloc(
        &gdraw->context_arena, sceGxmGetPrecomputedDrawSize(gdraw->mask_vp),
        SCE_GXM_PRECOMPUTED_ALIGNMENT);
    ...
    sceGxmPrecomputedDrawInit(&gdraw->mask_draw, gdraw->mask_vp,
                              gdraw->mask_draw_gpu);
    sceGxmPrecomputedDrawSetParams(
        &gdraw->mask_draw, SCE_GXM_PRIMITIVE_TRIANGLE_STRIP,
        SCE_GXM_INDEX_FORMAT_U16, gdraw->mask_ib, 5 * 2);
    ...
    gdraw_funcs.MakeTextureFromResource =
        (gdraw_make_texture_from_resource*)gdraw_psp2_MakeTextureFromResource;
...
void RADLINK gdraw_psp2_BeginCustomDraw(IggyCustomDrawCallbackRegion* region,
                                        float matrix[16]) {
    clear_renderstate();
    gdraw_GetObjectSpaceMatrix(matrix, region->o2w, gdraw->projection, 0.0f, 0);
}

void RADLINK gdraw_psp2_CalculateCustomDraw_4J(
...
GDrawTexture* RADLINK gdraw_psp2_MakeTextureFromResource(
    U8* file_in_memory, S32 len, IggyFileTexturePSP2* tex) {
    SceGxmErrorCode (*init_func)(SceGxmTexture* texture, const void* data,
                                 SceGxmTextureFormat texFormat, uint32_t width,
                                 uint32_t height, uint32_t mipCount) = NULL;

    switch (tex->texture.type) {
        case SCE_GXM_TEXTURE_SWIZZLED:
            init_func = sceGxmTextureInitSwizzled;
            break;
        case SCE_GXM_TEXTURE_LINEAR:
            init_func = sceGxmTextureInitLinear;
            break;
        case SCE_GXM_TEXTURE_TILED:
            init_func = sceGxmTextureInitTiled;
            break;
        case SCE_GXM_TEXTURE_SWIZZLED_ARBITRARY:
            init_func = sceGxmTextureInitSwizzledArbitrary;
            break;
    }

    if (!init_func) {
        IggyGDrawSendWarning(
            NULL, "Unsupported texture type in MakeTextureFromResource");
        return NULL;
    }

    SceGxmTexture gxm;
    SceGxmErrorCode err =
        init_func(&gxm, file_in_memory + tex->file_offset,
                  (SceGxmTextureFormat)tex->texture.format, tex->texture.width,
                  tex->texture.height, tex->texture.mip_count);
    if (err != SCE_OK) {
        IggyGDrawSendWarning(
            NULL, "Texture init failed in MakeTextureFromResource (bad data?)");
        return NULL;
    }
