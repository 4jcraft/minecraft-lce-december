...
LeaderboardManager* LeaderboardManager::m_instance =
    new PSVitaLeaderboardManager();  // Singleton instance of the
                                     // LeaderboardManager
...
HRESULT PSVitaLeaderboardManager::createTitleContext(const SceNpId& npId) {
    return sceNpScoreCreateTitleCtx(&s_npCommunicationId,
                                    &s_npCommunicationPassphrase, &npId);
}
...
HRESULT PSVitaLeaderboardManager::abortTransactionContext(
    int transactionContext) {
    return sceNpScoreAbortRequest(transactionContext);
}

HRESULT PSVitaLeaderboardManager::destroyTransactionContext(
    int transactionContext) {
    return sceNpScoreDeleteRequest(transactionContext);
}

HRESULT PSVitaLeaderboardManager::getFriendsList(
    sce::Toolkit::NP::Utilities::Future<sce::Toolkit::NP::FriendsList>&
        friendsList) {
    return sce::Toolkit::NP::Friends::Interface::getFriendslist(&friendsList,
                                                                false);
}
