...
enum EProductSKU { e_sku_SCEE, e_sku_SCEA, e_sku_SCEJ };
...
// MGH - moved these to the storage lib, as we need this data when parsing the
// DLC folders enum e_SONYDLCType
// {
...
public:
    ...
    virtual void GetSaveThumbnail(PBYTE* ppbThumbnailData,
                                  DWORD* pdwThumbnailSize, PBYTE* ppbDataImage,
                                  DWORD* pdwSizeImage);
    ...
    std::vector<SonyCommerce::ProductInfo>* GetProductList(
        int iIndex);  // default to fail if the additional details are not
                      // retrieved
    ...
    std::unordered_map<std::wstring, SONYDLC*>* GetSonyDLCMap() {
        return &m_SONYDLCMap;
    }
    ...
    static void DownloadAlreadyPurchasedSessionStartedCallback(LPVOID lpParam,
                                                               int err);
    ...
    void SaveDataTick();
    static void Callback_SaveGameIncomplete(
        void* pParam, C4JStorage::ESaveIncompleteType saveIncompleteType,
        int blocksRequired);
    static int NoSaveSpaceReturned(void* pParam, int iPad,
                                   C4JStorage::EMessageResult result);
    static int cbConfirmDeleteMessageBox(void* pParam, int iPad,
