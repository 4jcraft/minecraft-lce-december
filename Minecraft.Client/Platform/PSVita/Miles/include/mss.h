...
DXDEC void AILCALL AIL_set_listener_relative_receiver_array(
    HDIGDRIVER dig, MSS_RECEIVER_LIST* std::array, S32 n_receivers);
...
DXDEC EXPAPI void   AILCALL AIL_schedule_start_sample(HSAMPLE S, U64 mix_time_t/*
...
DXDEC EXPAPI void   AILCALL AIL_set_sample_id(HSAMPLE S,/*
...
DXDEC EXPAPI S32    AILCALL AIL_set_sample_loop_samples(HSAMPLE S, S32 loop_start_samples, S32 loop_end_/*
...
DXDEC S32 AILCALL
    AIL_quick_startup(S32 use_digital, S32 use_MIDI, U32 output_rate,
                      S32 output_bits, S32 output_channels);
    ...
    DXDEC HAUDIO AILCALL AIL_quick_load_named_mem(
        void const* mem, char const* filename, U32 size);
    ...
    DXDEC HAUDIO AILCALL AIL_quick_load_and_play(
        char const* filename, U32 loop_count, S32 wait_request);
    ...
    DXDEC void AILCALL AIL_quick_set_ms_position(HAUDIO audio,
                                                 S32 milliseconds);
...
typed// str// File-level data accessible to app//// This is valid after AIL_inspect_MP3() is called (even if the file contains no valid frames)//
   ...
   U8 *end_MP3// Pointer to last valid uint8_t in MP3 data area (before ID3v1 tag, if any)//// Information about current frame being inspected, valid if AIL_enumerate_MP3_frames() returns// TRUE//  
...
typed// str// File-level data accessible to app//// This is valid after AIL_inspect_Ogg() is called (even if the file contains no valid pages)//
   ...
   U8 *end_Ogg// Pointer to last valid uint8_t in data area// Information lifted from the header after AIL_inspect_Ogg() is called.
   S32 channel_count;
...
typedef struct {
    ...
    const char*
        curre  // output data - uint8_t offset for current frame.
            S32 byte_offset;
} BINKA_INFO;
...
enum
{
    ENVIRONMENT_GENERIC,  // factory default
        ...
        ENVIRONMENT_HALLWAY, ENVIRONMENT_STONECORRIDOR, ENVIRONMENT_ALLEY,
        ENVIRONMENT_FOREST, ENVIRONMENT_CITY, ENVIRONMENT_MOUNTAINS,
        ENVIRONMENT_QUARRY, ENVIRONMENT_PLAIN, ENVIRONMENT_PARKINGLOT,
...
DXDEC EXPAPI void       AILCALL AIL_set_sample_3D_volume_falloff(HSAMPLE S, MSSGRAPHPOINT* graph, S3/*
...
DXDEC EXPAPI HMSOUNDBANK AILCALL AIL_open_soundbank(char const *filename, char/*
...
EXPTYPE typedef str// If this changes at all, compiled banks must be versioned...
    ...
    $:DataLen The uint8_t count the asset requires if fully loaded.
...
AIL_add_cache_sounds_event_step(
    HMSSEVENTCONSTRUCT i_Event, const char* bankN/*!
    function
    {
        ExcludeOn = 1
...
DXDEC EXPAPI S32 AILCALL AIL_add_ramp_event_step(
    ...
    F32 i_Time, char const* i_Target, U8 i_Type, U8 i_ApplyTo/*
...
DXDEC EXPAPI S32 AILCALL AIL_add_setblend_event_step(HMSSEVENTCONSTRUCT i_Event,
    ...
    F32 const* i_OutMin, F32 const* i_OutMax, F32 const/*
...
DXDEC EXPAPI S32 AILCALL AIL_get_event_contents(HMSOUNDBANK bank, char con/*
...
enum EVENT_STEPTYPE
{
            EVENT_STEPTYPE_STARTSOUND = 1, EVENT_STEPTYPE_CONTROLSOUNDS,
            EVENT_STEPTYPE_APPLYENV, EVENT_STEPTYPE_COMMENT,
            EVENT_STEPTYPE_CACHESOUNDS, EVENT_STEPTYPE_PURGESOUNDS,
            EVENT_STEPTYPE_SETLIMITS, EVENT_STEPTYPE_PERSIST,
            EVENT_STEPTYPE_VERSION, EVENT_STEPTYPE_RAMP,
            EVENT_STEPTYPE_SETBLEND, EVENT_STEPTYPE_CLEARSTATE,
            ...
            typedef str                        /*!
                                        Represents a single step that needs to be executed for an
                                        event.
                       
                                        All of the members in the structures share the same definition
                                        as                        their counterpart params in
...
EXPTYPE typedef struct _MILESBANKFUNCTIONS
{
  ...
  MilesBankGetLoadedName * GetLoad/*
...
DXDEC EXPAPI void AILCALL AIL_set_event_sample_functions(HSAMPLE (*CreateSampleCallback)(char const* SoundName, char const* SoundFileName, HDIGDRIVER dig, void* UserBuffer, S32 UserBufferLen), void (*Rele/*
...
DXDEC EXPAPI void AILCALL AIL_event_system_state(HEVENTSYSTEM sys/*
...
typedef struct _FLTPROVIDER
{
                                    ...
                                    VFLT_ASSIGN_SAMPLE_VOICE
                                        assign_sample_voice;
                                    VFLT_RELEASE_SAMPLE_VOICE
                                        release_sample_voice;
