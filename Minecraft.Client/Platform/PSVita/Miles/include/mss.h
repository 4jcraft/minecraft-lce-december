...
#if defined(__RADWIN__) && !defined(__RADXENON__) && !defined(__RADXBOX__) && \
...
//
    ...
    defined(__RADWII__) || defined(__RADWIIU__) || defined(__RAD3DS__) ||     \
    defined(__RADIPHONE__) || defined(__RADXENON__) || defined(__RADPS4__) || \
    defined(__RADPS3__) || defined(__RADSPU__) || defined(__RADDURANGO__) ||  \
    defined(__RADWINRTAPI__)
#define MSS_STATIC_RIB
// WinRT is weird in that we statically pull in the RIBs, but we dynamically
// link Midi
...
#define MSSMAKEFOURCC(ch0, ch1, ch2, ch3)                                      \
    (((U32)(U8)(ch0) << 24) | ((U32)(U8)(ch1) << 16) | ((U32)(U8)(ch2) << 8) | \
     ((U32)(U8)(ch3)))
#endif
...
#elif defined(IS_PS3) || defined(IS_WII) || defined(IS_WIIU) || \
    defined(__RADWINRTAPI__) || defined(__RADSEKRIT2__)
#define MAX_SPEAKERS \
    8  // Up to 8 hardware output channels on PS3, PS2, Wii, WiiU
#elif defined(IS_PSP) || defined(IS_IPHONE) || defined(IS_3DS) || \
    defined(IS_PSP2) || defined(__RADANDROID__)
#define MAX_SPEAKERS 2  // Up to 2 hardware output channels on PSP
#else
#define MAX_SPEAKERS \
    6  // Up to 6 hardware output channels supported on other platforms
#endif
...
#define AIL_MAX_FILE_HEADER_SIZE \
    8192  // AIL_set_named_sample_file() requires at least 8K
...
#define DIG_F_MULTICHANNEL_8 \
    (DIG_F_MULTICHANNEL_MASK)  // (not actually supported)
...
#define DIG_F_ADPCM_STEREO_16 \
    (DIG_F_ADPCM_MASK | DIG_F_16BITS_MASK | DIG_F_STEREO_MASK)
#define DIG_F_ADPCM_MULTICHANNEL_16 \
    (DIG_F_ADPCM_MASK | DIG_F_16BITS_MASK | DIG_F_STEREO_MASK)
#define DIG_F_XBOX_ADPCM_MONO_16 (DIG_F_XBOX_ADPCM_MASK | DIG_F_16BITS_MASK)
#define DIG_F_XBOX_ADPCM_STEREO_16 \
    (DIG_F_XBOX_ADPCM_MASK | DIG_F_16BITS_MASK | DIG_F_STEREO_MASK)
#define DIG_F_XBOX_ADPCM_MULTICHANNEL_16 \
    (DIG_F_XBOX_ADPCM_MASK | DIG_F_16BITS_MASK | DIG_F_MULTICHANNEL_MASK)

#define DIG_F_NOT_8_BITS                                            \
    (DIG_F_16BITS_MASK | DIG_F_ADPCM_MASK | DIG_F_XBOX_ADPCM_MASK | \
     DIG_F_MULTICHANNEL_MASK)
...
typedef enum {
    ...
    MSS_SPEAKER_FRONT_CENTER =
        2,  // Also see microsoft.com/whdc/device/audio/multichaud.mspx
...
#define SMP_DONE \
    0x0002  // Sample has finished playing, or has
            // never been started
...
#define SMP_PLAYINGBUTRELEASED \
    0x0010  // Sample is playing, but digital handle
...
#define SEQ_DONE \
    0x0002  // Sequence has finished playing, or has
            // never been started
...
#define SEQ_PLAYINGBUTRELEASED \
    0x0010  // Sequence is playing, but MIDI handle
            // has been temporarily released
...
The Miles enumeration functions all work similarly - you set a local variable of
type HMSSENUM to MSS_FIRST and then call the enumeration function until it
returns 0.
...
#define DEFAULT_DLRS \
    32  // Ramp level changes over first 32 samples in each buffer to reduce
        // zipper noise
...
// This will fail if structure packing isn't correct for the compiler we are
// running.
...
typedef void*(AILCALL MSS_ALLOC_TYPE)(UINTa size, UINTa user,
                                      char const* filename, U32 line);
typedef void(AILCALL MSS_FREE_TYPE)(void* ptr, UINTa user, char const* filename,
                                    U32 line);

// helper functions that just turn around and call AIL_mem_alloc_lock
DXDEC void* AILCALL MSS_alloc_info(UINTa size, UINTa user, char const* filename,
                                   U32 line);
DXDEC void AILCALL MSS_free_info(void* ptr, UINTa user, char const* filename,
                                 U32 line);
...
#define MSS_DEC_CB_STACK_ALIGN(name) \
    DXDEC void AILCALL MSS_CALLBACK_ALIGNED_NAME(name)(void);
#else
...
#define RIB_NOT_ALL_AVAILABLE \
    1                     // Some requested functions/attribs not available
...
typedef enum {
    ...
    RIB_PERCENT,   // Used for 32-bit single-precision FP values to be reported
                   // as percentages
    RIB_BOOL,  // Used for Boolean-constrained integer values to be reported as
               // TRUE or FALSE
...
typedef S32(AILCALL* PROVIDER_PROPERTY)(HPROPERTY index, void* before_value,
...
#define FN(entry_name) \
    {RIB_FUNCTION, #entry_name, (UINTa) & (entry_name), RIB_NONE}
#define REG_FN(entry_name) \
    {RIB_FUNCTION, #entry_name, (UINTa) & (entry_name), RIB_NONE}

#define PR(entry_name, ID) \
    {RIB_PROPERTY, (entry_name), (UINTa) & (ID), RIB_NONE}
#define REG_PR(entry_name, ID, subtype) \
    {RIB_PROPERTY, (entry_name), (UINTa)(ID), subtype}

#define RIB_register(x, y, z) \
    RIB_register_interface((HPROVIDER)(x), y, ARY_CNT(z), z)
#define RIB_unregister(x, y, z) \
    RIB_unregister_interface((HPROVIDER)(ssx), y, ARY_CNT(z), z)
...
// passed to RIB DLLs in Miles 9 and up (so RIBS don't have to link to
// MSS32.dll)
typedef HPROVIDER AILCALL RIB_ALLOC_PROVIDER_HANDLE_TYPE(long module);

typedef RIBRESULT AILCALL
RIB_REGISTER_INTERFACE_TYPE(HPROVIDER provider, C8 const* interface_name,
                            S32 entry_count, RIB_INTERFACE_ENTRY const* rlist);

typedef RIBRESULT AILCALL RIB_UNREGISTER_INTERFACE_TYPE(
    HPROVIDER provider, C8 const* interface_name, S32 entry_count,
...
DXDEC S32 AILCALL RIB_MAIN_NAME(SRS)(HPROVIDER provider_handle, U32 up_down,
                                     RIB_ALLOC_PROVIDER_HANDLE_TYPE* rib_alloc,
                                     RIB_REGISTER_INTERFACE_TYPE* rib_reg,
                                     RIB_UNREGISTER_INTERFACE_TYPE* rib_unreg);
DXDEC S32 AILCALL RIB_MAIN_NAME(DTS)(HPROVIDER provider_handle, U32 up_down,
                                     RIB_ALLOC_PROVIDER_HANDLE_TYPE* rib_alloc,
                                     RIB_REGISTER_INTERFACE_TYPE* rib_reg,
                                     RIB_UNREGISTER_INTERFACE_TYPE* rib_unreg);
DXDEC S32 AILCALL
    RIB_MAIN_NAME(DolbySurround)(HPROVIDER provider_handle, U32 up_down,
                                 RIB_ALLOC_PROVIDER_HANDLE_TYPE* rib_alloc,
                                 RIB_REGISTER_INTERFACE_TYPE* rib_reg,
                                 RIB_UNREGISTER_INTERFACE_TYPE* rib_unreg);
DXDEC S32 AILCALL
    RIB_MAIN_NAME(MP3Dec)(HPROVIDER provider_handle, U32 up_down,
                          RIB_ALLOC_PROVIDER_HANDLE_TYPE* rib_alloc,
                          RIB_REGISTER_INTERFACE_TYPE* rib_reg,
                          RIB_UNREGISTER_INTERFACE_TYPE* rib_unreg);
DXDEC S32 AILCALL
    RIB_MAIN_NAME(OggDec)(HPROVIDER provider_handle, U32 up_down,
                          RIB_ALLOC_PROVIDER_HANDLE_TYPE* rib_alloc,
                          RIB_REGISTER_INTERFACE_TYPE* rib_reg,
                          RIB_UNREGISTER_INTERFACE_TYPE* rib_unreg);
DXDEC S32 AILCALL
    RIB_MAIN_NAME(BinkADec)(HPROVIDER provider_handle, U32 up_down,
                            RIB_ALLOC_PROVIDER_HANDLE_TYPE* rib_alloc,
                            RIB_REGISTER_INTERFACE_TYPE* rib_reg,
                            RIB_UNREGISTER_INTERFACE_TYPE* rib_unreg);
DXDEC S32 AILCALL
    RIB_MAIN_NAME(SpxDec)(HPROVIDER provider_handle, U32 up_down,
                          RIB_ALLOC_PROVIDER_HANDLE_TYPE* rib_alloc,
                          RIB_REGISTER_INTERFACE_TYPE* rib_reg,
                          RIB_UNREGISTER_INTERFACE_TYPE* rib_unreg);
DXDEC S32 AILCALL
    RIB_MAIN_NAME(SpxEnc)(HPROVIDER provider_handle, U32 up_down,
                          RIB_ALLOC_PROVIDER_HANDLE_TYPE* rib_alloc,
                          RIB_REGISTER_INTERFACE_TYPE* rib_reg,
                          RIB_UNREGISTER_INTERFACE_TYPE* rib_unreg);
DXDEC S32 AILCALL
    RIB_MAIN_NAME(Voice)(HPROVIDER provider_handle, U32 up_down,
                         RIB_ALLOC_PROVIDER_HANDLE_TYPE* rib_alloc,
                         RIB_REGISTER_INTERFACE_TYPE* rib_reg,
                         RIB_UNREGISTER_INTERFACE_TYPE* rib_unreg);
DXDEC S32 AILCALL
    RIB_MAIN_NAME(SpxVoice)(HPROVIDER provider_handle, U32 up_down,
                            RIB_ALLOC_PROVIDER_HANDLE_TYPE* rib_alloc,
                            RIB_REGISTER_INTERFACE_TYPE* rib_reg,
                            RIB_UNREGISTER_INTERFACE_TYPE* rib_unreg);
...
#ifdef IS_XENON
DXDEC S32 AILCALL
    RIB_MAIN_NAME(XMADec)(HPROVIDER provider_handle, U32 up_down,
                          RIB_ALLOC_PROVIDER_HANDLE_TYPE* rib_alloc,
                          RIB_REGISTER_INTERFACE_TYPE* rib_reg,
                          RIB_UNREGISTER_INTERFACE_TYPE* rib_unreg);
#endif

#define Register_RIB(name) \
    RIB_load_static_provider_library(RIB_MAIN_NAME(name), #name)
...
RIB_load_static_provider_library(RIB_MAIN_FUNC main, const char* description);
...
#ifndef MSS_ASI_VERSION  // MSSASI.H contents included if MSSASI.H not already
                         // included
...
typedef S32(AILCALLBACK* AILASIFETCHCB)(
...
typedef HASISTREAM(AILCALL* ASI_STREAM_OPEN)(
    MSS_ALLOC_TYPE* palloc, MSS_FREE_TYPE* pfree,
...
typedef S32(AILCALL* ASI_STREAM_PROCESS)(
    HASISTREAM stream,  // Handle of stream
...
typedef S32(AILCALL* ASI_STREAM_PROPERTY)(HASISTREAM stream, HPROPERTY property,
                                          void* before_value,
                                          void const* new_value,
                                          void* after_value);
...
typedef void(AILCALL* MIXER_FLUSH)(S32* dest, S32 len
#ifdef IS_X86
                                   ,
                                   U32 MMX_available
#endif
...
typedef void(AILCALL* MIXER_MERGE)(void const** src, U32* src_fract,
                                   void const* src_end, S32** dest,
                                   void* dest_end, S32* left_val,
                                   S32* right_val, S32 playback_ratio,
                                   S32 scale_left, S32 scale_right,
                                   U32 operation
#ifdef IS_X86
                                   ,
                                   U32 MMX_available
...
//
// "option" parameter is big_endian_output on Mac, MMX on x86, overwrite flag on
// PS2
//

typedef void(AILCALL* MIXER_COPY)(void const* src, S32 src_len, void* dest,
                                  U32 operation
...
typedef void(AILCALL* MIXER_FLUSH)(S32* dest, S32 len, U32 MMX_available);

//
// Perform audio mixing operation
//

typedef void(AILCALL* MIXER_MERGE)(U32 src_sel, U32 dest_sel, U32* src_fract,
                                   U32* src_offset, U32* dest_offset,
                                   U32 src_end_offset, U32 dest_end_offset,
                                   S32* left_val, S32* right_val,
                                   S32 playback_ratio, S32 scale_both,
                                   U32 operation);
...
typedef void(AILCALL* MIXER_COPY)(void const* src, S32 src_len, void* dest,
                                  U32 operation, U32 option);
#endif

typedef struct _MSS_BB  // Used in both MC and conventional mono/stereo
                        // configurations
{
    ...
    S32 speaker_offset;  // Destination offset in interleaved PCM block for left
                         // channel
} MSS_BB;
...
typedef void(AILCALL* MIXER_MC_COPY)(MSS_BB* build, S32 n_build_buffers,
                                     void* lpWaveAddr, S32 hw_format,
#ifdef IS_X86
                                     S32 use_MMX,
#endif
...
typedef void(AILCALL* MIXER_ADPCM_DECODE)(void* dest, void const* in,
...
typedef F32(AILCALLBACK* AILFALLOFFCB)(HSAMPLE sample, F32 distance,
                                       F32 rolloff_factor, F32 min_dist,
                                       F32 max_dist);

typedef S32(AILCALLBACK* AILEVENTCB)(HMDIDRIVER hmi, HSEQUENCE seq, S32 status,
                                     S32 data_1, S32 data_2);
...
typedef void(AILCALLBACK* AILBEATCB)(HMDIDRIVER hmi, HSEQUENCE seq, S32 beat,
                                     S32 measure);

typedef void(AILCALLBACK* AILSEQUENCECB)(HSEQUENCE seq);

typedef S32(AILCALLBACK* SS_STREAM_CB)(HSAMPLE S, S16* dest_mono_sample_buffer,
...
typedef enum {
    ...
    N_SAMPLE_STAGES,  // Placeholder for end of list (= # of valid sample
                      // pipeline stages)
    SP_OUTPUT = N_SAMPLE_STAGES,  // Used to set/get prefs/attribs on a driver's
                                  // output or matrix filter (if present)
...
typedef enum {
    ...
    DP_DEFAULT_FILTER,  // Must be "MSS pipeline filter" provider (sets the
                        // default)
...
typedef enum {
    ...
    WIN32_POSITION_ERR,  // Nonzero if DirectSound play cursor stops moving
    ...
    PSP_AUDIO_PORT,     // Port # Miles is using, -1 for simple audio, >= 0 for
                        // libwave

    PSP2_SUBMIT_THREAD,  // Handle to thread submitting chucks of audio to the
                         // hw
...
typedef struct _AIL_INPUT_INFO  // Input descriptor type
{
    ...
    S32 buffer_size;  // Maximum # of bytes to be passed to callback (-1 to use
                      // DIG_INPUT_LATENCY)
} AIL_INPUT_INFO;
...
typedef struct _S3DSTATE  // Portion of HSAMPLE that deals with 3D positioning
{
    ...
    F32 outer_angle;  // (Angles divided by two and convered to rads for
    ...
    F32 min_dist;
    S32 dist_changed;  // TRUE if min/max distances have changed and need to be
                       // sent to the hardware

    S32 auto_3D_atten;  // TRUE if distance/cone attenuation should be applied
                        // to wet signal
    F32 atten_3D;  // Attenuation due to distance/cone effects, calculated by
                   // software 3D positioner
    F32 rolloff;  // per sample rolloff factor to use instead of global rolloff,
                  // if non zero.

    F32 exclusion_3D;  // exclusion value computed by falloff graph. -1 if not
                       // affected.
    F32 lowpass_3D;    // low pass cutoff computed by falloff graph. -1 if not
                       // affected.

    F32 spread;

    HSAMPLE owner;  // May be NULL if used for temporary/internal calculations
    AILFALLOFFCB falloff_function;  // User function for min/max distance
...
typedef struct _SMPBUF {
    ...
    S32 reset_seek_pos;  // New destination offset in stream source data, for
                         // ASI codecs that care
} SMPBUF;
...
typedef struct _SAMPLE  // Sample instance
{
    ...
    // these are dynamic arrays sized as n_channels long (so 1 for mono, 2
    // stereo, 6 for 5.1)
    S32* left_val;
    S32* right_val;
    S32* last_decomp;
    ...
    // these are dynamic arrays pointing to dynamic arrays, each of the sub
    // arrays are n_channels long or [MAX_SPEAKERS][n_channels]
    F32** user_channel_levels;  // Channel levels set by
                                // AIL_set_sample_channel_levels()
                                // [source_channels][driver->logical_channels]
    S32** cur_scale;            // Calculated 11-bit volume scale factors for
                                // current/previous mixing interval
    S32** prev_scale;  // (These are all indexed by build buffer*2, not speaker
                       // indexes!)
    S32** ramps_left;

    // these are dynamic arrays
    F32* auto_3D_channel_levels;  // Channel levels set by 3D positioner
                                  // (always 1.0 if not 3D-positioned)
    F32* speaker_levels;  // one level per speaker (multiplied after user or 3D)

    S8* speaker_enum_to_source_chan;  // array[MSS_SPEAKER_xx] = -1 if not
                                      // present, else channel # 99% of the time
                                      // this is a 1:1 mapping and is zero.

    S32 lp_any_on;                       // are any of the low pass filters on?
    S32 user_channels_need_deinterlace;  // do any of the user channels require
                                         // a stereo sample to be deinterlaced?

    S32 n_buffers;  // # of buffers (default = 2)
    S32 head;
    S32 tail;
    S32 starved;      // Buffer stream has run out of data
    S32 exhaust_ASI;  // Are we prolonging the buffer lifetime until ASI output
                      // is exhausted?
    ...
    S32 orig_loop_count;  // Original loop properties specified by app, before
                          // any
    ...
    U32 bus_comp_input;    // The input to use for this bus's compressor, if we
                           // have one installed
    S32 bus_override_wet;  // If true, samples on this bus will use the bus's
                           // wet level instead of their own.
    U32 bus_signal_strength;  // The bus level.
    S32 bus_enable_limiter;   // If true, a basic limiter will be run on the
                              // samples prior to clamping to S16.
    S32 bus_limiter_atten;  // The attenuation that was applied on the last bus
                            // pass.

    S32 fade_to_stop;  // # of samples to fade to stop over. ( currently fixed
                       // at the volramp count )
    ...
    S32 is_3D;  // TRUE if channel levels are derived automatically from 3D
                // positional state, FALSE if they're controlled manually

    S3DSTATE S3D;  // Software version applies 3D positioning only if is_3D ==
                   // TRUE, but output filters always use it

#ifdef MSS_VFLT_SUPPORTED
    void* voice;  // Optional object used by output filter to store per-sample
                  // information such as DS3D buffers
#endif
...
typedef struct _DIG_INPUT_DRIVER  // Handle to digital input driver
{
    ...
    long device;
#ifdef IS_MAC
    char InternalRecordingState[128];  // Hide this so we dont' have to #include
                                       // OS stuff everywhere.
#endif
} DIG_INPUT_DRIVER;
...
typedef struct REVERB_SETTINGS {
    S32 room_type;   // Changes to this drive master_wet and
                     // duration/damping/predelay!
...
typedef struct _MSS_RECEIVER_LIST {
    ...
    S32 speaker_index[MAX_SPEAKERS];  // List of speakers affected by sounds in
                                      // this direction
    F32 speaker_level[MAX_SPEAKERS];  // Each speaker's degree of effect from
                                      // this source
...
typedef struct _D3DSTATE {
    ...
    S32 directional_channels[MAX_SPEAKERS +
                             1];  // Channel index, or -1 if virtual
...
typedef enum {
    ...
    MSS_MC_USE_SYSTEM_CONFIG =
        0x10,                  // Leave space between entries for new variations
...
typedef struct _DIG_DRIVER  // Handle to digital audio driver
{
    ...
    MSS_MC_SPEC channel_spec;  // Original "channels" value passed to
                               // AIL_open_digital_driver()

    D3DSTATE D3D;  // 3D listener parms for all platforms

#if defined(IS_PSP2) || defined(IS_PSP) || defined(IS_XENON) ||        \
    defined(IS_IPHONE) || defined(IS_MAC) || defined(IS_LINUX) ||      \
    defined(IS_3DS) || defined(IS_WIIU) || defined(__RADWINRTAPI__) || \
    defined(__RADSEKRIT2__) ||                                         \
    defined(__RADANDROID__)  // generic dig platforms
#define IS_GENERICDIG
    ...
    S32 bytes_per_channel;   // # of bytes per channel (always 1 or 2 for 8- or
                             // 16-bit hardware output)
    S32 samples_per_buffer;  // # of samples per build buffer / half-buffer
    S32 physical_channels_per_sample;  // # of channels per *physical* sample (1
                                       // or 2, or more in discrete MC mode)
    S32 logical_channels_per_sample;   // # of logical channels per sample (may
                                       // differ from physical channel count in
                                       // matrix formats)
    ...
    U32* sample_status;  // SMP_ flags: _FREE, _DONE, _PLAYING, moved out of
                         // SAMPLEs for faster iteration
    ...
    S32 n_build_buffers;  // # of build buffers actually used for output
                          // processing
    ...
    S32 scheduled_sample_count;  // # of samples that are waiting to be started
                                 // at an exact time.
    ...
    AILLPDIRECTSOUNDBUFFER*
        lpbufflist;      // List of pointers to secondary buffers
    ...
    //
    // Note: # of "logical" (source) channels per sample =
    // dig->channels_per_sample
    //       # of "physical" (DAC) channels per sample =
    //       dig->wformat.wf.nChannels
    //
    ...
    C8 wfextra[32];          // Extension to PCMWAVEFORMAT (e.g.,
    ...
    S32 DS_use_default_format;  // 1 to force use of default DS primary buffer
                                // format
...
#if defined(IS_MAC) || defined(IS_LINUX) || defined(IS_XENON) ||               \
    defined(IS_PS3) || defined(IS_WII) || defined(IS_PSP) ||                   \
    defined(IS_PSP2) || defined(__RADWINRTAPI__) || defined(__RADSEKRIT2__) || \
    defined(__RADANDROID__)
...
#if (defined(IS_WINDOWS) && !defined(__RADWINRTAPI__)) || defined(IS_MAC) || \
    defined(IS_LINUX)
...
#if defined(IS_XENON) || defined(IS_WII) || defined(IS_PS3) ||          \
    defined(IS_PSP) || defined(IS_3DS) || defined(IS_IPHONE) ||         \
    defined(IS_PSP2) || defined(IS_WIIU) || defined(__RADWINRTAPI__) || \
    defined(__RADSEKRIT2__) || defined(__RADANDROID__)
...
#define AIL_mem_alloc_lock_trk(size) \
    AIL_mem_alloc_lock_info(size, __FILE__, __LINE__)
#define AIL_file_read_trk(file, dest) \
    AIL_file_read_info(file, dest, __FILE__, __LINE__)
...
DXDEC void* AILCALL AIL_mem_alloc_lock_info(UINTa size, char const* file,
                                            U32 line);
DXDEC void AILCALL AIL_mem_free_lock(void* ptr);

DXDEC S32 AILCALL AIL_file_error(void);

DXDEC S32 AILCALL AIL_file_size_info(char const* filename, char const* caller,
                                     U32 line);

DXDEC void* AILCALL AIL_file_read_info(char const* filename, void* dest,
                                       char const* caller, U32 line);
...
#ifdef IS_WIIU
DXDEC void AILCALL AIL_set_wiiu_file_client(void* ptr_to_fsclient,
                                            void* ptr_to_fscmdblock);
#endif

#ifdef __RADIPHONE__
//
// On iOS, audio session interruptions stop the audio queues, and we have to
// manually restart them.
//
// This should be called whenever you get an Interruption Ended msg via your
...
DXDEC UINTa AILCALL AIL_set_timer_user(HTIMER timer, UINTa user);
...
typedef S32 (*RADSS_OPEN_FUNC)(struct _RadSoundSystem* i_SoundSystem,
                               U32 i_MinBufferSizeInMs, U32 i_Frequency,
                               U32 i_ChannelCount, U32 i_MaxLockSize,
                               U32 i_Flags);

DXDEC HDIGDRIVER AILCALL AIL_open_generic_digital_driver(
    U32 frequency, S32 bits, S32 channel, U32 flags, RADSS_OPEN_FUNC dig_open);
...
#define AIL_open_digital_driver(frequency, bits, channel, flags)     \
    AIL_open_generic_digital_driver(frequency, bits, channel, flags, \
                                    RADSS_XAudio2InstallDriver(0, 0))
...
DXDEC S32 AILCALL AIL_sprintf(char* dest, char const* fmt, ...);
...
DXDEC EXPAPI void AILCALL AIL_push_system_state(HDIGDRIVER dig, U32 flags,
                                                S16 crossfade_ms);
/*
    Pushes the current system state, allowing for a temporary "clean" driver to
   use, and then revert from.

    $:dig The driver to push
    $:flags Logical "or" of options controlling the extent of the push. See
   discussion.
    $:crossfade_ms The number of milliseconds to fade the transition over. [0,
   32767]

    By default (ie flags == 0), effectively nothing happens. Since the operation
   neither affects any subsystems nor resets the playing samples, a push
   immediately followed by a pop should have no audible effects.

    However, any samples started during the push will be stopped (via
   $AIL_end_fade_sample) when the system is popped. Streams will return SMP_DONE
   via $AIL_stream_status. It is up to the client code to perform any cleanup
   required.

    The flags can alter the above behavior in the following ways:

    $* <b>MILES_PUSH_RESET</b> - This flag causes the system to revert to a
   "new" state when pushed. Without any other flags this will only be apparent
   with samples - any playing samples will cease to be processed (though they
   will still report SMP_PLAYING). When the system is popped, these samples will
   resume.

    $* <b>MILES_PUSH_REVERB</b> - When present, reverb state will be affected in
   addition to sample state. If MILES_PUSH_RESET is present, the reverb will be
   cleared to zero on push. Otherwise, it will be retained, and only affected
   when popped.

    $* <b>MILES_PUSH_3D</b> - When present, 3d listener state will be affected
   in addition to sample state. If MILES_PUSH_RESET is present, the 3d listener
   state will be reverted to the same state as a new driver. Otherwise it will
   be retained and only affected when popped.

    $* <b>MILES_PUSH_VOLUME</b> - When present, master volume will be affected
   in addition to sample state. If MILES_PUSH_RESET is present, the master
   volume will be set to 1.0f, otherwise it will be retained and only affected
   when popped.

    $-

    If you want more control over whether a sample will be affected by a push or
   a pop operation, see $AIL_set_sample_level_mask.

*/

DXDEC EXPAPI void AILCALL AIL_pop_system_state(HDIGDRIVER dig,
                                               S16 crossfade_ms);
/*
    ...
    $:crossfade_ms The number of milliseconds to crossfade the transition over -
   [0, 32767]
...
/*
    ...
    $:return A value between 0 and MILES_MAX_STATES, representing the depth of
...
/*
    ...
    ${
        AIL_set_sample_level_mask(S, AIL_sample_level_mask(S) |= (1 <<
   (AIL_system_state_level(dig) - 1)));
    $}
*/

DXDEC EXPAPI U8 AILCALL AIL_sample_level_mask(HSAMPLE S);
/*
    Return the mask used to determine if the sample will play at a given system
   level.
...
/*
    ...
    $:on_off If non-zero, the limiter will be enabled, otherwise it will be
   disabled.

    By default limiters are off. Currently they are not configurable. They kick
   on around -10 db, and with a 0db signal will attenuate by about -18 db.
   Limiters run prior to the 16 bit clamp.
...
/*
    ...
    A bus allows you to treat a group of samples as one sample. With the bus
   sample you can do almost all of the things you can do with a normal sample
   handle. The only exception is you can't adjust the playback rate of the
   sample.

    Use $AIL_bus_sample_handle to get the HSAMPLE associated with a bus.

    Each call to AIL_allocate_bus adds a new bus, up to a total bus count of
   MAX_BUSSES. After the first call, two busses exist - the main bus and the
   first aux bus. The HSAMPLE returned is for the first aux bus (index 1)
*/

DXDEC EXPAPI void AILCALL AIL_bus_enable_limiter(HDIGDRIVER dig, S32 bus_index,
                                                 S32 on_off);
/*
    Enables a basic limiter to prevent clipping.
...
DXDEC EXPAPI HSAMPLE AILCALL AIL_bus_sample_handle(HDIGDRIVER dig,
                                                   S32 bus_index);
/*
    ...
    If the bus has not been allocated, or no busses have been allocated, this
   returns 0. This means that for the "Main Bus" - index 0 - it will still
   return zero if no additional busses have been allocated.
*/
...
DXDEC EXPAPI S32 AILCALL AIL_install_bus_compressor(HDIGDRIVER dig,
                                                    S32 bus_index,
                                                    SAMPLESTAGE filter_stage,
                                                    S32 input_bus_index);
/*
    Installs the Compressor filter on to a bus, using another bus as the input
   for compression/limiting.
    ...
    This installs a side chain compressor in to a given bus. It acts exactly
   like any other filter you would put on an HSAMPLE, except the input_bus_index
   bus pipe's its signal strength to the filter, allowing it to attenuate the
   bus_index bus based on another bus's contents.

    To control the compressor parameters, access the bus's HSAMPLE via
   $AIL_bus_sample_handle and use $AIL_sample_stage_property exactly as you
   would any other filter. The filter's properties are documented under
   $(Compressor Filter)
*/
...
DXDEC MSSVECTOR3D* AILCALL AIL_speaker_configuration(HDIGDRIVER dig,
...
DXDEC
void AILCALL AIL_sample_speaker_scale_factors(
    HSAMPLE S,  //)
...
DXDEC EXPAPI void AILCALL AIL_schedule_start_sample(HSAMPLE S,
                                                    U64 mix_time_to_start);
/*
...
/*
    Returns the mix time the sample is scheduled to start at, or 0 if not
   scheduled.
...
DXDEC EXPAPI void AILCALL AIL_start_sample_group(HDIGDRIVER dig, S32 start_id,
                                                 S32 set_to_id);
/*
    ...
    This function atomically calls $AIL_start_sample on all the samples to
   ensure the samples start in sync.
*/

DXDEC EXPAPI void AILCALL AIL_stop_sample_group(HDIGDRIVER dig, S32 stop_id,
                                                S32 set_to_id);
/*
    ...
    This function atomically calls $AIL_stop_sample on all the samples to ensure
   they stop at the same point.
*/

DXDEC EXPAPI void AILCALL AIL_resume_sample_group(HDIGDRIVER dig, S32 resume_id,
                                                  S32 set_to_id);
/*
    ...
    This function atomically calls $AIL_resume_sample on all the samples to
   ensure the samples start in sync.
*/
...
DXDEC void AILCALL
AIL_set_sample_playback_rate_factor(HSAMPLE S, F32 playback_rate_factor);

DXDEC void AILCALL AIL_set_sample_playback_delay(HSAMPLE S, S32 playback_delay);
...
DXDEC EXPAPI S32 AILCALL AIL_set_sample_loop_samples(HSAMPLE S,
...
/*
    ...
    For uncompressed samples, this largely reverts to
   $AIL_set_sample_loop_block, since the mapping is straightforward. For
   compressed formats (like bink audio or mp3), looping in sample space is non
   trivial and must be handled on a format-by-format basis. For the moment, only
   Bink Audio supports this functionality - all other ASI formats will return
   failure.
...
DXDEC F32 AILCALL AIL_sample_output_levels(
    HSAMPLE S, MSS_SPEAKER const* source_speaker_indexes,
    MSS_SPEAKER const* dest_speaker_indexes, F32* levels, S32 n_levels);
...
DXDEC void AILCALL AIL_sample_51_volume_levels(
    HSAMPLE S, F32* f_left_level, F32* f_right_level, F32* b_left_level,
    F32* b_right_level, F32* center_level, F32* sub_level);
DXDEC void AILCALL AIL_set_digital_master_reverb(HDIGDRIVER dig, S32 bus_index,
                                                 F32 reverb_decay_time,
                                                 F32 reverb_predelay,
                                                 F32 reverb_damping);

DXDEC void AILCALL AIL_digital_master_reverb(HDIGDRIVER dig, S32 bus_index,
                                             F32* reverb_time,
                                             F32* reverb_predelay,
                                             F32* reverb_damping);
...
DXDEC S32 AILCALL AIL_minimum_sample_buffer_size(HDIGDRIVER dig,
                                                 S32 playback_rate, S32 format);

DXDEC S32 AILCALL AIL_set_sample_buffer_count(HSAMPLE S, S32 n_buffers);
...
DXDEC AILSAMPLECB AILCALL AIL_register_SOB_callback(HSAMPLE S, AILSAMPLECB SOB);
...
DXDEC AILMIXERCB AILCALL AIL_register_mix_callback(HDIGDRIVER dig,
                                                   AILMIXERCB mixcb);
...
DXDEC SINTa AILCALL AIL_sample_user_data(HSAMPLE S, U32 index);
...
DXDEC AIL_INPUT_INFO* AILCALL AIL_get_input_info(HDIGINPUT dig);

DXDEC S32 AILCALL AIL_set_input_state(HDIGINPUT dig, S32 enable);
#endif
...
DXDEC S32 AILCALL AIL_channel_notes(HSEQUENCE S, S32 channel);
...
DXDEC AILTRIGGERCB AILCALL AIL_register_trigger_callback(HSEQUENCE S,
...
DXDEC SINTa AILCALL AIL_sequence_user_data(HSEQUENCE S, U32 index);
...
struct MilesAsyncRead {
    ...
    S32 volatile Status;  // This is only valid after a call to
                          // MilesAsyncFileWait or MilesAsyncFileCancel has
                          // succeeded.
    char Internal[48 + 128];
};

DXDEC S32 AILCALL MilesAsyncFileRead(struct MilesAsyncRead* i_Request);
DXDEC S32 AILCALL MilesAsyncFileCancel(
    struct MilesAsyncRead* i_Request);  // 1 if the request has completed, 0
                                        // otherwise. Use Wait if needed.
DXDEC S32 AILCALL
MilesAsyncFileStatus(struct MilesAsyncRead* i_Request,
                     U32 i_MS);  // 1 if complete, 0 if timeout exceeded.
...
typedef S32(AILCALLBACK* MilesAsyncFileRead_callback)(
    struct MilesAsyncRead* i_Request);
typedef S32(AILCALLBACK* MilesAsyncFileCancel_callback)(
    struct MilesAsyncRead* i_Request);  // 1 if the request has completed, 0
                                        // otherwise. Use Wait if needed.
typedef S32(AILCALLBACK* MilesAsyncFileStatus_callback)(
    struct MilesAsyncRead* i_Request,
    U32 i_MS);  // 1 if complete, 0 if timeout exceeded.
...
DXDEC void AILCALL AIL_set_async_callbacks(
    MilesAsyncFileRead_callback read, MilesAsyncFileCancel_callback cancel,
    MilesAsyncFileStatus_callback status, MilesAsyncStartup_callback startup,
...
typedef struct _STREAM {
    ...
    S32 reset_ASI[MSS_STREAM_CHUNKS];  // should we reset the ASI at the end of
                                       // the buffer?
    ...
    S32 asyncs_loaded[MSS_STREAM_CHUNKS];  // 0=unloaded, 1=loading, 2=loaded,
                                           // but not started
    S32 next_read_offset;  // offset to pass to the next read, so the seek
                           // occurs internally. -1 to not seek.
    ...
    S32 playcontrol;  // control: 0=stopped, 1=started, |8=paused, |16=sample
                      // paused
    ...
    S32 was_popped;  // set to 1 if the stream needs to be freed due to a system
                     // push/pop - causes SMP_DONE to be stream_status
} MSTREAM_TYPE;
...
DXDEC SINTa AILCALL AIL_stream_user_data(HSTREAM S, U32 index);
...
typedef S32(AILCALLBACK* AIL_file_seek_callback)(UINTa FileHandle, S32 Offset,
                                                 U32 Type);

typedef U32(AILCALLBACK* AIL_file_read_callback)(UINTa FileHandle, void* Buffer,
                                                 U32 Bytes);
...
typedef S32(AILCALLBACK* AIL_file_async_status_callback)(void* async, S32 wait,
                                                         U32* BytesRead);
...
DXDEC S32 AILCALL AIL_quick_startup(S32 use_digital, S32 use_MIDI,
                                    U32 output_rate, S32 output_bits,
                                    S32 output_channels);
...
DXDEC HAUDIO AILCALL AIL_quick_load_mem(void const* mem, U32 size);
...
DXDEC S32 AILCALL AIL_quick_play(HAUDIO audio, U32 loop_count);
...
DXDEC S32 AILCALL AIL_WAV_marker_by_index(void const* WAV_image, S32 n,
                                          C8 const** name);
...
typedef S32(AILCALLBACK* AILCODECSETPROP)(char const* property,
                                          void const* value);
...
typedef struct {
    ...
    //
    // This is valid after AIL_inspect_MP3() is called (even if the file
    // contains no valid frames)
    //

    U8* MP3_file_image;  // Original MP3_file_image pointer passed to
                         // AIL_inspect_MP3()
    ...
    U8* start_MP3_data;  // Pointer to start of data area in file (not
                         // necessarily first valid frame)
    U8* end_MP3_data;    // Pointer to last valid byte in MP3 data area (before
                         // ID3v1 tag, if any)

    //
    // Information about current frame being inspected, valid if
    // AIL_enumerate_MP3_frames() returns TRUE
    //

    S32 sample_rate;  // Sample rate in Hz (normally constant across all frames
                      // in file)
    S32 bit_rate;     // Bits/second for current frame
    S32 channels_per_sample;  // 1 or 2
    S32 samples_per_frame;    // Always 576 or 1152 samples in each MP3 frame,
                              // depending on rate

    S32 byte_offset;  // Offset of frame from start_MP3_data (i.e., suitable for
                      // use as loop point)
    S32 next_frame_expected;  // Anticipated offset of next frame to be
                              // enumerated, if any
    S32 average_frame_size;   // Average source bytes per frame, determined
                              // solely by bit rate and sample rate
    ...
    S32 MPEG1;  // Data copied directly from frame header, see ISO docs for
                // info...
    S32 MPEG25;
    S32 layer;
    S32 protection_bit;
...
typedef struct {
    ...
    // File-level data accessible to app
    //
    // This is valid after AIL_inspect_Ogg() is called (even if the file
    ...
    //
    // Information about current page being inspected, valid if
    // AIL_enumerate_Ogg_pages() returns TRUE
    //

    S32 page_num;  // 32-bit page sequence number from OggS header at byte
                   // offset 16

    S32 sample_count;  // Total # of samples already generated by encoder at the
                       // time the current page was written

    S32 byte_offset;  // Offset of page from start_Ogg_data (i.e., suitable for
                      // use as loop point)
    S32 next_page_expected;  // Anticipated offset of next page to be
                             // enumerated, if any
...
DXDEC void AILCALL AIL_inspect_Ogg(OGG_INFO* inspection_state,
                                   U8* Ogg_file_image, S32 Ogg_file_size);
...
DXDEC S32 AILCALL AIL_room_type(HDIGDRIVER dig, S32 bus_index);

DXDEC void AILCALL AIL_set_room_type(HDIGDRIVER dig, S32 bus_index,
                                     S32 room_type);
...
DXDEC EXPAPI void AILCALL AIL_set_sample_3D_volume_falloff(HSAMPLE S,
                                                           MSSGRAPHPOINT* graph,
                                                           S32 pointcount);
/*
    ...
    $:pointcount The number of points passed in. Must be less than or equal to
   MILES_MAX_FALLOFF_GRAPH_POINTS. Passing 0 removes the graph.

    This marks a sample as having a volume falloff graph. If a sample has a
   volume graph, it no longer attenuates as per the default falloff function,
   and as such, its "minimum distance" no longer has any effect. However, the
   "max distance" still clamps the sample to full attenuation.
   ...
   always.
    ...
    If the distance is past the last graph point, graph[pointcount-1].Y is
   returned.

    If either the output tangent type of the previous point, or the input
   tangent type of the next point are MILES_TANGENT_STEP, previous->Y is
   returned.

    Otherwise, the segment is evaluated as a hermite curve. ITX and ITY are
   ignore if ITYpe is MILES_TANGENT_LINEAR, and likewise OTX and OTY are ignored
   if OType is MILES_TANGENT_LINEAR.
*/

DXDEC EXPAPI void AILCALL AIL_set_sample_3D_lowpass_falloff(
    HSAMPLE S, MSSGRAPHPOINT* graph, S32 pointcount);
/*
   ...
   MILES_MAX_FALLOFF_GRAPH_POINTS. Passing 0 removes the graph.
    ...
    This marks a sample as having a low pass cutoff that varies as a function of
   distance to the listener. If a sample has such a graph,
   $AIL_set_sample_low_pass_cut_off will be called constantly, and thus
   shouldn't be called otherwise.

    The graph is evaluated the same as $AIL_set_sample_3D_volume_falloff.
*/

DXDEC EXPAPI void AILCALL AIL_set_sample_3D_exclusion_falloff(
    HSAMPLE S, MSSGRAPHPOINT* graph, S32 pointcount);
/*
   ...
   MILES_MAX_FALLOFF_GRAPH_POINTS. Passing 0 removes the graph.
    ...
    This marks a sample as having an exclusion that varies as a function of
   distance to the listener. If a sample has such a graph, auto_3D_wet_atten
   will be disabled to prevent double affects, as exclusion affects reverb wet
   level.

    The graph is evaluated the same as $AIL_set_sample_3D_volume_falloff.
*/

DXDEC EXPAPI void AILCALL AIL_set_sample_3D_spread_falloff(HSAMPLE S,
                                                           MSSGRAPHPOINT* graph,
                                                           S32 pointcount);
/*
   ...
   MILES_MAX_FALLOFF_GRAPH_POINTS. Passing 0 removes the graph.
    ...
    This marks a sample as having a spread that varies as a function of distance
   to the listener. See $AIL_set_sample_3D_spread.

    The graph is evaluated the same as $AIL_set_sample_3D_volume_falloff.
*/

DXDEC EXPAPI void AILCALL AIL_set_sample_3D_position_segments(
    HSAMPLE S, MSSVECTOR3D* points, S32 point_count);
/*
    Sets a sample's position as a series of line segments.

    $:S Sample to affect
    $:points The 3D points representing the line segments. 0 reverts to classic
   point based positioning. All segments are connected - N points represents N -
   1 chained line segments.
    $:point_count Size of points array. Minimum 2 (unless removing), max
   MILES_MAX_SEGMENT_COUNT

    This marks a sample as having a position that is not a single point. When 3D
   attenuation is computed, the closest point to the listener is found by
   walking each segment. That position is then used in all other computations
   (cones, falloffs, etc). Spatialization is done using all segments as a
   directional source.

    If there is neither spread falloff nor volume falloff specified, spread will
   be automatically applied when the listener is within min_distance to the
   closest point. See $AIL_set_sample_3D_spread_falloff and
   $AIL_set_sample_3D_volume_falloff.
...
/*
    ...
    Spread is how much the directionality of a sample "spreads" to more speakers
   - emulating the effect a sound has when it occupies more than a point source.
   For instance, a sound point source that sits directly to the left of the
   listener would have a very strong left speaker signal, and a fairly weak
   right speaker signal. Via spread, the signal would be more even, causing the
   source to feel as though it is coming from an area, rather than a point
   source.

    A spread of 1 will effectively negate any spatialization effects other than
...
/*
  ...
  This handle typedef refers to an open soundbank which is usually obtained from
  the $AIL_add_soundbank function.
*/

EXPGROUP(highlevel_util)

DXDEC EXPAPI HMSOUNDBANK AILCALL AIL_open_soundbank(char const* filename,
                                                    char const* name);
/*
    Open a sound bank. If you are using the event execution engine, use the add
   soundbank function provided there.

    $:return 0 on fail, or a valid HMSOUNDBANK.
    $:filename The filename of the soundbank to open.

    Opens a sound bank for use with the MSS8 high level functions. The sound
   bank must be closed with $AIL_close_soundbank. Use $AIL_add_soundbank if the
   Miles Event system is used.
*/
...
/*
    ...
    Close a soundbank previously opened with $AIL_open_soundbank. Presets/events
   loaded from this soundbank are no longer valid.
*/
...
/*
    ...
    $:return A pointer to the filename for the given soundbank, or 0 if bank is
   invalid.

    Returns a pointer to the filename for a soundbank. This pointer should not
   be deleted.
*/
...
/*
    ...
    $:return A pointer to the name of the sound bank, or 0 if the bank is
   invalid.

    The name of the bank is the name used in asset names. This is distinct from
   the file name of the bank.
...
/*
    ...
    Returns the memory used via AIL_mem_alloc_lock during the creation of this
   structure.
*/

DXDEC EXPAPI S32 AILCALL AIL_enumerate_sound_presets(HMSOUNDBANK bank,
                                                     HMSSENUM* next,
                                                     char const* list,
                                                     char const** name);
/*
  ...
  $:list Optional filter. If specified, presets will only enumerate from the
  given preset sound preset list.
  $:name The pointer to the currently enumerated preset name. This should not be
  deleted.
  ...
  $:return Returns 0 when enumeration is complete.
   ...
   ${
    ...
    Note that name should NOT be deleted by the caller - this points at memory
  owned by Miles.
*/

DXDEC EXPAPI S32 AILCALL AIL_enumerate_environment_presets(HMSOUNDBANK bank,
                                                           HMSSENUM* next,
                                                           char const* list,
                                                           char const** name);
/*
    ...
    $:next Enumeration token. Prior to first call, initialize to MSS_FIRST
    $:list Optional filter. If specified, presets will only enumerate from the
   given environment preset list.
    $:name The pointer to the currently enumerated preset name. This should not
   be deleted.
    $:return Returns 0 when enumeration is complete.

    Enumerates the environment presets available inside of a bank file. Example
   usage:

    ${
            HMSSENUM Token = MSS_FIRST;
            const char* PresetName = 0;
            while (AIL_enumerate_environment_presets(MyBank, &Token, 0,
   &PresetName))
            {
                printf("Found a preset named %s!", PresetName);
...
DXDEC EXPAPI S32 AILCALL AIL_enumerate_sound_assets(HMSOUNDBANK bank,
                                                    HMSSENUM* next,
                                                    char const** name);
/*
    ...
    $:bank Containing soundbank.
    $:next Enumeration token. Prior to first call, initialize to MSS_FIRST
    $:name The pointer to the currently enumerated sound name. This should not
   be deleted.
    $:return Returns 0 when enumeration is complete.
...
DXDEC EXPAPI S32 AILCALL AIL_enumerate_events(HMSOUNDBANK bank, HMSSENUM* next,
                                              char const* list,
                                              char const** name);
/*
    ...
    $:next Enumeration token. Prior to first call, initialize to MSS_FIRST
    $:list Optional filter. If specified, event will only enumerate from the
   given event list.
    $:name The pointer to the currently enumerated preset name. This should not
   be deleted.
    $:return Returns 0 when enumeration is complete.
...
DXDEC EXPAPI void* AILCALL AIL_find_environment_preset(HMSOUNDBANK bank,
                                                       char const* name);
/*
...
DXDEC EXPAPI void* AILCALL AIL_find_sound_preset(HMSOUNDBANK bank,
                                                 char const* name);
/*
...
DXDEC EXPAPI S32 AILCALL AIL_apply_raw_sound_preset(HSAMPLE sample,
                                                    void* preset);
/*
    ...
    Updates sample properties based on the desired settings specified in the
   given preset.
*/

DXDEC EXPAPI S32 AILCALL AIL_apply_sound_preset(HSAMPLE sample,
                                                HMSOUNDBANK bank,
                                                char const* name);
/*
    ...
    $:return Returns 0 on fail - check for sample/bank validity, and that the
   preset is in the correct bank.

    This will alter the properties on a given sample, based on the given preset.
*/

DXDEC EXPAPI S32 AILCALL AIL_unapply_raw_sound_preset(HSAMPLE sample,
                                                      void* preset);
/*
    Returns the properties altered by the preset to their default state.

    $:sample The sample to update.
    $:preset The raw preset data to unapply, returned from
   $AIL_find_sound_preset
*/

DXDEC EXPAPI S32 AILCALL AIL_unapply_sound_preset(HSAMPLE sample,
                                                  HMSOUNDBANK bank,
                                                  char const* name);
/*
   ...
   preset is in the correct bank.
    ...
    Presets may or may not affect any given property. Only the properties
   affected by the specified preset will have their values restored to default.
*/

typedef S32 (*MilesResolveFunc)(void* context, char const* exp, S32 explen,
                                EXPOUT void* output, S32 isfloat);
/*
    ...
    The function callback should convert variable expressions in to an output
   value of the requested type.
*/

DXDEC EXPAPI S32 AILCALL AIL_resolve_raw_sound_preset(void* preset,
                                                      void* context,
                                                      MilesResolveFunc eval);
/*
    Compute the value of properties for the current value of variables using the
   given lookup function.
    ...
    This function converts variable expressions that were stored in the preset
   in to values that can be used by the event system. The values are stored in
   the preset itself, all that has to happen is this is called with a valid
   resolve function prior to calling $AIL_apply_raw_sound_preset.
*/

DXDEC EXPAPI S32 AILCALL
AIL_resolve_raw_environment_preset(void* env, MilesResolveFunc eval);
/*
    Compute the value of properties for the current value of variables using the
   given lookup function.
    ...
    This function converts variable expressions that were stored in the
   environment in to values that can be used by the event system. The values are
   stored in the environment itself, all that has to happen is this is called
   with a valid resolve function prior to calling
    $AIL_apply_raw_environment_preset.
*/

DXDEC EXPAPI S32 AILCALL AIL_apply_raw_environment_preset(HDIGDRIVER dig,
                                                          void* environment);
/*
    Applies the environment to the given driver.

    $:dig The driver to modify.
    $:environment The raw environment data to apply, returned from
   $AIL_find_environment_preset

    Updates driver properties based on the desired settings specified in the
   given environment.
*/

DXDEC EXPAPI S32 AILCALL AIL_apply_environment_preset(HDIGDRIVER dig,
                                                      HMSOUNDBANK bank,
                                                      char const* name);
/*
...
DXDEC EXPAPI S32 AILCALL AIL_unapply_raw_environment_preset(HDIGDRIVER dig,
                                                            void* environment);
/*
    Returns the properties the environment affects to default state.

    $:dig The driver to modify.
    $:environment The raw environment data to unapply, returned from
   $AIL_find_environment_preset
*/

DXDEC EXPAPI S32 AILCALL AIL_unapply_environment_preset(HDIGDRIVER dig,
                                                        HMSOUNDBANK bank,
                                                        char const* name);
/*
    Restore the properties affected by the given preset to defaults.
   ...
   preset is in the correct bank.
    ...
    Presets may or may not affect any given property. Only the properties
   affected by the specified preset will have its value restored to default.
*/
...
/*
    ...
    $:SoundLimit The maximum number of instances of this sound that is allowed
   to play at once.
    $:IsExternal Nonzero if the sound is stored external to the sound bank. See
   the eventexternal sample.
    $:DurationMs The length of the sound asset, in milliseconds.
    $:StreamBufferSize If the sound is played as a stream, this is the buffer to
   use for this sound.
    $:IsAdpcm Nonzero if the asset is an adpcm sound, and needs to be
   initialized as such.
    $:AdpcmBlockSize The adpcm block size if the asset is adpcm encoded.
    $:MixVolumeDAC The attenuation to apply to all instances of this sound, as a
   DAC scalar.

    See $AIL_sound_asset_info.
*/

DXDEC EXPAPI S32 AILCALL AIL_sound_asset_info(HMSOUNDBANK bank,
                                              char const* name, char* out_name,
                                              MILESBANKSOUNDINFO* out_info);
/*
    ...
    $:out_name Optional - Pointer to a buffer that is filled with the sound
   filename to use for loading.
    $:out_info Pointer to a $MILESBANKSOUNDINFO structure that is filled with
   meta data about the sound asset.
...
DXDEC EXPAPI SINTa AILCALL AIL_get_marker_list(HMSOUNDBANK bank,
                                               char const* sound_name);
/*
    Return an opaque value representing the list of markers attached to a given
   sound name.

    $:bank The bank containing the sound asset.
    $:sound_name The name of the sound asset.

    $:return on fail/nonexistent list, or a nonzero opaque value to be passed to
   $AIL_find_marker_in_list.

    Returns the marker list for a given sound asset. This value should just be
   passed directly to $AIL_find_marker_in_list to retrieve the offset for a
   marker by name.
*/

DXDEC EXPAPI S32 AILCALL AIL_find_marker_in_list(SINTa marker_list,
...
/*
   ...
   $:is_samples returns whether the marker is at a sample location instead of a
   byte location.

   $:return -1 if the marker was not found, or the byte offset of the marker.

   Looks up an offset to use in functions such as $AIL_set_sample_position.
   marker_list can be retrieved with $AIL_get_marker_list.
*/
...
/*!
    function
    {
        ...
        Name = "AIL_create_event", "Creates an empty event to be filled with
   steps."

        ReturnType = "HMSSEVENTCONSTRUCT", "An empty event to be passed to the
   various step addition functions, or 0 if out of memory."

        Discussion = "Primarily designed for offline use, this function is the
   first step in creating an event that can be consumed by the MilesEvent
   system. Usage is as follows:
            ...
            Note that if immediately passed to AIL_enqueue_event(), the memory
   must remain valid until the following $AIL_complete_event_queue_processing.

            Events are generally tailored to the MilesEvent system, even though
   there is nothing preventing you from writing your own event system, or
   creation ui.
            "
    }
*/
...
/*!
    function
    {
        ...
        Name = "AIL_close_event", "Returns a completed event, ready for
   enqueueing in to the MilesEvent system."

        In = "HMSSEVENTCONSTRUCT", "i_Event", "The event to complete."

        ReturnType = "char*", "An allocated event std::string that can be passed
   to AIL_next_event_step or enqueued in the MilesEvent system via
   AIL_enqueue_event."

        Discussion = "The returned pointer must be deleted via
   AIL_mem_free_lock(). Note that if the MilesEvent system is used, the event
   pointer must remain valid through the following
   $AIL_complete_event_queue_processing call."
...
/*
    ...
    $:MILES_START_STEP_VOLUME Evict the quietest sound after mixing, using the
   loudest channel as the qualifier.
...
EXPTYPEEND
/*
    Determines the usage of the sound names list in the
   $AIL_add_start_sound_event_step.

    $:MILES_START_STEP_RANDOM Randomly select from the list, and allow the same
        sound to play twice in a row. This is the only selection type that
   doesn't require a state variable.
    $:MILES_START_STEP_NO_REPEATS Randomly select from the list, but prevent the
   last sound from being the same.
    $:MILES_START_STEP_IN_ORDER Play the list in order, looping.
    $:MILES_START_STEP_RANDOM_ALL_BEFORE_REPEAT Randomly select from the list,
   but don't allow duplicates until all sounds have been played.
    $:MILES_START_STEP_BLENDED Play *all* of the sounds, using the state
   variable as both the variable name to poll, and the name of the blend
   function to look up. The blend should have been specified prior to execution
   of this step in the runtime, see $AIL_add_setblend_event_step.
    $:MILES_START_STEP_SELECT_MASK Expect a value from the game to determine
   which sound to play, added in to the other selection type.
*/
...
/*!
    function
    {
        ...
        Name = "AIL_add_start_sound_event_step", "Adds a step to a given event
   to start a sound with the given specifications."

        In = "HMSSEVENTCONSTRUCT", "i_Event", "The event to add the step to."
        In = "const char*", "i_SoundNames", "The names and associated weights
   for the event step to choose from. If there are multiple names listed, the
   sound will be chosen at random based on the given weights. This std::string
   is of the form 'BankName1/SoundName1:Weight1:BankName2/SoundName2:Weight2:'
   etc. The std::string must always terminate in a ':'. Weight must be between 0
   and 200. To provide a null sound to randomly choose to not play anything, use
            an empty std::string as an entry."

        In = "const char*", "i_PresetName", "[optional] The name of the preset,
   of the form 'PresetList/PresetName'" In = "U8", "i_PresetIsDynamic", "Nonzero
   if the preset should poll the value of variables every frame, instead of only
   when applied." In = "const char*", "i_EventName", "[optional] The name of the
   event to execute upon completion of the sound, of the form
   'PresetList/PresetName'" In = "const char*", "i_StartMarker", "[optional] The
   name of a marker to use as the loop start point." In = "const char*",
   "i_EndMarker", "[optional] The name of a marker to use as the loop end
   point." In = "const char*", "i_StateVar", "[optional] The name of a variable
   to use for storing state associated with this start sound step." In = "char
   const*", "i_VarInit", "[optional] A list of variable names, mins, and maxes
   to use for randomizing the sound instance state." In = "const char*",
   "i_Labels", "[optional] A comma delimited list of labels to assign to the
   sound." In = "U32", "i_Streaming", "If nonzero, the sound will be set up and
   started as a stream." In = "U8", "i_CanLoad", "If nonzero, the sound is
   allowed to hit the disk instead of only accessing cached sounds. If true,
   this might cause a hitch." In = "U16", "i_Delay", "The minimum delay in ms to
   apply to the sound before start." In = "U16", "i_DelayMax", "The maximum
   delay in ms to apply to the sound before start." In = "U8", "i_Priority",
   "The priority to assign to the sound. If a sound encounters a limit based on
   its labels, it will evict any sound with a priority strictly less than the
   given priority." In = "U8", "i_LoopCount", "The loop count as per
   AIL_set_sample_loop_count." In = "const char*", "i_StartOffset", "[optional]
   The name of the marker to use as the sound's initial offset." In = "F32",
   "i_VolMin", "The min volume value to randomly select for initial volume for
   the sound. In LinLoud." In = "F32", "i_VolMax", "The max volume value to
   randomly select for initial volume for the sound. In LinLoud." In = "F32",
   "i_PitchMin", "The min pitch to randomly select from for initial playback. In
   sT." In = "F32", "i_PitchMax", "The max pitch to randomly select from for
   initial playback. In sT." In = "F32", "i_FadeInTime", "The time to fade the
   sound in over. Interpolation is linear in loudness." In = "U8",
   "i_EvictionType", "The basis for deciding what sound will get kicked out if a
   limit is hit when trying to play this sound. See
   $MILES_START_STEP_EVICTION_TYPE." In = "U8", "i_SelectType", "The method to
   use for selecting the sound to play from the sound name list. See
   $MILES_START_SOUND_SELECTION_TYPE."

        ReturnType = "S32", "Returns 1 on success."

        Discussion = "Adds an event that can start a sound. If the sound names
   list contains multiple entries, one will be selected randomly based on the
   given weights and the selection type. Weights are effectively ratios for
   likelihood. A sound with 100 weight will be twice as likely as a sound with
   50 weight. Some times you may want to have an event that only *might* play a
   sound. To do this, add a empty sound name with an associated weight.
            "
    }
*/
DXDEC
S32 AILCALL AIL_add_start_sound_event_step(
    HMSSEVENTCONSTRUCT i_Event, const char* i_SoundNames,
    const char* i_PresetName, U8 i_PresetIsDynamic, const char* i_EventName,
    const char* i_StartMarker, const char* i_EndMarker, char const* i_StateVar,
    char const* i_VarInit, const char* i_Labels, U32 i_Streaming, U8 i_CanLoad,
    U16 i_Delay, U16 i_DelayMax, U8 i_Priority, U8 i_LoopCount,
    const char* i_StartOffset, F32 i_VolMin, F32 i_VolMax, F32 i_PitchMin,
    F32 i_PitchMax, F32 i_FadeInTime, U8 i_EvictionType, U8 i_SelectType);
...
/*!
    function
    {
        ...
        Name = "AIL_add_cache_sounds_event_step", "Adds a step to an event to
   load a list of sounds in to memory for play."

        In = "HMSSEVENTCONSTRUCT", "i_Event", "The event to add on to."
        In = "const char*", "bankName", "The bank filename containing all of the
   sounds." In = "const char*", "i_Sounds", "A list of colon separated sounds to
   load from the bank file."

        ReturnType = "S32", "Returns 1 on success."

        Discussion = "In general events are not allowed to hit the disk in order
   to prevent unexpected hitching during gameplay. In order to facilitate that,
   sounds need to be preloaded by this event. Each cache step can only load
   sounds from a single bank file, so for multiple bank files, multiple steps
   will be needed.

            In order to release the data loaded by this event,
   AIL_add_uncache_sounds_event_step() needs to be called with the same
   parameters.

            If you are using MilesEvent, the data is refcounted so the sound
   will not be freed until all samples using it complete."
    }
*/
DXDEC
S32 AILCALL AIL_add_cache_sounds_event_step(HMSSEVENTCONSTRUCT i_Event,
                                            const char* bankName,
                                            const char* i_Sounds);
...
/*!
    function
    {
        ...
        Name = "AIL_add_uncache_sounds_event_step", "Adds a step to an event to
   free a list of sounds previously loaded in to memory for play."

        In = "HMSSEVENTCONSTRUCT", "i_Event", "The event to add on to."
        In = "const char*", "bankName", "The bank filename containing all of the
   sounds." In = "const char*", "i_Sounds", "A list of colon separated sounds
   from the bank file to uncache."

        ReturnType = "S32", "Returns 1 on success."

        Discussion = "This event released sounds loaded via
   AIL_add_cache_sounds_event_step()"
    }
*/
DXDEC
S32 AILCALL AIL_add_uncache_sounds_event_step(HMSSEVENTCONSTRUCT i_Event,
                                              const char* bankName,
                                              const char* i_Sounds);
...
EXPTYPEEND
/*
    Determines how the playhead is adjusted during a
   $AIL_add_control_sounds_event_step.
    ...
    $:MILES_CONTROL_STEP_STOP_NO_EVENTS Stop the affected sounds, and prevent
   their completion events from playing.
    $:MILES_CONTROL_STEP_STOP_FADE Stop the sound after fading the sound out
   linearly in loudness.
*/
...
/*!
    function
    {
        ...
        Name = "AIL_add_control_sounds_event_step", "Adds a step to an event to
   control sample playback by label."

        In = "HMSSEVENTCONSTRUCT", "i_Event", "The event to add on to."
        In = "const char*", "i_Labels", "[optional] A comma seperated list of
   labels to control." In = "const char*", "i_MarkerStart", "[optional] If
   exists, sets the loop start to the marker's offset." In = "const char*",
   "i_MarkerEnd", "[optional] If exists, sets the loop end to the marker's
   offset." In = "const char*", "i_Position", "[optional] If exists, sets the
   current playback position to the marker's offset." In = "const char*",
   "i_PresetName", "[optional] The name of the preset to apply, of the form
   Bank/PresetList/PresetName." In = "U8", "i_PresetApplyType", "If nonzero, the
   preset is applied dynamically(the variables are polled every frame)." In =
   "U8", "i_LoopCount", "If the loop count is not to be affected, pass
   MILES_CONTROL_STEP_IGNORELOOP. Otherwise, the sample's loop count will be set
   to this value." In = "U8", "i_Type", "The control type requested. See
   $MILES_CONTROL_STEP_TYPE."

        ReturnType = "S32", "Returns 1 on success."

        Discussion = "Controls playback of current instances. The sounds are
   matched either on name or label. If i_Labels is null, all sounds will be
   controlled.
            "
    }
*/
DXDEC
S32 AILCALL AIL_add_control_sounds_event_step(
    HMSSEVENTCONSTRUCT i_Event, const char* i_Labels, const char* i_MarkerStart,
    const char* i_MarkerEnd, const char* i_Position, const char* i_PresetName,
    U8 i_PresetApplyType, F32 i_FadeOutTime, U8 i_LoopCount, U8 i_Type);
...
/*!
    function
    {
        ...
        Name = "AIL_add_apply_environment_event_step", "Adds a step to an event
   to apply an environment preset."

        In = "HMSSEVENTCONSTRUCT", "i_Event", "The event to add on to."
        In = "const char*", "i_EnvName", "The name of the environment preset to
   apply, of the form EnvList/EnvName." In = "U8", "i_IsDynamic", "If nonzero,
   any variables in the environment are polled every frame."

        ReturnType = "S32", "Returns 1 on success."

        Discussion = "Applies the specified environment preset to the current
   HDIGDRIVER."
    }
*/
DXDEC S32 AILCALL AIL_add_apply_environment_event_step(
    HMSSEVENTCONSTRUCT i_Event, const char* i_EnvName, U8 i_IsDynamic);
...
/*!
    function
    {
        ...
        Name = "AIL_add_comment_event_step", "Adds a step that represents a
   comment to the user of the editing tool."

        In = "HMSSEVENTCONSTRUCT", "i_Event", "The event to add on to."
        In = "const char*", "i_Comment", "A std::string to display in the
   editing tool."

        ReturnType = "S32", "Returns 1 on success."

        Discussion = "This event is ignored in the runtime, and only exist for
   editing convenience."
    }
*/
DXDEC S32 AILCALL AIL_add_comment_event_step(HMSSEVENTCONSTRUCT i_Event,
...
/*
    ...
    $:MILES_RAMPTYPE_VOLUME The ramp will adjust the sample's volume, and will
   interpolate in loudness level. Target is in dB.
    $:MILES_RAMPTYPE_WET The ramp will affect the sample's reverb wet level, and
   will interpolate in loudness. Target is in dB.
    $:MILES_RAMPTYPE_LOWPASS The ramp will affect the sample's low pass cutoff.
   Interpolation and target are in Hz.
    $:MILES_RAMPTYPE_RATE The ramp will affect the sample's playback rate.
   Interpolation and target are in sT.
*/
...
/*
    ...
    $:MILES_INTERP_LINEAR The ramp will lerp between the current value and the
   target.
    $:MILES_INTERP_EXP The ramp will move toward the target slowly at first,
   then faster as it closes on its total time.
    $:MILES_INTERP_SCURVE The ramp will quickly move to about halfway, then
   slowly move, then move more quickly as it ends.
*/

DXDEC EXPAPI S32 AILCALL
AIL_add_ramp_event_step(HMSSEVENTCONSTRUCT i_Event, char const* i_Name,
                        char const* i_Labels, F32 i_Time, char const* i_Target,
                        U8 i_Type, U8 i_ApplyToNew, U8 i_InterpolationType);
/*
    Add an event step that updates or creates a new ramp in the runtime.

    $:i_Event The event to add the step to.
    $:i_Name The name of the ramp. If this name already exists, the ramp will
   shift its target to the new value.
    $:i_Labels The label query determining the sounds the ramp will affect.
    $:i_Time The length the time in seconds the ramp will take to reach its
   target.
    $:i_Target The target value, or a variable expression representing the
   target value. The target's type is dependent on i_Type.
    $:i_Type One of the $MILES_RAMP_TYPE values.
    $:i_ApplyToNew If 1, the ramp will affect sounds that start after the ramp
   is created. If not, it will only affect sounds that are playing when the ramp
   is created. This value can not be changed once the ramp has been created.
    $:i_InterpolationType The method the ramp will affect the target values. One
   of $MILES_INTERP_TYPE values.

    Ramps are means of interpolating aspects of samples. They are removed from
   the system if they are targeted to a value for their type that is a non-op -
   meaning 0 dB, 0 sT, or >24000 Hz.

    Ramps use the current value as the start point for the interpolation. They
   stay at the target point, so you can use the same ramp name to adjust a
   sound's volume down, and later ramp it back up.
*/

DXDEC EXPAPI S32 AILCALL AIL_add_setblend_event_step(
    HMSSEVENTCONSTRUCT i_Event, char const* i_Name, S32 i_SoundCount,
    F32 const* i_InMin, F32 const* i_InMax, F32 const* i_OutMin,
    F32 const* i_OutMax, F32 const* i_MinP, F32 const* i_MaxP);
/*
    ...
    $:i_Name The name of the blend. This is the name that will be
        referenced by the state variable in start sound, as well as the variable
   name to set by the game to update the blend for an instance.
    $:i_SoundCount The number of sounds this blend will affect. Max 10.
    $:i_InMin Array of length i_SoundCount representing the value of the blend
   variable the sound will start to fade in.
    $:i_InMax Array of length i_SoundCount representing the value of the blend
   variable the sound will reach full volume.
    $:i_OutMin Array of length i_SoundCount representing the value of the blend
   variable the sound will start to fade out.
    $:i_OutMax Array of length i_SoundCount representing the value of the blend
   variable the sound will cease to be audible.
    $:i_MinP Array of length i_SoundCount representing the pitch of the sound
   when it starts to fade in.
    $:i_MaxP Array of length i_SoundCount representing the pitch of the sound
   when it has completed fading out.

    This step only sets up the lookup for when a blended sound is actually
   started. When a blended sound plays, every frame it polls its state variable,
   then searches for a blend of the same name. If it finds both, then it uses
   its index in the start sounds list to find its relevant values from the
   blended sound definition.

    Once it has the correct values, it uses them to affect the sample as stated
   in the parameter docs above.
*/
...
/*!
   ...
   maximum number of playing sounds per label."

        In = "HMSSEVENTCONSTRUCT", "i_Event", "The event to add on to."
        In = "const char*", "i_SoundLimits", "A std::string of the form `"label
   count:anotherlabel count`"."

        ReturnType = "S32", "Returns 1 on success."

        Discussion = "Defines limits for instances of sounds on a per label
   basis. Sounds with multiple labels must fit under the limits for all of their
   labels. By default sounds are not limited other than the Miles max sample
   count."
    }
*/
DXDEC S32 AILCALL AIL_add_sound_limit_event_step(HMSSEVENTCONSTRUCT i_Event,
...
/*!
    function
    {
        ...
        Name = "AIL_add_persist_preset_event_step", "Adds a preset that applies
   to current sound instances, and continues to be applied to new sounds as they
   are started." In = "HMSSEVENTCONSTRUCT", "i_Event", "The event to add on to."
        In = "const char*", "i_PresetName", "The name of the preset, of the form
   PresetList/PresetName. See discussion." In = "const char*", "i_PersistName",
   "The name of this persisted preset, for future removal." In = "const char*",
   "i_Labels", "The labels to apply this preset to." In = "U8", "i_IsDynamic",
   "If nonzero, the preset polls its variables every frame."

        ReturnType = "S32", "Returns 1 on success."

        Discussion = "Defines a preset by name that remains in the system,
   testing against all started sounds for label match. If a match occurs, then
   the preset is applied to the new sound, before the preset specified in the
   startsound step itself.

            In order to remove a persisted preset, refer to it by name, but
   leave all other parameters null.

            Example:

            // Persist a preset for players.
            AIL_add_persist_preset_event_step(hEvent, ,
   `"Bank/PlayerEffects/Underwater`", `"Underwater`", `"player`");

            // Remove the above preset.
            AIL_add_persist_preset_event_step(hEvent, 0, `"Underwater`", 0);"
    }
*/
DXDEC S32 AILCALL AIL_add_persist_preset_event_step(HMSSEVENTCONSTRUCT i_Event,
                                                    const char* i_PresetName,
                                                    const char* i_PersistName,
                                                    const char* i_Labels,
                                                    U8 i_IsDynamic);

DXDEC EXPAPI S32 AILCALL AIL_get_event_contents(HMSOUNDBANK bank,
                                                char const* name,
                                                U8 const** event);
/*
    ...
    $:event Returns an output pointer to the event contents. Note that this
   std::string isn't null terminated, and thus shouldn't be checked via strlen,
   etc.
    $:return Returns 0 on fail.

    Normally, event contents are meant to be handled by the Miles high-level
   system via $AIL_enqueue_event, rather than inspected directly.
*/

DXDEC EXPAPI S32 AILCALL
AIL_add_clear_state_event_step(HMSSEVENTCONSTRUCT i_Event);
/*
    Clears all persistent state in the runtime.

    $:i_Event The event to add the step to.

    This removes all state that can stick around after an event in done
   executing. Ramps, Blends, Persisted Preset, etc.
*/

DXDEC EXPAPI S32 AILCALL AIL_add_exec_event_event_step(
    HMSSEVENTCONSTRUCT i_Event, char const* i_EventName);
/*
    ...
    When this step is encountered, the event is enqueued, so it will be executed
   the following frame (currently). It has the same parent event mechanics as a
   completion event, so the QueuedId for a sound started by it will be for the
   event that fired this step.
*/

DXDEC EXPAPI S32 AILCALL AIL_add_enable_limit_event_step(
    HMSSEVENTCONSTRUCT i_Event, char const* i_LimitName);
/*
...
DXDEC EXPAPI S32 AILCALL AIL_add_set_lfo_event_step(
    HMSSEVENTCONSTRUCT i_Event, char const* i_Name, char const* i_Base,
    char const* i_Amp, char const* i_Freq, S32 i_Invert, S32 i_Polarity,
    S32 i_Waveform, S32 i_DutyCycle, S32 i_IsLFO);
/*
    Adds a step to define a variable that oscillates over time.

    $:i_Event The event to add the step to.
        $:i_Name The nane of the variable to oscillate.
        $:i_Base The value to oscillate around, or a variable name to use as the
   base.
        $:i_Amp The maximum value to reach, or a variable name to use as the
   amplitude.
        $:i_Freq The rate at which the oscillation occurs, or a variable name to
   use as the rate. Rate should not exceed game tick rate / 2.
        $:i_Invert Whether the waveform should be inverted.
        $:i_Polarity Bipolar (1) or Unipolar (0) - whether the waveform goes
   around the base or only above it.
        $:i_Waveform Sine wave (0), Triangle (1), Saw (2), or Square(3)
        $:i_DutyCycle Only valid for square, determines what percent of the wave
   is "on". (0-100)
        $:i_IsLFO If zero, Base is the default value to assign the variable when
   the settings are applied, and the rest of the parameters are ignored.
*/

DXDEC EXPAPI S32 AILCALL AIL_add_move_var_event_step(
    HMSSEVENTCONSTRUCT i_Event, char const* i_Name, const F32 i_Times[2],
    const S32 i_InterpolationTypes[2], const F32 i_Values[3]);
/*
    ...
    $:i_Event The event to add the step to.
        ...
        $:i_InterpolationTypes The curve type for the two curves - Curve In (0),
   Curve Out (1), S-Curve (2), Linear (3)
        $:i_Values The initial, midpoint, and final values for the variable.

        The variable is locked to this curve over the timeperiod - no
   interpolation from a previous value is done.

        If an existing move var exists when the new one is added, the old one is
   replaced.
*/
...
//! Represents an immutable std::string that is not null terminated, and
...
/*!
    function
    {
        ...
        Name = "AIL_next_event_step", "Retrieves the next step in the event
   buffer, parsing it in to a provided buffer."

        In = "const U8*", "i_EventString", "The event returned by
   $AIL_close_event, or a previous call to $AIL_next_event_step" Out = "const
   EVENT_STEP_INFO*", "o_Step", "A pointer to the step struct will be stored
   here." In = "void*", "i_Buffer", "A working buffer for the function to use
   for parsing." In = "S32", "i_BufferSize", "The size in bytes of the working
   buffer."

        ReturnType = "U8 char*", "Returns 0 on fail or when the event
   std::string has been exhausted of steps. Otherwise, returns the std::string
   location of the next event step in the buffer."

        Discussion = "This function parses the event std::string in to a struct
   for usage by the user. This function should only be used by the MilesEvent
   system. It returns the pointer to the next step to be passed to this function
   to get the next step. In this manner it can be used in a loop:
            ...
            {
                pStep = 0;
                pCurrentStep = AIL_next_event_step(pCurrentStep, &pStep,
   EventBuffer, 4096); if (pStep == 0)
                {
                    // Error, or an empty event. If $AIL_last_error is an empty
   std::string, then it was an empty event. break;
                }
...
*/
DXDEC const U8* AILCALL AIL_next_event_step(const U8* i_EventString,
                                            struct EVENT_STEP_INFO** o_Step,
                                            void* i_Buffer, S32 i_BufferSize);
...
/*
    ...
    Only used if multiple event systems are running. See the eventmultiple
   example.
*/

DXDEC EXPAPI HEVENTSYSTEM AILCALL
AIL_startup_event_system(HDIGDRIVER dig, S32 command_buf_len,
                         EXPOUT char* memory_buf, S32 memory_len);
/*
   Initializes the Miles Event system and associates it with an open digital
   driver.

   $:dig The digital sound driver that this event system should use.
   $:command_buf_len An optional number of bytes to use for the command buffer.
   If you pass 0, a reasonable default will be used (currently 5K).
   $:memory_buf An optional pointer to a memory buffer buffer that the event
   system will use for all event allocations. Note that the sound data itself is
   not stored in this buffer - it is only for internal buffers, the command
   buffer, and instance data. Use 0 to let Miles to allocate this buffer itself.
   $:memory_len If memory_buf is non-null, then this parameter provides the
   length.  If memory_buf is null, the Miles will allocate this much memory for
   internal buffers. If both memory_buf and memory_len are null, the Miles will
   allocate reasonable default (currently 64K).
   $:return Returns 0 on startup failure.

   This function starts up the Miles Event System, which is used to trigger
   events throughout your game. You call it after $AIL_open_digital_driver.
*/

DXDEC EXPAPI HEVENTSYSTEM AILCALL AIL_add_event_system(HDIGDRIVER dig);
/*
    Creates an additional event system attached to a different driver, in the
   event that you need to trigger events tied to different sound devices.

    $:dig The digital sound driver to attach the new event system to.
    $:return A handle to the event system to use in various high level
   functions.

    Both systems will access the same set of loaded soundbanks, and are updated
   when $AIL_begin_event_queue_processing is called.

    To enqueue events to the new system, use $AIL_enqueue_event_system.

    To iterate the sounds for the new system, pass the $HEVENTSYSTEM as the
   first parameter to $AIL_enumerate_sound_instances.

    To access or set global variables for the new system, pass the $HEVENTSYSTEM
   as the context in the variable access functions.
...
/*
  ...
  This function will closes everything in the event system - it ignores
  reference counts. It will free all event memory, sound banks, and samples used
  by the system.
*/

DXDEC EXPAPI HMSOUNDBANK AILCALL AIL_add_soundbank(char const* filename,
                                                   char const* name);
/*
    Open and add a sound bank for use with the event system.

    $:filename Filename of the bank to load.
    $:name The name of the soundbank to load - this is only used for
   auditioning.
    $:return The handle to the newly loaded soundbank (zero on failure).

    This function opens the sound bank and makes it available to the event
   system.  The filename is the name on the media, and the name is the symbolic
   name you used in the Miles Sound Studio. You might, for example, be using a
   soundbank with a platform extension, like: 'gamebank_ps3.msscmp', and while
   using the name 'gamebank' for authoring and auditioning.

    Sound data is not loaded when this function is called - it is only loaded
   when the relevant Cache Sounds is played, or a sound requiring it plays.

    This function will access the disc, so you will usually call it at level
   load time.

    If you are using the Auditioner, $AIL_audition_startup and
   $AIL_audition_connect must be called prior to this function.
*/
...
/*
   ...
   This function closes a given soundbank. Any data references in the event
   system need to be removed beforehand - with $AIL_enqueue_event_by_name usage
   this should only be pending sounds with completion events.

   Any other data references still existing (queued events, persisted presets,
   etc) will report errors when used, but will not crash.

   Releasing a sound bank does not free any cached sounds loaded from the bank -
   any sounds from the bank should be freed via a Purge Sounds event step. If
   this does not occur, the sound data will still be loaded, but the sound
   metadata will be gone, so Start Sound events will not work. Purge Sounds will
   still work.

   This is different from Miles 8, which would maintain a reference count for
   all data.
*/

DXDEC U8 const* AILCALL AIL_find_event(HMSOUNDBANK bank,
...
/*
    ...
    $:bank The soundbank to search within, or 0 to search all open banks (which
   is the normal case).
    $:event_name The name of the event to find.  This name should be of the form
   "soundbank/event_list/event_name".
    $:return A pointer to the event contents (or 0, if the event isn't found).

    This function is normally used as the event parameter for
   $AIL_enqueue_event. It searches one or all open soundbanks for a particular
   event name.

    <b>This is deprecated</b>. If you know the event name, you should use
   $AIL_enqueue_event_by_name, or $AIL_enqueue_event with
    MILESEVENT_ENQUEUE_BY_NAME.

    Events that are not enqueued by name can not be tracked by the Auditioner.
*/

DXDEC EXPAPI U64 AILCALL AIL_enqueue_event_system(
    HEVENTSYSTEM system, U8 const* event, void* user_buffer,
    S32 user_buffer_len, S32 enqueue_flags, U64 apply_to_ID);
/*
    Enqueue an event to a specific system. Used only if you have multiple event
   systems running.
...
/*
    ...
    This is the most basic way to enqueue an event. It enqueues an event by
   name, and as a result the event will be tracked by the auditioner.

    For when you need more control over the event, but still want it to be
   tracked by the auditioner, it is equivalent to calling
   $AIL_enqueue_event_end_named($AIL_enqueue_event_start(), name)
...
/*
    ...
    This is used to pass more data to an event that will be executed. For
   instance, if an event is going to spatialize a sound, but there's no need to
   move the sound over the course of its lifetime, you can add positional data
   to the event via $AIL_enqueue_event_position. When a sound is started it will
   use that for its initial position, and there is no need to do any game object
   <-> event id tracking.
    ...
    The enqueue process is still completely thread safe. No locks are used,
   however only 8 enqueues can be "assembling" at the same time - if more than
   that occur, the $AIL_enqueue_event_start will yield the thread until a slot
   is open.

    The ONLY time that should happen is if events enqueues are started but never
   ended:
    ...
    ${
        ...
        if (position == 0)
            return; // OOPS! enqueue_token was leaked here, never to be
   reclaimed.

    $}

    Each event has a limit to the amount of data that can be attached to it.
   Currently this amount is 512 bytes - which should cover all use cases. If any
   enqueue functions return 0, then this amount has been reached. The
   ErrorHandler will be called as well, with $AIL_last_error reporting that the
   enqueue buffer was filled.
*/
...
/*
    ...
    Used to handle the case where you decided to not actually enqueue the event
   you've assembled.

    In general it's better to handle anything that can fail before actually
   starting to create the enqueue.
*/

DXDEC EXPAPI S32 AILCALL AIL_enqueue_event_position(S32* token, F32 x, F32 y,
                                                    F32 z);
/*
    ...
    If the event queued starts a sound, the sound's position will be set to the
   given coordinates.

    Setting the position of a sample automatically enables 3D spatialization.
*/

DXDEC EXPAPI S32 AILCALL AIL_enqueue_event_velocity(S32* token, F32 vx, F32 vy,
                                                    F32 vz, F32 mag);
/*
    ...
    If the event queued starts a sound, the sound's velocity will be set to the
   given std::vector.

    Setting the velocity of a sample does NOT automatically enable 3D
   spatialization.
*/

DXDEC EXPAPI S32 AILCALL AIL_enqueue_event_buffer(S32* token, void* user_buffer,
                                                  S32 user_buffer_len,
                                                  S32 user_buffer_is_ptr);
/*
    Attaches a user buffer to the event.

    $:token A token created with $AIL_enqueue_event_start.
    $:user_buffer Pointer to a user buffer to pass with the event. If
   user_buffer_is_ptr is 1, the pointer is copied directly and user_buffer_len
   is ignored.
    $:user_buffer_len The size of the user_buffer to attach to the event.
    $:user_buffer_is_ptr If 1, the pointer is copied and user_buffer_len is
   ignored.
    $:return 0 if the enqueue buffer is full

    User buffers are helpful for bridging the gap between game objects and sound
   objects.

    There are two use cases available in this function

    $* <b>Pointer</b> If user_buffer_is_ptr is 1, then the value passed to
   user_buffer is copied directly as the user buffer contents, and then exposed
   during sound enumeration. This is equivalent in spirit to the void* value
   that often accompanies callbacks. In this case, user_buffer_len is ignored,
   as user_buffer is never dereferenced.
    $* <b>Buffer</b> If user_buffer_is_ptr is 0, then user_buffer_len bytes are
   copied from user_buffer and carried with the event. During sound enumeration
   this buffer is made available, and you never have to worry about memory
   management.
    $-
    ...
    ${
        ...
        // Pointer - the "data" pointer will be copied directly, so we can't
   free() "data" until after the sound
    ...
    ${
        ...
        // Buffer - the "data" structure will be copied internally, so we can
   free() the data - or just use
    ...
    As noted in $AIL_enqueue_event_start(), there's only 512 bytes available to
   an enqueue, so that places an upper limit on the amount of data you can pass
   along. If the data is huge, then you should use user_buffer_is_ptr.
*/
...
/*
    ...
    When a sound starts, the given variable will be set to the given value prior
   to any possible references being used by presets.
*/
...
/*
    ...
    $:token A token created with $AIL_enqueue_event_start
    $:apply_to_ID The ID to use for filtering. This can be either a sound or
   event ID. For an event, it will apply to all sounds started by the event, and
   any events queued by that event.
    $:return 0 if the enqueue buffer is full

    IDs are assigned to events and sounds - for events, it is returned via the
   $AIL_enqueue_event_end_named function (or any other enqueue function). For
   sounds, you can access the assigned id during the enumeration process.
*/

DXDEC EXPAPI S32 AILCALL AIL_enqueue_event_context(S32* token,
                                                   HEVENTSYSTEM system);
/*
...
/*
    Passes in a selection value for start sound events to use for picking
   sounds.
    ...
    a selection, then the selection will replace the "1" with the number passed
   in:
...
DXDEC EXPAPI U64 AILCALL AIL_enqueue_event_end_named(S32 token,
                                                     char const* event_name);
/*
    Completes assembling the event and queues it to the command buffer to be run
   during next tick.

    $:token A token created with $AIL_enqueue_event_start.
    $:event_name The name of the event to run.
    $:return A unique ID for the event that can be used to identify sounds
   started by this event, or for filtering future events to the sounds started
   by this event.

    This function takes all of the data accumulated via the various enqueue
   functions and assembles it in to the command buffer to be run during the next
   $AIL_begin_event_queue_processing.

    As with all of the enqueue functions it is completely thread-safe.

    Upon completion of this function, the enqueue slot is release and available
   for another $AIL_enqueue_event_start.
*/

DXDEC EXPAPI U64 AILCALL AIL_enqueue_event(U8 const* event_or_name,
                                           void* user_buffer,
                                           S32 user_buffer_len,
                                           S32 enqueue_flags, U64 apply_to_ID);
/*
    Enqueue an event to be processed by the next
   $AIL_begin_event_queue_processing function.

    $:event_or_name Pointer to the event contents to queue, or the name of the
   event to find and queue. If an event, the contents must be valid until the
   next call to $AIL_begin_event_queue_processing. If a name, the std::string is
   copied internally and does not have any lifetime requirements, and
   MILES_ENQUEUE_BY_NAME must be present in enqueue_flags.
    $:user_buffer Pointer to a user buffer. Depending on
   $(AIL_enqueue_event::enqueue_flags), this pointer can be saved directly, or
   its contents copied into the sound instance. This data is then accessible
   later, when enumerating the instances.
    $:user_buffer_len Size of the buffer pointed to by user_buffer.
    $:enqueue_flags Optional $MILESEVENTENQUEUEFLAGS logically OR'd together
   that control how to enqueue this event (default is 0).
    $:apply_to_ID Optional value that is used for events that affect sound
   instances. Normally, when Miles triggers one of these event steps, it matches
   the name and labels stored with the event step. However, if you specify an
   apply_to_ID value, then event step will only run on sounds that matches this
   QueuedID,InstanceID,or EventID too.  This is how you execute events only
   specific sound instances.  QueuedIDs are returned from each call
   $AIL_enqueue_event. InstanceIDs and EventIDs are returned from
   $AIL_enumerate_sound_instances.
    $:return  On success, returns QueuedID value that is unique to this queued
   event for the rest of this program run (you can use this ID to uniquely
   identify sounds triggered from this event).

     This function enqueues an event to be triggered - this is how you begin
   execution of an event.  First, you queue it, and then later (usually once a
   game frame), you call $AIL_begin_event_queue_processing to execute an event.

     This function is very lightweight.  It does nothing more than post the
   event and data to a command buffer that gets executed via
   $AIL_begin_event_queue_processing.

    The user_buffer parameter can be used in different ways. If no flags are
   passed in, then Miles will copy the data from user_buffer (user_buffer_len
   bytes long) and store the data with the queued sound - you can then free the
   user_buffer data completely!  This lets Miles keep track of all your sound
   related memory directly and is the normal way to use the system (it is very
    convenient once you get used to it).

    If you instead pass the MILESEVENT_ENQUEUE_BUFFER_PTR flag, then user_buffer
   pointer will simply be associated with each sound that this event may start.
   In this case, user_buffer_len is ignored.

    In both cases, when you later enumerate the sound instances, you can access
   your sound data with the $(MILESEVENTSOUNDINFO::UserBuffer) field.

    You can call this function from any number threads - it's designed to be
   called from anywhere in your game.

    If you want events you queue to be captured by Miles Studio, then they have
   to be passed by name. This can be done by either using the convenience
   function $AIL_enqueue_event_by_name, or by using the
   MILESEVENT_ENQUEUE_BY_NAME flag and passing the name in event_or_name. For
   introduction to the auditioning system, see $integrating_events.
*/
...
EXPTYPEEND
/*
    The available flags to pass in $AIL_enqueue_event or
   $AIL_enqueue_event_system.

    $:MILESEVENT_ENQUEUE_BUFFER_PTR The user_buffer parameter passed in should
   not be duplicated, and instead should just tranparently pass the pointer on
   to the event, so that the $(MILESEVENTSOUNDINFO::UserBuffer) during sound
   iteration is just the same pointer. user_buffer_len is ignored in this case.

    $:MILESEVENT_ENQUEUE_FREE_EVENT The ownership of the memory for the event is
   passed to the event system. If this is present, once the event completes
   $AIL_mem_free_lock will be called on the raw pointer passed in to
   $AIL_enqueue_event or $AIL_enqueue_event_system. This is rarely used.

    $:MILESEVENT_ENQUEUE_BY_NAME The event passed in is actually a std::string.
   The event system will then look for this event in the loaded sound banks
...
/*
    ...
    $:return Return 0 on failure. The only failures are unrecoverable errors in
the queued events (out of memory, bank file not found, bad data, etc).  You can
get the specific error by calling $AIL_last_error.

      This function executes all the events currently in the queue. This is
where all major processing takes place in the event system.

            Once you execute this functions, then sound instances will be in one
of three states:

            $(MILESEVENTSOUNDSTATUS::MILESEVENT_SOUND_STATUS_PENDING)[MILESEVENT_SOUND_STATUS_PENDING]
- these are new sound instances that were created by events that had a "Start
Sound Step". Note that these instances aren't audible yet, so that you have a
chance to modify game driven properties (like the 3D position) on the sound
before Miles begins to play it.

            $(MILESEVENTSOUNDSTATUS::MILESEVENT_SOUND_STATUS_PLAYING)[MILESEVENT_SOUND_STATUS_PLAYING]
- these are sound instances that were previously started and are continuing to
play (you might update the 3D position for these, for example).

            $(MILESEVENTSOUNDSTATUS::MILESEVENT_SOUND_STATUS_COMPLETE)[MILESEVENT_SOUND_STATUS_COMPLETE]
- these are sound instances that finished playing since the last this frame (you
might use this status to free any game related memory, for example).

            You will normally enumerate the active sound instances in-between
calls to $AIL_begin_event_queue_processing and
$AIL_complete_event_queue_processing with $AIL_enumerate_sound_instances.

            $AIL_complete_event_queue_processing must be called after this
function to commit all the changes.

            Example usage:
${
            // enqueue an event
            $AIL_enqueue_event( EventThatStartsSounds, game_data_ptr, 0,
MILESEVENT_ENQUEUE_BUFFER_PTR, 0 );

            // now process that event
            $AIL_begin_event_queue_processing( );

            // next, enumerate the pending and complete sounds for game
processing MILESEVENTSOUNDINFO Info;

            HMSSENUM SoundEnum = MSS_FIRST;
            while ( $AIL_enumerate_sound_instances( &SoundEnum,
MILESEVENT_SOUND_STATUS_PENDING | MILESEVENT_SOUND_STATUS_COMPLETE, 0, &Info ) )
            {
                game_type * game_data = (game_type*) Info.UserBuffer; // returns
the game_data pointer from the enqueue

                if ( Info.Status == MILESEVENT_SOUND_STATUS_PENDING )
                {
                    // setup initial state
                    AIL_set_sample_3D_position( Info.Sample, game_data->x,
game_data->y, game_data->z );
                }
                else if ( Info.Status == MILESEVENT_SOUND_STATUS_COMPLETE )
                {
                    // Free some state we have associated with the sound now
that its done. game_free( game_data );
                }
            }
  ...
  $}
            ...
            Note that if any event step drastically fails, the rest of the
command queue is skipped, and this function returns 0! For this reason, you
shouldn't assume that a start sound event will always result in a completed
sound later.

            Therefore, you should allocate memory that you want associated with
a sound instance during the enumeration loop, rather than at enqueue time.
Otherwise, you need to detect that the sound didn't start and then free the
...
/*
  ...
  $:QueuedID A unique ID that identifies the queued event that started this
  sound. Returned from each call to $AIL_enqueue_event.
  $:EventID A unique ID that identifies the actual event that started this
  sound. This is the same as QueuedID unless the sound was started by a
  completion event or a event exec step. In that case, the QueuedID represents
  the ID returned from $AIL_enqueue_event, and EventID represents the completion
  event.
  $:InstanceID A unique ID that identified this specific sound instance (note
  that one QueuedID can trigger multiple InstanceIDs).
  $:Sample The $HSAMPLE for this playing sound.
  $:Stream The $HSTREAM for this playing sound (if it is being streamed, zero
  otherwise).
  ...
  $:UsedDelay The value actually used as a result of the randomization of delay
  for this instance
  $:UsedVolume The value actually used as a result of the randomization of pitch
  for this instance
  $:UsedPitch The value actually used as a result of the randomization of volume
  for this instance
  $:UsedSound The name of the sound used as a result of randomization. This
  pointer should NOT be deleted and is only valid for the until the next call in
  to Miles.
...
DXDEC EXPAPI void AILCALL AIL_set_variable_int(UINTa context, char const* name,
                                               S32 value);
/*
    ...
    $:context The context the variable is set for. Can be either a $HEVENTSYSTEM
            to set a global variable for a specific system, 0 to set a global
   variable for the default system, or an $HMSSENUM from
   $AIL_enumerate_sound_instances.
    $:name The name of the variable to set.
    $:value The value of the variable to set.

    Variables are tracked per sound instance and globally, and when a variable
   is needed by an event, it will check the relevant sound instance first,
   before falling back to the global variable list:
    ...
    ${
        ...
        // A preset referencing "MyVar" for FirstSound will get 10. Any other
   sound will
        // get 20.
    $}
...
DXDEC EXPAPI S32 AILCALL AIL_variable_int(UINTa context, char const* name,
                                          S32* value);
/*
    ...
    This function follows the same lookup pattern as the runtime - if the
   context is a sound instance, it checks the instance before falling back to
   global variables.
*/

DXDEC EXPAPI S32 AILCALL AIL_variable_float(UINTa context, char const* name,
                                            F32* value);
/*
    Retrieves a named variable.

    $:context The context to start the lookup at, same as
   $AIL_set_variable_float.
    $:name The name to look up.
...
/*
    ...
    Use this function when a disc error causes a slew of failed caches. Any
   sound source that has failed due to asynchronous load will get retried.
*/

DXDEC EXPAPI void AILCALL AIL_set_sound_start_offset(HMSSENUM sound, S32 offset,
                                                     S32 isms);
/*
    Specify the starting position for a pending sound.

    $:sound The enumeration from $AIL_enumerate_sound_instances representing the
   desired sound. The sound must be in the pending state.
    ...
    Use this function instead of manipulating the sample position directly via
   low level Miles calls prior to the sound starting. Generally you don't need
   to do this manually, since the sound designer should do this, however if you
   need to restart a sound that stopped - for example a stream that went to
   error - you will have to set the start position via code.

    However, since there can be a delay between the time the sound is first seen
   in the sound iteration and the time it gets set to the data, start positions
   set via the low level miles calls can get lost, so use this.

    See the <i>eventstreamerror.cpp</i> example program for usage.
*/

DXDEC EXPAPI S32 AILCALL AIL_enumerate_sound_instances(
    HEVENTSYSTEM system, HMSSENUM* next, S32 statuses, char const* label_query,
    U64 search_for_ID, EXPOUT MILESEVENTSOUNDINFO* info);
/*
...
You can pass 0 here, if you just want the first instance that matches.
    $:statuses Or-ed list of status values to enumerate. Use 0 for all status
types.
    $:label_query A query to match sound instance labels against.  Use 0 to skip
label matching.
    $:search_for_ID Match only instances that have a QueuedID,InstanceID,or
EventID that matches this value. Use 0 to skip ID matching.
    $:info Returns the data for each sound instance.
    $:return  Returns 0 when enumeration is complete.

    Enumerates the sound instances. This will generally be used between
      calls to $AIL_begin_event_queue_processing and
$AIL_complete_event_queue_processing to manage the sound instances.

    The label_query is a list of labels to match, separated by commas.  By
default, comma-separated values only have to match at least one label.  So, if
you used "level1, wind", then all sound instances that had either "level1"
<i>or</i> "wind" would match.  If you want to match <i>all</i> labels, then use
the + sign first (for example, "+level1, +wind" would only match sound instances
that had <i>both</i> "level1" and "wind").  You can also use the - sign before a
label to <i>not</i> match that label (so, "level1, -wind" would match all
"level1" labeled sound instances that didn't have a "wind" label).  Finally, you
can also use * and ? to match wildcard style labels (so, "gun*" would match any
sound instance with a label that starts with "gun").
...
); $AIL_set_sample_is_3D( Info.Sample, 1 ); $AIL_set_sample_3D_position(
Info.Sample, game_data->x, game_data->y, game_date->z );
              }
            }
...
/*
  ...
  instances aren't audible yet, so that you have a chance to modify game driven
  properties (like the 3D position) on the sound before Miles begins to play it.

  $:MILESEVENT_SOUND_STATUS_PLAYING Sound instances that were previously
            started and are continuing to play (you might update the 3D position
  for these, for example).

  $:MILESEVENT_SOUND_STATUS_COMPLETE Sound instances that finished playing
            since the last this frame (you might use this status to free any
  game related memory, for example).

  These are the status values that each sound instance can have.  Use
  $AIL_enumerate_sound_instances to retrieve them.
*/
...
/*
  ...
  $:MILESEVENT_SOUND_FLAG_MISSING_SOUND The event system tried to look up the
  sound requested from a Start Sound event and couldn't find anything in the
  loaded banks.
  $:MILESEVENT_SOUND_FLAG_EVICTED The sound was evicted due to a sound instance
  limit being hit. Another sound was selected as being higher priority, and this
  sound was stopped as a result. This can be the result of either a Label Sound
  Limit, or a limit on the sound itself.
  $:MILESEVENT_SOUND_FLAG_WAITING_ASYNC The sound is pending because the data
  for it is currently being loaded. The sound will start when sufficient data
  has been loaded to hopefully avoid a skip.
  $:MILESEVENT_SONUD_FLAG_PENDING_ASYNC The sound has started playing, but the
  data still isn't completely loaded, and it's possible that the sound playback
  will catch up to the read position under poor I/O conditions.
  $:MILESEVENT_SOUND_FLAG_FAILED_HITCH The sound meta data was found, but the
  sound was not in memory, and the Start Sound event was marked as "Must Be
  Cached". To prevent this, either clear the flag in the event, which will cause
  a start delay as the sound data is asynchronously loaded, or specify the sound
  in a Cache Sounds step prior to attempting to start it.
  $:MILESEVENT_SOUND_FLAG_FAILED_ASYNC The sound tried to load and the
  asynchronous I/O operation failed - most likely either the media was removed
  during load, or the file was not found.

  These are the flag values that each sound instance can have.  Use
  $AIL_enumerate_sound_instances to retrieve them. Instances may have more than
  one flag, logically 'or'ed together.
*/

DXDEC EXPAPI S32 AILCALL AIL_complete_event_queue_processing(void);
/*
  Completes the queue processing (which is started with
  $AIL_begin_event_queue_processing ).
  ...
  In $AIL_begin_event_queue_processing, all the new sound instances are queued
  up, but they haven't started playing yet.  Old sound instances that have
  finished playing are still valid - they haven't been freed yet.
  $AIL_complete_event_queue_processing actually starts the sound instances and
  frees the completed ones - it's the 2nd half of the event processing.

  Usually you call $AIL_enumerate_sound_instances before this function to manage
  all the sound instances.
*/

DXDEC EXPAPI U64 AILCALL AIL_stop_sound_instances(char const* label_query,
                                                  U64 apply_to_ID);
/*
    Allows the programmer to manually enqueue a stop sound event into the event
   system.

    $:label_query A query to match sound instance labels against.  Use 0 to skip
   label matching.
    $:apply_to_ID An optional value returned from a previous $AIL_enqueue_event
   or $AIL_enumerate_sound_instances that tells Miles to stop only those
   instances who's QueuedID,InstanceID,or EventID matches this value.
    $:return Returns a non-zero queue ID on success.

    Enqueues an event to stop all sounds matching the specified label query (see
   $AIL_enumerate_sound_instances for a description of the label_query format).

    Usually the programmer should trigger a named event that the sound designed
   can fill out to stop the necessary sounds, however, if a single sound (for
   example associated with an enemy that the player just killed) needs to be
   stopped, this function accomplishes that, and is captured by the auditioner
   for replay.
*/

DXDEC EXPAPI U64 AILCALL AIL_pause_sound_instances(char const* label_query,
                                                   U64 apply_to_ID);
/*
    Allows the programmer to manually enqueue a pause sound event into the event
   system.

    $:label_query A query to match sound instance labels against.  Use 0 to skip
   label matching.
    $:apply_to_ID An optional value returned from a previous $AIL_enqueue_event
   or $AIL_enumerate_sound_instances that tells Miles to pause only those
   instances who's QueuedID,InstanceID,or EventID matches this value.
    $:return Returns a non-zero queue ID on success.

    Enqueues an event to pause all sounds matching the specified label query
   (see $AIL_enumerate_sound_instances for a description of the label_query
   format).

    Usually the programmer should trigger a named event that the sound designed
   can fill out to pause the necessary sounds, however, if a single sound (for
   example associated with an enemy that has been put in to stasis) needs to be
   paused, this function accomplishes that, and is captured by the auditioner
   for replay.
*/

DXDEC EXPAPI U64 AILCALL AIL_resume_sound_instances(char const* label_query,
                                                    U64 apply_to_ID);
/*
    Allows the programmer to manually enqueue a resume sound event into the
   event system.

    $:label_query A query to match sound instance labels against.  Use 0 to skip
   label matching.
    $:apply_to_ID An optional value returned from a previous $AIL_enqueue_event
   or $AIL_enumerate_sound_instances that tells Miles to resume only those
   instances who's QueuedID,InstanceID,or EventID matches this value.
    $:return Returns a non-zero enqueue ID on success.

    Enqueues an event to resume all sounds matching the specified label query
   (see $AIL_enumerate_sound_instances for a description of the label_query
   format).

    Usually the programmer should trigger a named event that the sound designed
   can fill out to resume the necessary sounds, however, if a single sound (for
   example associated with an enemy that has been restored from stasis) needs to
   be resumed, this function accomplishes that, and is captured by the
   auditioner for replay.
*/

DXDEC EXPAPI U64 AILCALL AIL_start_sound_instance(
    HMSOUNDBANK bank, char const* sound, U8 loop_count, S32 should_stream,
    char const* labels, void* user_buffer, S32 user_buffer_len,
    S32 enqueue_flags);
/*
    Allows the programmer to manually enqueue a start sound event into the event
   system.

    $:bank The bank containing the sound to start.
    $:sound The name of the sound file to start, including bank name, e.g.
   "BankName/SoundName"
    $:loop_count The loop count to assign to the sound. 0 for infinite, 1 for
   play once, or just the number of times to loop.
    $:stream Non-zero if the sound playback should stream off the disc.
    $:labels An optional comma-delimited list of labels to assign to the sound
   playback.
    ...
    Usually the programmer should trigger an event that the sound designer has
   specifically create to start the appropriate sounds, but this function gives
   the programmer manual control, if necessary. <b>This function is not captured
   by the auditioner.</b>
*/
...
/*
   ...
   This function will clears the list of all events that you have previously
   enqueued.
*/

DXDEC EXPAPI S32 AILCALL AIL_set_sound_label_limits(HEVENTSYSTEM system,
                                                    char const* sound_limits);
/*
  Sets the maximum number of sounds that matches a particular label.

  $:sound_limits A std::string that defines one or more limits on a label by
  label basis.  The std::string should be of the form "label1name
  label1count:label2name label2count".
  $:return Returns 0 on failure (usually a bad limit std::string).

  Every time an event triggers a sound to be played, the sound limits are
  checked, and, if exceeded, a sound is dropped (based on the settings in the
  event step).

  Usually event limits are set by a sound designer via an event, but this lets
  the programmer override the limits at runtime. Note that this replaces those
  events, it does not supplement.
*/

DXDEC EXPAPI S32 AILCALL AIL_enumerate_preset_persists(
    HEVENTSYSTEM system, HMSSENUM* next, EXPOUT char const** name);
/*
    Enumerates the current persisted presets that active in the system.

    $:system The system to enumerate the persists for, or 0 to use the default
   system.
    $:next Enumeration token - initialize to MSS_FIRST before the first call.
    $:name Pointer to a char* that receives the name of the persist. NOTE
            that this pointer can change frame to frame and should be
   immediately copied to a client-allocated buffer if persistence is desired.
    $:return Returns 0 when enumeration is complete.

    This function lets you enumerate all the persisting presets that are
   currently active in the system.  It is mostly a debugging aid.
*/
...
/*
    ...
    This function is a debugging aid - it can be used to show all of the active
   allocations, active sounds, etc.

    You must delete the pointer returned from this function with
   $AIL_mem_free_lock.
*/
...
/*
  ...
  $:CommandBufferSize The size of the command buffer in bytes. See also the
  $AIL_startup_event_system.
  $:HeapSize The total size of memory used by the event system for management
  structures, and is allocated during startup. This does not include loaded file
  sizes.
  $:HeapRemaining The number of bytes in HeapSize that is remaining.
  $:LoadedSoundCount The number of sounds loaded and ready to play via cache
  event steps.
  $:PlayingSoundCount The number of sounds currently playing via start sound
  event steps.
  $:LoadedBankCount The number of sound banks loaded in the system via cache
  event steps, or AIL_add_soundbank.
  $:PersistCount The number of presets persisted via the persist event step.
  $:SoundBankManagementMemory The number of bytes used for the management of the
  loaded sound banks.
  $:SoundDataMemory The number of bytes used in file sizes - remember this is
  not included in HeapSize. Streaming overhead is not included in this number,
  only fully loaded sounds.

  This structure returns debugging info about the event system. It is used with
  $AIL_event_system_state.
*/
...
MilesBankGetSound(char const* SoundAssetName, char* SoundFileName,
                  MILESBANKSOUNDINFO* o_SoundInfo);
/*
  callback to return whether the sound asset is in the bank, and, if so, what
  the final data filename is.

  In order to externally deploy sound files, you will need to register your own
  GetSound callback. This is detailed in the eventexternal example program.

  This returns the len of the buffer required for the output file name if
  SoundFileName is zero.
*/
...
DXDEC EXPAPI void AILCALL AIL_set_event_sample_functions(
    HSAMPLE (*CreateSampleCallback)(char const* SoundName,
                                    char const* SoundFileName, HDIGDRIVER dig,
                                    void* UserBuffer, S32 UserBufferLen),
    void (*ReleaseSampleCallback)(HSAMPLE));
/*
    Allows you to manage sound data availability and sample handles.

    $:CreateSampleCallback Function that will be called when a sample handle is
   needed.
    $:ReleaseSampleCallback Function that will be called when a sample is no
   longer needed.

    A created sample is required to have all data pointers necessary to play -
   e.g. the event system needs to be able to just do a AIL_start_sample() on the
   returned handle and have it work.

    In the callback, SoundName is the name of the asset in Miles Studio, and
   SoundFileName is the value returned from Container_GetSound() (see also
   $AIL_set_event_bank_functions).

*/

DXDEC EXPAPI void AILCALL
AIL_set_event_bank_functions(MILESBANKFUNCTIONS const* Functions);
/*
  ...
  This function is used to completely override the high-level resource
  management system. It's not for overriding the IO - it's when you need much
  higher-level of control. Primarily targeted internally for the Auditioner to
  use, it also is used when deploying sound files externally.
*/
...
typedef void AILCALLBACK AuditionSetPersist(U64 i_EventId, char const* i_Name,
                                            char const* i_Preset);
typedef void AILCALLBACK AuditionEvent(char const* i_EventName, U64 i_EventId,
                                       U64 i_Filter, S32 i_Exists,
                                       void* i_InitBlock, S32 i_InitBlockLen);
typedef void AILCALLBACK AuditionSound(U64 i_EventId, U64 i_SoundId,
                                       char const* i_Sound,
                                       char const* i_Labels, float i_Volume,
                                       S32 i_Delay, float i_Pitch);
...
typedef void AILCALLBACK AuditionSoundLimited(U64 i_SoundId,
                                              char const* i_Label);
typedef void AILCALLBACK AuditionSoundEvicted(U64 i_SoundId, U64 i_ForSound,
                                              S32 i_Reason);
typedef void AILCALLBACK AuditionControl(U64 i_EventId, char const* i_Labels,
                                         U8 i_ControlType, U64 i_Filter);
...
typedef void AILCALLBACK AuditionAsyncQueued(U64 i_RelevantId, S32 i_AsyncId,
                                             char const* i_Asset);
...
typedef void AILCALLBACK AuditionSoundPosition(U64 i_Sound, float x, float y,
                                               float z);
...
typedef void AILCALLBACK AuditionVarState(char const* i_Var, U64 i_SoundId,
                                          S32 i_Int, void* i_4ByteValue);
typedef void AILCALLBACK AuditionRampState(char const* i_Ramp, U64 i_SoundId,
                                           S32 i_Type, float i_Current);
typedef void AILCALLBACK AuditionSoundState(U64 i_SoundId, float i_FinalVol,
                                            float i_3DVol, float i_BlendVol,
                                            float i_BlendPitch, float i_RampVol,
                                            float i_RampWet, float i_RampLp,
                                            float i_RampRate);

typedef void AILCALLBACK AuditionClearState();
typedef void AILCALLBACK AuditionCompletionEvent(U64 i_CompletionEventId,
                                                 U64 i_ParentSoundId);
typedef void AILCALLBACK AuditionAddRamp(U64 i_ParentSoundId, S32 i_Type,
                                         char const* i_Name,
                                         char const* i_Query, U64 i_EventId);
...
DXDEC void AILCALL
MilesEventSetAuditionFunctions(MILESAUDITIONFUNCTIONS const* i_Functions);
...
/*
    ...
    $:MILES_CONNECTED The Auditioner connected and successfully executed the
   handshake.
    $:MILES_CONNECT_FAILED The Auditioner couldn't connect - either the IP
   wasn't valid, or Miles Sound Studio wasn't accepting connections.
    $:MILES_HOST_NOT_FOUND The given host name could not be resolved to an IP.
    $:MILES_SERVER_ERROR We connected, but the server was either another app on
   the same port, or the server version was incorrect.
*/
...
/*
    ...
    $:i_Address The IP or host name of the computer running Miles Sound Studio.
   Use $AIL_audition_local_host to connect to the same machine as the runtime.
    $:return One of $MILESAUDITIONCONNECTRESULT

    The is a synchronous connection attempt to Miles Sound Studio - it will not
   return until it is happy with the connection and the server, or a failure
   occurs.
...
DXDEC EXPAPI S32 AILCALL AIL_audition_startup(S32 i_ProfileOnly, S32 i_Language,
                                              S32 i_Platform);
/*
    Binds the Auditioner to the Miles Event Runtime.

    $:i_ProfileOnly Specify 0 to use assets from the connected Miles Sound
   Studio, and 1 to use assets from disc.
    $:i_Language One of $MILESAUDITIONLANG, or zero to use Default assets. See
   comments below.
    $:i_Platform One of $MILESAUDITIONPLAT, or zero to use the current platform.
   See comments below.

    The Auditioner can run in one of two modes - the first is standard mode,
   where all assets are loaded from the server, and profiling data is sent back
   to the server. The second is Profiling mode, where the assets are loaded
   exactly as they would be under normal execution, but all of the profiling
   data is sent to the server.

    The $(AIL_audition_startup::i_Language) and the
   $(AIL_audition_startup::i_Platform) are used to determine what assets Miles
   Sound Studio sends the Auditioner, and as a result are not used in Profiling
   Mode. Otherwise these are equivalent to the options selected for compiling
   banks.
...
DXDEC EXPAPI void AILCALL AIL_event_system_state(HEVENTSYSTEM system,
                                                 MILESEVENTSTATE* state);
/*
  Returns an information structure about the current state of the Miles Event
  System.

  $:system The system to retrieve information for, or zero for the default
  system.
  $:state A pointer to a structure to receive the state information.

  This function is a debugging aid - it returns information for the event
  system.
*/
...
/*
    Returns the length of the first sound referenced in the named event, in
   milliseconds.

    $:i_EventName The name of an event that starts a sound.
    $:return The length in milliseconds, or 0 if there is an error, or the event
   has no sound references, or the sound was not found.

    This looks up the given event and searches for the first Start Sound event
   step, then uses the first sound name in its list to look up the length. As
   such, if the start sound step has multiple sounds, the rest will be ignored.
*/

// Callback for the error handler.
EXPAPI typedef void AILCALLBACK AILEVENTERRORCB(S64 i_RelevantId,
                                                char const* i_Resource);
/*
  The function prototype to use for a callback that will be made when the event
  system encounters an unrecoverable error.

  $:i_RelevantId The ID of the asset that encountered the error, as best known.
  EventID or SoundID.
  $:i_Resource A std::string representing the name of the resource the error is
  in regards to, or 0 if unknown.
...
/*
  ...
  You can define a function with this prototype and pass it to
  $AIL_register_random if you want to tie the Miles random calls in with your
  game's (for logging and such).
*/

DXDEC EXPAPI void AILCALL
AIL_set_event_error_callback(AILEVENTERRORCB* i_ErrorCallback);
/*
    Set the error handler for the event system.

    $:i_ErrorHandler The function to call when an error is encountered.

    Generally the event system handles errors gracefully - the only noticeable
   effect is that a given sound won't play, or a preset doesn't get set. As a
   result, the errors can sometimes be somewhat invisible. This function allows
   you to see what went wrong, when it went wrong.

    The basic usage is to have the callback check $AIL_last_error() for the
   overall category of failure. The parameter passed to the callback might
   provide some context, but it can and will be zero on occasion. Generally it
   will represent the resource std::string that is being worked on when the
   error occurred.

    Note that there are two out of memory errors - one is the event system ran
   out of memory - meaning the value passed in to $AIL_startup_event_system was
   insufficient for the current load, and the other is the memory used for sound
   data - allocated via $AIL_mem_alloc_lock - ran out.
*/
...
/*
  ...
  Use this function to set your own random function that the Miles Event System
  will call when it needs a random number. This lets you control the determinism
  of the event system.
*/
...
typedef HDRIVERSTATE(AILCALL* FLT_OPEN_DRIVER)(MSS_ALLOC_TYPE* palloc,
                                               MSS_FREE_TYPE* pfree, UINTa user,
...
typedef S32(AILCALL* FLT_POSTMIX_PROCESS)(HDRIVERSTATE driver,
                                          void* output_buffer);
...
typedef HSAMPLESTATE(AILCALL* FLTSMP_OPEN_SAMPLE)(HDRIVERSTATE driver,
                                                  HSAMPLE S, void* memory);

typedef FLTRESULT(AILCALL* FLTSMP_CLOSE_SAMPLE)(HSAMPLESTATE state);

typedef void(AILCALL* FLTSMP_SAMPLE_PROCESS)(
    HSAMPLESTATE state, void* source_buffer,
    void* dest_buffer,  // may be the same as src
    S32 n_samples, S32 is_stereo);
...
typedef S32(AILCALL* VFLT_ASSIGN_SAMPLE_VOICE)(HDRIVERSTATE driver, HSAMPLE S);

typedef void(AILCALL* VFLT_RELEASE_SAMPLE_VOICE)(HDRIVERSTATE driver,
                                                 HSAMPLE S);

typedef S32(AILCALL* VFLT_START_SAMPLE_VOICE)(HDRIVERSTATE driver, HSAMPLE S);
...
typedef S32(AILCALL* VSMP_SAMPLE_PROPERTY)(HSAMPLE S, HPROPERTY property,
                                           void* before_value,
                                           void const* new_value,
                                           void* after_value);
...
//
// Values for "Flags" property exported by all MSS Pipeline Filter and MSS
// Output Filter providers
//

#define FPROV_ON_SAMPLES \
    0x0001  // Pipeline filter that operates on input samples (and is enumerated
            // by AIL_enumerate_filters)
#define FPROV_ON_POSTMIX \
    0x0002  // Pipeline filter that operates on the post mixed output (capture
            // filter)
#define FPROV_MATRIX 0x0004  // This is a matrix output filter (e.g., SRS/Dolby)
#define FPROV_VOICE \
    0x0008  // This is a per-voice output filter (e.g., DirectSound 3D)
#define FPROV_3D 0x0010  // Output filter uses S3D substructure for positioning
#define FPROV_OCCLUSION \
    0x0020  // Output filter supports occlusion (doesn't need per-sample
            // lowpass)
#define FPROV_EAX \
    0x0040  // Output filter supports EAX-compatible environmental reverb
#define FPROV_SIDECHAIN \
    0x0080  // Filter has an "Input" property on the 3rd index for side
            // chaining.
...
typedef struct EAX_REVERB {
    ...
    F32 ReverbDelay;        // late reverberation delay time relative to initial
                            // reflection
...
typedef struct EAX_DISTORTION {
    ...
    F32 LowPassCutOff;  // Controls the cut-off of the filter pre-distortion
                        // (Hz)
    F32 EQCenter;     // Controls the center frequency of the EQ post-distortion
                      // (Hz)
...
typedef struct EAX_ECHO {
    ...
    F32 LRDelay;  // Controls the delay time between the first and second taps
                  // (seconds)
...
typedef struct EAX_VMORPHER {
    ...
    S32 Volume;    // -10000 to 0
    U32 PhonemeA;  // phoneme: 0 to 29 - A E I O U AA AE AH AO EH ER IH IY UH UW
                   // B D G J K L M N P R S T V Z
    S32 PhonemeACoarseTuning;  // (semitones)
    U32 PhonemeB;  // phoneme: 0 to 29 - A E I O U AA AE AH AO EH ER IH IY UH UW
                   // B D G J K L M N P R S T V Z
    S32 PhonemeBCoarseTuning;  // (semitones)
    U32 Waveform;  // Waveform selector - 0 = sinusoid, 1 = triangle, 2 =
                   // sawtooth
    F32 Rate;      // (Hz)
} EAX_VMORPHER;
