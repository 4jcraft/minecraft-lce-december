...
uint8_t* mallocAndCreateUTF8ArrayFromString(int iID) {
    ...
    uint32_t utf16Len;
    uint32_t utf8Len;
    result = sceCesUtf16StrGetUtf8Len(&context, (uint16_t*)wchString, iLen,
                                      &utf16Len, &utf8Len);
    ...
    result = sceCesUtf16StrToUtf8Str(&context, (uint16_t*)wchString, iLen,
                                     &utf16Len, strUtf8, utf8Len, &utf8Len);
    if (result != SCE_OK) {
