...
/*E The FIOS2 default maximum path is 1024, games can normally use a much
 * smaller value. */

/*E Buffers for FIOS2 initialization.
 * These are typical values that a game might use, but adjust them as needed.
 * They are of type int64_t to avoid alignment issues. */

/* 64 ops: */
int64_t g_OpStorage[SCE_FIOS_OP_STORAGE_SIZE(64, MAX_PATH_LENGTH) /
                        sizeof(int64_t) +
                    1];
/* 1024 chunks, 64KiB: */
int64_t g_ChunkStorage[SCE_FIOS_CHUNK_STORAGE_SIZE(1024) / sizeof(int64_t) + 1];
/* 16 file handles: */
int64_t g_FHStorage[SCE_FIOS_FH_STORAGE_SIZE(16, MAX_PATH_LENGTH) /
                        sizeof(int64_t) +
                    1];
/* 4 directory handles: */
int64_t
    g_DHStorage[SCE_FIOS_DH_STORAGE_SIZE(4, MAX_PATH_LENGTH) / sizeof(int64_t) +
                1];
...
char* getConsoleHomePath() { return contentInfoPath; }

char* getUsrDirRoot() { return driveRoot; }

char* getUsrDirPath() { return usrdirPath; }

char* getConsoleHomePathBDPatch() { return contentInfoPathBDPatch; }

char* getUsrDirPathBDPatch() { return usrdirPathBDPatch; }
...
VOID GetSystemTime(LPSYSTEMTIME lpSystemTime) {
    ...
    lpSystemTime->wDayOfWeek = sceRtcGetDayOfWeek(
...
VOID Sleep(DWORD dwMilliseconds) { C4JThread::Sleep(dwMilliseconds); }
...
LONG InterlockedCompareExchangeRelease(LONG volatile* Destination,
    ...
    return sceAtomicCompareAndSwap32((int32_t*)Destination, (int32_t)Comperand,
                                     (int32_t)Exchange);
}

LONG64 InterlockedCompareExchangeRelease64(LONG64 volatile* Destination,
                                           LONG64 Exchange, LONG64 Comperand) {
    return sceAtomicCompareAndSwap64((int64_t*)Destination, (int64_t)Comperand,
                                     (int64_t)Exchange);
}
...
VOID InitializeCriticalSection(PCRITICAL_SECTION CriticalSection) {
    ...
    int err = sceKernelCreateLwMutex(
        (SceKernelLwMutexWork*)(&CriticalSection->mutex), name,
        SCE_KERNEL_LW_MUTEX_ATTR_TH_PRIO | SCE_KERNEL_LW_MUTEX_ATTR_RECURSIVE,
        0, NULL);
...
VOID DeleteCriticalSection(PCRITICAL_SECTION CriticalSection) {
    int err = sceKernelDeleteLwMutex(
        (SceKernelLwMutexWork*)(&CriticalSection->mutex));
    PSVITA_ASSERT_SCE_ERROR(err);
}
...
VOID EnterCriticalSection(PCRITICAL_SECTION CriticalSection) {
    int err = sceKernelLockLwMutex(
        (SceKernelLwMutexWork*)(&CriticalSection->mutex), 1, NULL);
    PSVITA_ASSERT_SCE_ERROR(err);
}

VOID LeaveCriticalSection(PCRITICAL_SECTION CriticalSection) {
    int err = sceKernelUnlockLwMutex(
        (SceKernelLwMutexWork*)(&CriticalSection->mutex), 1);
    PSVITA_ASSERT_SCE_ERROR(err);
}

ULONG TryEnterCriticalSection(PCRITICAL_SECTION CriticalSection) {
    int err = sceKernelTryLockLwMutex(
        (SceKernelLwMutexWork*)(&CriticalSection->mutex), 1);
    if (err == SCE_OK) return true;
    return false;
}
...
VOID InitializeCriticalRWSection(PCRITICAL_RW_SECTION CriticalSection) {
    ...
    CriticalSection->RWLock = sceKernelCreateRWLock(
        name,
        SCE_KERNEL_RW_LOCK_ATTR_TH_PRIO | SCE_KERNEL_RW_LOCK_ATTR_RECURSIVE,
        NULL);
}
...
BOOL TlsFree(DWORD dwTlsIndex) {
    return PSVitaTLSStorage::Instance()->Free(dwTlsIndex);
}
LPVOID TlsGetValue(DWORD dwTlsIndex) {
    return PSVitaTLSStorage::Instance()->GetValue(dwTlsIndex);
}
BOOL TlsSetValue(DWORD dwTlsIndex, LPVOID lpTlsValue) {
    return PSVitaTLSStorage::Instance()->SetValue(dwTlsIndex, lpTlsValue);
}

// AP - all this virtual stuff has been added because Vita doesn't have a
// virtual memory system so we allocate 1MB real memory chunks instead and
// access memory reads and writes via VirtualCopyTo and VirtualCopyFrom which
// divides memcpys across multiple 1MB chunks if required
...
VOID VirtualMove(LPVOID lpDestOffset, LPVOID lpSrcOffset, SIZE_T dwSize) {
    int DestOffset =
        ((int)(lpDestOffset)-VIRTUAL_OFFSET);  // convert the pointer back into
                                               // a virtual offset
    int DestChunkOffset =
        DestOffset % VIRTUAL_PAGE_SIZE;  // what is the uint8_t offset within
                                         // the current 1MB page
    int DestPage =
        DestOffset / VIRTUAL_PAGE_SIZE;  // which 1MB page do we start on
...
BOOL VirtualWriteFile(LPCSTR lpFileName, LPCVOID lpBuffer,
    ...
    *lpNumberOfBytesWritten = 0;
    ...
    while (nNumberOfBytesToWrite) {
        ...
        WriteFileWithName(lpFileName, Data, BytesToWrite, &numberOfBytesWritten,
                          NULL);
...
// The data is mostly zlib compressed. Only blank areas are not so this will RLE
// just the zeros Yields about 2:1 compression
VOID VirtualCompress(LPVOID lpDest, LPDWORD lpNewSize, LPVOID lpAddress,
                     SIZE_T dwSize) {
        ...
        if (Data[Offset]) {
            ...
            if (!CountingZeros || CountingZeros == 255) {
                ...
                if (pDest) {
                    ...
                    pDest[NewSize + 1] =
                        0;  // this is how many zeros we counted
                }
                NewSize += 2;
            }
...
// The data in is mostly zlib compressed. Only blank areas are not so this will
// RLE just the zeros
...
BOOL WriteFileWithName(LPCSTR lpFileName, LPCVOID lpBuffer,
                       DWORD nNumberOfBytesToWrite,
                       LPDWORD lpNumberOfBytesWritten,
                       LPOVERLAPPED lpOverlapped) {
    char filePath[256];
    sprintf(filePath, "%s/%s", getUsrDirPath(), lpFileName);
    SceFiosSize bytesWritten = sceFiosFileWriteSync(NULL, filePath, lpBuffer,
...
BOOL ReadFile(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead,
    ...
    // sceFiosFHReadSync - Non-negative values are the number of bytes read, 0
    // <= result <= length. Negative values are error codes.
    SceFiosSize bytesRead = sceFiosFHReadSync(
        NULL, fh, lpBuffer, (SceFiosSize)nNumberOfBytesToRead);
    if (bytesRead < 0) {
        // error
        return FALSE;
    } else {
...
BOOL SetFilePointer(HANDLE hFile, LONG lDistanceToMove,
    ...
    switch (dwMoveMethod) {
        case FILE_BEGIN:
            whence = SCE_FIOS_SEEK_SET;
            break;
        case FILE_CURRENT:
            whence = SCE_FIOS_SEEK_CUR;
            break;
        case FILE_END:
            whence = SCE_FIOS_SEEK_END;
            break;
    };
...
DWORD GetFileAttributesA(LPCSTR lpFileName) {
    ...
    if (mountedPath.length() > 0) {
        strcpy(filePath, mountedPath.c_str());
    } else if (strstr(lpFileName, ":") !=
               0)  // colon in the filename, so it's fully qualified
        strcpy(filePath, lpFileName);
    else
        sprintf(filePath, "%s/%s", getUsrDirPath(), lpFileName);
...
DWORD GetTickCount() {
    ...
    // The system time is represented the time elapsed since the system starts
    // up in microseconds.
...
// we should really use libperf for this kind of thing, but this will do for
// now.
...
#ifndef _FINAL_BUILD
VOID OutputDebugStringW(LPCWSTR lpOutputString) { wprintf(lpOutputString); }

VOID OutputDebugString(LPCSTR lpOutputString) { printf(lpOutputString); }

VOID OutputDebugStringA(LPCSTR lpOutputString) { printf(lpOutputString); }
#endif  // _CONTENT_PACKAGE

BOOL GetFileAttributesExA(LPCSTR lpFileName,
...
int _wtoi(const wchar_t* _Str) { return wcstol(_Str, NULL, 10); }
...
DWORD XGetLocale() {
    ...
    switch (iLang) {
        case SCE_SYSTEM_PARAM_LANG_JAPANESE:
            return XC_LOCALE_JAPAN;
        case SCE_SYSTEM_PARAM_LANG_ENGLISH_US:
            return XC_LOCALE_UNITED_STATES;
        case SCE_SYSTEM_PARAM_LANG_FRENCH:
            return XC_LOCALE_FRANCE;
        ...
        case SCE_SYSTEM_PARAM_LANG_GERMAN:
            return XC_LOCALE_GERMANY;
        case SCE_SYSTEM_PARAM_LANG_ITALIAN:
            return XC_LOCALE_ITALY;
        case SCE_SYSTEM_PARAM_LANG_PORTUGUESE_PT:
            return XC_LOCALE_PORTUGAL;

        case SCE_SYSTEM_PARAM_LANG_RUSSIAN:
            return XC_LOCALE_RUSSIAN_FEDERATION;
        case SCE_SYSTEM_PARAM_LANG_KOREAN:
            return XC_LOCALE_KOREA;
        case SCE_SYSTEM_PARAM_LANG_CHINESE_T:
            return XC_LOCALE_CHINA;
        case SCE_SYSTEM_PARAM_LANG_PORTUGUESE_BR:
        ...
        case SCE_SYSTEM_PARAM_LANG_DUTCH:
            return XC_LOCALE_NETHERLANDS;
        case SCE_SYSTEM_PARAM_LANG_FINNISH:
            return XC_LOCALE_FINLAND;
        case SCE_SYSTEM_PARAM_LANG_SWEDISH:
            return XC_LOCALE_SWEDEN;
        case SCE_SYSTEM_PARAM_LANG_DANISH:
            return XC_LOCALE_DENMARK;
        case SCE_SYSTEM_PARAM_LANG_NORWEGIAN:
            return XC_LOCALE_NORWAY;
        case SCE_SYSTEM_PARAM_LANG_POLISH:
            return XC_LOCALE_POLAND;
        case SCE_SYSTEM_PARAM_LANG_TURKISH:
            return XC_LOCALE_TURKEY;
