...
// Initialises the shutdown manager - this needs to be called as soon as the
// game is started so it can respond as quickly as possible to shut down
// requests
void ShutdownManager::Initialise() {
    ...
    // Special case for storage manager, which we will manually set now to be
    // considered as running - this will be unset by StorageManager.ExitRequest
    // if required
...
// Called in response to a system request to exit the game. This just requests
// that the main thread should stop, and then the main thread is responsible for
// calling MainThreadHandleShutdown which starts the rest of the shut down
// process, then waits until it is complete.
...
// This should be called from the main thread after it has been requested to
// shut down (ShouldRun for main thread returns false), and before it returns
// control to the kernel. This is responsible for signalling to all other
// threads to stop, and wait until their completion before returning.
void ShutdownManager::MainThreadHandleShutdown() {
    ...
    app.DebugPrintf(
        "Shutdown manager: waiting on first batch of threads requested to "
        "terminate...\n");
    ...
    app.DebugPrintf(
        "Shutdown manager: waiting on storage manager to terminate...\n");
    WaitForSignalledToComplete();
    app.DebugPrintf("Shutdown manager: terminated.\n");
    ...
    // Finally shut down the spurs job queue - leaving until last so there
    // should be nothing else dependent on this still running
...
#ifdef __PS3__
void ShutdownManager::SysUtilCallback(uint64_t status, uint64_t param,
                                      void* userdata) {
