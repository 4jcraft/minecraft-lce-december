...
SCustomMapNode* CustomMap::find(const ChunkPos& Key) {
    unsigned int Hash = (Key.x & 0x00000001f) |
                        (Key.z << 5);  // hopefully this will produce a good
                                       // hash for a 1024 entry table
...
int CustomMap::end() { return m_NodePoolIndex; }

SCustomMapNode* CustomMap::get(int index) { return m_NodePool[index]; }
...
void CustomMap::resize() {
    ...
    if (m_NodePool) {
        NodePool = (SCustomMapNode**)realloc(
            m_NodePool, m_NodePoolSize * sizeof(SCustomMapNode));
    } else {
        NodePool =
            (SCustomMapNode**)malloc(m_NodePoolSize * sizeof(SCustomMapNode));
    }

    for (int i = 0; i < m_NodePoolSize - OldPoolSize; i += 1) {
        NodePool[i + OldPoolSize] =
            (SCustomMapNode*)malloc(sizeof(SCustomMapNode));
    }
