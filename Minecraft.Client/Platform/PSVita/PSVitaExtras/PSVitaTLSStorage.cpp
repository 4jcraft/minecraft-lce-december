...
PSVitaTLSStorage* PSVitaTLSStorage::Instance() { return &Singleton; }
...
void PSVitaTLSStorage::RemoveThread(int threadID) {
    ...
    std::vector<TLSArray*>::iterator iter =
        PSVitaTLSStorage_ActiveInfos.begin();
...
// AP - Oh my. It seems __thread doesn't like cpp files. I couldn't get it to be
// thread sensitive as it would always return a shared value. For now I've stuck
// m_values and accessor functions into user_malloc.c. Cheap cheap hack.
extern "C" {
...
BOOL PSVitaTLSStorage::Free(DWORD _index) {
    if (m_activeList[_index] == false) return false;  // not been allocated
...
BOOL PSVitaTLSStorage::SetValue(DWORD _index, LPVOID _val) {
    if (m_activeList[_index] == false) return false;
...
LPVOID PSVitaTLSStorage::GetValue(DWORD _index) {
    if (m_activeList[_index] == false) return NULL;
...
// m_values and accessor functions into user_malloc.c. Cheap cheap hack.
__thread void* m_values[64];
void user_setValue(unsigned int _index, void* _val) { m_values[_index] = _val; }
void* user_getValue(unsigned int _index) { return m_values[_index]; }
#endif