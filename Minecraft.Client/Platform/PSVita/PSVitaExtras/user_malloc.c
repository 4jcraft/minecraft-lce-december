...
// It was found from profiling that threads were stalling far too much waiting
// for their turn to malloc/free This manager will keep memory bound to a thread
// once allocated so it can be reused by the same thread later. It's only
// interested in chunks up to 1036 bytes in size, anything greater is not
// retained Todo : I may need to put in a memory flusher if I find threads
// holding onto memory too much.
...
typedef struct {
    Block** Malloc_MemoryPool;  // this is an array of available memory
                                // allocations up to 1036 bytes in size
    Block* Malloc_Blocks;  // this is a stack of available block nodes used to
                           // store memory chunks
    Block* Malloc_BlocksMemory
        [Malloc_BlocksMemorySize];  // this is a pool of block nodes allocated
                                    // in large chunks (256 blocks at a time)
    int Malloc_BlocksAlloced;  // this shows how many block node chunks have
                               // been allocated in Malloc_BlocksMemory
} SThreadStorage;
...
void user_malloc_init(void) {
    ...
    s_heapUid = sceKernelAllocMemBlock("UserAllocator",
                                       SCE_KERNEL_MEMBLOCK_TYPE_USER_RWDATA,
                                       HEAP_SIZE, SCE_NULL);
    if (s_heapUid < SCE_OK) {
        /**E Error handling */
        /**J エラー処理 */
...
// before a thread is destroyed make sure we free any space it might be holding
// on to
...
void* user_malloc(size_t size) {
    ...
    if (psStorage) {
        ...
        if (psStorage->Malloc_MemoryPool == NULL) {
            // create an array of pointers to Block nodes, one pointer for each
            // memory bytes size up to 1036
            psStorage->Malloc_MemoryPool =
                mspace_malloc(s_mspace, (MaxRetainedBytes + 1) * 4);
            for (int i = 0; i < (MaxRetainedBytes + 1); i += 1) {
                psStorage->Malloc_MemoryPool[i] = NULL;
            }
        }

        // are we interested in retaining this size of memory (less 4 bytes to
        // store the associated thread ID)
        if (size < (MaxRetainedBytes - 4)) {
            // add on space for the thread ID
            size += 4;

            // round to the nearest malloc boundary. This is what happens
            // internally in the malloc library
            if (size <= 12)
                size = 12;
            else
            ...
            if (psStorage->Malloc_MemoryPool[size]) {
                ...
                unsigned int* ThreadAddr =
                    (unsigned int*)((unsigned char*)p + (size - 4));
...
void user_free(void* ptr) {
    ...
    if (psStorage) {
        ...
        if (RoundedSize < (MaxRetainedBytes + 1)) {
            ...
            unsigned int* ThreadAddr =
                (unsigned int*)((unsigned char*)ptr + (RoundedSize - 4));

            // did this thread create this memory
            if (sceKernelGetThreadId() != ThreadAddr[0]) {
                // don't worry about retaining memory allocated from a different
                // thread. too much mucking about
                mspace_free(s_mspace, ptr);
                return;
            }

            // we need a block node to retain the memory on our stack. do we
            // have any block nodes available
            if (psStorage->Malloc_Blocks == NULL) {
                if (psStorage->Malloc_BlocksAlloced ==
                    Malloc_BlocksMemorySize) {
                    // Max blocks allocated
                    // The worst case for this running out is when the player
                    // has explored a large amount of the map in a single
                    // session and then quits. All the allocations are suddenly
                    // freed up so we need lots of space to store them.
                    printf(
                        "oh nos, max blocks allocated. increase "
                        "Malloc_BlocksMemorySize");
                    assert(0);
                }
                ...
                int chunkSize = 256;
                psStorage
                    ->Malloc_BlocksMemory[psStorage->Malloc_BlocksAlloced] =
                    mspace_malloc(s_mspace, chunkSize * sizeof(Block));

                // push all the new blocks onto the pool
                for (int i = 0; i < chunkSize; i += 1) {
                    Block* NewBlock = &psStorage->Malloc_BlocksMemory
                                           [psStorage->Malloc_BlocksAlloced][i];
            ...
            // set up the block data and retain it to the correct slot. We can
            // now reuse this memory on the next malloc
            NewBlock->Memory = ptr;
