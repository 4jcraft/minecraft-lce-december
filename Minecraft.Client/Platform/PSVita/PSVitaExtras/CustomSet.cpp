...
int CustomSet::end() { return m_NodePoolIndex; }

ChunkPos CustomSet::get(int index) { return m_NodePool[index]->key; }
...
void CustomSet::resize() {
    ...
    if (m_NodePool) {
        NodePool = (SCustomSetNode**)realloc(
            m_NodePool, m_NodePoolSize * sizeof(SCustomSetNode));
    } else {
        NodePool =
            (SCustomSetNode**)malloc(m_NodePoolSize * sizeof(SCustomSetNode));
    }

    for (int i = 0; i < m_NodePoolSize - OldPoolSize; i += 1) {
        NodePool[i + OldPoolSize] =
            (SCustomSetNode*)malloc(sizeof(SCustomSetNode));
    }
