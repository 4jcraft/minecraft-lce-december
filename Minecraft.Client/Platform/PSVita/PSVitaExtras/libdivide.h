...
either a do std::vector function or a dummy int.
...
#if LIBDIVIDE_USE_SSE2
    /* Treats the std::vector as either two or four packed values (depending on
     * the size), and divides each of them by the divisor, returning the packed
     * quotients. */
    __m128i perform_divide_vector(__m128i val) const {
