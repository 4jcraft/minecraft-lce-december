...
void* user_new(std::size_t size) throw(std::bad_alloc) {
    ...
    while ((ptr = (void*)std::malloc(size)) == NULL) {
        ...
        /**E When new_handler is a NULL pointer, bad_alloc is send. If not,
         * new_handler is called. */
        /**J new_handler が NULL ポインタの場合、bad_alloc
         * を送出する、そうでない場合、new_handler を呼び出す */
        if (!handler)
            throw std::bad_alloc();
        else
            (*handler)();
...
void* user_new(std::size_t size, const std::nothrow_t& x) throw() {
    ...
    while ((ptr = (void*)std::malloc(size)) == NULL) {
         ...
         */
        /**J new_handler を呼び出す、new_handler が bad_alloc
         * を送出した場合、NULL を返す */
        try {
            (*handler)();
        } catch (std::bad_alloc) {
