...
public:
    ...
    virtual HRESULT ElementContent(CONST WCHAR* strData, UINT DataLen,
                                   BOOL More) = 0;
    virtual HRESULT ElementEnd(CONST WCHAR* strName, UINT NameLen) = 0;

    virtual HRESULT CDATABegin() = 0;
    virtual HRESULT CDATAData(CONST WCHAR* strCDATA, UINT CDATALen,
                              BOOL bMore) = 0;
...
public:
    ...
    //      Parses from a buffer- if you pass a WCHAR buffer (and cast it), it
    //      will
