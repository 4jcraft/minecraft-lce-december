...
SONYDLC* CConsoleMinecraftApp::GetSONYDLCInfo(int iTexturePackID) {
    for (AUTO_VAR(it, m_SONYDLCMap.begin()); it != m_SONYDLCMap.end(); ++it) {
        if (it->second->iConfig == iTexturePackID) return it->second;
    }
    return NULL;
}

#define WRAPPED_READFILE(hFile, lpBuffer, nNumberOfBytesToRead,     \
                         lpNumberOfBytesRead, lpOverlapped)         \
    {                                                               \
        if (ReadFile(hFile, lpBuffer, nNumberOfBytesToRead,         \
                     lpNumberOfBytesRead, lpOverlapped) == FALSE) { \
            return FALSE;                                           \
        }                                                           \
    }
BOOL CConsoleMinecraftApp::ReadProductCodes() {
    ...
    // 4J-PB - Read the file containing the product codes. This will be
    // different for the SCEE/SCEA/SCEJ builds
    HANDLE file = CreateFile("PSVita/PSVitaProductCodes.bin", GENERIC_READ, 0,
                             NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (file == INVALID_HANDLE_VALUE) {
        DWORD error = GetLastError();
        app.DebugPrintf(
            "Failed to open ProductCodes.bin with error code %d (%x)\n", error,
            error);
        return FALSE;
    }
    ...
    if (dwFileSize != 0) {
        ...
        WRAPPED_READFILE(file, ProductCodes.chProductCode, PRODUCT_CODE_SIZE,
                         &bytesRead, NULL);
        WRAPPED_READFILE(file, ProductCodes.chSaveFolderPrefix,
                         SAVEFOLDERPREFIX_SIZE, &bytesRead, NULL);
        // WRAPPED_READFILE(file,ProductCodes.chDiscSaveFolderPrefix,SAVEFOLDERPREFIX_SIZE,&bytesRead,NULL);
        WRAPPED_READFILE(file, ProductCodes.chCommerceCategory,
                         COMMERCE_CATEGORY_SIZE, &bytesRead, NULL);
        // WRAPPED_READFILE(file,ProductCodes.chTexturePackID,SCE_NP_COMMERCE2_CATEGORY_ID_LEN,&bytesRead,NULL);
        // // TODO
        WRAPPED_READFILE(file, ProductCodes.chUpgradeKey, UPGRADE_KEY_SIZE,
                         &bytesRead, NULL);
        ...
        for (unsigned int i = 0; i < uiDLC; i++) {
            ...
            WRAPPED_READFILE(file, pDLCInfo->chDLCKeyname, sizeof(char) * uiVal,
                             &bytesRead, NULL);

            WRAPPED_READFILE(file, &uiVal, sizeof(int), &bytesRead, NULL);
            WRAPPED_READFILE(file, chDLCTitle, sizeof(char) * uiVal, &bytesRead,
                             NULL);
            app.DebugPrintf("DLC title %s\n", chDLCTitle);

            WRAPPED_READFILE(file, &pDLCInfo->eDLCType, sizeof(int), &bytesRead,
                             NULL);
            WRAPPED_READFILE(file, &pDLCInfo->iFirstSkin, sizeof(int),
                             &bytesRead, NULL);
            WRAPPED_READFILE(file, &pDLCInfo->iConfig, sizeof(int), &bytesRead,
                             NULL);
...
void CConsoleMinecraftApp::GetSaveThumbnail(PBYTE* ppbThumbnailData,
                                            DWORD* pdwThumbnailSize,
                                            PBYTE* ppbDataImage,
                                            DWORD* pdwSizeImage) {
    // on a save caused by a create world, the thumbnail capture won't have
    // happened
    if (m_ThumbnailBuffer.Allocated()) {
        if (ppbThumbnailData) {
            ...
            *pdwThumbnailSize = m_ThumbnailBuffer.GetBufferSize();
            memcpy(*ppbThumbnailData, m_ThumbnailBuffer.GetBufferPointer(),
                   *pdwThumbnailSize);
        }
        ...
        if (ppbThumbnailData) {
            ...
            StorageManager.GetDefaultSaveThumbnail(ppbThumbnailData,
                                                   pdwThumbnailSize);
        }
    }
    ...
    if (m_SaveImageBuffer.Allocated()) {
        if (ppbDataImage) {
            ...
            *pdwSizeImage = m_SaveImageBuffer.GetBufferSize();
            memcpy(*ppbDataImage, m_SaveImageBuffer.GetBufferPointer(),
                   *pdwSizeImage);
        }
...
void CConsoleMinecraftApp::GetScreenshot(int iPad, PBYTE* pbData,
                                         DWORD* pdwSize) {}
...
#endif
    m_eCommerce_State =
        eCommerce_State_Offline;  // can only init when we have a PSN user
    ...
    if (ProfileManager.IsSignedInLive(0)) {
        switch (m_eCommerce_State) {
            ...
            case eCommerce_State_Init:
                ...
                m_pCommerce->CreateSession(
                    &CConsoleMinecraftApp::CommerceInitCallback, this);
                break;
            case eCommerce_State_GetCategories:
                m_eCommerce_State = eCommerce_State_GetCategories_Pending;
                // get all categories for this product
                m_pCommerce->GetCategoryInfo(
                    &CConsoleMinecraftApp::CommerceGetCategoriesCallback, this,
                    &m_CategoryInfo, app.GetCommerceCategory());
            ...
            case eCommerce_State_GetProductList: {
                ...
                SonyCommerce::CategoryInfoSub category =
                    (SonyCommerce::CategoryInfoSub)(*iter);

                m_pCommerce->GetProductList(
                    &CConsoleMinecraftApp::CommerceGetProductListCallback, this,
                    &m_ProductListA[m_ProductListRetrievedC],
                    category.categoryId);
            }
            ...
            case eCommerce_State_AddProductInfoDetailed: {
                ...
                // for each of the products in the categories, get the detailed
                // info. We really only need the long description and price
                // info.
                SonyCommerce::CategoryInfo* pCategories = app.GetCategoryInfo();
                std::list<SonyCommerce::CategoryInfoSub>::iterator iter =
                    pCategories->subCategories.begin();
                ...
                SonyCommerce::CategoryInfoSub category =
                    (SonyCommerce::CategoryInfoSub)(*iter);
                std::vector<SonyCommerce::ProductInfo>* pvProductList =
                    &m_ProductListA[m_iCurrentCategory];

                // 4J-PB - there may be no products in the category
                if (pvProductList->size() == 0) {
                    CConsoleMinecraftApp::
                        CommerceAddDetailedProductInfoCallback(this, 0);
                } else {
                    assert(pvProductList->size() > m_iCurrentProduct);
                    SonyCommerce::ProductInfo* pProductInfo =
                        &(pvProductList->at(m_iCurrentProduct));
                    m_pCommerce->AddDetailedProductInfo(
                        &CConsoleMinecraftApp::
                            CommerceAddDetailedProductInfoCallback,
                        this, pProductInfo, pProductInfo->productId,
                        category.categoryId);
                }
            } break;
            case eCommerce_State_Checkout:
                m_pCommerce->CreateSession(
                    &CConsoleMinecraftApp::CheckoutSessionStartedCallback,
                    this);
            ...
            case eCommerce_State_RegisterDLC: {
                ...
                for (int i = 0; i < m_iCurrentCategory; i++) {
                    ...
                    for (int j = 0; j < pvProductList->size(); j++) {
                        ...
                        RegisterDLCData(&pProductInfo->productId[20], 0,
                                        pProductInfo->imageUrl);
                    }
                    iter++;
                }
            } break;
            ...
            case eCommerce_State_DownloadAlreadyPurchased_SessionStarted:
                ...
                m_pCommerce->DownloadAlreadyPurchased(
                    &CConsoleMinecraftApp::CommerceCheckoutCallback, this,
                    m_pchSkuID);
                break;

            case eCommerce_State_UpgradeTrial:
                m_pCommerce->CreateSession(
                    &CConsoleMinecraftApp::UpgradeTrialSessionStartedCallback,
                    this);
                m_eCommerce_State =
                    eCommerce_State_UpgradeTrial_WaitingForSession;
                break;
            case eCommerce_State_UpgradeTrial_SessionStarted:
                m_pCommerce->UpgradeTrial(
                    &CConsoleMinecraftApp::CommerceCheckoutCallback, this);
                m_eCommerce_State = eCommerce_State_UpgradeTrial_Pending;
                break;
        }

        // 4J-PB - bit of a hack to display the full version purchase after
        // signing in during a trial trophy popup
        if (m_bDisplayFullVersionPurchase &&
            ((m_eCommerce_State == eCommerce_State_Online) ||
             (m_eCommerce_State == eCommerce_State_Error))) {
            m_bDisplayFullVersionPurchase = false;
            ProfileManager.DisplayFullVersionPurchase(
                false, ProfileManager.GetPrimaryPad(),
...
void CConsoleMinecraftApp::ClearCommerceDetails() {
    ...
    for (int i = 0; i < m_ProductListCategoriesC; i++) {
        std::vector<SonyCommerce::ProductInfo>* pProductList =
            &m_ProductListA[i];
        pProductList->clear();
    }
...
void CConsoleMinecraftApp::GetDLCSkuIDFromProductList(char* pchDLCProductID,
                                                      char* pchSkuID) {
#ifdef VITA_COMMERCE_ENABLED
...
bool CConsoleMinecraftApp::UpgradeTrial() {
    if (m_eCommerce_State == eCommerce_State_Online) {
        ...
        C4JStorage::EMessageResult result =
            ui.RequestErrorMessage(IDS_PRO_UNLOCKGAME_TITLE, IDS_NO_DLCOFFERS,
...
#ifdef VITA_COMMERCE_ENABLED
std::vector<SonyCommerce::ProductInfo>* CConsoleMinecraftApp::GetProductList(
    int iIndex) {
    if ((m_bCommerceProductListRetrieved == false) ||
        (m_bProductListAdditionalDetailsRetrieved == false)) {
        return NULL;
    }
...
void CConsoleMinecraftApp::CommerceGetCategoriesCallback(LPVOID lpParam,
                                                         int err) {
#ifdef VITA_COMMERCE_ENABLED
    CConsoleMinecraftApp* pClass = (CConsoleMinecraftApp*)lpParam;

    if (err == 0) {
        pClass->m_ProductListCategoriesC =
            pClass->m_CategoryInfo.countOfSubCategories;
        // allocate the memory for the product info for each categories
        if (pClass->m_CategoryInfo.countOfSubCategories > 0) {
            pClass->m_ProductListA =
                (std::vector<SonyCommerce::ProductInfo>*)new std::vector<
                    SonyCommerce::ProductInfo>[pClass->m_CategoryInfo
...
void CConsoleMinecraftApp::CommerceGetProductListCallback(LPVOID lpParam,
                                                          int err) {
    CConsoleMinecraftApp* pClass = (CConsoleMinecraftApp*)lpParam;
    ...
    if (err == 0) {
        ...
        if (pClass->m_ProductListRetrievedC ==
            pClass->m_CategoryInfo.countOfSubCategories) {
            // we're done, so now retrieve the additional product details for
            // each product
...
// void CConsoleMinecraftApp::CommerceGetDetailedProductInfoCallback(LPVOID
// lpParam,int err)
// {
...
void CConsoleMinecraftApp::CommerceAddDetailedProductInfoCallback(
    LPVOID lpParam, int err) {
#ifdef VITA_COMMERCE_ENABLED
    CConsoleMinecraftApp* pClass = (CConsoleMinecraftApp*)lpParam;

    if (err == 0) {
        // increment the current product counter. When this gets to the end of
        // the products, move to the next category
        pClass->m_iCurrentProduct++;

        std::vector<SonyCommerce::ProductInfo>* pvProductList =
            &pClass->m_ProductListA[pClass->m_iCurrentCategory];

        // if there are no more products in this category, move to the next
        // category (there may be no products in the category)
        if (pClass->m_iCurrentProduct >= pvProductList->size()) {
            // MGH - change this to a while loop so we can skip empty
            // categories.
            do {
                pClass->m_iCurrentCategory++;
            } while (
                pClass->m_ProductListA[pClass->m_iCurrentCategory].size() ==
                    0 &&
                pClass->m_iCurrentCategory < pClass->m_ProductListCategoriesC);

            pClass->m_iCurrentProduct = 0;
            if (pClass->m_iCurrentCategory ==
                pClass->m_ProductListCategoriesC) {
...
void CConsoleMinecraftApp::CheckoutSessionStartedCallback(LPVOID lpParam,
                                                          int err) {
    CConsoleMinecraftApp* pClass = (CConsoleMinecraftApp*)lpParam;
    if (err == 0)
...
void CConsoleMinecraftApp::DownloadAlreadyPurchasedSessionStartedCallback(
    LPVOID lpParam, int err) {
    CConsoleMinecraftApp* pClass = (CConsoleMinecraftApp*)lpParam;
    if (err == 0)
        pClass->m_eCommerce_State =
...
void CConsoleMinecraftApp::UpgradeTrialSessionStartedCallback(LPVOID lpParam,
                                                              int err) {
    CConsoleMinecraftApp* pClass = (CConsoleMinecraftApp*)lpParam;
    if (err == 0)
...
int CConsoleMinecraftApp::GetCommerceState() { return m_eCommerce_State; }
            ...
            if (res == SCE_OK) {
                switch (systemEvent.systemEvent) {
                    ...
                    case SCE_APPMGR_SYSTEMEVENT_ON_RESUME:
                        app.DebugPrintf(
                            "SCE_APPMGR_SYSTEMEVENT_ON_RESUME event "
                            "received\n");
                        break;
                    ...
                    case SCE_APPMGR_SYSTEMEVENT_ON_STORE_REDEMPTION:
                        app.DebugPrintf(
                            "SCE_APPMGR_SYSTEMEVENT_ON_STORE_REDEMPTION event "
                            "received\n");
                        break;
                    default:
...
bool CConsoleMinecraftApp::CheckForEmptyStore(int iPad) {
    ...
    if (pCategories != NULL) {
        if (pCategories->countOfProducts > 0) {
            ...
            for (int i = 0; i < pCategories->countOfSubCategories; i++) {
                std::vector<SonyCommerce::ProductInfo>* pvProductInfo =
                    app.GetProductList(i);
    ...
    if (bEmptyStore) {
        ProfileManager.ShowSystemMessage(
            SCE_MSG_DIALOG_SYSMSG_TYPE_TRC_EMPTY_STORE, iPad);
    }
...
void printSaveState() {
    ...
    switch (StorageManager.GetSaveState()) {
        case C4JStorage::ESaveGame_Idle:
            strState = "ESaveGame_Idle";
            break;
        case C4JStorage::ESaveGame_Save:
            strState = "ESaveGame_Save";
            break;
        case C4JStorage::ESaveGame_SaveCompleteSuccess:
            strState = "ESaveGame_SaveCompleteSuccess";
            break;
        case C4JStorage::ESaveGame_SaveCompleteFail:
            strState = "ESaveGame_SaveCompleteFail";
            break;
        case C4JStorage::ESaveGame_SaveIncomplete:
            strState = "ESaveGame_SaveIncomplete";
            break;
        case C4JStorage::ESaveGame_SaveIncomplete_WaitingOnResponse:
            strState = "ESaveGame_SaveIncomplete_WaitingOnResponse";
            break;
        case C4JStorage::ESaveGame_Load:
            strState = "ESaveGame_Load";
            break;
        case C4JStorage::ESaveGame_LoadCompleteSuccess:
            strState = "ESaveGame_LoadCompleteSuccess";
            break;
        case C4JStorage::ESaveGame_LoadCompleteFail:
            strState = "ESaveGame_LoadCompleteFail";
            break;
        case C4JStorage::ESaveGame_Delete:
            strState = "ESaveGame_Delete";
            break;
        case C4JStorage::ESaveGame_DeleteSuccess:
            strState = "ESaveGame_DeleteSuccess";
            break;
        case C4JStorage::ESaveGame_DeleteFail:
            strState = "ESaveGame_DeleteFail";
            break;
        case C4JStorage::ESaveGame_Rename:
            strState = "ESaveGame_Rename";
            break;
        case C4JStorage::ESaveGame_RenameSuccess:
            strState = "ESaveGame_RenameSuccess";
            break;
        case C4JStorage::ESaveGame_RenameFail:
            strState = "ESaveGame_RenameFail";
            break;
        case C4JStorage::ESaveGame_GetSaveThumbnail:
            strState = "ESaveGame_GetSaveThumbnail";
            break;
        case C4JStorage::ESaveGame_GetSaveInfo:
            strState = "ESaveGame_GetSaveInfo";
            break;
        case C4JStorage::ESaveGame_SaveCache:
            strState = "ESaveGame_SaveCache";
            break;
        case C4JStorage::ESaveGame_ReconstructCache:
            strState = "ESaveGame_ReconstructCache";
            break;
    }
...
void CConsoleMinecraftApp::SaveDataTick() {
    ...
    switch (m_bSaveDataDeleteDialogState) {
        ...
        case eSaveDataDeleteState_continue: {
            C4JStorage::ESaveGameState eGameState =
                StorageManager.GetSaveState();
            printSaveState();

            if (eGameState ==
                C4JStorage::ESaveGame_SaveIncomplete_WaitingOnResponse) {
                if (m_bSaveDataDeleteDialogState ==
                    eSaveDataDeleteState_abort) {
                    app.DebugPrintf(
                        "[SaveDataTick] eSaveDataDeleteState_abort.\n");
                    StorageManager.CancelIncompleteOperation();
                } else if (m_bSaveDataDeleteDialogState ==
                           eSaveDataDeleteState_continue) {
                    app.DebugPrintf(
                        "[SaveDataTick] eSaveDataDeleteState_continue.\n");
                    StorageManager.ContinueIncompleteOperation();
                }
            } else if (eGameState == C4JStorage::ESaveGame_Idle) {
                app.DebugPrintf(
                    "[SaveDataTick] Storage Manager is idle, SaveDataDialog "
                    "reverting to idle state too.\n");
            }
...
void CConsoleMinecraftApp::Callback_SaveGameIncomplete(
    void* pParam, C4JStorage::ESaveIncompleteType saveIncompleteType,
    int blocksRequired) {
    app.DebugPrintf(
        "[Callback_SaveGameIncomplete] saveIncompleteType=%i, "
        "blocksRequired=%i,\n",
    ...
    if (saveIncompleteType == C4JStorage::ESaveIncomplete_OutOfQuota ||
        ...
        if (UIScene_LoadOrJoinMenu::isSaveTransferRunning()) {
            // 4J MGH - if we're trying to save from the save transfer stuff,
            // only show "ok", and we won't try to save again
            if (saveIncompleteType == C4JStorage::ESaveIncomplete_OutOfQuota)
                blocksRequired = -1;
            UINT uiIDA[1];
            uiIDA[0] = IDS_CONFIRM_OK;
            C4JStorage::EMessageResult res = ui.RequestErrorMessage(
                IDS_SAVE_INCOMPLETE_TITLE,
                IDS_SAVE_INCOMPLETE_EXPLANATION_QUOTA, uiIDA, 1,
                ProfileManager.GetPrimaryPad(), &NoSaveSpaceReturned,
                (void*)blocksRequired);

        } else {
            // 4J Stu - If it's quota then we definitely have to delete our
            // saves, so don't show the system UI for this case
            if (saveIncompleteType == C4JStorage::ESaveIncomplete_OutOfQuota)
                blocksRequired = -1;
            UINT uiIDA[2];
            uiIDA[0] = IDS_CONFIRM_OK;
            ...
            C4JStorage::EMessageResult res = ui.RequestErrorMessage(
                IDS_SAVE_INCOMPLETE_TITLE,
                IDS_SAVE_INCOMPLETE_EXPLANATION_QUOTA, uiIDA, 2,
                ProfileManager.GetPrimaryPad(), &NoSaveSpaceReturned,
                (void*)blocksRequired);
        }
    }
...
int CConsoleMinecraftApp::NoSaveSpaceReturned(
    void* pParam, int iPad, C4JStorage::EMessageResult result) {
    if (result == C4JStorage::EMessage_ResultAccept &&
        !UIScene_LoadOrJoinMenu::
            isSaveTransferRunning())  // MGH - we won't try to save again during
                                      // a save tranfer
    {
...
int CConsoleMinecraftApp::cbConfirmDeleteMessageBox(
    ...
    if (pClass != NULL && pClass->m_pSaveToDelete != NULL) {
        if (result == C4JStorage::EMessage_ResultDecline) {
            pClass->m_bSaveDataDeleteDialogState =
                eSaveDataDeleteState_deleting;
            C4JStorage::ESaveGameState eDeleteStatus =
                StorageManager.DeleteSaveData(pClass->m_pSaveToDelete,
                                              cbSaveDataDeleted, pClass);
        } else {
        ...
        // 4J-JEV: This could leave the storage library in a waiting for user
        // state.
...
void CConsoleMinecraftApp::initSaveIncompleteDialog(int spaceNeeded) {
    ...
    param.sysMsgParam->sysMsgType =
        SCE_SAVEDATA_DIALOG_SYSMSG_TYPE_NOSPACE_CONTINUABLE;
    ...
    if (ret == SCE_OK) {
        ...
        app.DebugPrintf(
            "[SaveDataIncompleteDialog] ERROR: INITIALISING DIALOG, "
            "sceSaveDataDialogInit() (0x%x).\n",
            ret);
    }
}
...
void CConsoleMinecraftApp::updateSaveIncompleteDialog() {
    ...
    if (dialogStatus == SCE_COMMON_DIALOG_STATUS_RUNNING) {
        ...
        if (dialogSubStatus == SCE_COMMON_DIALOG_STATUS_RUNNING) {
            ...
            if (ret != SCE_OK) {
                app.DebugPrintf(
                    "[SaveDataIncompleteDialog] ERROR: UPDATING DIALOG, "
                    "sceSaveDataDialogFinish() (0x%x).\n",
                    ret);
            }
        }
        ...
        if (ret == SCE_OK) {
            ...
            app.DebugPrintf(
                "[SaveDataIncompleteDialog] ERROR: TERMINATING DIALOG, "
                "sceSaveDataDialogTerm() (0x%x).\n",
                ret);
        }
    }
}
...
void CConsoleMinecraftApp::initSaveDataDeleteDialog() {
    ...
    if (ret == SCE_OK) {
        app.DebugPrintf(
            "[SaveDataDeleteDialog] Successfully initialised SaveDataDelete "
            "dialog.\n");
        ...
        if (StorageManager.ReturnSavesInfo() == NULL) {
            C4JStorage::ESaveGameState eSGIStatus = StorageManager.GetSavesInfo(
                ProfileManager.GetPrimaryPad(), NULL, this, "save");
        }

        // Dim background because sony doesn't do that.
        ui.showComponent(0, eUIComponent_MenuBackground, eUILayer_Tooltips,
                         eUIGroup_Fullscreen, true);

        StorageManager.SetSaveDisabled(true);
        EnterSaveNotificationSection();
    } else {
        app.DebugPrintf(
            "[SaveDataDeleteDialog] ERROR: INITIALISING DIALOG, "
            "sceSaveDataDialogInit() (0x%x).\n",
            ret);
    }
...
void CConsoleMinecraftApp::updateSaveDataDeleteDialog() {
    ...
    if (dialogStatus == SCE_COMMON_DIALOG_STATUS_RUNNING) {
        ...
        if (dialogSubStatus == SCE_COMMON_DIALOG_STATUS_RUNNING) {
            ...
            if (ret == SCE_OK) {
                ...
                if (dialogResult.result ==
                    ...
                    app.DebugPrintf(
                        "[SaveDataDeleteDialog] CANCELED OR ABORTED!\n");

                    // 4J-JEV: Check to ensure that finishedDeletingSaves is
                    // called only once.
                    if (m_bSaveDataDeleteDialogState ==
                ...
                if (dialogResult.result == SCE_COMMON_DIALOG_RESULT_OK) {
                    ...
                    ret = sceAppUtilSaveDataSlotGetParam(dialogResult.slotId,
                                                         &slotParam, NULL);
                    ...
                    if (ret == SCE_OK) {
                        ...
                        if (pSaveDetails != NULL) {
                            app.DebugPrintf(
                                "[SaveDataDeleteDialog] Searching for save "
                                "files:\n");

                            for (unsigned int i = 0; i < pSaveDetails->iSaveC;
                                 i++) {
                                app.DebugPrintf("\t- '%s'\n",
                                                pSaveDetails->SaveInfoA[i]
                                                    .UTF8SaveFilename);

                                void *buf1, *buf2;
                                buf1 = &(pSaveDetails->SaveInfoA[i]
                                             .UTF8SaveFilename);
                                buf2 = &slotParam.title;

                                if (0 == memcmp(buf1, buf2,
                            ...
                            app.DebugPrintf(
                                "[SaveDataDeleteDialog] ERROR: PERFORMING "
                                "DELETE OPERATION, pSavesDetails is null.\n");
                        }

                        if (pSaveInfo != NULL) {
                            app.DebugPrintf(
                                "[SaveDataDeleteDialog] User wishes to delete "
                                "slot_%d:\n\t"
                                "4jsaveindex=%d, filename='%s', title='%s', "
                                "subtitle='%s', size=%dKiB.\n",
                                dialogResult.slotId, saveindex,
                                pSaveInfo->UTF8SaveFilename, slotParam.title,
                                slotParam.subTitle, slotParam.sizeKiB);

                            UINT uiIDA[] = {IDS_CONFIRM_CANCEL, IDS_CONFIRM_OK};

                            ui.RequestErrorMessage(
                                IDS_TOOLTIPS_DELETESAVE, IDS_TEXT_DELETE_SAVE,
                                uiIDA, 2, 0, &cbConfirmDeleteMessageBox, this);
                            ...
                            app.DebugPrintf(
                                "[SaveDataDeleteDialog] ERROR: PERFORMING "
                                "DELETE OPERATION, cannot find file in our "
                                "saves list:\n"
                                "\t slotId=%i, title=%s, subtitle=%s,\n",
                                dialogResult.slotId, slotParam.title,
                                slotParam.subTitle);
                        }
                        ...
                        app.DebugPrintf(
                            "[SaveDataDeleteDialog] ERROR: UPDATING DIALOG, "
                            "sceAppUtilSaveDataGetParam() (0x%x).\n",
                            ret);
                    }
                }
                ...
                if (finishDialog) {
                    SceSaveDataDialogFinishParam finishParam;
                    ZeroMemory(&finishParam,
                               sizeof(SceSaveDataDialogFinishParam));
                    finishParam.flag = SCE_SAVEDATA_DIALOG_FINISH_FLAG_DEFAULT;

                    sceSaveDataDialogFinish(&finishParam);
                    if (ret == SCE_OK)
                        app.DebugPrintf(
                            "[SaveDataDeleteDialog] Successfully finished "
                            "saveDataDialog.\n");
                    else
                        app.DebugPrintf(
                            "[SaveDataDeleteDialog] ERROR: UPDATING DIALOG, "
                ...
                app.DebugPrintf(
                    "[SaveDataDeleteDialog] ERROR: UPDATING DIALOG, "
                    "sceSaveDataDialogGetResult() (0x%x).\n",
                    ret);
            }
        }
    } else if (dialogStatus == SCE_COMMON_DIALOG_STATUS_FINISHED) {
        ...
        if (ret == SCE_OK) {
            ...
            app.DebugPrintf(
                "[SaveDataDeleteDialog] ERROR: TERMINATING DIALOG, "
                "sceSaveDataDialogTerm() (0x%x).\n",
                ret);
        }
    }
...
void CConsoleMinecraftApp::finishSaveDataDeleteDialog() {
    ...
    InputManager.SetMenuDisplayed(0, false);
    ui.SetSysUIShowing(false);
    ui.removeComponent(eUIComponent_MenuBackground, eUILayer_Tooltips,
                       eUIGroup_Fullscreen);
}

void CConsoleMinecraftApp::getSaveDataDeleteDialogParam(
    SceSaveDataDialogParam* baseParam) {
...

    {
        ...
        SceAppUtilSaveDataSlot* pSavesList =
            new SceAppUtilSaveDataSlot[slots.size()];
    ...
    if (listParam.slotListSize > 0)
        listParam.focusPos = SCE_SAVEDATA_DIALOG_FOCUS_POS_LISTHEAD;
    else
        listParam.focusPos = SCE_SAVEDATA_DIALOG_FOCUS_POS_EMPTYHEAD;
...
void CConsoleMinecraftApp::releaseSaveDataDeleteDialogParam(
    SceSaveDataDialogParam* baseParam) {
...
int CConsoleMinecraftApp::cbSaveDataDeleted(void* pParam, const bool success) {
    ...
    if (pApp->m_bSaveDataDeleteDialogState == eSaveDataDeleteState_deleting) {
        ...
        if (ret != SCE_OK)	app.DebugPrintf("[SaveDataDeleteDialog] ERROR:
        UPDATING DIALOG, sceSaveDataDialogContinue() (0x%x).\n", ret);

        pApp->m_bSaveDataDeleteDialogState =
        eSaveDataDeleteState_waitingForUser;
...
void CConsoleMinecraftApp::finishedDeletingSaves(bool bContinue) {
    ...
    if (bContinue)
        m_bSaveDataDeleteDialogState = eSaveDataDeleteState_continue;
    else
        m_bSaveDataDeleteDialogState = eSaveDataDeleteState_abort;
}