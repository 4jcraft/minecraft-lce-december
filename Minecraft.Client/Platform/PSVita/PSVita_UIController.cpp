...
void ConsoleUIController::init(S32 w, S32 h) {
    ...
    gdraw_psp2_SetResourceMemory(
        GDRAW_PSP2_RESOURCE_texture, tex_handles,
        RenderManager.allocGPUCDMem(tex_mem_size, GDRAW_PSP2_TEXTURE_ALIGNMENT),
        tex_mem_size);
    gdraw_psp2_SetResourceMemory(
        GDRAW_PSP2_RESOURCE_vertexbuffer, vert_handles,
        RenderManager.allocGPURWMem(vert_mem_size,
    ...
    Hence, we recommend that you use two or three dynamic buffers in a
    round-robin fashion. This example uses as many dynamic buffers as it does
    back buffers (display queue entries), which is a good general rule of thumb.
    */
    S32 dynamic_buf_size = 1 * 1024 * 1024;  // Again, very generous!
    m_currentBackBuffer = 0;
    m_dynamicBuffer =
        new gdraw_psp2_dynamic_buffer[RenderManager.getDisplayBufferCount()];
    for (int i = 0; i < RenderManager.getDisplayBufferCount(); i++)
        gdraw_psp2_InitDynamicBuffer(
            &m_dynamicBuffer[i],
            RenderManager.allocGPUROMem(dynamic_buf_size, 4), dynamic_buf_size);

    /* Whew! Now that the GDraw memory configuration is set up, we need to
    initialize GDraw. But first, we need to allocate yet another block of video
    memory for GDraw to store some global data in - a few buffers that are
    shared between all Iggys. Unlike the rest of the memory configuration, the
    work area is reserved to GDraw and can't be freed or relocated afterwards
    without shutting GDraw and all attached Iggys down completely. But it's
    fairly small (16k as of this writing) so this shouldn't be much of a
    problem. It should be allocted in uncached, GPU-readable memory. */
    void* gdraw_context_mem =
        RenderManager.allocGPUROMem(GDRAW_PSP2_CONTEXT_MEM_SIZE, 1);

    /* GDraw also needs a single GPU notification label for its synchronization
    work. Since there's no other rendering in this example, we just use the
    first label. */
    volatile uint32_t* notify_region =
        RenderManager
            .getNotificationRegion();  // sceGxmGetNotificationRegion();

    /* Finally, we can actually create the GDraw GXM driver and pass it to Iggy.
     */
    gdraw_funcs = gdraw_psp2_CreateContext(
        RenderManager.getGXMShaderPatcher(), gdraw_context_mem,
        notify_region + 0, SCE_GXM_OUTPUT_REGISTER_FORMAT_UCHAR4);
...
void ConsoleUIController::render() {
    ...
    */
    ...
    gdraw_psp2_Begin(RenderManager.getGXMContext(),
                     RenderManager.getCurrentDisplaySurface(),
                     RenderManager.getCurrentDepthSurface(),
                     &m_dynamicBuffer[m_currentBackBuffer]);
    ++m_currentBackBuffer;
