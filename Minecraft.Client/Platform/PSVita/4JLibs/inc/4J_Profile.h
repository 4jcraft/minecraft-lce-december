...
// (1) A full online ID - either the primary login, or a sub-signin through to
// PSN. This has m_onlineID set up as a normal SceNpOnlineId, with dummy[0] set
// to 0 (2) An offline ID, where there is also a primary login on the system.
// This has m_onlineID set up to copy the primary SceNpOnlineId, except with
// dummy[0] set to the controller ID of this other player (3) An offline ID,
// where there isn't a primary PSN login on the system. This has SceNpOnlineId
// fully zeroed.
...
public:
    ...
    PlayerUID();
    PlayerUID(int userID, SceNpOnlineId& onlineID, bool bSignedInPSN,
              int quadrant);
    ...
    bool isPrimaryUser() const;  // only true if we're on the local machine and
                                 // signed into the first quadrant;
...
public:
    ...
    UINT RequestSignInUI(bool bFromInvite, bool bLocalGame,
                         ...
                         LPVOID lpParam, int iQuadrant = XUSER_INDEX_ANY);
    UINT DisplayOfflineProfile(int (*Func)(LPVOID, const bool, const int iPad),
                               LPVOID lpParam, int iQuadrant = XUSER_INDEX_ANY);
    UINT RequestConvertOfflineToGuestUI(int (*Func)(LPVOID, const bool,
                                                    const int iPad),
                                        LPVOID lpParam,
    ...
    typedef struct {
        int iPad;
        int (*m_fnFunc)(LPVOID lpParam, PBYTE pbThumbnail,
                        DWORD dwThumbnailBytes);
        LPVOID m_fnFunc_Param;
    } FUNCPARAMS;
    bool GetProfileAvatar(int iPad,
                          int (*Func)(LPVOID lpParam, PBYTE pbThumbnail,
                                      DWORD dwThumbnailBytes),
                          LPVOID lpParam);
    ...
    void DisplaySystemMessage(SceMsgDialogSystemMessageType _type,
                              int iQuadrant);
    ...
    void RichPresenceRegisterContextString(int ctxID, int strIndex,
                                           const char* str);
    ...
    void DisplayFullVersionPurchase(bool bRequired, int iQuadrant,
                                    int iUpsellParam = -1);
    void SetUpsellCallback(void (*Func)(LPVOID lpParam, eUpsellType type,
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    bool GetChatAndContentRestrictions(int iPad, bool thisQuadrantOnly,
                                       bool* pbChatRestricted,
                                       bool* pbContentRestricted, int* piAge);
    void SetServiceID(
        char* pchServiceID);  // needed for the ticket request for the chat
                              // restrictions of secondary PSN players
    ...
    int getUserID(int iQuadrant);  //  grab the PS4 userID for this quadrant
                                   //  (SCE_USER_SERVICE_USER_ID_INVALID if it's
                                   //  not signed in)
    ...
    bool SonyHttp_getDataFromURL(const char* szURL, void** ppOutData,
                                 int* pDataSize);
};
