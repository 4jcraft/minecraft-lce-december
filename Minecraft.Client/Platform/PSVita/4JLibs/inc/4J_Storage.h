...
#define MAX_DISPLAYNAME_LENGTH \
    128  // SCE_APPUTIL_SAVEDATA_SLOT_SUBTITLE_MAXSIZE on Vita

// #define MAX_SAVEFILENAME_LENGTH 32 // SCE_SAVE_DATA_DIRNAME_DATA_MAXSIZE on
// PS4
#define MAX_SAVEFILENAME_LENGTH \
    64  // SCE_APPUTIL_SAVEDATA_SLOT_TITLE_MAXSIZE on Vita
...
class CONTENT_DATA {
public:
    enum Type { e_contentLocked, e_contentUnlocked };
    int DeviceID;
    DWORD dwContentType;
    WCHAR wszDisplayName[256];
    CHAR szFileName[SCE_FIOS_PATH_MAX];
};
typedef CONTENT_DATA XCONTENT_DATA,
    *PXCONTENT_DATA;  // TODO - put back in when actually interfacing with game
...
public:
    ...
    enum {
        ...
        PROFILE_READTYPE_XBOXSETTINGS  // just read the settings (after a
                                       // notification of settings change)
    };
    ...
    void Init(unsigned int uiSaveVersion, LPCWSTR pwchDefaultSaveName,
              ...
              int iMinimumSaveSize,  // General manager initialisation
              int (*Func)(LPVOID, const ESavingMessage, int), LPVOID lpParam,
              LPCSTR szGroupID);
    void SetGameSaveFolderTitle(
        WCHAR*
            wszGameSaveFolderTitle);  // Sets the title to be set in the
                                      // param.sfo of saves (this doesn't vary,
                                      // the sub-title is used for the user cho
    void SetSaveCacheFolderTitle(
        WCHAR* wszSaveCacheFolderTitle);  // Sets the title to be set in the
                                          // param.sfo of the save cache
    void SetOptionsFolderTitle(
        WCHAR* wszOptionsFolderTitle);  // Sets the title to be set in the
                                        // param.sfo of the options file
    void SetGameSaveFolderPrefix(
        char* szGameSaveFolderPrefix);  // Sets the prefix to be added to the
                                        // unique filename of each save to
                                        // construct a final folder name
    void SetMaxSaves(int iMaxC);  // Sets the maximum number of saves to be
                                  // evaluated by GetSavesInfo etc.
    void SetDefaultImages(
        PBYTE pbOptionsImage, DWORD dwOptionsImageBytes, PBYTE pbSaveImage,
        DWORD dwSaveImageBytes,  // Sets default save image and thumbnail, which
                                 // can be used when saving a game that hasn't
                                 // generated any yet
        PBYTE pbSaveThumbnail, DWORD dwSaveThumbnailBytes);

    void SetIncompleteSaveCallback(
        void (*Func)(LPVOID, const ESaveIncompleteType, int blocksRequired),
        LPVOID param);  // Sets callback to be used in the event of a save
                        // method not being able to complete
    ...
    void ResetSaveData();        // Releases any internal storage being held for
                                 // previously saved/loaded data
    C4JStorage::ESaveGameState DoesSaveExist(
        bool* pbExists);  // Determine if current savegame exists on storage
                          // device
    ...
    C4JStorage::ESaveGameState GetSavesInfo(
        int iPad,
        int (*Func)(LPVOID lpParam, SAVE_DETAILS* pSaveDetails, const bool),
        LPVOID lpParam, char* pszSavePackName);  // Start search
    PSAVE_DETAILS ReturnSavesInfo();  // Returns result of search (or NULL if
                                      // not yet received)
    ...
    C4JStorage::ESaveGameState LoadSaveDataThumbnail(
        ...
        LPVOID lpParam);  // Get the thumbnail for an individual save referenced
                          // by pSaveInfo

    // Loading savedata & obtaining information from just-loaded file
    C4JStorage::ESaveGameState LoadSaveData(
        PSAVE_INFO pSaveInfo,
        int (*Func)(LPVOID lpParam, const bool, const bool),
        LPVOID lpParam);  // Loads savedata referenced by pSaveInfo, calls
                          // callback once complete
    unsigned int GetSaveSize();  // Obtains sizse of just-loaded save
    void GetSaveData(void* pvData,
                     unsigned int* puiBytes);  // Obtains pointer to, and size,
                                               // of just-loaded save
    bool GetSaveUniqueNumber(
        INT* piVal);  // Gets the unique numeric portion of the folder name used
                      // for the save (encodes m
    bool GetSaveUniqueFilename(
        char* pszName);  // Get the full unique "filename" used as part of the
                         // folder name for the save
    bool GetSaveUniqueFileDir(
        char* pszName);  // Get the full unique "filename" used as part of the
                         // folder name for the save

    // Saving savedata
    void SetSaveTitle(
        const wchar_t* UTF16String);  // Sets the name which is used as a
                                      // sub-title in the savedata param.sfo
    PVOID AllocateSaveData(
        unsigned int uiBytes);  // Allocate storage manager owned memory to the
                                // data which is to be saved to
    void SetSaveDataSize(
        unsigned int uiBytes);  // Set the actual size of data to be saved
    void GetDefaultSaveImage(
        PBYTE* ppbSaveImage,
        DWORD*
            pdwSaveImageBytes);  // Get the default save thumbnail (as set by
                                 // SetDefaultImages) for use on saving games t
    void GetDefaultSaveThumbnail(
        PBYTE* ppbSaveThumbnail,
        DWORD* pdwSaveThumbnailBytes);  // Get the default save image (as set by
                                        // SetDefaultImages) for use on saving
                                        // games that
    void SetSaveImages(
    ...
    // Handling of incomplete saves (either sub-files or save data). To be used
    // after game has had callback for an incomplete save event
    ...
    ESaveIncompleteType
    GetOptionsSaveError();         // Returns the save error [Options]
    void ClearOptionsSaveError();  // Clears any save error

    // Other file operations
    C4JStorage::ESaveGameState DeleteSaveData(
        PSAVE_INFO pSaveInfo, int (*Func)(LPVOID lpParam, const bool),
        LPVOID lpParam);  // Deletes savedata referenced by pSaveInfo, calls
                          // callback when comple
    C4JStorage::ESaveGameState RenameSaveData(
        int iRenameIndex, uint16_t* pui16NewName,
        int (*Func)(LPVOID lpParam, const bool),
        LPVOID lpParam);  // Renamed savedata with index from last established
                          // ReturnSavesInfo.
...
public:
    void SetSaveUniqueFilename(
        char* szFilename);  // MGH - made this public, used for the cross save
                            // stuff
    void EnableDownloadSave();  // CD - Used for cross/download-save, sets a
                                // flag for saving
...
public:
    ...
    void InitialiseProfileData(
        ...
        unsigned int*
            puiGameDefinedDataChangedBitmask);  // General initialisation
    int SetDefaultOptionsCallback(
        int (*Func)(LPVOID, PROFILESETTINGS*, const int iPad),
        LPVOID lpParam);  // Set a callback that can initialise a profile's
                          // storage to its default settings
    void SetOptionsDataCallback(
        int (*Func)(LPVOID, int iPad, unsigned short usVersion,
                    C4JStorage::eOptionsCallback),
        LPVOID lpParam);  // Sets callback that is called when status of any
                          // options has changed
    int SetOldProfileVersionCallback(int (*Func)(LPVOID, unsigned char*,
                                                 const unsigned short,
                                                 const int),
                                     LPVOID lpParam);

    // Getting and setting of profile data
    PROFILESETTINGS* GetDashboardProfileSettings(
        int iPad);  // Get pointer to the standard (originally xbox dashboard)
                    // profile data for one user
    void* GetGameDefinedProfileData(
        int iQuadrant);  // Get pointer to the game-defined profile data for one
                         // user
    ...
    void ReadFromProfile(
        int iQuadrant,
        int iReadType =
            PROFILE_READTYPE_ALL);  // Initiate read profile data for one user -
                                    // read type is ignored on this platform
    void WriteToProfile(int iQuadrant, bool bGameDefinedDataChanged = false,
                        bool bOverride5MinuteLimitOnProfileWrites =
                            false);    // Initiate write profile for one user
    void DeleteOptionsData(int iPad);  // Delete profile data for one user
    void ForceQueuedProfileWrites(
        int iPad =
            XUSER_INDEX_ANY);  // Force any queued profile writes to write now
    ...
    C4JStorage::ETMSStatus ReadTMSFile(
        ...
        int (*Func)(LPVOID, WCHAR*, int, bool, int), LPVOID lpParam,
        int iAction) {
    ...
    DWORD MountInstalledDLC(int iPad, DWORD dwDLC,
                            ...
                            LPVOID lpParam, LPCSTR szMountDrive = NULL);
