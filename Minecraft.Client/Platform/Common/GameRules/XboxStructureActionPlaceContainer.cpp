...
void XboxStructureActionPlaceContainer::addAttribute(
    ...
    if (attributeName.comp "facing") == 0)
	{
            int value = _fromString<int>(attributeValue);
            m_data = value;
                app.DebugPr"XboxStructureActionPlaceContainer: Adding parameter facing=%d\n",m_data);
        }
    else {
...
bool XboxStructureActionPlaceContainer::placeContainerInLevel(
    ...
    std::shared_ptr<Container> container = dynamic_pointer_cast<Container>(
        level->getTileEntity(worldX, worldY, worldZ));
                ...
                app.De"XboxStructureActionPlaceContainer - placing a container at (%d,%d,%d)\n", worldX, worldY, worldZ);
                if (container != NULL) {
                        level->setData( worldX, worldY, worldZ, m_data, Tile::UPDATE_CLI// Add items
			int slotId = 0;
