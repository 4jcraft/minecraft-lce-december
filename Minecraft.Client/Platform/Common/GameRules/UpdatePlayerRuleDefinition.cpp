...
void UpdatePlayerRuleDefinition::addAttribute(
    ...
    if (attributeName.comp "spawnX") == 0)
	{
            if (m_spawnPos == NULL) m_spawnPos = new Pos();
            int value = _fromString<int>(attributeValue);
            m_spawnPos->x = value;
                app.DebugPr"UpdatePlayerRuleDefinition: Adding parameter spawnX=%d\n",value);
        }
    else if (attributeName.comp "spawnY") == 0)
	{
            if (m_spawnPos == NULL) m_spawnPos = new Pos();
            int value = _fromString<int>(attributeValue);
            m_spawnPos->y = value;
                app.DebugPr"UpdatePlayerRuleDefinition: Adding parameter spawnY=%d\n",value);
        }
    else if (attributeName.comp "spawnZ") == 0)
	{
            if (m_spawnPos == NULL) m_spawnPos = new Pos();
            int value = _fromString<int>(attributeValue);
            m_spawnPos->z = value;
                app.DebugPr"UpdatePlayerRuleDefinition: Adding parameter spawnZ=%d\n",value);
        }
    else if (attributeName.comp "health") == 0)
	{
            int value = _fromString<int>(attributeValue);
            m_health = value;
            m_bUpdateHealth = true;
                app.DebugPr"UpdatePlayerRuleDefinition: Adding parameter health=%d\n",value);
        }
    else if (attributeName.comp "food") == 0)
	{
            int value = _fromString<int>(attributeValue);
            m_food = value;
            m_bUpdateFood = true;
    ...
    else if (attributeName.comp "yRot") == 0)
	{
            float value = _fromString<float>(attributeValue);
            m_yRot = value;
            m_bUpdateYRot = true;
                app.DebugPr"UpdatePlayerRuleDefinition: Adding parameter yRot=%f\n",value);
        }
    else {
