...
bool CollectItemRuleDefinition::onCollectItem(
    ...
    if (item != NULL && item->id == m_itemId &&
        ...
        if (!getComplete(rule)) {
                        GameRule::ValueType value = rule->getParame"iQuantity");
                        int quantityCollected = (value.i += item->count);
                        rule->setParame"iQuantity",value);

                        statusChanged = true;

                        if (quantityCollected >= m_quantity) {
                            setComplete(rule, true);
                                app.DebugPr"Completed CollectItemRule with info - itemId:%d, auxValue:%d, quantity:%d, dataTag:%d\n", m_itemId,m_auxValue,m_quantity,m_4JDataValue);
                                    ...
                                    rule->getConnection()->send(
                                        std::shared_ptr<
                                            ...
                                            new UpdateGameRuleProgressPacket(
                                                getActionType(),
                                                this->m_descriptionId, m_itemId,
                                                m_auxValue, this->m_4JDataValue,
                                                NULL, 0)));
                                }
                        }
        }
...
std::wstring CollectItemRuleDefinition::generateXml(std::shared_ptr<ItemInst// 4J Stu - This should be kept in sync with the GameRulesDefinition.xsd
	""d::wstring xml = L;
	if(item !"<CollectItemRule itemId=\"" + _toStrin"\" quantity=\"SET\" descriptionName=\"OPTIONAL\" promptName=\"OPTIONAL\"";
		if(item->getAuxVa" auxValue=\""l += L + _toString<int>(item"\""tAuxValue()) + L;
		if(item->get4JD" dataTag=\""ml += L + _toString<int>(it"\""get4JData())"/>\n";
		xml += L;
}
return xml;
}