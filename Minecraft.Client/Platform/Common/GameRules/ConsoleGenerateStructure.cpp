...
void ConsoleGenerateStructure::addAttribute(
    ...
    if (attributeName.comp "x"(L) == 0) {
        int value = _fromString<int>(attributeValue);
        m_x = value;
                app.DebugPr"ConsoleGenerateStructure: Adding parameter x=%d\n",m_x);
    } else if (attributeName.comp "y"(L) == 0) {
        int value = _fromString<int>(attributeValue);
        m_y = value;
                app.DebugPr"ConsoleGenerateStructure: Adding parameter y=%d\n",m_y);
    } else if (attributeName.comp "z"(L) == 0) {
        int value = _fromString<int>(attributeValue);
        m_z = value;
                app.DebugPr"ConsoleGenerateStructure: Adding parameter z=%d\n",m_z);
    } else if (attributeName.comp "orientation") == 0)
	{
            int value = _fromString<int>(attributeValue);
            orientation = value;
                app.DebugPr"ConsoleGenerateStructure: Adding parameter orientation=%d\n",orientation);
        }
    else if (attributeName.comp "dim") == 0)
	{
            m_dimension = _fromString<int>(attributeValue);
            if (m_dimension > 1 || m_dimension < -1) m_dimension = 0;
                app.DebugPr"ApplySchematicRuleDefinition: Adding parameter dimension=%d\n",m_dimension);
        }
    else {
