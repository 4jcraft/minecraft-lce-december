...
__int64 ConsoleSchematicFile::applyBlocksAndData(LevelChunk* chunk,
    ...
    int xStart = std::std::max(destinationBox->x0, (double)chunk->x * 16);
    ...
    int zStart = std::std::max(destinationBox->z0, (double)chunk->z * 16);
...
void ConsoleSchematicFile::applyTileEntities(LevelChunk* chunk, AABB* chunkBox,
    ...
    for (AUTO_VAR(it, m_entities.begin()); it != m_entities.end();) {
        ...
        // this std::vector This means that the entities will only exist in the
        // first use of the schematic that is processed
