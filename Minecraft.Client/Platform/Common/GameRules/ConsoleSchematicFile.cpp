...
void ConsoleSchematicFile::save_tags(DataOutputStream* dos) {
    ...
    ListTag<CompoundTag>* tileEntityTags = new ListTag<CompoundTag>();
        tag->"TileEntities", tileEntityTags);
...
__int64 ConsoleSchematicFile::applyBlocksAndData(LevelChunk* chunk,
    ...
    int zStart = std::max(destinationBox->z0, (double)chunk->z * 16);
        int zEnd = std::min(destinationBox->z1, (double
#ifdef _DEBUG < 4) + 16);
"Range is (%d,%d,%d) to (%d,%d,%d)\n",xStart,yStart,zStar#endif-1,yEnd-1,zEnd-1);
	...
	byteArray blockData = byteArray(Level::CHUNK_TILE_COUNT"Getting block data"nt(0,);
	chunk->getBlockData(blockData);
	PIXEndNamedEvent();
	byteArray dataData = byteArray(Level::HALF_CHUNK_TILE_COUNT"Getting Data data"ent(0,);
	chunk->getDataData(dataData)// Ignore light data);
	...
	if( rot == eSchematicRot_90 || rot == eSchematicRot_180 || rot == eSchematicRot_270 )
	{
	...
	else if( rot =// The initial pointer offsets for the different data types
        ...
        int z1 = zEnd - chunk->z * 16;
                        ...
                        ConsoleSchematicFile::setBlocksAndData(chunk,blockData,dataData,m_data, x0, yStart, z0, x1, yEnd, z1, blocksP, dataP, blo// update all pointer positions// For z start to z end		// Set blocks and data			// increment z by the right amount
...
delete blockData.data;
        chunk->recalcHeightmapOnly("Setting Data data"ent(0,);
	chunk->setDataData(dataData);
	PIXEndNamedEvent();
// A basic pass through to roughly do the lighting. At this point of post-processing, we don't have all the neighbouring chunks loaded in,// so any lighting here should be things that won't propagate out of this chunk.
	...
	r// At the point that this is called, we have all the neighbouring chunks loaded in (and generally post-processed, apart from this lighting pass), so// we can do the sort of lighting that might propagate out of the chunk.
__int64 ConsoleSchematicFile::applyLighting(LevelChunk *chunk, AABB *chunkBox, AABB *destinationBox, ESchematicRotation rot)
{
    ...
    int rowBlocksIncluded = (yEnd - yStart) * (zEnd - zStart);
    int blocksI  // Now actually do a checkLight on blocks that might need it,
                 // which should more accurately put everything in
                 // place
        for (int xx = xStart; xx < xEnd; xx++) for (int y = yStart; y < yEnd;
                                                    y++) for (int zz = zStart;
                                                              zz < zEnd; zz++) {
        int x = xx - chunk->x * 16;
        int z = zz - chunk->z * 16;
	...
	return blocksIncluded;
        ...
        void ConsoleSchematicFile::chunkCoordToSchematicCoord(
            AABB* destinationBox, int chunkX, int chunkZ,
		...
		schematicZ = (destinationBox->z1 - 1 - destinationBox->z0) - (chunkZ - destinationB// schematicX increases as chunkZ increases		// shcematicZ decreases as chunkX increases		
		schematicX = (destinationBox->z1 - 1 - destinationBox->z0) - (chunkZ - destinationBox->z0);
		schematicZ = chunkX - destinationBox->x0;// schematicX increases as chunkX increases		// schematicZ increases as chunkZ increases		
		...
		break;
                ...
                void ConsoleSchematicFile::schematicCoordToChunkCoord(
                    ...
                    ESchematicRotation rot, double& chunkX,
                    double&
		...
		chunkZ = (destinationBox->z1 - 1 - schematicX);// schematicX increases as chunkX increases		// schematicZ increases as chunkZ increases		
		...
		break;
                ...
                void ConsoleSchematicFile::applyTileEntities(
                    LevelChunk* chunk, AABB* chunkBox, AABB* destinationBox,
                    ESchematicRotation rot) {
                    for (AUTO_VAR(it, m_tileEntities.begin());
                         it != m_tileEntities.end(); ++it) {
                        ...
                        schematicCoordToChunkCoord(destinationBox, te->x, te->z,
                                                   rot, targetX, targetZ);

                        Vec3* pos = Vec3::newTemp(targetX, targetY, targetZ);
                        if (chunkBox->containsIncludingLowerBound(pos)) {
                            std::shared_ptr<TileEntity> teCopy =
                                chunk->getTileEntity((int)targetX & 15,
                                                     (int)targetY & 15,
                                                     (int)targetZ & 15);
                            ...
                            if (teCopy != NULL) {
                                ...
                                te  // Adjust the tileEntity position to world
                                    // coords from schematic
                                    // co-ords
                                    teCopy->x = targetX;
                                teCopy  // Remove the current tile
                                        // entityrgetZ//chunk->removeTileEntity(
                                        // (int)targetX & 15, (int)targetY & 15,
                                        // (int)targetZ & 15
                                        // );
                            }  // Adjust the tileEntity position to world coords
                               // from schematic
                               // co-ords
                            teCopy->x = targetX;
                            teCopy->y = targetY;
                ...
                for (AUTO_VAR(it, m_entities.begin());
                     it != m_entities.end();) {
                ...
                schematicCoordToChunkCoord(destinationBox, sour// Add 0.01 as the AABB::contains function returns false if a value is <= the lower bound
        ...
        schematicCoordToChunkCoord(destinationBox, frame->xTile, frame->zTile,
                                   rot, tileX, tileZ);
		...
		{
                        e->absMoveTo(tar#ifdef _DEBUG, targetZ,e->yRot,e"Adding entity type %d at (%f,%f,%f)\n"rintf(#endif,e->GetType(),e->x,e->y,e->z);

		e->setLevel(chunk->level);
		e->// default to being protected against despawning// 4J Stu - Until we can copy every type of entity, remove them from this std::vector// This means that the entities will only exist in the first use of the schematic that is processed//it = m_entities.erase(it);
		...
		++it;
                ...
                void ConsoleSchematicFile::generateSchematicFile(
                    DataOutputStream* dos, Level* level, int xStart, int yStart,
                    int zStart, int xEnd, int yEnd, int zEnd, bool bSaveMobs,
                    Compression::ECompressionTypes compressionType) {
                    assert(xEnd > xStart);
                    ass  // 4J Stu - Enforce even numbered positions to start
	...
	if (xStart > 0 && xStart % 2 != 0) xStart -= 1;
                    ...
                    if (zStart > 0 && zStart % 2 != 0)
                        zStart -= 1;
                    else if  // We want the end to be odd to have a total size
                             // that is
                             // even
                        if (xEnd > 0 && xEnd % 2 == 0)
                            xEnd += 1;
        ...
        int "Generating schematic file for area (%d,%d,%d) to (%d,%d,%d), %dx%dx%d\n",xStart,yStart,zStart,xEnd,yEnd,zEnd,xSize,ySize,zSize);

        if (dos != NULL) dos->writeInt(XBOX_SCHEMATIC_CURRENT_VERSION);

        if(dos//Write xSize->writeByte(compressionType);

	//Write ySize= NULL) dos->writeInt(xSize);

	//Write zSize= NULL) dos->writeInt(ySize);

	//byteArray rawBuffer = level->getBlocksAndData(xStart, yStart, zStart, xSize, ySize, zSize, false);
	int xRowSize = ySize * zSize;
	int blockCount = xSize * xRowSize// Position pointers into the data when not ordered by chunk
	int p = 0;
	...
	int y1 = yStart + ySize;
	if (y0 < 0) y0 = 0;
	if (y1 > Level::m// Every x is a whole row::maxBuildHeight;
	...
	for(int xPos = xStart; xPos < xStart + xSize; ++xPos)
	{
        ...
        for (int zPos = zStart; zPos < zStart + zSize;) {
            ...
            if (z0 < 0) z0 = 0;
            if (z1 > 16) z1 = 16;
                        getBlocksAndData(level->getChunk(xc, zc), &result, x0, y0, z0, x1, y1, z1, p, dataP, blockLi
	...
	CompoundTag tag;
	ListTag<CompoundTag> "tileEntities"ag = new ListTag<CompoundTag>(L);
...
void ConsoleSchematicFile::getBlocksAndData(LevelChunk *chunk, byteArray *data, int x0, int y0, int z0, int x1, int y1, int z1, int &blocksP, int // 4J Stu - Needs updated to work with higher worlds, should still work with non-optimised version below//int xs = x1 - x0;//int ys = y1 - y0;//int zs = z1 - z0;//if (xs * ys * zs == LevelChunk::BLOCKS_LENGTH)//{//	byteArray blockData = byteArray(data->data + blocksP, Level::CHUNK_TILE_COUNT);//	chunk->getBlockData(blockData);//	blocksP  += blockData.length;//	byteArray dataData = byteArray(data->data + dataP, 16384);//	chunk->getBlockLightData(dataData);//	dataP += dataData.length;//	byteArray blockLightData = byteArray(data->data + blockLightP, 16384);//	chunk->getBlockLightData(blockLightData);	//	blockLightP += blockLightData.length;//	byteArray skyLightData = byteArray(data->data + skyLightP, 16384);//	chunk->getSkyLightData(skyLightData);//	skyLightP += skyLightData.length;//	return;//}
		...
		for (int z = z0; z < z1; z++)
		{
            ...
            if (bHasUpper) {
                ...
                int len = (upperY1 - upperY0) / 2;
                                System::arraycopy(dataData, slot, data, dataP, len// 4J Stu - Allow ignoring light data dataData.data;
	...
	if(blockLightP > -1)
	{
                    ...
                    for (int x = x0; x < x1; x++)
                        for (int z = z0; z < z1; z++) {
                            if (bHasLower) {
                                int slot =
                                    (x << Level::genDepthBitsPlusFour |
                                     z << Level::genDepthBits | lowerY0) >>
                                    1;
                                int len = (lowerY1 - lowerY0) / 2;
                                System::arraycopy(blockLightData, slot, data,
                                                  blockLightP, len);
                                blockLightP += len;
                            }
                            if (bHasUpper) {
                                ...
                                System::arraycopy(blockLightData, slot, data,
                                                  blockLightP, len);
                                blockLightP +  // 4J Stu - Allow ignoring light
                                               // datatData.data;
                            }
	...
	if (skyLightP > -1) {
                                ...
                                for (int x = x0; x < x1; x++)
                                    for (int z = z0; z < z1; z++) {
                                        if (bHasLower) {
                                            int slot =
                                                (x << Level::
                                                         genDepthBitsPlusFour |
                                                 z << Level::genDepthBits |
                                                 lowerY0) >>
                                                1;
                                            int len = (lowerY1 - lowerY0) / 2;
                                            System::arraycopy(skyLightData,
                                                              slot, data,
                                                              skyLightP, len);
                                            skyLightP += len;
                                        }
                    ...
                    void ConsoleSchematicFile::setBlocksAndData(
                        LevelChunk * chunk, byteArray blockData,
                        byteArray dataData, byteArray inputData, int x0, int y0,
                            ...
                            for (int z = z0; z < z1; z++) {
                                if (bHasLower) {
                                    int slot =
                                        ...
                                        z << Level::genDepthBits | lowerY0;
                                    int len = lowerY1 - lowerY0;
                                    System::arraycopy(inputData, blocksP,
                                                      &blockData, slot, len);
                                    blocksP += len;
                                }
                                if (bHasUpper) {
	...
	if (skyLightP > -1) {
                                    ...
                                    ConsoleSchematicFile::
                                        getTileEntitiesInRegion(
                                            LevelChunk * chunk, int x0, int y0,
                                        ...
                                        for (AUTO_VAR(
                                                 it,
                                                 chunk->tileEntities.begin());
                                             it != chunk->tileEntities.end();
                                             ++it) {
