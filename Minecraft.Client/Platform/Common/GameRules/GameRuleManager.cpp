...
void GameRuleManager::saveGameRules(uint8_t** dOut, UINT* dSize) {
    ...
    for (UINT i = 0; i < 8; i++) dos.writeByte(0x0);
...
void GameRuleManager::writeRuleFile(DataOutputStream* dos) {
    ...
    // Write string lookup.
    int numStrings = ConsoleGameRules::eGameRuleType_Count +
...
bool GameRuleManager::readRuleFile(LevelGenerationOptions *lgo, uint8_t *dIn, UINT dSize, StringT//(DLCGameRulesFile *dlcFile, StringTable *strings)
{
	...
	for(int type = (int)ConsoleGameRules::eGameRuleType_Root; type < (int)ConsoleGameRules::eGameRuleType_Count; ++type)
	{
            for (UINT i = 0; i < numStrings; ++i) {
                if (tagsAndAtts[i].compare(wchTagNameA[type]) == 0) {
                                tagIdMap.insert( std::unordered_map<int, ConsoleGameRules::EGameRuleType>::value_type(i, (ConsoleGameRules::EGameRuleType// 4J-JEV: TODO: As yet unused.}
/*
	...
	for (UINT i = 0; i < numFiles; i++)
	{
                        ...
                        levelGenerator->loadSchematicFile(sFilename, ba.data,
                                                          ba.length);
...
void GameRuleManager::readAttributes(DataInputStream *dis, std::vector<std::wstring> *tagsAndAtts, GameRuleDefinition *rule)
{
                            ...
                            for (UINT att = 0; att < numAttrs; ++att) {
                                ...
                                if (rule != NULL)
                                    rule->addAttribute(tagsAndAtts->at(attID),
                                                       value);
                            }
}
...
void GameRuleManager::readChildren(DataInputStream *dis, std::vector<std::wstring> *tagsAndAtts, std::unordered_map<int, ConsoleGameRules::EGameRuleType> *tagIdMap, GameRuleDefinition *rule)
{
                            ...
                            for (UINT child = 0; child < numChildren; ++child) {
                                ...
                                readChildren(dis, tagsAndAtts, tagIdMap,
                                             childRule);
                            }
}
