...
public:
    ...
    static int DisplaySavingMessage(LPVOID pParam,
    ...
    LPVOID GetXuiServerActionParam(int iPad) {
    ...
    static int OldProfileVersionCallback(LPVOID pParam, unsigned char* pucData,
    ...
    static int DefaultOptionsCallback(LPVOID pParam,
...
#ifdef __ORBIS__
    static int OptionsDataCallback(LPVOID pParam, int iPad,
...
public:
    ...
    static void SignInChangeCallback(LPVOID pParam, bool bVal,
    ...
    static void NotificationsCallback(LPVOID pParam, DWORD dwNotification,
    ...
    static void UpsellReturnedCallback(LPVOID pParam, eUpsellType type,
    ...
    static int DLCMountedCallback(LPVOID pParam, int iPad, DWORD dwErr,
                                  DWORD dwLicenceMask);
    ...
    static int MarketplaceCountsCallback(LPVOID pParam,
    ...
    void AddMemoryTextureFile(const std::wstring& wName, PBYTE pbData,
                              DWORD dwBytes);
    void RemoveMemoryTextureFile(const std::wstring& wName);
    void GetMemFileDetails(const std::wstring& wName, PBYTE* ppbData,
                           DWORD* pdwBytes);
    ...
    void ProcessInvite(DWORD dwUserIndex, DWORD dwLocalUsersMask,
...
private:
    BYTE m_playerColours[MINECRAFT_NET_MAX_PLAYERS];  // An array of QNet
                                                      // small-id's
    unsigned int m_playerGamePrivileges[MINECRAFT_NET_MAX_PLAYERS];

public:
    void UpdatePlayerInfo(BYTE networkSmallId, SHORT playerColourIndex,
    ...
    static int DLCOffersReturned(void* pParam, int iOfferC, DWORD dwType,
                                 int iPad);
    DWORD GetDLCContentType(eDLCContentType eType) {
...
#ifdef _XBOX_ONE
    static int TMSPPFileReturned(LPVOID pParam, int iPad, int iUserData, LPVOID,
                                 WCHAR* wchFilename);
    std::unordered_map<std::wstring, DLC_INFO*>* GetDLCInfo();
#else
    static int TMSPPFileReturned(LPVOID pParam, int iPad, int iUserData,
...
public:
    ...
    void SetAdditionalSkinBoxes(DWORD dwSkinID, SKIN_BOX* SkinBoxA,
                                DWORD dwSkinBoxC);
    ...
    void SetAnimOverrideBitmask(DWORD dwSkinID,
    ...
    void SetRequiredTexturePackID(DWORD dwID) {
        m_dwRequiredTexturePackID = dwID;
    }

    virtual void GetFileFromTPD(eTPDFileType eType, PBYTE pbData, DWORD dwBytes,
                                PBYTE* ppbData, DWORD* pdwBytes) {
        *ppbData = NULL;
        *pdwBytes = 0;
    }
...
private:
    // std::vector of additional skin model parts, indexed by the skin texture
    // id
...
public:
    ...
    std::wstring getFilePath(DWORD packId, std::wstring filename,
                             bool bAddDataFolder,
...
private:
    ...
    std::wstring getRootPath(DWORD packId, bool allowOverride,
