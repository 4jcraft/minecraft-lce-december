...
INT CTelemetryManager::GetMode(DWORD dwUserId) {
    ...
    if (pMinecraft->localplayers[dwUserId] != NULL &&
        pMinecraft->localplayers[dwUserId]->level != NULL &&
        pMinecraft->localplayers[dwUserId]->level->getLevelData() != NULL) {
        GameType* gameType = pMinecraft->localplayers[dwUserId]
                                 ->level->getLevelData()
                                 ->getGameType();
...
builds
*/
INT CTelemetryManager::GetTitleBuildId() { return (INT)VER_PRODUCTBUILD; }
...
*/
INT CTelemetryManager::GetLevelInstanceID() { return (INT)m_levelInstanceID; }
...
INT CTelemetryManager::GetSingleOrMultiplayer() {
    ...
    if (app.GetLocalPlayerCount() == 1 &&
        ...
        singleOrMultiplayer = (INT)eSen_SingleOrMultiplayer_Multiplayer_Live;
               ...
               g_NetworkManager.GetOnlinePlayerCount() > 0) {
        singleOrMultiplayer =
            (INT)eSen_SingleOrMultiplayer_Multiplayer_Both_Local_and_Live;
    }
