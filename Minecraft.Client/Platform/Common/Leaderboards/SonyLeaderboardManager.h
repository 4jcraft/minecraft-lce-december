...
protected:
    ...
    EStatsState m_eStatsStat  // State of the stats read//	EFilterMode
                              // m_eFilterMode;

        ReadScore* m_scores;
        unsigned int m_maxRa//SceNpScoreRankData *m_stats;

public:
	virtual void Tick(//Open a session
	virtual bool OpenSession(//Close a session
	virtual void CloseSession(//Delete a session
	virtual void DeleteSession(//Write the given stats//This is called synchronously and will not free any memory allocated for views when it is done
	...
	virtual bool ReadStats_TopRank(LeaderboardReadListener *callback, int difficulty, EStatsType type, unsigned int startIndex, unsigned int readCount//Perform a flush of the stats
	virtual void FlushStats(//Cancel the current operation
	virtual void CancelOperation(//Is the leaderboard manager idle.
	virtual bool isIdle();
...
protect// 4J-JEV: Interface differences:// Sce NP score library function redirects.
