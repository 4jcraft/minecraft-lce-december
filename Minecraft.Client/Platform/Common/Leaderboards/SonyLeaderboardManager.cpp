...
bool SonyLeaderboardManager::getScoreByIds() {
    ...
    return tru                                           // Error.     
        error3 : if (ret != SCE_NP_COMMUNITY_ERROR_ABOR  // 0x8002a109     
                     ...
                     error1 : if (m_eStatsState != eStatsState_Canceled)
                         m_eStatsState = eStatsState_Failed;
                     app.DebugPr
...
void SonyLeaderboardManager::Tick()
{
            ...
            switch (m_eStatsState) {
                ...
                case eStatsState_Failed: {
                    ...
                    if (m_readListener != NULL)
                        m_readListener->OnStatsReadComplete(
                            eStatsReturn_NetworkError, 0, view);
        ...
        switch (statsType) {
            ...
            default:
                ...
                SceNpScoreRankData* SonyLeaderboardManager::filterJustScorers(
                    ...
                    int num2 = 0;
                    for (int i = 0; i < num; i++)
                        if (friendsData[i].hasData) num2++;
                    num = num2;
                    num2 = 0;
                    SceNpScoreRankData* out = new SceNpScoreRankData[num];
                    for (int i = 0; i < num; i++)
                        if (friendsData[i].hasData)
                ...
                */      // 4J-JEV: Unused, here if we want to switch LeaderboardManager::ViewOut to 'SceNpScorePlayerRankData'.     
                    ...
                    SceNpScorePlayerRankData* out =
                        new SceNpScorePlayerRankData[num];
                    for (int i = 0; i < num; i++) {
                        out[i].hasData = true;
                    ...
                    void SonyLeaderboardManager::toBinary(
                        void* out, SceNpScoreComment* in) {
                        std::string decoded =
                            base64_decode(std::string((char*)in));
                        memcpy(out, decoded.c_str(), RECORD_SIZE);
                    }

                    void SonyLeaderboardManager::fromBinary(
                        SceNpScoreComment * *out, void* in) {
                        *out = (SceNpScoreComment*)new unsigned char
                            [SCE_NP_SCORE_COMMENT_MAXLEN + 1];
                        std::string encoded = base64_encode(
                            (unsigned char const*)in, RECORD_SIZE);
                        memcpy(out, encoded.c_str(),
                               SCE_NP_SCORE_COMMENT_MAXLEN);
                    }

                    void SonyLeaderboardManager::toBase32(
                        SceNpScoreComment * out, void* in) {
			...
			assert(false);
                    ...
                    void SonyLeaderboardManager::fromBase32(
                        void* out, SceNpScoreComment* in) {
