...
if (uiTempDisabled) {
		...
		if(hintsOn)
		{
            ...
            if (!m_allTutorialsComplete &&
                ...
                if (currentFailedConstraint[m_CurrentState] != NULL &&
                    currentFailedConstraint[m_CurrentState]
                        ->isConstraintSatisfied(m_iPad)) {
	...
	if(!m_allTutorialsComplete && (taskChanged || m_hasStateChanged) )
	{
                ...
                for (unsigned int state = 0; state < e_Tutorial_State_Max;
                     ++state) {
		...
		)
	{
                if (currentFailedConstraint[m_CurrentState] != NULL) {
                    ...
                    message->m_promptId =
                        currentTask[m_CurrentState]->getPromptId();
                    message->m_allowFade =
                        currentTask[m_CurrentState]->AllowFade();
                    setMessage(message);
                    currentTask[m_CurrentState]->TaskReminders()
                        ? m_iTaskReminders = 1
                        : m_iTaskReminders = 0;
                } else {
                    setMessage(NULL);
...
bool Tutorial::setMessage(PopupMessageDetails *message)
{
            ...
            if (message != NULL && (message->m_messageId > 0 ||
                                    !message->m_messageString.empty())) {
                m_lastMessageState = m_CurrentState;
...
// If clearing the scene, make sure that the tutorial popup has its reference to this scene removed#ifndef _XBOX
	...
	bool hintsOn = m_isFullTutorial || app.GetGameSettings(m_// If we have completed this state, and it is one that occurs during normal gameplay then change back to the gameplay track
        ...
        activeTasks[m_CurrentState].erase(std::find(
            ...
            activeTasks[m_CurrentState].end(), currentTask[m_CurrentState]));
