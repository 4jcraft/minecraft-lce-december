...
protected:
    ...
    std::vector<TutorialTask*>
        ta  // We store a copy of the tasks for the main gameplay tutorial so
            // that we could display an overview menu
            std::vector<TutorialTask*>
                activeTasks[e_Tutorial_State_Max];
    ...
    bool m_freezeTime;
bool m_timeFroz  // D3DXVECTOR3 m_OriginalPosition;

    public : DWORD lastMessageTime;
...
public:
    ...
    void setHintCompleted(eTutorial_Hint hint);
        void setHintCompleted( TutorialHint *hint // completableId will be either a eTutorial_State value or eTutorial_Hint
	void setCompleted( int completableId );
	bool getCompleted( int completableI
#ifdef _XBOX
	void changeTutorialState(eTutorial_State newState, \
                                 CXuiScene* scene = NU #else 
	void changeTutorialState(eTutorial_State newState, \
                                 UIScene* scene = NU #endif 
	...
	int	GetTutorialDisplayMessageTime() {
        return m_iTutorialDisplayMessageTime  // Only for the main gameplay
                                              // tutorial
                   std::vector<TutorialTask*> *
               getTasks();
