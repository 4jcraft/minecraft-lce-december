...
void SQRNetworkPlayer::SendInternal(const void* data, unsigned int dataSize,
    ...
    bool bOutstandingPackets =
        (m_sendQueue.size() >
         0);  // check if there are still packets in the queue, we won't be
              // calling SendMoreInternal here if there are
    ...
    // if the queue had something in it already, then the UDP callback will fire
    // and call SendMoreInternal so we don't call it here, to avoid a deadlock
...
int SQRNetworkPlayer::WriteDataPacket(const void* data, int dataSize,
                                      AckFlags ackFlags) {
    ...
    *((DataPacketHeader*)packetData) = header;
...
#ifdef __PS3__
    int ret = cellRudpWrite(m_rudpCtx, packetData, packetSize,
                            0);  // CELL_RUDP_MSG_LATENCY_CRITICAL  );
#else                            // __ORBIS__ && __PSVITA__
    int ret = sceRudpWrite(m_rudpCtx, packetData, packetSize,
                           0);  // SCE_RUDP_MSG_LATENCY_CRITICAL  );
#endif
    if (ret == sc_wouldBlockFlag) {
        ...
        assert(ret == packetSize ||
               ret >
                   headerSize);  // we must make sure we've sent the entire
                                 // packet or the header and some data at least
        ret -= headerSize;
        if (ackFlags == e_flag_AckRequested) {
...
int SQRNetworkPlayer::ReadDataPacket(void* data, int dataSize) {
    ...
    if (header.GetAckFlags() == e_flag_AckRequested) {
        // Don't send the ack back directly from here, as this is called from a
        // rudp event callback, and we end up in a thread lock situation between
        // the lock librudp uses internally (which is locked already here since
        // we are being called in the event handler), and our own lock that we
        // do for processing our write queue
...
void SQRNetworkPlayer::ReadAck() {
    ...
    app.DebugPrintf(
        "RUDP ctx : %d : Time taken for ack - %4d ms : min - %4d : max %4d : "
        "avg %4d\n",
...
void SQRNetworkPlayer::SendMoreInternal() {
    EnterCriticalSection(&m_csQueue);
    assert(m_sendQueue.size() >
           0);  // this should never be called with an empty queue.
