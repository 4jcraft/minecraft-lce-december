...
bool SonyCommerce::m_contextCreated =
    false;  ///< npcommerce2 context ID created?
SonyCommerce::Phase SonyCommerce::m_currentPhase =
...
void SonyCommerce::CheckForTrialUpgradeKey_Callback(LPVOID param,
                                                    bool bFullVersion) {
    ...
    if (ProfileManager.IsFullVersion()) {
        ...
        ProfileManager.InitialiseTrophies(
            SQRNetworkManager_PS3::GetSceNpCommsId(),
            SQRNetworkManager_PS3::GetSceNpCommsSig());
    }
    m_bLicenseChecked = true;
}

bool SonyCommerce::LicenseChecked() { return m_bLicenseChecked; }
...
int SonyCommerce::getProductList(std::vector<ProductInfo>* productList,
                                 char* categoryId) {
    int ret = 0;
    uint32_t requestId;
    ...
    ret = sceNpCommerce2GetCategoryContentsStart(
        requestId, categoryId, 0, SCE_NP_COMMERCE2_GETCAT_MAX_COUNT);
    if (ret < 0) {
    ...
    ret = sceNpCommerce2GetCategoryContentsGetResult(
        requestId, m_commercebuffer, bufSize, &fillSize);
    if (ret < 0) {
        sceNpCommerce2DestroyReq(requestId);
        setError(ret);
        return ret;
    ...
    ret = sceNpCommerce2InitGetCategoryContentsResult(&result, m_commercebuffer,
                                                      fillSize);
    if (ret < 0) {
        setError(ret);
        return ret;
    ...
    for (int i = 0; i < result.rangeOfContents.count; i++) {
        ...
        if (contentInfo.contentType == SCE_NP_COMMERCE2_CONTENT_TYPE_PRODUCT) {
            ...
            // Get product info
            ret = sceNpCommerce2GetGameProductInfoFromContentInfo(&contentInfo,
                                                                  &productInfo);
            if (ret < 0) {
                sceNpCommerce2DestroyGetCategoryContentsResult(&result);
                setError(ret);
            ...
            if (productInfo.countOfSku == 1) {
                ...
                // but also keep the price as a value in case it's 0 - we need
                // to show "free" for that
                tempInfo.ui32Price = skuInfo.price;
                ret = sceNpCommerce2GetPrice(m_contextId, tempInfo.price,
                                             sizeof(tempInfo.price),
                                             skuInfo.price);
    ...
    if (categoryInfo.countOfSubCategory > 0) {
        ...
        for (int i = 0; i < result.rangeOfContents.count; i++) {
            ...
            if (contentInfo.contentType ==
                SCE_NP_COMMERCE2_CONTENT_TYPE_CATEGORY) {
                ret = sceNpCommerce2GetCategoryInfoFromContentInfo(
...
int SonyCommerce::getDetailedProductInfo(ProductInfoDetailed* pInfo,
    ...
    ret = sceNpCommerce2GetProductInfoGetResult(requestId, m_commercebuffer,
                                                bufSize, &fillSize);
    if (ret < 0) {
        sceNpCommerce2DestroyReq(requestId);
        m_errorCode = ret;
    ...
    ret = sceNpCommerce2InitGetProductInfoResult(&result, m_commercebuffer,
                                                 fillSize);
    if (ret < 0) {
        m_errorCode = ret;
        return ret;
    ...
    // Get rating info
    ret = sceNpCommerce2GetContentRatingInfoFromGameProductInfo(&productInfo,
                                                                &ratingInfo);
    if (ret < 0) {
        sceNpCommerce2DestroyGetProductInfoResult(&result);
        m_errorCode = ret;
    ...
    for (int index = 0; index < ratingInfo.countOfContentRatingDescriptor;
         index++) {
    ...
    strncpy(pInfo->productId, productInfo.productId,
            SCE_NP_COMMERCE2_PRODUCT_ID_LEN);
    strncpy(pInfo->productName, productInfo.productName,
            SCE_NP_COMMERCE2_PRODUCT_NAME_LEN);
    strncpy(pInfo->shortDescription, productInfo.productShortDescription,
            SCE_NP_COMMERCE2_PRODUCT_SHORT_DESCRIPTION_LEN);
    strncpy(pInfo->longDescription, productInfo.productLongDescription,
            SCE_NP_COMMERCE2_PRODUCT_LONG_DESCRIPTION_LEN);
    strncpy(pInfo->legalDescription, productInfo.legalDescription,
    ...
    strncpy(pInfo->ratingSystemId, ratingInfo.ratingSystemId,
            SCE_NP_COMMERCE2_RATING_SYSTEM_ID_LEN);
    strncpy(pInfo->ratingImageUrl, ratingInfo.imageUrl,
            SCE_NP_COMMERCE2_URL_LEN);

    // Get SKU info
    if (productInfo.countOfSku == 1) {
        ret = sceNpCommerce2GetGameSkuInfoFromGameProductInfo(&productInfo, 0,
                                                              &skuInfo);
        if (ret < 0) {
            sceNpCommerce2DestroyGetProductInfoResult(&result);
            m_errorCode = ret;
        ...
        ret = sceNpCommerce2GetPrice(m_contextId, pInfo->price,
                                     sizeof(pInfo->price), skuInfo.price);
        if (ret < 0) {
            sceNpCommerce2DestroyGetProductInfoResult(&result);
            m_errorCode = ret;
            return ret;
...
int SonyCommerce::addDetailedProductInfo(ProductInfo* info,
                                         const char* productId,
                                         char* categoryId) {
    int ret = 0;
    uint32_t requestId;
    ...
    // 	strncpy(tempInfo.productId, productInfo.productId,
    // SCE_NP_COMMERCE2_PRODUCT_ID_LEN); 	strncpy(tempInfo.productName,
    // productInfo.productName, SCE_NP_COMMERCE2_PRODUCT_NAME_LEN);
    // 	strncpy(tempInfo.shortDescription, productInfo.productShortDescription,
    // SCE_NP_COMMERCE2_PRODUCT_SHORT_DESCRIPTION_LEN);
    ...
    if (productInfo.countOfSku == 1) {
        ...
        ret = sceNpCommerce2GetPrice(m_contextId, info->price,
                                     sizeof(info->price), skuInfo.price);
        if (ret < 0) {
            sceNpCommerce2DestroyGetProductInfoResult(&result);
            m_errorCode = ret;
            return ret;
        ...
        // 4J-PB - more than one sku id! We have to be able to use the sku id
        // returned for a product, so there is not supposed to be more than 1
...
int SonyCommerce::checkout(CheckoutInputParams& params) {
    ...
    ret = sceNpCommerce2DoCheckoutStartAsync(
        m_contextId, skuIdsTemp, params.skuIds.size(), *params.memContainer);
    if (ret < 0) {
        return ret;
    }
...
int SonyCommerce::downloadList(DownloadListInputParams& params) {
    ...
    ret = sceNpCommerce2DoDlListStartAsync(
        m_contextId, app.GetCommerceCategory(), skuIdsTemp,
...
void SonyCommerce::UpgradeTrialCallback2(LPVOID lpParam, int err) {
    ...
    if (err != CELL_OK) {
        ...
        C4JStorage::EMessageResult result =
            ui.RequestMessageBox(IDS_PRO_UNLOCKGAME_TITLE, IDS_NO_DLCOFFERS,
...
int SonyCommerce::createContext() {
    ...
    if (m_contextCreated) {
        ...
        if (ret < 0) {
            app.DebugPrintf(4,
                            "createContext sceNpCommerce2DestroyCtx problem\n");
            return ret;
        }
    }
...
void SonyCommerce::commerce2Handler(uint32_t contextId, uint32_t subjectId,
                                    int event, int errorCode, void* arg) {
    // 	Event reply;
    ...
    switch (msg) {
        ...
        case e_message_commerceGetDetailedProductInfo: {
            ...
            if (ret < 0) {
                ...
                app.DebugPrintf(4,
                                "e_event_commerceGotDetailedProductInfo - %s\n",
                                m_pCategoryID);
            }
            break;
        }
...
void SonyCommerce::GetProductList(CallbackFunc cb, LPVOID lpParam,
...
void SonyCommerce::GetDetailedProductInfo(CallbackFunc cb, LPVOID lpParam,
...
void SonyCommerce::AddDetailedProductInfo(CallbackFunc cb, LPVOID lpParam,
...
void SonyCommerce::GetCategoryInfo(CallbackFunc cb, LPVOID lpParam,
                                   CategoryInfo* info, const char* categoryId) {
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
...
void SonyCommerce::Checkout(CallbackFunc cb, LPVOID lpParam,
                            const char* skuID) {
    if (m_memContainer != SYS_MEMORY_CONTAINER_ID_INVALID) {
        return;
    }
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
    int ret = sys_memory_container_create(
        &m_memContainer, SCE_NP_COMMERCE2_DO_CHECKOUT_MEMORY_CONTAINER_SIZE);
    if (ret < 0) {
...
void SonyCommerce::DownloadAlreadyPurchased(CallbackFunc cb, LPVOID lpParam,
                                            const char* skuID) {
    if (m_memContainer != SYS_MEMORY_CONTAINER_ID_INVALID) return;
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
    int ret = sys_memory_container_create(
        &m_memContainer, SCE_NP_COMMERCE2_DO_CHECKOUT_MEMORY_CONTAINER_SIZE);
