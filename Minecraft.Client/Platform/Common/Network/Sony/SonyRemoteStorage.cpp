...
// void remoteStorageGetCallback(LPVOID lpParam, SonyRemoteStorage::Status s,
// int error_code)
// {
// 	app.DebugPrintf("remoteStorageGetCallback err : 0x%08x\n");
// }
//
// void remoteStorageCallback(LPVOID lpParam, SonyRemoteStorage::Status s, int
// error_code)
// {
...
void SonyRemoteStorage::SetRetrievedDescData() {
    DescriptionData* pDescDataTest =
        (DescriptionData*)m_remoteFileInfo->fileDescription;
    ESavePlatform testPlatform = (ESavePlatform)MAKE_FOURCC(
        pDescDataTest->m_platform[0], pDescDataTest->m_platform[1],
        pDescDataTest->m_platform[2], pDescDataTest->m_platform[3]);
    if (testPlatform == SAVE_FILE_PLATFORM_NONE) {
        // new version of the descData
        DescriptionData_V2* pDescData2 =
            (DescriptionData_V2*)m_remoteFileInfo->fileDescription;
        m_retrievedDescData.m_descDataVersion =
            GetU32FromHexBytes(pDescData2->m_descDataVersion);
        m_retrievedDescData.m_savePlatform = (ESavePlatform)MAKE_FOURCC(
            pDescData2->m_platform[0], pDescData2->m_platform[1],
            pDescData2->m_platform[2], pDescData2->m_platform[3]);
        m_retrievedDescData.m_seed = GetU64FromHexBytes(pDescData2->m_seed);
        m_retrievedDescData.m_hostOptions =
            GetU32FromHexBytes(pDescData2->m_hostOptions);
        m_retrievedDescData.m_texturePack =
            GetU32FromHexBytes(pDescData2->m_texturePack);
        m_retrievedDescData.m_saveVersion =
            GetU32FromHexBytes(pDescData2->m_saveVersion);
        memcpy(m_retrievedDescData.m_saveNameUTF8, pDescData2->m_saveNameUTF8,
               sizeof(pDescData2->m_saveNameUTF8));
        assert(m_retrievedDescData.m_descDataVersion > 1 &&
               m_retrievedDescData.m_descDataVersion <=
                   sc_CurrentDescDataVersion);
    } else {
        ...
        m_retrievedDescData.m_hostOptions =
            GetU32FromHexBytes(pDescData->m_hostOptions);
        m_retrievedDescData.m_texturePack =
            GetU32FromHexBytes(pDescData->m_texturePack);
        m_retrievedDescData.m_saveVersion =
            SAVE_FILE_VERSION_COMPRESSED_CHUNK_STORAGE;  // the last save
                                                         // version before we
                                                         // added it to this
                                                         // data
        memcpy(m_retrievedDescData.m_saveNameUTF8, pDescData->m_saveNameUTF8,
               sizeof(pDescData->m_saveNameUTF8));
    }
}

void getSaveInfoReturnCallback(LPVOID lpParam, SonyRemoteStorage::Status s,
                               int error_code) {
...
static void getSaveInfoInitCallback(LPVOID lpParam, SonyRemoteStorage::Status s,
                                    int error_code) {
    SonyRemoteStorage* pRemoteStorage = (SonyRemoteStorage*)lpParam;
    if (error_code != 0) {
...
static void setSaveDataInitCallback(LPVOID lpParam, SonyRemoteStorage::Status s,
                                    int error_code) {
    SonyRemoteStorage* pRemoteStorage = (SonyRemoteStorage*)lpParam;
    if (error_code != 0) {
...
int SonyRemoteStorage::LoadSaveDataThumbnailReturned(LPVOID lpParam,
                                                     PBYTE pbThumbnail,
...
int SonyRemoteStorage::getDataProgress() {
    ...
    int progVal =
        m_dataProgress + (estimatedTransfered / float(totalSize)) * 100;
    if (progVal > nextChunk) return nextChunk;
    if (progVal > 99) {
...
void SonyRemoteStorage::GetDescriptionData(DescriptionData& descData) {
    ...
    if (m_thumbnailData) {
        ...
        DWORD uiTexturePack;
        char seed[22];
        app.GetImageTextData(m_thumbnailData, m_thumbnailDataSize,
