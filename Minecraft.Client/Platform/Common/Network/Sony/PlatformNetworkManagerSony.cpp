...
bool CPlatformNetworkManagerSony::IsLocalGame() { return m_bIsOfflineGame; }
bool CPlatformNetworkManagerSony::IsPrivateGame() { return m_bIsPrivateGame; }
bool CPlatformNetworkManagerSony::IsLeavingGame() { return m_bLeavingGame; }
void CPlatformNetworkManagerSony::ResetLeavingGame() { m_bLeavingGame = false; }

void CPlatformNetworkManagerSony::HandleStateChange(
    ...
    static const char* c_apszStateNames[] = {
        "SNM_STATE_INITIALISING", "SNM_STATE_INITIALISE_FAILED",
        "SNM_STATE_IDLE",         "SNM_STATE_HOSTING",
        "SNM_STATE_JOINING",      "SNM_STATE_STARTING",
        "SNM_STATE_PLAYING",      "SNM_STATE_LEAVING",
        "SNM_STATE_ENDING",
    };
    ...
    if (newState == SQRNetworkManager::SNM_STATE_HOSTING) {
        ...
        // 4J Stu - We may be accepting an invite from the DLC menu, so hide the
        // icon
    ...
    } else if (newState == SQRNetworkManager::SNM_STATE_IDLE &&
               oldState == SQRNetworkManager::SNM_STATE_JOINING) {
        if (idleReasonIsSessionFull) {
            ...
            g_NetworkManager.StateChange_JoiningToIdle(JOIN_FAILED_NONSPECIFIC);
               ...
               oldState == SQRNetworkManager::SNM_STATE_HOSTING) {
        m_bLeavingGame = true;
    ...
    // loading to created world. 4J Stu - If the client joins just as the host
    // is exiting, then they can skip to leaving without passing through ending
    else if (newState == SQRNetworkManager::SNM_STATE_ENDING) {
        g_NetworkManager.StateChange_AnyToEnding(
            oldState == SQRNetworkManager::SNM_STATE_PLAYING);

        // 4J-PB - Only the host can leave here - the clients will hang if
        // m_bLeavingGame is set to true here
        if (m_pSQRNet->IsHost()) {
            m_bLeavingGame = true;
        }
    }
    ...
    if (newState == SQRNetworkManager::SNM_STATE_IDLE) {
        // On PS3, sometimes we're getting a SNM_STATE_ENDING transition to
        // SNM_STATE_IDLE on joining, because the server context being deleted
        // sets the state away from SNM_STATE_JOINING before we detect the cause
        // for the disconnection. This means we don't pick up on the
        // joining->idle transition. Set disconnection reason here too for this
        // case.
        if (idleReasonIsSessionFull) {
...
void CPlatformNetworkManagerSony::HandleDataReceived(
    SQRNetworkPlayer* playerFrom, SQRNetworkPlayer* playerTo,
    ...
    if (playerTo->IsHost()) {
        ...
        // app.DebugPrintf( "Pushing data into host read queue for user
        ...
        if (socket != NULL) socket->pushDataToQueue(data, dataSize, false);
    } else {
        // If we are not the host the message must have come from the host, so
        // we care more about who it is addressed to
        ...
        if (socket != NULL) socket->pushDataToQueue(data, dataSize);
    }
}

void CPlatformNetworkManagerSony::HandlePlayerJoined(
    ...
    NetworkPlayerSony* networkPlayer =
        (NetworkPlayerSony*)addNetworkPlayer(pSQRPlayer);
    ...
    if (pSQRPlayer->IsLocal()) {
        ...
        if (m_pSQRNet->IsHost() && !m_bHostChanged) {
            ...
            for (AUTO_VAR(it, m_machineSQRPrimaryPlayers.begin());
                 it < m_machineSQRPrimaryPlayers.end(); ++it) {
    ...
    if (true)  // TODO m_pSQRNet->GetState() == QNET_STATE_GAME_PLAY)
    {
        ...
        for (unsigned int idx = 0; idx < XUSER_MAX_COUNT; ++idx) {
            if (m_pSQRNet->GetLocalPlayerByUserIndex(idx) != NULL)
                ++localPlayerCount;
        }
...
void CPlatformNetworkManagerSony::HandlePlayerLeaving(
    SQRNetworkPlayer* pSQRPlayer) {
    //__debugbreak();
    ...
    if (networkPlayer) {
        ...
        if (socket != NULL) {
            ...
            // We may get here either from the player requesting to exit the
            // game, in which case we they will already have left the game
            // server, or from a disconnection where we then have to remove them
            ...
            // TODO 4J Stu - We may still be using this at the point that the
            // player leaves the session. We need this as long as the game
            // server still needs to communicate with the player
            // delete socket;
        ...
        if (m_pSQRNet->IsHost() && !m_bHostChanged) {
            if (isSystemPrimaryPlayer(pSQRPlayer)) {
                ...
                for (unsigned int i = 0; i < m_pSQRNet->GetPlayerCount(); ++i) {
                    SQRNetworkPlayer* pSQRPlayer2 =
                        m_pSQRNet->GetPlayerByIndex(i);

                    if (pSQRPlayer2 != NULL && pSQRPlayer2 != pSQRPlayer &&
                        pSQRPlayer2->IsSameSystem(pSQRPlayer)) {
...
// Update our external data to match the current internal player slots, and
// resync back out (host only)
void CPlatformNetworkManagerSony::HandleResyncPlayerRequest(
    ...
    for (int i = 0; i < SQRNetworkManager::MAX_ONLINE_PLAYER_COUNT; i++) {
        if (aPlayers[i]) {
            ...
            memset(&m_hostGameSessionData.players[i], 0,
                   sizeof(m_hostGameSessionData.players[i]));
        }
    }
...
void CPlatformNetworkManagerSony::HandleDisconnect(bool bLostRoomOnly,
                                                   bool bPSNSignOut) {
    g_NetworkManager.HandleDisconnect(bLostRoomOnly, bPSNSignOut);
}

void CPlatformNetworkManagerSony::HandleInviteReceived(
...
bool CPlatformNetworkManagerSony::Initialise(
    ...
    // 4J-PB - seems we can't initialise both adhoc and psn comms - from Rohan -
    // "having adhoc matching and matching2 library initialised together results
    // in undesired behaviour", but probably having other parts initialised also
    // is 'undesirable'

    m_pSQRNet_Vita->Initialise();

    if (ProfileManager.IsSignedInPSN(ProfileManager.GetPrimaryPad())) {
        // we're signed into the PSN, but we won't be online yet, force a
        // sign-in online here
...
void CPlatformNetworkManagerSony::Terminate() { m_pSQRNet->Terminate(); }
...
int CPlatformNetworkManagerSony::CorrectErrorIDS(int IDS) {
    // Attempts to remap the following messages to provide something that PS3
    // TCRs are happier with
    //
    ...
    // Map to:
    //
    // IDS_ERROR_NETWORK				    - "A network error
    // has occurred" IDS_ERROR_NETWORK_TITLE			    - "Network
    // Error" IDS_ERROR_NETWORK_EXIT			    - "A network error
    // has occurred. Exiting to Main Menu." IDS_ERROR_PSN_SIGN_OUT
    // - You have been signed out from the "PSN". IDS_ERROR_PSN_SIGN_OUT_EXIT
    // - You have been signed out from the "PSN". Exiting to Main Menu

    // Determine if we'd prefer to present errors as a signing out issue, rather
    // than a network issue, based on whether we have a network connection at
    // all or not
...
bool CPlatformNetworkManagerSony::isSystemPrimaryPlayer(
    SQRNetworkPlayer* pSQRPlayer) {
...
bool CPlatformNetworkManagerSony::RemoveLocalPlayerByUserIndex(int userIndex) {
    SQRNetworkPlayer* pSQRPlayer =
        m_pSQRNet->GetLocalPlayerByUserIndex(userIndex);
    ...
    if (pNetworkPlayer != NULL) {
        ...
        if (socket != NULL) {
            // We can't remove the player from qnet until we have stopped using
            // it to communicate
            C4JThread* thread =
                new C4JThread(&CPlatformNetworkManagerSony::
                                  RemovePlayerOnSocketClosedThreadProc,
                              pNetworkPlayer, "RemovePlayerOnSocketClosed");
...
bool CPlatformNetworkManagerSony::IsAddingPlayer() { return false; }
...
bool CPlatformNetworkManagerSony::LeaveGame(bool bMigrateHost) {
    ...
    // TODO Possibly need to do multiple objects depending on how split screen
    // online works
    SQRNetworkPlayer* pSQRPlayer =
        m_pSQRNet->GetLocalPlayerByUserIndex(g_NetworkManager.GetPrimaryPad());
    INetworkPlayer* pNetworkPlayer = getNetworkPlayer(pSQRPlayer);
...
bool CPlatformNetworkManagerSony::_LeaveGame(bool bMigrateHost,
                                             bool bLeaveRoom) {
    // 4J Stu - Fix for #10490 - TCR 001 BAS Game Stability: When a party of
    // four players leave a world to join another world without saving the title
    // will crash. Changed this to make it threadsafe
    m_bLeavingGame =
        true;  // Added for Sony platforms but unsure why the 360 doesn't need
               // it - without this, the leaving triggered by this causes the
               // game to respond by leaving again when it transitions to the
               // SNM_STATE_ENDING state
...
void CPlatformNetworkManagerSony::HostGame(
...
void CPlatformNetworkManagerSony::_HostGame(
    ...
    m_pSQRNet->CreateAndJoinRoom(
        g_NetworkManager.GetPrimaryPad(), usersMask, &m_hostGameSessionData,
        sizeof(m_hostGameSessionData),
        IsLocalGame());  // Should be using: g_NetworkManager.GetLockedProfile()
                         // but that isn't being set currently
}
...
int CPlatformNetworkManagerSony::JoinGame(FriendSessionInfo* searchResult,
                                          int localUsersMask,
    ...
    if ((gameSession->playerCount + joinPlayerCount) >
        SQRNetworkManager::MAX_ONLINE_PLAYER_COUNT) {
...
bool CPlatformNetworkManagerSony::SetLocalGame(bool isLocal) {
    if (m_pSQRNet->GetState() == SQRNetworkManager::SNM_STATE_IDLE) {
        ...
        app.DebugPrintf(
            "Tried to change session type while not in idle or offline "
            "state\n");
    }
...
void CPlatformNetworkManagerSony::RegisterPlayerChangedCallback(
    int iPad,
...
void CPlatformNetworkManagerSony::UnRegisterPlayerChangedCallback(
    int iPad,
    void (*callback)(void* callbackParam, INetworkPlayer* pPlayer,
                     bool leaving),
    void* callbackParam) {
...
// Note that this does less than the xbox equivalent as we have
// HandleResyncPlayerRequest that is called by the underlying SQRNetworkManager
// when players are added/removed etc., so this call is only used to update the
// game host settings & then do the final push out of the data.
void CPlatformNetworkManagerSony::UpdateAndSetGameSessionData(
    ...
    // If this is called With a pNetworkPlayerLeaving, then the call has
    // ultimately started within SQRNetworkManager::RemoveRemotePlayersAndSync,
    // so we don't need to sync each change as that function does a sync at the
    // end of all changes.
...
int CPlatformNetworkManagerSony::RemovePlayerOnSocketClosedThreadProc(
    void* lpParam) {
...
bool CPlatformNetworkManagerSony::RemoveLocalPlayer(
    INetworkPlayer* pNetworkPlayer) {
    if (pNetworkPlayer->IsLocal()) {
        return m_pSQRNet->RemoveLocalPlayerByUserIndex(
...
CPlatformNetworkManagerSony::PlayerFlags::~PlayerFlags() { delete[] flags; }
...
// m_playerFlags std::vector without any gaps in it
void CPlatformNetworkManagerSony::SystemFlagRemovePlayer(
    INetworkPlayer* pNetworkPlayer) {
    for (unsigned int i = 0; i < m_playerFlags.size(); i++) {
...
// shares that system
void CPlatformNetworkManagerSony::SystemFlagSet(INetworkPlayer* pNetworkPlayer,
                                                int index) {
...
// here
bool CPlatformNetworkManagerSony::SystemFlagGet(INetworkPlayer* pNetworkPlayer,
                                                int index) {
...
void CPlatformNetworkManagerSony::TickSearch() {
    if (m_bSearchPending) {
        if (!m_pSQRNet->FriendRoomManagerIsBusy()) {
            ...
            m_pSearchResults = new SQRNetworkManager::SessionSearchResult
                [m_searchResultsCount];

            for (int i = 0; i < m_searchResultsCount; i++) {
                m_pSQRNet->FriendRoomManagerGetRoomInfo(i,
                                                        &m_pSearchResults[i]);
            }
            m_bSearchPending = false;
...
#ifdef __PSVITA__
            // in adhoc mode we can keep searching, as the friend list is
            // populated in callbacks 4J Stu - Every second seems a bit much as
            // it makes the friend list flash every time it updates. Changed
            // this to 5 seconds.
            if (usingAdhocMode()) searchDelay = 5000;
#endif
            if (m_SessionsUpdatedCallback != NULL &&
                (m_lastSearchStartTime + searchDelay) < GetTickCount()) {
...
std::vector<FriendSessionInfo*>* CPlatformNetworkManagerSony::GetSessionList(
    ...
    for (int i = 0; i < m_searchResultsCount; i++) {
        if (m_pSearchResults[i].m_extData) {
            ...
            // TODO - this mbstowcs shouldn't encounter any non-ascii
            // characters, but I imagine we'll want to actually use the online
            // name here which is UTF-8
            mbstowcs(newInfo->displayLabel,
                     m_pSearchResults[i].m_NpId.handle.data, 17);
            newInfo->displayLabelLength =
                strlen(m_pSearchResults[i].m_NpId.handle.data);
            newInfo->hasPartyMember = false;
            newInfo->searchResult = m_pSearchResults[i];
            newInfo->sessionId = m_pSearchResults[i].m_sessionId;
            memcpy(&newInfo->data, m_pSearchResults[i].m_extData,
                   sizeof(GameSessionData));
            if ((newInfo->data.isReadyToJoin) && (newInfo->data.isJoinable) &&
...
bool CPlatformNetworkManagerSony::GetGameSessionInfo(
...
void CPlatformNetworkManagerSony::GetFullFriendSessionInfo(
    FriendSessionInfo* foundSession,
    void (*FriendSessionUpdatedFn)(bool success, void* pParam), void* pParam) {
    m_pSQRNet->GetExtDataForRoom(foundSession->sessionId.m_RoomId,
                                 &foundSession->data, FriendSessionUpdatedFn,
                                 pParam);
}
...
void CPlatformNetworkManagerSony::removeNetworkPlayer(
    SQRNetworkPlayer* pSQRPlayer) {
    INetworkPlayer* pNetworkPlayer = getNetworkPlayer(pSQRPlayer);
...
INetworkPlayer* CPlatformNetworkManagerSony::getNetworkPlayer(
    SQRNetworkPlayer* pSQRPlayer) {
    return pSQRPlayer ? (INetworkPlayer*)(pSQRPlayer->GetCustomDataValue())
                      : NULL;
}
...
INetworkPlayer* CPlatformNetworkManagerSony::GetPlayerBySmallId(
...
bool CPlatformNetworkManagerSony::JoinGameFromInviteInfo(
    int userIndex, int userMask, const INVITE_INFO* pInviteInfo) {
    return m_pSQRNet->JoinRoom(pInviteInfo->m_RoomId, pInviteInfo->m_ServerId,
                               userMask, pInviteInfo);
}
...
void CPlatformNetworkManagerSony::SetSQRPresenceInfoFromExtData(
    ...
    memcpy(&presence->hostPlayerUID, &gsd->hostPlayerUID,
           sizeof(GameSessionUID));
...
void CPlatformNetworkManagerSony::MallocAndSetExtDataFromSQRPresenceInfo(
    ...
    if (presence->netVersion != 0) {
        memcpy(&gsd->hostPlayerUID, &presence->hostPlayerUID,
               sizeof(GameSessionUID));
...
bool CPlatformNetworkManagerSony::checkValidInviteData(
