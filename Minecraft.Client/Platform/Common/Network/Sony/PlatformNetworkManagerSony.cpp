...
void CPlatformNetworkManagerSony::HandlePlayerLeaving(
    ...
    if (networkPlayer) {
        ...
        void CPlatformNetworkManagerSony::HandleResyncPlayerRequest(
            ...
            for (int i = 0; i < SQRNetworkManager::MAX_ONLINE_PLAYER_COUNT;
                 i++) {
                if (aPlayers[i]) {
	...
	if( ret == 0 )
	{
        if (state == IPObtainedState) {
...
#endif
#ifdef __PSVITA__  // If we're in ad-hoc mode this problem definitely wasn't PSN
                   // related
            if (usingAdhocMode())
                preferSignoutError = fa #endif  // If we're the host we haven't
            ...
            switch (IDS) {
                ...
                br #ifdef __PSVITA__ case IDS_CONNECTION_LOST_SERVER:
                    if (preferSignoutError) {
            ...
            bool CPlatformNetworkManagerSony::isSystemPrimaryPlayer(
                ...
                return playerIsSystemPrimary  // We call this twice a frame,
                                              ...
                                              // things
                    void CPlatformNetworkManagerSony::DoWork #if 0 DWORD
                        dwNotifyId;
                ...
                m_pIQNet->DoWor #else TickSearch();
            ...
            int CPlatformNetworkManagerSony::GetLocalPlayerMask(
                int playerIndex) {
                return 1 << playerIndex;
            }

            bool CPlatformNetworkManagerSony::AddLocalPlayerByUserIndex(
                int userIndex) {
                return m_pSQRNet->AddLocalPlayerByUserIndex(userIndex);
            }

            bool CPlatformNetworkManagerSony::RemoveLocalPlayerByUserIndex(
                int userIndex) {
                SQRNetworkPlayer* pSQRPlayer =
...
bool CPlatformNetworkManagerSony::_StartGame #if 0  // Set the options that now
...
m_pIQNet->SetOpt(QNET_OPTION_PRESENCE_JOIN_MODE, &enablePres, sizeof BOOL);
	...
	return t#endif
        ...
        int CPlatformNetworkManagerSony::JoinGame(
            ...
            for (int i = 0; i < SQRNetworkManager::MAX_LOCAL_PLAYER_COUNT;
                 i++) {
            ...
            GameSessionData* gameSession =
                (GameSessionData*)(&searchResult->data);
            ...
            if (m_pSQRNet->JoinRoom(&searchResult->searchResult,
                                    localUsersMask)) {
        ...
        bool CPlatformNetworkManagerSony::
            _RunNetworkGame #if 0  // We delay actually starting the session so
                                   // that we know the game server is running by
                                   // the time the clients try to join// This
                                   // does result in a host advantage
...
void CPlatformNetworkManagerSony::UpdateAndSetGameSessionData(INetworkPlayer *pNetworkPlayerLea/*= NULL*/)
{
    ...
    m_hostGameSessionData.hostPlayerUID =
        GetHostPlayer()->GetUI #ifdef __PSVITA__ if (usingAdhocMode()) {
                ...
                for (unsigned int i = 0; i < m_playerFlags.size(); i++) {
                    ...
                    bool CPlatformNetworkManagerSony::SystemFlagGet(
                        ...
                        for (unsigned int i = 0; i < m_playerFlags.size();
                             i++) {
                            if (m_playerFlags[i]->m_pNetworkPlayer ==
                                pNetworkPlayer) {
                ...
                if (m_bSearchPending) {
                    if (!m_pSQRNet->FriendRoomManagerIsBusy()) {
                        m_searchResultsCount =
                            m_pSQRNet->FriendRoomManagerGetCount();
                        delete m_pSearchResults;
                        m_pSearchResults = new SQRNetworkManager::
                            SessionSearchResult[m_searchResultsCount];

                        for (int i = 0; i < m_searchResultsCount; i++) {
                            m_pSQRNet->FriendRoomManagerGetRoomInfo(
                                i, &m_pSearchResults[i]);
                        }
                        m_bSearchPending = false;
                    ...
                    if (!m_pSQRNet->FriendRoomManagerIsBusy())
                        ...
                        int searchDelay =
                            ...
                            if (usingAdhocMode()) searchDelay =
                                5 #endif if (m_SessionsUpdatedCallback !=
                                                 NULL &&
                                             (m_lastSearchStartTime +
                                              searchDelay) < GetTickCount()) {
...
// Loop through all the results.
		for( DWORD dwResult = 0; dwResult < m_currentSearchResultsCount[iPad]; dwResult++ )
		{
            ...
            if (memcmp(&pSearchResult->info.sessionID, &sessionId,
                       sizeof(SessionID)) != 0)
                continue;
            ...
            for (itFriendSession = friendsSessions[iPad].begin();
                 itFriendSession < friendsSessions[iPad].end();
                 ++itFriendSession) {
                sessionInfo = *itFriendSession;
                if (memcmp(&pSearchResult->info.sessionID,
                           &sessionInfo->sessionId, sizeof(SessionID)) == 0) {
                    sessionInfo->searchResult = *pSearchResult;
                    sessionInfo->displayLabel = new wchar_t[100];
                    ZeroMemory(sessionInfo->displayLabel,
                               100 * sizeof(wchar_t));
                    foundSession = true;
                    break;
                }
            ...
            if (pxnqi->bFlags & XNET_XNQOSINFO_TARGET_CONTACTED) {
                ...
                if (!(pxnqi->bFlags & XNET_XNQOSINFO_TARGET_DISABLED) &&
                    ...
                    if (foundSessionInfo->displayLabel != NULL)
                        delete[] foundSessionInfo->displayLabel;
                    foundSessionInfo->displayLabel = new wchar_t[100];
                    memcpy(foundSessionInfo->displayLabel,
                           sessionInfo->displayLabel, 100 * sizeof(wchar_t));
                    foundSessionInfo->displayLabelLength =
                        sessionInfo->displayLabelLength;
                    foundSessionInfo->hasPartyMember =
                        sessionInfo->hasPartyMember;
	...
	return fa#endif
        ...
        void CPlatformNetworkManagerSony::SetSessionsUpdatedCallback(
            void (*SessionsUpdatedCallback)(LPVOID pParam),
            LPVOID pSearchParam) {
        ...
        INetworkPlayer* CPlatformNetworkManagerSony::addNetworkPlayer(
            SQRNetworkPlayer* pSQRPlayer) {
            NetworkPlayerSony* pNetworkPlayer =
                new NetworkPlayerSony(pSQRPlayer);
        ...
        INetworkPlayer* CPlatformNetworkManagerSony::getNetworkPlayer(
            SQRNetworkPlayer* pSQRPlayer) {
            return pSQRPlayer
                       ? (INetworkPlayer*)(pSQRPlayer->GetCustomDataValue())
                       : NULL;
        }
        ...
        INetworkPlayer* CPlatformNetworkManagerSony::GetPlayerByXuid(
            PlayerUID xuid) {
        ...
        bool CPlatformNetworkManagerSony::JoinGameFromInviteInfo(
            ...
            return m_pSQRNet->JoinRoom(pInviteInfo->m_RoomId,
                                       pInviteInfo->m_ServerId, userMask,
                                       pInviteInfo);
        }

        void CPlatformNetworkManagerSony::SetSessionTexturePackParentId(
            int id) {
