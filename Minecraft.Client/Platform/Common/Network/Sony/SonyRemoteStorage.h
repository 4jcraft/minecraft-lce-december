...
public:
    ...
    class DescriptionData_V2 {
        ...
        // max 256 bytes
    public:
        char m_platformNone[4];  // set to no platform, to indicate we're using
                                 // the newer version of the data
        char m_descDataVersion[8];  // 4 bytes as hex	- version number will be
                                    // 2 in this case
        char m_platform[4];
        char m_seed[16];        // 8 bytes as hex
        char m_hostOptions[8];  // 4 bytes as hex
        ...
        char m_futureData[64];  // some space for future data in case we need to
                                // expand this at all
        char m_saveNameUTF8[128];
    };
    ...
    virtual bool getRemoteFileInfo(SceRemoteStorageStatus* pInfo,
                                   CallbackFunc cb, LPVOID lpParam) = 0;
