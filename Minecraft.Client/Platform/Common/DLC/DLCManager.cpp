...
void DLCManager::LanguageChanged(void) {
    for (AUTO_VAR(it, m_packs.begin()); it != m_packs.end(); ++it) {
        DLCPack* pack = (DLCPack*)*i  // update the language
                            pack->UpdateLanguage();
    }
}
...
DLCPack* DLCManager::getPack(const std::wstring& name) {
    ...
    return pac
        ...
        return pac #endif
            ...
            if (type != e_DLCType_All) {
                ...
                if (index >= m_packs.size()) {
                        app.DebugPr"DLCManager: Trying to access a DLC pack beyond the range of valid packs\n");
                        __debugbreak();
                }
                pack = m_packs[index];
            }
        ...
        DWORD DLCManager::getPackIndex(DLCPack * pack, bool& found,
                                       EDLCType /*= e_DLCType_All*/) {
            DWORD foundIndex = 0;
            found = false;
            if (pack == NULL) {
                app.DebugPr
        ...
        DWORD DLCManager::getPackIndexContainingSkin(const std::wstring& path,
                                                     bool& found) {
            DWORD foundIndex = 0;
            found = false;
            DWORD index = 0;
        ...
        DWORD DLCManager::checkForCorruptDLCAndAlert(bool showMes /*= true*/) {
            ...
            if (corruptDLCCount > 0 && showMessage) {
                ...
                if (corruptDLCCount == 1 && firstCorruptPack != NULL)
                    // pass in the pack format std::string
                    WCHAR wchFormat[132];
                swprintf(wchFormat, 1 "%ls\n\n%%ls",
    ...
    bool DLCManager::readDLCDataFile(DWORD & dwFilesProcessed,
    ...
    bool DLCManager::readDLCDataFile(DWORD & dwFilesProcessed,
        ...
        if (fromArchive && app.getArchiveFileSize(wPath) >= 0) {
            ...
            return processDLCDataFile(dwFilesProcessed, bytes.data,
                                      bytes.length, pack);
        } else if (fromArchive)
            return fa
#ifdef _WINDOWS64
	...
	HANDLE file = CreateFile(finalPath.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NU#else
	HANDLE file = CreateFile(path.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NU#endif
	if( file == INVALID_HANDLE_VALUE )
	{
            DWORD error = GetLastError();
                app.DebugPr"Failed to open DLC data file with error code %d (%x)\n", error, error);
	...
	if(bSuccess==FALSE)
        // need to treat the file as corrupt, and flag it, so can't call fatal
        // error//app.FatalLoadError();
    }
    else {
        CloseHandle(file);
    }
    if (bSuccess == FALSE)
        // Corrupt or some other error. In any case treat as corrupt
                app.DebugPr"Failed to read %s from DLC content package\n", path.c_str());
...
bool DLCManager::processDLCDataFile(DWORD& dwFilesProcessed, PBYTE pbData,
                                    DWORD dwLength, DLCPack* pack) {
    std::unordered_map<int, DLCManager::EDLCParameterType> parameterMapping;
    unsigned int
        uiCurrentByte =  // File format defined in the DLC_Creator// File
                         // format: Version 2// unsigned long, version
                         // number// unsigned long, t = number of parameter
                         // types// t * DLC_FILE_PARAM structs mapping strings
                         // to id's// unsigned long, n = number of files// n
                         // * DLC_FILE_DETAILS describing each file in the
                         // pack// n * files of the form// // unsigned long,
                         // p = number of parameters// // p * DLC_FILE_PARAM
                         // describing each parameter for this file// //
    ...
    if (uiVersion < CURRENT_DLC_VERSION_NUM) {
        if (pbData != NULL) delete[] pbData;
                app.DebugPr"DLC version of %d is too old to be read\n", uiVersion);
                return false;
    }
    pack->SetDataPointer(pbData);
    unsigned int uiParameterCount = *(unsigned int*)&pbData[uiCurrentByte];
    uiCurrentByte += sizeof(int);
        C4JStorage::DLC_FILE_PARAM *pParams = (C4JStorage::DLC_FILE_PARAM *)&pbData[uiCurrentByt//DWORD dwwchCount=0;
...
// Map DLC strings to application strings, then store the DLC index mapping to application index
		...
		pParams = (C4JStorage::DLC_FILE_PARAM *)&pbData[uiCurrentByte];//ulCurrentByte+=ulParameterCount * sizeof(C4JStorage::DLC_FILE_PARAM);
	...
	{
        dwTemp += sizeof(C4JStorage::DLC_FILE_DETAILS) +
                  pFile->dwWchCount * sizeof(WCHAR);
        pFile = (C4JStorage::DLC_FILE_DETAILS*)&pbData[dwTemp];
	}
	PBYTE pbTemp=((PBYTE )pF//+ sizeof(C4JStorage::DLC_FILE_DETAILS)*ulFileCount;
	...
	for(unsigned int i=0;i<uiFileCount;i++)
	{
        ...
        if (type == e_DLCType_TexturePack) {
                ...
                if (type == e_DLCType_PackConfig) {
                    ...
                    if (dlcFile != NULL)
                        dlcFile->addParameter(it->second,
                                              (WCHAR*)pParams->wchData);
                    else if (dlcTexturePack != NULL)
                        dlcTexturePack->addParameter(it->second,
                                                     (WCHAR*)pParams->wchData);
                }
            }
            pbTemp += sizeof(C4JStorage::DLC_FILE_PARAM) +
                      (sizeof(WCHAR) * pParams->dwWchCount);
            pParams = (C4JStorage::DLC_FILE_PARAM*)pbTemp;
            // pbTemp+=ulParameterCount *
            // sizeof(C4JStorage::DLC_FILE_PARAM);

            if (dlcTexturePack != NULL) {
                DWORD texturePackFilesProcessed = 0;
                bool validPack =
                    processDLCDataFile(texturePackFilesProcessed, pbTemp,
                                       pFile->uiFileSize, dlcTexturePack);
                        pack->SetDataPointer(NU// If it's a child pack, it doesn't own the data
			...
			++dwFilesProcessed;
                ...
                dlcFile->addData(
                    pbTemp,
                    pFile->uiFileSize);  // TODO - 4J Stu Remove the need for
                                         // this vSkinNames std::vector, or
            ...
            uiCurrentByte += sizeof(C4JStorage::DLC_FILE_DETAILS) +
                             pFile->dwWchCount * sizeof(WCHAR);
        ...
        if (pack->getDLCItemsCount(DLCManager::e_DLCType_Audio) > 0)
            // app.m_Audio.loadAudioDetails(pack);// TODO Should be able to
            // delete this data, but we can't yet due to how it is added to the
            // Memory textures (MEM_file)
...
{
        DWORD packId = 0;
        std::wstring wPath = convStringToWstring(pa
#ifdef _WINDOWS64
...
DWORD DLCManager::retrievePackID(PBYTE pbData, DWORD dwLength, DLCPack* pack) {
    ...
    unsigned int uiParameterCount = *(unsigned int*)&pbData[uiCurrentByte];
    uiCurrentByte += sizeof(int);
    C4JStorage::DLC_FILE_PARAM* pParams =
        (C4JStorage::DLC_FILE_PARAM*)&pbData[uiCurrentByte];
    for (unsigned int i = 0; i < uiParameterCount; i++)
    ...
    uiCurrentByte += sizeof(C4JStorage::DLC_FILE_PARAM) +
                     (pParams->dwWchCount * sizeof(WCHAR));
    pParams = (C4JStorage::DLC_FILE_PARAM*)&pbData[uiCurrentByte];
}

unsigned int uiFileCount = *(unsigned int*)&pbData[uiCurrentByte];
uiCurrentByte += sizeof(int);
C4JStorage::DLC_FILE_DETAILS* pFile =
    (C4JStorage::DLC_FILE_DETAILS*)&pbData[uiCurrentByte];
...
for (unsigned int i = 0; i < uiFileCount; i++) {
        ...
        if (it != parameterMapping.end()) {
            if (type == e_DLCType_PackConfig) {
                if (it->second == e_DLCParamType_PackId) {
                    std::wstring wsTemp = (WCHAR*)pParams->wchData;
    ...
    if (bPackIDSet)
        brea  // Move the pointer to the start of the next files data;
            pbTemp += pFile->uiFileSize;
    uiCurrentByte += sizeof(C4JStorage::DLC_FILE_DETAILS) +
                     pFile->dwWchCount * sizeof(WCHAR);
