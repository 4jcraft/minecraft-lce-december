...
bool DLCManager::readDLCDataFile(DWORD& dwFilesProcessed,
    ...
    HANDLE file = CreateFile(finalPath.c_str(), GENERIC_READ, 0, NULL,
    ...
    if (file == INVALID_HANDLE_VALUE) {
        ...
        app.DebugPrintf(
            "Failed to open DLC data file with error code %d (%x)\n", error,
            error);
...
bool DLCManager::processDLCDataFile(DWORD& dwFilesProcessed, PBYTE pbData,
    ...
    PBYTE pbTemp =
        ((PBYTE)pFile);  //+ sizeof(C4JStorage::DLC_FILE_DETAILS)*ulFileCount;
    ...
    for (unsigned int i = 0; i < uiFileCount; i++) {
        ...
        if (dlcTexturePack != NULL) {
            ...
            // TODO - 4J Stu Remove the need for this vSkinNames std::vector, or
...
DWORD DLCManager::retrievePackIDFromDLCDataFile(const std::string& path,
                                                DLCPack* pack) {
    DWORD packId = 0;
    std::wstring wPath = convStringToWstring(path);
