    ...
    for (;;) switch (state->mode) {
            ...
            case CODELENS:
                ...
                /* build code tables -- note: do not change the lenbits or
                   distbits values here (9 and 6) without reading the comments
                   in inftrees.h concerning the ENOUGH constants, which depend
                   on those values */
                state->next = state->codes;
                state->lencode = (const code FAR*)(state->next);
                state->lenbits = 9;
                ret = inflate_table(LENS, state->lens, state->nlen,
                                    &(state->next), &(state->lenbits),
                                    state->work);
                if (ret) {
...
uInt* dictLength;
{
    ...
    if (state->whave && dictionary != Z_NULL) {
        ...
        zmemcpy(dictionary + state->whave - state->wnext, state->window,
                state->wnext);
    }
...
z_streamp source;
{
    ...
    /* allocate space */
    copy = (struct inflate_state FAR*)ZALLOC(source, 1,
                                             sizeof(struct inflate_state));
