        ...
        LPVOID lpParam, PBYTE pbThumbnail, DWORD dwThumbnailBytes)
{
    ...
    if (pbThumbnail && dwThumbnailBytes) {
        pClass->m_saveDetails[pClass->m_iRequestingThumbnailId]
            .pbThumbnailData = new BYTE[dwThumbnailBytes];
        ...
        pClass->m_saveDetails[pClass->m_iRequestingThumbnailId]
            .dwThumbnailSize = 0;
        app.De"Save thumbnail data is NULL, or has size 0\n");
    }
...
int UIScene_LoadOrJoinMenu::LoadSaveCallback(LPVOID lpParam,bool bR//UIScene_LoadOrJoinMenu *pClass= (UIScene_LoadOrJoinMenu *)lpParam;// Get the save data now
    if(bRes)
    {
        app.De"Loaded save OK\n");
    }
    return 0;
}

UIScene_LoadOrJoinMenu::UIScene_LoadOrJoinMenu(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLay// Setup all the Iggy references we need for this scene
    initialiseMovie();
    ...
    m_iState=e_Saves//m_bRetrievingSaveInfo=false;
    ...
    m_controlJoinTimer.setVisible
#if defined(_XBOX_ONE) || defined(__ORBIS__)
    m_spaceIndicatorSa ""s.init(L, eControl_SpaceIndicator, 0, \
                                (4LL * 1024LL * 1024LL * 1 #endif);
    ...
    m_bSaveTransferInProgre#endifse;
    ...
    UpdateGamesList();
        ...
        m_initData =
            new JoinMenuInitDat  // 4J Stu - Fix for #12530 -TCR 001 BAS Game
                                 // Stability: Title will crash if the player
                                 // disconnects while starting a new world and
                                 // then opts to play the tutorial once they
                                 // have been returned to the Main
                                 // Menu.
                MinecraftServer::resetFlag  // If we're not ignoring
                                            // input, then we aren't
                                            // still waiting for the DLC
                                            // to mount, and can now
                                            // check for corrupt dlc.
                                            // Otherwise this will
                                            // happen when the dlc has
                                            // finished
                                            // mounting.
            if (!m_bIgnoreInput) {
            app.m_dlcManager.checkForCorruptDLCAndAlert();
            // 4J-PB - Only Xbox will not have trial DLC patched into the
            // game#ifdef _XBOX// 4J-PB - there may be texture packs we
            // don't have, so use the info from TMS for this

            DLC_INFO* pDLCInfo = N  // first pass - look to see if there are any
        ...
        app.De"+++ Adding TMSPP request for texture pack data\n");
...
void UIScene_LoadOrJoinMenu::updateToo#if defined __PS3__ || defined __ORBIS__ || defined __PSVITA__
	...
	else if(DoesMashUpWorldHaveFocu// If it's a mash-up pack world, give the Hide option
    ...
    e#if defined _XBOX_ONE
		if(ProfileManager.IsSignedInLive( m_iPad // Is there a save from 360 on TMS?
			iX=IDS_TOOLTIPS_SAVETRANSFER_DOWN#elif defined SONY_REMOTE_STORAGE_DOWNLOAD// Is there a save from PS3 or PSVita available?// Sony asked that this be displayed at all times so users are aware of the functionality. We'll display some text when there's no save available//if(app.getRemoteStorage()->saveIsAvailable())
		{
        ...
        iX = IDS_TOOLTIPS_CHAN #endifCE;
        
    }

    ui.SetTooltips( DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT, IDS_TOOLTIPS_BACK, iX, iY,-1,-1,iLB// B);
}
...
void UIScene_LoadOrJoinMenu::Initialise()
{
    ...
    m_iMashUpButtons  // Check if we're in the trial version
        if (ProfileManager.IsFullVersion() == false) {
        AddDefaultB
#if TO_BE_IMPLEMENTED
        m_pSavesList->SetCurSelVi #endif0);

    
    }
    else if(StorageManager.GetSaveDisabled#if defined(__PS3__) || defined(__ORBIS__) || defined (__PSVITA__)
        GetSa#elseo
#if TO_BE_IMPLEMENTED
        if (StorageManager.GetSaveDeviceSelected #endifd))
...
void UIScene_LoadOrJoinMenu::handleDe#ifdef __PSVITA__
...
void UIScene_LoadOrJoinMenu::handleGainFocus(bool navBack) {
    ...
    updateTooltip  // Add load online timer
    ...
    if (navBack) {
        ...
        m_bMultiplayerAllowed =
            ...
            ProfileManager.AllowedToPlayMultiplayer(
                m_iPad);  // re-enable button presses
        m_bIgnoreInput =
            false;  // block input if we're waiting for DLC to install, and wipe
                    // the saves list. The end of dlc mounting custom message
                    // will fill the list again
    ...
    XuiFi "CScene_FullscreenProgress" );
            HXUICLASS hClassConnectingProgress = XuiFi"CScene_ConnectingProgress" );
        ...
        if (XuiIsInstanceOf(hSceneFrom, hClassFullscreenProgress) ||
            ...
            m_labelNoGames.setVisibl#if TO_BE_IMPLEMENTED
            m_SavesList.InitFocus#endifd);
...
void UIScene_LoadOrJoinMenu::handleLoseFocu  // Kill load online timer
...
void UIScene_LoadOrJoinMenu::tick() {
    ...
    if (m_bE  // navigate forward or back
    {
        if (!m_bRetrievingSaveThumbnails) {
            // need to wait for any callback retrieving thumbnail to
            // complete
            navigateBack();
        }
        // Stop loading thumbnails if we navigate forwards
    if(hasFocus(m_iPa#ifdef SONY_REMOTE_STORAGE_DOWNLOAD// if the loadOrJoin menu has focus again, we can clear the saveTransfer flag now. Added so we can delay the ehternet disconnect till it's cleaned up
		if(m_eSaveTransferState == eSaveTransfer_Idle)
			m_bSaveTransferRunning #endife#if defined(_XBOX_ONE) || defined(__ORBIS__)
            ...
            m_bUpdateSaveSize = false;
              ...
              // Display the saves if we have them
                ...
                if (m_pSaveDetails != NULL) {
                    // CD - Fix - Adding define for ORBIS/XBOXONE#if
                    // defined(_XBOX_ONE) || defined(__ORBIS__)
                    m_spaceIndicatorSaves #endif();
    ...
    m_pSaveDetails->SaveInfoA[i]                      \
        .to #elif defined(__ORBIS__)
                    m_spaceIndicatorSaves.addSave(    \
            m_pSaveDetails->SaveInfoA[i].blocksUsed * \
            (32 * #endif #ifdef _DURANGO
                    m_buttonListSaves.addItem(        \
                      m_pSaveDetails->SaveInfoA[i].UTF16Sa "" Title, L);
                    ...
                    memcpy(m_saveDetails[i].UTF16SaveFilename,
                           ...
                           MAX_SAVEFILENAME #elseTH);
                    
                    m_buttonListSaves.addItem(
                        m_pSaveDetails->SaveInfoA[i].UTF8Sa "" Title, L);
        ...
        if(!m_bExitScene && m_bSavesDisplayed && !m_bRetrievingSaveThumbnails && !m_bAllLoaded)
        {
            if (m_iRequestingThumbnailId <
                ...
                app.De"Requesting the first thumbnail\n");
                ...
                C4JStorage::ESaveGameState eLoadStatus =
                    StorageManager.LoadSaveDataThumbnail(
                        ...
                        &LoadSaveDataThumbnailReturned, this);
        ...
        else if (m_bSavesDisplayed && m_bSaveThumbnailReady)
        {
            ...
            if (!m_bExitScene) {
                // convert to utf16
                uint16_t u16Message[MAX_SAVEFILENAME#ifdef _DURANGO
      // Already utf16 on durango
                memcpy(u16Message, m_saveDetails[m_iRequestingThumbnailId].UTF16SaveFilename, MAX_SAVEFILENAME#elif defined(_WINDOWS64)
                int result = ::MultiByteToWideChar(
                    CP_UTF8,      // convert from UTF-8
                    MB_ERR_INVALID// error on invalid chars
...
#else #ifdef __PS3
                size_t srcma #elsemax;
                
                uint32_t srcmax,
                    dstmax;
            uint32_t srcle #endifen;
            
                srcmax = MAX_SAVEFILENAME_LENGTH;
            dstmax = MAX_SAVEFILENAM
#if defined(__PS3__)
                L10nResult lres = UTF8stoUTF16s(                        \
    (uint8_t*)m_saveDetails[m_iRequestingThumbnailId].UTF8SaveFilename, \
    &srcmax, u16Message, #elseax);

                SceCesUcsContext context;
            sceCesUcsContextInit(&context);

                sceCesUtf8StrToUtf16Str(&context, (uint8_t *)m_saveDetails[m_iRequestingThumbnailId].UTF8SaveFilename,srcmax,&srclen,u16Message,dstmax,#endifn#endif
                ...
                {
                    app.De"Requesting another thumbnail\n");
                    // set the save to load
...
switch (m_iState) {
    ...
    AddDefaultButtons();
    m_bSavesDisplayed = true;
        m_controlSavesTimer.setVisible( false#endif
#ifdef _XBOX_ONE
...
// SAVE TRANSFERS#ifdef __ORBIS__// check the status of the PSPlus common dialog
	switch (sceNpCommerceDialogUpdateStatus())
	{
        case SCE_COMMON_DIALOG_STATUS_FINISHED: {
            ...
            if (Result
                    .authorized)  // they just became a PSPlus member
                ProfileManager.PsPlusUpdate(ProfileManager.GetPrimaryPad(),
                                            &Result);
        }
            else {
                // 4J-JEV: Fix for PS4 #5148 - [ONLINE] If the user attempts to
                // join a game when they do not have Playstation Plus, the title
...
void UIScene_LoadOrJoinMenu::GetSaveInfo() {
    unsigned int uiSave// This will return with the number retrieved in uiSaveC

    if(app.DebugSettingsOn() && app.GetLoadSavesFromFolderEnabled#ifdef __ORBIS__// We need to make sure this is non-null so that we have an idea of free space
        m_pSaveDetails=StorageManager.ReturnSavesInfo();
        if(m_pSaveDetails==NULL)
        {
        C4JStorage::ESaveGameState eSGIStatus =
            StorageManager.GetSavesInfo(m_iPad, "save" his,);
#endif }


        uiS#ifdef _XBOX
        File "GAME:\\Saves"#else);

        File "Saves" r(L#endif );

        if (savesDir.exists()) {
    m_saves = savesDir.listFiles();
    uiSaveC = (unsigned int)m_saves->size();
    // add the New Game and Tutorial after the saves list is retrieved, if there
    // are any saves// Add two for New Game and Tutorial
    ...
    for (unsigned int i = 0; i < listItems; i++) {
        ...
        name[wName.size()] = 0;
        m_buttonListSaves "" ddItem(name, L);
    }
    m_bSavesDisplayed = true;
    m_bAllLoaded = true;
    m_bIgnoreInput = false;
}
else              // clear the saves list
    m_bSavesDisp  // we're blocking the exit from this scene until
                  // complete
                      m_buttonListSaves.clearList();
m_iSaveInfoC = 0;
m_controlSavesTimer.setVisible(true);
...
m_pSaveDetails = StorageManager.ReturnSavesInfo();
if (m_pSaveDetails == NULL) {
            C4JStorage::ESaveGameState eSGIStatus= StorageManager.GetSavesInfo(m_"save"ULL,this,
#if TO_BE_IMPLEMENTED
        if(eSGIStatus==C4JStorage::ESGIStatus_NoSaves)
        {
                uiSaveC = 0;
                m_controlSavesTimer.setVisible(false);
            m_SavesList.SetEnable(TR#endif       }
...
void UIScene_LoadOrJoinMenu::AddDefaultButtons()
{
        ...
        for (AUTO_VAR(it, app.getLevelGenerators()->begin());
            ...
            LevelGenerationOptions*
                levelGen =  // retrieve the save icon from the texture pack, if
                            // there is one
            ...
            if (uiTexturePackID != 0) {
                unsigned int uiMashUpWorldsBitmask =
                    app.GetMashupPackWorlds(m_iPad);
                        ...
                        if((uiMashUpWorldsBitmask & (1<<(uiTexturePackID-1024))// this world is hidden, so skip
				continue;// 4J-JEV: For debug. Ignore worlds with no name.
		LPCWSTR wstr = levelGen->getWorldName();
		m_buttonListSaves.addItem( wstr );
		m_generators.push_back(levelGen);

        if(uiTexturePackID!=0// increment the count of the mash-up pack worlds in the save list
			m_iMashUpButtonsC++;
            ...
            if(dwImageBytes > 0 && pbImageData)
            {
                ...
                swprintf"tpack%08x"64,L,tp->getId());
                registerSubstitutionTexture(imageName, pbImageData,
                                            dwImageBytes);
...
{
        if (m_bIgnoreInput  // if we're retrieving save info, ignore key
                            // presses
        ...
        if(pres#if defined(__PS3__) || defined(__ORBIS__) || defined(__PSVITA__)
            m_bE#elseene=true;
            ...
            case #if TO_BE_IMPLEMENTED  // Change device// Fix for #12531 - TCR 001: BAS Game Stability: When a player selects to change a storage // device, and repeatedly backs out of the SD screen, disconnects from LIVE, and then selects a SD, the title crashes.
            m_bIgnoreInput = true;
            StorageManager.SetSaveDevice(
                &CScene_MultiGameJoinLoad::DeviceSelectReturned, this, true);
            ui.PlayUISF #endif_Press)
            ;
        // Save Transfer#ifdef _XBOX_ONE
        if (ProfileManager.IsSignedInLive(m_iPad)) {
            UIScene_LoadOrJoinMenu::s_ulFileSize = 0;
            LaunchSaveT #endifr #ifdef SONY_REMOTE_STORAGE_DOWNLOAD
		{
                bool bSignedInLive = ProfileManager.IsSignedInLive(iPad);
                if (bSignedInLive) {
                    LaunchSaveTransf #endif
                }
            }
            
        break;
    case #if defined(__PS3__) || defined(__PSVITA__) || defined(__ORBIS__)
		m_eAction = eAction_ViewInvites;
        if(pressed && iPad == ProfileManager.GetPrimaryPa#ifdef __ORBIS__// Check if PSN is unavailable because of age restriction
...
// are we offline?
 ...
 // get them to sign in to online
            ...
            else#ifdef __ORBIS__
                SQRNetworkManager_Orbis::Re#elif defined __PSVITA__
                SQRNetworkManager_Vita::Re#elseiteGUI();
        ...
        if(getControlFocus() == eControl_GamesList && m_buttonListGames.getItemCount() > 0)
        {
            ...
            ui.PlayUISFX(eSFX_Pre#endif // __PS3__ || __ORBIS__
        break;
            ...
            if ((m_iDefaultButtonsC != 0) &&
                ...
                if (StorageManager.GetSaveDisabled()) {
                    ...
                    ui.RequestAlertMessage(
                        ...
                        iPad, &UIScene_LoadOrJoinMenu::DeleteSaveDialogReturned,
                        this);
                } else {
                    if (StorageManager.EnoughSpaceForAMinSaveGame()) {
                        UINT uiIDA[4];
                        ...
                        int n #ifdef SONY_REMOTE_STORAGE_UPLOAD
                        if (ProfileManager.IsSignedInLive(
                                ProfileManager.GetPrimaryPad())) {
                            numOptions = 4;
                            uiIDA[3] = IDS_TOOLTIPS_SAVETRANSFER_UPLOAD;
#endif #if defined _XBOX_ONE || defined __ORBIS__
                            numOptions = 4;
                            uiIDA[3]#endifOPYSAVE;
                ...
                // delete the save game
                // Have to ask the player if they are sure they want to delete
                // this game
                UINT uiIDA[2];
                uiIDA[0] = IDS_CONFIRM_CANCEL;
                uiIDA[1] = IDS_CONFIRM_OK;
                ...
                else if(DoesMashUpWorldHaveF// hiding a mash-up world
    ...
    if (!levelGen->isTutorial()) {
        if (levelGen->requiresTexturePack()) {
            ...
            m_bIgnoreInput = true;
                                                app.HideMashupPackWorld(m_iPad,uiP// update the saves list
			...
			ui.PlayUISFX(eSFX_Press);
                ...
                break;
                case ACTION_MEN #ifdef _XBOXL:
                    
        if (m_bInParty) {
                        m_bShowingPartyGamesOnly = !m_bShowingPartyGamesOnly;
                        UpdateGamesList();
            CXuiSceneBase::PlayUISFX(eSFX_Pre#endif
                    }
                    
        break;
                case ACTION_MENU_LEFT:
                case ACTION_MENU_RIGHT:
                    // if we are on the saves menu, check there are games in the
                    // games list to move to
                    if (DoesSavesListHaveFocus()) {
...
int UIScene_LoadOrJoinMenu::KeyboardCompleteWorldNameCallback(LPVOID lpParam,bo// 4J HEG - No reason to set value if keyboard was cancelled
    UIScene_LoadOrJoinMenu *pClass=(UIScene_LoadOrJoinMenu *)lpParam;
    ...
    if (bRes)
    {
        ...
        InputManager.GetText(ui16T// check the name is valid
        if(ui16Text[0]#if (defined __PS3__ || defined __ORBIS__ || defined _DURANGO  || defined(__PSVITA__))// open the save and overwrite the metadata
            StorageManager.RenameSaveData(pClass->m_iSaveListIndex - pClass->m_iDefaultButtonsC, ui16Text,&UIScene_LoadOrJoinMenu::RenameSaveDataRet#endifpClass);
...
void UIScene_LoadOrJoinMenu::handleInitFocus(F64 controlId, F64 childId) {
    app.DebugPrint"UIScene_LoadOrJoinMenu::handleInitFocus - %d , %d\n", (int)controlId, (int)childId);
}

void UIScene_LoadOrJoinMenu::handleFocusChange(F64 controlId, F64 childId) {
    app.DebugPrint"UIScene_LoadOrJoinMenu::handleFocusChange - %d , %d\n", (int)controlId, (int)childId);
...
void UIScene_LoadOrJoinMenu::remoteStorageGetSaveCallback( \
    LPVOID lpParam, SonyRemoteStorage::Status s, int error_code)
    {
    a"remoteStorageGetCallback err : 0x%08x\n", error_code);
    assert(error_code == 0);
    ((UIScene_LoadOrJoinMenu*)lpParam)->LoadSave #endifoud();
    }
...
void UIScene_LoadOrJoinMenu::handlePress(F64 controlId, F64 childId) {
        switch ((int)controlId) {
            case eControl_SavesList: {
                ...
                int lGenID =
                    (int)childId - 1  // CD - Added for audio
                                   ui.PlayUISFX(eSFX_Press);
                ...
                if ((int)childId == JOIN_LOAD_CREATE_BUTTON_INDEX) {
                    ...
                    if (levelGen->isTutorial()) {
                        ...
                        // GetSaveGameInfo list because of
                        // sorting
     ...
     // check if this is a damaged save
                        PSAVE_INFO pSaveInfo =
                            &m_pSaveDetails->SaveInfoA[((int)childId) -
                                                       m_iDefaultButtonsC];
                if(pSaveInfo->thumbnailData == NULL && pSaveInfo->modif// no thumbnail data and time of zero and zero blocks useset for corrupt files
                {
...

                {
                        ...
                        if (app.DebugSettingsOn() &&
                            ...
                            LoadMenuInitData* params = new LoadMenuInitData();
                            params->iPad = m_iPad;
                            // need to get the iIndex from the list item, since
                            // the position in the list doesn't correspond to
                            // the GetSaveGameInfo list because of
                            // sorting
                            params->iSaveGameInfoIndex =
                                ((int)childId) - m_iDefaultButtonsC;
                            // params->pbSaveRenamed=&m_bSaveRenamed;
                            params->levelGen = NULL;
                        params->saveDetails = &m_saveDetails[ ((int)childId)-m_iDefa
#ifdef _XBOX_ONE
             // On XB1, saves might need syncing, in which case inform the user so they can decide whether they want to wait for this to happen
                        if( m_pSaveDetails->SaveInfoA[params->iSaveGameInfoIndex].needsSync )
                        {
                                ...
                                ui.RequestAlertMessage(
                                    IDS_LOAD_SAVED_WORLD,
                                    IDS_CONFIRM_SYNC_REQUIRED, uiIDA, 2,
                                    ProfileManager.GetPrimaryPad(),
                                    &NeedSyncMessageReturned, this);
                        }
#endif else
        ...
        case eControl_GamesList: {
            ...
            m_eAction = eAction_JoinGame  // CD - Added for audio
                            ui.PlayUISFX(eSFX_Press);
...
void UIScene_LoadOrJoinMenu::CheckAndJoinGame(int gameIndex) {
        if( m_buttonListGames.getItemCount() > 0 && gameIndex < m_currentSession#if defined(__PS3__) || defined(__ORBIS__) || defined(__PSVITA__)// 4J-PB - is the player allowed to join games?
		bool noUGC=false;
		...
		for(unsigned int i = 0; i < XUSER_MAX_COUNT; ++i)
		{
            if (InputManager.IsPadConnected(i) ||
                ProfileManager.IsSignedIn(i)) {
                                if (isSignedInLive && !ProfileManager.IsSignedInLive(// Record the first non signed in live pad
		...
		if  // not allowed to join#ifndef __PSVITA__
                    UINT uiIDA[1];
                        uiIDA[0]=IDS_// Not allowed to play online
			ui.RequestAlertMessage(IDS_ONLINE_GAME, IDS_CHAT_RESTRICTION_UGC, uiIDA, 1, m_iP#elseLL,t// Not allowed to play online
			ProfileManager.ShowSystemMessage( SCE_MSG_DIALOG_SYSMSG_TYPE_TRC_PSN_CHAT_RES#endifON, 0 );
			...
			return;
                ...
                m_bIgnoreInput = false;
                    ...
                    uiIDA[0] = IDS_C  // Check if PSN is unavailable because of
                                      // age restriction
                        int npAvailability = ProfileManager.getNPAvailability(
                            iPadNotSignedInLive);
                    if (npAvailability == SCE_NP_ERROR_AGE_RESTRICTION) {
                        m_bIgnoreInpu  // 4J Stu - This is a bit messy and is
                                       // due to the library incorrectly
                                       // returning false for IsSignedInLive if
                                       // the npAvailability isn't
                                       // SCE_OK
                                           ui.RequestErrorMessage(
                                               IDS_ONLINE_SERVICE_TITLE,
                                               IDS_CONTENT_RESTRICTION, uiIDA,
                                               1, iPadNotSignedInLive);
                    } else {
                        ui.RequestErrorMessage(
                            IDS_PRO_NOTONLINE_TITLE, IDS_PRO_NOTONLINE_TEXT,
                            uiIDA, 1, iPadNotSignedInLive,
                            &UIScene_LoadOrJoinMenu::MustSignInReturnedPSN,
                            this);
                    }
                    return;
                }
                else if (bPlayStationPlus == false) {
                        if(ProfileManager.RequestingPlaystationPlus(iPadWithNoPlaystationP// MGH -  added this so we don't try and upsell when we don't know if the player has PS Plus yet (if it can't connect to the PS Plus server).
				UINT uiIDA[1];
				uiIDA[0]=IDS_OK;
				ui.RequestAlertMessage(IDS_ERROR_NETWORK_TITLE, IDS_ERROR_NETWORK, uiIDA, 1, ProfileManager.GetPrimaryPad(), NULL, NULL);
				re// PS Plus upsell// 4J-PB - we're not allowed to show the text Playstation Plus - have to call the upsell all the time!// upsell psplus
			...
			iResult=sceNpCommerceDialogOpe//                     UINT uiIDA[2];//                     uiIDA[0]=IDS_CONFIRM_OK;//                     uiIDA[1]=IDS_PLAYSTATIONPLUS_SIGNUP;//                     ui.RequestMessageBox( IDS_FAILED_TO_CREATE_GAME_TITLE, IDS_NO_PLAYSTATIONPLUS, uiIDA,2,ProfileManager.GetPrimaryPad(),&UIScene_LoadOrJoinMenu::PSPlusReturned,this, app.GetStringTable(),NULL,0,false);
...
//CScene_MultiGameInfo::JoinMenuInitData *initData = new CScene_MultiGameInfo::JoinMenuInitData();
		m_initData->iPad = 0;;
		m_initData->selectedSession = m_currentSessions->at( g// check that we have the texture pack available// If it's not the default texture pack
		if(m_initData->selectedSession->data.texturePackParentId!=0)
		{
                        int texturePacksCount =
                            Minecraft::GetInstance()
                                ->skins->getTexturePackCount();
                        bool bHasTexturePackInstalled = false;

                        for (int i = 0; i < texturePacksCount; i++) {
                            TexturePack* tp =
                                Minecraft::GetInstance()
                                    ->skins->getTexturePackByIndex(i);
                            if (tp->getDLCParentPackId() ==
                                m_initData->selectedSession->data
                                    .texturePackParentId) {
                        ...
                        if(bHasTexturePackInstalled==f// upsell the texture pack// tell sentient about the upsell of the full version of the skin pack#ifdef _XBOX

				ULONGLONG ullOfferID_Full;
				app.GetDLCFullOfferIDForPackID(m_initData->selectedSession->data.texturePackParentId,&ullOfferID_Full);

				TelemetryManager->RecordUpsellPresented(m_iPad, eSet_UpsellID_Texture_DLC, ullOfferID_Full #endifFFFFFF);

				UINT uiIDA[2];

				uiIDA[0]=IDS_TEXTUREPACK_FU//uiIDA[1]=IDS_TEXTURE_PACK_TRIALVERSION;
				uiIDA[1]=IDS_CONFIRM// Give the player a warning about the texture pack missing
			...
			if (CGameNetworkManager::usingAdhocMode() && \
                            !SQRNetworkManager_AdHoc_Vita::          \
                                GetAdhocStat  // not connected to adhoc anymore,
                                              // must have connected back to PSN
                                              // to buy texture pack so sign in
                                              // again
                ...
                m_controlJoinTimer.setVis
#ifdef _XBOX;
  // Reset the background downloading, in case we changed it by attempting to download a texture pack
                    XBackgroundDownloadSetMode(XBACKGROUND_DOWNLO #endifE_AUTO);
                

		m_bIgnoreInput = true;
                ui.NavigateToScene(ProfileManager.GetPrimaryPad(),
                                   eUIScene_JoinMenu, m_initData);
            }
}

void UIScene_LoadOrJoinMenu::LoadLevelGen(LevelGenerationOptions *le// Load data from disc//File saveFile( L"Tutorial\\Tutorial" );//LoadSaveFromDisk(&saveFile);// clear out the app's terrain features list
    app.ClearTerrainFeaturePosition();

    StorageManager.ResetS// Make our next save default to the name of the level
    StorageManager.SetSaveTitle(levelGen->getDefaultSaveName().c_str());

    bool isClientSide = false;
    bool isPrivat// TODO int maxPlayers = MINECRAFT_NET_MAX_PLAYERS;
    int maxPlayers = 8;
    ...
    if(levelGen->requiresTexturePack())
    {
            ...
            Minecraft* pMinecraft = Minecraft::GetInstance();
        pMinecraft->skins->selectTexturePackById(param->texturePa//pMinecraft->skins->updateUI();
#ifndef _XBOX

    g_NetworkManager.FakeLocalP#endifoined();
...
{
            if (pParam != NULL) {
                UIScene_LoadOrJoinMenu* pScene =
                    (UIScene_LoadOrJoinMenu*)pParam;
                pScene->UpdateGamesList();
            }
}

void UIScene_LoadOrJoinMenu::UpdateGam// If we're ignoring input scene isn't active so do nothing
    if (m_bIgnoreInput// If a texture pack is loading, or will be loading, then ignore this ( we are going to be destroyed anyway)
    if( Minecraft::GetInstance()->skins->getSelected()->isLoadingData() || (Minecraft::GetInstance()->skins->needsUIUpdate() || ui.IsReloadingSkin()) // if we're retrieving save info, don't show the list yet as we will be ignoring press events
    if(!m_bSavesDisplayed)
    {
            return;
    }
    ...
    m_controlJoinTimer.setVisible(// if the saves list has focus, then we should show the Delete Save tooltip// if the games list has focus, then we should show the View Gamercard tooltip
    int iRB=-1;	
    int iY = -1;
    int iX=-1;

    delete m_currentSessions;
    m_currentSessions = g_NetworkManager.GetSessionList( m_iPad, 1, m_bShowingPartyGam// Update the xui list displayed
    ...
    if(filteredListS#if TO_BE_IMPLEMENTED
        if( !m_pGamesList->IsEnabled() )
        {
            m_pGamesList->SetEnable(TRUE);
            m_pGamesList->SetCurSel( #endif       }

        m_labelNoGames.setVisible( false );
        m_controlJoinTimer.setVisible( false );
}
#if TO_BE_IMPLEMENTED
        m_pGamesList->Set #endif(FALSE);

        m_controlJoinTimer.setVisible(false);
m_labelNoGames
    .setVi
#if TO_BE_IMPLEMENTED
        if (gamesListHasFocus) m_pGamesList->Init #endifm_iPad);
  // clear out the games list and re-fill
    m_buttonListGames.clearList();

if (filteredListSize > 0)  // Reset the focus to the selected session if it
                           // still exists
...
for (AUTO_VAR(it, m_currentSessions->begin()); it < m_currentSessions->end();
    ...
    wchar_t text "\0" ame[64] =
        L  // Is this a default game or a texture pack game?
        ...
        if (tp == NULL) {
            ...
            app.GetFileFromTPD(eTPDFileType_Icon, pbData, dwBytes, &pbImageData,
                               &dwImageBytes);
            if (dwImageBytes > 0 && pbImageData) {
                swprintf(t "%ls" eName, 64, L, sessionInfo->displayLabel);
...
bool UIScene_LoadOrJoinMenu::DoesSavesListHaveFocus() {
        if( m_buttonListSaves.has// check it's not the first or second element (new world or tutorial)
...
bool UIScene_LoadOrJoinMenu::DoesMashUpWorldHaveFocus() {
        if(m_buttonListSaves.ha// check it's not the first or second element (new world or tutorial)
		if(m_iSaveListIndex > (m_iDefaultButtonsC - 1))
		{
            return false;
		}
...
void UIScene_LoadOrJoinMenu::handleTimerComplete(int id) {
    switch (id) {
    case JOIN_LOAD_ONLINE_TIMER#ifdef _XBOX {
     ...
     // 					m_CheckboxPrivate.SetEnable(TRUE);
        ...
        // 4J-PB - Only Xbox will not have trial DLC patched into the game#ifdef _XBOX

    case CHECKFORAVAILABLETEXTUREPACKS_TIMER
#if defined(__PS3__) || defined(__ORBIS__) || defined(__PSVITA__)
         ...
         // retrieve the image - if we haven't already
                        std::wstring textureName = filenametowstring(pDLCInfo->chImageURL);
...
void UIScene_LoadOrJoinMenu::LoadSaveFromDisk(File *saveFile, ESavePl/*= SAVE_FILE_PLATFORM_LOCAL*/// we'll only be coming in here when the tutorial is loaded now

    StorageManager.Re// Make our next save default to the name of the level
    ...
    g_NetworkManager.FakeL#endifayerJoined();
    ...
    ui.NavigateToScene(ProfileManager.GetPrimaryPad(),eUIScene_FullscreenProgress
#ifdef SONY_REMOTE_STORAGE_DOWNLOAD
void UIScene_LoadOrJoinMenu::LoadSaveFromCloud()
{	

    wchar_t wFileName[128];
    mbstowcs(wFileName, app.getRemoteStorage()->getLocalFilename(), strlen(app.getRemoteStorage()->getL// plus null())+1); 
    File cloudFile(wFileName);


    StorageManager.Re// Make our next save default to the name of the level
    wchar_t wSaveName[128];
    mbstowcs(wSaveName, app.getRemoteStorage()->getSaveNameUTF8(), strlen(app.getRemoteStorage()->get// plus null())+1); 
	...
	param->texturePackId = app.getRemoteStorage()->ge
#ifndef _XBOXck();
...
{
    UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJo// results switched for this dialog// Check that we have a valid save selected (can get a bad index if the save list has been refreshed)
...
{
    wchar_t wSaveName[128];
    mbstowcs(wSaveName, remoteFilename, strlen(// plus nullme)+1)// 	processConsoleSave(wSaveName, L"ProcessedSave.bin");// 	File remoteFile(L"ProcessedSave.bin");
    File remoteFile(wSaveName);
    LoadSaveFromDisk(&remoteFile, SAVE_FI#endifTFORM_PS3);
}
...
{
    UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoi// results switched for this dialog// EMessage_ResultAccept means cancel
    switch(result)
    {
    case C4JStorage::EMess// renametDecline:  
        {
			pClass->m#ifdef _DURANGOrue;
// bring up a keyboard
            InputManager.RequestKeyboard(app.GetString(IDS_RENAME_WORLD_TITLE), (pClass->m_saveDetails[pClass->m_iSaveListIndex-pClass->m_iDefaultButtonsC]).UTF16SaveName,(DWORD)0,25,&UIScene_LoadOrJoinMenu::KeyboardCompleteWorldNameCallback,pClass,C_4JInput::EKey#elseMode_Default)// bring up a keyboard
            wchar_t wSaveNam//CD - Fix - We must memset the SaveName
            ZeroMemory(wSaveName, 128 * sizeof(wchar_t) );
            mbstowcs(wSaveName, pClass->m_saveDetails[pClass->m_iSaveListIndex - pClass->m_iDefaultButtonsC].UTF8SaveName, strlen(pClass->m_saveDetails// plus nullme)+1); 
            LPWSTR ptr = wSaveName;
            InputManager.RequestKeyboard(app.GetString(IDS_RENAME_WORLD_TITLE),wSaveName,(DWORD)0,25,&UIScene_LoadOrJoinMenu::KeyboardCompleteWorldNameCallback,pClass,C_4JInput::EKey#endifode_Default);

        }
        break;

    case C4JStorage::EMessage_// delete -Option:  
  // delete the save game// Have to ask the player if they are sure they want to delete this game
            UINT uiIDA[2];
            uiIDA[0]=IDS_CONFIRM_CANCEL;
            uiIDA[1]=IDS_CONFIRM_OK;
    ...
    case C4JStorage::EMessage_  // upload to cloud: 
        {
			UINT uiIDA[2];
			uiIDA[0]=IDS_CONFIRM_OK;
			uiIDA[1]=IDS_CONFIRM_CANCEL;
        ...
        // defined __ORBIS__
    case C4JStorage::EMessage_// copy saveOption: 
        {
			UINT uiIDA[2];
			uiIDA[0]=IDS_CONFIRM_OK;
			uiIDA[1]=IDS_CONFIRM_CANCEL;
...
int UIScene_LoadOrJoinMenu::SignInAdhocReturned(void* pParam, bool bContinue, \
    ...
    int UIScene_LoadOrJoinMenu::MustSignInTexturePack(
        void* pParam, int iPad, C4JStorage::EMessageResult result) {
        UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu*)pParam;

        if (result == C4JStorage::EMessage_ResultAccept) {
            SQRNetworkManager_Vita::AttemptPSNSignIn(
                &UIScene_LoadOrJoinMenu::MustSignInReturnedTexturePack, pClass);
        } else {
            pClass->m_bIgnoreInput = false;
        }
    ...
    int UIScene_LoadOrJoinMenu::MustSignInReturnedTexturePack(
        ...
        if (bContinue == true) {
            SONYDLC* pSONYDLCInfo = app.GetSONYDLCInfo(
            ...
            if (pSONYDLCInfo != NULL) {
                ...
                char chSkuID[SCE_NP_COMMERCE2_SKU_ID_LEN];
                        ...
                        memset(chSkuID,0,SCE_NP_COMMER// we have to retrieve the skuid from the store info, it can't be hardcoded since Sony may change it.// So we assume the first sku for the product is the one we want// MGH -  keyname in the DLC file is 16 chars long, but there's no space for a NULL terminating char
			memset(chKeyName, 0, sizeof(chKeyName));
			strncpy(chKeyName, pSONYDLCInfo-
#ifdef __ORBIS__);

			strcpy(#elsee, chKeyName);
"%s-%s"rintf(chName,,app.GetCommerceCat#endif),chKeyName);

			app.GetDLCSkuIDFromProductList(// 4J-PB - need to check for an empty store
...
int UIScene_LoadOrJoinMenu::TexturePackDialogReturned(
        void* pParam, int iPad, C4JStorage::EMessageResult result) {
    UIScene_LoadOrJoinMenu *pClass = (UIScene_LoadOrJoin// Exit with or without saving
    if(result==C4JStorage::EMessage_ResultAcc// we need to enable background downloading for the DLC
        XBackgroundDownloadSetMode(XBACKGROUND_DOWNLOAD_#if defined __PSVITA__ || defined __PS3__ || defined __ORBIS__
#ifdef __PSVITA__

		if(!ProfileManager.IsSignedInLive(ProfileManager.GetPrimaryPad()) && CGameNetworkManager::using// get them to sign in to online
			UINT uiIDA[2];
			uiIDA[0]=IDS_PRO_NOTONLINE_ACCEPT;
			uiIDA[1]=IDS_PRO_NOTONLINE_DECLINE;
			ui.RequestAlertMessage(IDS_PRO_NOTONLINE_TITLE, IDS_PRO_XBOXLIVE_NOTIFICATION, uiIDA, 2, ProfileManager.GetPrimaryPad(),&UIScene_LoadOrJoinMenu::MustSignInTexturePack,pCla#endif		return;
    }
		...
		if (ProfileManager.IsSignedIn(iPad))
		{
                        if (ProfileManager.IsSignedInLive(iPad)) {
                            ...
                            StorageManager.InstallOffer(
                                1, (WCHAR*)ProductId.c_str(), NULL,
                                NULL)  // 4J-JEV: Fix for XB1: #165863 - XR-074:
                                       // Compliance: With no active network
                                       // connection user is unable to convert
                                       // from Trial to Full texture pack and is
                                       // not messaged
                                       // why.
                                UINT uiIDA[1] = {IDS_CONFIRM_OK};
                            ui.RequestErrorMessage(
                                IDS_PRO_NOTONLINE_TITLE,
                                IDS_PRO_XBOXLIVE_NOTIFICATION, ui #endif iPad);
                        }
		}
...
int UIScene_LoadOrJoinMenu::MustSignInReturnedPSN( \
        ...
        if(result==C4JStorage::EM#if defined(__PS3__) 
	{

		SQRNetworkManager_PS3::AttemptPSNSignIn(&UIScene_LoadOrJoinMenu::PSN_#elif defined __PSVITA__

		SQRNetworkManager_Vita::AttemptPSNSignIn(&UIScene_LoadOrJoinMenu::PSN_#elsenReturned, pClass);

		SQRNetworkManager_Orbis::AttemptPSNSignIn(&UIScene_LoadOrJoinMenu::PSN_SignInReturne#endifass, false, iPad);
    ...
    return 0;
                ...
                int UIScene_LoadOrJoinMenu::PSN_SignInReturned(
                    void* pParam, bool bContinue, int iPad) {
                    UIScene_LoadOrJoinMenu* pClass =
                        (UIScene_LoadOrJoinMenu*)pParam;
                    if (bContinue == true) {
                        switch (pClass->m_eAction) {
                case// Check if we're signed in to LIVE
			if(ProfileManager.Is#if defined(__PS3__)			{

				int ret = sceNpBasicRecvMessageCustom(SCE_NP_BASIC_MESSAGE_MAIN_TYPE_INVITE, SCE_NP_BASIC_RECV_MESSAGE_OPTIONS_INCLUDE_BOOTABLE, SYS_MEMORY_CONTAINER_ID_INVAL"sceNpBasicRecvMessageCustom return %d ( %08x )\n"#elif defined __PSVITA__

				SQRNetworkManage#elsea::RecvInviteGUI();

				SQRNetworkManager#endif::RecvInviteGUI();
...
void UIScene_LoadOrJoinMenu::LaunchSaveTransfer()
                        {
                            ...
                            loadingParams->func =
                                &UIScene_LoadOrJoinMenu::
                                    DownloadSonyCrossSaveThreadProc;
                            ...
                            loadingParams->cancelFunc =
                                &UIScene_LoadOrJoinMenu::
                                    CancelSaveTransferCallback;
                            loadingParams->m_cancelFuncParam = this;
                            loadingParams->cancelText = IDS_TOOLTIPS_CANCEL;

                            ui.NavigateToScene(m_iPad,
                                               eUIScene_FullscreenProgress,
                                               loadingParams);
                        }

                        int UIScene_LoadOrJoinMenu::CreateDummySaveDataCallback(
                            LPVOID lpParam, bool bRes) {
                            UIScene_LoadOrJoinMenu* pClass =
                                (UIScene_LoadOrJoinMenu*)lpParam;
                            if (bRes) {
                                pClass->m_eSaveTransferState =
                                    eSaveTransfer_GetSavesInfo;
                            } else {
                pClass->m_eSaveTransferState = eSaveTransfer_"CreateDummySaveDataCallback failed\n");
                            }
                            return 0;
                        }
                        ...
                        UIScene_LoadOrJoinMenu::CrossSaveGetSavesInfoCallback(
                            LPVOID lpParam, SAVE_DETAILS * pSaveDetails,
                            bool bRes) {
                            UIScene_LoadOrJoinMenu* pClass =
                                (UIScene_LoadOrJoinMenu*)lpParam;
                            if (bRes) {
                ...
                pClass->m_eSaveTransferState = eSaveTransfer_"CrossSaveGetSavesInfoCallback failed\n");
                            }
                            return 0;
                        }

                        int UIScene_LoadOrJoinMenu::LoadCrossSaveDataCallback(
                            void* pParam, bool bIsCorrupt, bool bIsOwner) {
                            UIScene_LoadOrJoinMenu* pClass =
                                (UIScene_LoadOrJoinMenu*)pParam;
                ...
                pClass->m_eSaveTransferState = eSaveTransfer_"LoadCrossSaveDataCallback failed \n");
                            }
                            return 0;
                        }
                        ...
                        int UIScene_LoadOrJoinMenu::RemoteSaveNotFoundCallback(
                            ...
                            UIScene_LoadOrJoinMenu* pClass =
                                (UIScene_LoadOrJoinMenu*)pParam;
                            pClass->m_eSaveTransferState =
                                eSaveTran  // MGH -  added this global to force
                                           // the delete of the previous data,
                                           // for the remote storage saves//
                                           // need to speak to Chris why this is
                                           // necessary
                                bool g_bForceVitaSaveWipe = false;

                            int UIScene_LoadOrJoinMenu::
                                DownloadSonyCrossSaveThreadProc(
                                    LPVOID lpParameter) {
                                m_bSav #ifdef __PS3__ng = true;
                                
	StorageManager.SetSaveT #endifrInProgress(true);
                                
    Compression::UseDefaultThreadStorage();
                                UIScene_LoadOrJoinMenu* pClass =
                                    (UIScene_LoadOrJoinMenu*)lpParameter;
                                ...
                                while (pClass->m_eSaveTransferState !=
                                       eSaveTransfer_Idle) {
                                    switch (pClass->m_eSaveTransferState) {
                                        case eSaveTransfer_Idle:
                                            break;
                case eSaveTransfer_GetRemoteSave"UIScene_LoadOrJoinMenu getSaveInfo\n");
		...
		case eSaveTransfer_GettingRemoteSaveInfo:
                    ...
                    if (app.getRemoteStorage()->waitingForSaveInfo() == false) {
                        if (app.getRemoteStorage()->saveIsAvailable()) {
                            if (app.getRemoteStorage()
                                ...
                                // must be a newer version of the save in the
                                // cloud that we don't support
                                // yet
                                UINT uiIDA[1];
                                uiIDA[0] = IDS_CONFIRM_OK;
                                ui.RequestAlertMessage(
				...
				StorageManager.GetDefaultSaveImage(&pbDataSaveImage, // Get the default save thumbnail (as set by SetDefaultImages) for use on saving games t
				StorageManager.GetDefaultSaveThumbnail(&pbThumbnailData// Get the default save image (as set by SetDefaultImages) for use on saving games that 

				BYTE bTextMetadata[88];
				ZeroMemory(bTextMetadata,88);
				unsigned int hostOptions = app.getRemoteStorage(#ifdef __ORBIS__tions();

				app.SetGameHostOption(hostOptions, eGameHostOption_WorldSize// force the classic world size on, otherwise it's unknown and we can't expand#endif

				int iTextMetadataBytes = app.CreateImageTextData(bTextMetadata, app.getRemoteStorage()->getSaveSeed(), true, hostOptions, app.getRemoteStorage()->getS// set the icon and save image
		...
		"Failed to create dummy save file\n");
					pClass->m_eSaveTransferState = eSaveTransfer_Error;
                }
                                    }
                                    break;
                                    case eSaveTransfer_CreatingDummyFile:
                                        break;
                case eSaveTransfe// we can't cancel here, we need the saves info so we can delete the file
				if(pClass->m_saveTransferDownloadCancelled)
				{	
					WCHAR wcTemp[256];
					swprintf(wcTemp,256, app.// MGH - should change this string to "cancelling download"
					m_wstrStageText=wcTemp;
					pMinecraft->progressRenderer->progressStage( m_wstrStageText );
				}

				app.getRemoteStorage()->waitForStorageManagerIdl"CALL GetSavesInfo B\n"f();
				C4JStorage::ESaveGameState eSGIStatus= StorageManager.GetSavesInfo(pClass->m_iPad,&UIScene_LoadOrJoinMenu::CrossSaveGet"save"nfoCallback,pClass,);
		...
		case eSaveTransfer_GettingSavesInfo:
                ...
                case eSaveTransfer_GetFileData: {
                    ...
                    bool bGettingOK = app.getRemoteStorage()->getSaveData(
                        pClass->m_downloadedUniqueFilename,
                        SaveTransferReturned, pClass);
                    if (bGettingOK) {
                        pClass->m_eSaveTransferState =
                            eSaveTransfer_GettingFileData;
                    } else {
                                pClass->m_eSaveTransferState = eSaveTransfer_Er"app.getRemoteStorage()->getSaveData failed\n");
                    }
                }
                ...
                case eSaveTransfer_GettingFileData: {
                    ...
                    int dataProgress =
                        app.getRemoteStorage()->getDataProgress();
                pMinecraft->progressRenderer->progressStagePercentage(dataPro//swprintf(wcTemp, 256, L"Downloading data : %d", dataProgress);//app.GetString(IDS_SAVETRANSFER_STAGE_GET_DATA),0,pClass->m_ulFileSize);
                ...
                case eSaveTransfer_LoadSaveFromDisc: {
                    ...
                    for (int i = 0; i < pSaveDetails->iSaveC; i++) {
                                if(strcmp(pSaveDetails->SaveInfoA[i].UTF8SaveFilename, pClass->m_downloadedUniqueFi//found it 0)
				{
                    ...
                    if (saveInfoIndex == -1) {
                        pClass->m_eSaveTransferState = eSaveTransfer_Er
                            "CrossSaveGetSavesInfoCallback failed - couldn't "
                            "find "
                            "save\n"#ifdef __PS3__ else  // ignore the CRC on PS3	
                            C4JStorage::ESaveGameState eLoadStatus =
                                StorageManager.LoadSaveData(
                                    &pSaveDetails->SaveInfoA[saveInfoIndex],
                                    &LoadCrossSaveDat #elseback, pClass, true);
                        
				C4JStorage::ESaveGameState eLoadStatus =
                            StorageManager.LoadSaveData(
                                &pSaveDetails->SaveInfoA[saveInfoIndex],
                                &LoadCrossS #endifaCallback, pClass);
                ...
                case eSaveTransfer_CreatingNewSave: {
					...
					int iTextMetadataBytes = app.CreateImageTextData(bTextMetadata, app.getRemoteStorage()->getSaveSeed(), true, remoteHostOptions, app.getRemoteStorage()->getSa// set the icon and save image
					StorageManager.SetSaveImages(pbThumbnailData,dwThumbnailDataSize,pbDataSaveImage,dwDataSizeSaveImage,bTextMetadata,iT
                ...
                pClass->m_eSaveTransferState#elseaveTransfer_Saving;
				...
				pMinecraft->progressRenderer->progressStage(IDS_SAVETRA#endifSTAGE_CONVERTING);
                ...
                delete ba.data;
                    ...
                    case eSaveTransfer_Converting: {
                        pSave
                            ->C  // check if we need to convert this file from
                                 // PS3->PS4
                                     pClass->m_eSaveTransferState =
		...
		case eSaveTran// if we've arrived here, the save has been created successfully
				pClass->m_iState=e_SavesRepopulate;
				pClass->updateTooltips();
				UINT uiIDA[1];
				uiIDA[0]=IDS_CONFIRM_OK;
				app.getRemoteStorage()->wait// wait for everything to complete before we hand control back to the player
		...
		case // this is no longer used//pClass->m_eSaveTransferState = eSaveTransfer_Idle;
			}
			break;
            ...
            if (bSaveFileCreated) {
                if (pClass->m_saveTransferDownloadCancelled) {
                    WCHAR wcTemp[256];
						...
						m_wstrStageText=wcTemp;
						pMinecraft->progressRenderer->progressStage( m_wstrStageText );
						pMinecraft->progressRenderer->progressStage( m_wstr// if the save file has already been created we have to delete it again if there's been an error
					PSAVE_DETAILS pSaveDetails=StorageManager.ReturnSavesInfo();
					int saveInfoIndex = -1;
					for(int i=0;i<pSaveDetails->iSaveC;i++)
					{
                                                if(strcmp(pSaveDetails->SaveInfoA[i].UTF8SaveFilename, pClass->m_downloadedUniqueFilena//found it						{
							
							saveInfoIndex = i;
						}
                }
                if (saveInfoIndex == -1)
                                "eSaveTransfer_Error failed - couldn't find save\n");
                assert(0);
                ...
                pClass->m_eSaveTransferState = eSaveTransfer_ErrorMesssage;
                        ...
                        pClass->m_eSaveTransferState =
                            eSaveTransfer_ErrorMesssage;
                                ...
                                case eSaveTransfer_ErrorMesssage: {
                                    app.getRemoteStorage()
                                        ->wait  // wait for everything to
                                                // complete before we hand
                                                // control back to the
                                                // player
                                        if (pClass
                                                ->m_saveTransferDownloadCancelled) {
                                        pClass->m_eSaveTransferState =
                                            eSaveTransfer_Idle;
                                    }
                                    else {
                                        ...
                                        uiIDA[0] = IDS_CONFIRM_OK;
                                        UINT errorMessage =
                                            IDS_SAVE_TRANSFER_DOWNLOADFAILED;
                                        if (!ProfileManager.IsSignedInLive(
                                                ProfileManager
                                                    .GetPrimaryPad())) {
                                            errorMessag  // show "A network
                                                         // error has
                                                         // occurred."#ifdef
                                                         // __ORBIS__
						...
						if (!ProfileManager.IsSignedInPSN(
                                            ...
                                            ui.RequestErrorMessage(
                                                ...
                                                ProfileManager.GetPrimaryPad(),
                                                CrossSaveFinishedCallback,
                                                pClass);
                                            pClass->m_eSaveTransferState =
                                                eSaveTransfer_Finished;
                                        }
                                        // save file has been created, then
                                        // deleted.
                                        pClass->m_iState =
                                            e_SavesRepopulateAfterDelete;
                                        else pClass->m_iState =
                                            e_SavesRepopulate;
                                        pClass->updateTooltips();
                                    }
                                    break;
                case eSaveTransfe// waiting to dismiss the dialog
			break;
        }
        Sleep(50);
    }
	m_bSave#ifdef __PS3__g = false;

	StorageManager.SetSaveTr#endifInProgress(false);
...
void UIScene_LoadOrJoinMenu::SaveTransferReturned(LPVOID lpParam, SonyRemoteStorage::Status s, int error_code)
{
	...
	{
        pClass->m_eSaveTransferState = eSaveTransfer_"SaveTransferReturned failed with error code : 0x%08x\n", error_code);
	}
...
void UIScene_LoadOrJoinMenu::LaunchSaveUpload()
{
    ...
    completionData->iPad = DEFAULT_XUI_MENU_USER;
    loadingParams->completio// 4J-PB - Waiting for Sony to fix canceling a save upload
...
int UIScene_LoadOrJoinMenu::UploadSonyCrossSaveThreadProc( LPVOID lpParameter )
{
	...
	bool bAbortCalled = false;
    Minecraft *pMinecraft=Minecr// get the save file size
    ...
    while(pClass->m_eSaveUploadState!=eSaveUpload_Idle)
    {
        ...
        case eSaveUpload_Idle:
                ...
                case eSaveUpload_UploadingFileData: {
                    ...
                    int dataProgress =
                        app.getRemoteStorage()->getDataProgress();
                pMinecraft->progressRenderer->progressStagePercentage(dataPro//swprintf(wcTemp, 256, L"Uploading data : %d", dataProgress);//app.GetString(IDS_SAVETRANSFER_STAGE_GET_DATA),0,pClass->m_ulFileSize);
                swprintf(wcTemp,256, app.GetString(IDS_SAVETRANSFER_STAGE_PUT_DATA),dataProgress);

                m_wstrStageText=wcTemp;
                pMinecraft->progressRenderer->progressS// 4J-PB - Waiting for Sony to fix canceling a save upload
				if(pClass->m_saveTransferUploadCancelled && bAbortCal// we only really want to be able to cancel during the download of data, if it's taking a long time
					app.getRemoteStorage()->abort();
					bAbortCalled = true;
                }
                                } break;
                                case eSaveUpload_FileDataUploaded: {
                                    ...
                                    ui.RequestErrorMessage(
                                        ...
                                        CrossSaveUploadFinishedCallback,
                                        pClass);
                                    pClass->m_eSaveUploadState =
                                        esaveUpload_Finished;
                                } break;
                                    cas  // this is no longer
                                         // used//
                                         // pClass->m_eSaveUploadState =
                                         // eSaveUpload_Idle;
                                        break;
                ...
                case // waiting for dialog to be dismissed
			break;
        }
        Sleep(50);
    }
...
{
    UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu *) lpParam;
	pClass->m_saveTransferUploadCancelled "m_saveTransferUploadCancelled = true\n");
	ui.SetTooltips( DEFAULT_XUI_MENU_USER, -1, -// MGH -  added - remove the "cancel" tooltip, so the player knows it's underway (really needs a "cancelling" message)
...
{
	UIScene_LoadOrJoinMenu* pClass = (UIScene_// results switched for this dialog
	if(result==C4JStorage::EMes// upload the save 
	{
	...
	{
		pClass->m_bIgnoreInpu#endif // SONY_REMOTE_STORAGE_UPLOAD
...
int UIScene_LoadOrJoinMenu::DownloadXbox360SaveThreadProc( LPVOID lpParameter )
{
    ...
    RequestFil"completemarker"tainer, L );
    ...
    while((pStateContainer->m_eSaveTransferState!=C4JStorage::eSaveTransfer_Idle) && pStateContainer->m_bSaveTransferInProgress && !pStateContainer->m_bSaveTransferCancelled)
    {
        ...
        case C4JStorage::eSaveTransfer_Idle:
                    ...
                    switch (UIScene_LoadOrJoinMenu::s_eSaveTransferFile) {
                        case eSaveTransferFile_Marker:
                            if (UIScene_LoadOrJoinMenu::s_ulFileSize == 0) {
                                pMinecraft->progressRenderer->progressStage(
                                    IDS_SAVETRANSFER_NONE_FOUND);
                                pStateContainer->m_eSaveTransferState =
                ...
                RequestFil"metadata"ateContainer, L );
                break;
                        case eSaveTransferFile_SaveData:
                RequestFil"savedata"ateContainer, L );
                break;
                    };
                    break;
                ...
                case C4JStorage::eSaveTransfer_FileDataRetrieved:
                    switch (UIScene_LoadOrJoinMenu::s_eSaveTransferFile) {
            case eSave// MGH - the marker file now contains the save file version number// if the version is higher than we handle, cancel the download.
            ...
            case eSaveTransferFile_Metadata:
                ...
                std::wstring
                    sav  // 4J Stu - Don't set this any more. We added it so
                         // that we could share the ban list data for this
                         // save// However if the player downloads the
                         // same save multiple times, it will overwrite the
                         // previous version // with that filname, and
                         // they could have made changes to
                         // it.//StorageManager.SetSaveUniqueFilename((wchar_t
                         // *)saveUniqueName.c_str());

                    int thumbnailSize = dis.readInt();
                if (thumbnailSize > 0) {
                    byteArray ba(thumbnailSize);
                    // retrieve the seed value from the image metadata, we need
                    // to change to host options, then set it back
                    // again
                    ...
                    app.GetImageTextData(ba.data, ba.length,
                                         (unsigned char*)&szSeed, uiHostOptions,
                                         bHostOptionsRead, dwTe "%I64d" ck);
                    sscanf_s(szSeed, , &seedVal);

                                                app.SetGameHostOption(uiHostOptions, eGameHostOption// force the classic world size on, otherwise it's unknown and we can't expand


						BYTE bTextMetadata[88];
						ZeroMemory(bTextMetadata,88);

						int iTextMetadataBytes = app.CreateImageTextData(bTextMetadata, seedVal, true, ui// set the icon and save image					
                ...
                "savedata" eSize(pStateContainer, L );
            } break;
            case eSaveTransf#ifdef SPLIT_SAVES               {

					if(!pStateContainer->m_bSaveTransferCancelled) 
					{
						Conso"Temp name"riginal oldFormatSave( L, UIScene_LoadOrJoinMenu::s_transferData.data, UIScene_LoadOrJoinMenu::s_transferData.length, false, SAVE_FILE_PLATFORM_X360 );
						pSave = new ConsoleSaveFileSplit( &oldFormatSave, false, pMinecraft->progressRenderer );
            ...
            break;
        case C4J#if 0ge::eSaveTransfer_Converting:
            ...
            pSave->ConvertToLocalPlatform();
                ...
                if (pStateContainer->m_iProgress == 101) {
                    pStateContainer->m_eSaveTransferState =
                        C4JStorage::eSaveTransfer_Saving;
                    pStateContainer->m_iProgress = 0;
                    break;
                }
            pMinecraft->progressRenderer->progressStagePercent#endiftateContainer->m_iProgress);
            
            break;
            case C4JStorage:  // On Durango/Orbis, we need to wait for all the
                              // asynchronous saving processes to complete
                              // before destroying the levels, as that will
                              // ultimately delete// the directory
                              // level storage & therefore the ConsoleSaveSplit
                              // instance, which needs to be around until all
                              // the sub files have completed saving.#if
                              // defined(_DURANGO) ||
                              // defined(__ORBIS__)
                pMinecraft->progressRenderer->progressStage(
                ...
                // 4J Stu - DO NOT tick this here. The main thread should be the
                // only place ticking the StorageManager. You WILL get
                // crashes.//StorageManager.Tick();
                // #endif
                    }
#ifdef _XBOX_ONE delete pSave;
                    
			pMinecraft->progressRenderer->progressStage(
                        IDS_SAVE_TRANSF #endifNLOAD_AND_CONVERT_COMPLETE);
                    
			
            pStateContainer->m_eSaveTransferState =
                        C4JSt  // wipe the list and repopulate it
                               // 
                        if (!pStateContainer->m_bSaveTransferCancelled)
                            pStateContainer->m_pClass->m_iState =
                            e_SavesRepopulateA  // pClass->m_iProgress+=1;
                    ...
                    void UIScene_LoadOrJoinMenu::RequestFileSize(
                        SaveTransferStateContainer * pClass,
                        wchar_t * filename) {
                        Minecraft* pMinecr  // get the save file sizee();

    
    pMinecraft->progressRenderer->progressStart(IDS_SAVETRANSFER_TITLE_GET);
    pMinecraft->progressRenderer->progressStage( IDS
#ifdef _DEBUG_MENUS_ENABLEDILS);
    ...
    if(app.GetLoadSavesFromFolderEnabled())
    {
                            ...
                            "FakeTMSPP\\" targetFile(
                                std::wstring(L).append(filename));
                            if (targetFile.exists())
                                m_debugTransferDetails.ulFileLen =
                                    targetFile.length();

                            SaveTransferReturned(pClass,
                                                 &#endifgTransferDetails);
    }
    else

    {
                                ...
                                pClass->m_eSaveTransferState =
                                    StorageManager.SaveTransferGetDetails(
                                        ...
                                        pClass);
                                         ...
                                         C4JStorage::eSaveTransfer_Busy &&
                                     !pClass->m_bSaveTransferCancelled);
    }
                    }
                    ...
                    void UIScene_LoadOrJoinMenu::RequestFileData(
                        ...
                        swprintf(wcTemp,
                                 ...
                                 0, UIScene_LoadOrJoinMenu::s_ulFileSize);
                        m_wstrStageText = wcTemp;

                        pMinecraft->progressRend
#ifdef _DEBUG_MENUS_ENABLEDtageText);

    if (app.GetLoadSavesFromFolderEnabled()) "FakeTMSPP\\" File targetFile(
                            std::wstring(L).append(filename));
                        if (targetFile.exists()) {
                            HANDLE hSaveFile =
                                CreateFile(targetFile.getPath().c_str(),
                                           GENERIC_READ, 0, NULL, OPEN_EXISTING,
                                           FILE_FLAG_RANDOM_ACCESS, NULL);

                            m_debugTransferDetails.pbData =
                                new BYTE[m_debugTransferDetails.ulFileLen];

                            DWORD numberOfBytesRead = 0;
                            ReadFile(hSaveFile, m_debugTransferDetails.pbData,
                                     m_debugTransferDetails.ulFileLen,
                    ...
                    else 
    {
                        do {
                            ...
                            Sleep(1);
                            pClass->m_eSaveTransferState =
                                StorageManager.SaveTransferGetData(
                                    pClass->m_iPad,
                                    C4JStorage::eGlobalStorage_TitleUser,
                                    filename,
                                    &UIScene_LoadOrJoinMenu::
                ...
                int UIScene_LoadOrJoinMenu::SaveTransferReturned(
                    LPVOID lpParam, C4JStorage::SAVETRANSFER_FILE_DETAILS *
                                        pSaveTransferDetails) {
    SaveTransferStateContainer* pClass = (SaveTransferSt"Save Transfer - size is %d\n"pp.DebugPrintf(// if the file data is null, then assume this is the file size retrieval
    if(pSaveTransferDetails->pbData==NULL)
    {
                        pClass->m_eSaveTransferState =
                            C4JStorage::eSaveTransfer_FileSizeRetrieved;
                        UIScene_LoadOrJoinMenu::s_ulFileSize =
    ...
    else
    {
                        delete UIScene_LoadOrJoinMenu::s_transferData.data;
                        UIScene_LoadOrJoinMenu::s_transferData =
                            byteArray(pSaveTransferDetails->pbData,
                                      UIScene_LoadOrJoinMenu::s_ulFileSize);
                        pClass->m_eSaveTransferState =
                            C4JStorage::eSaveTransfer_FileDataRetrieved;
    }

    return 0;
                }

                int UIScene_LoadOrJoinMenu::SaveTransferUpdateProgress(
                    LPVOID lpParam, unsigned long ulBytesReceived) {
                    WCHAR wcTemp[256];

                    SaveTransferStateContainer* pClass =
                        (SaveTransferStateContainer*)lpParam;
    Minecraft *pMinecraft=Minecraft::GetInstance(// was cancelleds->m_bSaveTransferCancelled) 
    {
                        pMinecraft->progressRenderer->progressStage(
                            IDS_SAVE_TRANSFER_DOWNLOAD_CANCELLING);
                        swprintf(wcTemp,
                                 app.GetString(
                                     IDS_SAVE_TRANSFER_DOWNLOAD_CANCELLING));
                        m_wstrStageText = wcTemp;
    ...
    else
    {
                        unsigned int uiProgress =
                            (unsigned int)(((float)ulBytesReceived /
                                            float(UIScene_LoadOrJoinMenu::
                                                      s_ulFileSize)) *
                                           100.0f);
                        ...
                        swprintf(wcTemp,
                                 app.GetString(IDS_SAVETRANSFER_STAGE_GET_DATA),
                                 ((float)(ulBytesReceived)) / 1024000.0f,
                                 ((float)UIScene_LoadOrJoinMenu::s_ulFileSize) /
                                     1024000.0f);
                        m_wstrStageText = wcTemp;
                        pMinecraft->progressRenderer->progressStage(
                            m_wstrStageText);
    ...
    return 0;
                ...
                void UIScene_LoadOrJoinMenu::CancelSaveTransferCallback(
                    LPVOID lpParam) {
                    SaveTransferStateContainer* pClass =
                        (SaveTransferStateContainer*)lpParam;

                    if (!pClass->m_bSaveTransferCancelled) {
                        StorageManager.CancelSaveTransfer(
                            UIScene_LoadOrJoinMenu::
                                CancelSaveTransferCompleteCallback,
                            pClass);

                        pCla  // pClass->m_bSaveTransferInProgress=false;
                              // 
                    }
                    ...
                    UIScene_LoadOrJoinMenu::CancelSaveTransferCompleteCallback(
                        ...
                        SaveTransferStateContainer*
                            pClass =  // change the state to idle to get the
                                      // download thread to
                                      // terminate
                            pClass->m_eSaveTransferState =
                                C4JStorage::eSaveTransfer_Idle;
                    ...
                    int UIScene_LoadOrJoinMenu::NeedSyncMessageReturned(
                        void* pParam, int iPad,
                        C4JStorage::EMessageResult result) {
                        UIScene_LoadOrJoinMenu* pClass =
                            (UIScene_LoadOrJoinMenu*)pParam;
    ...
    if( result == C4JSt// navigate to the settings scene  {
        ...
        void* pParam, int iPad, C4JStorage::EMessageResult result)
                    {
                        ...
                        if (result == C4JStorage::EMessage_ResultAccept) {
                            ...
                            loadingParams->func =
                                &UIScene_LoadOrJoinMenu::CopySaveThreadProc;
                            ...
                            loadingParams->cancelFunc =
                                &UIScene_LoadOrJoinMenu::CancelCopySaveCallback;
                            loadingParams->m_cancelFuncParam = uniqueId;
                            loadingParams->cancelText = IDS_TOOLTIPS_CANCEL;

                            ui.NavigateToScene(iPad,
                                               eUIScene_FullscreenProgress,
                                               loadingParams);
                        } else {
                            pClass->m_bIgnoreInput = false;
                    ...
                    int UIScene_LoadOrJoinMenu::CopySaveThreadProc(
                        LPVOID lpParameter) {
                        Minecraft* pMinecraft = Minecraft::GetInstance();
                        pMinecraft->progressRenderer->progressStart(
                            IDS_PROGRESS_COPYING_SAVE);
                        pMinecraft->progressRenderer->progressStage(-1);
                        ...
                        if (pClass) {
                            ...
                            pClass->m_bCopyingCancelled =
                                fal  // Copy save data takes two callbacks - one
                                     // for completion, and one for progress.
                                     // The progress callback also lets us
                                     // cancel the operation, if we return
                                     // false.
                                         StorageManager.CopySaveData(
                                             &pClass->m_pSaveDetails->SaveInfoA
                                                  [pClass->m_iSaveListIndex -
                                                   pClass->m_iDefaultButtonsC],
                                             UIScene_LoadOrJoinMenu::
                                                 CopySaveDataReturned,
                                             UIScene_LoadOrJoinMenu::
                                                 CopySaveDataProgress,
                                             lpParameter);
                            ...
                            do {
                                Sleep(100);
                                ui.EnterCallbackIdCriticalSection();
                                pClass = (UIScene_LoadOrJoinMenu*)
                                             ui.GetSceneFromCallbackId(
                                                 (size_t)lpParameter);
                                if (pClass) {
                    ...
                    int UIScene_LoadOrJoinMenu::CopySaveDataReturned(
                        LPVOID lpParam, bool success,
                        C4JStorage::ESaveGameState stat) {
                        ui.EnterCallbackIdCriticalSection();
                        UIScene_LoadOrJoinMenu* pClass =
                            (UIScene_LoadOrJoinMenu*)ui.GetSceneFromCallbackId(
                                (size_t)lpParam);
                        ...
                        if (pClass) {
                if(su// wipe the list and repopulate it false;
			
			pClass->m_iState=e_SavesRepopulateAfterDelete;		
			ui.Leav#ifdef __ORBIS__calSection();
                        }
                        el  // you cancelled the save on exit after choosing
                            // exit and save? You go back to the Exit choices
                            // then.
                                uiIDA[0] = IDS_OK;
                        ...
                        if (stat ==
                            ...
                            ui.LeaveCallbackIdCriticalSection();
                            ui.RequestErrorMessage(
                                IDS_COPYSAVE_FAILED_TITLE,
                                IDS_COPYSAVE_FAILED_LOCAL, uiIDA, 1,
                                ProfileManager.GetPrimaryPad(),
                                CopySaveErrorDialogFinishedCallback, lpParam);
                        } else if (stat ==
                                   C4JStorage::
                            ...
                            ui.RequestErrorMessage(
                                IDS_COPYSAVE_FAILED_TITLE,
                                IDS_COPYSAVE_FAILED_QUOTA, uiIDA, 1,
                                ProfileManager.GetPrimaryPad(),
                                CopySaveErrorDialogFinishedCallback, lpParam);
                        } else {
                            pClass->m_bCopying = false;
#else.LeaveCallbackIdCriticalSection();
                }
                
			pClass->m_bCopying =
                    fa #endif ui.LeaveCallbackIdCriticalSection();
...
int UIScene_LoadOrJoinMenu::CopySaveErrorDialogFinishedCallback(
    void* pParam, int iPad, C4JStorage::EMessageResult result) {
    ui.EnterCallbackIdCriticalSection();
    UIScene_LoadOrJoinMenu* pClass =
        (UIScene_LoadOrJoinMenu*)ui.GetSceneFromCallbackId((size_t)pParam);
    if (pClass) {
        pClass->m_bCopying = false;
    }
    ui.Lea
#endif  // _XBOX_ONESection();
