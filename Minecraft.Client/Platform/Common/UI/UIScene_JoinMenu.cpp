...
void UIScene_JoinMenu::tick() {
    ...
    playerName);
		...
		m_labelLabels[eLabel_Difficulty].init(
            ...
            if (m_friendInfoUpdatedERROR) {
                m_buttonJoinGame.init(app.GetString(IDS_JOIN_GAME),
                                      eControl_JoinGame);
                ...
                m_friendInfoUpdatedERROR =
                    false  // Show a generic network error message, not
                           // always safe to assume the error was host
                           // quitting// without bubbling more info up
                           // from the network manager so this is the best
                           // we can do
                    UINT uiIDA[1];
        ...
        void UIScene_JoinMenu::friendSessionUpdated(bool success,
                                                    void* pParam) {
        ...
        std::wstring UIScene_JoinMenu::getMoviePath() { ret "JoinMenu"; }

        void UIScene_JoinMenu::handleInput(int iPad, int key, bool repeat,
	...
	case ACTION_MENU_Y:
                    if (m_selectedSession != NULL &&
                        ...
                        PlayerUID uid =
                            m_selectedSession->searchResult.m_playerXuids
        ...
        void UIScene_JoinMenu::handlePress(F64 controlId, F64 childId) {
            switch ((int)controlId) {
                case eControl_JoinGame: {
                    m_bIgnoreInput = true;  // CD - Added for audio
                        ui.PlayUISFX(eSFX_Pre
#ifdef _DURANGO
...
// Check if user-created content is allowed, as we cannot play multiplayer if it's not
	...
	if (noUGC) {
                                        ...
                                        ui.RequestUGCMessageBox(
                                            IDS_CONNECTION_FAILED, messageText);
                                    }
		...
		CGameNetworkManager::eJoinGameResult result =
                                        ...
                                        if (result != CGameNetworkManager::
                                            ...
                                            switch (result) {
                                                case CGameNetworkManager::
                                                    JOINGAME_FAIL_SERVER_FULL:
                                                    exitReasonStringId =
                                                        IDS_DISCONNECTED_SERVER_FULL;
                                                    break;
                                            }
                                            ...
                                            if (exitReasonStringId == -1) {
                                                ...
                                                uiIDA[0] = IDS_CONFIRM_OK;
                                                ui.RequestErrorMessage(
                                                    IDS_CONNECTION_FAILED,
                                                    exitReasonStringId, uiIDA,
                                                    1,
                                                    ProfileManager
                                                        .GetPrimaryPad());
			...
			PlayerUID selectedPlayerXUID =
                                        ...
                                        bool success =
                                            g_NetworkManager.GetGameSessionInfo(
                                                m_iPad,
                                                m_selectedSession->sessionId,
                                                m_selectedSession);

                                        if (success) {
                                            playersList.DeleteItems(
                                                0, playersList.GetItemCount());
                                            int selectedIndex = 0;
                                            for (unsigned int i = 0;
                                                 i < MINECRAFT_NET_MAX_PLAYERS;
                                                 ++i) {
                                                if (m_selectedSession->data
                                                        .players[i] != NULL) {
                                                    if (m_selectedSession->data
                                                            .players[i] ==
                                                        selectedPlayerXUID)
                                                        selectedIndex = i;
                                                playersList.InsertItems(i#ifndef _CONTENT_PACKAGE
						if(app.DebugSettingsOn() && (app.GetGameSettingsDebugMask()&(1L<<eDebugSetting_DebugLeaderboards)))
						{
                                                        playersList.SetText"WWWWWWWWWWWWWWWW" );
						}
#endif 
						{
