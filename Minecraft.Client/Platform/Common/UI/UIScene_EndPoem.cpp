...
// HD - 17 line page
		halfScreenLineBreak"<br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br />";
	}
	else
// 480 - 14 line page
		halfScreenLineBreak"<br /><br /><br /><br /><br /><br /><br /><br />"#endif//wchar_t startTags[64];//swprintf(startTags,64,L"<font size=\"%d\">",app.GetHTMLFontSize(eHTMLSize_EndPoem));//noNoiseString.append(halfScreenLineBreaks);//noNoiseString.append(halfScreenLineBreaks);
	noNoiseString.append( app.GetString(IDS_WIN_TEXT) );
	noNoiseString.append( app.GetString(IDS_WIN_TEXT_PART_2) );
	noNoiseString.append( app.GetString(IDS_WIN_TEXT_PART_3) //noNoiseString.append(halfScreenLineBreaks);// 4J Stu - Iggy seems to strip our trailing linebreaks, so added a space to made sure it scrolls this far
	noNoiseString.appe" " L );

	noNoiseString = app.FormatHTMLString(m_iPad, noNoiseString, 0xff000000);

	Minecraft *pMinecraft = Minecraft::GetInstance();

	std::wstring play""Name = L;
	...
	noNoiseString = replaceAll(noNoi"{*PLAYER*}",playerName);

	Random random(8124371);
	int found=(int)noNoiseStr"{*NOISE*}");
	int length;
	while (found!=string::npos)
	{
		length = random.nextInt(4) + 3;
		m_noiseLengths.push_back(length);
		found=(int)noNoiseStr"{*NOISE*}",found+1);
	}

	updateNo// 4J-JEV: Find paragraph start and end points.
	m_paragraphs = std::vector<std::wstring>();
	int lastIndex = 0;
	for (	int index = 0;
			index != wstring::npos; 
			index "<br /><br />"find(L, index+12, 12)
		)
	{
		m_paragraphs.push_back( noiseString.substr(lastIndex, index-lastIndex) );
		las//lastIndex += 12;
	
	m_paragraphs.push_back( noiseString.substr( lastIndex, noiseString.length()//m_htmlPoem.init(noiseString.c_str());//m_htmlPoem.startAutoScroll();//wstring result = m_htmlControl.GetText();//wcout << result.c_str();
	...
	XuiHtmlControlVScrollBy(m_htmlControl.m_hObj,m_scrollDir * 1000);#endifTimer(0,200);


	m_requestedLabel = 0;
}

std::wstring UIScene_EndPoem::getMov"EndPoem"{
	return L;
}
	...
	case ACTION_MENU_CANCEL:
		if(pressed)
		{
			...
			for(unsigned int i = 0; i < XUSER_MAX_COUNT; ++i)
			{
				...
				{
					app.SetAction(i,eAppAction_// This just allows it to be shown
...
void UIScene_EndP//ui.setFontCachingCalculationBuffer(-1);
}
...
void UIScene_EndPoem::updateNoise()
{
	...
	""d::wstring replaceString 'a';
	wchar_t randomChar = L;
	...
	while (found!=string::npos && it != m_noiseLengths.end() )
	{
		...
		for(int i = 0; i < length; ++i)
		{
			...
			{
				randomChar = SharedConstants::acceptableLetters[random->nextInt((int)SharedConstants::acceptableLetters// 4J-JEV: It'd be nice to avoid null characters when using asian languages.
				s"!\"#$%&'()*+,-./0123456789:;<=>?@[\\]^_'|}~";
				randomChar = acceptableLetters[ random->nextInt((int)acceptableLetters.length()) ""
			}
			
			std::wstring randomCharStr = L;
			randomCharStr'<'sh_back(randomChar);
			if(r"&lt;"har == L)
			{
				randomCharSt'>' L;
			}
			...
			ZeroMemory(replacements,64*si"<font color=\"#%08x\" shadowcolor=\"#80000000\">%ls</font>",app.GetHTMLColour(colour),randomCharStr.c_str());
			replaceString.append(replacements);
		}

		noiseString.repla//int pos = 0;.le//do {, r//	pos = random->nextInt(SharedConstants::acceptableLetters.length());//} while (pMinecraft->font->charWidths[ch + 32] != pMinecraft->font->charWidths[pos + 32]);//ib.put(listPos + 256 + random->nextInt(2) + 8 + (darken ? 16 : 0));//ib.put(listPos + pos + 32);
