...
int UIScene_AnvilMenu::KeyboardCompleteCallback(LPVOID lpParam,bool bRes// 4J HEG - No reason to set value if keyboard was cancelled
...
void UIScene_AnvilMenu::handleEditNamePressed() {
        setIgnoreInput(tr#if defined(__PS3__) || defined(__ORBIS__) || defined __PSVITA__
	...
	switch(language)
	{
        ...
        case XC_LANGUAGE_TCHINESE:
            InputManager.RequestKeyboard(
                ...
                (DWORD)m_iPad, 30, &UIScene_AnvilMenu::KeyboardCompleteCallback,
                this, C_4JInput::EKeyboardMode_Default);
            break;
            defaul  // 4J Stu - Use a different keyboard for non-asian languages
                    // so we don't have prediction on
                InputManager.RequestKeyboard(
                    app.GetString(IDS_TITLE_RENAME),
                    m_textInputAnvil.getLabel(), (DWORD)m_iPad, 30,
                    &UIScene_AnvilMenu::KeyboardCompleteCallback, this,
                    C_4JInput::EKeyboardMode_Alphabet_Extended);
                break#else
	InputManager.RequestKeyboard(app.GetString(IDS_TITLE_RENAME),m_textInputAnvil.getLabel(),(DWORD)m_iPad,30,&UIScene_AnvilMenu::KeyboardCompleteCallback,this,C_4JInput::EKeyboardMode_Defau#endif
}
...
void UIScene_AnvilMenu::handleDestroy#ifdef __PSVITA__
	app.DebugPr"missing InputManager.DestroyKeyboard on Vita !!!!!!\n"#endif// another player destroyed the anvil, so shut down the keyboard if it is displayed#if ( defined __PS3__ || defined __ORBIS__ || defined _DURANGO)
	InputManager.DestroyKeyboar#endif	
