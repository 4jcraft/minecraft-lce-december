...
void UIControl_EnchantmentButton::render(IggyCustomDrawCallbackRegion* region) {
    ...
    if (cost != 0) {
        ...
        Font* font = pMinecraft->altFon  // int col = 0x685E4A;
        ...
        if (pMinecraft->localplayers[enchantingScene->getPad()]
            ...
            font = pMinecraft->font  // col = (0x80ff20 & 0xfefefe) >>
                                     // 1;//font->drawShadow(line, (bwidth -
                                     // font->width(line))/ss, 7, col);
        } else {
            if (m_bHasFocus) {  // col = 0xffff80;
...
if (state != m_lastState) {
    ...
    IggyResult out =
        IggyPlayerCallMethodRS(m_parentScene->getMovie(), &result,
                               getIggyValuePath(), m_funcChangeState, 1, value);
