...
protected:
    ...
    virtual void updateComponents() {}
        virtual void handleGainFocus(bo// Returns true if lower scenes in this scenes layer, or in any layer below this scenes layers should be hidden
	virtual bool// Returns true if this scene should block input to lower scenes (works like hidesLowerScenes but doesn't interfere with rendering)// returns main panel if controls are not living in the root
	virtual UIControl* GetMainPanel();

	void removeControl( UIControl_Base *control, bool centr// RENDERINGid slideLeft();
	void slideRight();
	...
	boo//void customDrawSlotControl(IggyCustomDrawCallbackRegion *region, int iPad, int iID, int iCount, int iAuxVal, float fAlpha, bool isFoil, bool bDecorations);
...
private:
	...
	void _customDrawSlotControl(CustomDrawData *region, int iPad, std::shared_ptr<ItemInstance> item, float fAlpha// INPUTsFoil, bool bDecorations, bool usingCommandBuffer);

public:
	
	bool canHandleInput() {
        return m_bCanHandleInput; }
...
public:
	...
	bool controlHasFocus(UIControl_B// NAVIGATION;
	int getCon//void navigateForward(int iPad, EUIScene scene, void *initData = NULL);
	void navigateBack();

public:
	void setBackScene(UIScene *scene);
	UIScene *getBackScene();
	virtual void#ifdef _XBOX_ONEngComplete() {}
	virtual void HandleDLCInst#endif) {}
	...
	virtual void handleTouchInput(unsigned int iPad, S32
#ifdef _DURANGO, bool bPressed, bool bRepeat, bool bReleased){}


protected:
#endif
	virtual long long getDefaultGtcButtons() {
        return _360_GTC_BACK; }


	std::size_t GetCallbackUniqueId();
