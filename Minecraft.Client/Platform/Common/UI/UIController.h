...
private:
	...
	std::unordered_map<std::size_t, UIScene *> m_registeredCallbackScenes; // A collection of scenes and unique id's that are used in async callbacks so we can safely handle when they get destroyed
...
public:
	...
	std::size_t RegisterForCallbackId(UIScene *scene);
	void UnregisterCallbackId(std::size_t id);
	UIScene *GetSceneFromCallbackId(std::size_t id);
