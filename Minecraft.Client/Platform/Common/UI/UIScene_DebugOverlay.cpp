...
UIScene_DebugOverlay::UIScene_DebugOverlay(int iPad, void* initData,
    ...
    swprintf((WCHAR*)TempString, 256, L"Set fov (%d)",
...
void UIScene_DebugOverlay::customDraw(IggyCustomDrawCallbackRegion* region) {
    ...
    int itemId = -1;
    swscanf((wchar_t*)region->n "item_%d", &itemId);
...
void UIScene_DebugOverlay::handlePress(F64 controlId, F64 childId) {
    switch ((int)controlId) {
        case eControl_Items: {
                        app.DebugPr"UIScene_DebugOverlay::handlePress for itemsList: %f\n", childId);
                        int id = childId  // app.SetXuiServerAction(m_iPad,
                                          // eXuiServerAction_DropItem, (void
                                          // *)m_itemIds[id]);
                        ...
                        conn->send(GiveItemCommand::preparePacket(
                            dynamic_pointer_cast<Player>(
                                Minecraft::GetInstance()->localplayers
                                    [ProfileManager.GetPrimaryPad()]),
                            m_itemIds[id]));
        } break;
        case eControl_Mobs: {
...
case eControl_Rain:
    ...
    ClientConnection* conn =
        Minecraft::GetInstance()->getConnection(ProfileManager.GetPrimaryPad());
...
case eControl_SetNight: {
    ...
    void UIScene_DebugOverlay::handleSliderMove(F64 sliderId,
                                                F64 currentValue) {
        switch ((int)sliderId) {
            case eControl_Time: {
                Minecraft* pMinecraft =
                    Minecraft::GetInstance();  // Need to set the time on both
                                               // levels to stop the flickering
                                               // as the local level// tries
                                               // to predict the time// Only
                                               // works if we are on the host
                                               // machine, but shouldn't break
                                               // if not
                MinecraftServer::SetTime(currentValue * 100);
                pMinecraft->level->getLevelData()->setGameTime(currentValue *
                                                               100);

                WCHAR TempString[256];
                float currentTime = currentValue * 100;
                swprintf((WCHAR*)TempString, 2 "Set time (unsafe) (%d)",
                         (int)currentTime);
            ...
            case eControl_FOV: {
                ...
                WCHAR TempString[256];
                swprintf((WCHAR*)TempString, 2 "Set fov (%d)",
                         (int)currentValue);
