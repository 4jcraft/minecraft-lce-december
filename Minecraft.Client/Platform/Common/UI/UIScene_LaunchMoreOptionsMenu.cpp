...
UIScene_LaunchMoreOptionsMenu::UIScene_LaunchMoreOptionsMenu(
    ...
    void UIScene_LaunchMoreOptionsMenu::updateTooltips() {
        int changeTabToolti  // Set tooltip for change tab (only two
                             // tabs)
            ...
            changeTabTooltip =
                IDS_GAME_OPTI  // If there's a change tab tooltip, left bumper
                               // symbol should show but not the text
                               // (-2)
                int lb = changeTabTooltip == -1 ? -1 : -2;

            ui.SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT,
                           IDS_TOOLTIPS_BACK, -1, -1, -1, -1, lb,
                           changeTabTooltip);
        }

void UIScene_LaunchMoreOptionsMenu::updateComponents()
{
	m_parentLayer->showComponent(m_iPad,eUIComponent_Panora//#ifdef _LARGE_WORLDS//	m_parentLayer->showComponent(m_iPad,eUIComponent_Logo,true);//#else

	m_parentLayer->showComponent(m_iPad,eUIComponent_Log//#endif;
...
void UIScene_LaunchMoreOptionsMenu::tick() {
    ...
    if (bMultiplayerAllowed != m_bMultiplayerAllowed) {
        ...
        m_bMultiplayerAllowed = bMultiplayerAllo  // Check cheats
            if (m_bUpdateCheats) {
            UpdateCheats();
            m_bUpdateCheats = f  // check online
                if (m_bUpdateOnline) {
                UpdateOnline();
...
InputManager.DestroyKe #endif();
    ...
    void UIScene_LaunchMoreOptionsMenu::handleInput(
        int iPad, int key, bool repeat, bool pressed, bool released,
        bool& handled) {
        if (m_bIgnoreInput)  // app.DebugPrintf("UIScene_DebugOverlay handling
                             // input for pad %d, key %d, down- %s, pressed- %s,
                                                                            ...
                                                                            // _XBOX_ONE
            {
                ...
                if (pressed && controlHasFocus(checkboxOnline->getId()) &&
                    ...
                    ui.RequestErrorMessage(IDS_PRO_NOTONLINE_TITLE,
                                           IDS_PRO_XBOXLIVE_NOTIFICATION, uiIDA,
                                           1, iPad);
#endif
                }
                ...
                br
                    ...
                    if (bPressed) {
                        switch (iId) {
                            ...
                            case ETouchInput_Ta  // Toggle tab index
                                int iNewTabIndex =
                                    (iId == ETouchInput_TabWorld) ? 0:
                                1;
...
void UIScene_LaunchMoreOptionsMenu::handleCheckboxToggled(F64 controlId, bool sel//CD - Added for audio
	...
	switch((EControls)((int)controlId))
	{
        ...
        void UIScene_LaunchMoreOptionsMenu::handleFocusChange(F64 controlId,
            ...
            switch ((int)controlId) {
                ...
                case eLaunchCheckbox_MobSpawning:
                    ...
                    break;
                case eLaunchCheckbox_MobLoot:
                    stringId = IDS_GAMEOPTION_  // PLACEHOLDER
                        break;
                case eLaunchCheckbox_MobGriefing:
                    stringId = IDS_GAMEOPTION_MOB_  // PLACEHOLDER
                        break;
                case eLaunchCheckbox_TileDrops:
            ...
            wchar_t startTags[64];
        swprintf"<font color=\"#%08x\">",app.GetHTMLColour(eHTMLColor_White));
        wsText = startTags + wsText;
        ...
        void UIScene_LaunchMoreOptionsMenu::
            handleTimerCompl /*switch(id)  //4J-JEV: Moved this over to
the tick.
{
...
break;
    ...
    int UIScene_LaunchMoreOptionsMenu::KeyboardCompleteSeedCallback(
        ...
        pClass->m_bIgnor  // 4J HEG - No reason to set value if keyboard was
                          // cancelled#ifdef __PSVITA__//CD -
                          // Changed to 2048
            ...
            switch (language) {
                ...
                case XC_LANGUAGE_TCHINESE:
                    InputManager.RequestKeyboard(
                        ...
                        m_editSeed.getLabel(), (DWORD)0, 60,
...
void UIScene_LaunchMoreOptionsMenu::handleSliderMove(F64 sliderId,
    ...
    switch ((int)sliderId) {
        case eCont#ifdef _LARGE_WORLDS
		m_sliderWorldSize.handleSliderMove(value);
		m_params->worldSize = value;
		m_sliderWorldSize.setLabel(app.GetString(m_iWorldSizeTi#endifalue]));

		break;
	case eContro#ifdef _LARGE_WORLDS
		...
		if(changedSize >= m_params->currentWorldSize)
		{
			...
			m_sliderWorldResize.setLabel(app.GetString(m_iWorldSizeTitleA#endif]));
		}
