...
void UIScene_LeaderboardsMenu::CopyLeaderboardEntry(
	...
	for( unsigned int i=0 ; i<statsRow->m_statsSize ; i++ )
	{
        ...
        unsigned int uiVal =
            ...
            uiVal /= 10;
            iDigitC++;
#ifdef _DEBUG  // app.DebugPrintf("Value -
               // %d\n",leaderboardEntry->m_columns[i]);#endif
            if (iDigitC < 4) {  // m
                ...
                void UIScene_LeaderboardsMenu::PopulateLeaderboard(
                    ...
                    if (ret == LeaderboardManager::eStatsReturn_Success &&
                        ...
                        m_listEntries.setupTitles(
                            ...
                            app.GetString(IDS_LEADERBOARD_GAMERTAG))  // Update
                                                                      // entries
                                                                      // display
                            wchar_t entriesBuffer[40];
                        ...
                        swprintf(entriesBuffer, "%ls0",
                                 app.GetString(IDS_LEADERBOARD_ENTRIES));
                        m_labelEntries
                            .setLabel(entriesBuffer)  // Show the no results
                                                      // message#if
                                                      // !(defined(_XBOX)
                                                      // || defined(_WINDOWS64))
                                                      // // 4J Stu - Temp to get
                                                      // the win build running,
                                                      // but so we check this
                                                      // for other platforms
                            if (ret ==
