...
bool UIScene_Keyboard::allowRepeat(int key// 4J - TomK - we want to allow X and Y repeats!
	switch(key)
	{
    case ACTION_MENU_OK:
    ...
    case ACTION_MENU_A:
    case ACTION_MENU_B:
        case ACTION_MENU_PAUSEME//case ACTION_MENU_X://case ACTION_MENU_Y:
		return false;
	}
	return true;
}
...
void UIScene_Keyboard::handlePress(F64 controlId, F64 childId) {
    if ((int)controlId == 0)
        ...
        // string and pass it on to wherever it is needed.// we can not query
        // for m_KeyboardTextInput.getLabel() here because we're in an iggy
        // callback so we need to wait a frame.
        if (!m_bKeyboardDonePressed)
            // kick off done timer
            addTimer(KEYBOARD_DONE_TIMER_ID, KEYBOARD_DONE_TIMER_TIME);
    m_bKeyboardDonePressed = true;
...
void UIScene_Keyboard::handleTimerComplete(int id) {
    if (id == KEYBOARD_DONE_TIMER_ID)
        // remove timer
        killTimer(KEYBOARD_DONE_TIMER_ID)  // we're done here!
            KeyboardDonePressed();
}
}

void UIScene_Keyboard::KeyboardDonePressed(// Debug
	app.DebugPr"UI Keyboard - DONE - [%ls]\n", m_KeyboardTextInput.getLabel()// ToDo: Keyboard can now pass on its final string value and close itself down
	navigateBack();
}