...
void UIScene_CreativeMenu::handleTouchInput(unsigned int iPad, S32 x, S32 y, int iId, bool bPressed, bool bRepeat, bool bRel// perform action on release
	if(bReleased)
	{
	...
	if(bRepeat && iId == ETouchInput_TouchSlider && specs[m_curTab]->getPageCount() // calculate relative touch position on slider
		float fPosition = ((float)y - (float)m_TouchInput[ETouchInput_TouchSlider].getYPos() - m_controlMainPanel.getYPos()) /  (float)m_TouchInput[ETouchInput_TouchSlider].getHeigh// clamp		
		if(fPosition > 1)
			fPosition = 1.0f;
		else if(fPosition < 0)
			fPosition =// calculate page position according to page count
		int iCurrentPage = Math::round(fPosition * (specs[m_curTab]->getPageCount() - // set tab page
		m_tabPage[m_curTab] = iCurren// update tab
		switchTab(m_curTab);
    }
}
...
void UIScene_CreativeMenu::handleTimerComplete(int id) {
        if(id == GAME_CREATIVE_TOUCHUPDATE_TIMER// we cannot rebuild touch boxes in an iggy callback because it requires further iggy calls
		GetMainPanel()->UpdateControl();
		ui.TouchBoxRebuild(this);
		killTimer(GAME_CREATIVE_TOUCHUPDATE_TIMER_#endif
}
}
...
void UIScene_CreativeMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &ha// 4J-PB - going to ignore repeats on this scene
	if(repeat) //app.DebugPrintf("UIScene_CreativeMenu handling input for pad %d, key %d, down- %s, pressed- %s, released- %s\n", iPad, key, down?"TRUE":"FALSE", pressed?"TRUE":"FALSE", released?"TRUE":"FALSE");
    ...
    case VK_PAD_LT  // change the potion strength
    { ++m_tabDynamicPos[m_curTab];
      if (m_tabDynamicPos[m_curTab] >= specs[m_curTab]->m_dynamicGroupsCount)
          m_tabDynamicPos[m_curTab] = 0;
      switchTab(m_curTab); } break;
        default:
        UIScene_AbstractContainerMenu::handleInput(iPad, key, repeat, pressed,
                                                   released, handled);
        break;
	}
}
