...
IggyLibrary UIController::loadSkin(const std::wstring& skinPath,
    ...
    if (!skinPath.empty() && app.hasArchiveFile(skinPath)) {
        ...
        while (res = IggyDebugGetMemoryUseInfo(NULL, lib, "", 0, iteration,
                                               &memoryInfo)) {
...
// texture handle provided by GDraw, or NULL (NULL with throw an ActionScript 3
...
C4JStorage::EMessageResult UIController::RequestMessageBox(
    UINT uiTitle, UINT uiText, UINT* uiOptionA, UINT uiOptionC, DWORD dwPad,
    int (*Func)(LPVOID, int, const C4JStorage::EMessageResult), LPVOID lpParam,
    WCHAR* pwchFormatString, DWORD dwFocusButton, bool bIsError)
...
C4JStorage::EMessageResult UIController::RequestUGCMessageBox(
    UINT title /* = -1 */, UINT message /* = -1 */, int iPad /* = -1*/,
