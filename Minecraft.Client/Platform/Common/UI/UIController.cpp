...
static void* RADLINK AllocateFunction(void* alloc_callback_user_data,
    ...
    UIController* controller = (UIController*)alloc_callback_user_data;
        EnterCriticalSection(&controller->m_Allocatorlo#ifdef EXCLUDE_IGGY_ALLOCATIONS_FROM_HEAP_INSPECTOR
	void *alloc = __real_malloc(size_request#else
	void *alloc = malloc(size_request#endif
	*size_returned = size_requested;
	UIController::iggyAllocCount += size_requested;
	allocations[alloc] = size_requested;
	app.DebugPrintf(app.USER"Allocating %d, new total: %d\n", size_requested, UIController::iggyAllocCount);
...
static void RADLINK DeallocateFunction(void* alloc_callback_user_data,
    ...
    allocations.erase(ptr);
        app.DebugPrintf(app.USER"Freeing %d, new total %d\n", size, UIController::iggyAllocCou#ifdef EXCLUDE_IGGY_ALLOCATIONS_FROM_HEAP_INSPECTOR
	__real_free(p#else
	free(p#endif
	LeaveCriticalSection(&controller->m_Allocatorlock);
}
...
UIController::UIController() {
    ...
    m_mcTTFFont = NULL;
    m_moj7 = NULL;
    m_moj11 = NUL  // 4J-JEV: It's important that these remain the same, unless
                   // updateCurrentLanguage is going to be called.
        m_eCurrentFont = m_eTargetFont = eFont_NotLoa
#ifdef ENABLE_IGGY_ALLOCATOR
	InitializeCriticalSection( \
    &m_Allocatorlo #endif     // 4J Stu - This is a bit of a hack until we change the Minecraft initialisation to store the proper screen size for other platforms#if defined _WINDOWS64 || defined _DURANGO || defined __ORBIS__
            m_fScreenWidth = 1920.0f;
    m_fScreenHeight = 1080.0f;
    m_bScreenWidthSetup = t #else 
	m_fScreenWidth = 1280.0f;
    m_fScreenHeight = 720.0f;
    m_bScreenWidthSetup = fa #endif 
    ...
    InitializeCriticalSection(&m_navigationLock);
        InitializeCriticalSection(&m_registeredCallbackScenesC//m_bSysUIShowing=false;
	m_bSystemUIShowing=fa#ifdef __PSVITA__
	m_bTouchscreenPressed=fa#endif
...
void UIController::SetSysUIShowing(bool bVal) {
    if (bVal) app.DebugPr"System UI showing\n");
    else app.DebugPr"System UI stopped showing\n");
...
void UIController::SetSystemUIShowing(LPVOID lpParam, bool bVal) {
    ...
    pClass->SetSysUIShowing(bVal)  // SETUP
        void UIController::preInit(S32 width, S32 height) {
        m_fScreenWidth = width;
        m_fScreenHeight = height;
        m_bScreenWidthSetup = t
#ifdef ENABLE_IGGY_ALLOCATOR
    ...
    for (unsigned int i = 0; i < eUIGroup_COUNT; ++i) {
                m_groups[i] = new UIGroup((EUIGr

#ifdef ENABLE_IGGY_EXPLORER
	iggy_explorer =                                             \
    "127.0.0.1" ate(, 9190, malloc(IGGYEXP_MIN_STORAGE), \
                    IGGYEXP_MIN_STORAGE);
	if ( iggy_explorer // not normally an error, just an error for this demo!
		ap"Couldn't connect to Iggy Explorer, did you run it first?" );
    }
    else {
                IggyUseExplorer( m_groups[1]->getHUD()->getMovie(), iggy#endifr
#ifdef ENABLE_IGGY_PERFMON
	m_iggyPerfmonEnabled = false;
	iggy_perfmon = IggyPerfmonCreate(perf_malloc, perf_free, NULL);
	IggyInstallPerfmon(#endiferfmon);
    ...
    UIController::EFont UIController::getFontForLanguage(int language) {
        switch (language) {
            ...
            return e #ifdef _DURANGO case XC_LANGUAGE_SCHINESE:
                return eFon #endifChinese;
    ...
    UITTFFont* UIController::createFont(EFont fontLanguage) {
        switch(fo#if defined(__PS3__) || defined(__ORBIS__) || defined(__PSVITA__)
	case eFont_Japanese:		return"Mojangles_TTF_jaJP""Common/Media/font/JPN/DF-DotDotGothic16.ttf"// JPN0x// case eFont_SimpChinese:	Simplified Chinese is unsupported.
	case eFont_TradChinese:		return"Mojangles_TTF_cnTD""Common/Media/font/CHT/DFTT_R5.TTC"// CHT0x203B); 
	case eFont_Korean:			return"Mojangles_TTF_koKR""Common/Media/font/KOR/candadite2.ttf"// KOR0#else); 

	case eFont_Japanese:		return"Mojangles_TTF_jaJP""Common/Media/font/JPN/DFGMaruGothic-Md.ttf"// JPN0#ifdef _DURANGO
	case eFont_SimpChinese:		return"Mojangled_TTF_cnCN""Common/Media/font/CHS/MSYH.ttf"// CHS0#endif; 

	case eFont_TradChinese:		return"Mojangles_TTF_cnTD""Common/Media/font/CHT/DFHeiMedium-B5.ttf"// CHT0x2022); 
	case eFont_Korean:			return"Mojangles_TTF_koKR""Common/Media/font/KOR/BOKMSD.ttf"// KOR0#endif; // 4J-JEV, Cyrillic characters have been added to this font now, (4/July/14)// XC_LANGUAGE_RUSSIAN and XC_LANGUAGE_GREEK:
	default:					return NULL;
    }
}

void UIController::  // 4J-JEV: Language hasn't changed or is already
                     // changing.
    if ((m_eCurrentFont != m_eTargetFont) ||
        !UIString::setCurrentLanguage()) return;

DWORD nextLanguage = UIString::getCurrentLanguage();
        m_eTargetFont = getFontForLanguage(ne// flag a language change to reload the string tables in the DLC
	app.m_dlcManager.LanguageChanged();

	app.load// Switch to use new string table,
	
	if (m_eTargetFont == m_eCur// 4J-JEV: If we're ingame, reload the font to update all the text.
		if (app.GetGameStarted()) app.SetAction(ProfileManager.GetPrimaryPad(), eAppAction_ReloadFont);
		return;
        }

        if (m_eCurrentFont != eFont_NotLoaded)	a"[UIController] Font switch required for language transition to %i.\n", nextLanguage);
        else									a"[UIController] Initialising font for language %i.\n", nextLanguage);
        ...
        if(m_eTargetFont == eFon// these may have been set up by a previous language being chosen
		if (m_moj7 == NULL)		m_moj7  = new UIBitmapFont(SFontData::Mojangles_7);
		if (m_moj11 == NULL)	m_moj11 = new UIBitmapFont(SFontData::Moj// 4J-JEV: Ensure we redirect to them correctly, even if the objects were previously initialised.
		m_moj7->registerFont();
		m_moj11->registerFont();
            ...
            m_mcTTFFont = createFont(m_eTargetFont);
                ...
                a"[Iggy] Set font indirect to '%hs'.\n", m_mcTTFFont->getFontName().c_str());
                IggyFontSe "Mojangles7" 8(, -1, IGGY_FONTFLAG_all,
                                          m_mcTTFFont->getFontName().c_str(),
                                          -1, IGGY_FONTFLAG_none);
                IggyFontSe "Mojangles11"(, -1, IGGY_FONTFLAG_all,
                                         m_mcTTFFont->getFontName().c_str(), -1,
                                         IGGY_FONTFLAG_none);
        }
        else {
            asser  // Reload ui to set new font.
        ...
        bool UIController::UsingBitmapFont() {
            return m_eCurrentFont == eF  // TICKING;
        }

        void UIController::tick() {  // If necessary, change font.

            if ((m_navigateToHomeOnReload || m_bCleanupOnReload) &&
                !ui.IsReloadingSkin()) {
                ui.CleanUpSkinReload();

                if (m_navigateToHomeOnReload ||
            ...
            for (unsigned int i = 0; i < eUIGroup_COUNT; ++i) {
                m_groups[// TODO: May wish to skip ticking other groups here// Clear out the cached movie file data
	...
	for(AUTO_VAR(it, m_cachedMovieData.begin()); it != m_cachedMovieData.end();)
	{
            ...
            void UIController::loadSkins() {
                std::wstring "" at
                             ...
                             "skinPS3.swf" at #elif defined __PSVITA__
                             "skinVita.swf" th #elif defined
                                 _WINDOWS64 if (m_fScreenHeight == 1080.0f) {
                    "skinHDWin.swf"h = L;
                }
                else {
                    "skinWin.swf" ath =
                        #elif defined _DURANGO if (m_fScreenHeight == 1080.0f) {
                        "skinHDDurango.swf" L;
                    }
                    else {
                        "skinDurango.swf" =
                            L#elif defined __ORBIS__ if (m_fScreenHeight ==
                                                         1080.0f) {
                            "skinHDOrbis.swf" = L;
                        }
                        else {
                            "skinOrbis.swf"h = L
#endif  // Every platform has one of these, so nothing shared
                                if (m_fScreenHeight == 1080.0f) {
                m_iggyLibraries[eLibrary_Platform] = loadSkin"platformskinHD.swf");
                            }
                            else {
                m_iggyLibraries[eLibrary_Platform] = loadSkin"platformskin.swf" L
#if defined(__PS3__) || defined(__PSVITA__)
	m_iggyLibraries[eLibrary_GraphicsD"skinGraphics.swf"(L"skinGraphics.swf" L);
	m_iggyLibraries[eLibrary_Graph"skinGraphicsHud.swf""skinGraphicsHud.swf");
	m_iggyLibraries[eLibrary_Graphics"skinGraphicsInGame.swf""skinGraphicsInGame.swf");
	m_iggyLibraries[eLibrary_GraphicsTo"skinGraphicsTooltips.swf""skinGraphicsTooltips.swf");
	m_iggyLibraries[eLibrary_Graphics"skinGraphicsLabels.swf""skinGraphicsLabels.swf");
	m_iggyLibraries[eLibrary_"skinLabels.swf"in("skinLabels.swf", L);
	m_iggyLibraries[eLibrary_"skinInGame.swf"in("skinInGame.swf", L);
	m_iggyLibraries[eLibra"skinHud.swf"dSk"skinHud.swf", L);
	m_iggyLibraries[eLibrary_To"skinTooltips.swf"(L"skinTooltips.swf" L);
	m_iggyLibraries[eLibrary_D"skin.swf"loa"skin.swf"#endifL
#if (defined(_WINDOWS64) || defined(_DURANGO) || defined(__ORBIS__))
#if defined(_WINDOWS64)  // 4J Stu - Load the 720/480 skins so that we
                                 // have something to fallback on during
                                 // development#ifndef _FINAL_BUILD
	m_iggyLibraries[eLibraryFallback_GraphicsD"skinGraphics.swf"(L"skinGraphics.swf" L);
	m_iggyLibraries[eLibraryFallback_Graph"skinGraphicsHud.swf""skinGraphicsHud.swf");
	m_iggyLibraries[eLibraryFallback_Graphics"skinGraphicsInGame.swf""skinGraphicsInGame.swf");
	m_iggyLibraries[eLibraryFallback_GraphicsTo"skinGraphicsTooltips.swf""skinGraphicsTooltips.swf");
	m_iggyLibraries[eLibraryFallback_Graphics"skinGraphicsLabels.swf""skinGraphicsLabels.swf");
	m_iggyLibraries[eLibraryFallback_"skinLabels.swf"in("skinLabels.swf", L);
	m_iggyLibraries[eLibraryFallback_"skinInGame.swf"in("skinInGame.swf", L);
	m_iggyLibraries[eLibraryFallba"skinHud.swf"dSk"skinHud.swf", L);
	m_iggyLibraries[eLibraryFallback_To"skinTooltips.swf"(L"skinTooltips.swf" L);
	m_iggyLibraries[eLibraryFallback_D"skin.swf"loa"skin.swf"#endifL#endif);



	m_iggyLibraries[eLibrary_GraphicsD"skinHDGraphics.swf""skinHDGraphics.swf");
	m_iggyLibraries[eLibrary_Graph"skinHDGraphicsHud.swf""skinHDGraphicsHud.swf");
	m_iggyLibraries[eLibrary_Graphics"skinHDGraphicsInGame.swf""skinHDGraphicsInGame.swf");
	m_iggyLibraries[eLibrary_GraphicsTo"skinHDGraphicsTooltips.swf""skinHDGraphicsTooltips.swf");
	m_iggyLibraries[eLibrary_Graphics"skinHDGraphicsLabels.swf""skinHDGraphicsLabels.swf");
	m_iggyLibraries[eLibrary_"skinHDLabels.swf"(L"skinHDLabels.swf" L);
	m_iggyLibraries[eLibrary_"skinHDInGame.swf"(L"skinHDInGame.swf" L);
	m_iggyLibraries[eLibra"skinHDHud.swf"kin"skinHDHud.swf", L);
	m_iggyLibraries[eLibrary_To"skinHDTooltips.swf""skinHDTooltips.swf");
	m_iggyLibraries[eLibrary_D"skinHD.swf"adS"skinHD.swf"#endif // HD platforms
                            }

                            IggyLibrary UIController::loadSkin(
                                const std::wstring& skinPath,
                                const std::wstring& skinName) {
                                IggyLibrary lib =
                                    IGG  // 4J Stu - We need to load the
                                         // platformskin before the normal skin,
                                         // as the normal skin requires some
                                         // elements from the platform
                                         // skin
                                    if (!skinPath.empty() &&
                                        app.hasArchiveFile(skinPath)) {
                                    byteArray baFile =
                                        app.getArchiveFile(skinPath);
                                    lib = IggyLibraryCreateFromMemoryUTF16(
                                        (IggyUTF16*)skinName.c_str(),
                                        (void*)baFile.data, baFile.length,
                                        NULL);

                                    de #ifdef _DEBUG.data;
                                    ...
                                    while (res = IggyDebugGetMemoryUseInfo(
                                               ...
                                               &memoryInfo)) {
                                        totalStatic +=
                                            memoryInfo.static_allocation_bytes;
                        app.Debug"%ls - %.*s, static: %dB, dynamic: %dB\n", skinPath.c_str(), memoryInfo.subcategory_stringlen, memoryInfo.subcategory, memoryInfo.static_allocation_bytes, memoryInfo.dynamic_allocation_bytes);
                        ++iteration;
                                    }

                app.Debug"%ls - Total static: %dB (%dKB)\n", skinPath.c_str(), totalStatic,#endifStatic/1024);
                                }
                                return lib;
                            }

void UIControll// Destroy all scene swf
	for(unsigned int i = 0; i < eUIGro//m_bCloseAllScenes[i] = true;
		m_groups[i]-// Unload the current libraries// Some libraries reference others, so we destroy in reverse order
	for(int i = eLibrary_Count - 1; i >= 0; --i)
	{
                        if (m_iggyLibraries[i] != IGGY_INVALID_LIBRARY)
                            IggyLibraryDestroy(m_iggyLibraries[i]);
                        m_iggyLibraries[i] = IGGY_
#ifdef _WINDOWS64 }
                            // 4J Stu - Don't load on a thread on windows. I
                            // haven't investigated this in detail, so a quick
                            // fix
                            reloadSk #elseeadProc(this);

                        m_reloadSkinThread =
                            new C4JThread(reloadSkinThrea "Reload skin thread");
        m_reloadSkinThread->SetProcessor(CP// Navigate to the timer scene so that we can display something while the loading is happening
	ui.NavigateToScene(0,eUIScene_Timer,(void *)1,eUILayer_Tooltips,eUI//m_reloadSkinThread->Run();//// Load new skin
        // loadSkins();//// Reload all scene swf//for(int i = eUIGroup_Player1;
        // i <= eUIGroup_Player4; ++i)//{//	m_groups[i]->ReloadAll();//}////
        // Always reload the fullscreen
        // group//m_groups[eUIGroup_Fullscreen]->ReloadAll();#endif
...
{
        EnterCriticalSection(// MGH - added to prevent crash loading Iggy movies while the skins were being reloaded
	UIController *controller = (UICon// Load new skinm;
	
	control// Reload all scene swf
	for(int i = eUIGroup_Player1; i < eUIGroup_COUNT; ++i)
	{
                controller->m_groups[i]// Always reload the fullscreen group
	controller->m_groups[eUIGroup_Fullscre// 4J Stu - Don't do this on windows, as we never navigated forwards to start with#ifndef _WINDOW64

	controller->NavigateBack(0, false, eUIScene_COUNT, #endifer_Tooltips);
...
{
                            return m_reloadSkinThread &&
                                   (!m_reloadSkinThread->hasStarted() ||
...
void UIController::CleanUpSkinReload()
{
                ...
                if(!Minecraft::GetInstance()->skins->getSelected#ifdef _DURANGO			{
                                    DWORD result =
                                        StorageManager.U"TPACK" nst #elseDLC(L);

                                    DWORD result =
                                        StorageManager."TPACK" Ins #endifDLC();
                            ...
                            for (AUTO_VAR(it, m_queuedMessageBoxData.begin());
                                ...
                                ui.NavigateToScene(
                                    queuedData->iPad, eUIScene_MessageBox,
...
byteArray UIController::getMovieData(const std::ws// Cache everything we load in the current tick
...
void UIControl// If system/commerce UI up, don't handle input//if(!m_bSysUIShowing && !m_bSystemUIShowing)
	if(!m_b#ifdef ENABLE_IGGY_PERFMON
		if (m_iggyPerfmonEnabled)
		{
                        if (InputManager.ButtonPressed(
                                ProfileManager.GetPrimaryPad(),
                                ACTION_MENU_STICK_PRESS))
                            m_iggyPerfmonEnabled = !m_iggyPerfmon #endifd;
		}
		else

		{
                    ...
                    void UIControlle  // For each user, loop over each key type
                                      // and send messages based on the
                                      // state
                        for (unsigned int iPad = 0;
                             iPad < XUSER_MA #ifdef _DURANGO)
                        // 4J-JEV: Added exception for primary play who migh've
                        // uttered speech commands.
                        if (iPad != ProfileManager.GetPrimaryPad() &&
                            (!InputManager.IsPadConnected(iPad) ||
                             !InputManager.IsPadLocked #endif)) continue;

                    for (unsigned int key = 0; key <= ACTION_MAX_MENU; ++key) {
                        handleKeyPr
#ifdef __PSVITA__ }
//CD - Vita requires key press 40 - select [MINECRAFT_ACTION_GAME_INFO]
		handleKeyPress(iPad, MINECRAFT#endifN_GA
#ifdef _DURANGO
                    }

                    if (!app.GetGameStarted()) {
                        bool repeat = false;
                        int firstUnfocussedUn  // For durango, check for
                                               // unmapped
                                               // controllers
                            for (unsigned int iPad = XUSER_MAX_COUNT;
                                 iPad <
                                 (XUSER_MAX_COUNT + InputManager.MAX_GAMEPADS);
                                 ++iPad) {
                            if (InputManager.IsPadLocked(iPad) ||
                                !InputManager.IsPadConnected(iPad))
                                continue;

                            for (unsigned int key = 0; key <= ACTION_MAX_MENU;
                                 ++key) {
                                bool pressed =
                                    InputManager.Butto  // ToggleiPad,key);
                                                        //
                                    bool released =
                                        InputManager.Button  // ToggleiPad,key);
                                                             //

                                        if (pressed || released) {
                                    bool han  // Send the key to the fullscreen
                                              // group first
                                        m_groups[(int)eUIGroup_Fullscreen]
                                            ->handleInput(iPad, key, repeat,
                                                          pressed, released,
                                                          handled);
                        ...
                        if (ProfileManager.GetLockedProfile() >= 0 &&
                            ...
                            ProfileManager.RequestSignInUI(
                                ...
                                firstUnfocussedUnh #endifPad);
                        }
                    }
                }

                void UIController::handleKeyPress(unsigned int iPad,
                                                  unsigned int key) {
                    bool down = false;
                    bo       // Toggled = false;
                        boo  // Toggled = false;
                        ...
                        bool bTouchSc  // check the touchscreen// 4J-PB -
                                       // use the touchscreen for
                                       // quickselect
                            SceTouchData* pTouchData =
                                InputManager.GetTouchPadData(iPad, false);

                if((m_bTouchscreenPressed==false) && pTouchData->r// no active touch? clear active and highlighted touch UI elements
			m_ActiveUIElement = NULL;
			m_HighlightedUIE// fullscreen first	
			UIScene *pScene=m_groups[(int)eUIGroup_Fullscreen]->ge// also check tooltip scene if we're not touching anything in the main scene
			...
			// scene touch check
				if(TouchBoxHit(pScene,pTouchData->report[0].x,pTouchData->report[0].y))
				{
                    ...
                    bTouchScreenIn  // tooltip touch check
			...
			else
			{
                    ...
                    if  // scene touch check
                        if (TouchBoxHit(pScene, pTouchData->report[0].x,
                                        pTouchData->report[0].y)) {
                            down = pressed = m_bTouchscreenPressed = true;
                            bTouchScreenInpu  // tooltip touch check (if scene
                                              // exists but not component has
                                              // been
                                              // touched)
                                else if (TouchBoxHit(pToolTips,
                                                     pTouchData->report[0].x,
                                                     pTouchData->report[0].y)) {
                                down = pressed = m_bTouchscreenPressed = true;
                                bTouchScreenInput = true;
                            }
                                }
				else if(pT// tooltip touch check (if scene does not exist)
                ...
                else if(m_bTouchscreenPressed && pTouchData->r// fullscreen first	
			UIScene *pScene=m_groups[(int)eUIGroup_Fullscreen]->ge// also check tooltip scene if we're not touching anything in the main scene
			UIScene *pToolTips=m_groups[(int)eUIGroup_Fullscreen]->getTooltips();
			// scene touch check
				if(TouchBoxHit(pScene,pTouchData->report[0].x,pTouchData->report[0].y))
				{
            down = true;
            bTouchScreenIn  // tooltip touch check (if scene exists but not
            ...
            if  // scene touch check
                if (TouchBoxHit(pScene, pTouchData->report[0].x,
                                pTouchData->report[0].y)) {
                    down = true;
                    bTouchScreenInpu  // tooltip touch check (if scene exists
                                      // but not component has been
                                      // touched)
                        else if (TouchBoxHit(pToolTips, pTouchData->report[0].x,
                                             pTouchData->report[0].y)) {
                        down = true;
                ...
                else if(m_bTouchscreenPressed && pTouchData->r// released)
		{
    bTouchScreenInput = true;
	...
	// Start repeat timer
			m_actionRepeatTimer[iPad][key] = GetTickCount() + UI_REPEAT_KEY_DELAY_MS;
                ...
                else i  // Stop repeat timer
                    m_actionRepeatTimer[iPad][key] = 0;
                }
                el  // Check is enough time has elapsed to be a repeat
                    // key
                ...
                if (m_actionRepeatTimer[iPad][key] > 0 &&
                    ...
                    repeat = true;
                    pressed = true;
                                m_actionRepeatTimer[iPad][key] = currentTime + UI_REPEAT_KEY_REPEAT_R// handle touch input
		HandleTouchInput(iPad, key, pressed, re// ignore any other presses if the touchscreen has been used
		if(bTouchScre#endift) return;
                }

                down = InputManager.ButtonDown(iPad, key);
                pressed = InputManager.Butto  // ToggleiPad,key);
                              released =
                    InputManager.Button  // ToggleiPa//if(pressed)
                                         // app.DebugPrintf("Pressed
                                         // %d\n",key);//if(released)
                                         // app.DebugPrintf("Released
                                         // %d\n",key);// Repeat handling
                                         // Start repeat timer
                        m_actionRepeatTimer[iPad][key] =
                        GetTickCount() + UI_REPEAT_KEY_DELAY_MS;
                }
                else  // Stop repeat timer
                    m_actionRepeatTimer[iPad][key] = 0;
                }
                // Check is enough time has elapsed to be a repeat
                // key
                DWORD currentTime = GetTickCount();
                if (m_actionRepeatTimer[iPad][key] > 0 &&
                    currentTime > m_actionRepeatTimer[iPad][key]) {
                    ...
                    pressed = true;
                    m_actionRepeatTimer[iPad][key] =
                        currentTime + UI_REPEAT_KEY_REP
#ifndef _CONTENT_PACKAGE
#ifdef ENABLE_IGGY_PERFMON
                        if (pressed && !repeat &&
                            key == ACTION_MENU_STICK_PRESS) {
                        m_iggyPerfmonEnabled =
                            !m_igg #endifonE  // 4J Stu - Removed this
                                              // function#if 0#ifdef
                                              // __PS3__//if (	pressed
                                              // &&
                                              //	!repeat &&//
                            ...
                            //&&//	key == ACTION_MENU_STICK_PRESS)//{//
                            // static bool whichFont = true;//	if
                            // (whichFont) 	{//
                            // IggyFontSetIndirectUTF8( "Mojangles_7", -1,
                            // IGGY_FONTFLAG_all, "Mojangles_TTF",-1
                            //,IGGY_FONTFLAG_none );//
                            // IggyFontSetIndirectUTF8( "Mojangles_11", -1,
                            // IGGY_FONTFLAG_all, "Mojangles_TTF",-1
                            //,IGGY_FONTFLAG_none );//
                            // whichFont = false;//	}//	else//
                            //{	//		IggyFontSetIndirectUTF8(
                            //"Mojangles_7", -1, IGGY_FONTFLAG_all,
                            //"Mojangles_7",-1 ,IGGY_FONTFLAG_none );//
                            // IggyFontSetIndirectUTF8( "Mojangles_11", -1,
                            // IGGY_FONTFLAG_all, "Mojangles_11",-1
                            //,IGGY_FONTFLAG_none );//
                            // whichFont = true;//	}//}//else

                            if (presse  //!(app.GetGameSettingsDebugMask(ProfileManager.GetPrimaryPad())&(1L<<eDebugSetting_ToggleFont))
                                        //!&&
                                    key == ACTION_MENU_STICK_PRESS) {
                            __int64 totalStatic = 0;
                            __int64 totalDynamic = 0;
                app.Debug"********************************\n");
                app.Debug"BEGIN TOTAL SWF MEMORY USAGE\n\n");
                for (unsigned int i = 0; i < eUIGroup_COUNT; ++i) {
                    m_groups[i]->PrintTotalMemoryUsage(totalStatic,
                                                       totalDynamic);
                }
                for (unsigned int i = 0; i < eLibrary_Count; ++i) {
                    ...
                    if (m_iggyLibraries[i] != IGGY_INVALID_LIBRARY) {
                        ...
                        int iteration = 0;
                        while (res = IggyDebugGetMemoryUseInfo(
                                   NULL, m_iggy "" braries[i], , 0, iteration,
                                   &memoryInfo)) {
                        ...
                        app.Debug"Library static: %dB , Library dynamic: %d, ID: %d\n", libraryStatic, libraryDynamic, i);
                }
                app.Debug"Total static: %d , Total dynamic: %d\n", totalStatic, totalDynamic);
		...
		{
                    bool h  // Send the key to the fullscreen group
                            // first
                        m_groups[(int)eUIGroup_Fullscreen]
                            ->handleInput(iPad, key, repeat, pressed, released,
                                          handled);
                    if  // If it's not been handled yet, then pass the event
                        // onto the players specific group
                        m_groups
                        [(iPad + 1)]->handleInput(iPad, key, repeat, pressed,
                                                  released, handled);
                }
                        }
                    }

                    rrbool RADLINK UIController::ExternalFunctionCallback(
                        void* user_callback_data, Iggy* player,
                        IggyExternalFunctionCallUTF16* call) {
                        UIScene* scene =
                            (UIScene*)IggyPlayerGetUserdata(player);
                        ...
                        void UIController::renderScenes() {
                            PI "Rendering Iggy scenes"  // Only render player
                                                        // scenes if the game is
                                                        // started
                                if (app.GetGameStarted() &&
                                    !m_groups[eUIGroup_Fullscreen]
                                         ->hidesLowerScenes()) {
                                for (int i = eUIGroup_Player1;
                                     i < eUIGroup_COUNT; ++i) {
                        PI"Rendering layer %d scenes", i);
                        m_groups[i]->render();
                        PIXEndNam// Always render the fullscreen group
	PI"Rendering fullscreen scenes");
                        ...
                        if (m_iggyPerfmonEnabled) {
                            ...
                            pm_pad.field.dpad_left = InputManager.ButtonPressed(
                                ProfileManager.GetPrimaryPad(),
                                ACTION_MENU_LEFT);
                            pm_pad.field.dpad_right =
                                InputManager.ButtonPressed(
                                    ProfileManager.GetPrimaryPad(),
                                    ACTION_MENU_RIGHT);
                            pm_pad.field.button_up = InputManager.ButtonPressed(
                                ProfileManager.GetPrimaryPad(), ACTION_MENU_Y);
                            pm_pad.field.button_down =
                                InputManager.ButtonPressed(
                                    ProfileManager.GetPrimaryPad(),
                                    ACTION_MENU_A);
                            pm_pad.field.button_left =
                                InputManager.ButtonPressed(
                                    ProfileManager.GetPrimaryPad(),
                                    ACTION_MENU_X);
                            pm_pad.field.button_right =
                                InputManager.ButtonPressed(
                                    ProfileManager.GetPrimaryPad(),
                                    ACTION_MENU_B);
                            pm_pad.field.shoulder_left_hi =
                                InputManager.ButtonPressed(
                                    ProfileManager.GetPrimaryPad(),
                                    ACTION_MENU_LEFT_SCROLL);
                            pm_pad.field.shoulder_right_hi =
                                InputManager.ButtonPressed(
                                    ProfileManager.GetPrimaryPad(),
                                    ACTION_MENU_RIGHT_SCROLL);
                            pm_pad.field.trigger_left_low =
                                InputManager.ButtonPressed(
                                    ProfileManager.GetPrimaryPad(),
                                    ACTION_MENU_PAGEUP);
                pm_pad.field.trigger_right_low = InputManager.ButtonPressed(ProfileManager.GetPrimaryPad(),ACTIO//IggyPerfmonPadFromXInputStatePointer(pm_pad, &xi_pad);//gdraw_D3D_SetTileOrigin( fb,//	zb,//	PM_ORIGIN_X,
//	PM_ORIGIN_Y );
		
		IggyPerfmonTickAndDraw(iggy_perfmon, gdraw_funcs, &pm_pad,
			PM_ORIGIN_X, PM_ORIGIN_Y, getScreenWidth(), getS// perfmon draw area in window coords#endif
                        }
                                }
                                ...
                                void UIController::setupCusto  // Rest the clear
                                                               // rect
                                ...
                                m_customRenderingClearRect
                                    ...
                                    P "StartFrame" Event(0, );
                                RenderManager.StartFrame();
                                PIXEndNamedEvent();
                                gdraw_D3D1 #elif defined __PS3__
                                    RenderMa #elif defined __PSVITA__
                                        RenderMa #elif defined __ORBIS__
                                            RenderManager
                                                .  // Set up a viewport for the
                                                   // render that matches Iggy's
                                                   // own viewport, apart form
                                                   // using an opengl-style
                                                   // z-range (Iggy uses a
                                                   // DX-style range on PS4),
                                                   // so// that the renderer
                                                   // orthographic projection
                                                   // will work
                                    gdraw_orbi #endifiewport_4J();

                                RenderManager
                                    .S  // 4J Stu - We don't need to clear this
                                        // here as iggy hasn't written anything
                                        // to the depth buffer.// We DO however
                                        // clear after we render which is why we
                                        // still setup the rectangle
                                        // here//RenderManager.Clear(GL_DEPTH_BUFFER_BIT,
                                        // &m_customRenderingClearRect);//glClear(GL_DEPTH_BUFFER_BIT);

                                    P "Final setup" vent(0, );
                                glMatrixMode(GL_PROJECTION);
                                glLoadIdentity();
                                glOrtho(0, m_fScreenWidth, m_fScreenHeight, 0,
                                        1000, 3000);
                            ...
                            void UIController::setupCustomDrawMatrices(
                                UIScene * scene,
                                CustomDrawData * customDrawRegion) {
                                Minecraft* pMinecraft =
                                    Minecraf  // Clear just the region required
                                              // for this
                                              // control.
                                    float sceneWidth =
                                        (float)scene->getRenderWidth();
                                float sceneHeight =
                                    (float)scene->getRenderHeight();

                                LONG left, #ifdef __PS3__ttom;

        if(!RenderManager.IsHiDef() && !RenderManager.I// 4J Stu - Our SD target on PS3 is double width
		left = m_tileOriginX + (sceneWidth + customDrawRegion->mat[(0*4)+3]*sceneWidth);
		right = left + ( (sceneWidth * customDrawRegion->mat[0]) ) * customDrawR#endif>x1;
                            }
                            else {
                                left = m_tileOriginX +
                                       (sceneWidth +
                                        customDrawRegion->mat[(0 * 4) + 3] *
                                            sceneWidth) /
                                           2;
                                right =
                                    left +
                                    ((sceneWidth * customDrawRegion->mat[0]) /
                                     2) *
                                        customDrawRegion->x1;
                            }
                            ...
                            top = m_tileOriginY +
                                  (sceneHeight -
                                   customDrawRegion->mat[(1 * 4) + 3] *
                                       sceneHeight) /
                                      2;
                            bottom =
                                top + (sceneHeight *
                                       -customDrawRegion->mat[(1 * 4) + 1]) /
                                          2 * customDrawRegion->y1;

                            m_customRenderingClearRect.left =
                                std::min(m_customRenderingClearRect.left, left);
                            m_customRenderingClearRect.right = std::max(
                                m_customRenderingClearRect.right, right);
                            ;
                            m_customRenderingClearRect.top =
                                std::min(m_customRenderingClearRect.top, top);
                            m_customRenderingClearRect.bottom = std::max(
                                m_customRenderingClearRect.bottom, bottom);
                            ...
                            if (!m_bScreenWidthSetup) {
                                ...
                                if (pMinecraft != NULL) {
                                    m_fScreenWidth =
                                        (float)pMinecraft->width_phys;
                                    m_fScreenHeight =
                                        (float)pMinecraft->height_phys;
                                    m_bScreenWidthSetup = true;
                                }
                            }

        glLoadI// Iggy translations are based on a double-size target, with the origin in the centre
	glTranslatef((m_fScreenWidth + customDrawRegion->mat[(0*4)+3]*m_fScreenWidth)/2,(m_fScreenHeight - customDrawRegio// Iggy scales are based on a double-size target
	glScalef( (m_fScreenWidth * customDrawRegion->mat[0])/2,(m_fScreenHeight * -customDrawRegion->mat[(1*4) + 1])/2,1.0f);
                        ...
                        void U#ifdef __ORBIS__Cu  // TO BE IMPLEMENTED{

                            Re #elseanager.Clear(GL_DEPTH_BUFFER_BIT);

        RenderManager.Clear(GL_DEPTH_BU#endifIT//glClear(GL_DEPTH_BUFFER_BIT);
...
GDrawTexture * RADLINK UIController::TextureSubstitutionCreateCallback ( void * user_callback_data , IggyUTF16 * texture_name , S32 * width , S32 * height , void * * destroy_callback_data )
{
            ...
            AUTO_VAR(it, uiController->m_substitutionTextures.find(
                             (wchar_t*)texture_name));

            if (it != uiController->m_substitut
                      "Found substitution texture %ls, with %d "
                      "bytes\n",
            ...
            if (image.getData() != NULL) {
                ...
                int id = t->getTexture(
                    &image,
                    C4JRend  // 4J Stu - All our flash controls that allow
                             // replacing textures use a special 64x64
                             // symbol// Force this size here so that
                             // our images don't get scaled wildly	#if
                    ...
                    *destroy_callback
                    "Found substitution texture %ls (%d) - "
                    "%dx%d\n",
                    (wchar_t*)texture_name, id, image.getWidth(),
                    image.getHeight());
                return ui.getSubstitutionTexture(id);
            } else {
                        return "Could not find substitution texture %ls\n", (wchar_//Descriptiona//Callback received from Iggy when it stops using a user-defined texture.
void RADLINK UIController::TextureSubstitutionDestroyCallback ( void * user_callback_data , void * destroy_cal// Orbis complains about casting a pointer to an int
	LONGLONG llVal=(LONGLONG)destroy_callback_dat"Destroying iggy texture %d\n"bugPrintf(, id);
...
void UIController::registerSubstitutionTexture(const std::wstring &textur// Remove it if it already existsth)
{
                unregisterSubstitutionTexture(textureName, false);

                m_substitutionTextures[textureName] =
                    byteArray(pbData, dwLength);
}
...
{
                static bo  // If you're navigating to the multigamejoinload, and
                           // the player hasn't seen the updates message yet,
                           // display it now// display this message the first
                           // 3 times
                    ...
                    // if you're trying to navigate to the inventory,the
                    // crafting, pause or game info or any of the trigger scenes
                    // and there's already a menu up (because you were pressing
                    // a few buttons at the same time) then ignore the
                    // navigate
                ...
                case"IGNORING NAVIGATE - we're trying to navigate to a user selected scene when there's already a scene up: pad:%d, scene:%d\n", iPad, scene);
			return false;
			break;
		}
	}
	...
	switch(scene)
	{
	c// 4J Stu - The fullscreen progress scene should not interfere with any other scene stack, so should be placed in it's own group/layer
			layer = eUILayer_Fullscreen;
			group = eUIGroup_Fullscreen;
		}
		break;
	c// The connecting progress scene shouldn't interfere with other scenes
			layer = eUILayer_Fullscreen;
		}
// The end poem scene shouldn't interfere with other scenes, but will be underneath the autosave progress
	...
	if(group == eUIGroup_PAD)// If the game isn't running treat as user 0, otherwise map index directly from pad
			if( ( iPad != 255 ) && ( iPad >= 0 ) )
			{
	...
	LeaveCriticalSection(&m_nav"Navigate to scene"er.PrintElapsedTime(L//return true;);
...
bool UIController::NavigateBack(int iPad, bool forceUsePad, EUIScene eScene, EUILayer eLayer)
{
	...
	if( app.GetGameStarted() )
	{
		...
		if(!navComplete && ( iPad != 255 ) && ( iPad >= 0 ) )
		{
			...
			if(!m_groups[(int)group]->GetMenuDisplaye// 4J-PB - autosave in fullscreen doesn't clear the menuDisplayed flag
		else
		{
...
void UIController::NavigateToHomeM// Alert the app the we no longer want to be informed of ethernet connections
	app.SetLiveLinkRequired( false );

	Minecraft *p// 4J-PB - just about to switched to the default texture pack , so clean up anything texture pack related here// unload any texture pack audio// if there is audio in use, clear out the audio, and unmount the pack
	TexturePack *pTexPack=Minecraft::GetInstance()->skins->getSelected();


	DLCTexturePack *pDLCTexPa// get the dlc texture pack, and store it
		pDLCT// change to the default texture pack

	
	pMinecraft->skins->selectTexturePackById(TexturePackRepository::DEFAULT_TEXTURE_PAC// need to stop the streaming audio - by playing streaming audio from the default texture pack now// reset the streaming sounds back to the normal ones
		pMinecraft->soundEngine->SetStreamingSounds(eStream_Overworld_Calm1,eStream_Overworld_piano3,
			...
			eStream_CD_1);
		""inecraft->soundEngine// 		if(pDLCTexPack->m_pStreamedWaveBank!=NULL)// 		{// 			pDLCTexPack->m_pStreamedWaveBank->Destroy();// 		}// 		if(pDLCTexPack->m_pSoundBank!=NULL)// 		{// 			pDLCTexPack->m_pSoundBank->Destroy();// 		}#ifdef _XBOX_ONE
		

		DWORD resul"TPACK"rag#elseger.UnmountInstalledDLC(L);

		DWORD resu"TPACK"ora#endifger.UnmountInstalled"Unmount result is %d\n"app.DebugPrintf(,result);
	}
...
void UIController::CloseAllPlayersScenes()
{
	...
	for(unsigned i//m_bCloseAllScenes[i] = true;++i)
	{
...
{
	EUIGroup group;
	if( a// If the game isn't running treat as user 0, otherwise map index directly from pad
	...
	m_groups[(int)gro// This should cause the popup to dissappear
...
void UIController::setFul// Show/hide the tooltips for the fullscreen group
	m_groups[(int)eUIGroup_Fullscreen]->showComponent(ProfileManager.GetPrimaryPad(),eUIComponent_T// Show/hide tooltips for the other layers
	for(unsigned int i = (eUIGroup_Fullscreen+1); i < eUIGroup_COUNT; ++i)
	{
                            m_groups[i]->showComponent(i, eUIComponent_Tooltips,
                                                       eUILayer_Tooltips,
                                                       !displayed);
	}
                    }
                    ...
                    bool UIController::IsPauseMenuDisplayed(int iPad) {
                        EUIGroup  // If the game isn't running treat as user 0,
                                  // otherwise map index directly from
                                  // pad
                            if ((iPad != 255) && (iPad >= 0)) group =
                                (EUIGroup)(iPad + 1);
                        else group = eUIGroup_Fullscreen;
                    }
                    else {
...
{
            EUIGroup  // If the game isn't running treat as user 0, otherwise
                      // map index directly from
                      // pad
                if ((iPad != 255) && (iPad >= 0)) group = (EUIGroup)(iPad + 1);
            else group = eUIGroup_Fullscreen;
	}
	...
	return m_groups[(int)eUIGroup_Fullscreen]->IsIgnoreAutosaveMenuDisplayed() || (group != eUIGroup_Fullscreen && m_groups[(int)group]->IsIgnoreAutosaveMenuDisplayed());
                    ...
                    void UIController::SetIgnoreAutosaveMenuDisplayed(
                        int iPad, bool dis
                        "UIController::SetIgnoreAutosaveMenuDisplayed is not "
                        "implemented"
                        "\n");
                }
                    ...
                    if (bVal) {
                        if (iPad == XUSER_INDEX_ANY) {
                            for (int i = 0; i < XUSER_MAX_COUNT; i++) {
                                InputManager.SetMenuDisplayed(i,t// 4J Stu - Fix for #11018 - Functional: When the controller is unplugged during active gameplay and plugged back in at the resulting pause menu, it will demonstrate dual-functionality.
				m_bMenuToBeClosed[i]=false;
                            }
                        } else {
                        InputManager.SetMenuDisplayed(iPad,tr// 4J Stu - Fix for #11018 - Functional: When the controller is unplugged during active gameplay and plugged back in at the resulting pause menu, it will demonstrate dual-functionality.
			m_bMenuToBeClosed[iPad]=false;
                        ...
                        if (iPad == XUSER_INDEX_ANY) {
                            ...
                            m_bMenuToBeClose
#ifdef _DURANGO m_i  // 4J-JEV: When in-game, allow player to
                                     // toggle the 'Pause' and 'IngameInfo'
                                     // menus via
                                     // Kinnect.
                                if (Minecraft::GetInstance()->running)
                                    InputManager.SetEnabledGtcButtons(
                                        _360_GTC_ #endif _360_GTC_PAUSE |
                                        _360_GTC_VIEW);
                        }
                    }
                }
                ...
                void UIController::SetTooltipText(unsigned int iPad,
                                                  unsigned int tooltip,
                                                  int iTextID) {
                    EUIGroup  // If the game isn't running treat as user 0,
                              // otherwise map index directly from
                              // pad
                ...
                if (m_groups[(int)group]->getTooltips())
                    m_groups[(int)group]->getTooltips()->SetTooltipText(
                        tooltip, iTextID);
                }

                void UIController::SetEnableTooltips(unsigned int iPad,
                                                     BOOL bVal) {
                    EUIGroup  // If the game isn't running treat as user 0,
                              // otherwise map index directly from
                              // pad
                ...
                if (m_groups[(int)group]->getTooltips())
                    m_groups[(int)group]->getTooltips()->SetEnableTooltips(
                        bVal);
                }

                void UIController::ShowTooltip(unsigned int iPad,
                                               unsigned int tooltip,
                                               bool show) {
                    EUIGroup  // If the game isn't running treat as user 0,
                              // otherwise map index directly from
                ...
                if (m_groups[(int)group]->getTooltips())
                    m_groups[(int)group]->getTooltips()->ShowTooltip(tooltip,
                                                                     show);
                }

void UIController::SetTooltips( unsigned int iPad, int iA, int iB, int iX, int iY, int iLT, int iRT, int iLB, int iRB, int iLS, int iRS, int iBack,// 4J-PB - strip out any that are not applicable on the platform#ifndef _XBOX
	...
	if(iY=
#endifO #endifVIEW_GAMERPROFILE) iY = -1;
...
if (m_groups[(int)group]->getTooltips())
    m_groups[(int)group]->getTooltips()->SetTooltips(iA, iB, iX, iY, iLT, iRT,
                                                     iLB, iRB, iLS, iRS, iBack,
                                                     forceUpdate);
}

void UIController::EnableTooltip(unsigned int iPad, unsigned int tooltip,
                                 bool enable) {
    EUIGroup  // If the game isn't running treat as user 0, otherwise map index
              // directly from pad
        if ((iPad != 255)) group = (EUIGroup)(iPad + 1);
    else group = eUIGroup_Fullscreen;
}
...
void UIController::RefreshTooltips(unsigned in
                                   "UIController::RefreshTooltips is not "
                                   "implemented\n");
}

void UIController::AnimateKeyPress(int iPad, int iAction, bool bRepeat,
                                   bool bPressed, bool bReleased) {
    EU  // only animating button presslse)
    {
        retu  // If the game isn't running treat as user 0, otherwise
              // map index directly from
              // pad
            if ((iPad != 255) && (iPad >= 0)) group = (EUIGroup)(iPad + 1);
        else group = eUIGroup_Fullscreen;
...
void UIController::OverrideSFX(int iPad, int iAction, bool bVal) {
    EUIGroup g  // If the game isn't running treat as user 0, otherwise map
                // index directly from pad
        if ((iPad != 255) && (iPad >= 0)) group = (EUIGroup)(iPad + 1);
    else group = eUIGroup_Fullscreen;
}
else {
...
void UIController::PlayUISFX(ESoundEffect eSound) {
    __uint6  // Don't play multiple SFX on the same tick// (prevents horrible
             // sounds when programmatically setting multiple
             // checkboxes)
...
void UIController::DisplayGa  // The host decides whether these are on or
                              // off
...
EUIGroup group = (EUIGroup)(iPad + 1);
if (m_groups[(int)group]->getHUD()) m_groups[(int)gro// Update TutorialPopup in Splitscreen if no container is displayed (to make sure the Popup does not overlap with the Gamertag!)
...
void UIController::UpdateSelectedItemPos(
    unsigned in
    "UIController::UpdateSelectedItemPos not "
    "implemented\n");
}

void UIController::HandleDLCMountingComplete() {
    for (unsigned int i = 0; i < eU
...
void UICont  // app.DebugPrintf(app.USER_SR, "UIController::HandleDLCInstalled
             // not implemented\n");
    for (unsigned int i = 0; i < eUIGroup_COUNT; ++i) {
#ifdef _XBOX_ONEeDLCInstalled();
}
}

void UIController::HandleDLCLicenseChange() {
    for (unsigned int i = 0; i < eU
                                 "UIController::HandleDLCLicenseChange - "
                                 "m_groups[%d]\n",
                      i);
                m_gr#endif]->HandleDLCLicenseChange();
}
}

void UIController::HandleTMSDLCFileRetrieved(
    in
    "UIController::HandleTMSDLCFileRetrieved not "
    "implemented\n");
}

void UIController::HandleTMSBanFileRetrieved(
    in
    "UIController::HandleTMSBanFileRetrieved not "
    "implemented\n");
}
...
void UIController::SetTutorialDescription(int iPad, TutorialPopupInfo* info) {
    ...
    group = eUIGroup_Fullscreen;
        ...
        if(m_group// tutorial popup needs to know if a container menu is being displayed
		m_groups[(int)group]->getTutorialPopup()->SetContainerMenuVisible(m_groups[(int)group]->IsContainerMenuDisplayed());
		m_groups[(int)group]->getTutorialPopu
#ifndef _XBOXalDescription(info);
        }
        }

        void UIController::RemoveInteractSceneReference(int iPad,
                                                        UIScene* scene) {
            EUIGroup group;
            if ((iPad != 255) && (iPad >= 0))
                group = (EUIGroup)(iPad + 1);
...
#endifeInteractSceneReference(scene);
        }
        ...
        void UIController  // 4J Stu - We shouldn't need to implement
                           // this
        }

        void U  // 4J Stu - We might not need to implement
                // this"UIController::HideAllGameUIElements
                // not implemented\n");
        }

        void UIController::ShowOtherPlayersBas  // 4J Stu - We shouldn't need to
                                                // implement this
        }
        ...
        void UIController::UpdateTrialTimer(unsigned int iPad) {
            ...
            dwTimeTic
#ifndef _CONTENT_PACKAGEs - \
            dwTimeTic #else  // display the time - only if there's less than 3
                             // minutes#endif
                if (dwTimeTicks < 180)

            {
                int iMins = dwTimeTicks / 60;
                int iSeconds = d "%d:%02d"s % 60;
                swprintf(wcTime, 20, L, iMins, iSeconds);
                if (m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay())
                    m_groups[(int)eUIGroup_Fullscreen]
                        ->getPressStartToPlay()
                ...
                if (m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay())
                    m_groups[(
                        int)eUIGroup_Fullscreen] "" getPress  // are we out of
                                                              // time?lTimer(L);
            }

            // Trial over	if// bring up the pause menu to stop the trial
            // over message box being called again?
            if (!ui.GetMenuDisplayed(iPad)) {
                ui.NavigateToScene(iPad, eUIScene_PauseMenu, NULL,
                                   eUILayer_Scene);
        ...
        void UIController::UpdateAutosaveC #if !(defined(_XBOX_ONE) ||
                                                 defined(__ORBIS__))
            WCHAR wcAutosaveCountdown[100];
        swprintf(wcAutosaveCountdown, 100,
                 app.GetString(IDS_AUTOSAVE_COUNTDOWN), uiSeconds);
        if (m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay())
            m_groups[(int)eUIGroup_Fullscreen]
                ->getPressStartToPlay #endiftTrialTimer(wcAutosaveCountdown);
        }
        ...
        void UICont #ifndef _CONTENT_PACKAGEle(bool show) {
            if (show) {
                m_uiDebugConsole =
                    (UIComponent_DebugUIConsole*)m_groups[eUIGroup_Fullscreen]
                        ->addComponent(0, eUIComponent_DebugUIConsole,
                                       eUILayer_Debug);
            } else {
                m_groups[eUIGroup_Fullscreen]->removeComponent(
                    eUIComponent_DebugUIConsole, eUILaye #endifg);
                m_uiDebugConsole = NULL;
            }
        }

        void UIController : #ifndef _CONTENT_PACKAGEde(bool show) {
            if (show) {
                ...
                m_groups[eUIGroup_Fullscreen]->removeComponent(eUIComponent_DebugUIMarketingGuide, eUILayer_Debug#endif_uiDebugMarketingGuide = NULL;
            }
        }
        ...
        C4JStorage::EMessageResult UIController::RequestAlertMessage(
            ...
            return RequestMessageBox(uiTitle, uiText, uiOptionA, uiOptionC,
                                     dwPad, Func, lpParam, pwchFormatString, 0,
                                     false);
        }
...

        {
            ...
            bool completed  // Queue this message boxngSkin())
            {
                ...
                memcpy(queuedData->info.uiOptionA, param.uiOptionA,
                       param.uiOptionC * sizeof(UINT));
                queuedData->iPad =
                    dwPad  // Ensures that these don't get wiped out by a
                           // CloseAllScenes
                           // call
                        m_queuedMessageBoxData.push_back(queuedData);
            }
            else {
                completed = ui.NavigateToScene(dwPad, eUIScene_MessageBox, &param, layer, eUIGroup// This may happen if we had to queue the message box, or there was already a message box displaying and so the NavigateToScene returned false;
        ...
        C4JStorage::EMessageResult UICon /* = -1 */ equestUGCMessa /* = -1 */
            title /* = -1*/ INT message,
            int iPad,
            int (*Func)(LPVO /* = NULL*/ t C4JStorage::EM /* = NULL*/
                            t)  // Default title / messages)
        {
            if (title == -1) {
            ...
            if (message == -1) {
                message = IDS_NO_USER  // Default pad to primary playerE;
            }

            if (iPad == -1)
#ifdef __ORBIS__na  // Show the vague UGC system message in addition to
                            // our
                            // message
                ProfileManager.DisplaySystemMessage(
                    SCE_MSG_DIALOG_SYSMSG_TYPE_TRC_PSN_UGC_RESTRICTION, iPad);
            ret #elif defined(__PSVITA__) ResultAccept;

            ProfileManager.ShowSystemMessage(
                SCE_MSG_DIALOG_SYSMSG_TYPE_TRC_PSN_CHAT_RESTRICTION, iPad);
            UINT uiIDA[1];
            uiIDA[0] = IDS_CONFIRM_OK;
            return ui.RequestAlertMessage(title, IDS_CHAT_RESTRICTI #elseC,
                                          uiIDA, 1, iPad, Func, lpParam);

            UINT uiIDA[1];
            uiIDA[0] = IDS_CONFIRM_OK;
            return ui.RequestAlertMessage(title, m #endif, uiIDA, 1, iPad, Func,
                                          lpParam);
        }

        C4JStorage::EMessageResult UIController::Reque /* = -1 */
            estrictedMessa /* = -1 */ title /* = -1*/ INT message,
            int iPad,
            int (*Func)(LPVO /* = NULL*/ t C4JStorage::EM /* = NULL*/
                            t)  // Default title / messages)
        {
            if (title == -1) {
                title = IDS_FAILED_TO_CREATE #if defined (_XBOX_ONE) ||
                        defined(_WINDOWS64)  // IDS_CONTENT_RESTRICTION doesn't
                                             // exist on XB1
                        message = IDS_NO #else_CREATED_CONTENT_PRIVILEGE_CREATE;
#endifessage  // Default pad to primary player
            }
            ...
            re #elseC4JStorage::EMessage_ResultAccept;
        ...
        void UIController::
                setFont /* 4J-JEV: As described in an email from
      Sean. If your `optional_temp_buffer` is NULL, Iggy will
      allocate the temp buffer on the stack during Iggy draw calls.
      The size of the buffer it will allocate is 16 bytes times
      `max_chars` in 32-bit, and 24 bytes times `max_chars` in
      64-bit. If the stack of the thread making the draw call is
      not large enough, Iggy will crash or otherwise behave
      incorrectly.
      */ #if defined __ORBIS__ ||
            defined _DURANGO ||
            defined _WIN64 #else static const int CHAR_SIZE = 24;
#endif static const int CHAR_SIZE = 16;
        ...
        if (length < 0) {
            ...
            m_tempBuffer = new char[CHAR_SIZE * length];
                IggySetFontCachingCalculationBuffer(length,// Returns the first scene of given type if it exists, NULL otherwise 
UIScene *UIController::FindScene(EUIScene sceneType)
{
        UIScene* pScene = NULL;

        for (int i = 0; i < eUIGroup_COUNT; i++) {
                pSce#ifdef __PS3__i]->FindScene(sceneType);
#elseif(pScene != NULL) return pScene;

#endifpScene != nullptr) retu
#ifdef __PSVITA__ }

                return pScene;
        }

        void UIController::TouchBoxAdd(UIControl * pControl,
                                       UIScene * pUIScene) {
        ...
        void UIController::TouchBoxAdd(UIControl * pControl, EUIGroup eUIGroup,
            ...
            S32 iMainPanelOf  // 4J-TomK add main panel offset if controls do
                              // not live in the root
                              // scene
                if (pMainPanelControl) {
                iMainPanelOffsetX = pMainPanelControl->getXPos();
                iMainPanelOffse// 4J-TomK override control width / height where needed
	if(puiElement->pControl->getCo// Sliders are never scaled but masked, so we have to get the real width from AS
		UIControl_Slider *pSlider = (UIControl_Slider *)puiElement->pControl;
		iControlWidth = pSlider->GetRealWidth();
            }
        else if(puiElement->pControl->getControlType// The origin of the TexturePackList is NOT in the top left corner but where the slot area starts. therefore we need the height of the slot area itself.
        ...
        else if(puiElement->pControl->getControlT// The height and width of this control changes per how to play page
	...
	else if(puiElement->pControl->getContr// The height and width of this control changes per how to play page
        ...
        puiElement->x2 =
            (S32)(((float)pControl->getXPos() + (float)iControlWidth +
                   (float)iMainPanelOffsetX));
        puiElement->y2 =
            (S32)(((float)pControl->getYPos() + (float)iControlHeight +
                   (float)iMainPanelOffsetY));

        if (puiElement->pControl->getControlType() == UICo "NO CONTROL!" trol) {
            app.DebugPrintf();
        }

        if(puiElement->x1 == puiElement->x2 || puiElement->y1 "NOT adding touchbox %d,%d,%d,%d\n"intf(,puiElement->x1,puiElement->y1,puiElement->x2,puiElem"Adding touchbox %d,%d,%d,%d\n"ugPrintf(,puiElement->x1,puiElement->y1,puiElement->x2,puiElement->y2);
        ...
        void UIController::TouchBoxRebuild(UIScene* pUIScene) {
            EUIGroup eUIGroup = pUIScene->GetParentLayerGroup();
            EUILayer eUILayer = pUIScene->GetParentLayer()->m_iLayer;
            EUISce  // if we delete an element, it's possible that the scene has
                    // re-arranged all the elements, so we need to rebuild the
                    // boxes// rebuild
                    // boxesBoxesClear(pUIScene);

                AUTO_VAR(itEnd, pUIScene->GetControls()->end());
            for (AUTO_VAR(it, pUIScene->GetControls()->begin()); it != itEnd;
                 it++) {
                ...
                if (control->getControlType() == UIControl::eButton ||
                    ...
                    // 4J-TomK update the control (it might have been moved by
                    // flash / AS)
        ...
        void UIController::TouchBoxesClear(UIScene* pUIScene) {
            ...
            for (AUTO_VAR(it,
                          m_TouchBoxes[eUIGroup][eUILayer][eUIscene].begin());
                 it != itEnd; it++) {
        ...
        bool UIController::TouchBoxHit(UIScene* pUIScene, S32 x, S32 y) {
            EUIGroup eUIGroup = pUIScene->GetParentLayerGroup();
            EUILayer eUILayer = pUIScene->GetParentLayer()->m_iLayer;
            EUISce  // 4J-TomK let's do the transformation from touch resolution
                    // to screen resolution here, so our touchbox values always
                    // are in screen
                    // resolution!
            ...
            if (m_TouchBoxes[eUIGroup][eUILayer][eUIscene].size() > 0) {
                AUTO_VAR(itEnd,
                         m_TouchBoxes[eUIGroup][eUILayer][eUIscene].end());
                for (AUTO_VAR(
                         it,
                         m_TouchBoxes[eUIGroup][eUILayer][eUIscene].begin());
                     it != itEnd; it++) {
                    UIELEMENT* element = (UIELEMENT*)*it;
                        if(element->pControl->getHidden() == fa// ignore removed controlsgetVisible()) 
			{
                if ((x >= element->x1) && (x <= element->x2) &&
                    ...
                    if (!m_bTouchscre
                        ...
                        ;
                    m  // remember the currently highlighted
                       // element
                        m_HighlightedUIElement = element;

                    // app.DebugPrintf("MISS at x = %i y = %i\n", (int)x,
                    // (int)y);
                    m_Highlig  // e// Handle Touch Inpute//rn false;
                }

void UIController::HandleTouchInput(unsigned int iPad, unsigned int key, bool bPr// no input? no handling! bReleased)
{
        if(!b// override for instand repeat without delay!
		if(m_bTouchscreenPressed && m_ActiveUIElement && (
			...
			m_ActiveUIElement->pControl->getControlType() == UICont// the above controls need to be controllable without having the finger over them
		else
			return;
	}
...
// PRESSED HANDLINGepeat && !bReleased)	"touch input pressed\n"	app.DebugPrintf();
		switch(m_ActiveUIElement->pControl->getControlTy// set focuscase UIControl::eButton:
			
			UIControl_Button *pButton=(UIControl_Button *)m_ActiveUIElement->pControl;
			pButton->getParentScene()->SetFocusToElement(m_// override bPressed to false. we only want the button to trigger on touch release!
			bPressed = false;
// set focuscase UIControl::eSlider:
		...
		// set focusse UIControl::eCheckBox:
			
			UIControl_CheckBox *pCheckbox=(UIControl_CheckBox *)m_ActiveUIElement->pControl;
			pCheckbox->getParentScene()->SetFocusToElement(m_// override bPressed. we only want the checkbox to trigger on touch release!
			bPressed = false;
			b// set focus to listol::eButtonList:
			
			UIControl_ButtonList *pButtonList=(UIControl_Butto//pButtonList->getParentScene()->SetFocusToElement(m_ActiveUIElement->pControl->getId());// tell list where we tapped it so it can set focus to the correct button
			pButtonList->SetTo// override bPressed. we only want the ButtonList to trigger on touch release!
			bPressed = false;
			break;// set focus to listTexturePackList:
			
			UIControl_TexturePackList *pTexturePackList=(UIControl_TexturePackList *)m_ActiveUIElement->pControl;
			pTexturePackList->getParentScene()->SetFocusToElement(m_// tell list where we tapped it so it can set focus to the correct texture pack
			pTexturePackList->SetTouchFocus((float)x - (float)m_ActiveUIElement->x1, (float)y - (// override bPressed. we only want the TexturePack List to trigger on touch release!
			bPressed = false;
			// set focuse UIControl::eTextInput:
			
			UIControl_TextInput *pTextInput=(UIControl_TextInput *)m_ActiveUIElement->pControl;
			pTextInput->getParentScene()->SetFocusToElement(m_// override bPressed to false. we only want the textinput to trigger on touch release!
			bPressed = false;
			bre// handle dynamic label scrollingel:
			
			UIControl_DynamicLabel *pDynamicLabel=(UIControl_DynamicLabel *)m_ActiveUIElement->pControl;
			// override bPressed to false true);
			
			bPressed = false;
			// handle dynamic label scrollingel:
			
			UIControl_HTMLLabel *pHtmlLabel=(UIControl_HTMLLabel *)m_ActiveUIElement->pControl;
// override bPressed to false true);
			
			bPressed = false;
			break;// set focus to listLeaderboardList:
			
			UIControl_LeaderboardList *pLeaderboardList=(UIControl_Leaderboar// tell list where we tapped it so it can set focus to the correct button
			pLeaderboardList->SetTo// override bPressed. we only want the ButtonList to trigger on touch release!
			bPressed = false;
			bre// pass on touch input to relevant parent scene so we can handle it there!
			m_ActiveUIElement->pControl->getParentScene()->handleTouchInput(iPad, x, y, m_ActiveUIElement->pControl->get// override bPressed to falseeased);
			
			bPressed = false;
"PRESSED - UNHANDLED UI ELEMENT\n"rintf(// REPEAT HANDLING
	}
	else if(bRepeat)	
	{
                switch(m_ActiveUIElement->pControl->getControlTy/* no action */e UIControl::eButton:
			
// handle slider movementl::eSlider:
			
			UIControl_Slider *pSlider=(UIControl_Slider *)m_ActiveUIElement->pControl;
			float fNewSliderPos = ((float)x - (float)m_ActiveUIElement->x1) / (float)pSlider->GetRealWidth();
			pSlider->SetSliderTouchPos(fNewSliderPos);
		/* no action */UIControl::eCheckBox:
			
			bRepeat = false;
			bPressed = false;
			b// handle button list scrollingList:
...
// override bPressed & bRepeat to false	
			bPressed = false;
			bRepeat = false;
			break;// handle button list scrollingList:
			...
			bre// override bPressed to falseontrol:
			// pass on touch input to relevant parent scene so we can handle it there!
			m_ActiveUIElement->pControl->getParentScene()->handleTouchInput(iPad, x, y, m_ActiveUIElement->pControl->get// override bRepeat to falseleased);
			
			bRepeat = false;
"REPEAT - UNHANDLED UI ELEMENT\n"Printf(// RELEASED HANDLING}
	}
	if(bReleased)	"touch input released\n"app.DebugPrintf();
		switch(m_ActiveUIElement->pControl->getControlTy// trigger button on release (ONLY if the finger is still on it!)
			if(m_HighlightedUIElement && m_ActiveUIElement->pControl == m_HighlightedUIElement->pControl)
				bPressed = true;
/* no action */e UIControl::eSlider:
			
		// trigger checkbox on release (ONLY if the finger is still on it!)
			if(m_HighlightedUIElement && m_ActiveUIElement->pControl == m_HighlightedUIElement->pControl)
			{
                UIControl_CheckBox* pCheckbox =
                    (UIControl_CheckBox*)m_ActiveUIElement
                        ->p  // only proceed if checkbox is enabled!
                             //
                            pCheckbox->TouchSetCheckbox(
                                !pCheckbox->IsChecked());
			}
			bReleased = false;
			b// trigger buttonlist on release (ONLY if the finger is still on it!)
			if(m_HighlightedUIElement && m_ActiveUIElement->pControl == m_HighlightedUIElement->pControl)
			{
			...
			break;// trigger texturepack list on release (ONLY if the finger is still on it!)
			if(m_HighlightedUIElement && m_ActiveUIElement->pControl == m_HighlightedUIElement->pControl)
			{
			...
			break;/* no action */ol::eLeaderboardList:
			
			bre// trigger only if touch is released over the same component!
			if(m_HighlightedUIElement && m_ActiveUIElement->pControl == m_H// pass on touch input to relevant parent scene so we can handle it there!
				m_ActiveUIElement->pControl->getParentScene()->handleTouchInput(iPad, x, y, m_ActiveUIElement->pControl->getId(),// override bReleased to false;
                }

                bReleased = false;
"RELEASED - UNHANDLED UI ELEMENT\n"intf(// only proceed if there's input to be handled
...
void UIController::SendTouchInput(unsigned int iPad, unsigned int key, bool bPressed, bool bRepeat, boo// Send the key to the fullscreen group first
	m_groups[(int)eUIGroup_Fullscreen]->handleInput(iPad, key, bRepeat, bPressed, // If it's not been handled yet, then pass the event onto the players specific group
		m_groups[(iPad+1)]->handleInput(iPad, key, bRepe

#endifssed, bReleased, handled);
        }
        }
