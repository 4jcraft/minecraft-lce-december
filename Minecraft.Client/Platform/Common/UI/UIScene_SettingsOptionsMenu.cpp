...
UIScene_SettingsOptionsMenu::UIScene_SettingsOptionsMenu(int iPad,
    ...
    swprintf(startTags "<font color=\"#%08x\">",
             app.GetHTMLColour(eHTMLColor_White));
    wsText = startTags + wsText;

        m_labelDifficultyText.init(wsText// If you are in-game, only the game host can change in-game gamertags, and you can't change difficulty// only the primary player gets to change the autosave and difficulty settings
	...
	if(bRemoveInGameGamertags)
	{
        ...
        // 4J-JEV: Changing languages in-game will produce many a bug.// MGH -
        // disabled the language select for the patch build, we'll re-enable
        // afterwards// 4J Stu - Removed it with a preprocessor def as we turn
        // this off in various places#ifdef _ENABLE_LANGUAGE_SELECT
        if (app.GetGameStarted()) {
            removeControl(&m_buttonLanguageSelect, false);
        } else {
                m_buttonLanguageSelect.init(IDS_LANGUAGE_SELECTOR, eControl_Languages)#else
	removeControl( &m_buttonLanguageSelect, fals#endif
...
std::wstring UIScene_SettingsOptionsMenu::getMoviePath()
{
        if (app.GetLocalPlayerCount() > 1) {
            ret "SettingsOptionsMenuSplit";
        } else {
            ret "SettingsOptionsMenu";
        }
}
...
void UIScene_SettingsOptionsMenu::handlePress(F64 controlId, F64 childId//CD - Added for audio
...
void UIScene_SettingsOptionsMenu::handleReload() {
    ...
    m_checkboxShowTooltips.init(
        IDS_IN_GAME_TOOLTIPS, eControl_ShowTooltips,
        (app.GetGameSettings(m_iPad, eGameSetting_Tooltips) != 0));
        m_checkboxInGameGamertags.init(IDS_IN_GAME_GAMERTAGS,eControl_InGameGamertags,(app.GetGameSettings(m_iPad,eGameSetting_GamertagsVisible)!=0)// check if we should display the mash-up option
	if(m_bNotInGame && app.GetMashupPackWorlds(m_iPad)!=0xFFFFFFFF)
// the mash-up option is needed
		m_bMashUpWorldsUnhideOption=true;
}
...
for (unsigned int i = 0; i < 9; ++i) {
    if (i == 0) {
        swprintf(autosaveLabels[i], 2 "%ls",
                 app.GetString(IDS_SLIDER_AUTOSAVE_OFF));
    } else {
        swprintf(autosaveLabels[i], 2 "%ls: %d %ls",
                 app.GetString(IDS_SLIDER_AUTOSAVE), i * 15,
                 app.GetString(IDS_MINUTES));
    }
}
m_sliderAutosave.setAllPossibleLabels(9, autosaveLabels);
        m_sliderAutosave.init(autosaveLabels[ucValue],eControl_Autosave,0,8,ucVal
#if defined(_XBOX_ONE) || defined(__ORBIS__)
	removeControl(&m_sliderAutosave, tr #endif 
	...
	for(unsigned int i = 0; i < 4; ++i)
	{
    swprintf(difficultyLabels[i], 2 "%ls: %ls",
             app.GetString(IDS_SLIDER_DIFFICULTY),
             app.GetString(m_iDifficultyTitleSettingA[i]));	
	}
	m_sliderDifficulty.setAllPossibleLabels(4,difficultyLabels);
	...
	wchar_t startTags[64];
	swprintf(star"<font color=\"#%08x\">",app.GetHTMLColour(eHTMLColor_White));
 	wsText= startTags + wsText;

	m_labelDifficultyText.init(wsT// If you are in-game, only the game host can change in-game gamertags, and you can't change difficulty// only the primary player gets to change the autosave and difficulty settings
	bool bRemoveDifficulty=false;
	bool bRemoveAutosave=false;
	bool bRemoveInGameGamertags=false;
	...
	if(bRemoveInGameGamertags)
	{
                removeControl(&m_checkboxInGameGamertags, tr// MGH - disabled the language select for the patch build, we'll re-enable afterwards// 4J Stu - Removed it with a preprocessor def as we turn this off in various places#ifdef _ENABLE_LANGUAGE_SELECT// 4J-JEV: Changing languages in-game will produce many a bug.
	if (app.GetGameStarted())	
	{
        removeControl(&m_buttonLanguageSelect, false);
	}
	...
	removeControl( &m_buttonLanguageSelect,#endif );
...
void UIScene_SettingsOptionsMenu::handleSliderMove(F64 sliderId, F64 currentValue)
{
    ...
    switch ((int)sliderId) {
        case eControl_Autosave:
            ...
            app.SetGameSettings(
                m_iPad,
                eGameSetting_Autosave,  // Update the autosave timer
                app.SetAutosaveTimerTime();

                break;
                case eControl_Difficulty : m_sliderDifficulty.handleSliderMove(
                    value);

                app.SetGameSettings(m_iPad, eGameSetting_Difficulty, value);

                std::wstring wsText =
                    app.GetString(m_iDifficultySettingA[value]);
                EHTMLFontSize size = eHTMLSize_Normal;
                if (!RenderManager.IsHiDef() && !RenderManager.IsWidescreen()) {
                    size = eHTMLSize_Splitscreen;
                } wchar_t startTags[64];
                swprintf "<font color=\"#%08x\">",
                app.GetHTMLColour(eHTMLColor_White));
            wsText = startTags + wsText;
...
void UIScene_SettingsOptionsMenu::setGam// check the checkboxes
	...
	app.SetGameSettings(m_iPad,eGameSetting_Tooltips,m_checkboxShowTooltips.IsChe// the mashup option will only be shown if some worlds have been previously hidden
	if(m_bMashUpWorldsUnhideOption && m_checkboxMashupWorlds.IsC// unhide all worlds
		app.EnableMashupPackWorlds// 4J-PB - don't action changes here or we might write to the profile on backing out here and then get a change in the settings all, and write again on backing out there//app.CheckGameSettingsChanged(true,pInputData->UserIndex);
        }