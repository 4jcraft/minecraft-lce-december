...
void UIScene_SignEntryMenu::tick() {
    ...
    if (m_bConfirmed) {
        ...
        m_sign->setChanged();
                ...
                Minecraft *pMinecraft=Minecraft::GetInstance(// need to send the new data
...
void UIScene_SignEntryMenu::handleInput(int iPad, int key, bool repeat,
    ...
    switch (key) {
        case ACTION_MENU_CANCEL:
                if(p// user backed out, so wipe the sign
		""td::wstring temp=L;
...
int UIScene_SignEntryMenu::KeyboardCompleteCallback(LPVOID lp// 4J HEG - No reason to set value if keyboard was cancelled
