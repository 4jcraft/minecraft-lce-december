...
UIScene_FullscreenProgress::UIScene_FullscreenProgress(int iPad, void* initData,
    ...
    swprintf(startTags "<font color=\"#%08x\"><p align=center>",
             app.GetHTMLColour(eHTMLColor_White));
    wsText = startTags + wsTex "</p>";
...
void UIScene_FullscreenProgress::tick() {
	...
	if( exitcode != STILL// If we failed (currently used by network connection thread), navigate back
		if( exitcode != S_OK )
		{
                        if( exitcode == ERROR_CANCEL// Current thread cancelled for whatever reason// Currently used only for the CConsoleMinecraftApp::RemoteSaveThreadProc thread// Assume to just ignore this thread as something else is now running that will// cause another action
			}
		/*m_threadCompleted = true;
				...
				Minecraft *pMinecraft=Minecraft::GetInstance();
				pMinecraft->progressRenderer->progressStartNoAbort( exitReasonStringId );*///app.NavigateBack(m_CompletionData->iPad);
            ...
            m_threadCompleted = true;
						...
						navigateBack();
                                ...
                                case e_ProgressCompletion_NavigateBack:
                                        a"e_ProgressCompletion_NavigateBack\n");
                                        {
                                            ui.UpdatePlayerBasePositions();
                                            navigateBack();
                                        }
                                ...
                                case e_ProgressCompletion_NavigateBackToScene:
                                        a"e_ProgressCompletion_NavigateBackToScene\n");
                                        ui.UpdatePlayerBasePos  // 4J Stu - If
                                                                // used
                                                                // correctly
                                                                // this scene
                                                                // will not have
                                ...
                                case e_ProgressCompletion_CloseUIScenes:
                                        a"e_ProgressCompletion_CloseUIScenes\n");
                                        ui.CloseUIScenes(
                                            m_CompletionData->iPad);
                                        ui.UpdatePlayerBasePositions();
                                        break;
                                case e_ProgressCompletion_CloseAllPlayersUIScenes:
                                        a"e_ProgressCompletion_CloseAllPlayersUIScenes\n");
                                        ui.CloseAllPlayersScenes();
                                        ui.UpdatePlayerBasePositions();
                                        break;
                                case e_ProgressCompletion_NavigateToHomeMenu:
                                        a"e_ProgressCompletion_NavigateToHomeMenu\n");
                                        ui.NavigateToHomeMenu();
                                        ui.UpdatePlayerBasePositions();
                                        break;
                                default:
...
void UIScene_FullscreenProgress::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, boo//if( m_showTooltips )//ui.AnimateKeyPress(m_iPad, key, repeat, pressed, released);
...
void UIScene_FullscreenProgress::handlePress(F64 controlId, F64 childId) {
        if(m_threadCompleted && (int)controlId == eControl// This assumes all buttons can only be pressed with the A button
		ui.AnimateKeyPress(m_iPad, ACTION_MENU_A, false, tr// if there's a complete function, call it
		if(m_completeFunc)
		{
        ...
        case e_ProgressCompletion_NavigateBackToScene:
                        a"e_ProgressCompletion_NavigateBackToScene\n");
                        ui.UpdatePlayerBaseP  // 4J Stu - If used correctly this
                                              // scene will not have interfered
                                              // with any other scene at all, so
                                              // just navigate
                                              // back
                        navigateBack();
                        break;
...
void UIScene_FullscreenProgress::handleTimerComplete(int id) {
    switch (id) {
        case TIMER_FULLSCREEN_// display the next tip
			std::wstring wsText=app.FormatHTMLString(m_iPad,app.GetString(app.GetNextTip()));
			wchar_t startTags[64];
			swp"<font color=\"#%08x\"><p align=center>",app.GetHTMLColour(eHTMLColor_White));
			wsText= st"</p>"s + wsText + L;
