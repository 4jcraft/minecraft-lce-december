...
void UIScene_HUD::customDraw(IggyCustomDrawCallbackRegion* region) {
    ...
    if (slot == -1) {
        ...
        if (item != NULL) {
            ...
            if (ucAlpha < 80) {  // check if we have the timer running for the
                                 // opacity
...
void UIScene_HUD::handleReload() {
    ...
    m_showFood = true;
    m_showAir = fa  // get's initialised invisible anyways, by setting it to
                    // false we ensure it will remain visible when switching in
                    // and out of split screen!
        m_showArmour = true;
    m_showExpBar = true;
    ...
    for (unsigned int i = 0; i < CHAT_LINES_COUNT; ++i) {
        m_labelChatText[i].i "" t(L);
    }
    m_labelJukebox.i "" t(L);
...
void UIScene_HUD::SetFood(int iFood, int iLastFood, bool bP// Ignore iLastFood as food doesn't flash
	int maxFood//, iLastFood);
...
void UIScene_HUD::SetRegenerationEffect(bool bEnabled) {
    if (bEnabled != m_bRegenEffectEnabled) {
                app.De"SetRegenerationEffect to %s\n","TRUE"l"FALSE":);
...
void UIScene_HUD::SetSelectedLabel(const std::wstring &// 4J Stu - Timing here is kept the same as on Xbox360, even though we do it differently now and do the fade out in Flash rather than directly setting opacity
...
void UIScene_HUD::SetHorseHealth(int health, b /*= false*/) {
    if (m_bRidingHorse && m_horseHealth != health) {
                app.De"SetHorseHealth to %d\n", health);
...
void UIScene_HUD::SetHorseJumpBarProgress(float progress) {
    if (m_bRidingHorse && m_horseJumpProgress != progress) {
                app.De"SetHorseJumpBarProgress to %f\n", progress);
...
void UIScene_HUD::handleTimerComplete(int id) {
    ...
    if (pMinecraft->localplayers[m_iPad] != NULL) {
        Gui* pGui =
            pMinecraf  // DWORD messagesToDisplay = min( CHAT_LINES_COUNT,
        ...
        for (unsigned int i = 0; i < CHAT_LINES_COUNT; ++i) {
            ...
            m_labelChatText[i]."" tLabel(L);
                ...
                m_labelJukebox.setOpacity( //setVisible(anyVisible);
    }
