...
UITTFFont::UITTFFont(const std::string& name, const std::string& path,
    ...
    std::wstring wPath = convStringToWstring(path);
        HANDLE file = CreateFile(wPath.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NU#else
	HANDLE file = CreateFile(path.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NU#endif
	if( file == INVALID_HANDLE_VALUE )
	{
        DWORD error = GetLastError();
                app.DebugPr"Failed to open TTF file with error code %d (%x)\n", error, error);
                assert(false);
	}
