...
UIScene_HowToPlay::UIScene_HowToPlay(int iPad, void* initData,
    ...
    wsTemp.replace(wsT "%d" find(L), 2, "8"d ::wstring(L));
    ...
    wsTemp = app.GetString(IDS_VILLAGER_OFFERS_ITEM);
        wsTemp = repl"{*VILLAGER_TYPE*}",app.GetString(IDS_VILLAGER_PRIEST));
        wsTemp.replac"%s"Temp.find(L),2, app.GetString(IDS_TILE_LIGHT_GEM));
        m_labels[ eHowToPlay_LabelTrading_VillagerOffers].init(wsTe// Extract pad and required page from init data. We just put the data into the pointer rather than using it as an address.
	size_t uiInitData = ( size_t )( initData );
	
	EHowToPlayPage eStartPage = ( EHowToPlayPage )( ( uiInitData >> 16// Ignores MSB which is set to 1!
...
std::wstring UIScene_HowToPlay::getMoviePath() {
        if(app.GetLocalPlayerCount() > 1"HowToPlaySplit";
}
els "HowToPlay" rn L;
}
}
...
int iX = -1;
        ...
        else if  ( ( iPage + 1 ) == eHowToPlay_N// No next page.
        ...
        void UIScene_HowToPlay::handleInput(int iPad, int key, bool repeat,
            ...
            case ACTION_MENU_X:
                if(p// Previous page
			int iPrevPage = ( int )( m_eCurrP// 4J Stu - Add back for future platforms#if 0// No What's New for the first PS4 and Xbox One builds
			if(true)
			{
        ...
        m_eCurrP// Turn on just what we need for this screen.
	SHowToPlayPageDef* pDef = &( gs_aPageDefs[ m_eC// Replace button identifiers in the text with actual button images.
	std::wstring replacedText = app.FormatHTMLString(m_iPad, app.GetString( pDef->m// 4J-PB - replace the title with the platform specific title, and the platform name//	replacedText = replaceAll(replacedText,L"{*TITLE_UPDATE_NAME*}",app.GetString(IDS_TITLE_UPDATE_NAME));
	replacedText = repla"{*KICK_PLAYER_DESCRIPTION*}",app.GetString(IDS_KICK_PL#ifdef _XBOX_ONE));

	replacedText = repla"{*PLATFORM_NAME*}"L,app.GetString(I#endifTFORM_NAME));

	replacedText = repla"{*BACK_BUTTON*}"t,L,app.GetString(IDS_BACK_BUTTON));
        replacedText = repla"{*DISABLES_ACHIEVEMENTS*}",app.GetString(IDS_HOST_OPTION_DISABLE// 4J-JEV: Temporary fix: LOC: Minecraft: XB1: KO: Font: Uncategorized: Squares appear instead of hyphens in FIREWORKS description
	if (!ui.UsingBitmapFont())
	{
                ...
                int lastIndex = 0;
                "\r\n" int index = finalText.find(L, lastIndex, 2);
                index != wstring "\r\n";
	...
	TelemetryManager->RecordMenuShown(m_iPad, eUIScene_HowToPlay, (ETel
#ifdef __PSVITA__ubMenuId) ePage);
#endif
