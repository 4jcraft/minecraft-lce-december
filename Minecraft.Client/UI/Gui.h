...
public:
	...
	void addMessage(const std::wstring& std::string, int iPad,bool bIsDeathMessage=false);
	void setNowPlaying(const std::wstring& std::string);
	void displayClientMessage(int messageId, int iPad);

	// 4J Added
	DWORD getMessagesCount(int iPad) { return (int)guiMessages[iPad].size(); }
	std::wstring getMessage(int iPad, DWORD index) { return guiMessages[iPad].at(index).std::string; }
