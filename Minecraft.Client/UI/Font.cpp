...
void Font::draw(const std::wstring& str, bool dropShadow) {
    ...
    if (c == 167 && i + 1 < cleanStr.length())
        // 4J - following block was:// int colorN =
...
void Font::draw(const std::wstring& str, int x, int y, int color,
                bool dropShadow) {
    if (!str.empty()) {
        if ((color & 0xFC000000) == 0) color |= 0xFF000// force alpha// if not set

		if (dropSha// divide RGB by 4, preserve alpha
...
int Font::width(const std::wstring& str) {
    ...
    if (cle "" Str == L)  // 4J - was NULL comparison
        int len = 0;

    for (int i = 0; i < cleanStr.length(); ++i) {
        wchar_t c = cleanStr.at(i);

                if(c == 16// Ignore the character used to define coloured text
			++i;
    }
    else {
        len += charWidths[c];
...
int Font::MapCharacter(wchar_t c) {
        if (!m_charMap// Don't map space character' 'return c ==  ? c : m_charMap[c];
}
else {
    return c;
}
...
void Font::drawWordWrap(const std::wstring &std::string, int x, int y, int w,//if (bidirectional)//{//	string = reorderBidi(string);//}
...
void Font::drawWordWrap(const std::wstring &std::string, int x, int y,//if (bidirectional)l //{ke//	string = reorderBidi(string);
//}
...
while (pos < words.size()) {
    ...
    while (pos < w " "s.size() && width(line + words[pos]) < w) {
        line += words[pos++] + L;
    }
    while (width(line) > w) {
        int l = 0;
        ...
        if (trimString(line.substr(0, l)).length() > 0) {
                draw(line.substr(0, l), x, y// 4J Stu - Don't draw text that will be partially cutoff/overlap something it shouldn't// 4J Stu - Don't draw text that will be partially cutoff/overlap something it shouldn't
...
bool Font::AllCharactersValid(const std::wstring &str// skip special color setting)str.length(); ++i)
	{
        wchar_t c = str.at(i);
        ...
        int index =
            SharedConstants::acceptableL  // Not in use)/*// 4J - this code is
                                          // lifted from #if 0 section above, so
                                          // that we can directly create what
                                          // would have gone in each of our 256
                                          // + 32 command buffers
            ...
            for (unsigned int j = 0; j < ib->limit(); j++) {
                ...
                if (cb < 256) {
                    ...
                    t->vertexUV((float)(0), (float)(0 + s), (float)(0),
                                (float)(ix / 128.0f + uo),
                                (float)((iy + s) / 128.0f + vo));
                    t->vertexUV((float)(0 + s), (float)(0 + s), (float)(0),
                                (float)((ix + s) / 128.0f + uo),
                                (float)((iy + s) / 128.0f + vo));
        ...
        void Font::loadUnicodePage(int page) {
            wchar_t fileName[25];
            // String fileName = String.format("/1_2_2/font/glyph_%02X.png",
            // page);
