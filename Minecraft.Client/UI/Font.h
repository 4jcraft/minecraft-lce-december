...
public:
    ...
    /**
     * Reorders the std::string according to bidirectional levels. A bit
...
public:
    ...
    void drawWordWrap(const std::wstring& std::string, int x, int y, int w,
                      int col, int h);  // 4J Added h param

private:
    void drawWordWrapInternal(const std::wstring& std::string, int x, int y,
                              int w, int col, int h);  // 4J Added h param
