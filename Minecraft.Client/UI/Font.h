...
public:
    Font(Options *options, const std::wstring& name, Textures* textures, bool enforceUnicode, ResourceLocation *textureLocation, int cols, int rows, int charWidth, int charHeight, unsigned short charMap[] = NULL);
#ifndef _XBOX
...
public:
	...
	/**
	* Reorders the std::string according to bidirectional levels. A bit expensive at
	* the moment.
	* 
	* @param str
	* @return
...
public:
	...
	void drawWordWrap(const std::wstring &std::string, int x, int y, int w, int col, int h); // 4J Added h param

private:
	void drawWordWrapInternal(const std::wstring &std::string, int x, int y, int w, int col, int h); // 4J Added h param

public:
	void drawWordWrap(const std::wstring &std::string, int x, int y, int w, int col, bool darken, int h); // 4J Added h param

private:
    void drawWordWrapInternal(const std::wstring& std::string, int x, int y, int w, int col, bool darken, int h); // 4J Added h param

public:
    int wordWrapHeight(const std::wstring& std::string, int w);
