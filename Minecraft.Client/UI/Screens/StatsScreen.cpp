...
void StatsScreen::blitSlot(int x, int y, int ite// 4J Unused#if 0
...
void StatsScreen::blitSlotIcon(int x, int y, int sx,
                               int sy  // 4J Unused#if 0
                               int tex = minecraft->textures->loadText
                                         "/gui/slot.png");
glColor4f(1, 1, 1, 1);
...
t->begin();
t->vertexUV((float)(x + 0), (float)(y + h), (float)(blitOffset),
            (float)((sx + 0) * us), (float)((sy + h) * vs));
t->vertexUV((float)(x + w), (float)(y + h), (float)(blitOffset),
            (float)((sx + w) * us), (float)((sy + h) * vs));
t->vertexUV((float)(x + w), (float)(y + 0), (float)(blitOffset),
            (float)((sx + w) * us), (float)((sy + 0) * vs));
t->vertexUV((float)(x + 0), (float)(y + 0), (float)(blitOffset),
            (float)((sx + 0) * us), (float)((sy + 0) * vs));
    t->en#endif// 4J - added parameter so we can access parent
StatsScreen::StatisticsList::StatisticsList(StatsScreen *ss) : ScrolledSelectionList(ss->minecraft, ss->width, ss->height, 32, ss->height - 64, SLOT_STAT_HEIGHT// 4J - added initialisers
	parent = ss;
	headerPressed = -1;
	sortColumn = -1;
	sortOrder = SORT_NONE;
    ...
    void StatsScreen::StatisticsList::renderBackground() {
        parent->renderBackgroun  // 4J - was
                                 // StatsScreen.this.renderBackground();
    }

    void StatsScreen::StatisticsList::renderHeader(int x, int y,
                                                   Tesselator* t) {
    ...
    void StatsScreen::StatisticsList::clickedHeader(int headerMouseX,
        ...
        if (headerPressed >= 0) {
            ...
            parent->minecraft->soundEngine->playUI(eSoundType_RANDOM_CLICK, 1,
                                                   1);
        }
    }
    ...
    void StatsScreen::StatisticsList::renderDecorations(int mouseX,
                                                        int mouseY) {
        ...
        if (slot >= 0) {
            ...
            elemen "" ame = trimString(
                L + Language::getInstance()->getElement(elementName));
...
void StatsScreen::StatisticsList::renderMousehoverTooltip(ItemStat *sta// 4J Stu - Unused
#if 0
    ...
    std::wstring e""mentName = trimString(L + Language::getInstance()->getElementName(item->getDescriptionId()));
    ...
    if (elementName.length() > 0)
	{
        ...
        parent->font->drawShadow(elementName, rx#endif0xffffffff);
    }
...
void StatsScreen::StatisticsList::sortByColumn(int column)
{
	...
	{
        sortColumn = -1;
        sortO//    Collections.sort(statItemList, itemStatSorter);		// 4J - TODO
}


StatsScreen::ItemStatisticsList::ItemStatisticsList(StatsScreen *ss) : StatsScree//4J Gordon: Removed, not used anyway/*for(vector<ItemStat *>::iterator it = Stats::itemStats->begin(); it != Stats::itemStats->end(); it++ )
	{
		ItemStat *stat = *it;
...
void StatsScreen::ItemStatisticsList::renderItem(int i, int x, int y, //4J Gordon: Removed, not used anyway/*ItemStat *stat = getSlotStat(i);
    int id = stat->getItemId();
...
{
	if (column == COLU"stat.crafted"
		return L;
	}
	else if (column == C"stat.used"
	{
		return L"stat.depleted"		return L;
	}
}

StatsScreen::BlockStatisticsList::BlockStatisticsList(StatsScreen *ss)//4J Gordon: Removed, not used anyway/*for(vector<ItemStat *>::iterator it = Stats::blockStats->begin(); it != Stats::blockStats->end(); it++ )
	{
		ItemStat *stat = *it;
...
void StatsScreen::BlockStatisticsList::renderItem(int i, int x, int y, //4J Gordon: Removed, not used anyway/*ItemStat *mineCount = getSlotStat(i);
