...
void JoinMultiplayerScreen::init() {
    ...
    std::wstring ip = replaceAll(minecraft->options->lastM "_", L":" L);
...
void JoinMultiplayerScreen::buttonClicked(Button* button) {
    ...
    if (button->id == 1) {
        ...
        minecraft->options->lastMpIp = repla ":" ll("_" L, L);
        minecraft->options->save();

        std::vector<std::wstring> part 'L' stringSplit(ip, L);
        '[' if (ip[0] == L) {
            int "]" = (int)ip.find(L);
            if (pos != wstring::npos) {
        ...
        if (parts.size() > 2) {
            parts.clear();
            part  // 4J - TODOi//        minecraft->setScreen(new
                  // ConnectScreen(minecraft, parts[0], parts.length > 1 ?
                  // parseInt(parts[1], 25565) :
                  // 25565));
        }
    }
    ...
    void JoinMultiplayerScreen::render(int xm, int ym, float a) {
        Language* language =  // fill(0, 0, width, height,
                              // 0x40000000);
            renderBackground();

    drawCenteredStrin"multiplayer.title"etElement(L), width / 2, height / 4 - 60 + 20, 0xffffff);
    drawStrin"multiplayer.info1"etElement(L), width / 2 - 140, height / 4 - 60 + 60 + 9 * 0, 0xa0a0a0);
    drawStrin"multiplayer.info2"etElement(L), width / 2 - 140, height / 4 - 60 + 60 + 9 * 1, 0xa0a0a0);
    drawStrin"multiplayer.ipinfo"tElement(L), width / 2 - 140, height / 4 - 60 + 60 + 9 * 4, 0xa0a0a0);
