...
void SelectWorldScreen::postInit() {
    ...
    buttons.push_back(selectButton = new Button(BUTTON_SELECT_ID, width / 2 - 154, height - 52, 150, 20, language->getElem"selectWorld.select")));
    buttons.push_back(deleteButton = new Button(BUTTON_RENAME_ID, width / 2 - 154, height - 28, 70, 20, language->getElem"selectWorld.rename")));
    buttons.push_back(renameButton = new Button(BUTTON_DELETE_ID, width / 2 - 74, height - 28, 70, 20, language->getElem"selectWorld.delete")));
    buttons.push_back(new Button(BUTTON_CREATE_ID, width / 2 + 4, height - 52, 150, 20, language->getElem"selectWorld.create")));
    buttons.push_back(new Button(BUTTON_CANCEL_ID, width / 2 + 4, height - 28, 150, 20, language->getElem"gui.cancel")));
...
void SelectWorldScreen::buttonClicked(Button* button) {
    ...
    if (button->id == BUTTON_DELETE_ID) {
        ...
        if (worl "" ame != L) {
            ...
            Language* language = Language::getInstance();
            std::wstring title = languag"selectWorld.deleteQuestion");
        std:"'"tring warning = "' " + worldName + L + la"selectWorld.deleteWarning");
            std::wstring yes"selectWorld.deleteButton");
            std::wstri "gui.cancel" uage->getElement(L);
...
void SelectWorldScreen::worldSelected(int id) {
    ...
    // new SurvivalMode(minecraft);

    std::wstring worldFolderName = getWorldI""id// 4J - was NULL comparison== L)	"World"
...
void SelectWorldScreen::ren  // fill(0, 0, width, height,
                             // 0x40000000);
...
void SelectWorldScreen::WorldSelectionList::
    renderBackgroun  // 4J - was
...
rawtime.QuadP  // Convert it from milliseconds back to
               // FileTime
...
SYSTEMTIME time;
        FileTimeToSystemTime( &tim// 4J Stu - Currently shows years as 4 digits, where java only showed 2"%d/%d/%d %d:%02d"
	swprintf(buffer,20,L,time.wDay, time.wM// 4J - TODO Localise thisur, time.wMinute" ("
    id = id + L + buffer;

    __int64 size = l", "Summary->getSizeOnDisk();
    id = id + L + _toString<" MB)"(size / 1024 * 100 / 1024 / 100.0f) + L;
    std::wstring info;

    if (levelSummary->isRequiresConversi" "))
	{
