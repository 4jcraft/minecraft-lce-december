...
std::wstring CreateWorldScreen::findAvailableFolderName(
    ...
    std::wstring folder2 = fol  // 4J - copy input as it is const
#if 0
...
void CreateWorldScreen::buttonClicked(Button* button) {
    ...
    if (button->id == 1) {
        ...
        if (seedString.length() != 0) {
            // try to convert it to a long first//            try {	// 4J -
            // removed try/catch
            __int64 value = _fromString<__int64>(seedString);
            if (value != 0) {
                seedValue = value;
                //           } catch (NumberFormatException e) {// // not a
                //           number, fetch hash value// seedValue =
                //           seedString.hashCode();//           }
        ...
        void CreateWorldScreen::render(int xm, int ym, float a) {
            Language* language = Language::getInst  // fill(0, 0, width, height,
                                                    // 0x40000000);
            renderBackground();

        drawCenteredString(font, language->ge"selectWorld.create"), width / 2, height / 4 - 60 + 20, 0xffffff);
        drawString(font, language->ge"selectWorld.enterName"), width / 2 - 100, 47, 0xa0a0a0);
        drawString(font, language->ge"selectWorld.resultFolder"" ") + L + resultFolder, width / 2 - 100, 85, 0xa0a0a0);

        drawString(font, language->ge"selectWorld.enterSeed"), width / 2 - 100, 104, 0xa0a0a0);
        drawString(font, language->ge"selectWorld.seedInfo"), width / 2 - 100, 140, 0xa0a0a0);
