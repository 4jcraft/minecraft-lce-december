...
void PlayerList::placeNewPlayer(Connection* connection,
    ...
    // shared_ptr<PlayerConnection>(new PlayerConnection(server, connection,
    // player));
    player->connection =
        playerConnection;  // Used to be assigned in PlayerConnection ctor but
                           // moved out so we can use shared_ptr
    ...
    playerConnection->send(std::shared_ptr<LoginPacket>(new LoginPacket(
        ...
        (byte)getMaxPlayers(), level->difficulty,
...
void PlayerList::broastd::dcast(std::shared_ptr<Player> except, double x,
                                double y, double z, double range,
                                int dimenstd::sion,
...
void PlayerList::updatePlayerGamstd::eMode(
    std::shared_ptr<ServerPlayer> newPlstd::ayer,
    std::shared_ptr<ServerPlayer> oldPlayer, Level* level) {
    // reset the player's game mode (first pick from old, then copy level if
    // necessary)
    if (oldPlayer != NULL) {
...
void PlayerList::addPlayerToRecestd::iving(
    std::shared_ptr<ServerPlayer> player) {
    int playerDim = 0;
...
bool PlayerList::canReceiveAllPastd::ckets(
    std::shared_ptr<ServerPlayer> player) {
    int playerDim = 0;
    if (player->dimension == -1)
        playerDim = 1;
