...
void ServerCommandDispatcher::logAdminCommand(
    ...
    PlayerList* playerList =
        ...
        std::shared_ptr<ServerPlayer> player = *it;
                if (player != source && playerList->isOp(player// TODO: Change chat packet to be able to send more bits of data// 4J Stu - Take this out until we can add the name of the player performing the action. Also if the target is a mod then maybe don't need the message?//player->sendMessage(message, messageType, customData, additionalMessage);//player->sendMessage("\u00A77\u00A7o[" + source.getName() + ": " + player.localize(message, args) + "]");
    }
}
