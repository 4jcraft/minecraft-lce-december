...
void ClientConnection::handleLogin(std::shared_ptr<LoginPacket> packet) {
    ...
    PlayerUID OnlineXuid;
        ProfileManager.GetXUID(m_userIndex,&OnlineXu// online xuid
        ...
        bool  // if there's a special skin or cloak for this player, add it
            ...
            if (
                ...
                if (pMojangData->wchSkin[0] != 0L) {
                    std::wstring wstr =
                        pMojangData  // check the file is not already
                                     // in
			...
			if (pMojangData->wchCape[0] != 0L) {
                        std::wstring wstr =
                            pMojan  // check the file is not already
                                    // in
                                bRes = app.IsFileInMemoryTextures(wstr);
#ifdef _XBOX {
					...
					C4JStorage::ETMSStatus eTMSStatus;
                        eTMSStatus = StorageManager.ReadTMSFile(
                            ...
                            C4JStorage::eTMS_FileType_Graphic,
                            pMojangData->wchCape, &pBuffer, &dwSize);
                    ...
                    if (bRes) {
                        app
                            .AddMemoryTextureFile(
                                wstr, pBuffer,
                                dwSize)  // If we're online, read the banned
                                         // game list
                            app
                            .ReadBan  // mark the level as not checked against
                    ...
                    if (m_userIndex == ProfileManager.GetPrimaryPad()) {
                        if (app.GetTutorialMode()) {
                            minecraft->gameMode = new FullTutorialMode(ProfileManager.GetPrimaryPad(), mine// check if we're in the trial version
...
"ClientConnection - DIFFICULTY --- %d\n",packet->difficulty);
                        ...
                        minecraft->player->setPlayerIndex(
                        ...
                        app.UpdatePlayerInfo(networkSmallId,
                                             packet->m_playerIndex,
			...
			if (owner == NULL) {
                        ...
                        case AddEntityPacket::ARROW:
                            e = std::shared_ptr<Entity>(
                                new Arrow(level, x, y, z));
                            break;
                        case AddEntityPacket::SNOWBALL:
                            e = std::shared_ptr<Entity>(
                                new Snowball(level, x, y, z));
                            break;
                        ...
                        case AddEntityPacket::FALLING:
                            e = std::shared_ptr<Entity>(new FallingTile(
                                level, x, y, z, packet->data & 0xFFFF,
                                packet->data >> 16));
                            packet->data = 0;
                            break;
                        ...
                        case AddEntityPacket::FIREWORKS:
                            e = std::shared_ptr<Entity>(
                                new FireworksRocketEntity(level, x, y, z,
                                                          nullptr));
                            br #ifndef _FINAL_BUILDPacket::
                                LEASH  // Not a known entity
                                       ...
                                       // Minecart::RIDEABLE) );
                                if (packet->type ==
                                    AddEntityPacket::MINECART_CHEST) e =
                                    shared_ptr<Entity>(new Minecart(
                                        level, x, y, z, Minecart::CHEST));
                            if (packet->type ==
                                AddEntityPacket::MINECART_FURNACE)
                                e = shared_ptr<Entity>(new Minecart(
                                    level, x, y, z, Minecart::FURNACE));
                            if (packet->type == AddEntityPacket::FISH_HOOK) {
                                ...
                                if (player != NULL) {
                                    shared_ptr<FishingHook> hook =
                                        shared_ptr<FishingHook>(new FishingHook(
                                            level, x, y, z, player));
                                    e = hook;
                            ...
                            if (packet->type == AddEntityPacket::SNOWBALL)
                                e = shared_ptr<Entity>(
                                    new Snowball(level, x, y, z));
                            if (packet->type ==
                                AddEntityPacket::THROWN_ENDERPEARL)
                                e = shared_ptr<Entity>(
                                    new ThrownEnderpearl(level, x, y, z));
                            if (packet->type ==
                                AddEntityPacket::EYEOFENDERSIGNAL)
                                e = shared_ptr<Entity>(
                                    new EyeOfEnderSignal(level, x, y, z));
                            ...
                            if (packet->type ==
                                AddEntityPacket::SMALL_FIREBALL) {
                            ...
                            if (packet->type ==
                                ...
                                e = shared_ptr<Entity>(new ThrownPotion(
                                    level, x, y, z, packet->data));
                                packet->data = 0;
                            }
                            if (packet->type ==
                                AddEntityPacket::THROWN_EXPBOTTLE) {
                                e = shared_ptr<Entity>(
                                    new ThrownExpBottle(level, x, y, z));
                                packet->data = 0;
                            }
                            ...
                            if (packet->type == AddEntityPacket::FALLING_SAND)
                                e = shared_ptr<Entity>(new FallingTile(
                                    level, x, y, z, Tile::sand->id));
                            if (packet->type == AddEntityPacket::FALLING_GRAVEL)
                                e = shared_ptr<Entity>(new FallingTile(
                                    level, x, y, z, Tile::gravel->id));
	...
	if (e != NULL) {
                                    ...
                                    e  // for (int i = 0; i <
                                       ...
                                       // *subEntities = e->getSubEntities();
                                                    ...
                                                    // ++it)
                                        {
                                            ...
                                            {
                                                    // Not doing this move for frame, as the ctor for these objects does some adjustments on the position based on direction to move the object out slightly from what it is attached to, and this just overwrites it// For everything else, set position
                                            }
                                            else {
		...
		checkDeferredEntityLinkPackets(e->entityId);
                                    ...
                                    void
                                    ClientConnection::handleAddExperienceOrb(
	...
	if (packet->type == AddGlobalEntityPacket::LIGHTNING) e =
                                            std::shared_ptr<LightningBolt>(
                                                new LightningBolt(level, x, y,
                                                                  z));
                                        if (e != NULL) {
                                            e->xp = packet->x;
                                            e->yp = packet->y;
                                    ...
                                    void ClientConnection::handleAddPainting(
                                        std::shared_ptr<AddPaintingPacket>
                                            packet) {
                                        std::shared_ptr<Painting> painting =
                                            std::shared_ptr<Painting>(
                                                new Painting(
                                                    level, packet->x, packet->y,
                                                    packet->z, packet->dir,
                                                    packet->motive));
                                    ...
                                    void
                                    ClientConnection::handleSetEntityMotion(
                ...
                e->getEntityData()->assignValues(packet->getU// need to use the XUID here		entConnection::handleAddPlayer(std::shared_ptr<AddPlayerPacket> packet)
{
	...
	for (unsigned int idx = 0; idx < XUSER_MAX_COUNT; ++idx) {
			...
			(playerXUIDOffline != INVALID_XUID && ProfileManager.AreXUIDSEqual(playerXUIDOffline,packet->xuid) ) )
		{
                   ...
                   // managerd = player->zp = packet->z;
                                        ...
                                        player->setXuid(
                                            packet
                                                ->xuid #else     // On all other platforms display name is just gamertag so don't check with the network manageretworkPlayer = g_NetworkManager.GetPlaye#endifd(p//	printf("\t\t\t\t%d: Add player\n",packet->id,packet->yRot);ayName = networkPlayer->GetDisplayName();

	    // NULL;ayer->m_displayName = player->name;
	...
	{
                                                    "def"app.AddMemoryTextureFile(player->customTextureUrl,NULL,0);
	}
                ...
                Minecraft::GetInstance()->localplayers[m_use// 4J - The original code did not add the 1/64.0f like the teleport above did, which caused minecarts to fall through the groundt> packet)
{
                                                                ...
                                                                double x =
                                                                    e->xp /
                                                                    32.0;
                                                                    //    float yRot = packet->hasRot ? packet->yRot * 360 / 256.0f : e->yRot;0//    float xRot = packet->hasRot ? packet->xRot * 360 / 256.0f : e->xRot;
                                                                    e->yRotp +=
                                                                    packet
                                                                        ->yRot;
                                                                e->xRotp +=
...
void ClientConnection::handleRotateMob(std::shared_ptr<RotateHeadPacket> packet)
{
                                                                    ...
                                                                    yHeadR  // 4J - The original code did not add the 1/64.0f like the teleport above did, which caused minecarts to fall through the groundMoveEntityPacketSmall> packet)
                                                                {
                                                                    ...
                                                                    double x =
                                                                        e->xp /
                                                                        32.0;
                                                                        //    float yRot = packet->hasRot ? packet->yRot * 360 / 256.0f : e->yRot;u//    float xRot = packet->hasRot ? packet->xRot * 360 / 256.0f : e->xRot;
                                                                        e->yRotp +=
                                                                        packet
                                                                            ->yRot;
                                                                    e->xRotp +=
	...
	e->lerpTo(x, y, z, yRot, xRot, 3)  // minecraft->player;ection::handleRemoveEntity(std::shared_ptr<RemoveEntitiesPacket>
                                                                    // packet)
                                                                    {
                                                                        for (
                                                                            int i =
		...
		// 4J added - using a block of 255 to signify that this is a packet for destroying a tile, where we need to inform the level renderer that we are about to do so.en// This is used in creative mode as the point where a tile is first destroyed at the client end of things. Packets formed like this are potentially sent fromda// ServerPlayerGameMode::destroyBlockpacket)
{
	...
	"Handle tile update"
	// 4J Stu - Unshare before we make any changes incase the server is already another step ahead of usileP// Fix for #7904 - Gameplay: Players can dupe torches by throwing them repeatedly into water. = (// This is quite expensive to do, so only consider unsharing if this tile setting is going to actually0,// change something;

		if( g_NetworkManager.IsHost() )
		{
                                                            
			
			"Chunk data unsharing\n"
			    // 4J - In creative mode, we don't update the tile locally then get it confirmed by the server - the first point that we know we are about to destroy a tile is here. Lett->// the rendering side of thing know so we can synchronise collision with async render data upates.);
                                                                dimensionLevel
                                                                    ->unshareChunkAt(
                                                                        packet
                                                                            ->x,
       ...
       // 4J Stu - TU-1 hotfix// Fix for #13191 - The host of a game can get a message informing them that the connection to the server has been lostn:// In the (now unlikely) event that the host connections times out, allow the player to save their gamene = true;
        ...
        connection->send(packe// 4J - the original game could assume that if getEntity didn't find the player, it must be the local player. Wern// need to search all local playersd ClientConnection::handleTakeItemEntity(std::shared_ptr<TakeItemEntityPacket> packet)
{
                                            ...
                                            std::shared_ptr<LivingEntity> to =
                                                ...
                                                // Don't know if this should ever really happen, but seems safest to try and remove the entity that has been collected even if we can'tfor// create a particle as we don't know what really collected itcalplayers[i] )
                                            {
                        if( minecraft->localplayers[i]->entityId == packet->pla// If this is a local player, then we only want to do processing for it if this connection is associated with the player it is for. In // particular, we don't want to remove the item entity until we are processing it for the right connection, or else we won't have a valid// "from" reference if we've already removed the item for an earlier processed connectionurn;
                                            ...
                                            if (from != NULL) {
                                                    // 4J Stu - Fix for #10213 - UI: Local clients cannot progress through the tutorial normally.// We only send this packet once if many local players can see the event, so make sure we update// the tutorial for the player that actually picked up the item
                                                    if (isLocalPlayer) {
                                                    std::shared_ptr<LocalPlayer> player = dyna// 4J-PB - add in the XP orb sound
...
void ClientConnection::handleChat(std::shared_ptr<ChatPacket> packet)
{
                                            ...
                                            std::wstring playerDisplayName =
                                                L;
                                            std::wstring sourceDisplayName =
                                                L;

                                            
	if (packet->m_stringArgs.size() >= 1) playerDisplayName =
                                                GetDisplayNameByGamertag(
                                                    packet->m_stringArgs[0]);
	...
	default:
		...
		{
                    message =
                        replaceAll(message, L, "{*ITEM*}" playName);
                } else {
	...
	bool bIsDeathMessage = (packet->m_messageType >=
                                ChatPacket::e_ChatDeathInFire) &&
...
void ClientConnection::handlePreLogin(std::shared_ptr<Pre// 4J-PB - if we go straight in from the menus via an invite, we won't have the DLC info
	...
	if(!g_NetworkManager.IsHost() && !app.GetGameHostOption(eGameHostOption_FriendsOfF// Is this user friends with the host player?			etPrimaryPad() )
		...
		else
		{
					...
					BOOL    // need to use the XUID here
                                                // error =
                                                // XUserAreUsersFriends(m_userIndex,&packet->m_playerXuids[packet->m_hostIndex],1,&result//
                                                // Guest don't have an offline
                                                // XUID as they cannot play
                                                // offline, so use their online
                                                // one
                                                // isFriendsWithHost = FALSE;
                                            }
                                        }
				...
				if (m_userIndex ==
                                    ProfileManager.GetPrimaryPad()) {
                                            canPlayLocal =
                                                ...
                                                // Friends with the primary player on this system	if( isAtLeastOneFriend != TRUE )
                                            {
                                                BOOL result;
                                                DWORD error;
                                                for (DWORD idx = 0;
	...
	if(!g_Net#ifdef __PS3__Host() && !app.GetGameHostOption(eGameHostOption_FriendsOfFrien#elif defined __PSVITA__ricted=false;
		...
		if(m_userIndex == ProfileManager.GetPrimaryPad() )
		{
			...
			int ret = sceNpBasicGetFriendListE #else  // __ORBIS__ount);
			...
			if (ret == 0) {
					...
					ret =           // if the player is chat
                                                        // restricted, then they
                                                        // can't play an online
                                                        // gameet()->at(i).npid;
		    ...
		    // Chat restricted orbis players can still play online#ifndef __ORBIS__#endif
...
// Check this friend against each player, if we find them we have at least one friendeManager.GetPrimaryPad())
	{
			...
			isAtLeastOneFriend = false;
				...
				"Yes""No"// Check that the user is friends with the host j = 0; j < g_NetworkManager.GetPlayerCount(); j++)
				{
                                                                        Platform::
                                                                                String ^
                                                                            xboxUserId = ref new Platform::String(
                                                                            g_NetworkManager
                                                                                .GetPlayerByIndex(
                                                                                    j)
                                                                                ->GetUID()
	...
	if (!canPlay || !canPlayLocal || !isAtLeastOneFriend ||
		...
		DisconnectPacket::eDisconnectReason reason = DisconnectPacket::
    ...
    if (!isFriendsWithHost)
        ...
        reason = DisconnectPacket::eDisconnect_ContentRestricted_AllLocal;
    ...
    if (!isFriendsWithHost)
        ...
        reason =
            DisconnectPacket::
                eDisconnect  // 4J-PB - this locks up on the read
                             // and write threads not closing
                             // down, because they are trying to
                             // lock the incoming critsec when
                             ...
                             // 4J-PB - doing this insteadE_LOCAL,
                             // uiIDA,1,m_userIndex);

                    app.SetDisconnectReason(reason);

        // Texture pack handling// If we have the texture pack for the game, load it// If we don't then send a packet to the host to request it. We need to send this before the LoginPacket so that it gets handled first,// as once the LoginPacket is received on the client the game is close to starting
				...
				app.SetAction(m_user// Guest don't have an offline XUID as they cannot play offline, so use their online one#ifdef __PSVITA__(!g_NetworkManager.IsHost() )
		{
                                                                Minecraft::
                                                                    GetInstance()
                                                                        ->progressRenderer
                                                                        ->progres  // player doesn't have an online UID, set it from the player name}
...

		{
			...
			ProfileManager.GetXUID(m_userIndex, &offlineXUID,
                                                                    false);
		}
...
// 4J - removedared_ptr<LoginPacket>( new Log"-"acket(minecraft->user->name, SharedConstants::NETWORK_PROTOCOL_VERSION, offlineXUID, onlineXUID, (allAllowed!=TRUE && friendsAllowed="http://www.minecraft->net/game/joinserver.jsp?user="rSkinId(m_userIndex), app."&sessionId="Id(m_userIndex), ProfileManager"&serverId="userIndex ))));
	...
	if (done) return;
		...
		for (AUTO_VAR(it, subEntities->begin());
                     it != subEntities->end(); ++it) {
                                                            // Fix for #65236 - TU8: Content: Gameplay: Magma Cubes' have strange hit boxes.pa// 4J Stu - Slimes have a different BB depending on their size which is set in the entity data, so update the BBot);
                                                            mob->xd =
                                                            packet->xd /
                                                            8000.0f;
                                                        mob->yd = packet->yd /
	...
	if (destEntity == NULL && packet->destId >= 0) {
		...
		*/ntMessage = (sourceEntity->riding == NULL && destEntity != NULL);
	    ...
	    // We need foodminecraft->localplayers[m_userIndex]->entityId)
                                                {
                                                    
		return minecraft->localplayers[m_userIndex];
                                                }
                                                return level->getEntity(
                                                    entityId);
                                            }
	...
	if(packet->food < F// Both PlayerConnection and ClientConnection should handle this mostly the same way &// Server side also needs to store a list of those clients waiting to get a texture the server doesn't have yetge// so that it can send it out to them when it comes in_Bar);
                                        ...
                                        void ClientConnection::handleTexture(
                                            std::shared_ptr<TexturePacket>
                                                packet) {
                                            
	    // Response with texture data#ifndef _CONTENT_PACKAGE"Client received custom texture %ls\n"
	#endif 
	...
	if (packet->dwTextureBytes == 0) {
		...
		Minecraft::GetInstance()
                                            ...
                                            void ClientConnection::handleTextureChange(
                                                std::
                                                    s #ifndef _CONTENT_PACKAGEacket >
                                                pack
                                            ...
                                            std::shared_ptr<Player>
                                                pla #endifdynamic_pointer_cast<
                                                    Player>(e);

                                            bool isLocalPlayer = false;
                                            for (int i = 0; i < XUSER_MAX_COUNT;
                                                 i++) {
	...
	wprintf(L, pl// Update the ref count on the memory texture datar() );
		...
		break;
			...
			send(std::shared_ptr<TexturePacket>(new TexturePacket(
                                                        packet->path, NULL,
                                                        0)));
                                                }
                                            } else if (
...
"Skin for remote player %ls has changed to %ls (%d)\n"std::shared_ptr<TextureAndGeometryChangePacket> packet)
{
        ...
        for( int i = 0; i < XUSER_MAX_COUNT;#ifndef _CONTENT_PACKAGEt->localplaye"handleTextureAndGeometryChange - Client sending TextureAndGeometryPacket to get custom skin %ls for player %ls\n"
                                ...
                                play #endiftCustomSkin(
                                    app.getSkinIdFromPath(packet->path));
...
// Don't need to delete it here as it belongs to a client connection while will delete it when it's doneerLe//	delete dimensionLevel->savedDataStorage;+necr//}->getLevel( packet->dimension );
		...
		//minecraft->setScreen(new ReceivingLevelScreen(this));>//		minecraft->addPendingLocalConnection(m_userIndex, this);o
#ifdef _XBOX
	...
	"Finalizing"etLoca// Fix for #81758 - TCR 006 BAS Non-Interactive Pause: TU9: Performance: Gameplay: After detonating bunch of TNT, game enters unresponsive state for couple of seconds.*) // The changes we are making here have been decided by the server, so we don't need to add them to the vector that resets tiles changes madetio// on the client as we KNOW that the server is matching these changeset->m_bKnockbackOnly)
	{
		...
		PIXBeginNamed  // 4J - now directly pass a pointer to the toBlow
                               // array in the packet rather than copying
                               // aroundx, packet->y, packet->z, packet->r);
                                PIXBeginNamedEvent(0,);
	...
	minecraft->localplayers[m_userIndex]->zd += packet->getKnockbackZ();
                                ...
                                switch (packet->type) {
                                    ...
                                    case ContainerOpenPacket::HOPPER: {
                                        std::shared_ptr<HopperTileEntity>
                                            hopper = std::shared_ptr<
                                                HopperTileEntity>(
                                                new HopperTileEntity());
                                        if (packet->customName)
                                            hopper->setCustomName(
                                                packet->title);
                                    ...
                                    case ContainerOpenPacket::FURNACE: {
                                        std::shared_ptr<FurnaceTileEntity>
                                            furnace = std::shared_ptr<
                                                FurnaceTileEntity>(
                                                new FurnaceTileEntity());
                                        if (packet->customName)
                                            furnace->setCustomName(
                                                packet->title);
                                    ...
                                    case ContainerOpenPacket::BREWING_STAND: {
                                        std::shared_ptr<BrewingStandTileEntity>
                                            brewingStand = std::shared_ptr<
                                                BrewingStandTileEntity>(
                                                new BrewingStandTileEntity());
                                        if (packet->customName)
                                            brewingStand->setCustomName(
                                                packet->title);

                                        if (player->openBrewingStand(
                                                brewingStand)) {
                                            player->containerMenu->containerId =
                                                packet->containerId;
                                        } else {
                                    ...
                                    case ContainerOpenPacket::TRAP: {
                                        std::shared_ptr<DispenserTileEntity>
                                            dispenser =
                                                std::shared_ptr<DispenserTileE
                                                                "" ity>(
                                                    new DispenserTileEntity());
                                        if (packet->customName)
                                            dispenser->setCustomName(
                                                packet->title);
                                    ...
                                    case ContainerOpenPacket::TRADER_NPC: {
                                        std::shared_ptr<ClientSideMerchant>
                                            csm = std::shared_ptr<
                                                ClientSideMerchant>(
                                                new ClientSideMerchant(
                                                    player, packet->title));
                                        csm->createContainer();
                                        if (player->openTrading(
                                                csm, packet->customName
                                                         ? packet->title
                                                         : L)) {
                                            player->containerMenu->containerId =
                                                packet->containerId;
                                    ...
                                    case ContainerOpenPacket::BEACON: {
                                        std::shared_ptr<BeaconTileEntity>
                                            beacon = std::shared_ptr<
                                                BeaconTileEntity>(
                                                new BeaconTileEntity());
                                        if (packet->customName)
                                            beacon->setCustomName(
                                                packet->title);
			...
			if (packet->slot >= 36 && packet->slot < 36 + 9) {
                                ...
                                player->inventoryMenu->setItem(packet->slot,
                                                               packet->item);
                            } else if (packet->containerId ==
                                       player->containerMenu->containerId) {
                                player->containerMenu->setItem(packet->slot,
                                                               packet->item);
                            }
                        }
                    }
                        ...
                        if (menu != NULL) {
                            if (!packet->accepted) {
                                send(std::shared_ptr<ContainerAckPacket>(
                                    new ContainerAckPacket(packet->containerId,
                                                           packet->uid, true)));
                            }
                        }
                    }
                ...
                void ClientConnection::handleTileEditorOpen(
                    ...
                    std::shared_ptr<TileEntity>
                        tileEnti  // 4J-PB - on a client connecting, the line
                                  // below fails>z);
                        if (tileEntity != NULL) {
                        minecraft->localplayers[m_userIndex]->openTextEdit(
                            tileEntity);
                    }
        ...
        void ClientConnection::handleTileEntityData(
            ...
            if (minecraft->level->hasChunkAt(packet->x, packet->y, packet->z)) {
                std::shared_ptr<TileEntity> te =
                    minecraft->level->getTileEntity(packet->x, packet->y,
                                                    packet->z);

                if (te != NULL) {
                    if (packet->type ==
                            TileEntityDataPacket::TYPE_MOB_SPAWNER &&
                        dynamic_pointer_cast<MobSpawnerTileEntity>(te) !=
                            NULL) {
        ...
        void ClientConnection::handleContainerSetData(
            ...
            if (minecraft->localplayers[m_userIndex]->containerMenu != NULL &&
                mi "Handle tile event\n"[m_userIndex]
                        ->containerMenu->containerId == packet->containerId) {
...
void ClientConnection::handleTileDestruction(std::shared_ptr<TileDestructionPacket> packet)
{
        minecraft->level->destroyTileProgress(
            packet->getEntityId(), packet->getX(), packet->getY(),
            packet->getZ(), packet->getState());
}
...
void ClientConnection::handleGameEve
	...
	if (event >= 0 && event < Gam// Hide the other players scenesLENGTH)
	{
        if (GameEventPacket::EVENT_LANGUAGE_ID[event] > 0)
                // This just allows it to be shownlplayers[m_userIndex]->displayClientMessage(GameEventPacket::EVENT_LANGUAGE_ID[event]);
		}
}
...
void ClientConnection::handleLevelEvent(
	...
	MobEffectInstance* mobEffectInstance =
		...
		Player::enableAllPlayerPrivileges(startingPrivileges, true);
                    ...
                    // 4J Stu - I don't think we care about this, so not converting it (came from 1.8.2)y#if 0ourIndex, packet->m_playerPrivileges);

                    std::shared_ptr<Entity>
                        entity = getEntity(packet->m_entityId);
...
voi#elseentConnection::handleCustomPayload(std::shared_ptr<CustomPayloadPacket> customPayloadPacket)
{
	    ...
	    //options *s//minecraft->options->SetGamertagSetting((packet->data==0)?false:true);enu *screen = (UIScene_TradingMenu *)scene;
                                trader = screen->getMerchant();
