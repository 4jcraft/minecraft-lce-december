...
void PlayerChunkMap::PlayerChunk::add(std::shared_ptr<ServerPlayer> player, bool sendPa/*= true*///app.DebugPrintf("--- Adding player to chunk x=%d\tz=%d\n",x, z);
    if (std::find(players.begin(),players.end(),player) != players.en// 4J-PB - At the start of the game, lots of chunks are added, and we can then move into an area that is outside the diameter of our starting area, // but is inside the area loaded at the start.
		app.De"--- Adding player to chunk x=%d\t z=%d, but they are already in there!\n",pos.x, pos.z);
		r//assert(false);// 4J - was			throw new IllegalStateException("Failed to add player. " + player + " already is in chunk " + x + ", " + z);
}

player->seenChunks
    .inser  // 4J Added the sendPacket check. See PlayerChunkMap::add for the
            // usage
    if (sendPacket)
...
void PlayerChunkMap::PlayerChunk::remove(std::shared_ptr<ServerPlayer> player) {
    PlayerChunkMap::PlayerChunk*
        toDe  // app.DebugPrintf("--- PlayerChunkMap::PlayerChunk::remove
              // x=%d\tz=%d\n",x,z);
            AUTO_VAR(it, std::find(players.begin(), players.end(), player));
    if (it == players.end())
        "--- INFO - Removing player from chunk x=%d\t z=%d, but they are not in that chunk!\n",pos.x, pos.z);
...
if (players.size() == 0) {
    ...
    if (it != parent->chunks.end()) {
        // Don't delete until the end of the function, as this might be this
        // instance
...
playe  // 4J - I don't think there's any point sending these anymore, as we
       // don't need to unload chunks with fixed sized maps// 4J - We do need
       // to send these to unload entities in chunks when players are dead. If
       // we do not and the entity is removed// while they are dead, that
       // entity will remain in the clients world
    ...
    if (thisNetPlayer != NULL) {
        ...
        // wprintf(L"Sending ChunkVisiblity packet false for chunk (%d,%d) to
        // player %ls\n", x, z, player->name.c_str()
        // );
                                player->connection->send( std::shared_ptr<ChunkVisibilityPacket>( new ChunkVisibilityPacket(pos.x, po//app.DebugPrintf("PlayerChunkMap::PlayerChunk::remove - QNetPlayer is NULL\n");
    }
}
...
void PlayerChunkMap::PlayerChunk::tileChanged(int x, int y, int z) {
    ...
    if (changes < MAX_CHANGES_BEFORE_RESEND) {
        ...
        // 4J added - make sure that any tile updates for the chunk at this
        ...
        void PlayerChunkMap::PlayerChunk::broadcast(
            std::shared_ptr<Packet> packet) {
            std::vector<std::shared_ptr<ServerPlayer> > sentTo;
            for (unsigned int i = 0; i < players.size(); i++) {
                // 4J - don't send to a player we've already sent this data to
                // that shares the same machine. TileUpdatePacket,//
                // ChunkTilesUpdatePacket and SignUpdatePacket all used to limit
                // themselves to sending once to each machine// by only
                // sending to the primary player on each machine. This was
                // causing trouble for split screen// as updates were only
                // coming in for the region round this one player. Now these
                // packets can be sent to any// player, but we try to
                // restrict the network impact this has by not resending to the
                // one
                // machine
                bool dontSend = false;
                if (sentTo.size()) {
                    ...
                    if (thisPlayer == NULL) {
                        ...
                        for (unsigned int j = 0; j < sentTo.size(); j++) {
                            ...
                            if (otherPlayer != NULL &&
                                ...
                                dontSend = true;
                                // 4J Changed to get the flag index for the
                                // player before we send a packet. This flag is
                                // updated when we queue// for send the first
                                // BlockRegionUpdatePacket for this chunk to
                                // that player/players system. Therefore there
                                // is no need to// send tile updates or other
                                // updates until that has been
                                // sent
                                int flagIndex =
                                    ServerPlayer::getFlagIndexForChunk(
                                        pos, parent->dimension);
                                if (player->seenChunks.find(pos) !=
                                        player->seenChunks.end() &&
                                    (player->connection->isLocal() ||
                                     g_NetworkManager.SystemFlagGet(
                                         player->connection->getNetworkPlayer(),
                                         flagIndex))) {
                                    player->connection
                                        ->  // Now also check round all the
                                            // players that are involved in this
                                            // game. We also want to send the
                                            // packet// to them if their
                                            // system hasn't received it
                                            // already, but they have received
                                            // the first BlockRegionUpdatePacket
                                            // for this// chunk// Make sure
                                            // we are only doing this for
                                            // BlockRegionUpdatePacket,
                                            // ChunkTilesUpdatePacket and
                                            // TileUpdatePacket.// We'll be
                                            // potentially sending to players
                                            // who aren't on the same level as
                                            // this packet is intended for,//
                                            // and only these 3 packets have so
                                            // far been updated to be able to
	...
	if (!((packet->getId() == 51) || (packet->getId() == 52) ||
              (packet->getId() == 53))) {
                                    ...
                                    for (int i = 0;
                                         i < parent->level->getServer()
                                                 ->getPlayers()
                                                 ->players.size();
                                         i++) {
                std::shared_ptr<ServerPlayer> // Don't worry about local players, they get all their updates through sharing level with the server anyway
		if ( player->connection == NUL// Don't worry about this player if they haven't had this chunk yet (this flag will be the// same for all players on the same system)
		
		int flagIndex = ServerPlayer::getFlagIndexForChunk(pos,parent->dimension);
		if(!g_NetworkManager.SystemFlagGet// From here on the same rules as in the loop above - don't send it if we've already sent to the same system
		bool dontSend = false;
		if( sentTo.size() )
		{
                                            INetworkPlayer* thisPlayer =
            ...
            broadcast(level->getTileEntity(x, y,// 4J added, to allow limiting of region update packets createdESEND)
	{
		...
		if (!prioritised) {
                                                ...
                                                int zp =
                                                    zChangeMin + pos.z * 16;
                                                int xs =
                                                    xChangeMax - xChangeMin + 1;
                                                int ys =
                                                    yCh  // Fix for buf #95007 :
                                                         // TCR #001 BAS Game
                                                         // Stability: TU12:
                                                         // Code: Compliance:
                                                         // More than 192
                                                         // dropped items causes
                                                         // game to freeze or
                                                         // crash.// Block
                                                         // region update
                                                         // packets can only
	...
	return didRegionUpdate;
                                        ...
                                        void
                                        PlayerChunkMap::PlayerChunk::broadcast(
                                        ...
                                        PlayerChunkMap::PlayerChunkMap(
                                            ServerLevel * level, int dimension,
                                            int radius) {
                                        ...
                                        void PlayerChunkMap::tick() {
                                            ...
                                            if (time - lastInhabitedUpdate >
                                                Level::TICKS_PER_DAY / 3) {
                                                lastInhabitedUpdate = time;

        for (// 4J Stu - Going to let our changeChunks handler below deal with thishunk = knownC//chunk.broadcastChanges();
 // 4J - some changes here so that we only send one region update per tick. The chunks themselves also// limit their resend rate to once every MIN_TICKS_BETWEEN_REGION_UPDATE ticks
...
void PlayerChunkMap::getChunkAndRemovePlayer(int x, int z,
                                                                ...
                                                                __int64 id =
                                                                    ...
                                                                    ((z +
                                                                      0x7fffffffLL)
                                                                     << 32)  // 4J - added - actually create & add player to a playerchunk, if there is one queued for this player.
                                                                    void
                                                                        Pla  // Find the nearest chunk request to the playerrverPlayer> player)
                                                                {
                                                                    if (addRequests
                                                                            .size()) {
                                                                                ...
                                                                                int distSq =
                                                                                    ...
                                                                                    (zm -
                                                                                     pz) *
                                                                                        (zm -
                                                                                         pz);
                                if( ( minDistSq == -1 // If we found one at all, then do this onenDistSq = distSq;
					itNearest = it;
                                                                            }
                                                                        }
                                                                    }
    ...
    for (int legSize = 1; legSize <= size * 2; legSize++) {
                                                                        for (
                                                                            ...
                                                                            for (
                                                                                ...
                                                                                // Final legetChunkAndAddPlayer(targetX, targetZ, player);
                                                                            }
                                                                        }
                                                                    }
                                                                }
	...
	for (AUTO_VAR(it, addRequests.begin()); it != addRequests.end();) {
                                                                    if (it->player ==
                                                                        player) {
                                                                        it =
                                                                            addRequest  // If the distance between x and xc;
                                                                    }
                                                                }
                                                            }
