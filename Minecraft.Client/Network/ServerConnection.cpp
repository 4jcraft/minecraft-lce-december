...
void ServerConnec  // MGH - changed this so that the the CS lock doesn't cover
                   // the tick (was causing a lockup when 2 players tried to
                   // join)
    EnterCriticalSection(&pending_cs);
...
for (unsigned int i = 0; i < tempPending.size(); i++) {
    std::shared_ptr <
        Pendin  //        try {	// 4J - removed try/catch// }
                //        catch (Exception e) {
...
bool ServerConnection::addPendingTextureRequest(
    ...
    if (it == m_pendingTextureRequests.end()) {
        m_pendingTextureReq  // 4J Stu - We want to request this texture from
                             // everyone, if we have a duplicate it's most
                             // likely because the first person we asked for it
                             // didn't have it// eg They selected a skin then
                             // deleted the skin pack. The side effect of this
                             // change is that in certain cases we can send a
                             // few more requests, and receive// a few more
                             // responses if people join with the same skin in a
                             // short space of
                             // time
            return true;
    }

    void ServerConnection::handleTextureReceived(
        const std::wstring& textureName) {
        AUTO_VAR(it, std::find(m_pendingTextureRequests.begin(),
                               m_pendingTextureRequests.end(), textureName));
        if (it != m_pendingTextureRequests.end()) {
