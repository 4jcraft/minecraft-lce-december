...
public:
    ...
    virtual LevelChunk* create(int x, int z);
        LevelChunk *create(int x, int z, bool asyncPostProces// 4J added
    virtual LevelChunk *getChunk(int x, int#ifdef _LARGE_WORLDS	
	LevelChunk *getChunkLoadedOrUnloaded(int x, int // 4J added
	...
	void updateOverwriteHellChunk(LevelChunk* origChunk, LevelChunk* playerChunk, int xMin, int xMax, int zMin, int zM
#endif 
	virtual LevelChunk** \
getCache(){return cache  // 4J added// 4J-JEV Added; Remove chunk from the
                         // toDrop queue.#ifdef _LARGE_WORLDS	
	void dontDrop(int x, int #endif
...
private:
	...
	void updatePostProcessFlag(short flag, int x, int z, int xo, int zo, LevelChunk *// 4J added
...
priv#ifdef _LARGE_WORLDS
	static const int MAX_SAVES =#else// 4J Stu - Was 24, but lowering it drastically so that we can trickle save chunks
	static const int MAX_SAVES #endif
