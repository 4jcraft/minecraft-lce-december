...
private:
    ...
    CRITICAL_SECTION m_closeP /* 4J - removed
 ...
 File banFile, ipBanFile, opFile, whiteListFile;
    ...
    int sendAllPlayer  // 4J Added to maintain which players in which dimensions
                       // can receive all packet types
...
public:
        void validatePlayerSpawnPosition(std::shared_ptr<ServerPlayer>// 4J Added
    void add(std::shared_ptr<ServerPlayer> player);
    void std::move(std::shared_ptr<ServerPlayer> player);
    void std::remove(std::shared_ptr<ServerPlayer> player);
	...
	void repositionAcrossDimension(std::shared_ptr<Entity> entity, int lastDimension, ServerLevel *oldLevel, ServerLevel *newLevel);
    void tick();
	bool isTrackingTile(int x, int y, int // 4J addednsion);		
	void prioritiseTileChanges(int x, int y, int// 4J addedension);	
...
public:
	...
	bool isOp(std::shared_ptr<Ser// 4J Addedplayer); 
	...
	std::vector<ServerPlayer> *getPlayers(Pos *position, int rangeMin, int rangeMax, int count, int mode, int levelMin, int levelMax, std::unordered_map<std::wstring, int> *scoreRequirements, const std::wstring &playerName,	const std::wstring &teamName, Level *level);

private:
	bool meetsScoreRequirements(std::shared_ptr<Player> player, std::unordered_map<std::wstring, int> scoreRequirements);
...
public:
    ...
    void broadcast(std::shared_ptr<Player> except, double x, double y, double z, double range, int dimension, std::shared_pt// 4J Added ProgressListener *progressListener param and bDeleteGuestMaps param
    void saveAll(ProgressListener *progressListener, bool bDeleteGuestMaps = false);
    void whiteList(const std::wstring& playerName);
    void blackList(const std::ws//    Set<String> getWhiteList();		/ 4J removed
	...
	int getPlayerCount();
	int getPlayerCount(S// 4J Added*level); 
	int getMaxPlayers();
...
public:
	void setAllowCheatsForAllPlayers(boo// 4J Addedands);

	
	void kickPlayerByShortId(BYTE networkSmallId);
	void closePlayerConnectionBySmallId(BYTE networkSmallId);
	bool isXuidBanne// AP added for Vita so the range can be increased once the level starts
