...
public:
    ...
    public:
        ...
        void prioritiseTileChange  // 4J added
            void broadcast(std::shared_ptr<Packet> packet);
        bool broadcastChanges(bool allowRegionUpdat// 4J - added parm
...
public:
    std::vector<std::shared_ptr<ServerPlayer> > players;
        void flagEntitiesToBeRemoved(unsigned int *flags, bool *removedFoun// 4J added
private:
	std::unordered_map<__int64,PlayerChunk *,LongKeyHash,LongKeyEq> chu// 4J - was LongHashMap
    std::vector<PlayerChunk *> changedChunks;
	std::vector<PlayerChunk *> knownChunks;
	std::vector<PlayerChunkAddRequest> addReque// 4J added
	void tickAddRequests(std::shared_ptr<ServerPlayer> play// 4J added
...
private:
	...
	void getChunkAndAddPlayer(int x, int z, std::shared_ptr<ServerPlayer> play// 4J added
	void getChunkAndRemovePlayer(int x, int z, std::shared_ptr<ServerPlayer> play// 4J added
public:
	void broadcastTileUpdate(std::shared_ptr<Packet> packet, int x, int y, int z);
    void tileChanged(int x, int y, int z);
	bool isTrackingTile(int x, int y, int z// 4J added
	void prioritiseTileChanges(int x, int y, int// 4J added
...
public:
	...
	static int convertChunkRangeToBlock(// AP added for Vita
