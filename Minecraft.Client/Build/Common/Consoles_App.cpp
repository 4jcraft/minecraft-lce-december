...
bool CMinecraftApp::LoadFireworksMenu(int iPad,std::shared_ptr<LocalPlayer> player, int x, int y, int z)
{
	bool success = true;
...
bool CMinecraftApp::LoadEnchantingMenu(int iPad,std::shared_ptr<Inventory> inventory, int x, int y, int z, Level *level, const std::wstring &name)
{
	bool success = true;
...
bool CMinecraftApp::LoadTradingMenu(int iPad, std::shared_ptr<Inventory> inventory, std::shared_ptr<Merchant> trader, Level *level, const std::wstring &name)
{
	bool success = true;
...
bool CMinecraftApp::LoadHopperMenu(int iPad ,std::shared_ptr<Inventory> inventory, std::shared_ptr<HopperTileEntity> hopper)
{
	bool success = true;
...
bool CMinecraftApp::LoadHopperMenu(int iPad ,std::shared_ptr<Inventory> inventory, std::shared_ptr<MinecartHopper> hopper)
{
	bool success = true;
...
bool CMinecraftApp::LoadHorseMenu(int iPad ,std::shared_ptr<Inventory> inventory, std::shared_ptr<Container> container, std::shared_ptr<EntityHorse> horse)
{
	bool success = true;
...
bool CMinecraftApp::LoadBeaconMenu(int iPad ,std::shared_ptr<Inventory> inventory, std::shared_ptr<BeaconTileEntity> beacon)
{
	bool success = true;
...
std::wstring CMinecraftApp::toStringOptionsStatus(const C4JStorage::eOptionsCallback &eStatus)
{
#ifndef _CONTENT_PACKAGE
	switch(eStatus)
	{
	...
	for(int i=0;i<XUSER_MAX_COUNT;i++)
	{
		...
		{			
			...
			case eAppAction_ExitWorld:
				...
				// 4J-PB - cancel any possible std::string verifications queued with LIVE
...
std::wstring CMinecraftApp::GetActionReplacement(int iPad, unsigned char ucAction)
{
	...
	swprintf(std::string,128,L"<img src=\"%ls\" align=\"middle\" height=\"%d\" width=\"%d\"/>", replacement.c_str(), size, size);
...
std::wstring CMinecraftApp::GetIconReplacement(unsigned int uiIcon)
{
	...
	swprintf(std::string,128,L"<img src=\"Icon_Shank\" align=\"middle\" height=\"%d\" width=\"%d\"/>", size, size);
			...
			if((pDLC->eDLCType==e_DLC_TexturePacks) || (pDLC->eDLCType==e_DLC_MashupPacks))
			{
				...
				{		
					...
					{			
						...
						if(!bPresent)
						{	
							// this may already be present in the std::vector because of a previous trial/full offer
...
std::wstring CMinecraftApp::getFilePath(DWORD packId, std::wstring filename, bool bAddDataFolder, std::wstring mountPoint)
{
	std::wstring path = getRootPath(packId, true, bAddDataFolder, mountPoint) + filename;
	File f(path);
	if(f.exists())
	{
		return path;
...
std::wstring CMinecraftApp::getRootPath(DWORD packId, bool allowOverride, bool bAddDataFolder, std::wstring mountPoint)
{
