    ...
    if (val == stats.end()) {
        ...
        if (stat != GenericStats::timePlayed())
            app.DebugPr
                ...
                UINT_MAX  // If value is larger than USHRT_MAX and is not
                          // designated as large, cap it to USHRT_MAX
                if (val->second.stats[difficulty] > USHRT_MAX &&
                    !isLargeStat(stat)) val->second.stats[difficulty] =
                    USHRT_MAX;
    }

    requiresSave =
        tru  // If this stat is on a leaderboard, mark that leaderboard as
             // needing updated
...
bool StatsCounter::canTake(Achievement *ach//4J Gordon: Remove achievement dependencies, always able to take
	return true;
}
...
void StatsCounter::tick(int player) {
    ...
    if (requiresSave && saveCounter == 0)
                save(player// 4J-JEV, we don't want to write leaderboards in the middle of a game.// EDIT: Yes we do, people were not ending their games properly and not updating scores.// #ifndef __PS3__
	if( flushCounter > 0 )
	{
            --flushCounter;
            if (flushCounter == 0) flushLeaderboards()  // #endif
}

void StatsCounter::clear(// clear out the stats when someone signs out
	stats.clear();
}

void StatsCounter::parse(void* dat#ifndef _DURANGO// 4J-PB - If this is the trial game, let's just make sure all the stats are empty// 4J-PB - removing - someone can have the full game, and then remove it and go back to the trial// 	if(!ProfileManager.IsFullVersion())// 	{// 		stats.clear();// 		return;// 	}//Check that we don't already have any stats
	assert( stats.size() == 0 //Pointer to current position in stat array
	PBYTE pbData=(PBYTE)data;
	pbData+=sizeof(GAME_SETTINGS);
	unsigned short* statData = (unsigned short*)pb//data + (STAT_DATA_OFFSET/sizeof(unsigned short));//Value being read
	StatContainer newVa//For each stat
...
// 4J-PB - If this is the trial game, don't save any stats
	if(!ProfileManager.IsFullVersion()//Check we're going to have enough room to store all possible stats
	...
	assert( uiTotalStatsSize <= (CConsoleMinecraftApp::GAME_DEFINED_PROFILE_DATA_BYTES-sizeof(G//Retrieve the data pointer from the profile#if ( defined __PS3__ || defined __ORBIS__ || defined _DURANGO || defined __PSVITA__ )
	PBYTE pbData = (PBYTE)StorageManager.GetGameDefinedP#elseeData(player);

	PBYTE pbData = (PBYTE)ProfileManager.GetGameDefinedP#endifData(player);

	pbData+=sizeof//Pointer to current position in stat array//unsigned short* statData = (unsigned short*)data + (STAT_DATA_OFFSET/sizeof(unsigned short));
	unsigned short* statData = (unsign//Reset all the data to 0 (we're going to replace it with the map data)
	memset(statData, 0, CConsoleMinecraftApp::GAME_DEFINED_PROFILE_DATA_BYTES-sizeof//For each stat);

	
	StatsMap::iterator val;
	std::vector<Stat *>::iterator end = Stats::all->end();
	for( std::vector<Stat *>::iterator iter = Stats::all->begin() ; iter != //If the stat is in the map write out it's value
		val = stats.find(*iter);
		if( !(*iter)->isAchievement() )
		{
    if (!isLargeStat(*iter)) {
...
void StatsCounter::setLeaderboardProperty(XUSER_PROPERTY* prop, DWORD id, unsigned int value)"Setting property id: %d to value %d\n", id, value);
prop->dwPropertyId = id;
prop->value.type = XUSER_DATA_TYPE_INT32;
prop->value.nData = (int)  // Just making sure we clamp the unsigned int to the
...
// 4J-PB - If this is the trial game, no writing
	if(!ProfileManager.IsFullVersion(//unsigned int locale = XGetLocale();
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_KILLS_EASY\n");
    ...
    scores[viewCount].m_commentData.m_kills.m_spider =
        getValue(Stats::killsSpider, eDifficulty_Easy);
                scores[viewCount].m_commentData.m_kills.m_spiderJockey = getValue(Stats::killsSpiderJockey,	e//scores[viewCount].m_commentData.m_kills.m_zombiePigman = getValue(Stats::killsZombiePigman,		eDifficulty_Easy);
		...
		scores[viewCount].m_score = scores[viewCount].m_commentData.m_kills.m_zombie
								  ...
								  + scores[viewCount].m_commentData.m_kills.m_creeper
								  + scores[viewCount].m_commentData.m_kills.m_spider
								  + scores[viewCount].m_commentData.m_kills.m_sp// + getValue(Stats::killsZombiePigman,			eDifficulty_Easy);
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_KILLS_NORMAL\n");
    ...
    scores[viewCount].m_commentData.m_kills.m_spider =
        getValue(Stats::killsSpider, eDifficulty_Normal);
                scores[viewCount].m_commentData.m_kills.m_spiderJockey = getValue(Stats::killsSpiderJockey,	eDi//scores[viewCount].m_commentData.m_kills.m_zombiePigman = getValue(Stats::killsZombiePigman,		eDifficulty_Normal);
		...
		scores[viewCount].m_score = scores[viewCount].m_commentData.m_kills.m_zombie
								  ...
								  + scores[viewCount].m_commentData.m_kills.m_creeper
								  + scores[viewCount].m_commentData.m_kills.m_spider
								  + scores[viewCount].m_commentData.m_kills.m_sp// + getValue(Stats::killsZombiePigman,			eDifficulty_Normal);
								  + scores[viewCount].m_commentData.m_kills.m_zombiePigman
								  + scores[viewCount].m_commentData.m_kills.m_slime;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_KILLS_HARD\n");
    ...
    scores[viewCount].m_commentData.m_kills.m_spider =
        getValue(Stats::killsSpider, eDifficulty_Hard);
                scores[viewCount].m_commentData.m_kills.m_spiderJockey = getValue(Stats::killsSpiderJockey,	e//scores[viewCount].m_commentData.m_kills.m_zombiePigman = getValue(Stats::killsZombiePigman,		eDifficulty_Hard);
		...
		scores[viewCount].m_score = scores[viewCount].m_commentData.m_kills.m_zombie
								  ...
								  + scores[viewCount].m_commentData.m_kills.m_creeper
								  + scores[viewCount].m_commentData.m_kills.m_spider
								  + scores[viewCount].m_commentData.m_kills.m_sp// + getValue(Stats::killsZombiePigman,			eDifficulty_Hard);
								  + scores[viewCount].m_commentData.m_kills.m_zombiePigman
								  + scores[viewCount].m_commentData.m_kills.m_slime;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_MININGBLOCKS_PEACEFUL\n");
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_MININGBLOCKS_EASY\n");
    scores[viewCount].m_difficulty = 1;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_MININGBLOCKS_NORMAL\n");
    scores[viewCount].m_difficulty = 2;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_MININGBLOCKS_HARD\n");
    scores[viewCount].m_difficulty = 3;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_FARMING_PEACEFUL\n");
    scores[viewCount].m_difficulty = 0;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_FARMING_EASY\n");
    scores[viewCount].m_difficulty = 1;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_FARMING_NORMAL\n");
    scores[viewCount].m_difficulty = 2;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_FARMING_HARD\n");
    scores[viewCount].m_difficulty = 3;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_TRAVELLING_PEACEFUL\n");
    scores[viewCount].m_difficulty = 0;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_TRAVELLING_EASY\n");
    scores[viewCount].m_difficulty = 1;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_TRAVELLING_NORMAL\n");
    scores[viewCount].m_difficulty = 2;
	...
	{
    scores[viewCount].m_iPad = iPad;"Updating leaderboard view LEADERBOARD_TRAVELLING_HARD\n");
    scores[viewCount].m_difficulty = 3;
...
if (viewCount > 0) {
	...
	XUSER_PROPERTY trave// arcade leaderboard

	if( modifiedBoards & LEADERBOARD_KILLS_EASY )
	"Updating leaderboard view LEADERBOARD_KILLS_EASY\n");
		...
		setLeaderboardProperty( &killsEasyProperties[LeaderboardManager::eProperty_Kills_SpiderJockey	], PROPERTY_KILLS_SPIDERJOCKEY,		getValue(Stats::killsSpiderJockey,	eDi// 4J-PB - this could overflow//setLeaderboardProperty( &killsEasyProperties[LeaderboardManager::eProperty_Kills_ZombiePigman	], PROPERTY_KILLS_ZOMBIEPIGMAN,		getValue(Stats::killsZombiePigman,	eDifficulty_Easy) + getValue(Stats::killsNetherZombiePigman,	eDifficulty_Easy) );
	...
	if( modifiedBoards & LEADERBOARD_KILLS_NORMAL )
	"Updating leaderboard view LEADERBOARD_KILLS_NORMAL\n");
		...
		setLeaderboardProperty( &killsNormalProperties[LeaderboardManager::eProperty_Kills_SpiderJockey	], PROPERTY_KILLS_SPIDERJOCKEY,	getValue(Stats::killsSpiderJockey,	eDiff// 4J-PB - this could overflow
		ULONGLONG ulTemp=getValue(Stats::killsZombiePigman,	eDifficulty_Normal) + getValue(Stats::killsNetherZombiePigman,	eDi//setLeaderboardProperty( &killsNormalProperties[LeaderboardManager::eProperty_Kills_ZombiePigman	], PROPERTY_KILLS_ZOMBIEPIGMAN,	getValue(Stats::killsZombiePigman,	eDifficulty_Normal) + getValue(Stats::killsNetherZombiePigman,	eDifficulty_Normal) );
	...
	if( modifiedBoards & LEADERBOARD_KILLS_HARD )
	"Updating leaderboard view LEADERBOARD_KILLS_HARD\n");
		...
		setLeaderboardProperty( &killsHardProperties[LeaderboardManager::eProperty_Kills_SpiderJockey	], PROPERTY_KILLS_SPIDERJOCKEY,	getValue(Stats::killsSpiderJockey,	eDi// 4J-PB - this could overflow
		ULONGLONG ulTemp=getValue(Stats::killsZombiePigman,	eDifficulty_Hard) + getValue(Stats::killsNetherZombiePigman,	e//setLeaderboardProperty( &killsHardProperties[LeaderboardManager::eProperty_Kills_ZombiePigman	], PROPERTY_KILLS_ZOMBIEPIGMAN,	getValue(Stats::killsZombiePigman,	eDifficulty_Hard) + getValue(Stats::killsNetherZombiePigman,	eDifficulty_Hard) );
	...
	if( modifiedBoards & LEADERBOARD_FARMING_EASY )
	"Updating leaderboard view LEADERBOARD_FARMING_EASY\n");
	...
	if( modifiedBoards & LEADERBOARD_FARMING_HARD )
	"Updating leaderboard view LEADERBOARD_FARMING_HARD\n");
	...
	if( viewCount > 0 )
	{
		...
		{"Successfully wrote %d leadeboard views\n",#endif // Xbox	#endif // ndef _DURANGO
}

void StatsCounter::#ifndef _DURANGO)
{
	...
	statBoards.insert( std::make_pair(Stats::blocksMined[Tile::wheat_Id], LEADERBOARD_FARMING_PEACEFUL) );
	statBoards.insert( std::make_pair(Stats::blocksMined[Tile::mushroom_brown_Id], LEADERBOARD_FARMING_PEACEFUL) );
	...
	statBoards.insert( std::make_pair(#endif:minecartOneM, LEADERBOARD_TRAVELLING_PEACEFUL) )#ifndef _DURANGOsert( std::make_pair(Stats::boatOneM, LEADERBOARD_TRAVELLING_PEACEFUL) );
...
bool StatsCounter::isLargeStat(Stat* stat)
{
	...
	Stat*** end#endifRGE_STATS[LARGE_STATS_COUNT];
...
{
	std::vector<Stat*>::iterator statsEnd = "%ls\t\t%u\t%u\t%u\t%u\n"std::vector<Stat*>::iterator statsIter = Stats::all->begin() ; statsIter!=statsEnd ; ++statsIter )
	{
		app.DebugPrintf(,
			(*sta
#ifdef _DEBUGc_  // To clear leaderboards set DEBUG_ENABLE_CLEAR_LEADERBOARDS to
                 // 1 and set DEBUG_CLEAR_LEADERBOARDS to be the bitmask of what
                 // you want to clear//Leaderboards are updated on game exit so
                 // enter and exit a level to trigger the clear//#define
                 // DEBUG_CLEAR_LEADERBOARDS
                 // (LEADERBOARD_KILLS_EASY | LEADERBOARD_KILLS_NORMAL |
                 // LEADERBOARD_KILLS_HARD)#define DEBUG_CLEAR_LEADERBOARDS
                 // (0xFFFFFFFF)#define
                 // DEBUG_ENABLE_CLEAR_LEADERBOARDS
...
void StatsCounter::WipeLeaderboards()
{
	...
	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGBLOCKS_NORMAL )	XUserResetStatsViewAllUsers(STATS_VIEW_MINING_BLOCKS_N// 	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGORE_PEACEFUL )		XUserResetStatsViewAllUsers(STATS_VIEW_MINING_ORE_PEACEFUL, NULL);D// 	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGORE_EASY )			XUserResetStatsViewAllUsers(STATS_VIEW_MINING_ORE_EASY, NULL);// 	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGORE_NORMAL )		XUserResetStatsViewAllUsers(STATS_VIEW_MINING_ORE_NORMAL, NULL);// 	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGORE_HARD )			XUserResetStatsViewAllUsers(STATS_VIEW_MINING_ORE_HARD, NULL);
	...
	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_TRAVELLING_NORMAL )		XUserResetStatsViewAllUsers(STATS_VIEW_TRAVELL// 	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_NETHER_PEACEFUL )		XUserResetStatsViewAllUsers(STATS_VIEW_NETHER_PEACEFUL, NULL);I// 	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_NETHER_EASY )			XUserResetStatsViewAllUsers(STATS_VIEW_NETHER_EASY, NULL);// 	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_NETHER_NORMAL )			XUserResetStatsViewAllUsers(STATS_VIEW_NETHER_NORMAL, NULL);// 	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_NETHER_HARD )			XUserResetStatsViewAllUsers(STATS_VIEW_NETHER_HARD, NULL);

	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_TRAVELLING_TOTAL )		XUserResetStatsViewAllUsers(STATS_VIEW_TRAVE#endif	OTA#endifL);
	if( LeaderboardManager::Instance()->OpenSession() )
	{
		writeStats();
		LeaderboardManager::Instance()->CloseSession();
