...
public:
	...
	bool isUnderSpawnProtection(Level *level, int x, int y, int z, std::shared_ptr<Player> player);
...
//    void addTickable(Tickable tickable);	// 4J removed
    ...
    void info(const std::wstring& std::string);
    void warn(const std::wstring& std::string);
