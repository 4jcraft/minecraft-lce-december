...
ServerScoreboard::getStartTrackingPackets(Objective* objective) {
    retur  // vector<shared_ptr<Packet> > *packets = new
           // vector<shared_ptr<Packet> >();//packets.push_back(
           // shared_ptr<SetObjectivePacket>( new SetObjectivePacket(objective,
           // SetObjectivePacket::METHOD_ADD)));//for (int slot = 0; slot <
           // DISPLAY_SLOTS; slot++)//{//	if (getDisplayObjective(slot) ==
           // objective) packets.push_back(
           // shared_ptr<SetDisplayObjectivePacket>( new
           // SetDisplayObjectivePacket(slot, objective)));//}//for (Score
           // score : getPlayerScores(objective))//{//
           // packets.push_back( shared_ptr<SetScorePacket>( new
           // SetScorePacket(score,
           // SetScorePacket::METHOD_CHANGE)));//}//return
           // packets;
}

void ServerScoreboard::startTrackingObjective(Objective *obje//vector<shared_ptr<Packet> > *packets = getStartTrackingPackets(objective);//for (ServerPlayer player : server.getPlayers().players)//{//	for (Packet packet : packets)//	{//		player.connection.send(packet);//	}//}	//trackedObjectives.push_back(objective);
}
