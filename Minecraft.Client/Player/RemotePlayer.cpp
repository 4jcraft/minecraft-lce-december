...
void RemotePlayer::tick() {
    ...
    if (!hasStartedUsingItem && isUsingItemFlag() &&
        ...
        stopUsingItem();
        hasStartedUsingItem = false;
        //        if (eatItem != null) {//            if (eatItemTickCount <=
        //        25 && eatItemTickCount % 4 == 0) {//
        //        spawnEatParticles(eatItem, 5);//            }//
        //        eatItemTickCount--;//            if (eatItemTickCount <= 0)
    ...
    void RemotePlayer::aiStep() {
        ...
        bob += (tBob - bob) * 0.4f;
        tilt += (tTilt - tilt) *
                0.8f;  // 4J Stu - Brought forward change from 1.3 to fix #64688
                       // - Customer Encountered: TU7: Content: Art: Aura of
                       // enchanted item is not displayed for other players in
                       // online game
        ...
        void RemotePlayer::animateRe  //        Player.animateRespawn(this,
                                      //        level);
    }
