...
ServerPlayer::ServerPlayer(MinecraftServer* server, Level* level,
    ...
    if (!level->dimension->hasCeiling &&
        ...
        int radius = std::max(
            5, server->getSpawnProtectionRadius() -
                   6)  // 4J added - do additional checking that we aren't
                       // putting the player in deep water. Give up after 20 or
                       // goes just// in case the spawnPos is somehow in a
                       // really bad spot and we would just lock here.
            int waterDepth = 0;
        int attemptCount = 0;
        int xx2, yy2, zz2;
...
footSize = 0;
        ...
        heightOffset // 4J - this height used to be set up after moveTo, but that ends up with the y value being incorrect as it depends on this offset
	this->moveTo(xx + 0.5, yy, zz + 0.5, 0, 0// 4J Handled later//while (!level->getCubes(this, bb).empty())//{
	//	setPos(x, y + 1, z);//}
	// m_UUID = name;// 4J Added
	lastBrupSendTickCount = 0;
        }

        ServerPlayer::
            ~ServerPlayer(  // delete [] lastCarried.data;// 4J added - add
                            // bits to a flag array that is passed in, to
                            // represent those entities which have small Ids,
                            // and are in our vector of entitiesToRemove.// If
                            // there aren't any entities to be flagged, this
                            // function does nothing. If there *are* entities to
                            // be added, uses the removedFound as an input to//
                            // determine if the flag array has already been
                            // initialised at all - if it has been, then just
                            // adds flags to it; if it hasn't, then memsets the
                            // output// flag array and adds to it for this
                            // ServerPlayer.
                void ServerPlayer::flagEntitiesToBeRemoved(unsigned int* flags,
                    ...
                    for (AUTO_VAR(it, entitiesToRemove.begin());
                         it != entitiesToRemove.end(); it++) {
                        int index = *it;
                        if (index < 2048) {
                ...
                void ServerPlayer::readAdditionalSaveData(
                    CompoundTag* entityTag) {
                    Player::readAdditionalSaveData(entityTag);

                    if (entityTag->conta "playerGameType"))
                    // 4J Stu - We do not want to change the game mode for the
                    // player, instead we let the server override it
                    // globally//if
                    // (MinecraftServer::getInstance()->getForceGameType())//{
                    // //
                ...
                if (entityTag->conta "GameRules") && grs != NULL) {
                byteArray ba = entityTag->getByteAr"GameRules");
                ByteArrayInputStream bais(ba);
                DataInputStream dis(&bais);
                grs->read(&dis);
                dis.close();
                bais.close(//delete [] ba.data;
        }
        }
        ...
        void ServerPlayer::addAdditonalSaveData(CompoundTag* entityTag) {
            ...
            if (grs != NULL) {
                ...
                grs->write(&dos);
                entityTag->putByteAr"GameRules", baos.buf);
                baos.buf.data = NULL;
                dos.close();
                baos.close();
                // 4J Stu - We do not want to change the game mode for the
                // player, instead we let the server override it
                // globally//entityTag->putInt(L"playerGameType",
        ...
        containerMenu->broadcastChanges(// 4J-JEV, hook for Durango event 'EnteredNewBiome'.
	...
	flushEntitiesToRemove()// 4J Stu - Split out here so that we can call this from other places
void ServerPlayer::flushEntitiesToRemove()
{
            while (!entitiesToRemove.empty()) {
                ...
                connection->send(std::shared_ptr<RemoveEntitiesPacket>(new RemoveEntitiesPacket// 4J - have split doTick into 3 bits, so that we can call the doChunkSendingTick separately, but still do the equivalent of what calling a full doTick used to do, by calling this method
void ServerPlayer::doTick(bool sendChunks, bool /*=false*/hunks, bo/*=false*/ortal)
{
...
void ServerPlayer::doTickA()
{
                    ...
                    for (unsigned int i = 0; i < inventory->getContainerSize();
                         i++) {
                        std::shared_ptr<ItemInstance> ie =
                            inventory->getItem(i);
                        if  // 4J - removed condition. These were getting lower
                            // priority than tile update packets etc. on the
                            // slow outbound queue, and so were extremely slow
                            // to send sometimes,// particularly at the
                            // start of a game. They don't typically seem to be
                            // massive and shouldn't be send when there isn't
                            // actually any updating to do.
                        if (Item::items[ie-// && connection->countDelayedPackets() <= 2)
			{
                                std::shared_ptr<Packet> packet =
                                    (dynamic_cast<ComplexItem*>(
                                         Item::items[ie->id])
                                         ->getUpdatePacket(
                                             ie, level,
                                             dynamic_pointer_cast<Player>(
                                                 shared_from_this())));
                                if (packet != NULL) {
                                        connection->send(packe// 4J - split off the chunk sending bit of the tick here from ::doTick so we can do this exactly once per player per server tick
void ServerPlayer::doChunkSendingTick(//	printf("[%d] %s: sendChunks: %d, empty: %d\n",tickCount, connection->getNetworkPlayer()->GetUID().getOnlineID(),sendChunks,chunksToSend.empty());
	if (!chunksToSend.empty())
	{
                                        ChunkPos nearest = chunksToSend.front();
                                        bool  // 4J - reinstated and optimised
                                              // some code that was commented
                                              // out in the original, to make
                                              // sure that we always// send
                                              // the nearest chunk to the
                                              // player. The original uses the
                                              // bukkit sorting thing to try and
                                              // avoid doing this, but// the
                                              // player can quickly wander away
                                              // from the centre of the spiral
                                              // of chunks that that method
                                              // creates, long before
                                        ...
                                        for (AUTO_VAR(it, chunksToSend.begin());
                                            ...
                                            ChunkPos chunk = *it;
                                            if (level->isChunkFinalised(
                                                    chunk.x, chunk.z)) {
                                                double newDist =
                                                    chunk.distanceToSqr(x, z);
                                                if ((!nearestValid) ||
                                                    (newDist < dist)) {
                                                    nearest = chunk;
                                                    dist = chunk.distanceToSqr(
                                                        x, z);
                                                    nearestValid =  //        if
                                                                    //        (nearest
                                                                    //        != NULL)		// 4J - removed as we don't have references here
                                                        if (nearestValid) {
                                                        bo  //                if
                                                            //                (dist
                                                            //                <
                                ...
                                bool canSendToPlayer = MinecraftServer::chunkPacketManagement_CanSendTo(connectio//				app.DebugPrintf(">>> %d\n", canSendToPlayer);//				if( connection->getNetworkPlayer() )//				{//					app.DebugPrintf("%d: canSendToPlayer %d, countDelayedPackets %d GetSendQueueSizeBytes %d done: %d\n",//						connection->getNetworkPlayer()->GetSmallId(),//						canSendToPlayer, connection->countDelayedPackets(),//						g_NetworkManager.GetHostPlayer()->GetSendQueueSizeMessages( NULL, true ),//						connection->done);//				}

		
				if( dontDelayChunks ||
#ifdef _XBOX_ONElayer  // The network manager on xbox one doesn't
                               // currently split data into slow & fast queues -
                               // since we can only measure// both
                               // together then bytes provides a better metric
                               // than count of data items to determine if we
                               // should avoid queueing too much
                               // up
					(g_NetworkManager.GetHostPlayer()->GetSendQueueSizeBytes(#elif defined _XBOX2 )&&

					(g_NetworkManager.GetHostPlayer()->GetSendQueueSizeMessag#elseULL, true ) < 4 )&&

					(connection->countDelayedPackets() < 4 )&&
					(g_NetworkManager.GetHostPlayer()->GetSendQueueSizeMessag#endif L, tru//(tickCount - lastBrupSendTickCount) > (connection->getNetworkPlayer()->GetCurrentRtt()>>4) &&
					!connection->done) )
				{
                                                                lastBrupSendTickCount =
                                                                    tickCount;
                                                                okToSend = true;
                                        MinecraftServer::chunkPacketManagement_DidSendTo(connectio//					static unordered_map<wstring,__int64> mapLastTime;//					__int64 thisTime = System::currentTimeMillis();//					__int64 lastTime = mapLastTime[connection->getNetworkPlayer()->GetUID().toString()];//					app.DebugPrintf(" - OK to send (%d ms since last)\n", thisTime - lastTime);//					mapLastTime[connection->getNetworkPlayer()->GetUID().toString()] = thisTime;
                                        //					app.DebugPrintf("
                                        //- <NOT
                                        // OK>\n");
				}
                                ...
                                bool chu// Don't send the chunk to the local machine - the chunks there are mapped directly to the server chunks. We could potentially stop this process earlier on by not adding// to the chunksToSend list, but that would stop the tile entities being broadcast too
				if( !// force here to disable sharing of data// Don't send the chunk if we've set a flag to say that we've already sent it to this machine. This stops two things// (1) Sending a chunk to multiple players doing split screen on one machine// (2) Sending a chunk that we've already sent as the player moves around. The original version of the game resends these, since it maintains//     a region of active chunks round each player in the "infinite" world, but in our finite world, we don't ever request that chunks be//     unloaded on the client and so just gradually build up more and more of the finite set of chunks as the player moves
					if( !g_NetworkManager.SystemFlagGet(connection->getNetworkPlayer(),f//						app.DebugPrintf("Creating BRUP for %d %d\n",nearest.x, nearest.z);
		"Creation BRUP for sending\n");
						...
						__int64 af//						app.DebugPrintf(">>><<< %d ms\n",after-before);
						PIXEndNamedEvent();
						if( dontDelayChunks ) packet->shouldDelay = false;

						if( packet->sh// Other than the first packet we always want these initial chunks to be sent over// QNet at a lower priority
							...
							nearest.x * 16 + 14, Level::maxBuildHeight - 2, neare// Don't send TileEntity data until we have sent the block data
                                                                    ...
                                                                    std::vector<std::shared_ptr<
                                                                        ...
                                                                        level->getTileEntitiesInRegion(
                                                                            ...
                                                                            nearest.z *
                                                                                    16 +
                                                                                16);
                                        for (unsigned i// 4J Stu - Added delay param to ensure that these arrive after the BRUPs from above// Fix for #9169 - ART : Sign text is replaced with the words �Awaiting approval�.
						broadcast(tes->at(i), !connection->isLocal() && !dontDelayChunks);
					}
					delete tes;
                                                            }
#ifndef _CONTENT_PACKAGErP  // check if there's a debug dimension change
                                    // requested//if(app.GetGameSettingsDebugMask(ProfileManager.GetPrimaryPad())&(1L<<eDebugSetting_GoToNether))//{//
...
bool S// 4J: Not relevant to console serversrc// Allow falldamage on dedicated pvpservers -- so people cannot cheat their way out of 'fall traps'//bool allowFallDamage = server->isPvpAllowed() && server->isDedicatedServer() && server->isPvpAllowed() && (dmgSource->msgId.compare(L"fall") == 0);
	if (!server->isPvpAllowed() && invulnerableTime > 0 && dmgSource != DamageSourc// 4J Stu - Fix for #46422 - TU5: Crash: Gameplay: Crash when being hit by a trap using a dispenser// getEntity returns the owner of projectiles, and this would never be the arrow. The owner is sometimes NULL.
...
bool ServerPlayer::canHarmPlastd::yer(std::shared_ptr<Player> target)
{
        if (!server-// 4J: Added for checking when only player name is provided (possible player isn't on server), e.g. can harm owned animals
bool ServerPlayer::canHarmPlastd::yer(std::wstring targetName)
{
                                                            bool canHarm = true;
...
void ServerPlay"Start win game\n"n(int i)
{
                                                            if (!connection
                                                                     ->hasClientTickedOnce())
                                                                return;
	...
	else
	{
                                                        ...
                                                        server->getPlayers()
                                                            ->to  // 4J Added
                                                                  // delay
                                                                  // parampointer_cast<ServerPlayer>(shared_from_this()),
                                                                  // i);
                                                                lastSentExp =
                                                            -1;
                                                        /*= false*/ alth = -1;
...
void ServerPlayer::rstd::ide(std::sh// 4J Removed this - The act of riding will be handled on the client and will change the positionne// of the player. If we also teleport it then we can end up with a repeating movements, e.g. bouncing// up and down after exiting a boat due to slight differences in position on the client and server//connection->teleport(x, y, z, yRot, xRot);
                                            ...
                                            void
                                            ServerPlayer::openTextEstd::dit(
                                                std::shared_ptr<TileEntity>
                                                    sign) {
                                                std::shared_ptr<SignTileEntity>
                                                    signTE =
                                                        dynamic_pointer_cast<
                                                            SignTileEntity>(
                                                            sign);
                                            ...
                                            void ServerPlayer::
                                                ...
                                                containerCounter =
                                                    (containerCounter % 100) +
                                                    1;
                                            }

                                            bool ServerPlayer::startCrafting(
                                                int x, int y, in "" z) {
                                                if (containerMenu ==
                                                    inventoryMenu) {
                                                    nextContainerCounter();
                connection->send( std::shared_ptr<ContainerOpenPacket>( new ContainerOpenPacket(containerCounter, ContainerOpenPacket::WORKBENC"ServerPlayer tried to open crafting container when one was already open\n" x, y, z);
		...
		containerMenu->addSlotListener(this);
                                            ...
                                            bool ServerPlayer::openFireworks(
                                                int x, int "" int z) {
                                                if (containerMenu ==
                                                    inventoryMenu) {
                                                    nextContainerCounter();
                                                    ...
                                                    nextContainerCounter();
                connection->send( std::shared_ptr<ContainerOpenPacket>( new ContainerOpenPacket(containerCounter, ContainerOpenPacket:"ServerPlayer tried to open crafting container when one was already open\n"ry, level, x, y, z);
		containerMenu->containerId = containerCounter;
		containerMenu->addSlotListener(this);
                                                } else {
                                                    app.DebugPrintf();
                                            ...
                                            bool ServerPlayer::startEnchanting(
                                                int x, int y, int z,
                                                costd::nst std::wstri "" &
                                                    name) {
                                                if (containerMenu ==
                                                    inventoryMenu) {
                                                    nextContainerCounter();
                connection->send(std::shared_ptr<ContainerOpenPacket>( new ContainerOpenPacket(containerCounter, ContainerOpenPacket::ENCHANTMENT, name.empt"ServerPlayer tried to open enchanting container when one was already open\n"(inventory, level, x, y, z);
		containerMenu->containerId = containerCounter;
		containerMenu->addSlotListener(this);
                                                } else {
                                                    app.DebugPrintf();
                                            ...
                                            bool ServerPlayer::startRepa
                                                "" ing(int x, int y, int z) {
                                                if (containerMenu ==
                                                    inventoryMenu) {
                                                    nextContainerCounter();
                                                    connection->send(
                ...
                app.DebugPrintf(// 4J-JEV: Added to distinguish between ender, bonus, large and small chests (for displaying the name of the chest).nContainer(std::shared_ptr<Container> container)
{
                                                        if (containerMenu ==
                                                            inventoryMenu) {
                                                            nextContainerCounter();
		...
		int containerType = container->getContainerType();
                ...
                connection->send( std::shared_ptr<ContainerOpenPacket>( new ContainerOpenPacket(containerCounter, containerType, container->getCustomName(), con"ServerPlayer tried to open container when one was already open\n"ntainerMenu = new ContainerMenu(inventory, container);
		containerMenu->containerId = containerCounter;
		containerMenu->addSlotListener(this);
                                                        } else {
                                                            app.DebugPrintf();
...
bool ServerPlayer::openHopstd::per(std::shared_ptr<HopperTileEntity> container)
{
                                                        if (containerMenu ==
                                                            inventoryMenu) {
                                                            nextContainerCounter();
                connection->send( std::shared_ptr<ContainerOpenPacket>( new ContainerOpenPacket(containerCounter, ContainerOpenPacket::HOPPER, container->getCu"ServerPlayer tried to open hopper container when one was already open\n";
...
bool ServerPlayer::openHopstd::per(std::shared_ptr<MinecartHopper> container)
{
                                                        if (containerMenu ==
                                                            inventoryMenu) {
                                                            nextContainerCounter();
                connection->send( std::shared_ptr<ContainerOpenPacket>( new ContainerOpenPacket(containerCounter, ContainerOpenPacket::HOPPER, contai"ServerPlayer tried to open minecart hopper container when one was already open\n");
		containerMenu = new HopperMenu(inventory, container);
		containerMenu->containerId = containerCounter;
		containerMenu->addSlotListener(this);
                                                        } else {
...
bool ServerPlayer::openFurnace(std::shared_ptr<FurnaceTileEntity> furnace)
{
                                                        if (containerMenu ==
                                                            inventoryMenu) {
                                                            nextContainerCounter();
                                                            connection->send(std::shared_ptr<
                                                                             ContainerOpenPacket>(
                                                                new ContainerOpenPacket(
                                                                    containerCounter,
                                                                    ContainerOpenPacket : "ServerPlayer tried to open furnace when one was already open\n" furnace
                                                                        ->hasCustomName())));
                                                            containerMenu =
                                                                new FurnaceMenu(
                                                                    inventory,
...
bool ServerPlayer::openTrap(std::shared_ptr<DispenserTileEntity> trap)
{
                                                        if (containerMenu ==
                                                            inventoryMenu) {
                                                            nextContainerCounter();
                connection->send( std::shared_ptr<ContainerOpenPacket>( new ContainerOpenPacket(containerCounter, trap->GetType() == eTYPE_DROPPERTILEENTITY ? ContainerOpenPacket::DRO"ServerPlayer tried to open dispenser when one was already open\n"ContainerSize(), trap->hasCustomName() ) ) );
...
bool ServerPlayer::openBrewingStstd::and(std::shared_ptr<BrewingStandTileEntity> brewingStand)
{
                                                        if (containerMenu ==
                                                            inventoryMenu) {
                                                            nextContainerCounter();
...
bool ServerPlayer::openBeastd::con(std::shared_ptr<BeaconTileEntity> beacon)
{
                                                        if (containerMenu ==
                                                            inventoryMenu) {
                                                            nextContainerCounter();
...
bool ServerPlayer::openTradstd::ing(std::shared_ptr<Merchant> traderTarget, costd::nst std::wstring &name)
{
                                                        if (containerMenu ==
                                                            inventoryMenu) {
                                                            nextContainerCounter();
                ...
                connection->send(std::shared_ptr<ContainerOpenPacket>(new ContainerOpenPacket(containerCounter// just to make sure the offers are matched to the containerntainer->getContainerSize(), !name.empty())));
...
bool ServerPlayer::openHorseInventstd::ory(std::shared_ptr<EntityHorse> horse, std::shared_ptr<Container> container)
{
                                                        if (containerMenu !=
                                                            inventoryMenu) {
                                                            closeContainer();
                                                        ...
                                                        connection->send(std::shared_ptr<
                                                            ...
                                                            new ContainerOpenPacket(
                                                                ...
                                                                horse
                                                                    ->entityId)));
        containerMenu = new HorseInventoryMenu(inventory, container, hors// Do not send this packet!erI//= c// This is a horrible hack that makes sure that inventory clicks tr// that the client correctly predicted don't get sent out to thetai// client again.std::ex, std::shared_ptr<ItemInstance> item)
{
                                                            if (dynamic_cast<
                                                                    ResultSlot*>(
                                                                    container->getSlot(
...
void ServerPlayer::refreshContainer(AbstractContainerMenu *container, std::vector<std::shared// 4J - added, so that furnace updates also have this hackred_ptr<ContainerSetContentPacket// Do not send this packet!ack//(co// This is a horrible hack that makes sure that inventory clicks_pt// that the client correctly predicted don't get sent out to theven// client again.()) ) );
}
...
void ServerPlayer::setPl#ifndef _DURANGOxxa, float yya, bool jumping, bool sneaking)
{
    if (riding != NULL) {
        if (xxa >= -1 && xxa <= 1) this->xxa = xxa;
        if (yya >= -1 && yya <= 1) this->y #elseyya;
        this->jumping = jumping;
        this->setSneaking(sneaking);
    }
}

void ServerPlayer::awardS  // byteArray deleted in AwardStatPacket
                           // destructor.L#endif	delete [] param.data;
    return;
	}
                                                            ...
                                                            case IDS_CANT_SPAWN_IN_PEACEFUL:
                                                                for (
                                                                    ...
                                                                    server
                                                                        ...
                                                                        .size();
                                                                    i++) {
                        std::shared_ptr<"Tried to send a chat packet to the player with an unhandled messageId\n"m_this()==player)
			{
                            player
                                ->c  // Language *language =
                                     // Language::getInstance();w //wstring
                                     // languageString =
                                     // app.GetString(messageId);//language->getElement(messageId);
                                     // c//connection->send(
                                     // shared_ptr<ChatPacket>( new
                                     // ChatPacket(L"", messageType) )
                                     // );ize(); i++)
                            {
                                ...
                                if (shared_from_this() == player) {
                                    player->connection->send(
                                        std::shared_ptr<
                                            ...
                                            name,
                                            ChatPacket::e_ChatPlayerMaxBoats)));
                                }
                            }
                            break;
                                                                ...
                                                                ServerPlayer::
                                                                    magicCstd::rit(
                                                                        std::shared_ptr<
                                                                            Entity>
                                                                            entity) {
        getLevel()->getTracker()->broadca/*= e_ChatCustom*/rom_this(), std::/*= -1*/tr<AnimatePacket>( new AnimatePacket(ent/*= L""*/atePacket::MAGIC_CRITICAL_HIT) ));
                                                                }
                                                                ...
                                                                ServerPlayer::
                                                                    getLevel() {
                                                                    return  // 4J: Removed permission level S/*if( server->getPlayers()->isOp(dynamic_pointer_cast<ServerPlayer>(shared_from_this())) )
                                                                    {
                                                                        return server
