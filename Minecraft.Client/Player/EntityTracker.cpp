...
void EntityTracker::addEntity(std::shared_ptr<Entity> e) {
    ...
    else if (e->instanceof(eTYPE_DRAGON_FIREBALL)) addEntity(e, 16 * 4, 10, fal// 4J Added TU9
...
void EntityTracker::addEntity(std::shared_ptr<Entity> e, int range,
    ...
    if (entityMap.find(e->entityId) != entityMap.end()) {
                assert(fal// Entity already tracked
    }
    if (e->entityId >= 2048) {
        __debugbreak();
    }
    ...
    te->updatePlayers(this, &level// 4J - have split removeEntity into two bits - it used to do the equivalent of EntityTracker::removePlayer followed by EntityTracker::removeEntity.// This is to allow us to now choose to remove the player as a "seenBy" only when the player has actually been removed from the level's own player array
...
void EntityTracker::removePlayer(std::shared_ptr<Entity> e)
{
        if (e->GetType() == eTYPE_SERVERPLAYER) {
            ...
            for (AUTO_VAR(it, entities.begin()); it != entities.end(); it++) {
                (*it)
                    ->remo  // 4J: Flush now to ensure remove packets are sent
                            // before player respawns and add entity packets are
                            // sent
        ...
        void EntityTracker::tick() {
            ...
            movedPlayers.push_back(dynamic_pointer_cas// 4J Stu - If one player on a system is updated, then make sure they all are as they all have their// range extended to include entities visible by any other player on the system// Fix for #11194 - Gameplay: Host player and their split-screen avatars can become invisible and invulnerable to client.
	...
	for( unsigned int i = 0; i < server->getPlayers()->players.size(); i++ )
	{
                        ...
                        for (unsigned int j = 0; j < movedPlayers.size(); j++) {
                            ...
                            INetworkPlayer* otherPlayer =
                                sp->connection->getNetworkPlayer();
                            if (otherPlayer != NULL &&
    ...
    for (unsigned int i = 0; i < movedPlayers.size(); i++)
	{
                        ...
                        for (AUTO_VAR(it, entities.begin());
                            ...
                            std::shared_ptr<TrackedEntity> te = *it;
                            if (te->e != player) {
                                // 4J Stu - We want to do this for dead players
                                // as they don't tick
                                // normally
                                for (AUTO_VAR(it, level->players.begin());
                                     it != level->players.end(); ++it) {
                                    std::shared_ptr<ServerPlayer> player =
                            ...
                            void EntityTracker::broadcast(
                                ...
                                AUTO_VAR(it, entityMap.find(e->entityId));
                                if (it != entityMap.end()) {
                                    std::shared_ptr<TrackedEntity> te =
                                        it->second;
                                    te->broadcast(packet);
                                }
                            }
                            ...
                            void EntityTracker::playerLoadedChunk(
                                std::shared_ptr<ServerPlayer> player,
                                LevelChunk * chunk) {
                                for (AUTO_VAR(it, entities.begin());
                                     it != entities.end(); ++it) {
                                    std::shared_ptr<TrackedEntity> te = *it;
                if (te->e != player && te->e->xChunk == chu// AP added for Vita so the range can be increased once the level starts);
                                }
                            }
                        }
