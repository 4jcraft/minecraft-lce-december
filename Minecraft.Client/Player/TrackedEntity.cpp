...
void TrackedEntity::tick(EntityTracker* tracker,
    ...
    if (lastRidingEntity != e->riding ||
        ...
        lastRidingEntity = e->riding;
                broadcast(std::shared_ptr<SetEntityLinkPacket>(new SetEntityLinkPacket(SetEntityLinkPacket::RIDING, e, e->r// Moving forward  special case for item frames
	...
	else if (tickCount % updateInterval == 0 || e->// 4J: Moved this as it's shared->isDirty())
	{
            ...
            if (e->riding == NULL) {
                ...
                int xa = xn - xp;
                int ya = yn - yp;
                        int za = zn -// 4J - this pos flag used to be set based on abs(xn) etc. but that just seems wrong
			bool pos = abs(xa) >= TOLERANCE_LEVEL || abs(ya) >= TOLERANCE_LEVEL || abs(za) >= TOLERANCE_LEVEL || (tickCoun// Keep rotation deltas in +/- 180 degree range );
			...
			if (tickCount > 0 || e->i// 4J: Modifed, see above e->instanceof(eTYPE_PLAYER)) 
			{
                    if (xa < -128 || xa >= 128 ||
                        ...
                        || (e->GetType() == eTYPE_BOAT &&
                            teleportDelay > 20 * 20)) {
                        teleportDelay = 0;
                                        packet = std::shared_ptr<TeleportEntityPacket>( new TeleportEntityPacket(e//			printf("%d: New teleport rot %d\n",e->entityId,yRotn);	
					yRotp = yRotn;
					xRotp = xR// 4J If the movement is small enough, and there's no xrot, then use the new smaller packets
						if( ( xa >= -16 ) && ( xa <= 15 ) &&
							( za >= -16 ) && ( za <= 15 ) &&
						...
						{
                                                        packet = std::shared_ptr<MoveEntityPacket>( new MoveEntityPacket::PosRot(e->entityId, (//					printf("%d: New posrot %d + %d = %d\n",e->entityId,yRotp,yRota,yRotn);// 4J If the movement is small enough, then use the new smaller packets
						if( ( xa >= -8 ) && ( xa <= 7 ) &&
                        ...
                        if (!attributes->empty()) {
                            broadcastAndSend(
                                std::shared_ptr<UpdateAttributesPacket>(
                                    new UpdateAttributesPacket(e->entityId,
                                                               attributes)));
                        }

                        // 4J-PB - due to the knockback on a player being hit,
                        // we need to send to all players, but limit the network
                        // traffic here to players that have not already had it
                        // sent to their
                        // system//
                        // 4J - don't send to a player we've already sent this
                        // data to that shares the same machine. 	st//
                        // EntityMotionPacket used to limit themselves to
                        // sending once to each machine// by only sending to
                        // the primary player on each machine. This was causing
                        // trouble for split screen// as only the primary
                        // player would get a knockback velocity. Now these
                        ...
                        for (AUTO_VAR(it, seenBy.begin()); it != seenBy.end();
                            ...
                            if (sentTo.size()) {
                                ...
                                if (thisPlayer == NULL) {
                                    ...
                                    for (unsigned int j = 0; j < sentTo.size();
                                         j++) {
                                        std::shared_ptr<ServerPlayer> player2 =
                                            sentTo[j];
                                        INetworkPlayer*
                                            otherPl  // #ifdef _DEBUGconnecti//
                            ...
                            void TrackedEntity::broadcastAndSend(
                                std::shared_ptr<Packet> packet) {
                            ...
                            void TrackedEntity::broadcastRemoved() {
                                for (AUTO_VAR(it, seenBy.begin());
                                     it != seenBy.end(); it++) {
                                    (*it)->entitiesToRemove.push_back(
                                        e->entityId);
                                }
                            }

                            void TrackedEntity::
                                removePlayer  // 4J-JEV: Added for code reuse.p)
                            {
                                ...
                                if (it != seenBy.end()) {
                                    ...
                                    if (playersRange >
                                        TRACKED_ENTITY_MINIMUM_VIEW_DISTANCE) {
                                        playersRange -=
                                            sp->getPla  // 4J - added. Try and
                                                        // find other players
                                                        // who are in the same
                                                        ...
                                                        // machine, and extend
                                                        // our visibility;
                                            // so things are consider visible to
                                            // this player if they are near the
                                            // other one. This is because we
                                            ...
                                            // if they are the entity we are
                                            // tracking, or the player we've
                                            // been passed as input, or in
                                            // another
                                            ...
                                            // )
                                            {
                                                ...
                                                if (ep == sp) continue;
                                                if (ep == e) continue;
                                if( ep->dimension != // 4J Stu - We call update players when the entity has moved more than a certain amount at the start of it's tickayer !// Before this call we set xpu, ypu and zpu to the entities new position, but xp,yp and zp are the old position until later in the tick.// Therefore we should use the new position for visibility checks//xp / 32;//zp / 32;
					
					double xd = ep->x - xpu; 
					double zd = ep->z - zp// 4J Stu - We need to ensure that we send the mount before the rider, so check that the player has been added to the seenBy listanBeSeenBy |= canBySeenBy(ep);
                                            }
                                        }
                                    }
                                }
                        ...
                        void TrackedEntity::updatePlayer(
                            ...
                            if (visibility == eVisibility_SeenAndVisible &&
                                ...
                                std::shared_ptr<Packet> packet =
                                    getAddEntityPacket();
                                sp->connection
                                    "TrackedEntity:: Player '%ls' is now "
                        ...
                        app.DebugPrintf(,
                                ...
                                if (trackDelta && !isAddMobPacket) {
                                    sp->connection->send(
                                        std::shared_ptr<SetEntityMotionPacket>(
                                            new SetEntityMotionPacket(
                                                e->entityId, e->xd, e->yd,
                                                e->zd)));
                                }

                                if (e->riding != NULL) {
                                    sp->connection->send(
                                        std::shared_ptr<SetEntityLinkPacket>(
                                            new SetEntityLinkPacket(
                                                SetEntityLinkPacket::RIDING, e,
                                                e->riding)));
                                }
                                if (e->instanceof(eTYPE_MOB) &&
                                    dynamic_pointer_cast<Mob>(e)
                                            ->getLeashHolder() != NULL) {
                                    sp->connection->send(
                                        std::shared_ptr<SetEntityLinkPacket>(
                                            new SetEntityLinkPacket(
                                ...
                                if (e->instanceof(eTYPE_LIVINGENTITY)) {
                                    ...
                                    std::vector<MobEffectInstance*>*
                                        activeEffects = mob->getActiveEffects();
                                ...
                                i  // 4J - for some reason this isn't currently
                                   // working, and is causing players to not
                                   // appear until we are really close to them.
                                   // Not sure>e// what the conflict is between
                                   // the java & our version, but removing for
                                   // now as it is causing issues and we
                                   // shouldn't *really* need it// TODO -
                                   // investigate further//
                                   // return
...
std::shared_ptr<Packet> TrackedEntity::getAddEntityPacket()
{
                    ...
                    if (e->instanceof(eTYPE_ITEMENTITY)) {
                        std::shared_ptr<AddEntityPacket> packet =
                            std::shared_ptr<AddEntityPacket>(
                                new AddEntityPacket(e, AddEntityPacket::ITEM, 1,
                                                    yRotp, xRotp, xp, yp, zp));
                        return packet;
        }
	else if (e->i// 4J Added yHeadRotp param to fix #102563 - TU12: Content: Gameplay: When one of the Players is idle for a few minutes his head turns 180 degrees.
		...
		return std::shared_ptr<AddPlayerPacket>( new AddPlayerPacket( player, xuid, OnlineXuid, xp, yp, zp, yRotp, xRotp, yHeadRotp ) );
	}
	else if (e->instanceof(eTYPE_MINECART))
	{
                    ...
                    return std::shared_ptr<AddEntityPacket>(new AddEntityPacket(
                        e, AddEntityPacket::MINECART, minecart->getType(),
                        yRotp, xRotp, xp, yp, zp));
	}
	...
	else if (e->instanceof(eTYPE_ENDERDRAGON))
	{
                    ...
                    return std::shared_ptr<AddMobPacket>(
                        new AddMobPacket(dynamic_pointer_cast<LivingEntity>(e),
                                         yRotp, xRotp, xp, yp, zp, yHeadRotp));
	}
	else if (e->instanceof(eTYPE_FISHINGHOOK))
	{
                    std::shared_ptr<Entity> owner =
                        dynamic_pointer_cast<FishingHook>(e)->owner;
                    return std::shared_ptr<AddEntityPacket>(new AddEntityPacket(
                        e, AddEntityPacket::FISH_HOOK,
                        owner != NULL ? owner->entityId : e->entityId, yRotp,
                        xRotp, xp, yp, zp));
	}
	...
	else if (e->instanceof(eTYPE_SNOWBALL))
	{
                    return std::shared_ptr<AddEntityPacket>(
                        new AddEntityPacket(e, AddEntityPacket::SNOWBALL, yRotp,
                                            xRotp, xp, yp, zp));
	}
	else if (e->instanceof(eTYPE_THROWNPOTION))
	{
                    return std::shared_ptr<AddEntityPacket>(new AddEntityPacket(
                        ...
                        ((dynamic_pointer_cast<ThrownPotion>(e))
                             ->getPotionValue()),
                        yRotp, xRotp, xp, yp, zp));
	}
	...
	{
                    return std::shared_ptr<AddEntityPacket>(new AddEntityPacket(
                        e, AddEntityPacket::THROWN_EXPBOTTLE, yRotp, xRotp, xp,
                        yp, zp));
	}
	else if (e->instanceof(eTYPE_THROWNENDERPEARL))
	{
                    return std::shared_ptr<AddEntityPacket>(new AddEntityPacket(
                        e, AddEntityPacket::THROWN_ENDERPEARL, yRotp, xRotp, xp,
                        yp, zp));
	}
	else if (e->instanceof(eTYPE_EYEOFENDERSIGNAL))
	{
                    return std::shared_ptr<AddEntityPacket>(new AddEntityPacket(
                        e, AddEntityPacket::EYEOFENDERSIGNAL, yRotp, xRotp, xp,
                        yp, zp));
	}
	else if (e->instanceof(eTYPE_FIREWORKS_ROCKET))
	{
                    return std::shared_ptr<AddEntityPacket>(
                        new AddEntityPacket(e, AddEntityPacket::FIREWORKS,
                                            yRotp, xRotp, xp, yp, zp));
	}
	else if (e->instanceof(eTYPE_FIREBALL))
	{
                    ...
                    std::shared_ptr<Fireball> fb =
                        dynamic_pointer_cast<Fireball>(e);
                    std::shared_ptr<AddEntityPacket> aep = nullptr;
                    if (fb->owner != NULL) {
                        aep = std::shared_ptr<AddEntityPacket>(
                            new AddEntityPacket(e, type, fb->owner->entityId,
                                                yRotp, xRotp, xp, yp, zp));
                    } else {
                        aep = std::shared_ptr<AddEntityPacket>(
                            new AddEntityPacket(e, type, 0, yRotp, xRotp, xp,
                                                yp, zp));
                    }
	...
	{
                    return std::shared_ptr<AddEntityPacket>(
                        new AddEntityPacket(e, AddEntityPacket::PRIMED_TNT,
                                            yRotp, xRotp, xp, yp, zp));
	}
	else if (e->instanceof(eTYPE_ENDER_CRYSTAL))
	{
                    return std::shared_ptr<AddEntityPacket>(
                        new AddEntityPacket(e, AddEntityPacket::ENDER_CRYSTAL,
                                            yRotp, xRotp, xp, yp, zp));
	}
	else if (e->instanceof(eTYPE_FALLINGTILE))
	{
                    std::shared_ptr<FallingTile> ft =
                        dynamic_pointer_cast<FallingTile>(e);
                return std::shared_ptr<AddEntityPacket>( new AddEntityPacket(e, AddEntityPacket::FALLING, ft->tile | (ft->data << 16), "eTYPE_ITEM_FRAME xyz %d,%d,%d\n"else if (e->instanceof(eTYPE_PAINTING))
	{
	...
	else if (e->instanceof(eTYPE_ITEM_FRAME))
	{
                        ...
                        std::shared_ptr<AddEntityPacket> packet =
                            std::shared_ptr<AddEntityPacket>(
                                new AddEntityPacket(
                                    e, AddEntityPacket::ITEM_FRAME, frame->dir,
                                    yRotp, xRotp, xp, yp, zp));
	...
	else if (e->instanceof(eTYPE_LEASHFENCEKNOT))
	{
                        std::shared_ptr<LeashFenceKnotEntity> knot =
                            dynamic_pointer_cast<LeashFenceKnotEntity>(e);
                        std::shared_ptr<AddEntityPacket> packet =
                            std::shared_ptr<AddEntityPacket>(
                                new AddEntityPacket(e,
	...
	else if (e->instanceof(eTYPE_EXPERIENCEORB))
	{
                        return std::shared_ptr<AddExperienceOrbPacket>(
                            new AddExperienceOrbPacket(
                                dynamic_pointer_cast<ExperienceOrb>(e)));
	}
	else
	{
                        assert(false);
