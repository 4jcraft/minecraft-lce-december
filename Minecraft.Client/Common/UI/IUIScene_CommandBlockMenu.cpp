...
void IUIScene_CommandBlockMenu::ConfirmButtonClicked()
{
	...
	Minecraft::GetInstance()->localplayers[GetPad()]->connection->send(std::shared_ptr<CustomPayloadPacket>(new CustomPayloadPacket(CustomPayloadPacket::SET_ADVENTURE_COMMAND_PACKET, baos.toByteArray())));
}
