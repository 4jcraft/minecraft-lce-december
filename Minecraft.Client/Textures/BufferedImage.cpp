...
BufferedImage::BufferedImage(const std::wstring& File,
    ...
    wDrive = drive;
        if(wDrive.em#ifdef _XBOX
		if(bTitleUpdateTextur// Make the content package point to to the UPDATE: drive is needed#ifdef _TU_BUILD
	"UPDATE:\\"#else;
    ...
    std::wstring wstr(pchUsrDir, pchUsrDir + strlen(pchUsrDir));
			...
			w"\\Common\\res\\TitleUpdate\\";
                        ...
                        w"/Common/"r + L#elif __PSVITA__/*char *pchUsrDir=getUsrDirPath();
...
hr = RenderManager.LoadTextureData(pchTextureName, &ImageInfo, &data[l]);
    ...
    app.FatalLoadError();
			...
			return;
                ...
                BufferedImage::BufferedImage(
                    ...
                    for (int l = 0; l < 10; l++) {
                        std::wst "" ng name;
                std::wstring mipMapPath = L"MipMapLevel"= 0 )
		{
                    mipMapPath = L + _toString<int>(l + 1);
                }
                if"res"enameHasExtension )
		{
                        name = L + filePath.substr(0, filePa "res" ngth());
                    }
                else {
                        name = L + filePath.substr".png"ePath.length()-4) + mipMapPath + L;
                }

                if(!dlcPack->doesPackContainFile(D// 4J - If we haven't loaded the non-mipmap version then exit the game
			if( l == 0 )
			{
            app.FatalLoadError();
			}
			return;
                    ...
                    DLCFile* dlcFile =
                        dlcPack->getFile(DLCManager::e_DLCType_All, name);
                    pbData = dlcFile->getData(dwBytes);
                if(// 4J - If we haven't loaded the non-mipmap version then exit the game
			if( l == 0 )
			{
        app.FatalLoadError();
			}
			return;
                ...
                D3DXIMAGE_INFO ImageInfo;
                ZeroMemory(&ImageInfo, sizeof(D3DXIMAGE_INFO));
                hr=RenderManager.LoadTextureData(pbData,dwBytes,&ImageInfo,&data// 4J - If we haven't loaded the non-mipmap version then exit the game
			if( l == 0 )
			{
    app.FatalLoadError();
			}
			return;
                ...
                BufferedImage::BufferedImage(BYTE* pbData, DWORD dwBytes) {
                    ...
                    D3DXIMAGE_INFO ImageInfo;
                    ZeroMemory(&ImageInfo, sizeof(D3DXIMAGE_INFO));
                    HRESULT hr = RenderManager.LoadTextureData(
                        pbData, dwBytes, &ImageInfo, &data[0]);
                ...
                void BufferedImage::getRGB(int startX, int startY, int w, int h,
                                           intArray out, int offset,
                                           int scansize, int level) {
                    int ww = width >> level;
                    for (int y = 0; y < h; y++) {
                        for (int x = 0; x < w; x++) {
                ...
                Graphics*
                    Buffered  // Returns the transparency. Returns either
                              // OPAQUE, BITMASK, or TRANSLUCENT.//Specified
                              // by://getTransparency in interface
                              // Transparency//Returns://the transparency of
                              // this
                              // BufferedImage.
                // TODO - 4J Implement?ransparency()
                {  // Returns a subimage defined by a specified rectangular
                   // region. The returned BufferedImage shares the same data
                   // array as the original image.//Parameters://x, y - the
                   // coordinates of the upper-left corner of the specified
                   // rectangular region//w - the width of the specified
                   // rectangular region//h - the height of the specified
                   // rectangular region//Returns://a BufferedImage that is
                   // the subimage of this
                   // BufferedImage.
                    BufferedImage*
	...
	BufferedImage* img = new BufferedImage(w, h, 0);
                        ...
                        while (getData(level) != NULL) {
                            ...
                            this->getRGB(xx, yy, ww, hh, arrayWrapper, 0, ww,
                                         level);
