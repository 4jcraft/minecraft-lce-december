...
std::vector<Texture*>* TextureManager::createTextures(
    ...
    TexturePack* texturePack =
        ...
        :                   // Most important -- so it doesn't get uploaded to
           ...
           int clamp = Tex  // 4J Stu - Don't clamp as it causes issues with how
                            // we signal non-mipmmapped textures to the pixel
                            // shader //Texture::WM_CLAMP;
    ...
    if (te "res/" ack->hasFile(L + filename, false)) {
            ...
            if (app.GetBootedFromDiscPatch()) {
                ...
                drive =
                    Minecraft::GetInstance()
                        ->skins
                        ->getDefault() -  // BufferedImage *image = new
                    ...
                    BufferedImage* image = texturePack->getImageResource(
                        filename, false, true, drive);
                MemSect(0);
        ...
        if (isAnimation// TODO: Read this information from the animation file later
...
// This could end as 0 frames	
		...
		for (int i = 0; i < frameCount; i++)
		{
                    ...
                    Texture* texture = createTexture(
                        texName, mode, frameWidth, frameHeight, clamp, format,
                        minFilter, magFilter,
...
//return result;et//} catch (FileNotFoundException e) {//	Minecraft.getInstance().getLogger().warning("TextureManager.createTexture called for file " + filename + ", but that file does not exist. Ignoring.");//} catch (IOException e) {
//	Minecraft.getInstance().getLogger().warning("TextureManager.createTexture encountered an IOException when " + "trying to read file " + filename + ". Ignoring.");//}
	...
	return result;
                    ...
                    std::wstring TextureManager::getTextureNameFromPath(
                        ...
                        File file(filename);
                        return file.getName().substr(
                            0, '.' le.getName().find_last_of(L));
                    }
...
bool TextureManager::isAnimation(const std::wstring &filename, TexturePack *texturePac"/"{
                        ...
                        return Minecraft::GetInstance()
                            ->skins->getSelected()
                            ->hasFile(dataFileName, !hasOriginalImage);
}
