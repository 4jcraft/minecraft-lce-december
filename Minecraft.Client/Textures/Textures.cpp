...
void Textures::loadIndexedTextures() {
    ...
    for (int i = 0; i < TN_COUNT - 2; i++) {
                preLoadedIdx[i] = loadTexture((TEXTURE_NAME)i, std::wstring(preLoaded[i]".png");
    }
}
...
intArray Textures::loadTexturePixels(TEXTURE_NAME texId,
    ...
    {
        intArray id = pixelsMap[resourceName// 4J - if resourceName isn't in the map, it should add an element and as that will use the default constructor, its// internal data pointer will be NULL
        if (id.data != NULL) return id;
   // 4J - removed try/catch//    try {
        intArray re//wstring in = skin->getResource(resourceName);
		if (f// 4J - removed - was ( in == NULL)
		{
            res = loadTexturePixels(missingNo);
		}
		else
		{
                        BufferedImage *bufImage = readImage(texId, resourceNa//in);
	...
	*/
    ...
    int Textures::loadTexture(int idx) {
        if (idx == -1) {
            ...
            // 4J added - textures default to standard 32-bit RGBA format, but
            // where we can, use an 8-bit format. There's 3 different varieties
            // of these currently// in the renderer that map the single 8-bit
            // channel to RGBA differently.
void Textures::setTextureFormat(const std::wstring& resourceName// 4J Stu - These texture formats are not currently in the render header#ifdef _XBOX
	if(resourceName"/environment/clouds.png")
	{
                TEXTURE_FORMAT = C4JRender::TEXTURE_FORMAT_R1G1B1Ax;
	}
	else if(resourceName"%blur%/misc/pumpkinblur.png")
	{
                TEXTURE_FORMAT = C4JRender::TEXTURE_FORMAT_R0G0B0Ax;
	}
	else if(resourceName"%clamp%/misc/shadow.png")
	{
                TEXTURE_FORMAT = C4JRender::TEXTURE_FORMAT_R0G0B0Ax;
	}
	else if(resourceName"/environment/snow.png")
	{
                TEXTURE_FORMAT = C4JRender::TEXTURE_FORMAT_RxGxBxAx;
	}
	else if(resourceName"/1_2_2/misc/explosion.png")
	{
                TEXTURE_FORMAT = C4JRender::TEXTURE_FORMAT_RxGxBxAx;
	}
#endif 
...
int Textures::loadTexture(TEXTURE_NAME texId, const std::// 	char buf[256];a// 	wcstombs(buf, resourceName.c_str(), 256);// 	printf("Textures::loadTexture name - %s\n",buf);//if (resourceName.compare(L"/terrain.png") == 0)//{//	terrain->getStitchedTexture()->bind(0);//	return terrain->getStitchedTexture()->getGlId();//}//if (resourceName.compare(L"/gui/items.png") == 0)//{//	items->getStitchedTexture()->bind(0);//	return items->getStitchedTexture()->getGlId();//}// If the texture is not present in the idMap, load it, otherwise return its id
	...
	int id = MemoryTracker::genTex"%blur%"


	std::wstring prefix = L;
	bool blur = resourceName.substr(0, pr//resourceName.startsWith("%blur%");
	if (blur) pathName = r"%clamp%"me.substr(6);

	prefix = L;
	bool clamp = resourceName.substr(0, pr//resourceName.startsWith("%clamp%");
	if (clamp) pa//wstring in = skins->getSelected()->getResource(pathName);// 4J - removed was ( in == NULL)) 
	{
                loadTexture(missingN// 4J Stu - Get resource above just returns the name for texture packs
		BufferedImage *bufI//in); readImage(texId, pathName); 
...
void Textures::loadTexture(BufferedImage *img//	printf("Textures::loadTexture BufferedImage with blur and clamp %d\n",id);
    ...
    delete[] ne  // 4J-PB - In the new XDK, the CreateTexture will fail if the
                 // number of mipmaps is higher than the width & height passed
                 // in will allow!
        int iWidthMips = 1;
    int iHeightMips = 1;
    ...
    while ((8 << iHeightMips) < h) iHeightMips++;
                ...
                iMipLevels=(iWidthMips<iHei//RenderManager.TextureSetTextureLevels(5);	// 4J added
		if(iMipLevels>5)iMipLevels = 5;
		RenderManager.Te// 4J addedtureLevels(iMipLevels);	
	}
	RenderManager.TextureData(w,h,pixel//glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL12.GL_BGRA, GL12.GL_UNSIGNED_INT_8_8_8_8_REV, pixels);
    ...
    for (int level = 1; level < iMipLevels; level++) {
        ...
        // data
        unsigned int* tempDa  // 4J - added - have we loaded mipmap data for
                              // this level? Use that rather than generating if
                              // possible
            if (img->getData(level)) {
                                memcpy( tempData, im#ifndef _XBOXevel // Swap ARGB to RGBA
            ...
            for (int x = 0; x < ww; x++)
                for (int y = 0; y < hh; y++) {
                    int c0 =
                        pixels->getInt(((x * 2 + 0) + (y * 2 + 0) * ow) * 4);
                    int c1 =
                        pixels->getInt(((x * 2 + 1) + (y * 2 + 0) * ow) * 4);
                    int c2 =
                        pixels->getInt(((x * 2 + 1) + (y * 2 + 1) * ow) * 4);
                                                int c3 = pixels->getInt(((x #ifndef _XBOX * 2 + // 4J - convert our RGBA texels to ARGB that crispBlend is expecting
						c0 = ( ( c0 >> 8 ) & 0x00ffffff ) | ( c0 << 24 );
						c1 = ( ( c1 >> 8 ) & 0x00ffffff ) | ( c1 << 24 );
						c2 = ( ( c2 >> 8 ) & 0x00ffffff ) | ( c2 << 24 );
						c3 = ( ( c3 >> #endif0x00ffffff ) | ( c3 << 24 );

						int col = Texture::crispBlend(Texture::crispBlend(c0, #ifndef _XBOX:crispB// 4J - and back from ARGB -> RGBA	
						col = ( co#endif ) | (( col >> 24 ) & 0xff);
...
void Textures::replaceTexture(intArray rawPixels, int w// Removed in Java
#if 0td ::bind(id);
    ...
    pixels->put(newPixels);
    pixels->position(0)->limit(newPixel// Newth// glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w, h, GL12.GL_BGRA, GL12.GL_UNSIGNED_INT_8_8_8_8_REV, pixels);	#ifdef _XBOX

	RenderManager.Te#elseDataUpdate(pixels->getBuffer(),0);

	RenderManager.TextureData#endif(0// Oldh,pix//glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w, h, GL_RGBA, GL_UNSIGNED_BYTE, pixels);// 4J - added. This is a more minimal version of replaceTexture that assumes the texture bytes are already in order, and so doesn't do any of the extra copying round// that the original java version does
...
#ifdef _XBOXEXTURE_WRAP_T, GL_REPEAT);

	RenderManage#elsetureDataUpdate(rawPixels.data, 0);

	RenderManager.TextureDat#endife(0,// 4J - added. This is a more minimal version of replaceTexture that assumes the texture bytes are already in order, and so doesn't do any of the extra copying round// that the original java version does
...
void Textures::replaceTextureDirect(shortArray rawPixels, int w, int h, int id)
{
	...
	RenderManager.TextureDat#endife(0, 0, w, h, rawPixels.data, 0);
...
void Textures::removeHttpTexture(const std::wstring& url)
{
            ...
            if (texture != NULL) {
                ...
                if (texture->count == 0) {
                    ...
                    http  // 4J-PB - adding for texture in memory (from global
                          // title
                          // storage)
                        int Textures::loadMemTexture(
        ...
        if(texture == NULL &// If we haven't loaded it yet, but we have the data for it then add it
		texture = addMemTexture(url, new MobSkinMemTextureProcessor() );
                    }
                    if (texture != NULL) {
                if (texture->loadedImag// 4J - Disable mipmapping in general for skins & capes. Have seen problems with edge-on polys for some eg mumbo jumbo"dlcskin"
			if( ( url.substr(0,7) == L"dlccape") ||
				( url.substr(0,7) == L ) )
			{
                            MIPMAP = false;
			}
...
MemTexture *Textures::addMemTexture(const std::wstring& name,MemTextureProcessor *processor)
{
        ...
        if (it != memTextures.end()) {
                texture = (*i// can we find it in the app mem files?	
		PBYTE pbData=NULL;
		DWORD dwBytes=0;		
		...
		if(dwBytes!=0)
		{
                ...
                memText  // 4J Stu - Make an entry for this anyway and we can
                         // populate it
                         // later
        ...
        // MemTexture *Textures::getMemTexture(const wstring& url,
...
void Textures::tick(// 4J added updateTextures parameter & tickDynamics// 4J - added - if we aren't updating the final renderer textures, just tick each of the dynamic textures instead. This is used so that in frames were we have multiple// ticks due to framerate compensation, that we don't lock the renderer textures twice needlessly and force the CPU to sync with the GPU.
		if( !updateTextures// 4J - added - tell renderer that we're about to do a block of dynamic texture updates, so we can unlock the resources after they are done rather than a series of locks/unlocks//RenderManager.TextureDynamicUpdateStart();		
		terrain->cycleAnimationFrames();
 //RenderManager.TextureDynamicUpdateEnd();	// 4J added - see comment above// 4J - go over all the memory textures once per frame, and free any that haven't been used for a while. Ones that are being used will// have their ticksSinceLastUse reset in Textures::loadMemTexture.
...
void Textures::reloadAll()
{
    ...
    loadedImages.clear();
        ...
        loadI// 4J Stu - These are not used any more	//WaterColor::init(loadTexturePixels(L"misc/watercolor.png"));//GrassColor::init(loadTexturePixels(L"misc/grasscolor.png"));//FoliageColor::init(loadTexturePixels(L"misc/foliagecolor.png"));

	stitch(
#if 0kins->clearInvalidTexturePacks();
	...
	for( std::unordered_map<std::wstring, intArray>::iterator it = pixelsMap.begin(); it != itEndPM; it++ )
	{
        std::wstring name = it->first;
                BufferedImage *image = r#endifge(// Recalculate fontsme//Minecraft::GetInstance()->font->loadCharacterWidths();te//Minecraft::GetInstance()->altFont->loadCharacterWidths();
...
Icon *Textures::getMissingIcon(int type)
{
        switch (type) {
            ...
            case Icon::TYPE_TERRAIN:
                return terrain
                    ->getMi  // 4J was InputStream *inedImage
                             // *Textures::readImage(TEXTURE_NAME te// is this
                             // image one of the Title Update ones?
                {
	...
	bool isTu = IsTUImage(texId "res/");
                    ...
                    if (!skins->isUsingDefaultSkin() &&
                        skins->getSelected()->hasFile(L + name, false)) {
                        drive =
                            skins -  // new
                                     // BufferedImage(name,false,isTu,drive);s->getSelected()->getImageResource(name,
                                     // false, isTu, drive)#ifdef
                                     // __PS3__
                    } else {
                        ...
                        else 
		{
                ...
                if(IsO//new BufferedImage(name,false,isTu,drive);mg = skins->getDefault()->getImageResource(name,false,isTu,drive)"1_2_2/"//new BufferedImage(L"/1_2_2" + name,false,isTu,drive);etImageResource(L + name, false, // Match the preload images from their enum to avoid a ton of string comparisonsemSect(0);
    return img;
                    }
...
TEXTURE_NAME TUImages[] = {
                        ...
                        TN_MOB_VILLAGER_VILLAGER, TN_MOB_VILLAGER_FARMER,
                        TN_MOB_VILLAGER_LIBRARIAN, TN_MOB_VILLAGER_PRIEST,
                        TN_MOB_VILLAGER_SMITH,
                        TN_MOB_VIL  // TU9BUTCHER,
	...
	TN_TILE_ENDER_CHEST,
                        ...
                        TN_MOB_HORSE_CHESTNUT, TN_MOB_HORSE_CREAMY,
                        TN_MOB_HORSE_DARKBROWN, TN_MOB_HORSE_GRAY,
                        TN_MOB_HORSE_MARKINGS_BLACKDOTS,
                        TN_MOB_HORSE_MARKINGS_WHITE,
                        TN_MOB_HORSE_MARKINGS_WHITEDOTS,
                        TN_MOB_HORSE_MARKINGS_WHITEFIELD, TN_MOB_HORSE_SKELETON,
                        TN_MOB_HORSE_WHITE, TN_MOB_HORSE_ZOMBIE, TN_MOB_MULE,
                        TN_MOB_HORSE_ARMOR_DIAMOND, TN_MOB_HORSE_ARMOR_GOLD,
	...
	NULL
                ...
                TEXTURE_NAME OriginalImages[] = {
                    TN_MOB_CHAR,   TN_MOB_CHAR1, TN_MOB_CHAR2,
                    TN_MOB_CHAR3,  TN_MOB_CHAR4, TN_M "misc/watercolor.png" R6,
                    TN_MOB_CHAR7,
                ...
                wchar_t* OriginalImagesPaths[] = {L,

                                                  NULL};

                bool Textures::IsOriginalImage(TEXTURE_NAME texId,
                                               const std::wstring& name) {
                    int i = 0;
                    if (texId < TN_COUNT) {
