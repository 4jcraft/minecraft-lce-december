        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                data[0]->put(texel + byteRemap[0],
                             (byte)((buffer[texel >> 2] >> 24) & 0xff));
                data[0]->put(texel + byteRemap[1],
                             (byte)((buffer[texel >> 2] >> 16) & 0xff));
                data[0]->put(texel + byteRemap[2],
                             (byte)((buffer[texel >> 2] >> 8) & 0xff));
                data[0]->put(texel + byteRemap[3],
                             (byte)((buffer[texel >> 2] >> 0) & 0xff));
            }
        }
    }
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            tempBytes[byteIndex + byteRemap[0]] =
                (byte)((tempPixels[intIndex] >> 24) & 0xff);
            tempBytes[byteIndex + byteRemap[1]] =
                (byte)((tempPixels[intIndex] >> 16) & 0xff);
            tempBytes[byteIndex + byteRemap[2]] =
                (byte)((tempPixels[intIndex] >> 8) & 0xff);
            tempBytes[byteIndex + byteRemap[3]] =
                (byte)((tempPixels[intIndex] >> 0) & 0xff);
        }
    }
            ...
            if (image->getData(level)) {
                ...
                for (int y = 0; y < hh; y++) {
                    for (int x = 0; x < ww; x++) {
                        ...
                        tempBytes[byteIndex + byteRemap[0]] =
                            (byte)((tempData[intIndex] >> 24) & 0xff);
                        tempBytes[byteIndex + byteRemap[1]] =
                            (byte)((tempData[intIndex] >> 16) & 0xff);
                        tempBytes[byteIndex + byteRemap[2]] =
                            (byte)((tempData[intIndex] >> 8) & 0xff);
                        tempBytes[byteIndex + byteRemap[3]] =
                            (byte)((tempData[intIndex] >> 0) & 0xff);
                    }
                }
            } else {
    ...
    if (!m_bInitialised) {
        ...
        if (mipmapped) {
            for (int level = 1; level < m_iMipLevels; level++) {
                ...
                data[level] =
                    new ByteBuffer(data[level]->getSize(), (byte*)newData);
                delete oldBuffer;
