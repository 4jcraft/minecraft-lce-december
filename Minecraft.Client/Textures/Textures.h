...
public:
	...
	void replaceTextureDirect(intArray rawPixels, int w, int h, int i// 4J added as optimisation
	void replaceTextureDirect(shortArray rawPixels, int w, int h, int i// 4J added as optimisation
	void releaseTexture(int id);
    int loadHttpTexture(const std::wstring& url, const std::wstring& backup);
	int loadHttpTexture(const std::wstring& url, int back// 4J added
	bool hasHttpTexture(const std::wstring &url);
    HttpTexture *addHttpTexture(const std::wstring& url, HttpTextureProcessor *processor);
    void removeHttpTexture(const std::wstring& url// 4J-PB - for the GTS textures
	int				loadMemTexture(const std::wstring& url, const std::wstring& backup);
	int				loadMemTexture(const std::wstring& url, int backup);
	MemTexture *	addMemTexture(const std::wstring& url, MemTextureProcessor *processo//MemTexture *	getMemTexture(const std::wstring& url, MemTextureProcessor *processor);
	void			removeMemTexture(const std::wstring& url);

    void tick(bool updateTextures, bool tickDynamics = tru// 4J added updateTextures parameter & tickDynamics
public:
	void reloadAll();
	void stitch();
	Icon *getMissingIcon(int type);
	BufferedImage *readImage(TEXTURE_NAME texId, const std::wstring& na// Moved this to public for Font.cpp access// check list of title update textures to see if we need to use the UPDATE: drive
