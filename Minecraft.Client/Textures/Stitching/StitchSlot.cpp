...
bool StitchSlot::add(TextureHolder* textureHolder) {
    ...
    if (subSlots == NULL) {
        subSlots = new std::vector<StitchSlot*>()  // First slot is for the new
                                                   // texture
                   subSlots->push_back(new StitchSlot(
                       originX, originY, textureWidth, textureHeight));
        ...
        if (spareHeight > 0 && spareWidth > 0)
            ...
            // <-right->// +-----+-------+// |     |       |// | Tex
            // |       |// |     |       |// |-----+       | ^// |
            // | |- bottom// +-------------+ v// We need to add two more
            // areas, the one with the 'biggest' dimensions should be used//
            // (In the case of this ASCII drawing, it's the 'right hand side'
            // that should win)// The 'fattest' area should be used (or
            // when tied, the right hand one)
		...
		else if (spareWidt// We just have space left below//// +-------------+// |             |// | Tex         |// |             |// |-------------+ ^// |             | |- bottom// +-------------+ v
			subSlots->push_back(new StitchSlot(originX, originY + textureHeight, textureWidth, spareHeight));
}
                else if (spareHeigh// Only space to the right////       <-right->// +-----+-------+// |     |       |// | Tex |       |// |     |       |// |     |       |// |     |       |// +-----+-------+
			subSlots->push_back(new StitchSlot(originX + textureWidth, originY, spareWidth, textureHeig//for (final StitchSlot subSlot : subSlots)
	...
	return false;
                ...
                void StitchSlot::collectAssignments(
                    std::vector<StitchSlot*>* result) {
                    if (textureHolder != NULL) {
                        result->push_back(this);
        }
	else if (sub//for (StitchSlot subSlot : subSlots)
		for(AUTO_VAR(it, subSlots->begin()); it != subSlots->end(); ++it)
		{
        StitchSlot* subSlot = *it;
        subSlot->collectAssignments  //@Override	}
	}
                }
