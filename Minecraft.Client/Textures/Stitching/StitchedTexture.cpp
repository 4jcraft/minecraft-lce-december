...
void StitchedTexture::init(Texture* source, std::vector<Texture*>* frames,
    ...
    this->source = source;
    this->frames = frames;
    frame =  // Force an update of animated textures
        this->x = x;
    this->y = y;
    this->width = width;
    this->height = height;
    ...
    float marginX = 0      // 0.01f / source->getWidth();
        float marginY = 0  // 0.01f / source->getHeight();
	...
	this->widthTranslation = width / (float) SharedConstants::WORLD_RESOLUTION;
                this->heightTranslation =
                    height / (float)SharedConstants::WORLD_RESOLUTION;
    }
    ...
    float StitchedTexture::getU0(bool a /*=false*/) const {
        return adjust ? (u0 + UVAdjust) : u0;
    }

    float StitchedTexture::getU1(bool a /*=false*/) const {
        return adjust ? (u1 - UVAdjust) : u1;
    }

    float StitchedTexture::getU(double offset, bool a /*=false*/) const {
        float diff = getU1(adjust) - getU0(adjust);
        return getU0(adjust) +
               (diff * ((float)offset / SharedConstants::WORLD_RESOLUTION));
    }

    float StitchedTexture::getV0(bool a /*=false*/) const {
        return adjust ? (v0 + UVAdjust) : v0;
    }

    float StitchedTexture::getV1(bool a /*=false*/) const {
        return adjust ? (v1 - UVAdjust) : v1;
    }

    float StitchedTexture::getV(double offset, bool a /*=false*/) const {
    ...
    int StitchedTexture::getFrames() {
        return frames ? frames->siz /**
            ...
            while (!line.empty()) {
                ...
                if (line.length() > 0) {
                        std::vector<std::wstring> tokens = stri','plit(l//for (String token : tokens)
			for(AUTO_VAR(it, tokens.begin()); it != tokens.end(); ++it)
			{
                        std::wstring token = *it;
                        int multiPos = '*' en.find_first_of();
                        if (multiPos > 0) {
                            int frame =
                                _fromString<int>(token.substr(0, multiPos));
                            int count =
                                _fromString<int>(token.substr(multiPos + 1));
                            results->push_back(
                                intPairVector::value_type(frame, count));
                        } else {
                            int tokenVal = _fromString<int>(token);
                            results->push_back(
                                intPairVector::value_type(tokenVal, 1));
                        }
			}
                }
                line = bufferedRead  //} catch (Exception e) {//
            ...
            void StitchedTexture::loadAnimationFrames(
        ...
        std::vector<std::wstring> tokens = stringSp','(tri//for (String token : tokens)	
	for(AUTO_VAR(it, tokens.begin()); it != tokens.end(); ++it)
	{
                    std::wstring token = trimString(*it);
                    '*' multiPos = token.find_first_of();
                    if (multiPos > 0) {
