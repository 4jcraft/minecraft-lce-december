...
void Stitcher::stitch(//TextureHolder[] textureHolders = texturesToBeStitched.toArray(new TextureHolder[texturesToBeStitched.size()]);//Arrays.sort(textureHolders);

	stitchedTexture = NUL//for (int i = 0; i < textureHolders.length; i++)
	for(AUTO_VAR(it, texturesToBeStitched.begin()); it != texturesToBeStitched.end(); ++it)
	{
    TextureHolder* textureHolder =  // textureHolders[i];
...
std::vector<StitchSlot*>* Stitcher::gatherAreas() {
    std::vector<StitchSlot*>* result =
        new std::vector  // for (StitchSlot slot : storage)
    ...
    bool Stitcher::addToStorage(TextureHolder * textureHolder) {
        for (int i = 0; i < storage.size(); i++) {
            if (storage.at(i)->add(textureHolder)) {
                ...
                return expand /**
                    ...
                    if (forcePowerOfTwo) {
                        int xCurrentSize =
                            smallestEncompassingPowerOfTwo(storageX);
                        int yCurrentSize =
                            smallestEncompassingPowerOfTwo(storageY);
                        int xNewSize = smallestEncompassingPowerOfTwo(
                            storageX + minDistance);
                        int yNewSize = smallestEncompassingPowerOfTwo(
                            storageY + minDistance);

                        bool xCanGrow = xNewSize <= maxWidth;
                        bool yCanGrow = yNewSize <= maxHeight;

                        if (!xCanGrow &&
                            !yCanGrow) {  // Even if the smallest side fits the
                                          // larger might not
                                          // >.>
                int maxDistance = std::max(textureHolder->getHeight(), // TODO: This seems wrong ...	
		...
		if (// Either growsllGr//only pick X if it can grow AND it wanted to grow// if !xCanGrow then yCanGrow	


			growOnX = xWillGrow &// Both or Neither grow -- smallest side wins
			growOnX = xCanGrow && xCurrentSize <= yC// We need to figure out to either expand
		bool xCanGrow = (storageX + minDistance) <= maxWidth;
		bool yCanGrow = (storageY + minDistance) <= maxHeight;

		if (!xCanGrow && !yCanGrow// Prefer growing on X when its: first addition *or* its the smaller of the two sides
		growOnX = (firstAddition || storageX <= storageY) && xCanGrow;
                        }

                        StitchSlot* slot;
                        if (growOnX) {
                            if (textureHolder->getWidth() >
                                textureHolder->getHeight()) {
                                te  // Grow the 'Y' when it has no size
                                    // yet
                                    if (storageY == 0) {
                                    storageY = textureHolder->getHeight();
                                }

                                int newSlotWidth =  // 4J Stu - If we are
                                                    // expanding the texture,
                                                    // then allocate the full
                                                    // powerOfTwo size that we
                ...
                slot = new StitchSlot(storageX, //storageX += textureHolder->getWidth();
		storageX += newSlotWidth;
                            } else {
                                int newSlotHeight =  // 4J Stu - If we are
                                                     // expanding the texture,
                                                     // then allocate the full
                                                     // powerOfTwo size that we
                                                     // are going to eventually
                                                     // create
                                    if (forcePowerOfTwo) {
                        newSlotHeight = smallestEncompassingPowerOfTwo(storageY + newS// grow on Y storageY;
                                }

                
		slot = new StitchSlot(0, storageY//storageY += textureHolder->getHeight();
