    ...
    const std::wstring& name)  // throws FileNotFoundException
{
    ...
    if (app.GetBootedFromDiscPatch()) {
        ...
        pchUsrDir = app.GetBDUsrDirPath(pchTextureName);
                app.DebugPr"DefaultTexturePack::getResourceImplementation - texture %s - Drive - %s\n",pchTextureName,pchUsrDir);
    } else {
        const char* pchTextureName = wstringtofilename(name);
...
void DefaultTexturePack::unl#ifdef _XBOX// Unload skin
	XuiFre"TexturePack");
XuiFre "" isuals(L);
CXuiSceneBase::GetInstance()->SetVisu "" Prefix(L);
CXuiSceneBase::GetInstance()->SkinChanged(CXuiSceneBase::GetInstance() -
#endifj);
