                        ...
                        if (success) {
                            DLCUIDataFile* uiDLCFile =
                                (DLCUIDataFile*)texturePack->m_dlcDataPack
                                    ->addFile(DLCManager::e_DLCType_UIData,
                                              L"TexturePack.xzp");
                ...
                */
                        ...
                        if (!dlcFile->getGrfPath().empty()) {
                            ...
                            if (grf.exists()) {
                                ...
                                HANDLE fileHandle = CreateFile(
                                    pchFilename,   // file name
                                    GENERIC_READ,  // access mode
                                    0,  // share mode // TODO 4J Stu - Will we
                                        // need to share file? Probably not
                                        // but...
                                    NULL,           // Unused
                                    OPEN_EXISTING,  // how to create // TODO 4J
                                                    // Stu - Assuming that the
                                                    // file already exists if we
                                                    // are opening to read from
                                                    // it
                                    FILE_FLAG_SEQUENTIAL_SCAN,  // file
                                                                // attributes
                                    NULL                        // Unsupported
                                );
#else
                ...
                if (pack->getDLCItemsCount(DLCManager::e_DLCType_Audio) > 0) {
                    ...
                    Minecraft::GetInstance()->soundEngine->SetStreamingSounds(
                        iOverworldStart, iOverworldStart + iOverworldC - 1,
                        iNetherStart, iNetherStart + iNetherC - 1, iEndStart,
                        iEndStart + iEndC - 1,
