...
private:
    ...
    int emptyTime;
    bool m_bAtLeastOnePlayer  // 4J Added
        static WeighedTreasureArray
            RANDOM_BON  // 4J - brought forward from 1.3.2

                std::vector<TileEventData>
                    tileEvents[2];
...
public:
    ...
    bool AllPlayersAreSleeping() {
        return allPlayersS  // 4J added for a message to other players
            bool isAtLeastOnePlayerSleeping() {
            return m_bAtLeastOnePlayerSleeping;
        }

    protected:
    ChunkSource* createChunk  // 4J - was virtual, but was called from parent
                              // ctor
        public
    ...
    void generateBonusItemsNea  // 4J - brought forward from 1.3.2

        public : Pos* getDimensionSpecificSpawn();

        void S  // 4j Added for XboxOne PLM

            void save(bool force, ProgressListener* progressListener,
                      bool bAutosave = false);
        void saveToDisc(ProgressListener *progressListener, bool a// 4J Added

private:
	void saveLevelData();

	typedef std::unordered_map<int, std::shared_ptr<Entity> , IntKeyHash2, IntKeyEq> intEntityMap;
	intEntityMap enti// 4J - was IntHashMap, using same hashing function as this uses
protected:
...
public:
	...
	void queueSendTileUpdate(int x, int y// 4J Added
private:
	void runQueuedSendTile// 4J Added// 4J - added for implementation of finite limit to number of item entities, tnt and falling block entities
public:
	...
	bool atEntityLimi// 4J: Addedd_ptr<Entity> e); 

	virtual bool newPrimedTntAllowed();
	virtual bool newFallingTileAllowed();

	void flagEntitiesToBeRemoved(unsigned int // 4J addedl *// 4J addedd);		

	
	static c// Each of these need to be duplicated for each level in the current game. As we currently only have 2 (over/nether), making this constant
