...
#include "../Core/ServerScoreboard.h"
...
ServerLevel::ServerLevel(MinecraftServer* server,
    ...
    chunkSourceXZSize =
        chunkSource
            ->m_XZSiz  // 4J - The listener used to be added in
                       // MinecraftServer::loadLevel but we need it to be set up
                       // before we do the next couple of things, or else chunks
                       // get loaded before we have the entity tracker set up to
                       // listen to them
        this->server = server;
        server->setLevel(dimension, thi// The listener needs the server to have the level set up... this will be set up anyway on return of this ctor but setting up early here
	...
	scoreboard = new ServerScoreboard(server//shared_ptr<ScoreboardSaveData> scoreboardSaveData = dynamic_pointer_cast<ScoreboardSaveData>( savedDataStorage->get(typeid(ScoreboardSaveData), ScoreboardSaveData::FILE_ID) );//if (scoreboardSaveData == NULL)//{
	//	scoreboardSaveData = shared_ptr<ScoreboardSaveData>( new ScoreboardSaveData() );//	savedDataStorage->set(ScoreboardSaveData::FILE_ID, scoreboardSaveData);//}
	//scoreboardSaveData->setScoreboard(scoreboard);//((ServerScoreboard *) scoreboard)->setSaveData(scoreboardSaveData);// This also used to be called in parent ctor, but can't be called until chunkSource is created. Call now if required.
...
// 4J - added initialisers// 4J Stu - Allowing spawn edit for our game, and consider a better solution for the possible griefing
	canEditSpawn = t//false;
	noSave = false;
...
PIXEndNamedEvent();
        ...
        if (newDark != skyDarken) {
            skyDarken = newDark;
                if (!SharedConstants::TEXTURE_LIGHT// 4J - change brought forward from 1.8.2
		{
...
//4J - temporarily disabling saves as they are causing gameplay to generally stutter quite a lot
	...
	__int64 time = levelData->getGameTime() +// 4J Stu - Putting this back in, but I have reduced the number of chunks that save when not forced#ifdef _LARGE_WORLDS
	if (time % (saveInterval) == (dimension->id +#else
	if (time % (saveInterval) == (dimension->id * dimension->id * (saveInterval/#endif
//app.DebugPrintf("Incremental save\n");
		PIXBeginNamedEve"Incremental save");
		save(false, NULL);
		PIXEndNamedEvent();
// 4J : WESTY : Changed so that time update goes through stats tracking update code.//levelData->setTime(time);
	setGameTime(levelData->getGameTime() + 1);
	if (getGameRules()->getBoolean(GameRules::RULE_DAYLIGHT))
// 4J: Debug setting added to keep it at day time#ifndef _FINAL_BUILD		
        ...
        PIXBeginNamedEve"Tick portal forcer");
        portalForcer->tick(getGameTime());
        PIXEndNamedEvent(  // repeat after tile ticks
            PIXBeginNamedEve "runTileEvents");
        runTileEvents();
        PIXEndNamedEvent(// 4J Added
	runQueuedSendTileUpdates();
        ...
        if (allPlayersSleepin// all players are sleeping, but have they slept long enough?
		AUTO_VAR(itEnd, players.end());
		for (std::vector<std::shared_ptr<Player> >::iterator it = players.b//                System.out.println(player->entityId + ": " + player->getSleepTimer());
        ...
        void ServerLevel::validateSpawn() {
            ...
            levelData->setXSpawn(
                xSpawn);  // 4J - Changes made here to move a section of code
                          // (which randomly determines which tiles in the
                          // current chunks to tick, and is very cache
                          // unfriendly by nature)// This code now has a thread
                          // of its own so it can wait all it wants on the cache
                          // without holding the main game thread up. This
                          // slightly changes how things are// processed, as we
                          // now tick the tiles that were determined in the
                          // previous tick. Have also limited the amount of
                          // tiles to be ticked to 256 (it never seemed to
                          // creep// up much beyond this in normal play anyway,
                          // and we need some finite
                          // limit).// Index
                          // into the arrays used by the update
                          // thread
            int iLev = 0;
            if (dimension->id == -1) {
                iLev = 1;
            ...
            EnterCr  // This section processes the tiles that need to be ticked,
                     // which we worked out in the previous tick (or haven't
	...
	for (int i = 0; i < m_updateTileCount[iLev]; i++) {
                ...
                if (hasChunkAt(x, y, z)) {
                    int id = getTile(x, y, z);
                        if (Tile::tiles[id] != NULL && Tile/*if(id == 2) ++grassTicks;
				else if(id == 11) ++lavaTicks;
				else ++otherTicks;*/
				Tile::tiles[id]->tick(th//printf("Total ticks - Grass: %d, Lava: %d, Other: %d, Total: %d\n", grassTicks, lavaTicks, otherTicks, grassTicks + lavaTicks + otherTicks);
	m_updateTileCount[iLev] = 0;
	m_updateChunkCount[iLev] = 0;
	LeaveCriticalSection(&m_update// AP moved this outside of the loop
	int prob = 100000;
	if(app.GetGameSettingsDebugMask()&(1L<<eDebugSett
#ifdef __PSVITA__ng  // AP - see CustomSet.h for and
                             // explanation
	...
	AUTO_VAR(itEndCtp, chunksToPoll.end());
		...
		if (random->nextInt(prob) == 0 && isRaining() && isThundering())
		{
                        ...
                        if (isRainingAt(x, y, z)) {
                                addGlobalEntity( std::shared_ptr<LightningBolt>( new Lightn// 4J - changes here brought forrward from 1.2.3
		if (random->nextInt(16) == 0)
		{
                                ...
                                if (shouldFreeze(x + xo, yy - 1, z + zo)) {
                                    setTileAndUpdate(x + xo, yy - 1, z + zo,
                                                     Tile::ice_Id);
                                }
                                if (isRaining() &&
                                    shouldSnow(x + xo, yy, z + zo)) {
                                    setTileAndUpdate(x + xo, yy, z + zo,
                                                     Tile::topSnow_Id);
                                }
                                if (isRaining()) {
                                    ...
                                    if (b->hasRain()) {
                                        int tile =
                                            getTile(x + xo, yy - 1, z + zo);
                                        if (tile != 0) {
                                                Tile::tiles[tile]->handleRain(this, x + xo, yy// 4J - lighting change brought forward from 1.8.2
		checkLight(xo + random->nextInt(16), random->nextInt(128), zo + random->nextInt(16));
                                    ...
                                    bool ServerLevel::isTileToBeTickedAt(
                                        int x, int y, int z, int tileId) {
                                        TickNextTickData td =
                                            TickNextTickData(x, y, z, tileId);
                                        return std::find(toBeTicked.begin(),
                                                         toBeTicked.end(),
                                                         td) !=
                                               toBeTicked.end();
                                    }

                                    void ServerLevel::addToTickNextTick(
                                        int x, int y, int z, int tileId,
                                        int tickDelay) {
                                        addToTickNextTick(x, y, z, tileId,
                                                          tickDelay, 0);
                                    }
                                    ...
                                    void ServerLevel::addToTickNextTick(
                                        ...
                                        if (hasChunksAt(x - r, y - r, z - r,
                                                        x + r, y + r, z + r)) {
                                            if (tileId > 0) {
                                                td.delay(
                                                    tickDelay +
                                                    levelData->getGameTime());
                                                td.setPriorityTilt(
                                                    priorityTilt);
                                            }
                                            EnterCriticalSection(
                                                &m_tickNextTickCS);
                                            if (tickNextTickSet.find(td) ==
                                                tickNextTickSet.end()) {
                                    ...
                                    void ServerLevel::forceAddTileTick(
                                        int x, int y, int z, int tileId,
                                    ...
                                    bool ServerLevel::tickPendingTicks(
                                        bool force) {
                                        EnterCriticalSection(&m_tickNextTickCS);
                                        int count =
                                            (int)tickNextTickList.size();
                                        int count2 =
                                            (int)tickNextTickSet.size();
                                        if  // TODO 4J Stu - Add new exception
                                            // types//throw new
                                            // IllegalStateException("TickNextTick
                                            // list out of
                                            // synch");
                                    }
                                    ...
                                    for (int i = 0; i < count; i++) {
                                        ...
                                        itTickList =
                                            tickNextTickList.erase(itTickList);
                                    ...
                                    for (AUTO_VAR(it, toBeTicked.begin());
                                        ...
                                        int r = 0;
                                        if (hasChunksAt(td.x - r, td.y - r,
                                                        td.z - r, td.x + r,
                                                        td.y + r, td.z + r)) {
                                            int id = getTile(td.x, td.y, td.z);
                                            if (id > 0 && Tile::isMatching(
                                                              id, td.tileId)) {
                                                Tile::tiles[id]->tick(
                                                    this, td.x, td.y, td.z,
                                                    random);
                                            ...
                                            addToTickNextTick(td.x, td.y, td.z,
                                                              td.tileId, 0);
                                        }
                                    }
                                ...
                                ServerLevel::fetchTicksInChunk(
                                    LevelChunk * chunk, bool remove) {
                                    EnterCriticalSection(&m_tickNextTickCS);
                                    std::vector<TickNextTickData>* results =
                                        new std::vector<TickNextTickData>;
...
{
                                if (!server->isAnimals() &&
                                    (e->instanceof(eTYPE_ANIMAL) ||
                                     e->instanceof(eTYPE_WATERANIMAL))) {
                                    e->remove();
                                }
                                if (!server->isNpcsEnabled() &&
                                    (dynamic_pointer_cast<Npc>(e) != NULL)) {
                                    e->remove();
                                }
                                Level::tick(e, actual);
}
...
ChunkSource *ServerLevel::createChunkSource()
{
                                ChunkStorage* storage =
                                    levelStorage->createChunkStorage(dimension);
                                cache = new ServerChunkCache(
                                    this, storage,
...
bool ServerL// 4J-PB - This will look like a bug to players, and we really should have a message to explain why we're not allowing lava to be placed at or near a spawn point// We'll need to do this in a future update// 4J-PB - Let's allow water near the spawn point, but not lava
...
void ServerLevel::initializeLevel(Leve/**
...
void ServerLevel::setInitialSpawn(LevelSettings *levelSettings)
{
                        if (!dimension->mayRespawn()) {
                            levelData->setSpawn(
                                0, dimension->getSpawnYPosition(), 0);
                            return;
                        }
                        ...
                        std::vector<Biome*> playerSpawnBiomes =
                            biomeSource->getPlayerSpawnBiomes();
                        Random random(getSeed());

                        TilePos* fi  // (Level.MAX_LEVEL_SIZE - 100) * 0;0, 16 *
	...
	int minXZ = -(dimension->getXZSize() * 16) / 2;
                        ...
                        if (findBiome != NULL) {
                            xSp "Level::setInitialSpawn - Unable to find spawn "
                                "biome\n" indBiome;
                        } else {
	...
	if (levelSetting// 4J - brought forward from 1.3.2enerateBonusItemsNearSpawn();
                        }
                        isFindingSpawn =
                            f  // once we've found the initial spawn, try to
                               // find a location for theeB// starting bonus
	...
	static const int r = 20;
                        ...
                        BonusChestFeature* feature =
                            new BonusChestFeature(RANDOM_BONUS_ITEMS, 16);
                        for (int attempt = 0; attempt < 16; attempt++) {
                            int x = levelData->getXSpawn() +
                                    random->nextInt(6) - random->nextInt(6);
                            int z = levelData->getZSpawn() +
                                    random->nextInt(6) - random->nextInt(6);
                            int y = getTopSolidBlock(x, z) + 1;

                            if (feature->place(this, random, x, y, z,
                                               (attempt == 15))) {
                                break;
                            }
                        }
                        dele  // 4j Added for XboxOne
                              // PLMevel::getDimensionSpecificSpawn()
                        {
...
void ServerLevel::save(bool forc// 4J-PB - check that saves are enabledbool bAutosave)
{
	...
	if (StorageManager.GetSaveDisabled()) return;
                            ...
                            if (progressListener != NULL) {
                                if (bAutosave) {
                                    ...
                                    "Saving level data"->progressStartNoAbort(
	...
	if (bAutosave) {
			...
			std::vector<LevelChunk*>* loadedChunkList =
                                    ...
                                    void ServerLevel::entityAdded(
                                        std::shared_ptr<Entity> e) {
                                        Level::entityAdded(e);
                                        enti  // for (int i = 0; i < es.length;
	...
	entityRemovedExtra(e);		
                                ...
                                bool ServerLevel::addGlobalEntity(
                                    std::shared_ptr<Entity> e) {
                                    ...
                                    std::shared_ptr<Packet> p =
                                        std::shared_ptr<EntityEventPacket>(
                                            new EntityEventPacket(e->entityId,
                                                                  event));
                                    server->getLevel(dimension->id)
                                        ->getTracker()
                                        ->broadcastAndSend(e, p);
                                }

                                std::sha  // instead of calling super, we run
                                          // the same explosion code here except
                                                ...
                                                for (unsigned int j = 0;
                                                    ...
                                                    if (otherPlayer != NULL &&
                                                        thisPlayer
                                                            ->IsSameSystem(
                                                                otherPlayer))
                                                        // app.DebugPrintf("Sending
                                                        // %s with knockback
                                                        // (%f,%f,%f)\n",
                                                        // knockbackOnly?"knockbackOnly":"allExplosion",knockbackVec->x,knockbackVec->y,knockbackVec->z);nock//
                                                                        ...
                                                                        server
                                                                            ...
                                                                            ->broadcast(
                                                                                ...
                                                                                dimension
                                                                                    ->id,
                                                                                std::shared_ptr<
                                                                                    TileEventPacket>(new TileEventPacket(
                                                                                    te.getX(),
                                                                                    te.getY(),
                                                                                    te.getZ(),
                                                                                    te.getTile(),
...
{
        std::shared_ptr<Packet> packet = std::shared_ptr<LevelParticlesPacket>( new LevelParticlesPacket(name, (float) x, (float) y, (float) z, (float) xDist, (float) yDist, (float) zDist, (f// 4J Stu - Sometimes we want to update tiles on the server from the main thread (eg SignTileEntity when string verify returns)er = dynamic_pointer_cast<ServerPlayer>(*it);
...
bool ServerLevel::atEntityLimit(std::shared_ptr<Entity> e)
{
	...
	else if(// Maintain a cound of primed tnt & falling tiles in this level(&m_limiterCS);
	...
	return atLimit;
		...
		LeaveCriticalSection(&m_limiterCS);
			...
			m_itemEntities.erase(it);  //		printf("entity
                                                                ...
                                                                e->instanceof(
                                                                    eTYPE_HANGING_ENTITY)) {
                                                                EnterCriticalS  //			printf("Item to remove found\n");//		printf("entity removed: item entity count now %d\n",m_itemEntities.size());gingEntities.end(),e));
                                                                    if (it !=
                                                                        m_hangingEntities
                                                                            .end()) {
			...
			m_hangingEntit  //		printf("entity removed:
                                                                    ...
                                                                    Enter  //			printf("Item
                                                                           ...
                                                                           // count
                                                                           // now
                                                                           //%d\n",m_arrowEntities.size());n(),m_arrowEntities.end(),e));
                                                                        if (it !=
                                                                            m_arrowEntities
                                                                                .end()) {
		...
		LeaveCriticalSection(&m_limiterCS);
                                                                ...
                                                                ServerLevel::
                                                                    runUpdate(
                                                                        void*
                                                                            lpParam) {
        ShutdownManager::HasStarted(ShutdownManager::eRunUpdateThrea"Updating tiles to be ticked"utdow// 4J Stu - Grass and Lava ticks currently take up the majority of all tile updates, so I am limiting them!ShutdownManager::ShouldRun(ShutdownManager::eRunUpdateThread)) break;

		PIXBeginNamedEvent(0,);
		// 4J - some of these tile ticks will check things in neighbouring tiles, causing chunks to load/create that aren't yet in memory.Enter// Try and avoid this by limiting the min/max x & z values that we will try and inspect in this chunk according to what surround chunks are loaded
				...
				for (int j = 0; j < 80; j++)
				{
                                                                        m_randValue[iLev] =
                                                                            ...
                                                                            m_level[iLev]
                                                                                ->addend;
                                        int val = (m_randValue// This array access is a cache miss pretty much every time) || ( x > maxx ) ) continue;
					int z = ((val >> 8) & 15);
					if( ( z < minz ) || ( z > maxz // 4J Stu - Grass and Lava ticks currently take up the majority of all tile updates, so I am limiting them
					int id = lc->getTile(x,y,z);
					if( m_updateTileCount[iLev] >= MAX_UPDATES ) break;

					// 4J Stu - Added shouldTileTick as some tiles won't even do anything if they are set to tick and use up one of our updates&& grassTicks >= MAX_GRASS_TICKS) || (id == Tile::calmLava_Id && lavaTicks >= MAX_LAVA_TICKS) ) continue;
					...
					if (Tile::tiles[id] != NULL && Tile::tiles[id]->isTicking() && Tile::tiles[id]->shouldTileTick(m_level[iLev],x + (cx * 16), y, z + (cz * 16) ) )
					{
                                                                            ...
                                                                            m_updateTileX
                                                                                ...
                                                                                [m_updateTileCount
                                                                                    ...
                                                                                    (cx *
                                                                                     16);
                                                m_upd#ifdef __PS3__[m_updateTileC#endif //__PS3__
