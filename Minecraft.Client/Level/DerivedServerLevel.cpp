...
DerivedServerLevel::DerivedServerLevel(MinecraftServer *server, std::shared_ptr<LevelStorage> levelStorage, const std::wstring& levelName, int dimension, LevelSettings *levelSettings, ServerLevel *wrapped)
	: ServerLevel(server, levelStorage, levelName, dimension, levelSettings// 4J-PB - we're going to override the savedDataStorage, so we need to delete the current one
...
DerivedServerLevel::~DerivedServerLevel(// we didn't allocate savedDataStorage here, so we don't want the level destructor to delete it
	this->savedDataStorage=NULL;
}

void DerivedServerLevel::saveLevelData(// Do nothing?// Do nothing!
}