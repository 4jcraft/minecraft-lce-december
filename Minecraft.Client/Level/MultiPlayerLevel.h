...
public:
    ...
    std::shared_ptr<Entity> removeEntity(int id);
        virtual void removeEntities(std::vector<std::shared_ptr<Entity> // 4J Added override
...
public:
	void animateT// 4J added;			
	std::unordered_set<in// 4J addedAnimate;	
...
public:
	...
	virtual void setDayTime(// 4J Stu - Added so we can have multiple local connections
