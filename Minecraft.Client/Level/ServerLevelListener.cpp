...
void ServerLevelListener::playSound(int iSound, double x, double y, double z,
    ...
    if (iSound < 0) {
                app.DebugPr"ServerLevelListener received request for sound less than 0, so ignoring\n");
    } else
        // 4J-PB - I don't want to broadcast player sounds to my local machine,
        // since we're already playing these in the
        // LevelRenderer::playSound.// The PC version does seem to do this
        // and the result is I can stop walking , and then I'll hear my footstep
        // sound with a delay
...
void ServerLevelListener::playSoundExceptPlayer(std::shared_ptr<Player> player,
    ...
    if (iSound < 0) {
                app.De"ServerLevelListener received request for sound less than 0, so ignoring\n");
    }
    e  // 4J-PB - I don't want to broadcast player sounds to my local machine,
       // since we're already playing these in the
       // LevelRenderer::playSound.// The PC version does seem to do this and
       // the result is I can stop walking , and then I'll hear my footstep
       // sound with a delay
        server->getPlayers()
            ->broadcast(player, x, y, z, volume > 1 ? 16 * volume : 16,
                        level->dimension->id,
                        std::shared_ptr<LevelSoundPacket>(new LevelSoundPacket(
...
void ServerLevelListener::destroyTileProgress(int id, int x, int y,//for (ServerPlayer p : server->getPlayers()->players)
