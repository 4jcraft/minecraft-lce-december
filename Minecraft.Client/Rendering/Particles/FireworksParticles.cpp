...
/*fadeColors != NULL &&*/ fadeColors.length > 0)
	{
...
FireworksParticles::FireworksSparkParticle::FireworksSparkParticle(
    ...
    size *= 0.75f;
        ...
        lifetime = 48 + random->nextInt(#ifdef __PSVITA__
	noPhysics = tru// 4J - optimisation, these are just too slow on Vita to be running with physics on#else
	noPhysics = fa#endif
...
void FireworksParticles::FireworksSparkParticle::tick() {
    ...
    yd -= 0.004;
        std::move(xd, yd// 4J - changed so these don't attempt to collide with entities
	xd *= 0.91f;
	yd *= 0.91f;
	zd *= 0.91f;
