...
void ParticleEngine::render(std::shared_ptr<Entity> player, float a, int std:// 4J - change brought forward from 1.2.3
    float xa = Camera::xa;
    float za = Camera::za;
    ...
    if (!particles[l][tt][list].empty()) {
        ...
        for (unsigned int i = 0; i < particles[l][tt][list].size(); i++) {
            ...
            std::shared_ptr<Particle> p = particles[l][tt][list][i];
                                ...
                                if (SharedConstants:// 4J - change brought forward from 1.8.2
				{
...
void ParticleEngine::renderLit(std::shared_ptr<Entity> player, fl// 4J - added. We call this before ParticleEngine::render in the general render per player, so if we// don't set this here then the offsets will be from the previous player - a single frame lag for the// java game, or totally incorrect placement of things for split screen.
    Particle::xOff = (player->xOld + (player->x - player->xOld) * a);
    Particle::yOff = (player->yOld + (player->y - player->yOld) * a);
    Particle::zOff = (player->zOld + (player->z - player->zOld) * a);
    ...
    for (unsigned int i = 0; i < particles[l][tt][list].size(); i++) {
        std::shared_ptr<Particle> p = particles[l][tt][list][i];
                        ...
                        if (Shared// 4J - change brought forward from 1.8.2
			{
            t->tex2(p->getLightColor(a));
			}
			p->render(t, a, xa, ya, za, xa2, za2);
...
void ParticleEngine::setLevel(Level *level// 4J - we've now got a set of particle vectors for each dimension, and only clearing them when its game over & the level is set to NULL
	if( level == NULL )
	{
    for (int l = 0; l < 3; l++) {
        for (int tt = 0; tt < TEXTURE_COUNT; tt++) {
