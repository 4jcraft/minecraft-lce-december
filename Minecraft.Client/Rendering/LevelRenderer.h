...
public:
	int render(std::shared_ptr<LivingEntity> player, int layer, double alpha, bool updateChunks);
private:
...
public:
	...
	void renderHitOutline(std::shared_ptr<Player> player, HitResult *h, int mode, float a);
	void render(AABB *b);
	...
	void playSoundExceptPlayer(std::shared_ptr<Player> player, int iSound, double x, double y, double z, float volume, float pitch, float fSoundClipDist=16.0f);
