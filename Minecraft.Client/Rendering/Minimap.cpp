...
void Minimap::reloadColours() {
    ...
    // Minimap::render, although with some uint8_t order changes
    for (int i = 0; i < (14 * 4);
...
void Minimap::render(std::shared_ptr<Player> player, Textures* textures,
    ...
    std::vector<MapItemSavedData::MapDecoration*>
        ...
        fIco  // 4J - moved to -0.04 (was -0.02) to stop z fighting
            for (AUTO_VAR(it, m_edgeIcons.begin()); it != m_edgeIcons.end();
                 it++) {
            MapItemSavedData::MapDecoration* dec = *it;

            char imgIndex = dec->img;
            imgIndex  // 4J Stu - For item frame renders, the player is NULL. We
                      // do not want to show player icons on the
                      // frames.
