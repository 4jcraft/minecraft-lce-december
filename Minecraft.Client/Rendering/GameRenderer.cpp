...
void GameRenderer::pick(float a) {
    ...
    hovered = nullptr;
    float overlap = 1;
    vector<shared_ptr<Entity> >* objects = mc->level->getEntities(
...
void GameRenderer::renderLevel(float a, __int64 until) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (cameraFlip == 0) {
            ...
            {
                shared_ptr<Player> player =
                    dynamic_pointer_cast<Player>(cameraEntity);
        ...
        */
        ...
        levelRenderer->renderDestroyAnimation(
            ...
            dynamic_pointer_cast<Player>(cameraEntity), a);
        glDisable(GL_BLEND);
            ...
            if (t > 0) {
                if (Tile::tiles[t]->material == Material::lava) {
                    mc->particleEngine->add(
                        shared_ptr<SmokeParticle>(new SmokeParticle(
                            level, x + xa,
                    ...
                    mc->particleEngine->add(
                        shared_ptr<WaterDropParticle>(new WaterDropParticle(
                            level, x + xa,
            ...
            if (yy0 != yy1) {
                ...
                if (level->getBiomeSource()->scaleTemp(temp, floor) >= 0.15f) {
                    ...
                    t->vertexUV(x + xa + 0.5, yy0, z + za + 0.5, 1 * s,
                                yy0 * s / 4.0f + ra * s);
                    t->color(br, br, br, 0.0f);  // 4J - added to soften the top
                                                 // visible edge of the rain
