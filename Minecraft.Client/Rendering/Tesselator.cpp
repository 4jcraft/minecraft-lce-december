...
void Tesselator::end() {
...
#endif
    {
        ...
        if (!hasColor) {
            ...
            // we have a shader that can cope without them
...
// described above) the 8 uint8_t vertex is followed by a further 8 bytes which
...
#define X11Y11Z10N_Z_SHIFT 22
    ...
    const uint32_t uX =
        ((int32_t(std::max(std::min(((x) * 2047.f - 1.f) * 0.5f, 1023.f),
                           -1024.f)) &
          (X11Y11Z10N_X_MASK >> X11Y11Z10N_X_SHIFT))
         << X11Y11Z10N_X_SHIFT);
    const uint32_t uY =
        ((int32_t(std::max(std::min(((y) * 2047.f - 1.f) * 0.5f, 1023.f),
                           -1024.f)) &
...
void Tesselator::normal(float x, float y, flo
	...
	_normal = _C#elif __PSVITA__1Z// AP - casting a negative value to 'uint8_t' on Vita results in zero. changed to a signed 8 value
