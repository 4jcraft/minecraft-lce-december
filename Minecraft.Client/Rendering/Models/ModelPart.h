...
public:
    ...
    ModelPart(Model *model, int xTexOffs, int yTexOffs// MGH - had to add these for PS3, as calling constructors from others was only introduced in c++11 - https://en.wikipedia.org/wiki/C++11#Object_construction_improvement
	...
	ModelPart *addBoxWithMask(float x0, float y0, float z0, int w, int h, int d, int faceMas// 4J added
    void addBox(float x0, float y0, float z0, int w, int h, int d, float g);
	void addHumanoidBox(float x0, float y0, float z0, int w, int h, int d, float// 4J - to flip the poly 3 uvs so the skin maps correctly
