...
void ModelHorse::render(std::shared_ptr<Entity> entity, float time, float r,
    ...
    if (saddled) {
        ...
        if (!adult) {
            ...
            void ModelHorse::setRotation(ModelPart * model, float x, float y,
                                         float z) {
                model->xRot = x;
                model->yRot = y;
                model->zRot = z;
            ...
            void ModelHorse::prepareMobModel(std::shared_ptr<LivingEntity> mob,
                                             float wp, float ws, float a) {
	...
	if (ws > 0.2f)
	{
		...
		Head->y = standing * -6.f + eating * 11.0f + (1.0f - std::max(standing, eating)) * Head->y;
		Head->z = standing * -1.f + eating * -10.f + (1.0f - std::max(standing, eating)) * Head->z;
	...
	*/
	{
                    ...
                    Leg1B->y = Leg1A->y +
                               (Mth::sin(r90 + standAngle +
                                         iStanding * (-legAnim1 * 0.5f * ws)) *
                                7.f);
                    Leg1B->z = Leg1A->z +
                               (Mth::cos(r270 + standAngle +
                                         iStanding * (-legAnim1 * 0.5f * ws)) *
                                7.f);
                    ...
                    float llegRot = (r300 + -bobValue) * standing +
                                    -legXRotAnim * iStanding;
                    ...
                    Leg1A->xRot =
                        standAngle + (-legAnim1 * 0.5f * ws) * iStanding;
                    Leg1B->xRot = (-5 * Mth::DEGRAD) * standing +
                                  ((-legAnim1 * 0.5f * ws) -
                                   std::max(0.0f, legAnim1 * .5f * ws)) *
                                      iStanding;
                    Leg1C->xRot = Leg1B->xRot;

                    Leg2A->xRot =
                        standAngle + (legAnim1 * 0.5f * ws) * iStanding;
                    Leg2B->xRot = (-5 * Mth::DEGRAD) * standing +
                                  ((legAnim1 * 0.5f * ws) -
                                   std::max(0.0f, -legAnim1 * .5f * ws)) *
                                      iStanding;
                    ...
                    Leg3B->xRot =
                        (Leg3A->xRot +
                         PI * std::max(0.0f, (.2f + bobValue * .2f))) *
                            standing +
                        (legXRotAnim + std::max(0.0f, legAnim1 * 0.5f * ws)) *
                            iStanding;
			...
			SaddleL->xRot = -60 / 57.29578f;
                    ...
                    SaddleR->zRot = -legXRotA  // TODO: Magic number2->zRot =
                                               // -legXRotAnim / 5.f;
                }
            }
