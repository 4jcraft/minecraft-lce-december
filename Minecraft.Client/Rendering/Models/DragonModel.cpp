...
#include "../../../Minecraft.World/Entities/Mobs/EnderDragon.h"
...
void DragonModel::render(std::shared_ptr<Entity> entity, float time, float r,
    ...
    for (int i = 0; i < 5; i++) {
        ...
        neck->yRot =
            rotWrap(dragon->getHeadPartYRotDiff(i, start, p))* PI /
            180.0f * rotSc  // 4J replaced "p[0] - start[0] with call to
                            // getHeadPartYRotDiff
                neck->xRot =
                rr +
                (float)(dragon->getHeadPartYOffset(i, start, p))* PI /
                    180.0f * rotScale* 5  // 4J replaced "p[1] - start[1]" with
                                          // call to getHeadPartYOffset
    ...
    head->yRot = rotWrap(dragon->getHeadPartYRotDiff(6, start, p))* PI /
                 180.0f  // 4J replaced "p[0] - start[0] with call to
                         // getHeadPartYRotDiff
                 head->xRot =
                     (float)(dragon->getHeadPartYOffset(6, start, p))* PI /
                     180.0f * rotScale* 5  // 4J Added
