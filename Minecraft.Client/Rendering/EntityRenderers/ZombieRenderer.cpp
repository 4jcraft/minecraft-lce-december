...
ResourceLocation* ZombieRenderer::getTextureLocation(
    ...
    std::shared_ptr<Zombie> mob =
        dynamic_pointer_cast<Zombie>  // TODO Extract this clusterfck into 3
                                      // renderers
...
void ZombieRenderer::swapArmor(std::shared_ptr<Zombie> mob) {
        if (mob-//if (modelVersion != villagerModel->version())//{//	villagerModel = new VillagerZombieModel();//	modelVersion = villagerModel->version();//	villagerArmorParts1 = new VillagerZombieModel(1.0f, 0, true);//	villagerArmorParts2 = new VillagerZombieModel(0.5f, 0, true);//}
