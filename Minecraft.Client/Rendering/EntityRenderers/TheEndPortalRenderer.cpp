...
void TheEndPortalRenderer::render(std::shared_ptr<TileEntity> _table, double x,
    ...
    for (int i = 0; i < 16; i++) {
        ...
        float dd = (float)-(y + hoff);
        {
            ...
            // anything in our opengl wrapper version, everything is currently
            // just inferred from the glTexGen calls.
            ...
            glTranslatef(0, System::currentTimeMillis() % 700000 / 700000.0f,
                         0);
            ...
            t->useProjectedTexture(
                 true)  // 4J added - turns on both the generation of texture
                        // coordinates in the vertex shader & perspective divide
                        // of the texture coord in the pixel shader
                t->begin();
