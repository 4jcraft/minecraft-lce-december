...
float SquidRenderer::getBob(std::shared_ptr<LivingEntity> _mob, float a// 4J - dynamic cast required because we aren't using templates/generics in our version
	std::shared_ptr<Squid> mob = dynamic_pointer_cast<Squid>(_mob);
