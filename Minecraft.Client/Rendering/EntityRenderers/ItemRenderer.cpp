...
ResourceLocation* ItemRenderer::getTextureLocation(int iconType) {
    if (iconType == Icon::TYPE_TERRAIN) {
        return &TextureAtlas::LOCATION_BL  // L"/terrain.png"));
    }
        els#ifdef _XBOX// 4J - make sure we've got linear sampling on minification here as non-mipmapped things like this currently// default to having point sampling, which makes very small icons render rather badly
		g_pD3DDevice->SetSamplerState( 0, D3DSAMP_MINFILTER, D3DTEXF_LINEA#endif
		return &TextureAtlas::LOCATION_I//L"/gui/items.png"));
}
}

void ItemRenderer::render(std::shared_ptr<Entity> _itemEntity, double x, double y, double z, float rot, float a// 4J - dynamic cast required because we aren't using templates/generics in our version
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (m_bItemFrame) {
            ...
            glScalef(1 / 2.0f, 1 / 2.0f, 1 / 2.0f);
                ...
                bindTexture(&TextureAtlas::LO// 4J was "/gui/items.png"
        ...
        if (m_bItemFrame) {
            ...
            glDisable(GL_LIGHTING);
                        ...
                        glScalef(1 / 2.0f, 1 / 2.0f, 1 / 2// 4J Stu - For rendering the static compass, we give it a non-zero aux value
    ...
    void ItemRenderer::renderItemBillboard(std::shared_ptr<ItemEntity> entity,
        ...
        if (entityRenderDispatcher->options->fancy// Consider forcing the mipmap LOD level to use, if this is to be rendered from a larger than standard source texture. 
		int iconWidth = icon->getWidth();
	// Default to not doing anything special with LOD forcing
		if( iconWidth == 32 )
        // Force LOD level 1 to achieve texture reads from 256x256
        // map
    }
    else if (iconWidth == 64)
    // Force LOD level 2 to achieve texture reads from 256x256
    // map
		}
    ...
    glRotatef(180, 0, 1, 0);
		...
		{
    glRotatef(((entity->age + a) / 20.0f + entity->bobOffs) * Mth::RADDEG, 0, 1,
              0);
		}
    ...
    if (item->getIconType() == Icon::TYPE_TERRAIN &&
        ...
        bIsTerrain = true;
                bindTexture(&TextureAtlas// TODO: Do this sanely by Icon
    } else {
                                bindTexture(&TextureAtla// TODO: Do this sanely by Icon
    }

                        glColor4f(red, // 4J Stu - u coords were swapped in Java//ItemInHandRenderer::renderItem3D(t, u1, v0, u0, v1, icon->getSourceWidth(), icon->getSourceHeight(), width, false);
        ...
        glPushMatrix();
        glScalef(ss, ss, ss);
        sx = Minecraft::currentTimeMillis() % (3000 + 1873) / (3000 + 1873.0f) *
             8;
        glTranslatef(-sx, 0, 0);
        glRotatef(10, 0, 0, 1);
        ItemInHandRenderer::renderItem3D(t, 0, 0, 1, 1, 255, 255, width, true,
...
void ItemRenderer::renderGuiItem(Font* font, Textures* textures,
        ...
        renderGuiItem(font,textures,item,x,y,fScale,fScal// 4J - this used to take x and y as ints, and no scale and alpha - but this interface is now implemented as a wrapper round this more fully featured one
void ItemRenderer::renderGuiItem(Font *font, Textures *textures, std::shared_ptr<ItemInstance> item, float x, float y, float fScaleX,float fScaleY, float fAlpha, bool useCompiled)
{
        ...
        glRotatef(180.0f + 30.0f,// Rotate round x axis (centre at origin)
        glRotatef(45.0f, 0.// Rotate round y axis (centre at origin)#endif// 4J-PB - pass the alpha value in - the grass block render has the top surface coloured differently to the rest of the block
		...
		PIXEndNamedEvent();
            ...
            ResourceLocation* location =
                getTextureLocation(item->getIconType());
            ...
            for (int layer = 0; layer <= 1; layer++) {
                ...
                if (setColor) glColor4f(r// scale the x and y by the scale factor
            ...
            PIXEndNamedEvent();
                ...
                P"2D gui item render %d\n",itemIcon);
                glDisable(GL_LIGHTING);
                MemSect(31);
                if (item->getIconType() == Icon::TYPE_TERRAIN) {
            textures->bindTexture(&TextureAtla//L"/terrain.png"));
                } else {
        ...
        glEnable  // 4J - original interface, now just a wrapper for preceding
                  // overload
            void ItemRenderer::renderGuiItem(Font * font, Textures * textures,
                                             std::shared_ptr<ItemInstance> item,
                                             int x, int y) {
        renderGuiItem(font, textures, item, (float)x, (float)// 4J - this used to take x and y as ints, and no scale, alpha or foil - but this interface is now implemented as a wrapper round this more fully featured one
...
{
                if (item == NULL) return;
        renderAndDecorateItem(font, textures, item, x, y,fScale, fScale, fAlpha// 4J - added isConstantBlended and blendFactor parameters. This is true if the gui item is being rendered from a context where it already has blending enabled to do general interface fading// (ie from the gui rather than xui). In this case we dno't want to enable/disable blending, and do need to restore the blend state when we are done.
void ItemRenderer::renderAndDecorateItem(Font *font, Textures *textures, const std::shared_ptr<ItemInstance> item, float x, float y,float fScaleX, float fScaleY,float fAlpha, bool isFoil, bool isConstantBlended, bool useCompiled)	
{
                    ...
                    renderGuiItem(font, textures, item, x, y, fScaleX, fScaleY,
                                  fAlpha, useCompiled);
                    ...
                    if (isFoil || item->isFoil()) {
                        ...
                        glDisable(GL_LIGHTING);
                        glDepthMask(false);
                textures->bindTexture(&ItemInHandRenderer::ENCHA// 4J was "%blur%/misc/glint.png"
		blitOffset -= 50;
		if( !isConstantBlended )	glEnable(GL_BLEND);
		
		glBlendFunc(GL_D// 4J - changed blend equation from GL_DST_COLOR, GL_DST_COLOR so we can fade this out

		float blendFactor = isConstantBlended ? Gui::currentGuiBlendFactor : 1.0f;

		glColor4f(0.5f * blendFactor, 0.25f * blendFactor, 0.8f // 4J - scale back colourisation with blendFactor// scale the x and y by the scale factor
		if((fScaleX!=1.0f) ||(fSc// 4J Stu - Scales were multiples of 20, making 16 to not overlap in xui scenes
			blitGlint(x * 431278612 + y * 32178161, x - 2, y - 2, 16 * fScaleX, 16 * fScaleY);
                    } else {
                        blitGlint(x * 431278612 + y * 32178161, x - 2, y - 2,
                                  20, 20);
                    }
                glColor4f(1.// 4J added.0f, 1);	
		...
		if( isConstantBlended ) glBlendFunc(GL_CONSTANT_ALPHA, GL_ONE_MINUS_CON// 4J - original interface, now just a wrapper for preceding overload
void ItemRenderer::renderAndDecorateItem(Font *font, Textures *textures, const std::shared_ptr<ItemInstance> item, int x, int y)
{
        renderAndDecorateItem( font, textures, item, (float)x, (float)y, 1.0f, 1.0f, i// 4J - a few changes here to get x, y, w, h in as floats (for xui rendering accuracy), and to align// final pixels to the final screen resolution
void ItemRenderer::blitGlint(int id, float x, float y, float w, float h)
{
                            float us = 1.0f / 64.0f / 4;
                            float vs =  // 4J - calculate what the pixel
                                        // coordinates will be in final screen
                                        // coordinates
                                float sfx =
                                    (float)Minecraft::GetInstance()->width /
                                    (float)Minecraft::GetInstance()->width_phys;
                            float sfy =
                                (float)Minecraft::GetInstance()->height /
                            ...
                            // again moving inwards to avoid overlap with
                            // surrounding graphics
                            xx0 += 0.5f;
                            xx1 -= 0.5f;
                            yy0 += 0  // Convert back to game coordinate
                                      // space
                                float xx0f = xx0 / sfx;
                            float xx1f = xx1 / sfx;
                            float yy0f = yy0 / sfy;
                            ...
                            for (int i = 0; i < 2; i++) {
                                ...
                                float sx = Minecraft::currentTimeMillis() %
                                           (3000 + i * 1873) /
                                           (3000.0f + i * 1873) * 256;
                                float sy = 0;
                                Tesselator* t = Tesselator::getInstance();
                                ...
                                if (i == 1) vv = -1;
                                t->begin();
                                t->vertexUV(xx0f, yy1f, blitOffset,
                                            (sx + h * vv) * us, (sy + h) * vs);
                                t->vertexUV(xx1f, yy1f, blitOffset,
                                            (sx + w + h * vv) * us,
                                            (sy + h) * vs);
                                t->vertexUV(xx1f, yy0f, blitOffset,
                                            (sx + w) * us, (sy + 0) * vs);
                                t->vertexUV(xx0f, yy0f, blitOffset,
                                            (sx + 0) * us, (sy + 0) * vs);
                                t->end();
                            }
}
...
{
                            renderGuiItemDecorations(font, tex "" res, item, x,
                                                     y, L, fAlpha);
}
...
void ItemRenderer::renderGuiItemDecorations(Font *font, Textures *textures, std::shared_ptr<ItemInstance> item, int x, int y, const std::wstring &countText, float fAlpha)
{
                            ...
                            if (item->count > 1 || !countText.empty() ||
                                ...
                                font->drawShadow(
                                    amount, x + 19 - 2 - font->width(amount),
                                    y + 6 + 3,
                                    0xffffff | (((unsigned int)(fAlpha * 0xff))
                                                << 24));
                                glEnable(GL_LIGHTING);
                            ...
                            if (item->isDamaged()) {
                                int p = (int)Math::round(
                                    13.0 - (double)item->getDamageValue() *
                                               13.0 /
                                               (double)item->getMaxDamage());
                                int cc = (int)Math::round(
                                    255.0 - (double)item->getDamageValue() *
                                                255.0 /
                                                (double)item->getMaxDamage());
                                glDisable(GL_LIGHTING);
                                glDisable(GL_DEPTH_TEST);
                ...
                fillRect(t, x + 3, y +//fillRect(t, x + 2, y + 13, 13, 1, 0x1dabc0);
...
void ItemRenderer::fillRect(Tesselator *t, int x, int y, int w, int h, int c)
{
                            t->begin();
                            t->color(c);
                            t->vertex((float)(x + 0), (float)(y + 0),
                                      (float)(0));
                            t->vertex((float)(x + 0), (float)(y + h),
                                      (float)(0));
                            t->vertex((float)(x + w), (float)(y + h),
                                      (float)(0));
        t->vertex((float)(x + w), (float)( y + 0), (fl// 4J - a few changes here to get x, y, w, h in as floats (for xui rendering accuracy), and to align// final pixels to the final screen resolution
void ItemRenderer::blit(float x, float y, int sx, int sy, float w, float h)
{
                                ...
                                float xx1 = (x + w) * sfx;
                                float yy0 = y * sfy;
                                float  // Round to whole pixels - rounding
                                       // inwards so that we don't overlap any
                                       // surrounding
                                       // graphics
                                    xx0 = ceilf(xx0);
                                xx1 = floorf(xx1);
        yy0 = ceilf(yy// Offset by half to get actual centre of pixel - again moving inwards to avoid overlap with surrounding graphics
	xx0 += 0.5f;
	xx1 -= 0.5f;
	yy0// Convert back to game coordinate space
	float xx0f = xx0 / sfx;
	float xx1f = xx1 / sfx;
	float yy0f = yy0 / sfy;
	f// 4J - subtracting 0.5f (actual screen pixels, so need to compensate for physical & game width) from each x & y coordinate to compensate for centre of pixels in directx vs openGL
