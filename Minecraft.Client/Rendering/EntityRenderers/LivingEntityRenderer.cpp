...
void LivingEntityRenderer::render(std::shared_ptr<Entity> _mob, double x,
        ...
        armor->young = model->youn /*try*/
	{
            ...
            if (mob->isRiding() &&
                mob->riding->instanceof(eTYPE_LIVINGENTITY)) {
            ...
            for (int i = 0; i < MAX_ARMOR_LAYERS; i++) {
                ...
                if (armorType > 0) {
                    ...
                    if ((armorType & 0xf0) == 16) {
                        ...
                        armor->render(mob, wp, ws, bob, headRot - bodyRot,
                                      headRotx, fScale, true);
                        // 4J - added condition here for rendering player as
                        // part of the gui. Avoiding rendering the glint here as
                        // it involves using its own blending, and for gui
                        // rendering// we are globally blending to be able
                        // to offer user configurable gui opacity. Note that I
                        // really don't know why GL_BLEND is turned off at the
                        // end of the first// armour layer anyway, or why
                        // alpha testing is turned on... but we definitely don't
                        // want to be turning blending off during the gui
                        // render.
                        if (!entityRenderDispatcher->isGuiRender) {
                            if ((armorType & 0xf) == 0xf) {
                                ...
                                for (int j = 0; j < 2; j++) {
                                    ...
                                    float brr = 0.76f;
                                    glColor4f(0.5f * brr, 0.25f * brr,
                                              0.8f * brr, 1);
                                    glBlendFunc(GL_SRC_COLOR, GL_ONE);
                                    glMatrixMode(GL_TEXTURE);
                                    glLoadIdentity();
                                    float uo =
                                        time * (0.001f + j * 0.003f) * 20;
                                    float ss = 1 / 3.0f;
                                    glScalef(ss, ss, ss);
                                    glRotatef(30 - (j) * 60.0f, 0, 0, 1);
                                    glTranslatef(0, uo, 0);
                                    glMatrixMode(GL_MODELVIEW);
                                    armor->render(mob, wp, ws, bob,
                                                  headRot - bodyRot, headRotx,
                                                  fScale, false);
                                }
                ...
                if (((overlayColor >> 24) & 0xff) > 0 || mob->hurtTime > 0 ||
                    ...
                    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
                        glDepthFunc(GL_EQ// 4J - changed these renders to not use the compiled version of their models, because otherwise the render states set// about (in particular the depth & alpha test) don't work with our command buffer versions
			if (mob->hurtTime > 0 || mob->deathTime > 0)
			{
                        glColor4f(br, 0, 0, 0.4f);
                        model->render(mob, wp, ws, bob, headRot - bodyRot,
			...
			glEnable(GL_TEXTURE_2D);
                ...
                glDisable(GL_RESCALE_NOR/* catch (Exception e)
	{
	e.printStackTrace();
	}*/
	...
	MemSect(0);
            ...
            void LivingEntityRenderer::renderModel(
                std::shared_ptr<LivingEntity> mob, float wp, float ws,
            ...
            void LivingEntityRenderer::setupPosition(
                std::shared_ptr<LivingEntity> mob, double x, double y,
                double z) {
                glTranslatef((float)x, (float)y, (float)z);
            }

            void LivingEntityRenderer::setupRotations(
                std::shared_ptr<LivingEntity> mob, float bob, float bodyRot,
                float a) {
...
"Dinnerbone"= L"Grumm"ame == L)
    ...
    if (!mob->instanceof(eTYPE_PLAYER) ||
        ...
        glRotatef(180, 0, 0, 1);
            ...
            float LivingEntityRenderer::getAttackAnim(
                std::shared_ptr<LivingEntity> mob, float a) {
                return mob->getAttackAnim(a);
            }

            float LivingEntityRenderer::getBob(
                std::shared_ptr<LivingEntity> mob, float a) {
            ...
            void LivingEntityRenderer::renderArrows(
                std::shared_ptr<LivingEntity> mob, float a) {
                ...
                if (arrowCount > 0) {
                    ...
                    for (int i = 0; i < arrowCount; i++) {
                        glPushMatrix();
                        ModelPart* modelPart =
                            model->getRandomModelPart(random);
                        Cube* cube = modelPart->cubes[random.nextInt(
                            modelPart->cubes.size())];
                        modelPart->translateTo(1 / 16.0f);
                        float xd = random.nextFloat();
                        float yd = random.nextFloat();
                        float zd = random.nextFloat();
                        float xo =
                            (cube->x0 + (cube->x1 - cube->x0) * xd) / 16.0f;
                        float yo =
                            (cube->y0 + (cube->y1 - cube->y0) * yd) / 16.0f;
                        float zo =
                            (cube->z0 + (cube->z1 - cube->z0) * zd) / 16.0f;
                        ...
                        arrow->yRotO = arrow->yRot =
                            (float)(atan2(xd, zd) * 180 / PI);
                        arrow->xRotO = arrow->xRot =
                            (float)(atan2(yd, sd) * 180 / PI);
                        double x = 0;
                        double y = 0;
                        double z = 0;
                        float yRot = 0;
            ...
            int LivingEntityRenderer::prepareArmorOverlay(
                std::shared_ptr<LivingEntity> mob, int layer, float a) {
                return prepareArmor(mob, layer, a);
            }

            int LivingEntityRenderer::prepareArmor(
                std::shared_ptr<LivingEntity> mob, int layer, float a) {
                return -1;
            }
            ...
            float LivingEntityRenderer::getFlipDegrees(
                std::shared_ptr<LivingEntity> mob) {
                return 90;
            }

            int LivingEntityRenderer::getOverlayColor(
                std::shared_ptr<LivingEntity> mob, float br, float a) {
                return 0;
            }

            void LivingEntityRenderer::scale(std::shared_ptr<LivingEntity> mob,
                                             float a) {}

            void LivingEntityRenderer::renderName(
                std::shared_ptr<LivingEntity> mob, double x, double y,
                double z) {
                if (shouldShowName(mob) || Minecraft::renderDebug()) {
                    ...
                    double dist = mob->distanceToSqr(
                        entityRenderDispatcher->cameraEntity);
                    ...
                    if (dist < maxDist * maxDist) {
                        ...
                        if (!msg.empty()) {
                            if (mob->isSneaking()) {
                                        if ( app.GetGameSettings(eGameSetting_// 4J-PB - turn off gamertag render 
						return;
                            }

                                        if(app.GetGameHostOption(eGameHostOptio// turn off gamertags if the host has set them off
						return;
                        }

                        Font* font = getFont();
                        glPushMatrix();
                        glTranslatef((float)x + 0,
                                     (float)y + mob->bbHeight + 0.5f, (float)z);
                        glNormal3f(0, 1, 0);
...
void LivingEntityRenderer::renderNameTags(std::shared_ptr<LivingEntity> mob,
    ...
    if (mob->isSleeping()) {
        renderNameTag(mob, msg, x, y - 1.5f, z, 64);
                ...
                renderNameTag(// 4J Added parameter for color here so that we can colour players names
void LivingEntityRenderer::renderNameTag(std::shared_ptr<LivingEntity> mob, const std::wstring &name, double x, double y, dou/*= 0xff000000*/st, int color )
{
        if ( app.GetGameSettings(eGame// 4J-PB - turn off gamertag render
		return;
	}

	if(app.GetGameHostOption(eGameH// turn off gamertags if the host has set them off
		return;
    }
    ...
    glRotatef(this->entityRenderDispatcher->playerRotX, 1, 0, 0);
        ...
        glScalef(-s, -s, s// 4J Stu - If it's beyond readable distance, then just render a coloured box
            ...
            case Player::ePlayerNameValid_NotSet:
                if (font->AllCharactersValid(name)) {
                    ...
                    player->SetPlayerNameValidState(true);
                                ...
                                memset(wchName,0,size"%d"CHAR)*2);
