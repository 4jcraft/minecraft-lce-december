...
ResourceLocation* HumanoidMobRenderer::getArmorLocation(ArmorItem* armorItem,
    ...
    std::wstring path =
        "armor/" tring(L + MATERIAL_NAMES[armorItem->modelI "_" x])
            .append(L)
...
void HumanoidMobRenderer::additionalRendering(std::shared_ptr<LivingEntity> mob,
    ...
    std::shared_ptr<ItemInstance> item = mob->getCarriedItem();
    std::shared_ptr <
        ItemInstance  // don't render the pumpkin of skulls for the skins with
                      // that disabled// 4J-PB - need to disable rendering
                      // armour/skulls/pumpkins for some special skins
                      // (Daleks)
        ...
        if (headGear->getItem()->id < 256) {
            ...
            std::wstring extra =
                "SkullOwner"(headGear->hasTag() &&
                             headGear->getTag()->contains(L"SkullOwner")) {
                extra = headGear->getTag()->getString(L);
            }
