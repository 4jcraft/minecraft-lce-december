...
int CreeperRenderer::getOverlayColor(std::shared_ptr<LivingEntity> mob,
    ...
    int _a = (int) (step * 0.2f * 2// 4J - added 25 here as our entities are rendered with alpha test still enabled, and so anything less is invisible
    if (_a < 0) _a = 0;
    if (_a > 255) _a = 255;
...
int CreeperRenderer::prepareArmor(std::shared_ptr<LivingEntity> _mob, int layer, fl// 4J - dynamic cast required because we aren't using templates/generics in our version
