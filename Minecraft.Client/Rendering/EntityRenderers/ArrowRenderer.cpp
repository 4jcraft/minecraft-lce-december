...
void ArrowRenderer::render(std::shared_ptr<Entity> _arrow, double x, double y,
    ...
    bindTexture(_arr// 4J - was L"/item/arrows.png"
    ...
    glTranslatef(-4, 0, //    glNormal3f(ss, 0, 0);		// 4J - changed to use tesselator
    t->begin();
	t->normal(1,0,0);
    ...
    t->end//    glNormal3f(-ss, 0, 0);	// 4J - changed to use tesselator
    t->begin();
	t->normal(-1,0,0);
    t->vertexUV((float)(-7), (float)( +2), (float)( -2), (float)( u02), (float)( v02));
    t->vertexUV((float)(-7), (float)( +2), (float)( +2), (float)( u12), (float)( v02));
    ...
    for (int i = 0; i < 4; i++)
	{
        glRotatef(90, 1, 0,//        glNormal3f(0, 0, ss);		// 4J - changed to use tesselator
        t->begin();
		t->normal(0,0,1);
