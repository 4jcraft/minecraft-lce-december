...
void ProgressRenderer::_progressStart(int // 4J Stu - Removing all progressRenderer rendering. This will be replaced on the xbox
    if (!minecraft->running)
	{
        if (noAbort//        throw new StopGameException();		// 4J - removed
    }
	
	EnterCriticalSection( &ProgressRenderer::s_progress );
	lastPercent = 0;
    this->title = title;
	LeaveCriticalSection( &ProgressRenderer::s_pr
#if 0);
...
void ProgressRenderer::progressStagePercentage(// 4J Stu - Removing all progressRenderer rendering. This will be replaced on the xbox
	EnterCriticalSection( &ProgressRenderer::s_progress );
	lastPercent = i;
	LeaveCriticalSection( &ProgressRenderer::s_pr
#if 0);
    ...
    int id = minecraft->textures->loa"/gui/background.png");
