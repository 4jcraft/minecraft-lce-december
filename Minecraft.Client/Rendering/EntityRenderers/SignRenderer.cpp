...
void SignRenderer::render(std::shared_ptr<TileEntity> _sign, double x, double y,
    ...
    bindTexture(&SIGN_LOCATI// 4J was L"/item/sign.png"
	...
	std::wstr// need to send the new data// Get the current language setting from the console
	DWORD dwLanguage = XGetLanguage( );

    for (int i = 0; i < MAX_SIGN_LIN// 4J - was sign.messages.length
	{
        if (sign->IsVerified()) {
            if (sign->IsCensored()) {
                switch (dwLanguage) {
        ...
        if (i == sign->GetSelectedLine()) {
            "> " sg = L" <" sg + L;
            font->draw(msg, -font->width(msg) / 2, i * 10 - (MAX_SIGN_LINES) * // 4J - (MAX_SIGN_LINES) was sign.messages.length
        } else {
            font->draw(msg, -font->width(msg) / 2, i * 10 - (MAX_SIGN_LINES) * // 4J - (MAX_SIGN_LINES) was sign.messages.length
        }
    }
