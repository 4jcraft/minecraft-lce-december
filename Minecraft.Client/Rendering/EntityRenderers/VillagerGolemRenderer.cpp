...
void VillagerGolemRenderer::additionalRendering(std::shared_ptr<LivingEntity> _mob, float a// 4J - original version used generics and thus had an input parameter of type Blaze rather than shared_ptr<Entity>  we have here - // do some casting around instead
	std::shared_ptr<VillagerGolem> mob = dynamic_pointer_cast<VillagerGolem>(_mob);
	MobRenderer::additionalRendering(mob, a);
	if (mob->getOfferFlowerTick() == 0) return;

	glEnable(GL_RESCALE_NORMAL);
	glPushMa// dont ask me how I got the flower into his hand.
	...
	glColor4f(1, 1, 1, 1);
	bindTexture(&TextureAtlas::LOCATION// TODO: By Icon
