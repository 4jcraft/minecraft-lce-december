...
int PlayerRenderer::prepareArmor(std::shared_ptr<LivingEntity> _player,
    ...
    // our version
        std::shared_ptr<Player> player = dynamic_pointer_cast<Player>(_player// 4J-PB - need to disable rendering armour for some special skins (Daleks)
...
void PlayerRenderer::prepareSecondPassArmor(std::shared_ptr<LivingEntity> _player, int layer, fl// 4J - dynamic cast required because we aren't using templates/generics in our version
    ...
    if (dynamic_cast<ArmorItem*>(item)) {
        ...
        float brightness =
            SharedConstants::TEXTURE_LIGHTING ? 1 : player->getBrightness(a);
        glColor3f(brightness, brightness, brightness);
    }
	}
}

void PlayerRenderer::render(std::shared_ptr<Entity> _mob, double x, double y, double z, flo// 4J - dynamic cast required because we aren't using templates/generics in our version
        ...
        if (anim == UseAnim_block) {
            armorParts1->holdingRightHand = armorParts2->holdingRightHand =
                humanoidModel->holdingRightHand = 3;
                                ...
                                armorParts1->bowAndArrow = armorParts2->bowAndArrow = humanoidModel->bow// 4J added, for 3rd person view of eating
	if( item != NULL && mob->getUseItemDuration() > 0 && item->getUseAnim// These factors are largely lifted from ItemInHandRenderer to try and keep the 3rd person eating animation as similar as possible
        ...
        if (mob->isSneaking() && !mob->instanceof(eTYPE_LOCALPLAYER)) {
            // Check if an idle animation is
            // needed
            if (mob->getAnimOverrideBitmask() &
                (1 << HumanoidModel::eAnim_HasIdle)) {
                if (mob->isIdle()) {
	...
	if (pAdditionalModelParts && pAdditionalModelParts->size() != 0) {
            ...
            void PlayerRenderer::additionalRendering(
                std::shared_ptr<LivingEntity> _mob, float a) {
                float brightness = SharedConstants::TEXTURE_LIGHTING
                                       ? 1
                                       : _mob->getBrightness(a);
                glColor3f(brightness, brightness, brightness);

                LivingEntityRenderer::additionalRendering(_mob, a);
                LivingEntity  // 4J - dynamic cast required because we aren't
                              // using templates/generics in our
                              // version
                    std::shared_ptr<Player>
                        mob = dynamic_pointer_cast<Player>(_mob);
		...
		unsigned int uiAnimOverrideBitmask =
                ...
                if ((uiAnimOverrideBitmask &
                    ...
                    if (headGear->getItem()->id < 256) {
                        ...
                        std::wstring extra = L;
                        if (he "SkullOwner" ag() &&
                            headGear->getTag()->contains(L))
                            "SkullOwner" xtra =
                                headGear->getTag()->getString(L);
                    }
                                SkullTileRenderer::instance->renderSkull(-0.5f, 0, -0.5f, Facing::UP, 180, headGear->getAuxVal// need to add a custom texture for deadmau5  }

	
	if (mob != NULL && app.isXuidDeadmau5( mob->g""Xuid() ) && bindTexture(mob->customTextureUrl, L ))
	{
                        for (int i = 0; i < 2; i++) {
                            float yr =
                                (mob->yRotO + (mob->yRot - mob->yRotO) * a) -
                                (mob->yBodyRotO +
                                 (mob->yBodyRot - mob->yBodyRotO) * a);
                            float xr =
                                mob->xRotO + (mob->xRot - mob->xRotO) * a;
                            glPushMatrix();
                            glRotatef(yr, 0, 1, 0);
                            glRotatef(xr, 1, 0, 0);
                            ...
                            float s = 8 / 6.0f;
                            glScalef(s, s, s);
            humanoidModel->renderEars(1 / 16.0f,tr// 4J: removed  /*boolean loaded = mob->getCloakTexture()->isLoaded();
	...
	if (bindTexture(mob->customTextureUrl2, L) && !mob->isInvisible())
	{
                                ...
                                double xd = (mob->xCloakO +
                                             (mob->xCloak - mob->xCloakO) * a) -
                                            (mob->xo + (mob->x - mob->xo) * a);
                                double yd = (mob->yCloakO +
                                             (mob->yCloak - mob->yCloakO) * a) -
                                            (mob->yo + (mob->y - mob->yo) * a);
                                double zd = (mob->zCloakO +
                                             (mob->zCloak - mob->zCloakO) * a) -
                                            (mob->zo + (mob->z - mob->zo) * a);

                                float yr = mob->yBodyRotO +
                                           (mob->yBodyRot - mob->yBodyRotO) * a;
                                ...
                                float pow =
                                    mob->oBob + (mob->bob - mob->oBob) * a;

                                flap +=
                                    sin((mob->walkDistO +
                                         (mob->walkDist - mob->walkDistO) * a) *
                                        6) *
                                    32 * pow;
        if (mob->isSne// 4J Stu - Fix for sprint-flying causing the cape to rotate up by 180 degrees or more
    ...
    if (item != NULL)
	{
                                ...
                                if (mob->fishing != NULL) {
            item = std::shared_ptr<ItemInstance>( new ItemInstan//null;::stick) );
                                }
		...
		if (mob->getUseItemDuration() > 0) {
                                ...
                                if (item->id < 256 &&
                                    ...
                                    float s = 10 / 16.0f;
                                    glTranslatef(0 / 16.0f, 2 / 16.0f,
                                                 5 / 16.0f);
                                    glRotatef(-20, 0, 1, 0);
                                    glScalef(s, -s, s);
                                    glRotatef(-100, 1, 0, 0);
                                    glRotatef(45, 0, 1, 0);
                                } else if (Item::items[item->id]
                                               ->isHandEquipped()) {
                                    float s = 10 / 16.0f;
                        ...
                        void PlayerRenderer::renderNameTags(
    ...
    if (dist < 10 * 10) {
                                ...
                                Objective* objective =
                                    scoreboard->getDisplayObjective(
                                        Scoreboard::DISPLAY_SLOT_BELOW_NAME);

                                if (objective != NULL) {
                                    Score* score = scoreboard->getPlayerScore(
                                        player->getAName(), objective);
    ...
    LivingEntityRenderer::renderNameTags(player, x, y, z, msg, scale, dist);
                ...
                void PlayerRenderer::scale(std::shared_ptr<LivingEntity> player,
                                           float a) {
                    float s = 15 / 16.0f;
                    glScalef(s, s, s);
                }
                ...
                void PlayerRenderer::renderHand() {
                    ...
                    armorParts1->eating = armorParts2->eating =
                        humanoidModel->eating = humanoidModel->idle = false;
                    humanoidModel->attackTime = 0;
    humanoidModel->setupAnim(0,// 4J-PB - does this skin have its arm0 disabled? (Dalek, etc)
...
void PlayerRenderer::setupPosition(std::shar// 4J - dynamic cast required because we aren't using templates/generics in our version
	std::shared_ptr<Player> mob = dynamic_pointer_cast<Player>(_mob);

    if (mob->isAlive() && mob->isSleeping())
	{
                    LivingEntityRenderer::setupPosition(
                        mob, x + mob->bedOffsetX, y + mob->bedOffsetY,
                        z + mob->bedOffsetZ);
...
void PlayerRenderer::setupRotations(std::shar// 4J - dynamic cast required because we aren't using templates/generics in our version
	std::shared_ptr<Player> mob = dynamic_pointer_cast<Player>(_mob);
...
void PlayerRenderer::renderShadow(std::shared_ptr<Entity> e, double x, double y,
                ...
                if(player != NULL && player->hasInvisib// 4J Added overriden;
                    }
...
void PlayerRenderer::bindTexture(std::shared_ptr<Entity> entity) {
                ...
                ResourceLocation* PlayerRenderer::getTextureLocation(
                    ...
                    return new ResourceLocation(
                        (_TEXTURE_NAME)player->getTexture());
                }