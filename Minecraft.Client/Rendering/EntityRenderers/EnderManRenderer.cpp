...
void EndermanRenderer::additionalRendering(std::shared_ptr<LivingEntity> _mob, float a// 4J - original version used generics and thus had an input parameter of type Boat rather than shared_ptr<Entity>  we have here - // do some casting around instead
	std::shared_ptr<EnderMan> mob = dynamic_pointer_cast<EnderMan>(_mob);
    ...
    glColor4f(1, 1, 1, 1);
                bindTexture(&TextureAtlas::LOCATION// TODO: bind by icon
...
int EndermanRenderer::prepareArmor(std::shared_ptr<LivingEntity> _mob, int layer, fl// 4J - original version used generics and thus had an input parameter of type Boat rather than shared_ptr<Entity>  we have here - // do some casting around instead
	std::shared_ptr<EnderMan> mob = dynamic_pointer_cast<EnderMan>(_mob);

	if (layer != 0) return -1;

	bindTexture(&ENDERMAN_E// 4J was L"/mob/enderman_eyes.png"
	float br = 1;
	glEnab// 4J Stu - We probably don't need to do this on 360 either (as we force it back on the renderer)// However we do want it off for other platforms that don't force it on in the render lib CBuff handling// Several texture packs have fully transparent bits that break if this is off#ifdef _XBOX

    glDisable(G#endifA_TEST);
