...
void SkullTileRenderer::renderSkull(float x, float y, float z, int face,
    ...
    switch (type) {
        ...
        case SkullTileEntity::TYPE_ZOMBIE:
                bindTexture(&ZOMBIE_LOCATION//model = zombieModel;
		break;
	case SkullTileEntity::TYPE_CHA//if (!extra.empty())//{		//	wstring url = "http://skins.minecraft.net/MinecraftSkins/" + StringUtil.stripColor(extra) + ".png";//	if (!instance->tileEntityRenderDispatcher->textures->hasHttpTexture(url))//	{	//		instance->tileEntityRenderDispatcher->textures->addHttpTexture(url, new MobSkinTextureProcessor());//	}	//	bindTexture(url, "/mob/char.png");//}		//else
		{
