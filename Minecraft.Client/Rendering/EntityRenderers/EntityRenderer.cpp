...
void EntityRenderer::renderShadow(std::shared_ptr<Entity> e, double x, double y,
    ...
    double ex = e->xOld + (e->x - e->xOld) * a;
        double ey = e->yOld + (e->y - e->yOld) * a + e->getShadowHeightOffs(// 4J-PB - local players seem to have a position at their head, and remote players have a foot position.// get the shadow to render by changing the check here depending on the player type
	...
	for (int xt = x0; xt <= x1; xt++)
        ...
        if (t > 0 && level->getRawBrightness(xt, yt, zt) > 3) {
            renderTileShadow(
                Tile::tiles[t], x,
                y + e->getShadowHeightOffs() + fYLocalPlayerShadowOffset, z, xt,
                yt, zt, pow, r, xo,
                yo + e->getShadowHeightOffs() + fYLocalPlayerShadowOffset, zo);
        }			
			}
	tt->end();
...
void EntityRenderer::renderTileShadow(Tile* tt, double x, double y, double z,
    ...
    if (a > 1) a = 1;
        ...
        t->color(1.0f, 1.0f, 1.0f, (float) // glColor4f(1, 1, 1, (float) a);
	...
	float v1 = (float) ((z - (z1)) / 2 / r + 0.5f// u0 = 0;// v0 = 0;// u1 = 1;// v1 = 1;
...
void EntityRenderer::render(AABB* bb, double xo, double yo, double zo) {
    ...
    t->offset(0, 0, 0);
    t->end();
        glEnable(GL_TEXTURE_2// model.render(0, 1)
}
...
void EntityRenderer::postRender(std::shared_ptr<Entity> entity, double x,
        ...
        if( !entityRenderDispatcher->isGuiRende// 4J - added, don't render shadow in gui as it uses its own blending, and we have globally enabled blending for interface opacity
	{
