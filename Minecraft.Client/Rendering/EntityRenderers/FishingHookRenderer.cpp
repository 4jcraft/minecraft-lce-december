...
void FishingHookRenderer::render(std::shared_ptr<Entity> _hook, double x,
    ...
    int xi = 1;
    int yi = 2;
    bindTexture(hoo// 4J was L"/particles.png"
    ...
    if (hook->owner != NULL)
	{
        ...
        double yOffset =
            hook->owner == dynamic_pointer_cast<Player>(
                               Minecraft::GetInstance()->player)
                ? 0
                : hook->owner
                      ->getHeadHeight()  // 4J-PB - changing this to be per
                                         // player//if
                                         // (this->entityRenderDispatcher->options->thirdPersonView)
