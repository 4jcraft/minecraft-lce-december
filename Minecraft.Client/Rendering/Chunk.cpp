...
void Chunk::rebuild() {
	...
	TileRenderer *tileRenderer = new TileRenderer(region, this->x, this->y, this->z, ti// AP - added a caching system for Chunk::rebuild to take advantage of// Basically we're storing of copy of the tileIDs array inside the region so that calls to Region::getTile can grab data // more quickly from this array rather than calling CompressedTileStorage. On the Vita the total thread time spent in // Region::getTile went from 20% to 4%.	#ifdef __PSVITA__
	int xc = x >> 4;
	int zc = z >> 4;
	((Region*)region)->setCachedTiles(tileIds,#endifc);// We now go through the vertical section of this level chunk that we are interested in and try and establish// (1) if it is completely empty// (2) if any of the tiles can be quickly determined to not need rendering because they are in the middle of other tiles and//     so can't be seen. A large amount (> 60% in tests) of tiles that call tesselateInWorld in the unoptimised version//     of this function fall into this category. By far the largest category of these are tiles in solid regions of rock.
	bool empty = true;
	for( int yy = y0; yy < y1; yy++ )
	{
        for (int zz = 0; zz < 16; zz++) {
            for (int xx = 0; xx < 16;
                 xx++)  // 4J Stu - tile data is ordered in 128 blocks of full
            ...
            unsigned char tileId =
                tileIds[offset +
                        (((xx + 0) << 11) | ((zz + 0) << 7) | (indexY + 0))];
            if (tileId > 0)
                empty = fa  // Don't bother trying to work out neighbours for
                            // this tile if we are at the edge of the chunk -
                            // apart from the very// bottom of the world
                            // where we shouldn't ever be able to see
                    if (yy == (Level::maxBuildHeight - 1)) continue;
            if ((xx == 0) || (xx == 15)) continue;
            if ((zz == 0) || (zz == 15))
                continue;  // Establish whether this tile and its neighbours are
                           // all made of rock, dirt, unbreakable tiles, or have
                           // already// been determined to meet this
                           // criteria themselves and have a tile of 255
                           // set.
            ...
            if (!((tileId == Tile::stone_Id) || (tileId == Tile::dirt_Id) ||
                  (tileId == Tile::unbreakable_Id) || (tileId == 255)))
                cont  // Treat the bottom of the world differently - we
                      // shouldn't ever be able to look up at this, so consider
                      // tiles as invisible// if they are surrounded on
            ...
            tileId =
                tileIds[yPlusOneOffset +
                        (((xx + 0) << 11) | ((zz + 0) << 7) | indexYPlusOne)];
            if (!((tileId == Tile::stone_Id) || (tileId == Tile::dirt_Id) ||
                  (tileId == Tile::unbreakable_Id) || (tileId == 255)))
                conti  // This tile is surrounded. Flag it as not requiring to
...
PIXEndNamed  // Nothing at all to do for this chunk?
    if (emp  // 4J - added - clear any renderer data associated with
             // this
...
for (int currentLayer = 0; currentLayer < 2; currentLayer++) {
    ...
    bool started =  // 4J - changed loop order here to leave y as the innermost
        ...
        for (int x = x0; x < x1; x++) {
            ...
            if (indexY >= Level::COMPRESSED_CHUNK_SECTION_HEIGHT) {
                ...
                // earlier optimisation
                                        unsigned char tileId = tileIds[ offset + ( ( ( x - x0 ) << 11 ) | ( ( z - z0 ) << 7 ) | indexY// If flagged as not visible, drop out straight away
					if( tileId == 0xff ) //					int tileId = region->getTile(x,y,z);
					if (tileId > 0)
					{
                    if (!started) {
                        started = true;
                        ...
                        glPushMatrix();
                        glDepthMa                 // 4J added
                            t->useCompactVertice  // 4J added
                            translateToPos();
                        float ss =
                            1.000001  // 4J - have removed this scale as I don't
                                      // think we should need it, and have now
                                      // optimised the vertex// shader
                                      // so it doesn't do anything other than
                    ...
                    if (currentLayer == 0 && tile->isEntityTile()) {
                        ...
                        if (TileEntityRenderDispatcher::instance->hasRenderer(
                                et)) {
		...
		if (currentLayer == 0)
		{
                    levelRenderer->clearGlobalChunkFlag(
                        ...
                        LevelRenderer::CHUNK_FLAG #endifUT);
		}


		if#ifdef __PSVITA__// AP - make sure we don't attempt to render chunks without cutout geometry
			if( t->getCutOutFound() )
			{
                    levelRenderer->setGlobalChunkFlag(
                        this->x, this->y, this->z, level,
                        LevelRenderer::CHUNK_FLAG_ #endifT);
			}

			t->end();
			bounds.addBound// 4J MGH - added
			glPopMatrix();
			glEndList();
			t->useCompactVer// 4J added);	 
			t->offset(0, 0, 0);
            } else {
                rendered = false;
            }
        ...
        if ((currentLayer == 0) && (!renderNextLayer)) {
            ...
            bre  // 4J MGH - added this to take the bound from the value calc'd
                 // in the tesselator
                if (bb) {
	...
	int key = levelRenderer->getGlobalIndexForChunk(this->x, this->y,
            ...
            if (renderableTileEntities.size()) {
                ...
                if( it != globalRenderableTileEntities->// We've got some renderable tile entities that we want associated with this chunk, and an existing list of things that used to be.// We need to flag any that we don't need any more to be removed, keep those that we do, and add any new ones// First pass - flag everything already existing to be removed
			for( AUTO_VAR(it2, it->second.begin()); it2 != it->second.end(); it2++ )
			{
                                (*it2)->setRenderRemoveStage(TileEntity::e_RenderRemoveStageFlaggedAtCh// Now go through the current list. If these are already in the list, then unflag the remove flag. If they aren't, then add
			for( int i = 0; i < renderableTileEntities.size(); i++ )
			{
                        ...
                        // Easy case - nothing already existing for this chunk.
                        // Add them all in.
                        for (int i = 0; i < renderableTileEntities.size();
                             i++) {
                            (*globalRenderableTileEntities)[key].push_back(
                                renderableTileEntities[i]);
                            // Another easy case - we don't want any renderable
                            // tile entities associated with this chunk. Flag
                            // all to be removed.
                            AUTO_VAR(it,
                                     globalRenderableTileEntities->find(key));
                            if (it != globalRenderableTileEntities->end()) {
                                for (AUTO_VAR(it2, it->second.begin());
...
#elsedN  // Find the removed ones:// 4J - original code for this section:/*
                        Set<TileEntity> newTileEntities =
    ...
    std::unordered_set<std::shared_ptr<TileEntity> >
        ...
        for( std::unordered_set<std::shared_ptr<TileEntity> >::iterator it = oldTileEntities.begin(); it != endIt; it++ // 4J - newTileEntities is now renderableTileEntities with any old ones from oldTileEntitesRemoved (so just new things added)

	EnterCriticalSection(globalRenderableTileEntities_cs);
	endIt = newTileEntities.end();
	for( std::unordered_set<std::shared_ptr<TileEntity> >::iterator it = newTileEntities.begin(); it != endIt; it++ )
	{
                            g  // 4J - All these new things added to
                               // globalRenderableTileEntities

                                AUTO_VAR(endItRTE,
                                         renderableTileEntities.end());
        for( std::vector<std::shared_ptr<TileEntity> >::iterator it = renderableTileEntities.begin(); // 4J - oldTileEntities is now the removed itemse(*it);
	}
	...
	{
                            if (oldTileEntities.find(*it) !=
                                oldTileEntities.end()) {
                                it = globalRenderableTileEntities->erase(it);
                            } else {
#endif }  // 4J - These removed items are now also removed from
          // globalRenderableTileEntities
...
#ifdef __PS3__HUNK_FLAG_COMPILED);
	PIXEndNamedEvent();
	return;
                            }
...
ChunkRebuildData g_rebuildDataIn __attribute__((__aligned__(16)));
        ...
        C4JSpursJob_ChunkUpdate chunkJob(&g_rebuildD// only need to create the tiles on the first layerurrentLayer == 0)		
	{
                                    p.submitJob(&tileJob);
                                    p.submitSync();
	}
	...
	assert(g_rebuildD//	if (!dirty) return;dDataOut.m_x0);
                            ...
                            int z0 = z;
                            int x1 = x + SIZE;
                            int  //	unordered_set<shared_ptr<TileEntity> >
                                 // oldTileEntities(renderableTileEntities.begin(),renderableTileEntities.end());
	...
	for (int currentLayer = 0; currentLayer < 2; currentLayer++)
	{
                            ...
                            {
                        glNewList(lists + cur// 4J addedGL_COMPILE);
			MemSect(0);
			glP// 4J added;
			glDepthMask(true);	
			t->useCompa// 4J - have removed this scale as I don't think we should need it, and have now optimised the vertex// shader so it doesn't do anything other than translate with this matrix anyway	#if 0

			glTranslatef(-zs / 2.0f, -ys / 2.0f, -zs / 2.	#endif	glScalef(ss, ss, ss);
			...
			t->offset((float)(-this->x), (float)(-this->y), (float)(-this->z));
                            ...
                            intArray_SPU tesselatorArray(
                                (unsigned int*)
                                    g_rebuildDataIn.m_tesselator.m_PPUArray);
                            g_rebuildDataIn.m_tesselator._array =
                                &tesselatorArray;
                            g_rebuildDataIn.m_currentLayer = currentLayer;
                            g_tileCompressDataIn.setForChunk(&region, x0, y0,
                                                             z0);
                            RunSPURebuild();
                            g_rebuildDataOut.storeInTesselator();
                            pOutData = &g_rebuildDataOut;

                if(// 4J - changed loop order here to leave y as the innermost loop for better cache performance
		for (int z = z0; z < z1; z++)
		{
                                ...
                                if (tileId > 0) {
                                    if (currentLayer == 0 &&
                                        ...
                                        if (TileEntityRenderDispatcher::instance
                                                ->hasRenderer(et)) {
                                            renderableTileEntities.push_back(
                                                et);
                                        }
                                    }
                                    ...
                                    if (flags &
                                        ChunkRebuildData::
                                            e_flag_SPURenderCodeMissing) {
                                        Tile* tile = Tile::tiles[tileId];
                                        int rend  //							renderNextLayer
                                                  //= true;;
	...
	{
                            bb->set(
                                bounds.boundingBox[0], bounds.boundingBox[1],
                                bounds.boundingBox[2], bounds.boundingBox[3],
                                bounds.boundingBox[4], bounds.boundingBox[5]);
	}


	if(pOutData// 4J - have rewritten the way that tile entities are stored globally to make it work more easily with split screen. Chunks are now// stored globally in the levelrenderer, in a hashmap with a special key made up from the dimension and chunk position (using same index// as is used for global flags)#if 1
	...
	if( renderableTileEntities.size() )
	{
                AUTO_VAR(it, globalRenderab// We've got some renderable tile entities that we want associated with this chunk, and an existing list of things that used to be.// We need to flag any that we don't need any more to be removed, keep those that we do, and add any new ones// First pass - flag everything already existing to be removed
			...
			for( int i = 0; i < renderableTileEntities.size(); i++ )
			{
                                    AUTO_VAR(
                                        ...
                                        std::find(it->second.begin(),
                                                  ...
                                                  renderableTileEntities[i]));
                                    if (it2 == it->second.end()) {
                                        (*globalRenderableTileEntities)[key]
                                            .push_back(
                                                renderableTileEntities[i]);
                                    } else {
                                        (*it2)  // Easy case - nothing already
		...
		if( it != globalRenderableTileEntities->end() )
		{
                                                    for (AUTO_VAR(it2,
                                                         ...
                                                         it->second.end();
                                                         it2++) {
                                (*it2)->setRenderRemoveStage(TileEntity::e_RenderRemove#elseFl// Find the removed ones:}
	// 4J - original code for this section:le/*
	...
	AUTO_VAR(endIt, oldTileEntities.end());
	for( std::unordered_set<std::shared_ptr<Ti// 4J - newTileEntities is now renderableTileEntities with any old ones from oldTileEntitesRemoved (so just new things added)

	EnterCriticalSection(globalRenderableTileEntities_cs);
	endIt = newTileEntities.end();
	for( std::unordered_set<std::shared_ptr<TileEntit// 4J - All these new things added to globalRenderableTileEntities{
...
int Chunk::getList(int layer)
{
                                                    ...
                                                    bool Chunk::isEmpty() {
                                                        if (!levelRenderer->getGlobalChunkFlag(
                                                                x, y, z, level,
                                                                LevelRenderer::
                                                                    CHUNK_FLAG_COMPILED))  // 4J - not used, but if this starts being used again then we'll need to investigate how best to handle it.H);
                                                    }
